...
helpviewer_keywords: 
  ...
  - "sqlmaint 实用工具"
  - "维护数据库 [SQL Server]"
  - "备份 [SQL Server], sqlmaint 实用工具"
  - "命令提示符实用工具 [SQL Server], sqlmaint"
  - "维护计划 [SQL Server], 命令提示符"
  - "备份 [SQL Server], sqlmaint 实用工具"
 ...
 *backup_path* 指定备份的目录。 如果还指定了 **-UseDefDir**，则不需要 *backup_path*。如果同时指定了二者，则 **-UseDefDir** 会覆盖后者。 备份的存放地址可以是目录或磁带设备地址（例如，\\\\.\TAPE0）。 数据库备份的文件名按如下格式自动生成：  
...
### C. 将计划中的所有数据库备份到默认 x:\Program Files\Microsoft SQL Server\MSSQL13.MSSQLSERVER\MSSQL\Backup 目录下各自的子目录中。 同时，删除所有超过两个星期的备份  
  
```  
sqlmaint -S MyServer -PlanName MyUserDBPlan -BkUpDB -BkUpMedia DISK -UseDefDir -CrBkSubDir -DelBkUps 2weeks  
```  
  
### D. 将数据库备份到默认 x:\Program Files\Microsoft SQL Server\MSSQL13.MSSQLSERVER\MSSQL\Backup 目录中。  
