 ...
 **-d** _**database\_name**_ <a name="d"></a>   
 指定要连接到的数据库。 默认情况下，bcp.exe 连接到用户的默认数据库。 如果指定了 * *-d database_name 和三部分名称（database_name，作为第一个参数传递到），则会发生错误，因为不能同时指定数据库名称。 如果 database_name 以连字符 (-) 或正斜杠 (/) 开头，则不要在 -d 与数据库名称之间添加空格   。  
