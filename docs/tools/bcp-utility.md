---
title: bcp 实用工具
description: 大容量复制程序 (bcp) 实用工具在 SQL Server 实例与用户指定格式的数据文件之间批量复制数据。
ms.prod: sql
 ...
 **-x**<a name="x"></a>  
 结合使用该格式和 -f format_file 选项，可生成基于 XML 的格式化文件，而不是默认的非 XML 格式化文件。 在导入或导出数据时， **-x** 不起作用。 如果不与格式和 -f format_file 一起使用，则将生成错误。  
