...
ms.topic: "article"
helpviewer_keywords: 
  - "命令提示符实用工具 [SQL Server], sqlservr"
  - "命令提示符 [SQL Server], 暂停/继续 SQL Server 实例"
  - "启动 SQL Server 实例"
  - "命令提示符 [SQL Server], 继续 SQL Server 实例"
  - "sqlservr 实用工具"
  - "暂停 SQL Server 实例"
  - "停止 SQL Server 实例"
  - "继续 SQL Server"
  - "命令提示符 [SQL Server], 停止 SQL Server 实例"
  - "命令提示符 [SQL Server], 启动 SQL Server 实例"
  - "继续 SQL Server 实例"
...
> [!NOTE]  
>  使用此选项时，将无法通过使用 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 服务管理器或 **net stop** 命令停止 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]。如果注销计算机，则 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 将停止。）  
 ...
 **-n**  
 用于启动 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 的命名实例。 如果不设置 **-s** 参数，则尝试启动默认实例。 必须先在命令提示符处切换到实例的相应 BINN 目录，然后才能启动 **sqlservr.exe**。 例如，如果 Instance1 为其二进制文件使用了 \mssql$Instance1，则用户必须位于 \mssql$Instance1\binn 目录中才能启动 **sqlservr.exe -s instance1**。 如果用 **-n** 选项启动 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 实例，则最好也使用 **-e** 选项，否者将不会记录 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 事件。  
  
 **-T** *trace#*  
 指示 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 实例启动时，指定的跟踪标志 (*trace#*) 应同时生效。 跟踪标记用于以非标准行为启动服务器。 有关详细信息，请参阅[跟踪标志 (Transact-SQL)](../Topic/Trace%20Flags%20\(Transact-SQL\).md)。  
  
> [!IMPORTANT]  
>  指定跟踪标志时，请使用 **-T** 来传递跟踪标志号。 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 接受小写的 t (**-t**)；但是 **-t** 通常用于设置 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 支持工程师所需的其他内部跟踪标志。  
 ...
 **-g** *memory_to_reserve*  
 ...
 除非你在 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 错误日志中看到下列任何警告，否则请使用 **-g** 参数的默认值：  
