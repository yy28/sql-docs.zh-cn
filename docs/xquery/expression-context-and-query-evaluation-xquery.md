...
-   **静态上下文**– 这是在查询编译阶段。 根据可用信息，有时会在对查询进行静态分析过程中产生错误。  
...
-   为下列内容初始化前缀和命名空间绑定：  
    ...
    -   使用 WITH XMLNAMESPACES 定义的所有命名空间。 有关详细信息，请参阅[将命名空间添加到查询使用 WITH XMLNAMESPACES](../relational-databases/xml/add-namespaces-to-queries-with-with-xmlnamespaces.md))。  
  
    -   查询 Prolog 中定义的所有命名空间。 请注意，Prolog 中的命名空间声明可覆盖 WITH XMLNAMESPACES 中的命名空间声明。 例如，在下面的查询中，WITH XMLNAMESPACES 声明将其绑定到命名空间的前缀 (pd) (`http://someURI`)。 但是，在 WHERE 子句中，查询 Prolog 覆盖该绑定。  
...
-   如果查询类型化**xml**列或变量，与列或变量关联的 XML 架构集合的组件可导入到的静态上下文。 有关详细信息，请参阅 [类型化的 XML 与非类型化的 XML 的比较](../relational-databases/xml/compare-typed-xml-to-untyped-xml.md)。  
  
-   对于导入架构中的每个原子类型，也可在静态上下文中使用转换函数。 下面的示例说明了这一点。 在此示例中，指定的查询针对类型化**xml**变量。 与此变量关联的 XML 架构集合定义了原子类型 myType。 对应于此类型，强制转换函数， **myType()**，可在静态分析过程。 查询表达式 (`ns:myType(0)`) 返回 myType 类型的值。  
    ...
    ```  
     ...
     在下面的示例中的强制转换函数**int**的表达式中指定内置 XML 类型。  
...
3.  对查询执行静态类型化。 这可确保查询类型安全。 例如，以下查询返回一个静态的错误，因为**+** 运算符要求数值基元的类型自变量：  
    ...
    ```  
     ...
     有关详细信息，请参阅[的 XQuery 和静态类型](../xquery/xquery-and-static-typing.md)。  
...
-   对于 XML 构造，仅支持 strip 构造模式。 这是默认设置。 因此，构造的元素节点的类型为**xdt： 非类型化**类型和特性属于**xdt:untypedAtomic**类型。  
...
-   初始化上下文项、上下文位置和上下文大小等表达式的主要项，如下所示。 请注意，所有这些值可以通过覆盖[nodes （） 方法](../t-sql/xml/nodes-method-xml-data-type.md)。  
  
    -   **Xml**数据类型设置的上下文项，正在处理，到文档节点的节点。  
...
-   **当前日期和时间**上下文函数**fn:current-日期**， **fn:current-时间**，和**fn:current-dateTime**，不是支持。  
