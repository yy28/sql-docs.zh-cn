...
-   为下列内容初始化前缀和命名空间绑定：  
    ...
    -   查询 Prolog 中定义的所有命名空间。 请注意，Prolog 中的命名空间声明可覆盖 WITH XMLNAMESPACES 中的命名空间声明。 例如，在以下查询中，WITH XMLNAMESPACES 声明将其绑定到命名空间的前缀 (pd) (`http://someURI`)。 但是，在 WHERE 子句中，查询 Prolog 覆盖该绑定。  
