...
-   您可能希望知道表达式返回的是 XML 元素还是特定名称和类型的属性节点。  
 ...
 您可以在序列类型匹配时使用 `instance of` 布尔运算符。 有关`instance of`表达式的详细信息，请参阅[SequenceType expression &#40;XQuery&#41;](../xquery/sequencetype-expressions-xquery.md)。  
...
```  
 ...
 现在，如果类型化的 XML 实例指定 <`root`> 元素的值， `instance of empty()`则返回 False。  
...
### <a name="example-querying-against-an-xml-type-column"></a>示例：对 xml 类型列进行查询  
 在下面的示例中，针对[!INCLUDE[ssSampleDBobject](../includes/sssampledbobject-md.md)]数据库中**Xml**类型的指令列指定了查询。 因为它具有关联的架构，因此是类型化 XML 列。 XML 架构定义整数类型的 `LocationID` 属性。 因此，在序列表达式中， `instance of xs:integer?`返回 True。  
...
```  
 ...
 以下三个查询中的所有 XQuery 表达式都将返回 <`root`> 元素的元素节点子级。 因此，序列类型表达式 `instance of node()` 将返回 True，另外两个表达式 `instance of text()` 和 `instance of document-node()` 将返回 False。  
...
```  
 ...
 下面的查询使用`instance of element (firstName)`表达式来确定 <`customer`> 的第一个子元素是否是名称 <`firstName`> 的元素。 在这种情况下，查询将返回 True。  
...
```  
 ...
 如果从实例中删除`firstName` <> 元素，则查询将返回 False。  
...
### <a name="example-b"></a>示例 B  
 ...
 在下面的示例中，正在查询`Customer`的 XML 实例中的两个 <> 元素是两种不同的类型`CustomerType` ： `SpecialCustomerType`和。 假设您想知道表达式返回的 <`Customer`> 元素的类型。 以下 XML 架构集合定义 `CustomerType` 和 `SpecialCustomerType` 类型。  
...
### <a name="example-c"></a>示例 C  
 此示例使用属性测试。 下列 XML 架构定义了带有 CustomerID 属性和 Age 属性的 CustomerType 复杂类型。 Age 属性是可选的。 对于特定的 XML 实例，您可能需要确定 <`customer`> 元素中是否存在 Age 属性。  
