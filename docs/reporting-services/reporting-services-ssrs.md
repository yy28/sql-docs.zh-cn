...
helpviewer_keywords: 
  - "报表 [Reporting Services]"
  - "SSRS"
  - "报表 [Reporting Services], 关于报表"
  - "Reporting Services"
...
# Reporting Services (SSRS)
  使用 SQL Server Reporting Services (SSRS) 提供的一系列现成工具和服务本地创建、部署和管理移动和分页报表。 
...
* **新式 Web 门户**，可在任何新式浏览器中查看。 在新的门户中，可以组织和显示移动和分页报表以及新的 KPI。 此外，可以在门户上存储 Excel 工作簿和 Power BI Desktop 报表。 
...
* [Reporting Services 中的新增功能](../reporting-services/sql-server-reporting-services-ssrs-中的新增功能.md)
...
现在仍存在核心 BI 工作负荷，因此我们对其进行了改进。 现在，可通过更新的新功能，使用 SQL Server Data Tools (SSDT) 中的[报表生成器](../reporting-services/report-builder/report-builder-in-sql-server-2016.md)或[报表设计器](../reporting-services/tools/reporting-services-in-sql-server-data-tools-ssdt.md)创建新式报表。 
...
对于本机模式 Reporting Services 的最终用户，前门是可在任何新式浏览器中查看的新式 Web 门户。 可在新门户中访问所有这些移动和分页报表以及新的 KPI。 可以标记收藏夹并管理内容（如果拥有该角色）。 
...
Web 门户仍具有传统文件夹层次结构，因此用户可组织内容。 根据类型组织内容：移动报表、KPI、分页报表以及 Power BI Desktop 报表、Excel 工作簿、共享数据集和用作报表构建块的共享数据源。 可在此处放心存储和管理这些内容。

在新的 Web 门户中，仍可计划报表处理、按需访问报表并订阅已发布的报表。

了解有关 [Reporting Services Web 门户](../reporting-services/web-portal-ssrs-native-mode.md)的详细信息。
...
## <a name="includessrsnoversiontokenssrsnoversionmdmd-programming-features"></a>[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] 编程功能  
...
## <a name="browse-content-by-area"></a>按区域浏览内容  
* [后向兼容性](../reporting-services/向后兼容性-reporting-services.md) 
...
* [SQL Server 移动报表发布服务器](../reporting-services/mobile-reports/create-mobile-reports-with-sql-server-mobile-report-publisher.md)  
