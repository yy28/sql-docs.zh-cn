...
-   列出订阅：（报表上的 [ReportOperation Enumeration](../Topic/ReportOperation%20Enumeration.md)，且用户是订阅所有者）或 ReadAnySubscription。  
...
-   列出订阅：ManageAlerts 或（报表上的 [CreateAlerts](https://technet.microsoft.com/library/microsoft.sharepoint.spbasepermissions.aspx)，且用户是订阅所有者、订阅为定时订阅）。  
