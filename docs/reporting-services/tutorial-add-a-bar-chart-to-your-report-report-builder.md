...
> [!NOTE]  
> 在本教程中，将向导的多个步骤合并为一个过程。 有关如何浏览到报表服务器的分步说明，创建数据集，并选择数据源，请参阅本系列第一个教程：[教程： 创建基本表报表 &#40;报表生成器 &#41;](../reporting-services/tutorial-creating-a-basic-table-report-report-builder.md).  
...
1.  上**选择图表类型**页上，柱形图是默认图表类型。  
  
2.  单击**栏**，然后单击**下一步**。  
  
    上**排列图表字段**页上，有四个字段**可用字段**窗格： FirstName、 LastName、 SalesYear2015 和 SalesYear2014。  
...
如果在运行报表时没有显示名字，则您可以手动刷新数据。 仍然处于预览模式，在**运行**选项卡中**导航**组中，单击**刷新**。  
...
4.  单击 **“排序”**。 **更改排序选项**页显示的排序表达式列表。 默认情况下，此列表具有一个与原始类别组表达式相同的排序表达式。  
  
5.  在**作为排序依据**，单击**[SalesYear2015]**。  
  
6.  在**顺序**列表中，选择**A 到 Z** ，以便从大到小 2015年销售额按顺序显示名称。
...
4.  有关**图例位置**，选择一个不同的位置。 例如，将图例位置设置为底部中间。  
  
    如果将图例置于图表的顶部或底部，则图例的布局将会从垂直改为水平。 你可以选择从一个不同的布局**布局**下拉列表。  
...
2.  选择的单词**图表标题**在图表的顶部，然后键入： **Sales for 2014 和 2015年**。  
  
3.  在属性窗格中，选择，标题设置**颜色**到**黑色**和**FontSize**到**12pt**。 
...
3.  上**主页**选项卡 >**数**组 >**货币**。 水平轴标签将更改为货币。  
  
3.  （可选）删除小数位数。 附近**货币**菜单上，单击**减少小数位数**按钮两次。  
...
7.  在**轴标题**框中，键入**以千为单位销售**，然后按 enter 键。  
...
3.  在**类别组**区域中，右键单击该 [LastName] 字段，并依次**类别组属性**。  
  
4.  单击 **“筛选器”**。 **更改筛选器**页可以显示的筛选器表达式的列表。 默认情况下，此列表为空。  
  
5.  单击 **“添加”**。 此时将显示一个新的空白筛选器。  
  
6.  在**表达式**，类型**[Sum(SalesYear2015)]**。 这将创建基础表达式`=Sum(Fields!SalesYear2015.Value)`，您可以看到是否则单击该**fx**按钮。  
  
7.  验证数据类型是否为**文本**。  
  
8.  在**运算符**，选择**Top N**从下拉列表。  
...
如果在运行报表时没有对结果进行筛选，则可以手动刷新数据。 上**运行**选项卡中**导航**组中，单击**刷新**。  
...
4.  选择**2015年的前五个卖方**，然后在**字体**节**主页**选项卡上，将字体大小设置为**10**。  
