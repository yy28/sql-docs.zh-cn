...
1.  在**解决方案资源管理器**，右键单击**Default.aspx**，然后单击**视图设计器**。  
  
2.  如果 default.aspx 上已经有 ReportViewer 控件，请跳到 **第 4 步**。 否则为从**AJAX Extensions**组中**工具箱**窗口中，拖动**ScriptManager**到设计图面上的控件。  
  
3.  从**报表**组中，将**ReportViewer**控件添加到设计图面下**ScriptManager**控件。  
...
6.  在“生成”菜单上，单击“生成网站”。  
    ...
    编译报表并将任何错误，例如中的报表表达式的语法错误出现在**错误列表**区域。 单击**错误列表**以显示在 Visual Studio 窗口底部**错误列表**区域。  
