...
了解有关的中央部分[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Reporting Services 安装。 它包含一个处理引擎以及用于添加功能的扩展。

> [!NOTE]
> 与 SharePoint 的 reporting Services 集成 SQL Server 2016 之后将不再可用。

在两种部署模式; 之一中运行的 Reporting Services 报表服务器本机模式或 SharePoint 模式。 请参阅功能比较的 [SharePoint 模式和本机模式的功能比较](#bkmk_featuresupport) 部分。  
  
 **安装：** Reporting Services 安装的信息，请参阅[安装 Reporting Services](../install-windows/install-reporting-services.md)。

## <a name="overview-of-report-server-modes"></a>报表服务器模式概述

 处理引擎（处理器）是报表服务器的核心。 处理器确保报告系统的完整性，但无法修改或扩展。 扩展插件也是处理器，但执行的是非常具体的功能。 Reporting Services 包括每种类型的支持的扩展的一个或多个默认扩展。 您可以向报表服务器中添加自定义扩展插件。 这样做使您可以扩展报表服务器以支持当前不支持的功能；自定义功能的示例包括对单一登录技术的支持、默认呈现扩展插件不能处理的应用程序格式的报表输出，以及到打印机或应用程序的报表传递。  
 ...
 Reporting Services 报表服务器为报表服务器实例支持两种部署模式：  
  
-   **本机模式**： 包括带有 SharePoint web 部件，报表服务器在其中运行为应用程序服务器提供所有处理和管理功能，以独占方式通过 Reporting Services 组件的本机模式。 使用 Reporting Services configuration manager 和 SQL Server Management Studio 配置本机模式报表服务器。  
...
|**自定义安全扩展插件**|是|Reporting Services 自定义安全扩展插件无法部署或报表服务器上使用。 报表服务器包括一个特殊用途的安全扩展插件，只要将报表服务器配置为以 SharePoint 集成模式运行，便会使用此插件。 该安全扩展插件是一个内部组件，并且是执行集成操作所必需的组件。|  
...
|**.RDL 报表**|是|是<br /><br /> .RDL 报表可以在运行 Reporting Services 报表服务器在本机模式或 SharePoint 模式。|  
|**.RDLX 报表**|“否”|是<br /><br /> Power View。RDLX 报表只能对运行 Reporting Services 报表服务器在 SharePoint 模式下。|  
...
## <a name="native-mode"></a>本机模式
 ...
 下图显示 Reporting Services 本机模式部署的三层体系结构。 它显示了数据层中的报表服务器数据库和数据源、中间层中的报表服务器组件，以及表示层中的客户端应用程序和内置或自定义工具。 还显示了服务器组件之间的请求流和数据流，以及哪些组件发送和检索数据存储区中的内容。  
  
 ![Reporting Services 体系结构](../../reporting-services/report-server-sharepoint/media/reporting-serv-arch.gif "Reporting Services 体系结构")  
...
### <a name="native-mode-with-sharepoint-web-parts"></a>带有 SharePoint Web 部件的本机模式
 ...
 Reporting Services 提供了两个 web 部件，你可以安装和注册的实例上[!INCLUDE[winSPServ](../../includes/winspserv-md.md)]2.0 或更高版本，或[!INCLUDE[spPortalServ](../../includes/spportalserv-md.md)]2003年或更高版本。 从 SharePoint 站点，可以使用 web 部件查找和查看在本机模式下运行的报表服务器上的存储和处理的报表。 Reporting services 的早期版本中引入了这些 web 部件。  
  
## <a name="sharepoint-mode"></a>SharePoint 模式

 在 SharePoint 模式下，报表服务器必须在 SharePoint 服务器场中运行。 运行 Reporting Services SharePoint 共享服务和一个或多个 Reporting Services 服务应用程序的 SharePoint 应用程序服务器表示报表服务器处理、 呈现和管理功能。 SharePoint 站点提供对报表服务器内容和操作的前端访问。  
  
 SharePoint 模式需要：  
  
-   [!INCLUDE[SPF2010](../../includes/spf2010-md.md)] 或 [!INCLUDE[SPS2010](../../includes/sps2010-md.md)]。  
  
-   正确的 Reporting Services 外接程序用于 SharePoint 2010 产品版本。  
  
-   具有安装了 Reporting Services 共享的服务和至少一个 Reporting Services 服务应用程序的 SharePoint 应用程序服务器。  
  
 下图显示在 SharePoint 模式 Reporting Services 环境：  
  
 ![SSRS SharePoint 功能的体系结构](../../reporting-services/report-server-sharepoint/media/rs-sharepoint-architecture.gif "SSRS SharePoint 功能的体系结构")  
  
||Description|  
|-|-----------------|  
|**(1)**|Web 服务器或 Web 前端 (WFE)。 必须在每个你想要使用 web 应用程序功能，例如，查看报表或任务，如管理数据源或订阅的 Reporting Services 管理页的 web 服务器上安装的 Reporting Services 外接程序。|  
|**(2)**|外接程序安装客户端与应用程序服务器，通过 Reporting Services 服务代理通信的 URL 和 SOAP 终结的点。|  
|**(3)**|运行 Reporting Services 共享的服务的应用程序服务器。 横向扩展的报表处理是作为的一部分，SharePoint 场并通过将 Reporting Services 服务添加到其他应用程序服务器管理的。|  
|**(4)**|你可以在不同的配置，包括权限、 电子邮件、 代理和订阅创建多个 Reporting Services 服务应用程序。|  
|**(5)**|报表、数据源和 SharePoint 内容数据库中存储的其他项。|  
|**(6)**|Reporting Services 服务应用程序创建报表服务器、 temp 和数据警报功能的三个的数据库。 应用于所有 SSRS 服务应用程序的配置设置存储于 **RSReportserver.config** 文件中。|  
...
|在 SharePoint 模式下安装 Reporting Services。|[安装用于 SharePoint 2010 的 Reporting Services SharePoint 模式](http://msdn.microsoft.com/47efa72e-1735-4387-8485-f8994fb08c8c)|  
