---
title: 模型方法 - 报表服务器 Web 服务 | Microsoft Docs
description: 了解可用于在报表服务器 Web 服务中管理模型的这些方法。
ms.date: 03/06/2017
...
ms.locfileid: "79198234"
---
# <a name="model-methods---report-server-web-service"></a>模型方法 - 报表服务器 Web 服务
  可以使用这些方法来管理模型。  
  
|方法|操作|  
|------------|------------|  
|<xref:ReportService2010.ReportingService2010.GenerateModel%2A>|在共享数据源上生成默认模型。|  
|<xref:ReportService2010.ReportingService2010.GetModelItemPermissions%2A>|检索与模型项关联的用户权限。|  
...
|<xref:ReportService2010.ReportingService2010.ListModelPerspectives%2A>|列出用户可用的模型和透视。|  
|<xref:ReportService2010.ReportingService2010.RegenerateModel%2A>|基于对数据源架构的更改更新现有模型。|  
|<xref:ReportService2010.ReportingService2010.RemoveAllModelItemPolicies%2A>|删除与指定模型中的模型项关联的所有策略。|  
|<xref:ReportService2010.ReportingService2010.SetModelDrillthroughReports%2A>|将一组钻取报表与某个模型关联。|  
