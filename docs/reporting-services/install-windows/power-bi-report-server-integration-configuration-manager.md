...
4. 注册包括用户从报表服务器登录时使用的重定向 URL。  应用 ID 和 URL 将保存到报表服务器数据库中。 重定向 URL 将在对 Azure 的身份验证调用期间使用，以便该调用可以返回到报表服务器。 例如，当用户登录或将项固定到仪表板。

5. 应用程序 ID 和 URL 将显示在 Configuration Manager 中。
