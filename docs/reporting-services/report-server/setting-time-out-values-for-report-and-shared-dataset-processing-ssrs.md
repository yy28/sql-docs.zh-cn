...
## <a name="setting-a-query-time-out-for-a-shared-dataset"></a>设置共享数据集的查询超时  
...
## <a name="setting-a-report-execution-time-out"></a>设置报表执行超时  
 可以通过设置报表执行超时值来限制报表服务器用于处理报表的时间量。 可以在 Web 门户中指定报表执行超时值。 可以为“站点设置”页中的所有报表设置默认值，然后覆盖特定报表的“执行”属性页中的值。 默认情况下，该值设置为 1800 秒。 有关详细信息，请参阅 [设置报表处理属性](../../reporting-services/report-server/set-report-processing-properties.md)。  
  
## <a name="how-report-execution-time-out-values-are-evaluated"></a>如何计算报表执行超时值  
