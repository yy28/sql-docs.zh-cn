...
4.  请确认连接类型为“Microsoft SQL Server”，然后在“连接字符串”框中键入 **Data Source = \<servername>**  
    ...
    **\<servername>**（例如，Report001）指定安装了 SQL Server Database Engine 实例的计算机。 因为此报表的数据不是从 SQL Server 数据库中提取的，所以不需要包括数据库的名称。 指定服务器上的默认数据库仅用于对查询进行分析。  
...
2. 在表体（标题和页脚区域之间）内单击，然后拖动生成列表框。 将列表框设为 7 英寸高，6.25 英寸宽。 若要获取精确大小，请在“位置”下的“属性”窗格中，键入“宽度”和“高度”属性的值。
  
    > [!NOTE]  
    > 此报表使用纸张大小 Letter (8.5 X11) 以及 1 英寸的边距。 高于 9 英寸或宽于 6.5 英寸的列表框可能会生成空白页。  
  
2.  在列表框内单击，右键单击列表顶部的工具栏，然后单击“Tablix 属性”。  
...
2.  在“插入”选项卡上，单击“文本框”。 在列表左上角的之前添加的矩形内部单击，通过拖动将文本框设为约 3.45 英寸宽，5 英寸高。  
...
29. 仍然选中“`[Sum(Sales)]`”，在“主文件夹”选项卡上，依次单击“数字”组和“货币”。  
...
12. 在“主文件夹”选项卡上，单击“字体”，对标题进行以下设置：
