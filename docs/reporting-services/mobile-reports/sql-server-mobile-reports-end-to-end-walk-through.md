---
title: "SQL Server 移动报表︰ 的端到端演练 | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "03/30/2017"
...
---
# SQL Server 移动报表︰ 的端到端演练
在 [!INCLUDE[ssRSnoversion_md](../../includes/ssrsnoversion-md.md)] Web 门户上使用 [!INCLUDE[PRODUCT_NAME](../../includes/ss-mobilereptpub-long.md)] 创建适用于任何屏幕大小的移动报表，并在 Power BI 移动应用中查看它们。
...
* 要创建数据源和 KPI、发布数据集和移动报表，需要[!INCLUDE[ssRSCurrent_md](../../includes/ssrscurrent-md.md)][以本机模式访问报表服务器](https://msdn.microsoft.com/library/ms143711.aspx)。  
* 要[创建共享数据集](#shared-dataset)，需[安装报表生成器](http://www.microsoft.com/download/details.aspx?id=50016)。  
...
## <a name="a-nameshared-datasetcreate-a-shared-dataseta"></a><a name="shared-dataset">创建共享数据集</a>  
  
使用现有的 [!INCLUDE[PRODUCT_NAME](../../includes/ssrsnoversion.md)] 客户端工具（例如 [!INCLUDE[ssBIDevStudioFull_md](../../includes/ssbidevstudiofull-md.md)] 中的报表设计器）创建共享数据集。  本演练使用 [!INCLUDE[PRODUCT_NAME](../../includes/ssrbnoversion.md)]。 [安装报表生成器](http://www.microsoft.com/download/details.aspx?id=50016)，或从你的 Web 门户启动它。 需要创建三个数据集，一个用于 KPI 值，一个用于 KPI 趋势，包含更多字段的那一个用于 Reporting Services 移动报表。   
...
## <a name="a-namecreate-kpicreate-a-kpia"></a><a name="create-KPI">创建 KPI</a>  
...
## <a name="a-namecreate-mobile-reportcreate-a-reporting-services-mobile-reporta"></a><a name="create-mobile-report">创建 Reporting Services 移动报表</a>  
