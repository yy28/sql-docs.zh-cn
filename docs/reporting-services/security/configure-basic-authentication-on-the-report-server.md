...
1.  在文本编辑器中打开 RSReportServer.config。  
     ...
     该文件位于 \<驱动器>:\Program Files\Microsoft SQL Server\MSRS13.MSSQLSERVER\Reporting Services\ReportServer。  
...
|元素|必需|有效值|  
|-------------|--------------|------------------|  
|LogonMethod|是<br /><br /> 如果不指定值，将使用 3。|**2** = 网络登录，针对要对纯文本密码进行身份验证的高性能服务器。<br /><br /> **3** = 明文登录，在此情况下，登录凭据保留在随各 HTTP 请求一起发送的身份验证包中，这样，该服务器在连接到网络中的其他服务器时可以模拟该用户。 （默认值）<br /><br /> 注意：[!INCLUDE[ssRSCurrent](../../includes/ssrscurrent-md.md)] **不**支持值 0（针对交互登录）和 1（针对批处理登录）。|  
