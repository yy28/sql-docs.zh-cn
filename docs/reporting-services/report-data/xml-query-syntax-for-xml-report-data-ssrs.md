...
ms.topic: "article"
helpviewer_keywords: 
  - "命名空间 [Reporting Services]"
  - "数据处理扩展插件 [Reporting Services], 数据源"
  - "xmldp [Reporting Services]"
  - "XML [Reporting Services], 数据检索"
...
# 用于 XML 报表数据的 XML 查询语法 (SSRS)
  在 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]中，可以为 XML 数据源创建数据集。 定义数据源后，可以为数据集创建查询。 根据数据源所指向的 XML 数据类型，可以通过包括 XML **Query** 或元素路径来创建数据集查询。 XML **查询** 以 **\<查询>** 标记开头，并且包括因数据源而异的命名空间和 XML 元素。 元素路径与命名空间无关，它使用与 XPath 类似的语法指定要使用的来自基础 XML 数据的节点和节点属性。 有关元素路径的详细信息，请参阅[用于 XML 报表数据的元素路径语法 (SSRS)](../../reporting-services/report-data/element-path-syntax-for-xml-report-data-ssrs.md)。  
...
> [!NOTE]  
>  第一个 Web 服务示例列出了使用 <xref:ReportService2006.ReportingService2006.ListChildren%2A> 方法的报表服务器的内容。 若要运行此查询，必须创建新的数据源并将连接字符串设置为 http://localhost/reportserver/reportservice2006.asmx。 <xref:ReportService2006.ReportingService2006.ListChildren%2A> 方法采用两个参数：**项**和**递归**。 将**项**的默认值设置为 **/**，将**递归**的默认值设置为 **1**。  
