...
ms.topic: "article"
helpviewer_keywords: 
  - "无凭据选项 [Reporting Services]"
  - "模拟 [Reporting Services]"
  - "用户模拟 [Reporting Services]"
  - "Windows 身份验证 [Reporting Services]"
  - "数据源 [Reporting Services], 安全性"
  - "连接字符串 [Reporting Services]"
  - "无人参与的报表处理 [Reporting Services]"
  - "报表 [Reporting Services], 安全性"
  - "远程数据源 [Reporting Services]"
  - "凭据 [Reporting Services]"
  - "身份验证 [Reporting Services]"
  - "外部数据源 [Reporting Services]"
  - "提示的凭据 [Reporting Services]"
  - "多个连接"
  - "存储的凭据 [Reporting Services]"
  - "安全性 [Reporting Services], 数据源"
  - "Windows 集成安全性 [Reporting Services]"
...
>  凭据也可以用于对访问报表服务器的用户进行身份验证。 有关向报表服务器验证用户身份的信息将在其他主题中介绍。  
 ...
 与外部数据源的连接是在创建报表时定义的。 发布报表后可以单独管理该连接。 可以指定静态连接字符串或表达式，这允许用户从动态列表中选择数据源。 有关如何指定数据源类型和连接字符串的详细信息，请参阅[数据连接、数据源以及连接字符串（报表生成器和 SSRS）](../../reporting-services/report-data/data-connections-data-sources-and-connection-strings-report-builder-and-ssrs.md)。  
...
-   在 [Reporting Services 登录对话框（报表生成器）](../../reporting-services/report-builder/reporting-services-login-dialog-box-report-builder.md)中输入的报表服务器凭据。  
...
-   在嵌入数据源的[“数据源属性”对话框，“凭据”（报表生成器）](../Topic/Data%20Source%20Properties%20Dialog%20Box,%20Credentials%20\(Report%20Builder\).md)页输入的数据源凭据。  
  
     报表服务器使用这些凭据与外部数据源建立数据连接。 对于某些类型的数据源，可以将凭据安全地存储在报表服务器上。 利用这些凭据，其他用户无需提供凭据即可运行报表进行基础数据连接。  
  
-   运行数据集查询，刷新数据集字段或预览报表时，在[输入数据源凭据对话框（报表生成器）](../../reporting-services/report-data/enter-data-source-credentials-dialog-box-report-builder.md)中输入的数据源凭据。  
 ...
 有关详细信息，请参阅[“数据源属性”对话框 ->“常规”（报表生成器）](../Topic/Data%20Source%20Properties%20Dialog%20Box,%20General%20\(Report%20Builder\).md)和[在报表生成器中预览报表](../../reporting-services/report-builder/previewing-reports-in-report-builder.md)。  
...
-   报表是一个使用了父报表的凭据的子报表。  
 ...
 上述情况下，报表服务器使用一个您必须事先定义的无人参与的执行帐户连接到远程数据源。 由于报表服务器不能使用它的服务凭据连接到远程服务器，因此必须指定一个可由报表服务器用于建立连接的帐户。 有关创建此帐户的详细信息，请参阅[配置无人参与的执行帐户 (SSRS Configuration Manager)](../../reporting-services/install-windows/configure-the-unattended-execution-account-ssrs-configuration-manager.md)。  
