...
-   **存储在数据库中的 SQL Server 空间数据。** 可以使用一个查询，该查询从 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 关系数据库中指定 **SQLGeometry** 或 **SQLGeography** 数据类型。 有关详细信息，请参阅 [SQL Server 联机丛书](http://go.microsoft.com/fwlink/?linkid=98335)中的[空间数据类型概述](../../relational-databases/spatial/spatial-data-types-overview.md)。  
