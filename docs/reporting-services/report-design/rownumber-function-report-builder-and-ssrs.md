...
## 注释  
 正如**运行值** 返回聚合函数的运行值一样， [RowNumber](../../reporting-services/report-design/runningvalue-function-report-builder-and-ssrs.md) 也返回指定作用域内行计数的运行值。 指定作用域时，需要指定何时将行计数重新设置为 1。  
