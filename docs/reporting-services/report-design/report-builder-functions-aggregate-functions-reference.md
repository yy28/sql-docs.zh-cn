...
|正文|是<br /><br /> 注释 2|是|仅限当前作用域或包含作用域中的项<br /><br /> 注释 3|是|是|是|是|  
|报表参数|是|仅限列表中前面的参数<br /><br /> 注释 4|是|是|是|是|是|  
|字段|是|是|是|是|是|是|是|  
|查询参数|是|是|是|是|是|是|是|  
|组表达式|是|是|是|是|是|是|是|  
|排序表达式|是|是|是|是|是|是<br /><br /> 注释 5|是|  
|筛选表达式|是|是|是|是|是|是<br /><br /> 注释 6|是|  
|代码|是|是<br /><br /> 注释 7|是|是|是|是|是|  
|报表语言|是|是|是|是|是|是|是|  
|变量|是|是|是|是|是|当前作用域或包含作用域|是|  
|聚合|是|是|仅在页眉/页脚中|仅在报表项聚合中|是|是|是|  
|Lookup 函数|是|是|是|是|是|是|是|  
...
|-------------|------------------|---------------|--------------------|--------------|-------------------------------------|---------------------------|----------------------|------------------------|  
|运行值|是|是|是|是|是|是|是|是|  
|第一个<br /><br /> 上一次|是|是|是|是|是|是|是|是|  
|Previous|是|是|是|是|是|是|是|是|  
|Sum 和其他预排序函数|是|是|是|是|是|是|是|是|  
|ReportItem 聚合|是|是|是|是|是|是|是|是|  
|Lookup 函数|是|是<br /><br /> 注释 1|是<br /><br /> 注释 1|是<br /><br /> 注释 1|是<br /><br /> 注释 1|是<br /><br /> 注释 1|是|是|  
|Aggregate 函数|是|是|是|是|是|是|是|是|  
