...
3.  在“表达式”窗格中，键入以下表达式：  
     ...
     “填充”属性需要 nnyy 格式的字符串，其中 nn 是一个数字，而 yy 是度量单位。 该示例表达式将生成一个字符串，该字符串使用 **Level** 函数根据递归级别增加填充的大小。 例如，级别为 1 的行会产生 (2 + (1\*10))=12pt 的填充，而级别为 3 的行会产生 (2 + (3\*10))=32pt 的填充。 有关 **Level** 函数的详细信息，请参阅 [Level](../../reporting-services/report-design/level-function-report-builder-and-ssrs.md)。  
