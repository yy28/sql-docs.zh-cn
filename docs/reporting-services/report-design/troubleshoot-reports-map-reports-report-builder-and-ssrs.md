...
## 使用来自 ESRI 形状文件的空间数据的层始终具有嵌入的数据。  
...
## 我在报表中对地图设置了居中和视图。 当我重新打开报表时，为何地图视图并不相同？  
 如果当您打开报表时，读取空间数据所需的用户凭据对于此报表不可用，将使用占位符空间数据。 根据您为地图视区设置的居中和缩放选项，地图视图可能在不同的层上居中。  
  
 若要重载空间数据并使用保存在报表中的地图视图中心，请右键单击地图视区，然后单击“重载”。 在为空间数据源输入凭据之后，该层将加载空间数据，并将还原地图视图。  
...
## 当我将标签或工具提示设置为表达式时，获得一个有关数据集作用域的表达式错误。  
...
## 另请参阅  
 ...
 [报表生成器故障排除](http://msdn.microsoft.com/zh-cn/3806fc48-56f8-44d1-a3c1-df8c33cce0a3)  
