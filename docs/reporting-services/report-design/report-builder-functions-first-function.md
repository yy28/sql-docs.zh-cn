---
title: First 函数（报表生成器）| Microsoft Docs
description: 使用 First 函数返回报表生成器中指定表达式的给定作用域中的第一个值。
ms.date: 03/07/2017
 ...
 **First** 函数只能用在当前作用域（默认）相关的组筛选表达式中。  
 ...
 还可在页眉中使用 **First** 函数，返回 **ReportItems** 集合中属于某页面的第一个值，以便在页面中生成显示首项和尾项的字典样式标题。  
