---
title: "格式设置仪表 （报表生成器和 SSRS） 上的指针 |Microsoft 文档"
ms.custom: 
ms.date: 03/01/2017
...
# <a name="formatting-pointers-on-a-gauge-report-builder-and-ssrs"></a>设置仪表上指针的格式（报表生成器和 SSRS）
 ...
 可以通过指针上右键单击并选择对指针设置外观属性**径向指针属性**或**线性指针属性**。 每个仪表指针都包含一组相同的属性。 此外，每个仪表类型还包含相应的特有外观属性：  
...
##  <a name="HowPointer"></a> 指针与数据的连接方式  
 ...
 指针上右键单击时添加数据后，你将收到**清除指针**和**删除指针**选项。 **“清除指针值”** 选项将删除附加到仪表的字段，但仍将在仪表上显示指针。 **“删除指针”** 选项将从仪表中删除字段，并且从视图中删除指针。 如果向该仪表重新添加字段，则将重新显示该默认指针。 将指针的“隐藏”  属性设置为 **True**时，指针不会在设计图面上隐藏，而是在运行时隐藏。  
  
##  <a name="DisplayingMultiple"></a> 在仪表上显示多个指针  
 可以为仪表添加多个指针以便指向基于相同刻度的多个值。 这对同时显示较低值和较高值很有用。 若要指定多个指针相同刻度的仪表上，右键单击仪表内的任意位置，然后单击**添加指针**快捷菜单上。 或者，可以通过右键单击仪表中的任意位置并单击添加小数位数**添加缩放**。 然后，可以添加新指针，该指针将与最新刻度自动关联。  
  
 指针重叠时，指针的绘制顺序由它们添加到仪表中的顺序确定。 更改数据窗格中的字段顺序无法对指针的绘制顺序进行重新排序。 若要更改的多个指针的绘制顺序，请打开属性窗格中，单击**指针 （...）**. 然后，更改指针在指针集合中的顺序。  
