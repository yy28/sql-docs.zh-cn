---
title: "地图向导和地图层向导 （报表生成器和 SSRS） |Microsoft 文档"
ms.custom: 
ms.date: 03/14/2017
...
###  <a name="Shapefile"></a>什么是 ESRI 形状文件？  
 ESRI 形状文件是一个文件集，其中的数据符合 Environmental Systems Research Institute, Inc. (ESRI) 形状文件空间数据格式。 一组文件通常包括*\<文件名 >*.shp 文件，其中包含空间数据和支持文件， *\<文件名 >*.dbf。  
