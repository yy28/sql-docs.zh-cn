...
|字段|默认值|描述|  
|---------|-----------------|---------------|  
|预生成事件命令行|InclusionThresholdSetting|指定要在项目生成之前执行的命令行。 单击“编辑预生成事件”可修改该命令行。|  
|生成后事件命令行|InclusionThresholdSetting|指定要在项目生成之后执行的命令行。 单击“编辑生成后事件”可修改该命令行。|  
...
|字段|默认值|描述|  
|---------|-----------------|---------------|  
|启动操作|InclusionThresholdSetting|指定在调试您的项目时要运行的脚本或外部程序。|  
...
使用此页，可以定义与跨数据库引用相关联的服务器变量和数据库变量。 此外，你可以指定这些变量的值。 有关详细信息，请参阅 [在数据库项目中使用引用](http://msdn.microsoft.com/en-us/library/bb386242.aspx)。  
