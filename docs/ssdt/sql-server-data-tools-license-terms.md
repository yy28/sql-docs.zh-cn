...
&nbsp;&nbsp;**b.  其他 Microsoft 程序。** 该软件包含其他 Microsoft SQL Server 技术、Microsoft Visual Studio 2015 Shell（独立）Redistributable Package、Microsoft Visual Studio 2015 Shell（集成）Redistributable Package、Microsoft Visual Studio Tools for Applications 2015、.NET Framework、Visual C++ Redistributable for Visual Studio 2015 和 Microsoft Report Viewer 2016 Runtime 以及在此处获得许可的软件。 这些组件需遵守单独的协议及其自己的产品支持政策，如该组件的安装目录中或随软件在“Licenses”文件夹中提供的许可条款中所述。  
...
* 任何道路交通数据或鸟瞰图（或关联元数据）。  
使用 Bing 地图时还需遵守 https://go.microsoft.com/?linkid=9710837 处提供的 Bing 地图最终用户使用条款和 https://go.microsoft.com/fwlink/?LinkID=248686 处提供的 Bing 地图隐私声明。  
...
**5.第三方声明。** 该软件可能包含第三方组件，这些组件可能具有单独的法律声明或者需遵守其他协议，软件附带的 ThirdPartyNotices 文件中可能对该情况进行了说明。  如果这类组件需遵守其他协议，则以下关于损害的免责声明、限制和排除也会适用。  
