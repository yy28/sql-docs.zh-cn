...
|Configuration_PopulateLargeSaleTable|应用的企业版本更改，然后为作为其他历史记录 2012年日历年填充的数据量很大。|
...
|ReseedSequenceBeyondTableValue|用于在使用相同的序列任何表中重新定位到值之外的下一步序列值。 (如`DBCC CHECKIDENT`为标识列等效序列但跨多个表可能)。|
