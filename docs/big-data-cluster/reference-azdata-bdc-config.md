...
[azdata bdc config show](#azdata-bdc-config-show) | 显示 BDC 的当前配置或指定的本地文件的配置，即 custom/cluster.json。
...
#### `--source -s`
配置文件源：['aks-dev-test', 'kubeadm-dev-test', 'minikube-dev-test']
#### `--force -f`
强制覆盖目标文件。
#### `--accept-eula -a`
...
### <a name="optional-parameters"></a>可选参数
#### `--config-profile -c`
默认配置文件：['aks-dev-test', 'kubeadm-dev-test', 'minikube-dev-test']
#### `--type -t`
要查看的配置类型。
`cluster`
#### `--accept-eula -a`
...
显示 BDC 的当前配置或指定的本地文件的配置，即 custom/cluster.json。 如果只想获取一个部分，该命令还可以采用 json 路径。  还可以指定要输出到的目标文件。  如果未指定目标文件，会直接在终端输出。
```bash
...
```
在本地配置文件中，在 json 密钥路径的末尾获取值，带有条件语句
```bash
...
### <a name="optional-parameters"></a>可选参数
#### `--config-file -c`
大数据群集配置文件路径（如果不想要当前登录到的群集的配置，即 custom/cluster.json）
#### `--target -t`
...
#### `--config-file -c`
要设置的配置的大数据群集配置文件路径，即 custom/cluster.json
#### `--json-values -j`
值的 json 路径的键值对列表：key1.subkey1=value1,key2.subkey2=value2。 你可以提供内联 json 值，如 key='{"kind":"cluster","name":"test-cluster"}'，或 提供文件路径，如 key=./values.json。 Add 不支持条件语句。  有关路径的示例，请参阅 http://jsonpatch.com/ 。  如果要访问某个数组，须通过指示索引来实现，如 key. 0 = value
### <a name="global-arguments"></a>全局参数
#### `--debug`
提高日志记录详细程度以显示所有调试日志。
#### `--help -h`
...
要设置的配置的大数据群集配置文件路径，即 custom/cluster.json
#### `--json-values -j`
值的 json 路径的键值对列表：key1.subkey1=value1,key2.subkey2=value2。 你可以提供内联 json 值，如 key='{"kind":"cluster","name":"test-cluster"}'，或 提供文件路径，如 key=./values.json。 Replace 通过 jsonpath 库支持条件语句。  若要使用条件语句，路径开头应使用 $。 这样可执行诸如 -j $.key1.key2[?(@.key3=='someValue'].key4=value 的条件语句。 可以参阅下面的示例。 如需其他帮助，请参阅： https://jsonpath.com/
### <a name="global-arguments"></a>全局参数
#### `--debug`
提高日志记录详细程度以显示所有调试日志。
#### `--help -h`
