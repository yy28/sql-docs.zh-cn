...
[笔记本](notebooks-guidance.md)最初是作为开放源代码项目实现的，现已在 [Azure Data Studio](https://docs.microsoft.com/sql/azure-data-studio/download) 中实现。 可以在文本单元格中使用 markdown 作为文本，并使用其中一个可用核心在代码单元格中编写代码。
...
从可用模板中选择目标配置模板。 可用的配置文件是根据前一对话框中选择的部署目标类型来筛选的。
   ...
   ![部署配置模板 步骤 1](media/deploy-notebooks/deployment-configuration-template.png)
...
注意：用于控制器和 SQL Server 的是同一个帐户。
   ...
   ![群集设置](media/deploy-notebooks/cluster-settings.png)

2. 输入适当的 Docker 设置

   ![Docker 设置](media/deploy-notebooks/docker-settings.png)

3. 如果 AD 身份验证可用，请输入 AD 设置

   ![Active Directory 设置](media/deploy-notebooks/active-directory-settings.png)
...
Spark 实例可以随 HDFS 一起提供。 它包含在存储池中，或者单独位于 Spark 池中。
   ...
   ![服务设置](media/deploy-notebooks/service-settings.png)
...
已预填充了默认的终结点。 但可以对它们进行适当的更改。
   ...
   ![终结点设置](media/deploy-notebooks/endpoint-settings.png)

#### <a name="storage-settings"></a>存储设置

存储设置包括数据和日志的存储类和声明大小。 这些设置可应用于存储、数据和 SQL Server 主池。

   ![存储设置](media/deploy-notebooks/storage-settings.png)
