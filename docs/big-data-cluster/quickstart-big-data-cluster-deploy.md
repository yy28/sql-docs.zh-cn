...
description: 了解如何使用部署脚本在 Azure Kubernetes Service [!INCLUDE[big-data-clusters-2019](../includes/ssbigdataclusters-ver15.md)] (AKS) 上部署 (预览)。
author: MikeRayMSFT
ms.author: mikeray
...
在本教程中, 将使用示例 python 部署脚本部署[!INCLUDE[big-data-clusters-2019](../includes/ssbigdataclusters-ver15.md)]到 Azure Kubernetes 服务 (AKS)。

> [!TIP]
> AKS 只是为大数据群集托管 Kubernetes 的一种选择。 若要了解其他部署选项以及如何自定义部署选项, 请参阅[如何在 Kubernetes 上[!INCLUDE[big-data-clusters-2019](../includes/ssbigdataclusters-ss-nover.md)]部署](deployment-guidance.md)。
...
SQL Server 2019 大数据群集早期采用者计划的参与者需要以下参数：“Docker 用户名”和“Docker 密码”。 从 CTP 3.2 开始，不再需要它们。
   ...
   > [!NOTE]
   > `sa` 帐户是安装过程中在 SQL Server 主实例上创建的系统管理员。 创建部署后，`MSSQL_SA_PASSWORD` 可以通过在主实例容器中运行 `echo $MSSQL_SA_PASSWORD` 来发现环境变量。 出于安全考虑，请在部署后更改主实例上的 `sa` 密码。 有关详细信息，请参阅[更改 SA 密码](../linux/quickstart-install-connect-docker.md#sapassword)。
...
> [!IMPORTANT]
> 由于下载大数据群集组件的容器映像需要一定的时间，整个部署可能需要很长时间。 但是，这个过程应不会长达几个小时。 如果你的部署遇到问题, 请参阅[监视和故障排除[!INCLUDE[big-data-clusters-2019](../includes/ssbigdataclusters-ss-nover.md)] ](cluster-troubleshooting-commands.md)。
...
> [!TIP]
> 有关如何监视和排查部署问题的详细信息, 请参阅[监视和故障[!INCLUDE[big-data-clusters-2019](../includes/ssbigdataclusters-ss-nover.md)]排除](cluster-troubleshooting-commands.md)。
...
如果是在 Azure [!INCLUDE[big-data-clusters-2019](../includes/ssbigdataclusters-ss-nover.md)]中进行测试, 则应在完成时删除 AKS 群集以避免意外费用。 如果你打算继续使用它，请不要删除该群集。
...
部署脚本配置了 Azure Kubernetes Service，并部署了 SQL Server 2019 大数据群集。 你还可以选择通过手动安装来自定义未来的部署。 若要详细了解如何部署大数据群集以及如何自定义部署, 请参阅[如何在 Kubernetes 上部署[!INCLUDE[big-data-clusters-2019](../includes/ssbigdataclusters-ss-nover.md)] ](deployment-guidance.md)。
