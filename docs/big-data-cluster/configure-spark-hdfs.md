...
在大数据群集中，我们定义了 3 个服务：`hdfs`、`spark` 和 `sql`。 恰巧每个服务都映射到相同的命名配置类别。 所有网关配置都将跳到类别 `gateway`。 

例如，服务 `hdfs` 中的所有配置都属于类别 `hdfs`。 请注意，所有 Hadoop（核心站点）、HDFS 和 Zookeeper 配置都属于类别 `hdfs`；所有 Livy/Spark/Yarn/Hive 元存储配置属于类别“Spark”。 
