...
以下文章提供了**azdata**工具中的**bdc hdfs**命令参考。 有关其他**azdata**命令的详细信息, 请参阅[azdata reference](reference-azdata.md)。

## <a name="commands"></a>命令
|     |     |
| --- | --- |
[azdata bdc hdfs shell](#azdata-bdc-hdfs-shell) | HDFS shell 是用于 HDFS 文件系统的简单交互式命令行界面。
[azdata bdc hdfs ls](#azdata-bdc-hdfs-ls) | 列出给定文件或目录的状态。
[azdata bdc hdfs 存在](#azdata-bdc-hdfs-exists) | 确定文件或目录是否存在。  如果存在, 则返回 True; 否则返回 False。
[azdata bdc hdfs mkdir](#azdata-bdc-hdfs-mkdir) | 在指定的路径创建目录。
[azdata bdc hdfs mv](#azdata-bdc-hdfs-mv) | 将指定的文件或路径移动到指定的位置。
[azdata bdc hdfs create](#azdata-bdc-hdfs-create) | 在指定位置创建文本文件。  可以通过 data 参数添加简单的文本内容。
[azdata bdc hdfs cat](#azdata-bdc-hdfs-cat) | 读取文件的内容。  偏移量和长度 (字节) 是可选参数。
...
[azdata bdc hdfs 装载](reference-azdata-bdc-hdfs-mount.md) | 管理 HDFS 中远程存储的安装。
## <a name="azdata-bdc-hdfs-shell"></a>azdata bdc hdfs shell
HDFS shell 是用于 HDFS 文件系统的简单交互式命令行界面。
```bash
...
在指定位置创建文本文件。  可以通过 data 参数添加简单的文本内容。
```bash
