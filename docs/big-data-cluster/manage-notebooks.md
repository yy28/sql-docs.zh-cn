...
[笔记本](notebooks-guidance.md)最初作为开放源代码项目实现，现已并入 [Azure Data Studio](https://docs.microsoft.com/sql/azure-data-studio/download)。 可以在文本单元格中使用 markdown 作为文本，并使用其中一个可用核心在代码单元格中编写代码。
...
1. 笔记本已经过优化，因此只需选择“运行单元”  。 此操作将单独运行笔记本中的每个单元，直到笔记本运行完成。
1. 如果发现错误，Jupyter Book 将建议一个可运行以修复错误的笔记本。 按照建议的步骤操作，然后再次运行该笔记本。

## <a name="next-steps"></a>后续步骤
有关 Azure Data Studio 中笔记本的详细信息，请参阅[如何使用 SQL Server 中的笔记本](notebooks-guidance.md)。
