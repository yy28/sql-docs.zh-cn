---
title: mssqlctl 群集存储池装入参考
titleSuffix: SQL Server big data clusters
description: Mssqlctl 群集存储池装入命令的参考文章。
author: rothja
ms.author: jroth
manager: craigg
ms.date: 05/22/2019
...
ms.locfileid: "65995025"
---
# <a name="mssqlctl-cluster-storage-pool-mount"></a>mssqlctl 群集存储池装入

[!INCLUDE[tsql-appliesto-ssver15-xxxx-xxxx-xxx](../includes/tsql-appliesto-ssver15-xxxx-xxxx-xxx.md)]

以下文章提供了参考**群集存储池装入**中的命令**mssqlctl**工具。 有关其他详细信息**mssqlctl**命令，请参阅[mssqlctl 引用](reference-mssqlctl.md)。

## <a name="commands"></a>命令
|     |     |
| --- | --- |
[mssqlctl 群集存储池装入创建](#mssqlctl-cluster-storage-pool-mount-create) | 在 HDFS 中创建的远程存储的装载。
[mssqlctl 群集存储池装入删除](#mssqlctl-cluster-storage-pool-mount-delete) | 删除远程存储在 HDFS 中的装载。
[mssqlctl 群集存储池装入状态](#mssqlctl-cluster-storage-pool-mount-status) | Mount(s) 的状态。
## <a name="mssqlctl-cluster-storage-pool-mount-create"></a>mssqlctl 群集存储池装入创建
在 HDFS 中创建的远程存储的装载。
```bash
...
## <a name="mssqlctl-cluster-storage-pool-mount-delete"></a>mssqlctl 群集存储池装入删除
删除远程存储在 HDFS 中的装载。
```bash
...
#### `--verbose`
提高日志记录详细程度。 使用--debug 可获取完整的调试日志。
## <a name="mssqlctl-cluster-storage-pool-mount-status"></a>mssqlctl 群集存储池装入状态
Mount(s) 的状态。
```bash
