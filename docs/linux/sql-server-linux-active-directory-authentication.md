...
本文档介绍如何配置[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]支持 Active Directory (AD) 身份验证，也称为集成身份验证的 linux 操作系统上。 AD 身份验证使在 Windows 或 Linux 进行身份验证到已加入域的客户端[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]使用其域凭据和 Kerberos 协议。 AD 身份验证通过具有以下优点[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]身份验证：  
• 用户进行身份验证通过单一登录，而不会提示输入密码。   
• 通过创建的 AD 组的登录名，你可以管理访问和权限中的[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]使用 AD 组成员身份。  
• 每个用户具有单个标识整个组织，因此不必能够跟踪其[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]登录名对应于哪些用户。   
• AD，可在你的组织之间强制实施了集中式的密码策略。   

## <a name="prerequisites"></a>必要條件
在配置 AD 身份验证之前，您需要：
- 设置你的网络上的 AD 域控制器 (Windows)  
- 安装[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]
...
## <a name="step-1-join-includessnoversiondocsincludesssnoversion-mdmd-host-to-ad-domain"></a>步骤 1： 加入[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]主机到 AD 域
存在大量的工具以帮助您加入[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]到你的 AD 域的主机。 本演练使用 **[realmd](https://www.freedesktop.org/software/realmd/docs/guide-active-directory-join.html)**，流行的开放源包。 如果你尚未安装的 realmd 和 Kerberos 客户端包上[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]使用你的 Linux 分发的包管理器的主机：  
```bash  
# RHEL
...
运行以下命令以确认[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]主机配置为使用的 AD 域控制器作为 DNS 名称服务器：
```bash  
sudo realm discover contoso.com -v
```  
如果找不到你的域，需要配置你[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]主机机以用作 DNS 名称服务器的 AD 域控制器的 IP 地址。 若要执行此操作的特定步骤取决于你的网络设备配置、 域配置和 Linux 分发。 以下是一些示例方法。
...
一旦你已确认您的 DNS 配置正确，请通过运行以下命令以加入域。 你将需要使用在 AD 将新计算机加入到域中具有足够的权限的 AD 帐户进行身份验证。 具体而言，此命令将在 AD 中创建新的计算机帐户、 创建`/etc/krb5.keytab`承载 keytab 文件和配置中的域`/etc/sssd/sssd.conf`:
```bash                                     
...
## <a name="step-2-create-ad-user-for-includessnoversiondocsincludesssnoversion-mdmd-and-set-spn"></a>步骤 2： 创建 AD 用户[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]和设置 SPN  
...
现在，设置此帐户使用 ServicePrincipalName (SPN)`setspn.exe`工具。 SPN 必须格式化为完全按照下面的示例中指定的方式： 你可以找到的完全限定的域名称[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]通过运行的主机`hostname --all-fqdns`上[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]主机和 TCP 端口应为 1433年，除非已配置[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]若要使用不同的端口号。  
```PowerShell   
...
有关详细信息，请参阅 [为 Kerberos 连接注册服务主体名称](/sql/database-engine/configure-windows/register-a-service-principal-name-for-kerberos-connections.md)。  

## <a name="step-3-configure-includessnoversiondocsincludesssnoversion-mdmd-service-keytab"></a>步骤 3： 配置[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]服务 keytab  
首先，在上一步中创建 AD 帐户中查看此密钥版本编号 (kvno)。 通常，它将是 2，但它可能是另一个整数，如果您多次更改帐户的密码。 上[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]主机计算机，运行以下命令：
...
任何人有权访问此`keytab`文件可以模拟[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]在域中，因此请确保你限制对此类的文件访问，它仅`mssql`帐户具有读取权限：  
```bash  
sudo chown mssql:mssql /var/opt/mssql/secrets/mssql.keytab
sudo chmod 400 /var/opt/mssql/secrets/mssql.keytab
```  
接下来，配置[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]才能使用此`keytab`Kerberos 身份验证的文件：  
```bash  
...
连接到[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]并创建新的、 基于 AD 的登录名：  
```Transact-SQL  
CREATE LOGIN [CONTOSO\user] FROM WINDOWS;
```   

验证登录名现在列在[sys.server_principals](/sql/relational-databases/system-catalog-views/sys-server-principals-transact-sql.mc)系统目录视图：  
```Transact-SQL  
SELECT name FROM sys.server_principals;
```  

## <a name="step-5-connect-to-includessnoversiondocsincludesssnoversion-mdmd-using-ad-authentication"></a>步骤 5： 连接到[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]使用 AD 身份验证  
登录到客户端计算机使用域凭据。 现在你可以连接到[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]无需你的密码，重新输入通过使用 AD 身份验证。 如果你创建的 AD 组的登录名，只要 AD 用户是该组的成员可以连接的方式相同。  
...
登录到已加入域的 Windows 客户端使用域凭据。 请确保[!INCLUDE[ssmanstudiofull-md](../../docs/includes/ssmanstudiofull-md.md)]安装，然后连接到你[!INCLUDE[ssNoVersion](../../docs/includes/ssnoversion-md.md)]通过指定实例**Windows 身份验证**中**连接到服务器**对话框。  
...
• ADO.NET:[连接字符串语法](https://msdn.microsoft.com/library/ms254500.aspx)   
