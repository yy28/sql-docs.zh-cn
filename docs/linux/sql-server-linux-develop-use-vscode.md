...
1. 在命令面板中，从下拉列表中选择“扩展  ：安装扩展”  。 

1. 在“扩展”窗格中，键入“mssql”   。

1. 选择“SQL Server (mssql)”扩展，然后选择“安装”   。

   ![安装 mssql 扩展](./media/sql-server-linux-develop-use-vscode/vscode-extension.png)

1. 安装完成后，选择“重新加载”以启用扩展  。
...
1. 在下方状态栏上选择“纯文本”，或按“Ctrl”+“K” > “M”，然后从“语言”下拉列表中选择“SQL”      。 
...
1. 键入 sql 以显示 mssql 命令，或键入 sqlcon，然后从下拉列表中选择“MS SQL   ：连接”  。
...
1. 然后选择“创建”为 SQL Server 创建新的连接配置文件  。

1. 按照提示为新连接配置文件指定属性。 指定每个值后，按“Enter”继续  。
   ...
   > [!TIP]
   > 如果连接失败，请尝试通过 Visual Studio Code “输出”面板中的错误消息来诊断问题  。 要打开“输出”面板，请选择“查看” > “输出”    。 另外，请查看[连接故障排除建议](./sql-server-linux-troubleshooting-guide.md#connection)。
...
1. 在代码段中，键入 `TutorialDB` 以替换“DatabaseName”：
...
1. 按“Ctrl”+“Shift”+“E”执行 Transact - SQL 命令    。 在查询窗口中查看结果。
   ...
   ![创建数据库消息](./media/sql-server-linux-develop-use-vscode/vscode-create-database-messages.png)
...
1. 键入 sql 以显示 mssql 命令，或键入 sqluse，然后选择“MS SQL   ：使用数据库”命令  。

1. 选择新的“TutorialDB”数据库  。 

   ![使用数据库](./media/sql-server-linux-develop-use-vscode/vscode-use-database.png)

1. 在代码编辑器中，键入 sql 以显示片段，选择“sqlCreate Table”后按“Enter”    。

1. 在片段中，键入表名 `Employees`。

1. 按“Tab”键转到下一个字段，然后键入 `dbo` 作为架构名称  。
...
1. 按“Ctrl”+“Shift”+“E”执行命令    。 将在“结果”窗口中显示两个结果集  。 
...
1. 再次打开网格上下文菜单，选择“另存为 JSON”将结果保存到“.json”文件   。

1. 为 JSON 文件指定文件名。 

1. 验证 JSON 文件是否在 Visual Studio Code 中保存和打开。

   ![另存为 JSON](./media/sql-server-linux-develop-use-vscode/vscode-save-as-json.png)
