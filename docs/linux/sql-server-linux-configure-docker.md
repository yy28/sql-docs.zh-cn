...
SQL Server Linux 容器映像上的文档的所有点基于 Ubuntu 的容器。 从 SQL Server 2019 预览开始，可以使用基于 Red Hat Enterprise Linux (RHEL) 上的容器。 更改从容器存储库**mcr.microsoft.com/mssql/server:2019-CTP3.1-ubuntu**到**mcr.microsoft.com/mssql/rhel/server:2019-CTP3.1**中的所有 docker 命令。
...
若要在具有特定时区的 Linux 容器中运行 SQL Server，配置**TZ**环境变量。 若要查找正确的时区值，请运行**tzselect**命令从 Linux bash 提示符：

```bash
tzselect
```

选择时区后, **tzselect**显示类似于以下输出：
...
此信息可用于在 Linux 容器中设置相同的环境变量。 下面的示例演示如何在中的容器中运行 SQL Server`Americas/Los_Angeles`时区：
...
- 请确保满足中指定的最小内存和磁盘要求[先决条件](quickstart-install-connect-docker.md#requirements)快速入门文章部分。
