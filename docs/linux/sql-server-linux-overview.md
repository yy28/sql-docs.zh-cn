...
> [!NOTE]
>  Docker本身可运行在多个平台之上，这意味着，你可以在 Linux、 Mac 和 Windows 上运行的 Docker 映像。

## <a name="connect"></a>连接

安装完成后，连接到你 Linux 计算机上的 SQL Server 实例。 你可以连接本地或远程和各种工具和驱动程序。 快速入门教程演示如何使用[sqlcmd](sql-server-linux-setup-tools.md)命令行工具。 其他工具包括：
