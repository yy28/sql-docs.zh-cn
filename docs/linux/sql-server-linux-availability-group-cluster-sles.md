...
`cluster-recheck-interval` 表示群集检查资源参数、约束或其他群集选项中的所做更改的轮询间隔。 如果副本发生故障，则群集将尝试按 `failure-timeout` 值和 `cluster-recheck-interval` 值所绑定的间隔重启副本。 例如，如果 `failure-timeout` 设置为 60 秒且 `cluster-recheck-interval` 设置为 120 秒，则会在 60 秒至 120 秒的间隔内尝试重启。 建议将 failure-timeout 设置为 60 秒，将 cluster-recheck-interval 设置为大于 60 秒的值。 建议不要将 cluster-recheck-interval 设置为较小的值。

要将属性值更新为 `2 minutes`，请运行：
...
> [!IMPORTANT] 
> 如果已拥有由 Pacemaker 群集管理的可用性组资源，请注意，所有使用最新 Pacemaker 包 1.1.18-11.el7 的发行版在其值时为 false 时，都会为 start-failure-is-fatal 群集设置引入行为更改。 此更改会影响故障转移工作流。 如果主要副本发生中断，则群集应故障转移到其中某个可用的次要副本。 相反，用户会注意到群集会一直尝试启动发生故障的主要副本。 如果该主要副本永远不会联机（由于永久中断），则群集永远不会故障转移到其他的可用次要副本。 由于此更改，先前建议使用的用于设置 start-failure-is-fatal 的配置不再有效，并且必须将设置还原为其默认值 `true`。 此外，必须更新 AG 资源以包含 `failover-timeout` 属性。 
>
>要将属性值更新为 `true`，请运行：
>
>```bash
...
有关详细信息，请参阅[从头构建 Pacemaker 群集](https://clusterlabs.org/pacemaker/doc/en-US/Pacemaker/1.1/html/Clusters_from_Scratch/)、[隔离和 Stonith](https://clusterlabs.org/doc/crm_fencing.html) 和 [SUSE HA 文档：隔离和 STONITH](https://www.suse.com/documentation/sle_ha/book_sleha/data/cha_ha_fencing.html)。
