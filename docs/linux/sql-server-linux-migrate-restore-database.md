...
SQL Server 的备份和还原功能是从在 Windows 上的 SQL Server 的数据库迁移到在 Linux 上的 SQL Server 自 2017 年 1 RC2 的建议的方法。 在本教程中，将会演练使用备份将数据库移到 Linux 和还原技术所需的步骤。
...
* 具有安装以下版本的 Linux 计算机：
  * SQL Server 自 2017 年 1 RC2。 请参阅安装快速入门的[RHEL](quickstart-install-connect-red-hat.md)， [SLES](quickstart-install-connect-suse.md)，或[Ubuntu](quickstart-install-connect-ubuntu.md)。
  * SQL Server 自 2017 年 1 RC2[命令行工具](sql-server-linux-setup-tools.md)。
