...
可以将你的数据库和数据迁移到 SQL Server 2017 Linux 上运行。 选用的方法取决于源数据和特定方案。 以下部分提供多种迁移方案的最佳做法。
...
可将其他数据库系统上的数据库迁移到 Linux 版 SQL Server 2017。 这包括 Microsoft Access、 DB2、 MySQL、 Oracle 和 Sybase 数据库。 在此方案中，使用 SQL Server Management Assistant (SSMA) 自动执行到 Linux 上 SQL Server 的迁移。 有关详细信息，请参阅[使用 SSMA 将数据库迁移到 Linux 上的 SQL Server](sql-server-linux-migrate-ssma.md)。  
