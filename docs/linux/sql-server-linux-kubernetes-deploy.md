...
在 Kubernetes 上部署包含 SQL Server 运算符、 SQL Server 容器和负载均衡器服务。 运算符可自动协调可用性组。 此文章介绍了如何：

- 部署运算符、 SQL Server 容器和负载均衡的服务
- 连接到与服务的可用性组
- 将数据库添加到可用性组
...
1. 使用密码创建 Kubernetes 机密`sa`帐户和 SQL Server 实例的主密钥。
...
可以使用[Kubernetes 仪表板与 Azure Kubernetes 服务 (AKS)](https://docs.microsoft.com/azure/aks/kubernetes-dashboard)来监视部署。
...
- 从输出`kubectl get services`命名空间`ag1`。
 ...
 服务为每个 SQL Server 容器创建的负载平衡服务。 使用这些 IP 地址作为终结点直接连接到群集中的 SQL Server 实例。
