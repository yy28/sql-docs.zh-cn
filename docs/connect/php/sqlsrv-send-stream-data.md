...
## <a name="example"></a>示例  
下面的示例将以流的形式打开产品评论，并将其发送到服务器。 禁用执行时发送所有流数据的默认行为。 该示例假定已在本地计算机上安装了 SQL Server 和 [AdventureWorks](http://go.microsoft.com/fwlink/?LinkID=67739) 数据库。 当从命令行运行该示例时，所有输出都将写入控制台。  
