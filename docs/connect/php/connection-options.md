...
|CEKeystoreProvider<br />CEKeystoreName<br />CEKeystoreEncryptKey|字符串|指定的路径、 名称和始终加密功能自定义密钥存储提供程序的加密密钥。 为了正确配置自定义密钥存储提供程序，在连接时，必须设置所有三个值。 有关详细信息，请参阅[PHP 应用程序中启用始终加密](../../connect/php/enabling-always-encrypted-php-application.md)。|未设置任何值。|
...
|ColumnEncryption|**启用**或**已禁用**|指定是否已启用始终加密功能。 有关详细信息，请参阅[PHP 应用程序中启用始终加密](../../connect/php/enabling-always-encrypted-php-application.md)。|Disabled|  
