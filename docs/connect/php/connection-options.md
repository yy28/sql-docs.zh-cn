...
|DecimalPlaces<br /><br />（在 PDO_SQLSRV 驱动程序中不受支持）|介于 0 和 4 （含） 之间的整数|指定小数位数设置格式时提取金额值。<br /><br />此选项仅适用于 FormatDecimals 为 true。 将忽略任何负整数或值大于 4。|默认精度和小数位数|
...
|FormatDecimals<br /><br />（在 PDO_SQLSRV 驱动程序中不受支持）|1 或 **，则返回 true**为提取十进制字符串的格式。<br /><br />0 或**false**默认十进制格式设置行为。|指定是否要添加前导为在适当的时候十进制字符串置零，并使`DecimalPlaces`money 类型设置格式的选项。 如果保留 false，则使用返回精确的精度和省略前导零的值小于 1 的默认行为。<br /><br />有关详细信息，请参阅[格式设置十进制字符串和 Money 值](../../connect/php/formatting-decimals-sqlsrv-driver.md)。|false (0)|
