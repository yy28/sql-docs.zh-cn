...
*$options* [可选]：设置<a name="properties">查询属性</a>的关联阵列。 下表列出了受支持的键和相应值：
...
|ReturnDatesAsStrings|**true** 或 **false**<br /><br />默认值是 **false**秒。|将语句配置为以字符串的形式检索日期和时间类型 (true  )。 有关详细信息，请阅读[如何：使用 SQLSRV 驱动程序检索日期和时间类型作为字符串](../../connect/php/how-to-retrieve-date-and-time-type-as-strings-using-the-sqlsrv-driver.md)。
