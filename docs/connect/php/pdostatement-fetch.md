...
$fetch_style：可选（整数）符号，指定行数据的格式。 请参阅 $fetch_style 的可能值列表的“备注”部分。 默认值为 PDO::FETCH_BOTH。此提取方法中的  $fetch_style 将替代在 PDO::query 方法中指定的 $fetch_style。  
  
$cursor_orientation：可选（整数）符号，指明要在准备语句指定 `PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL` 时检索的行。 请参阅 $cursor_orientation 的可能值列表的“备注”部分。 有关使用可滚动游标的示例，请参阅 [PDO::prepare](../../connect/php/pdo-prepare.md) 。  
  
$cursor_offset：可选（整数）符号，指定要在 $cursor_orientation 是 PDO::FETCH_ORI_ABS 或 PDO::FETCH_ORI_REL 且 PDO::ATTR_CURSOR 是 PDO::CURSOR_SCROLL 时提取的行。  
...
|PDO::FETCH_BOTH|指定按列名称和基于 0 的顺序进行索引的数组。 这是默认值。|  
...
|--------------------------|---------------|  
|PDO::FETCH_ORI_NEXT|检索下一行。 这是默认值。|  
