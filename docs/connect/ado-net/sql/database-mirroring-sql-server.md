...
下面是定目标到已配置镜像的数据库的 <xref:Microsoft.Data.SqlClient.SqlConnection> 对象的简化事件序列：  
...
如果你省略故障转移伙伴服务器的名称，且主体数据库在客户端应用程序首次连接时不可用，那么 <xref:Microsoft.Data.SqlClient.SqlException> 会抛出。  
  
当 <xref:Microsoft.Data.SqlClient.SqlConnection> 成功打开时，服务器会返回故障转移伙伴名称，此名称取代连接字符串中提供的任何值。  
