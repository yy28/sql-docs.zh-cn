...
description: 介绍对获取 SQL Server 运行时统计信息的支持。
ms.date: 08/15/2019
dev_langs:
- csharp
...
ms.locfileid: "80925482"
---
# <a name="provider-statistics-for-sql-server"></a>用于 SQL Server 的提供程序统计信息

[!INCLUDE[Driver_ADONET_Download](../../../includes/driver_adonet_download.md)]

从 .NET Framework 版本 2.0 和 .NET Core 版本 1.0 开始，Microsoft SqlClient Data Provider for SQL Server 支持运行时统计信息。 必须通过将 <xref:Microsoft.Data.SqlClient.SqlConnection.StatisticsEnabled%2A> 对象的 <xref:Microsoft.Data.SqlClient.SqlConnection> 属性设置为 `True` 来启用统计信息。 启用统计信息后，可以通过 <xref:System.Collections.IDictionary> 对象的 <xref:Microsoft.Data.SqlClient.SqlConnection.RetrieveStatistics%2A> 方法检索 <xref:Microsoft.Data.SqlClient.SqlConnection> 引用，以“即时快照”的方式查看统计信息。 可以将该列表枚举为一组名称/值对字典条目。 这些名称/值对是无序的。 随时都可以调用 <xref:Microsoft.Data.SqlClient.SqlConnection.ResetStatistics%2A> 对象的 <xref:Microsoft.Data.SqlClient.SqlConnection> 方法来重置计数器。 如果尚未启用统计信息收集，则不会生成异常。 此外，如果在未首先调用 <xref:Microsoft.Data.SqlClient.SqlConnection.RetrieveStatistics%2A> 的情况下调用 <xref:Microsoft.Data.SqlClient.SqlConnection.StatisticsEnabled%2A>，则检索的值是每个条目的初始值。 如果启用统计信息，请运行应用程序一段时间，然后禁用统计信息，检索到的值将反映收集的值，直到禁用统计信息为止。 收集的所有统计值都基于每个连接。  
  
## <a name="statistical-values-available"></a>可用的统计值  
Microsoft SQL Server 提供程序中目前提供 18 个不同的项。 可用的项数可以通过 **返回的** 接口引用的 Count 属性进行访问<xref:System.Collections.IDictionary><xref:Microsoft.Data.SqlClient.SqlConnection.RetrieveStatistics%2A>。 提供程序统计信息的所有计数器均使用公共语言运行时 <xref:System.Int64> 类型（C# 和 Visual Basic 中为 long），宽度为 64 位  。 int64 数据类型的最大值由 int64.MaxValue 字段定义，最大值为 ((2^63)-1))   。 当计数器的值达到此最大值时，它们将不再被认为是准确的。 这意味着 int64.MaxValue-1((2^63)-2) 实际上是任何统计信息的最大有效值  。  
  
> [!NOTE]
>  字典用于返回提供程序统计信息，因为返回的统计信息的数量、名称和顺序可能会在将来发生变化。 应用程序不应依赖于在字典中找到的特定值，而应检查该值是否存在，并相应地设置分支。  
  
下表显示了当前可用的统计值。 请注意，各个值的键名未在 Microsoft .NET Framework 和 .NET Core 的所有区域版本之间进行本地化。  
