...
有关 SqlClient 中的连接字符串关键字的详细信息，请参阅 <xref:Microsoft.Data.SqlClient.SqlConnection.ConnectionString%2A>。  
...
- 如果应用程序使用 `ApplicationIntent=ReadWrite`（在下面论述）且将辅助副本位置配置为只读访问。  
  
只读辅助副本不支持 <xref:Microsoft.Data.SqlClient.SqlDependency>。  
