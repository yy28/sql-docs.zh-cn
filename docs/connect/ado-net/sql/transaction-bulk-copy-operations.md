---
title: 事务和大容量复制操作
description: 描述如何在事务中执行大容量复制操作，包括如何提交或回滚事务。
ms.date: 08/15/2019
dev_langs:
- csharp
...
可以指定现有 <xref:Microsoft.Data.SqlClient.SqlTransaction> 对象作为 <xref:Microsoft.Data.SqlClient.SqlBulkCopy> 构造函数中的参数。 在这种情况下，大容量复制操作在现有事务中执行，并且不对事务状态进行任何更改（即，既不提交也不中止该事务）。 这允许应用程序将大容量复制操作包含在带有其他数据库操作的事务中。 但是，如果未指定 <xref:Microsoft.Data.SqlClient.SqlTransaction> 对象并传递空引用，而且连接具有活动的事务，则会引发异常。  
  
如果需要在发生错误时回滚整个大容量复制操作，或者如果大容量复制应作为可回滚的更大进程的一部分执行，则可以向 <xref:Microsoft.Data.SqlClient.SqlBulkCopy> 构造函数提供 <xref:Microsoft.Data.SqlClient.SqlTransaction> 对象。  
