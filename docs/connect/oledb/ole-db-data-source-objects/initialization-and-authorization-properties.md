...
|DBPROP_AUTH_ENCRYPT_PASSWORD|SQL Server 的 OLE DB 驱动程序使用标准[!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]的安全机制来隐藏密码。<br /><br /> SQL Server 的 OLE DB 驱动程序将返回 DB_S_ERRORSOCCURRED, 尝试设置属性值。 DBPROP 结构的 dwStatus 成员指示 DBPROPSTATUS_NOTSUPPORTED  。|  
...
|DBPROP_AUTH_MASK_PASSWORD|SQL Server 的 OLE DB 驱动程序使用标准[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]的安全机制来隐藏密码。<br /><br /> SQL Server 的 OLE DB 驱动程序将返回 DB_S_ERRORSOCCURRED, 尝试设置属性值。 DBPROP 结构的 dwStatus 成员指示 DBPROPSTATUS_NOTSUPPORTED  。|  
...
|DBPROP_INIT_PROTECTION_LEVEL|SQL Server 的 OLE DB 驱动程序不支持与实例的[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]连接的保护级别。<br /><br /> SQL Server 的 OLE DB 驱动程序将返回 DB_S_ERRORSOCCURRED, 尝试设置属性值。 DBPROP 结构的 dwStatus 成员指示 DBPROPSTATUS_NOTSUPPORTED  。|  
...
|属性 ID|描述|  
|-----------------|-----------------|  
|SSPROP_AUTH_ACCESS_TOKEN<a href="#table1_1"><sup>**1**</sup></a>|类型：VT_BSTR<br /><br /> R/W：读/写<br /><br /> 默认值：VT_EMPTY<br /><br /> 说明: 用于对 Azure Active Directory 进行身份验证的访问令牌。 <br/><br/>**注意:** `UID`指定此属性以及`PWD` `Trusted_Connection`、、、或`Authentication`连接字符串关键字或其对应的属性/关键字是错误的。|
|SSPROP_AUTH_MODE<a href="#table1_1"><sup>**1**</sup></a>|类型：VT_BSTR<br /><br /> R/W：读/写<br /><br /> 默认值：VT_EMPTY<br /><br /> 说明: 指定使用的 SQL 或 Active Directory 身份验证。 有效值为<br/><ul><li>`(not set)`: 由其他关键字确定的身份验证模式。</li><li>`(empty string)`: 取消设置以前设置的身份验证模式。</li><li>`ActiveDirectoryPassword:`使用登录 ID 和密码 Active Directory 身份验证。</li><li>`ActiveDirectoryIntegrated:`使用当前登录用户的 Windows 帐户凭据 Active Directory 集成身份验证。</li><br/>**注意:** **建议**使用`Integrated Security` (或`Trusted_Connection`) 身份验证关键字或其`Authentication`对应属性的应用程序将关键字 (或其对应的属性) 的值设置为`ActiveDirectoryIntegrated` , 以启用新的加密和证书验证行为。<br/><br/><li>`SqlPassword:`使用登录 ID 和密码进行身份验证。</li><br/>**注意:** **建议**使用`SQL Server`身份验证的应用程序将`Authentication`关键字 (或其对应的属性) 的值设置为`SqlPassword` , 以启用新的加密和证书验证行为。</ul>|
