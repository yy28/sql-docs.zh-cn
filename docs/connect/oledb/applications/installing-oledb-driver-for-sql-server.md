---
title: 安装用于 SQL Server 的 OLE DB 驱动程序 |Microsoft Docs
...
运行安装程序并进行您首选的选择。 SQL Server 的 OLE DB 驱动程序可以是与早期版本的 Microsoft OLE DB 访问接口并行安装。
...
通过 msoledbsql.msi，可以将 OLE DB 驱动程序分发适用于 SQL Server。 您可能需要部署的应用程序，然后安装用于 SQL Server OLE DB 驱动程序。 安装多个包（对于用户而言就像是一次安装）的一种方法就是使用链接器和引导程序技术。 有关详细信息，请参阅[为 Visual Studio 2005 创作自定义引导程序包](http://go.microsoft.com/fwlink/?LinkId=115667)和[添加自定义系统必备](http://go.microsoft.com/fwlink/?LinkId=115668)。  
