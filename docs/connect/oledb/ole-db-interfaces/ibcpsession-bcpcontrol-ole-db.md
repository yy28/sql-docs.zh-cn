---
title: IBCPSession::BCPControl（OLE DB 驱动程序）| Microsoft Docs
description: 了解 IBCPSession::BCPControl 如何设置 OLE DB Driver for SQL Server 中的大批量复制操作的选项。
ms.custom: ''
ms.date: 07/03/2018
ms.prod: sql
...
|BCP_OPTION_FMTXML|用于指定生成的格式化文件应采用 XML 格式。 默认情况下关闭此选项，此时将格式化文件作为文本文件保存。 XML 格式化文件提供更大的灵活性，但具有某些额外约束。 例如，不能同时为字段指定前缀和终止符，而在较早的格式化文件中则可以执行此操作。<br /><br /> 注意：只有当 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 工具和 OLE DB Driver for SQL Server 一起安装后，才支持 XML 格式化文件。|  
