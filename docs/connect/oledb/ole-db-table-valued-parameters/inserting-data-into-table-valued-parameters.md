...
## <a name="push-model-loads-all-table-valued-paremeter-data-in-memory"></a>推送模型（在内存中加载所有表值参数数据）  
 ...
 SQL Server 的 OLE DB 驱动程序开始执行该命令后，将从此表值参数行集对象提取的表值参数值，并将其发送到服务器中。  
...
-   IRowset::RestartPosition  
 ...
 OLE DB 驱动程序的 SQL Server 将一个或多个行一次从读取使用者行集合对象以支持表值参数中的流式处理行为。 例如，用户可能将表值参数行集数据对磁盘 （不在内存中），并可能实现的功能通过 OLE DB 驱动程序所需的 SQL Server 时，从磁盘读取数据。  
  
 使用者将通过对表值参数行集对象使用 IAccessor::CreateAccessor for SQL Server 通信到 OLE DB 驱动程序及其数据格式。 当从使用者缓冲区读取数据时，访问接口将验证所有可写入和非默认列是否至少可用于一个取值函数句柄，并使用相应的句柄来读取列数据。 若要避免多义性，则应将表值参数行集列和绑定之间的一一对应关系。 与同一列的重复绑定将导致错误。 此外，每个访问器需要具有*iOrdinal* DBBindings 的序列中的成员。 为每个行，访问器数将有任意多个 irowset:: Getdata 调用，并且调用的顺序将按顺序排列的*iOrdinal*从到更高版本较低的值的值。  
 ...
 在执行时，OLE DB 驱动程序的 SQL Server 将回调到要提取行和读取列数据的行集对象。  
