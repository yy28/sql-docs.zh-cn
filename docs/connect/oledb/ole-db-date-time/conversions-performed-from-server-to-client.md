...
## <a name="conversions"></a>转换  
 下表说明了返回到客户端的类型与绑定中的类型之间的转换。 对于输出参数，如果已调用 ICommandWithParameters::SetParameterInfo 和中指定的类型*pwszDataSourceType*不在服务器上，隐式转换的实际类型将由服务器执行的匹配并返回到客户端的类型将与通过 ICommandWithParameters::SetParameterInfo 指定的类型匹配。 当服务器的转换规则是在本文中的描述不同，则可能导致意外的转换结果。 例如，在必须提供默认日期时，[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 使用 1900-1-1，而不是 1899-12-30。  
