...
description: 处理 Transact-SQL 分布式查询时，OLE DB Driver for SQL Server 支持从链接的服务器返回架构信息。
ms.custom: ''
ms.date: 06/12/2018
ms.prod: sql
...
> [!NOTE]  
>  尽管 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 支持同义词，但 OLE DB Driver for SQL Server 不返回同义词的元数据。  
