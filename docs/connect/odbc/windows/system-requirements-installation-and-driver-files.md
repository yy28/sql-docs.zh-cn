...
| 适用于 Windows 的 ODBC Driver 11 for [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] | 可以安装在还具有一个或多个 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 版本的计算机上。 |
...
在连接字符串中指定的驱动程序名称是 `ODBC Driver 11 for SQL Server` 或 `ODBC Driver 13 for SQL Server` （对于13和13.1）或 `ODBC Driver 17 for SQL Server`。
...
从以下链接之一运行 `msodbcsql.msi` 时，会安装该驱动程序：
...
`bcp.exe` 和 `sqlcmd.exe` 安装在版本11的 `%PROGRAMFILES%\Microsoft SQL Server\Client SDK\ODBC` 的 `110\Tools` 子文件夹中，`130\Tools` 为13和13.1。
