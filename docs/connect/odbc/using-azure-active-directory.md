---
title: 使用 ODBC 驱动程序的 Azure Active Directory |SQL Server 的 Microsoft 文档
ms.custom: ''
ms.date: 03/21/2018
ms.prod: sql
...
Microsoft ODBC Driver for SQL Server 或更高版本 13.1 允许 ODBC 应用程序连接到的 SQL Azure 实例在 Azure Active Directory 中使用联合的标识，并且用户名/密码、 Azure Active Directory 访问令牌或 Windows集成的身份验证 (_Windows 驱动程序_)。 ODBC 驱动程序版本 13.1，Azure Active Directory 访问令牌身份验证是_仅限 Windows_。 ODBC 驱动程序版本 17 和 17.1 支持跨所有平台 （Windows、 Linux 和 Mac） 进行此身份验证。 新 Azure Active Directory 交互式身份验证登录 id 是适用于 Windows ODBC 驱动程序版本 17.1 中引入的。 所有这些都是通过使用新 DSN 和连接字符串关键字，以及连接属性完成的。
...
`Authentication=ActiveDirectoryInteractive` Azure Active Directory 到 SQL Azure 的交互式身份验证
...
五个选项对应于`Trusted_Connection=Yes`(现有旧版 Windows 仅 SSPI 集成的身份验证) 和`Authentication=` `ActiveDirectoryIntegrated`， `SqlPassword`， `ActiveDirectoryPassword`，和`ActiveDirectoryInteractive`分别。

### <a name="sqldriverconnect-prompt-windows-driver-only"></a>SQLDriverConnect 提示符 （仅 Windows 驱动程序）

提示对话框中时，它请求完成连接所需的信息显示 SQLDriverConnect 包含 Azure AD 身份验证的三个新选项：

![ServerLogin.png](windows/ServerLogin.png)

这些选项对应于在 DSN 安装程序 UI 上面的相同五个。
...
`server=Server;database=Database;Authentication=ActiveDirectoryIntegrated;`
7. (_Windows 驱动程序_。)AAD 交互式身份验证使用 Azure 多因素身份验证技术来设置连接。 在此模式下，通过提供的登录 ID，Windows Azure 身份验证对话框触发，并允许用户输入密码以完成连接。 用户名是在连接字符串中传递的。
...
>- 使用 Active Directory 交互式 （仅 Windows 驱动程序） 的身份验证进行连接`UID`必须指定。
...
> [!NOTE]
> ODBC 驱动程序版本 13.1 仅支持此身份验证在_Windows_。
...
~~~
    ...
    free(pAccToken);
~~~
下面是一个用于 Azure Active Directory 交互式身份验证的连接字符串示例。 请注意它不包含 PWD 字段，因为将使用 Windows Azure 身份验证屏幕输入的密码。
~~~
