...
> [!NOTE]
> ODBC 驱动程序不支持使用 Active Directory 联合身份验证服务进行 AKV 身份验证。 如果对 AKV 使用的是 Azure Active Directory 身份验证，并且 Active Directory 配置包括联合服务，则身份验证可能会失败。
