...
## <a name="cursor-conversion"></a>游标转换  
 ...
 使用[!INCLUDE[ssVersion2000](../../includes/ssversion2000-md.md)]，当更新可通过 ResultSet.TYPE_SCROLL_SENSITIVE 和 ResultSet.CONCUR_UPDATABLE 结果的数据时设置，显示消息"游标只读的"引发异常。 发生此异常的原因是 [!INCLUDE[ssVersion2000](../../includes/ssversion2000-md.md)] 已针对该结果集执行了隐式游标转换，而没有返回所请求的可更新游标。  
...
## <a name="cursor-updating"></a>游标更新  
 ...
 调用以更新后\<类型 >，受影响的列不能访问通过 get\<类型 > 直到 updateRow 或[cancelRowUpdates](../../connect/jdbc/reference/cancelrowupdates-method-sqlserverresultset.md)已调用。 这将避免以下问题：使用与服务器返回的类型不同的类型更新列，而随后调用的 getter 可能会引起给出不准确结果的客户端类型转换。 调用 get\<类型> 将引发异常并显示“在调用 updateRow() 或 cancelRowUpdates() 之前，无法访问已更新列”的消息。  
