---
title: JDBC 驱动程序入门 |Microsoft 文档
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql-non-specified
ms.prod_service: drivers
ms.service: ''
ms.component: jdbc
ms.reviewer: ''
ms.suite: sql
ms.technology:
- drivers
ms.tgt_pltfrm: ''
ms.topic: article
ms.assetid: 5f4c5d0d-c8d9-48e6-b0a5-56d308140c51
caps.latest.revision: 3
author: MightyPen
ms.author: genemi
manager: jhubbard
ms.workload: On Demand
ms.openlocfilehash: 0d34785b887c804083c3e844eb64185c6b910d89
ms.sourcegitcommit: 9351e8b7b68f599a95fb8e76930ab886db737e5f
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 04/06/2018
---
# <a name="getting-started-with-the-jdbc-driver"></a>JDBC 驱动程序入门

[!INCLUDE[Driver_JDBC_Download](../../includes/driver_jdbc_download.md)]

## <a name="getting-started"></a>入门  
* [步骤 1：配置用于 Java 开发的开发环境](../../connect/jdbc/step-1-configure-development-environment-for-java-development.md)  
* [步骤 2：创建用于 Java 开发的 SQL 数据库](../../connect/jdbc/step-2-create-a-sql-database-for-java-development.md)  
* [步骤 3：使用 Java 连接到 SQL 的概念证明](../../connect/jdbc/step-3-proof-of-concept-connecting-to-sql-using-java.md)
