...
[!INCLUDE[Driver_JDBC_Download](../../includes/driver_jdbc_download.md)]
 ...
 可通过使用 [SQLServerConnection](../../connect/jdbc/reference/sqlserverconnection-class.md) 类的 [setHoldability](../../connect/jdbc/reference/setholdability-method-sqlserverconnection.md) 方法设置结果集的保持能力。 可以使用 ResultSet.CLOSE_CURSORS_AT_COMMIT 或结果时使用的 setHoldability 方法设置保持能力，设置保持能力的 ResultSet.HOLD_CURSORS_OVER_COMMIT 常量。  
 ...
 结果集的保持能力是在仅为服务器端游标创建结果集时，与结果集相关联的 SQLServerConnection 对象的保持能力。 它不适用于客户端游标。 与客户端游标的所有结果集将始终都具有 ResultSet.HOLD_CURSORS_OVER_COMMIT 的保持能力值。  
