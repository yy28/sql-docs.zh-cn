...
|REMARKS|**String**|与列关联的注释。<br /><br /> **注意：** 对于此列，[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 始终会返回 Null。|  
...
|IS_AUTOINCREMENT|**String**|如果列是自动递增的，则为“是”。 如果列不是自动递增的，则为“否”。 如果驱动程序无法确定列是否为自动递增，则为 ""（空字符串）。 <sup>1</sup>|  
|SS_UDT_CATALOG_NAME|**String**|包含用户定义类型 (UDT) 的目录名称。 <sup>1</sup>|  
|SS_UDT_SCHEMA_NAME|**String**|包含用户定义类型 (UDT) 的架构名称。 <sup>1</sup>|  
|SS_UDT_ASSEMBLY_TYPE_NAME|**String**|采用完全限定名称的用户定义类型 (UDT)。 <sup>1</sup>|  
|SS_XML_SCHEMACOLLECTION_CATALOG_NAME|**String**|在其中定义 XML 架构集合名称的目录的名称。 如果找不到目录名称，此变量则会包含一个空字符串。 <sup>1</sup>|  
|SS_XML_SCHEMACOLLECTION_SCHEMA_NAME|**String**|在其中定义 XML 架构集合名称的架构的名称。 如果找不到架构名称，则为空字符串。 <sup>1</sup>|  
|SS_XML_SCHEMACOLLECTION_NAME|**String**|XML 架构集合的名称。 如果找不到名称，则为空字符串。 <sup>1</sup>|  
