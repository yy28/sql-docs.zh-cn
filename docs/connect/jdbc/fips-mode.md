...
Fips，很可能需要证书 (.cert) 导入到任一 PKCS 或特定于提供程序的格式。 使用以下代码片段导入 SSL 证书并将其存储在工作目录中具有相应的密钥存储格式。 _TRUST_STORE_PASSWORD_ Java 密钥存储的是你的密码。 
...
下面的示例导入带有 BouncyCastle 提供程序 PKCS12 格式的 Azure SSL 证书。 在名为的工作目录中导入证书_MyTrustStore_PKCS12_通过使用以下代码片段：
