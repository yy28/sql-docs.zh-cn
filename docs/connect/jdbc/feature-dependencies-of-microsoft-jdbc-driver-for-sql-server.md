...
- `azure-keyvault`： （可选） 的 Always Encrypted Azure 密钥保管库功能 azure 密钥保管库提供程序
- `adal4j`： 用于 Java 的 Azure Active Directory 身份验证功能和 Azure 密钥保管库功能 （可选） azure Active Directory 库
...
**例如：** 时要使用 Azure 密钥保管库功能，需要重新声明`azure-keyvault`依赖项和`adal4j`项目的 POM 文件中的依赖关系。 请参阅以下代码片段：
