...
| lockTimeout<br/><br/>ssNoversion<br/><br/>-1 | 在等待多少毫秒后数据库报告锁定超时。默认行为是无限期等待。 如果指定，该值将成为此连接上所有语句的默认值。 请注意， **Statement.setQueryTimeout()** 可以用来设置特定语句的超时值。 该值可为 0，这表示无需等待。 |
...
| sslProtocol<br/><br/>String<br/><br/>TLS | 从开始 JDBC Driver 6.4 for SQL Server，可以使用此属性以指定要在安全的连接过程中考虑的 TLS 协议。 <br/>可取值包括：TLS  、TLSv1  、TLSv1.1  和 TLSv1.2  。 <br/><br/>有关详细信息，请参阅[SSLProtocol](https://github.com/Microsoft/mssql-jdbc/wiki/SSLProtocol)。 |
