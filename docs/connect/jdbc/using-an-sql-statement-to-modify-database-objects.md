...
> [!NOTE]  
> 修改数据库中对象的 SQL 语句称为“数据定义语言 (Data Definition Language, DDL)”语句。 这些包括语句如下所述`CREATE TABLE`， `DROP TABLE`， `CREATE INDEX`，和`DROP INDEX`。 有关 [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] 支持的 DDL 语句类型的详细信息，请参阅 [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] 联机丛书。
