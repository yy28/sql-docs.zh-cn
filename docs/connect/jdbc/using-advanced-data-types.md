...
从数据库中检索非二进制大值数据类型（例如 varchar(max) 数据类型）时，一种方法是将此数据读取为字符流  。 以下实例使用了 [SQLServerStatement](../../connect/jdbc/reference/executequery-method-sqlserverstatement.md) 类的 [executeQuery](../../connect/jdbc/reference/sqlserverstatement-class.md) 方法从数据库中检索数据，并将其返回为结果集。 然后，使用 [SQLServerResultSet](../../connect/jdbc/reference/getcharacterstream-method-sqlserverresultset.md) 类的 [getCharacterStream](../../connect/jdbc/reference/sqlserverresultset-class.md) 方法从结果集读取大值数据。  
...
在大多数情况下，用于在数据库上更新或修改较大值的推荐方法是使用 [ 命令（例如 ](../../connect/jdbc/reference/sqlserverpreparedstatement-class.md)[ 和 ](../../connect/jdbc/reference/sqlservercallablestatement-class.md)）通过 [!INCLUDE[tsql](../../includes/tsql-md.md)]SQLServerPreparedStatement`UPDATE` 类和 `WRITE`SQLServerCallableStatement`SUBSTRING` 类来传递参数。  
...
有关 xml 数据类型的详细信息，请参阅  **联机丛书中的“xml 数据类型”** [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。  
...
有关 spatial 数据类型的信息，请参阅[使用 Spatial 数据类型](../../connect/jdbc/use-spatial-datatypes.md)。  
