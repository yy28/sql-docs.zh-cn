...
|Purchasing|从供应商处购买的库存项和有关供应商的详细信息。|  
...
|人名|包含所有使用该应用程序的人员的用户名、联系人信息, 以及由广域人员在客户组织中所涉及的人员。 这包括人员、客户、供应商和其他任何联系人。 对于已被授予使用系统或网站权限的人员, 这些信息包括登录详细信息。|
...
|(|省/市/自治区是国家/地区的组成部分。 此表详细介绍了相关信息, 包括描述每个国家/地区边界的空间数据。|
...
- 单个序列 (TransactionID) 用于以下表:CustomerTransactions、SupplierTransactions 和 StockItemTransactions。 这说明一组表可以有单个序列。
- 某些列具有适当的默认值。
...
|RecordColdRoomTemperatures|将传感器数据列表作为表值参数 (TVP), 并将数据`Warehouse.ColdRoomTemperatures`应用于临时表。|
...
|Configuration_ApplyFullTextIndexing|将全文索引`Application.People`应用于`Sales.Customers` `Purchasing.Suppliers`、、和`Warehouse.StockItems`。 `Website.SearchForPeople`将、`Website.SearchForSuppliers` 、、替换`Website.SearchForStockItemsByTags`为使用全文索引的替换过程。 `Website.SearchForCustomers` `Website.SearchForStockItems`|
|Configuration_ApplyPartitioning|将表分区`Sales.CustomerTransactions`应用于`Purchasing.SupplierTransactions`和, 并重新排列索引以适应。|
...
|Configuration_EnableInMemory|添加内存优化文件组 (在 Azure 中不工作时), 将`Warehouse.ColdRoomTemperatures`替换`Warehouse.VehicleTemperatures`为内存中等效项, 并迁移`Website.OrderList`数据`Website.OrderLineList`, `Website.OrderIDList`使用 `Website.SensorDataList`内存优化等效项, 删除并重新创建使用`Website.InvoiceCustomerOrders`这些`Website.InsertCustomerOrders`表类型`Website.RecordColdRoomTemperatures`的过程、和。|
