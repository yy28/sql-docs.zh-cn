...
> [!IMPORTANT]  
>  如[BottomPercent](../mdx/bottompercent-mdx.md)函数， **TopPercent**函数始终中断层次结构。  
...
## <a name="example"></a>示例  
 下面的演练将帮助了解中的负值的效果*Numeric_Expression*。 首先，将生成一些可展示该行为的上下文。  
...
```  
 ...
 请注意，查询要求百分之百 (100%)；这意味着应返回所有行。 但是，由于在负值*Numeric_Expression* ，返回只包含一行。  
