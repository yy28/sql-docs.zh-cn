...
## <a name="remarks"></a>备注  
 CREATE MEMBER 语句定义的计算成员在整个会话期间均可用，因此可在会话期间用于多个查询。 有关详细信息, 请参阅[创建会话作用域的&#40;计算&#41;成员 MDX](https://docs.microsoft.com/analysis-services/multidimensional-models/mdx/mdx-calculated-members-session-scoped-calculated-members)。  
  
 还可以定义供一个查询使用的计算成员。 若要定义只供一个查询使用的计算成员，请在 SELECT 语句中使用 WITH 子句。 有关详细信息, 请参阅[创建查询作用域的&#40;计算&#41;成员 MDX](https://docs.microsoft.com/analysis-services/multidimensional-models/mdx/mdx-calculated-members-query-scoped-calculated-members)。  
...
## <a name="scope"></a>范围  
 ...
 查询作用域  
 计算成员的可见性和生存期限制在查询中。 计算成员在单独的查询中定义。 查询作用域将覆盖会话作用域。 有关详细信息, 请参阅[创建查询作用域的&#40;计算&#41;成员 MDX](https://docs.microsoft.com/analysis-services/multidimensional-models/mdx/mdx-calculated-members-query-scoped-calculated-members)。  
  
 会话作用域  
 计算成员的可见性和生存期限制在创建计算成员时所在的会话中。 (如果在计算成员上发出了 DROP MEMBER 语句, 则生存期小于会话持续时间。)CREATE MEMBER 语句创建具有会话作用域的计算成员。  
...
## <a name="standard-properties"></a>标准属性  
 每个计算成员都有一个默认属性集。 当客户端应用程序连接到[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]时, 默认属性是受支持的, 或者可在管理员选择的情况下受支持。  
