...
1. 首次运行 [!INCLUDE[name-sos](../includes/name-sos-short.md)] 时，应该会打开“欢迎”页  。 如果未显示“欢迎”页，选择“帮助” > “欢迎”    。 选择“新建连接”以打开“连接”窗格   ：
   
   ![新建连接图标](media/quickstart-sql-server/new-connection-icon.png)

1. 本文使用 SQL 登录名，但也支持 Windows 身份验证   。 按如下所示填写字段：
...
查询编辑器仍连接到 master 数据库，但我们想要在 TutorialDB 数据库中创建一个表   。 
...
1. 将以下代码片段粘贴到查询窗口中，并单击“运行”  ：

   > [!NOTE]
   > 可以将此追加到或覆盖编辑器中之前的查询。 请注意，单击“运行”仅执行选定的查询  。 如果未选择任何内容，单击“运行”将执行编辑器中所有的查询  。
...
你现在已成功连接到 SQL Server 并运行查询，下面请尝试[代码编辑器教程](tutorial-sql-editor.md)。
