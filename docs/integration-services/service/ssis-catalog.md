...
>   
>  如果 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 资源作为群集故障转移的一部分进行故障转移，则正在运行的包不重新启动。 您可以使用检查点来重新启动包。 有关详细信息，请参阅 [通过使用检查点重新启动包](../../integration-services/packages/restart-packages-by-using-checkpoints.md)。  
...
-   无效的字符包括 ASCII/Unicode 字符 1 到 31 之间，引号 （"），小于 (\<)、 大于 (>)、 竖线 (|)、 退格符 (\b)、 null (\0) 和制表符 (\t)。  
  
-   名称不得包含前导空格或尾随空格。  
  
-   不允许将 @ 作为第一个字符，但后续字符可以使用@。  
...
-   对于环境中，使用 **权限** 页 [NIB：环境属性](http://msdn.microsoft.com/en-us/6a91a8d4-0006-4cfd-9759-3e4295ae452b)。  
...
|验证超时（秒）|VALIDATION_TIMEOUT|指定项目验证或包验证可运行的最大秒数，超过该秒数后运行将停止。 默认值为 300 秒。<br /><br /> 执行验证是一个异步操作。 项目或包越大，验证所用的时间就会越长。<br /><br /> 有关验证项目和包的信息，请参阅 [Integration Services Data Types in Expressions](../../integration-services/expressions/integration-services-data-types-in-expressions.md)。|  
...
5.  如果要将 SSISDB 数据库还原到从未创建 SSISDB 目录的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例，请执行以下操作生成 sp_ssis_startup 的 CREATE PROCEDURE 脚本。 有关详细信息，请参阅 [CREATE PROCEDURE (Transact-SQL)](../../t-sql/statements/create-procedure-transact-sql.md)。  
...
5.  运行以下语句以将 sp_ssis_startup 过程设置为自动执行。 有关详细信息，请参阅 [sp_procoption (Transact-SQL)](../../relational-databases/system-stored-procedures/sp-procoption-transact-sql.md)。  
...
7.  使用下列方法之一还原主密钥。 有关加密的详细信息，请参阅 [Encryption Hierarchy](../../relational-databases/security/encryption/encryption-hierarchy.md)。  
    ...
    -   **方法 1**  
        ...
        >   
        >  **无法对当前的主密钥进行解密。由于指定了 FORCE 选项，因此忽略错误。**  
        >   
...
-   在升级 SQL Server 实例之前，未从 AlwaysOn 可用性组中删除数据库。 这样可以防止数据库自动升级。 有关详细信息，请参阅 [Upgrading SSISDB in an availability group](#Upgrade)。  
...
3.  为每个 SQL Server 实例启用 Alwayson 可用性组。 有关详细信息，请参阅 [启用 AlwaysOn 可用性组](../../database-engine/availability-groups/windows/enable-and-disable-always-on-availability-groups-sql-server.md) 。  
...
> -   必须启用**SSIS 支持 Alwayson** *后*将 SSISDB 添加到 Alwayson 可用性组。  
...
####  <a name="Step2"></a> 步骤 2：将 SSISDB 添加到 AlwaysOn 可用性组  
 ...
 ![新的可用性组](../../integration-services/service/media/ssis-newavailabilitygroup.png "新的可用性组")  
...
####  <a name="Step3"></a> 步骤 3：为 AlwaysOn 启用 SSIS 支持  
 ...
 ![在为始终启用支持](../../integration-services/service/media/ssis-enablesupportforalwayson.png)  
  
> [!WARNING]  
>  为 AlwaysOn 启用 SSIS 支持之前，不支持 SSISDB 数据库的自动故障转移。  
  
 将表中显示新添加的辅助副本从 Always On 可用性组。 单击 **“连接…”** 按钮，然后输入身份验证凭据以连接到副本。 用户帐户必须是每个副本上的 sysadmin 组以启用对 Alwayson 的 SSIS 支持的成员。 成功连接到每个副本后，单击“确定”  按钮以启用对 AlwaysOn 的 SSIS 支持。  
 
如果**启用 Always On 支持**上下文菜单上的选项似乎被禁用后你已完成其他先决条件，尝试以下操作：
1.  通过单击刷新的上下文菜单**刷新**选项。
2.  请确保你要连接到主节点。 你必须启用 Always On 的主节点上的支持。
3.  请确保 SQL Server 版本为 13.0 或更高版本。 SSIS 支持 Alwayson 仅在 SQL Server 2016 及更高版本上。
