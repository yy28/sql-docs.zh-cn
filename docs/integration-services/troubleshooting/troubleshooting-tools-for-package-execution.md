...
[!INCLUDE[ssis-appliesto](../../includes/ssis-appliesto-ssvrpluslinux-asdb-asdw-xxx.md)]
 ...
 在设计时，[!INCLUDE[ssBIDevStudioFull](../../includes/ssbidevstudiofull-md.md)] 提供了用于暂停包执行的断点、“进度”窗口和数据查看器。 通过断点，你可以在数据通过数据流时对其进行监视。 但是，运行已部署的包时，这些功能不可用。 用于排除已部署包故障的主要技术如下：  
...
## <a name="catch-and-handle-package-errors-by-using-event-handlers"></a>使用事件处理程序捕获和处理包错误  
 使用事件处理程序响应由包和包对象引发的事件。  
  
-   **创建 OnError 事件的事件处理程序**。 在事件处理程序中，可以使用“发送邮件”任务向管理员通知失败。 使用脚本任务和自定义逻辑获取用于故障排除的系统信息或者清理临时资源和不完整的输出。 有关详细信息，请参阅 [Integration Services (SSIS) 事件处理程序](../../integration-services/integration-services-ssis-event-handlers.md)。  
...
-   **向错误输出添加易于理解的信息**。 为了更加了解错误输出，除了由错误输出提供的两个数字标识符之外，还可以添加错误消息和列名。 有关如何通过使用脚本添加这两个附加列的示例，请参阅 [使用脚本组件增强错误输出](../../integration-services/extending-packages-scripting-data-flow-script-component-examples/enhancing-an-error-output-with-the-script-component.md)。  
...
## <a name="troubleshoot-package-execution-by-using-logging"></a>使用日志记录对包执行进行故障排除  
 你可以通过日志记录跟踪正在运行的包中发生的大部分问题。 日志提供程序捕获有关指定事件的信息，供以后分析，并以其中某种受支持的格式保存该信息。  支持日志提供程序格式包括数据库表、平面文件、XML 文件。  
...
-   **增强默认日志记录输出**。 通常，每次该包运行时日志记录都会向日志记录目标中追加行。 虽然日志记录输出的每一行都使用包的名称和唯一标识符来标识包，也通过唯一的 ExecutionID 来标识包的执行，但单一列表中的大量日志记录输出会使分析变得困难。  
    ...
    3.  **考虑捕获行计数数据**。 考虑另行创建一个表，用于存储行计数信息。其中，每个包执行实例均由其 ExecutionID 标识。 使用行计数转换在数据流的关键点将行计数保存到一系列变量中。 添加执行 SQL 任务，以将一系列值插入到表的行中，供以后分析和报告。  
...
-   **在已知的无效包元素上配置 DelayValidation 属性**。 为了防止具有已知配置问题的包元素发生验证错误，请将 DelayValidation 设置为 True   。 例如，包包含一个数据流任务，该任务使用执行 SQL 任务在运行时创建表后才存在的目标表。 可以在包级或包中的各个任务和容器级启用 **DelayValidation** 属性。  
...
## <a name="troubleshoot-errors-without-a-description"></a>无说明错误的故障排除  
 如果遇到了没有附带说明的 [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 错误，则可以在 [Integration Services 错误和消息引用](../../integration-services/integration-services-error-and-message-reference.md)中找到说明。 该列表当前不包括故障排除信息。  
