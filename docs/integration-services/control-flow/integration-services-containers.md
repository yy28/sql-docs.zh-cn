---
description: Integration Services 容器
...
|[任务宿主容器](../../integration-services/control-flow/task-host-container.md)|为单个任务提供服务。|  
 ...
 包和事件处理程序也是容器类型。 有关详细信息，请参阅 [Integration Services (SSIS) 包](../../integration-services/integration-services-ssis-packages.md)和 [Integration Services (SSIS) 事件处理程序](../../integration-services/integration-services-ssis-event-handlers.md)。  
...
|**禁用**|指示容器是否运行的布尔值。 此属性的默认值为 **False**。<br /><br /> 有关详细信息，请参阅 <xref:Microsoft.SqlServer.Dts.Runtime.DtsContainer.Disable%2A>。|  
