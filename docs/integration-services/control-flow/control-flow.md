...
ms.topic: "article"
helpviewer_keywords: 
  - "控制流 [Integration Services], 元素"
  - "SSIS 控制流元素"
  - "SQL Server Integration Services 控制流元素"
...
# 控制流
 ...
 ![具有六个任务和一个容器的控制流](../../integration-services/control-flow/media/ssis-controlflowelmt.gif "具有六个任务和一个容器的控制流")  
...
## 控制流实现  
 ...
 ![具有包的控制流设计器的屏幕快照](../../integration-services/control-flow/media/samplecontrolflow.gif "具有包的控制流设计器的屏幕快照")  
...
-   添加连接管理器。  
     ...
     多个任务需要连接到数据源，因此必须将任务需要的连接管理器添加到包。 根据所使用的枚举器类型，Foreach 循环容器可能也需要连接管理器。 可以在逐项构造控制流时或开始构造控制流前添加连接管理器。 有关详细信息，请参阅 [Integration Services (SSIS) 连接](../../integration-services/connection-manager/integration-services-ssis-connections.md)和[创建连接管理器](../Topic/Create%20Connection%20Managers.md)。  
