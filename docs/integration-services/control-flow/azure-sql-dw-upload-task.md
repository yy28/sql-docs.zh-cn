---
title: "Azure SQL DW 上载任务 |Microsoft 文档"
ms.custom: 
ms.date: 12/16/2016
...
**Azure SQL DW 上载任务**的组成部分[用于 Azure 的 SQL Server Integration Services (SSIS) 功能包](../../integration-services/azure-feature-pack-for-integration-services-ssis.md)。
...
ColumnDelimiter|指定标记每一列末尾的一个或多个字符。 例如 &#124;（管道），\t （制表符） ' （单引号）"（双引号） 和 0x5c （反斜杠）。
...
TableName|指定目标表的名称。 请选择一个现有的表名称，或创建一个新通过选择**\<新建表 … >**。
