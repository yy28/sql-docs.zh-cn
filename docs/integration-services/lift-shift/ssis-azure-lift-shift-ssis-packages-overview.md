...
现在，可将 SQL Server Integration Services (SSIS) 项目、包和工作负荷移到 Azure 云。
-   在 Azure SQL 数据库的 SSIS 目录 (SSISDB) 或 SQL 数据库托管实例（预览版）中存储和管理 SSIS 项目和包。
...
有关 Azure-SSIS Integration Runtime 先决条件的信息，请参阅[在 Azure 中部署和运行 SSIS 包 - 先决条件](https://docs.microsoft.com/azure/data-factory/tutorial-deploy-ssis-packages-azure#prerequisites)。
...
**预配**。 必须先预配 SSIS 目录 (SSISDB) 和 Azure SSIS Integration Runtime，然后才能在 Azure 中部署和运行 SSIS 包。 按照此文章中的预配步骤操作：[在 Azure 中部署和运行 SSIS 包](https://docs.microsoft.com/azure/data-factory/tutorial-deploy-ssis-packages-azure)。
...
有关如何使用 Windows 身份验证从云连接到本地数据源的信息，请参阅[使用 Windows 身份验证连接到数据和文件共享](ssis-azure-connect-with-windows-auth.md)。

有关如何连接到文件和文件共享的信息，请参阅[使用 Azure 中部署的 SSIS 包打开并保存文件](ssis-azure-files-file-shares.md)。
...
还可以安装其他组件，例如可以安装默认情况下未安装的驱动程序。 有关详细信息，请参阅 [Azure-SSIS 集成运行时的自定义安装](/azure/articles/data-factory/how-to-configure-azure-ssis-ir-custom-setup)。
...
若要开始使用，请参阅[在 Azure 中部署和运行 SSIS 包](ssis-azure-deploy-run-monitor-tutorial.md)。

### <a name="connect-to-ssisdb"></a>连接到 SSISDB

承载 SSISDB 的 SQL 数据库的名称（格式为 `<sql_database_name>.database.windows.net`）将成为四部分名称的第一部分，从 SSDT 和 SSMS 中部署和运行包时会使用到该名称。 有关如何连接到 Azure 中的 SSIS 目录数据库的信息，请参阅[连接到 Azure 中的 SSIS 目录 (SSISDB)](ssis-azure-connect-to-catalog-database.md)。
...
有关使用 SSMS 和 Integration Services 部署向导的部署示例，请参阅[在 Azure 中部署和运行 SSIS 包](ssis-azure-deploy-run-monitor-tutorial.md)。
...
若要计划安排 Azure SQL 数据库中存储的包的执行，可使用多种工具。 有关详细信息，请参阅[计划 Azure 中的 SSIS 包](ssis-azure-schedule-packages.md)。
...
-   [在 Azure 中部署和运行 SSIS 包](ssis-azure-deploy-run-monitor-tutorial.md)
