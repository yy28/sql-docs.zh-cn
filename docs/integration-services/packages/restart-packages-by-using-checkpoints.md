---
title: "通过使用检查点重新启动包 | Microsoft Docs"
ms.custom: ""
ms.date: "03/01/2017"
...
## 保护检查点文件  
 包级别的保护不包括保护检查点文件，必须单独保护这些文件。 检查点数据只能存储在文件系统中，应当使用操作系统访问控制列表 (ACL) 来保护用于存储该文件的位置或文件夹。 重要的是务必保护检查点文件，因为它们包含了有关包状态的信息，包括变量的当前值。 例如，变量可能包含由很多行私人数据（如电话号码）组成的记录集。 有关详细信息，请参阅[访问包使用的文件](../../integration-services/security/access-to-files-used-by-packages.md)。  
  
### 配置检查点属性  
  
-   [配置检查点以重新启动失败的包](../../integration-services/packages/configure-checkpoints-for-restarting-a-failed-package.md)  
