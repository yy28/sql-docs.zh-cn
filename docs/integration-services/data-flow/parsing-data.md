...
-   快速分析是一组简单快捷的分析例程，不支持区域设置特定的数据类型转换，仅支持最常用的日期和时间格式。 有关详细信息，请参阅 [Fast Parse](../Topic/Fast%20Parse.md)。  
  
-   标准分析是一组功能齐全的分析例程，支持可在 Oleaut32.dll 和 Ole2dsip.dll 中使用的自动数据类型转换 API 所提供的所有数据类型转换。 有关详细信息，请参阅 [Standard Parse](../Topic/Standard%20Parse.md)。  
