...
helpviewer_keywords: 
  ...
  - "比较数据"
  - "数据类型 [Integration Services], 数据流"
  - "分析 [Integration Services]"
  - "字符串比较"
  - "数据流 [Integration Services], 数据选项"
...
## 数据流故障排除  
 将包部署到 [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 目录后，您可以在执行期间对包中的数据流进行分析，以检查性能或查找其他问题。 通过提供的标准报表，您可以查看包状态和历史记录，并查询提供了有关包执行的详细信息的数据库视图。 您还可以在执行期间动态添加和删除分流点以面向包的特定组件。 有关详细信息，请参阅[数据流分析](../../integration-services/performance/analysis-of-data-flow.md)。  
