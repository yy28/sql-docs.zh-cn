...
helpviewer_keywords: 
  - "源 [Integration Services], 平面文件"
  - "文本文件读取 [Integration Services]"
  - "平面文件"
  - "平面文件源"
...
-   指定平面文件源是否将列中长度为零的字符串解释为空值。  
 ...
 平面文件源输出中的输出列包含 FastParse 属性。 FastParse 指示该列是使用 [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 提供的速度较快但不区分区域设置的较快分析例程，还是使用区分区域设置的标准分析例程。 有关详细信息，请参阅 [Fast Parse](../Topic/Fast%20Parse.md) 和 [Standard Parse](../Topic/Standard%20Parse.md)。  
