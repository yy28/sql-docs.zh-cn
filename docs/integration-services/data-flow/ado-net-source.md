...
helpviewer_keywords: 
  - "ADO.NET 源"
  - "源 [Integration Services], ADO.NET"
  - "源 [Integration Services], DataReader"
...
## 数据类型支持  
 ...
 可以将 DT_NTEXT 数据类型更改为 DT_WSTR 数据类型，也可以将 DT_WSTR 更改为 DT_NTEXT。 通过在 ADO NET 源的 **“高级编辑器”** 对话框中设置 **DataType** 属性可更改数据类型。 有关详细信息，请参阅 [Common Properties](../Topic/Common%20Properties.md)。  
...
>  如果将 [!INCLUDE[vstecado](../../includes/vstecado-md.md)] 连接管理器的 **Type System Version** 属性设置为**SQL Server 2005**，则系统会将 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 日期数据类型转换为 DT_WSTR。  
 ...
 当 [!INCLUDE[vstecado](../../includes/vstecado-md.md)] 连接管理器将提供程序指定为 .NET Data Provider for [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] (SqlClient) 时，系统会将用户定义的数据类型 (UDT) 转换为 [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 二进制大型对象 (BLOB)。 当系统转换 UDT 数据类型时会应用下列规则：  
