...
|属性|数据类型|Description|  
|--------------|---------------|-----------------|  
|Connectionmanager|字符串|指定连接管理器的名称。|  
|ValidateExternalMetadata|Boolean|指示在设计时是否已使用外部数据源对缓存转换进行了验证。 如果将该属性设置为 **False**，则会在运行时针对外部数据源进行验证。<br /><br /> 默认值为 **True**。|  
|AvailableInputColumns|字符串|可用输入列的列表。|  
|InputColumns|字符串|选定输入列的列表。|  
|CacheColumnName|字符串|指定映射到选定输入列的列名称。<br /><br /> CacheColumnName 属性中的列名称必须与“缓存连接管理器编辑器”的“列”页中列出的对应列的名称相匹配。<br /><br /> 有关详细信息，请参阅 [Cache Connection Manager Editor](../../../integration-services/data-flow/transformations/cache-connection-manager-editor.md)|  
...
|表达式|字符串|表示有条件拆分转换所计算的条件的表达式。 列由沿袭标识符表示。|  
|FriendlyExpression|字符串|表示有条件拆分转换所计算的条件的表达式。 列由列名称表示。<br /><br /> 此属性的值可以使用属性表达式来指定。|  
...
|属性|数据类型|Description|  
|--------------|---------------|-----------------|  
|ASConnectionId|字符串|连接对象的唯一标识符。|  
|ASConnectionString|字符串|某个 [!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)] 项目或 [!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)] 数据库的连接字符串。|  
|CatalogName|字符串|[!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)] 数据库的名称。|  
|ModelName|字符串|数据挖掘模型的名称。|  
|ModelStructureName|字符串|挖掘结构的名称。|  
|ObjectRef|字符串|标识转换使用的数据挖掘结构的 XML 标记。|  
|QueryText|字符串|转换所使用的预测查询语句。|  
...
|属性|数据类型|Description|  
|--------------|---------------|-----------------|  
|表达式|字符串|表示有条件拆分转换所计算的条件的表达式。 列由列的 **LineageID** 属性表示。|  
|FriendlyExpression|字符串|表示有条件拆分转换所计算的条件的表达式。 列由列名称表示。<br /><br /> 此属性的值可以使用属性表达式来指定。|  
...
|ReferenceMetadataXML|字符串|[!INCLUDE[ssInternalOnly](../../../includes/ssinternalonly-md.md)]|  
|ReferenceTableName|字符串|查找表的名称。 该名称必须是有效的 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 标识符名称。 例如，如果名称包含空格，则必须用方括号将名称括起来。|  
...
|JoinToReferenceColumn|字符串|该值用于指定列所联接的引用表中的列的名称。|  
...
|CopyFromReferenceColumn|字符串|该值用于指定引用表中提供输出列中的值的列名称。|  
...
|属性|数据类型|Description|  
|--------------|---------------|-----------------|  
|CopyFromReferenceColumn|字符串|从中复制列的引用表中的列的名称。|  
|JoinToReferenceColumns|字符串|源列所联接的引用表中的列的名称。|  
...
|PivotKeyValue|字符串|复制到转换输出列的值。<br /><br /> 此属性的值可以使用属性表达式来指定。<br /><br /> 在 [Unpivot Transformation](../../../integration-services/data-flow/transformations/unpivot-transformation.md)中所述的逆透视应用场景中，透视值为以下文本值：Ham、Coke、Milk、Beer 和 Chips。 这些值在由 **“透视键值列名”** 选项指定的新 Product 列中显示为文本值。|  
