---
title: "以编程方式添加数据流任务 |Microsoft 文档"
ms.custom: 
ms.date: 03/04/2017
...
# <a name="adding-the-data-flow-task-programmatically"></a>以编程方式添加数据流任务
 ...
 有三种类型的数据流任务中的组件：**数据流源**，**数据流转换**，和**数据流目标**，该顺序中所示[!INCLUDE[ssIS](../../includes/ssis-md.md)]设计器工具箱。 这些类型又简称为源、转换和目标。 顾名思义，数据流从源到转换，然后到达目标。 这是简化的数据流说明，只为说明概念，但是数据流任务却很灵活很强大，足以处理多个源以及将许多向多个目标发送输出的转换连接在一起。  
