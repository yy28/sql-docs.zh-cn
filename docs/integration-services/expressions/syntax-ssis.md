...
ms.topic: "article"
helpviewer_keywords: 
  - "表达式 [Integration Services], 语法"
  - "语法 [Integration Services]"
...
## 标识符  
 ...
 表达式可包含 [!INCLUDE[ssIS](../../includes/ssis-md.md)] 提供的系统变量和自定义变量。 在表达式中引用的变量必须包含 @ 前缀。 例如，使用 @Counter 引用 `Counter` 变量。 @ 字符不是变量名的一部分，它仅向表达式计算器指示该标识符是一个变量。 有关详细信息，请参阅[标识符 (SSIS)](../../integration-services/expressions/identifiers-ssis.md)。  
  
## 文字  
 表达式可以包含数值、字符串和布尔文字。 表达式中使用的字符串文字必须用引号引起来。 数值和布尔文字不使用引号。 表达式语言包含用于常用转义字符的转义序列。 有关详细信息，请参阅[文字 (SSIS)](../../integration-services/expressions/literals-ssis.md)。  
