---
title: "查看数据类型映射 （SQL Server 导入和导出向导） |Microsoft 文档"
ms.custom: 
ms.date: 03/16/2017
...
如果在“列映射”  对话框的“映射”  列表中指定了一个可能无法成功的数据类型映射， [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 导入和导出向导将显示“查看数据类型映射”  页。 在此页上，可查看为了使源数据与目标兼容向导必须执行的数据类型转换的相关详细信息。 此信息包括可视提示来区分应成功从可能会导致错误或截断的转换的数据类型转换。 对于每个转换，你都可以决定是否接受向导建议的转换，也可以指定如何处理可能发生的错误。   
...
## <a name="screen-shot-of-the-review-data-type-mapping-page"></a>“查看数据类型映射”页的屏幕截图
 以下屏幕截图显示的示例**评审 Data Type Mapping**向导页。
 
 在此示例中:
 -   用户已指定中的映射**列映射**可能不会成功的对话框。
 -   “表”  列表中行上的警告图标指示，在将查询结果中的至少一列数据转换为目标表中的兼容数据类型时出现问题。
 -   中的第一行上的警告图标**数据类型映射**列表指示从映射**int**到源列的数据类型**smalldatetime**数据类型目标列可能会导致数据丢失。
 
 ![导入和导出向导的查看数据类型映射页](../../integration-services/import-export-data/media/review-mapping.png "的导入和导出向导查看数据类型映射页") 
 
## <a name="review-the-source-and-destination-tables"></a>查看源表和目标表  
 “查看数据类型映射”  页的上半部分是“表”  列表，其中列出了要从源复制到目标的表。 若要查看单个表的转换信息，请在“表”  列表中选择表。 中的页面的下半部分显示所选表的各个列的转换信息**数据类型映射**网格。

在此示例中，用户提供的查询的结果将复制到目标位置 Sales.CustomerNew2 表。 警告图标指示从查询结果的至少一列的数据转换为目标表中兼容的数据类型问题。
...
## <a name="review-the-data-type-mappings"></a>查看数据类型映射  
 “查看数据类型映射”  页的中间部分是“数据类型映射”  列表。 此网格提供有关在中选择的源表中的列的详细的转换信息**表**页的上半部分中的列表。

在此示例中，每个列在源位置将复制到具有相同名称和目标位置的数据类型的列。 中的第一行上的警告图标**数据类型映射**列表指示从映射**int**到源列的数据类型**smalldatetime**数据类型目标列可能会导致数据丢失。
