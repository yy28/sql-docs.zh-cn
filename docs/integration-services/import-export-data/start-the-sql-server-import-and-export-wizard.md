...
ms.locfileid: "92195885"
---
# <a name="start-the-sql-server-import-and-export-wizard"></a>启动 SQL Server 导入和导出向导

[!INCLUDE[sqlserver-ssis](../../includes/applies-to-version/sqlserver-ssis.md)]

用本主题中介绍的其中一种方法启动 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 导入和导出向导，以从支持的任何数据源导入数据以及将数据导出到该数据源。

> [!IMPORTANT]
> 本主题仅介绍如何 **启动** 向导。 如需其他信息，请参阅[相关任务和内容](#related-tasks-and-content)。

可以启动向导：

- 如果要将向导导入或导出任何支持的数据源，可使用 [“开始”菜单](#start-menu)。
- 如果要将向导导入或导出任何支持的数据源，可使用 [命令提示符](#command-prompt)。
- 如果要将向导导入或导出 SQL Server，可使用 [SQL Server Management Studio (SSMS)](#sql-server-management-studio-ssms)。
- 如果要将向导导入或导出 SQL Server，可使用 [具有 SQL Server Data Tools (SSDT) 的 Visual Studio](#visual-studio)。
...
2. 单击以下选项之一。
    ...
    ![启动向导“开始”](../../integration-services/import-export-data/media/start-wizard-start-64.png)

## <a name="command-prompt"></a>命令提示符
...
![启动向导 cmd](../../integration-services/import-export-data/media/start-wizard-cmd.png)
...
- **查看有关向导工作原理的简短示例。**
  ...
  - **如果想查看屏幕快照。** 查看此仅占用一页的简单示例 - [开始使用这一简单的导入和导出向导示例](../../integration-services/import-export-data/get-started-with-this-simple-example-of-the-import-and-export-wizard.md)。
