...
-   相似性为“精确”：如果两条不同记录的相同字段中的值必须完全相同，其值才能被视为匹配，则选择 **“精确”** 。 如果完全相同，则该域的匹配分数将设置为“100”，DQS 将使用此分数以及规则中其他域的分数来确定总匹配分数。 否则，该域的匹配分数将设置为“0”，并将继续处理规则直至下一个条件。 如果您为某个数字域设置了一个匹配规则并选择 **“类似”**，则可以按百分比或整数格式输入容差。 对于 date 类型的域，您可以输入容差为天、 月或年 （整数） 如果您选择 **类似**; 没有日期域没有百分比容差。 如果您选择 **“精确”**，则没有此选项。  
...
3.  单击 **“打开知识库”** 以便在现有知识库中创建或修改匹配策略。 选择知识库，选择 **“匹配策略”**，然后单击 **“下一步”**。 还可以单击 **“最近的知识库”**下的某个知识库。 如果您打开已关闭时正在处理匹配策略的知识库，将会前进到在其中关闭匹配策略活动的阶段 (如中所示 **状态** 知识文库知识库表中或在下面的知识库名称中的列 **最近的知识库**)。 如果您打开包括匹配策略且已完成的知识库，则将进入 **“匹配策略”** 页。 如果您打开不包括匹配策略但已完成的知识库，则将进入 **“映射”** 页。  
  
##  <a name="MatchingStage"></a> 映射阶段  
 在映射阶段中，您确定要为其创建匹配策略的数据源，并将源列映射到域以使域可用于匹配策略活动。  
  
1.  在 **映射** 页上，要为数据库中，创建一个策略将保留 **数据源** 作为 **SQL Server**, ，选择你想要创建的策略中的数据库 **数据库**, ，然后选择的表或视图中 **表/视图**。 源数据库必须与 [!INCLUDE[ssDQSServer](../includes/ssdqsserver-md.md)]位于同一 SQL Server 实例中。 否则，该数据库不会显示在下拉列表中。  
...
5.  单击 **加号 （+）** 控件将行添加到映射表或 **减号 （-）** 控件可删除行。  
  
6.  单击 **“预览数据源”** 可查看您选择的 SQL Server 表或视图中的数据，或查看您所选 Excel 工作表中的数据。  
  
7.  单击 **查看/选择复合域** 查看知识库中可用的复合域的列表并选择适合于映射。  
...
3.  如果您想使匹配要求更严格，则提高 **“最低匹配分数”** 的值。 有关最低匹配分数详细信息，请参阅 [如何设置匹配规则参数](#MatchingRules)。  
...
10. 选择 **重叠的群集** 从下拉列表，以便显示透视记录以及所有群集的后续记录执行匹配后，即使组群集具有共用记录。 选择 **“不重叠的群集”** ，以便在执行匹配后显示具有共用记录作为单个群集的群集。  
  
11. 单击 **重新加载来自源的数据** 以便从数据源的数据复制到临时表并其重新编制索引时运行匹配策略。 单击 **针对以往数据执行** 以运行匹配策略，而无需将数据复制到临时表和对数据重新编制索引。 对于第一次运行匹配策略，或如果您在**“映射”** 页中更改映射，然后在出现的弹出菜单中按 **“是”** ，则 **“针对以往数据执行”** 为禁用状态。 在这两种情况下，您都必须重新编制索引。 如果匹配策略未发生变化，则不需要重新编制索引。 “针对以往数据执行”有助于改善性能。  
...
13. 您可以处理 **“匹配结果”** 表中的数据，如下所示：  
    ...
    -   在 **%丢弃关闭框**, ，从下拉列表中，增量为"5"中选择一个百分比。 匹配结果表中将显示其匹配分数大于或等于该百分比的所有行。  
  
    -   如果您双击匹配结果表中的记录，DQS 显示 **匹配分数详细信息** 弹出窗口，显示透视记录和源记录 （和所有字段中的值），它们，并向下钻取匹配的记录之间的分数。 深化信息显示透视记录和源记录的每个字段中的值，以便您可以比较它们，并显示每个字段对这两个字段的总体匹配分数所贡献的匹配分数。  
...
1.  在 **匹配结果** 页上，选择 **重叠的群集** 从下拉列表，以便显示透视记录以及所有群集的后续记录执行匹配后，即使组群集具有共用记录。 选择 **“不重叠的群集”** ，以便在执行匹配后显示具有共用记录作为单个群集的群集。  
  
2.  单击 **重新加载来自源的数据** 以便从数据源的数据复制到临时表并其重新编制索引时运行匹配策略。 单击 **针对以往数据执行** 以运行匹配策略，而无需将数据复制到临时表和对数据重新编制索引。 对于第一次运行匹配策略，或如果您在**“映射”** 页中更改映射，然后在出现的弹出菜单中按 **“是”** ，则 **“针对以往数据执行”** 为禁用状态。 在这两种情况下，您都必须重新编制索引。 如果匹配策略未发生变化，则不需要重新编制索引。 “针对以往数据执行”有助于改善性能。  
  
3.  单击 **“启动”** 以针对已定义的所有规则运行匹配过程。  **匹配结果** 表显示记录 ID、 群集编号以及数据列 (包括那些未在匹配规则) 为在群集中每个记录。 群集中的前导记录是随机选定的。 (您确定存活记录由上选择存活规则 **导出** 页上，当您运行匹配项目。)群集中的每个附加行都被认为是重复的行；结果表中会提供其匹配分数（与透视记录比较）。  
...
8.  单击 **“关闭”** 可保存所做的工作，并返回到 DQS 主页。 知识库的状态将显示字符串“匹配策略 - ”和当前状态。 如果单击了 **关闭** 当你处于 **匹配结果** 屏幕上，则状态将显示:"匹配策略-结果"。 如果当你处于关闭单击 **匹配策略** 屏幕上，则状态将显示:"匹配策略 – 匹配策略"。 单击 **“关闭”**后，若要执行 **“知识发现”** 活动，您必须返回 **“匹配策略”** 活动，单击 **“完成”**，然后单击 **“是”** 发布该知识库，或单击 **“否”** 保存对该知识库所做的工作并退出。  
