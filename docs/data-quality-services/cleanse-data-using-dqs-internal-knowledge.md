...
3.  接下来，选择数据输出：只是导出清理的数据，还是导出清理的数据以及清理信息。  
    ...
    -   **数据和清理信息**：单击此单选按钮以导出每个域的以下数据：  
        ...
        -   **记录状态**：除了让每个映射域的状态字段 ** (\<DomainName> _Status**) ，" **记录状态** " 字段也会显示记录的状态。 如果记录中的任何域状态为 " *新建* " 或 " *正确*"，则 " **记录状态** " 设置为 " *正确*"。 如果记录中的任何域状态为 "已 *建议*"、" *无效*" 或 "已 *更正*"，则 " **记录状态** " 将设置为相应的值。 例如，如果记录中的任何域 *状态为 "建议"*，则 " **记录状态** " 设置为 " *建议*"。  
