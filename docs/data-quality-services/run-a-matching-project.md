...
3.  单击 **“打开数据质量项目”** 以便在现有数据质量项目中执行匹配。 选择项目，然后单击 **“下一步”**。 (或者，可以单击下的一个项目 **最近的数据质量项目**。)如果您打开某一匹配项目已关闭的将会前进到在其中关闭了匹配项目活动的阶段 (如中所示 **状态** 或下的项目名称项目表中列 **最近的数据质量项目**)。 如果您打开某一匹配项目已完成，则将进入 **导出** 页面 （和不能返回到之前的屏幕）。  
...
4.  单击 **加号 （+）** 控件将行添加到映射表或 **减号 （-）** 控件可删除行。  
  
5.  单击 **“预览数据源”** 可查看您选择的 SQL Server 表或视图中的数据，或查看您所选 Excel 工作表中的数据。  
  
6.  单击 **查看/选择复合域** 来查看知识库中可用的复合域的列表，选择作为适用于映射。  
...
1.  在 **匹配页**, ，选择 **重叠的群集** 从下拉列表，以便显示透视记录以及所有群集的后续记录执行匹配后，即使组群集具有共用记录。 选择 **“不重叠的群集”** ，以便在执行匹配后显示具有共用记录作为单个群集的群集。  
  
2.  单击 **重新加载来自源的数据** （默认值），将数据从数据源复制到临时表，其重新编制索引时运行匹配项目。 单击 **针对以往数据执行** 以运行匹配项目，而不将数据复制到临时表和对数据重新编制索引。 对于第一次运行匹配项目，或如果您在**“映射”** 页中更改映射，然后在出现的弹出菜单中按 **“是”** ，则 **“针对以往数据执行”** 为禁用状态。 在这两种情况下，您都必须重新编制索引。 如果匹配项目未发生变化，则不需要重新编制索引。 “针对以往数据执行”有助于改善性能。  
...
7.  如果您选择非透视记录表中，单击 **查看详细信息** 图标 （或双击的记录），DQS 将显示 **匹配分数详细信息** 弹出窗口，显示双击的记录和其透视记录 （和所有字段中的值），它们，并向下钻取匹配的之间的分数评分工作成果的每个字段。 双击透视记录将不会显示该弹出窗口。  
  
8.  单击 **“全部折叠”** 图标将折叠在 **“匹配结果”** 表中显示的记录，以便仅包含透视记录，而不包含重复记录。 单击 **“全部展开”** 将展开在“匹配结果”表中显示的记录，以便包含所有重复记录。  
  
9. 若要拒绝来自匹配结果的记录，请单击该记录的 **“拒绝”** 复选框。  
  
10. 若要更改最低匹配分数，用于确定记录必须要显示的匹配级别，请选择 **最小值。匹配分数** 右侧的表的上方的图标，然后输入较大的数字。 默认情况下，最低匹配分数设为 80%。 单击 **“刷新”** 可更改表的内容。  
...
3.  选中的复选框 **匹配结果** 可将匹配结果 （请参阅上面的说明） 导出到 SQL Server 数据库中的指定表或者指定的.csv 或 Excel 文件。 选中的复选框 **存活结果** 可将存活结果 （请参阅上面的说明） 导出到 SQL Server 数据库中的指定表或者指定的.csv 或 Excel 文件。  
...
7.  按如下说明选择存活规则：  
    ...
    -   选择 **透视记录** （默认值） 以将存活记录标识为 DQS 任意选择的初始透视记录。  
