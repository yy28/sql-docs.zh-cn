...
### 将多个字段映射到共享值、规则和属性的域  
 您无法将两个字段映射到同一个域，但您可以将一个字段映射到一个域，然后将第二个字段映射到已链接到第一个域的域。 这样就将字段映射到具有相同内容和属性（名称和说明除外）的两个不同域。 有关详细信息，请参阅 [将两个字段映射到链接域](#Map)。  
...
3.  从 **域列表** 上 **域管理** 页上，右键单击你想要链接到一个新域，然后单击域 **创建链接域**。  
...
-   中的设置 **引用数据**, ，**域规则**, ，**域值**, ，和 **基于字词的关系** 选项卡的域管理页可以更改链接的域或域，它链接到，并将另一个域继承所做的更改。  
