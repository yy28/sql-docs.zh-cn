...
2.  当在域属性中启用拼写检查器时，它在中启用 **管理域值** 页上， **域值** 页或 **基于字词的关系** 页上，与 **管理和查看结果** 页。 若要禁用拼写检查器在这些页上的，单击 **启用/禁用拼写检查器** 图标。 单击图标将更改拼写检查器在该页上的状态。 同样，如果 **启用拼写检查器** 禁用域的属性，则单击 **启用/禁用拼写检查器** 图标使拼写检查器的页上。 如果您退出该页后又返回到该页，则按钮状态将再次由 **“启用拼写检查器”** 域属性确定。  
...
##  <a name="How"></a> 拼写检查器的原理  
 ...
 DQS 将在 **“值”** 表的 **“值”** 列和 **“更正为”** 列中的值提出建议。 当您在 **“值”** 列中选择一项建议时，值类型设置为 **“错误”**，并且建议复制到 **“更正为”** 列，就像是您手动插入的一样。 如果有现有的更正，它将成为一项建议。 在 **“清理”** 活动的 **“管理和查看结果”** 页中，当您在 **“更正为”** 列中选择一项建议时，DQS 将用所做选择替换当前选择的值，并且当前选择的值将成为一项建议。 在 **管理和查看结果** 页 **清理** 活动，不建议在记录级别 （下部网格） 中进行。  
