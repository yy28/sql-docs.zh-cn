---
title: 使用 SQL Operations Studio （预览版） 进行连接时使用 Active Directory 身份验证 (Kerberos) |Microsoft Docs
description: 了解如何启用 Kerberos 以使用 Active Directory 身份验证的 SQL Operations Studio （预览版）
ms.custom: tools|sos
ms.date: 11/17/2017
ms.prod: sql
...
- 将在 macOS 加入到 Active Directory 域控制器，按照 [这些步骤] (https://support.apple.com/kb/PH26282?viewlocale=en_US&locale=en_US)。



## <a name="configure-kdc-in-krb5conf"></a>Krb5.conf 中配置 KDC

编辑`/etc/krb5.conf`在所选的编辑器中。 配置以下密钥
