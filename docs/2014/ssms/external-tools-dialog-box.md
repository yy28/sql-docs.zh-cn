...
# <a name="external-tools-dialog-box"></a>外部工具对话框
  使用 "**外部工具**" 对话框可以将外部工具（如 SQLCMD 或记事本）添加到 "**工具**" 菜单。 通过添加外部工具，您可以在[!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)]环境中工作时轻松地启动其他应用程序。 启动此工具时，可以指定参数及工作目录。 此外，“输出”**** 窗口中可以显示某些工具的输出。 通过“工具”**** 菜单可以打开“外部工具”**** 对话框。  
