...
###  <a name="Security"></a> 安全性  
 若要配置用户以创建或执行[!INCLUDE[msCoName](../../includes/msconame-md.md)][!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]代理作业，必须首先将现有的 SQL Server 登录名或 msdb 角色添加到以下项之一[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]代理固定数据库角色的 msdb 数据库中：SQLAgentUserRole、 SQLAgentReaderRole 或 SQLAgentOperatorRole。  
