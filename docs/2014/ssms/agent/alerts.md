...
-   **严重性级别**  
     ...
     [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]代理在发生特定严重级别的任何错误时触发警报。 例如，可以指定严重级别 15 来响应 Transact-SQL 语句中的语法错误。  
...
-   **事件文本**  
     ...
     [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]当指定的事件在事件消息中包含特定文本字符串时，代理将发出警报。 例如，可以定义警报来响应包含特定表名或特定约束的消息。  
...
-   计数器和**值****时发出警报**  
     ...
     警报的阈值和导致警报的行为。 阈值是数字。 行为是下列之一：“低于”****、“等于”**** 或“大于”**** 指定的值。 
  **值** 是描述性能条件计数器的数字。 例如，若要为性能对象 **SQLServer:Locks** 设置在 **Lock Wait Time** 超过 30 分钟时要发生的警报，则可以选择“大于”**** 并“指定 30 作为值”****。  
...
-   **命名空间**  
     ...
     [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]代理作为 WMI 客户端注册到用于查询事件的 WMI 命名空间。  
  
-   **查询**  
  
     [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]代理使用提供的 Windows Management Instrumentation 查询语言（WQL）语句来标识特定事件。  
