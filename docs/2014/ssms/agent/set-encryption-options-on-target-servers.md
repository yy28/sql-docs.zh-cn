...
# <a name="set-encryption-options-on-target-servers"></a>在目标服务器上设置加密选项
 ...
 若要为特定的主服务器/目标服务器通信通道配置所需的适当安全级别，请[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]将目标服务器上的代理注册表子项**\\\HKEY_LOCAL_MACHINE \software\microsoft\microsoft SQL Server***instance_name*>**\SQLServerAgent\MsxEncryptChannelOptions(REG_DWORD)** \<instance_name \SQLServerAgent\MsxEncryptChannelOptions （REG_DWORD）设置为以下值之一。 \<*instance_name*> 的值为 **MSSQL.**_n_。 例如， **MSSQL.1** 或 **MSSQL.3**。  
