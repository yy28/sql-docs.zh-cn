...
## <a name="managing-reporting-services-projects-using-sql-server-management-studio"></a>使用 SQL Server Management Studio 管理 Reporting Services 项目  
 ...
 可通过使用“共享计划”文件夹来管理共享计划以及管理报表服务器数据库（ReportServer、ReportServerTempdb）。 将 Report Server 数据库移到新的或不同的 SQL Server 数据库引擎（）时，还会在 Master 系统数据库中创建 RSExecRole [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] [!INCLUDE[ssDE](../includes/ssde-md.md)] 。 有关这些任务的详细信息，请参阅以下主题：  
