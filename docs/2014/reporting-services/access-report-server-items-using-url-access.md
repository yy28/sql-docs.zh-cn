...
## <a name="access-a-resource"></a>访问资源  
 若要访问资源，请使用*rs： Command*=*GetResourceContents*参数。如果资源与浏览器（如图像）兼容，则会在浏览器中打开该资源。 否则，系统会提示您打开文件或资源或将其保存到磁盘。  
...
## <a name="access-the-contents-of-a-folder"></a>访问文件夹的内容  
 若要访问文件夹的内容，请使用*rs： Command*=*GetChildren*参数。 将返回一个一般的文件夹导航页，其中包含指向在所请求文件中包含的子文件夹、报表、数据源和资源的链接。 例如：  
