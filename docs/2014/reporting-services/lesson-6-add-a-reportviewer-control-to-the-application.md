...
2.  从**AJAX Extensions**组中**工具箱**窗口中，拖动**ScriptManager**到设计图面上的控件。  
  
3.  从“报表”组中，将一个 ReportViewer 控件拖到设计图面上的 ScriptManager 控件下。  
  
4.  通过单击 **ReportViewer** 控件右上角的箭头，打开 **ReportViewer Tasks** 窗口。  
  
5.  在**选择报表**框中，选择你创建的父报表。  
  
     选择某个报表后，将自动创建在该报表中使用的数据源的实例。 生成代码以实例化每个 DataTable (并将其[数据集](http://msdn.microsoft.com/library/system.data.dataset\(v=vs.100\).aspx)容器)。 [ObjectDataSource](http://msdn.microsoft.com/library/system.web.ui.webcontrols.objectdatasource\(v=vs.100\).aspx)控件添加到设计图面上，与报表中使用每个数据源对应。 此数据源控件为自动配置。  
  
     如果你使用 Microsoft Visual Studio 2012，请确保如果中列出的完全限定的名称，与是与项目的命名空间，完全限定的 DataSet1 绑定 ObjectDataSource 控件**选择业务对象**下拉列表框 (例如，Projectnamespace.DataSet1TableAdapters.ProductTableAdapter)。 通过右键单击对象数据源，然后单击访问列表框**配置数据源**。  
