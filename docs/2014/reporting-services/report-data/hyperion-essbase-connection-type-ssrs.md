...
##  <a name="Extended"></a> 扩展字段属性  
 [!INCLUDE[extEssbase](../../includes/extessbase-md.md)] 数据处理扩展插件支持扩展字段属性。 扩展的字段属性是除了`Value`和`IsMissing`定义为数据集字段的数据处理扩展插件。 扩展属性包括预定义属性和自定义属性。 预定义属性是对多个数据源通用的属性。 自定义属性对于每个数据源都是唯一的。  
  
 扩展字段属性不作为可拖至报表布局的项出现在“报表数据”窗格中。 相反，您的属性的父字段拖动到报表，然后更改默认属性由`Value`到你想要使用的属性。  
  
 当您将鼠标光标停在查询设计器“元数据”窗格中的某个字段上时，扩展字段属性的名称便会显示在工具提示中。 可用于浏览基础数据的查询设计器的详细信息，请参阅[Hyperion Essbase Query Designer User Interface](hyperion-essbase-query-designer-user-interface.md)。  
...
### <a name="predefined-field-properties"></a>预定义的字段属性  
 通常受多个数据访问接口支持并出现在报表数据集的基础 MDX 查询中的预定义字段属性。 例如，MEMBER_UNIQUE_NAME 映射到预定义的报表数据集字段属性的 MDX 维度属性`UniqueName`。 若要在文本框中包含唯一名称值，请使用表达式 `=Fields!`*\<FieldName>*`.UniqueName`。  
