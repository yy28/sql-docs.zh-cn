...
ms.locfileid: "36128711"
---
# <a name="html-device-information-settings"></a>HTML 设备信息设置
  下表列出了以 HTML 格式呈现时的设备信息设置。  
  
> [!IMPORTANT]  
>  下表列出的带 **(\*)** 的设备信息设置不推荐使用，不应在新应用程序中使用。 有关详细信息，请参阅[Deprecated Features in SQL Server 2014 中的 SQL Server Reporting Services](deprecated-features-in-sql-server-reporting-services-ssrs.md)。  
...
|`HTMLFragment`|指示是否创建一个 HTML 片段以取代完整 HTML 文档。 HTML 片段在 TABLE 语句中包含报表内容，并忽略 HTML 和 BODY 元素。 默认值是 `false`。 使用 SOAP 进行呈现`HTMLFragment`属性设置为`true`创建包含会话信息可以用于正确地请求图像的 Url。 图像必须为报表服务器数据库中的已上载资源。|  
...
|**Parameters**|指示是显示还是隐藏工具栏的参数区域。 如果将此参数设置为值`true`，将显示工具栏的参数区域。 此参数的默认值是`true`。|  
|`PrefixId`|如果用于`HTMLFragment`，将指定的前缀添加到所有`ID`中创建 HTML 片段的属性。|  
...
|`Toolbar`|指示是显示还是隐藏工具栏。 此参数的默认值是`true`。 如果此参数的值是`false`，将忽略所有剩余选项 （除文档结构图中）。 如果您忽略此参数，则自动为支持工具栏的呈现格式显示此工具栏。<br /><br /> 当您使用 URL 访问以呈现报表时，将呈现报表查看器工具栏。 此工具栏不通过 SOAP API 呈现。 但是，`Toolbar`设置会影响报表的显示方式的设备信息时使用 SOAP`Render`方法。 如果此参数的值是`true`时使用 SOAP 呈现到 HTML，呈现报表中的，仅第一节。 如果值为 `false`，则整个 HTML 报表将呈现为单个 HTML 页。|  
|`UserAgent`|`user-agent`正在发出请求，在 HTTP 请求中找到的浏览器的字符串。|  
