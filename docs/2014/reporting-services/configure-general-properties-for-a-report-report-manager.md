---
title: 配置报表 （报表管理器） 的常规属性 |Microsoft Docs
ms.custom: ''
ms.date: 03/07/2017
...
3.  单击 **“属性”** 选项卡。  
     ...
     或者，如果**内容**页是详细信息视图中，单击属性页图标：  
  
     ![属性页图标](media/prop.gif "属性页图标")  
  
4.  **常规**显示属性页，并可以按如下所示配置属性：  
  
    -   在中**属性**部分中，您可以修改报表名称和说明。  
  
    -   可以选择**在列表视图中隐藏**复选框可以隐藏项，方式排列项横向和纵向页面的默认页面布局 （列表视图） 中打开页面时。  
  
    -   在中**报表定义**部分中，单击**编辑**可提取报表定义的副本。 在本地对报表定义所做的修改不保存在报表服务器上。  
  
         或者，若要更新报表定义的.rdl 文件中，单击**更新**。  
  
        > [!NOTE]  
        >  如果更新某一报表定义，则必须在更新完成后重置数据源设置。  
  
    -   使用**删除**或**移动**按钮可以删除或移动报表。  
