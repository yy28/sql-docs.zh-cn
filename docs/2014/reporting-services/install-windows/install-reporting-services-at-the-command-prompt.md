...
```  
 ...
 下面的示例安装的 SQL Server 功能，包括所有[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]纯模式和 SharePoint 模式。  
