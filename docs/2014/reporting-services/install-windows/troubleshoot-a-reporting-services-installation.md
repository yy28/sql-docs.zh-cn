 ...
 **说明：** 如果在 SharePoint 模式中成功安装 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]，并在 SharePoint 2010 的 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 外接程序后，你在以下两个菜单中看不到 "SQL Server Reporting Services"，则表示 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 服务尚未注册:  
...
2.  验证 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 服务在页面上将状态显示为 "**已启动**"： SharePoint 2010 管理中心-> "**应用程序管理**"-> "**管理服务器上的服务**"  
 ...
 **解决方法：** 错误消息包含三个建议的步骤来纠正此问题。 消息 "A Report Server URL 未配置 ..." 中的第一个建议 在与 [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)]之前的报表服务器版本集成时相关。 使用 **SQL Server Reporting Services (2008 和 2008 R2)** 在 **常规应用程序设置**页上完成针对以前报表服务器版本的 SharePoint 配置。  
 ...
 **解决方法：** 确认您有 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 服务应用程序且已正确对其进行配置。 有关详细信息，请参阅[安装 sharepoint 2010 Reporting Services Sharepoint 模式](../../../2014/sql-server/install/install-reporting-services-sharepoint-mode-for-sharepoint-2010.md)中的 "创建 Reporting Services 服务应用程序" 部分  
...
1.  删除以下注册表项：  
    ...
    -   **HKLM\SYSTEM\CurrentControlSet\Services\MSRS 2011 Web 服务**  
  
    -   **HKLM\SYSTEM\CurrentControlSet\Services\MSRS 2011 Windows 服务**  
...
2.  打开命令窗口，然后在提示符下键入以下命令：  
    ...
    -   **运行 \<** *.net 2.0 Framework 目录* **> \InstallUtil.exe \<** *报表服务器 Bin 目录* **> \ReportingServicesLibrary.dll**  
        ...
        > [!NOTE]  
        >  将 \< *.net 2.0 Framework 目录*> 替换为 .NET Framework 2.0 文件的物理路径，并将 \<*报表服务器 Bin 目录*> 替换为 Report Server Bin 文件的物理路径。  
...
###  <a name="ConfigPropsMissing"></a> 从 SQL Server 2005 升级后，未配置 ReportServerExternalURL 和 PassThroughCookies 配置属性  
 从 [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] 升级到 [!INCLUDE[ssRSCurrent](../../includes/ssrscurrent-md.md)] 时，`ReportServerExternalURL` 和 `PassThroughCookies` 配置属性并未在升级过程中得到配置。 `ReportServerExternalURL` 是一个可选属性，并且仅当使用 SharePoint 2.0 Web 部件并且希望用户能够检索报表并在新的浏览器窗口中打开该报表时，才应设置此属性。 有关 `ReportServerExternalURL`的详细信息，请参阅[配置文件&#40;中的 url&#41;Configuration Manager SSRS](../../reporting-services/install-windows/urls-in-configuration-files-ssrs-configuration-manager.md)。 仅当使用自定义身份验证方法时，才需要 `PassThroughCookies`。 有关 `PassThroughCookies`的详细信息，请参阅[配置报表管理器以传递自定义身份验证 cookie](../security/configure-the-web-portal-to-pass-custom-authentication-cookies.md)。  
...
###  <a name="Default2005InstallBreaks2008"></a>在运行 SQL Server 2012 Reporting Services 的计算机上 SQL Server 2005 Reporting Services 的默认实例安装失败  
 如果尝试在已运行 [!INCLUDE[ssRSCurrent](../../includes/ssrscurrent-md.md)]实例的计算机上安装 [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 的默认实例，则 [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 实例将无法安装，并出现以下错误消息：  
...
-   如果 [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 实例不需要为默认实例，则可以在安装 [!INCLUDE[ssRSCurrent](../../includes/ssrscurrent-md.md)] 实例之后，将 [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 实例作为命名实例安装。  
...
-   [论坛：SQL Server Reporting Services](http://social.msdn.microsoft.com/Forums/sqlreportingservices/threads)  
 ...
 ![SharePoint 设置](https://docs.microsoft.com/analysis-services/analysis-services/media/as-sharepoint2013-settings-gear.gif "SharePoint 设置")[通过 Microsoft SQL Server 连接（ https://connect.microsoft.com/SQLServer/Feedback) 来提交反馈和联系信息](https://connect.microsoft.com/SQLServer/Feedback)。  
