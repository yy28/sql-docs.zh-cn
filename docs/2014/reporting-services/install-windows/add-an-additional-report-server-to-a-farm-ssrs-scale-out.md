...
> [!IMPORTANT]  
>  并非所有 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 版本都支持 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]的扩展。 有关详细信息，请参阅[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]部分[支持的 SQL Server 2014 的版本功能](../../../2014/getting-started/features-supported-by-the-editions-of-sql-server-2014.md)。  
...
|安装 SharePoint Server 2010 SP1。|如果你现有的 SharePoint 场具有 SharePoint 2010 SP1 安装下载并安装 SharePoint 2010 SP1 从：[http://support.microsoft.com/kb/2460045](http://go.microsoft.com/fwlink/p/?linkID=219697)。<br /><br /> 有关 SharePoint 2010 SP1 的详细信息，请参阅 [安装 Office 2010 SP1 和 SharePoint 2010 SP1 时的已知问题](http://support.microsoft.com/kb/2532126)：|  
|运行 SharePoint 产品配置向导以便向场中添加服务器。|1） 在**Microsoft SharePoint 2010 产品**程序组中，单击**Microsoft SharePoint 2010 产品配置向导**。<br /><br /> 2） 在**连接到服务器场**页上，选择**连接到现有场**单击**下一步**。<br /><br /> 3） 在**指定配置数据库设置**页上，键入用于在现有服务器场和配置数据库的名称的数据库服务器的名称。 单击“下一步” 。<br />**\*\* 重要\* \*** 如果你看到类似于以下的错误消息，并且验证您具有权限，然后验证中的 SQL Server 网络配置为启用哪些协议**Sql ServerConfiguration Manager**:"无法连接到数据库服务器。 确保数据库存在，是 Sql Server，并且您具有适当的权限来访问服务器。"<br />**\*\* 重要\* \*** 如果看到页**服务器场产品和修补程序状态**，你将需要查看页上的信息并使用所需的文件更新服务器，然后才能继续使用将服务器加入到场中。<br /><br /> 4） 在**指定场安全设置**页键入场通行短语，然后单击**下一步**。 在确认页上单击 **“下一步”** 以便运行向导。<br /><br /> 5） 单击**下一步**运行**场配置向导**。|  
|验证服务器已添加到 SharePoint 场。|1) 在 SharePoint 管理中心的“系统设置”组中，单击“管理此场中的服务器”。<br /><br /> 2) 确认新的服务器已添加并且状态正确。<br /><br /> 3） 请注意你看不到服务**SQL Server Reporting Services 服务**运行。 该服务将在下一步中安装。<br /><br /> 4） 从 WFE 角色中删除此服务器，请单击**管理服务器上的服务**和停止该服务**Microsoft SharePoint Foundation Web 应用程序**。|  
|安装和配置 Reporting Services SharePoint 模式。|运行 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 安装。 有关详细信息的安装[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]SharePoint 模式，请参阅[安装 Reporting Services SharePoint 模式用于 SharePoint 2010](../../../2014/sql-server/install/install-reporting-services-sharepoint-mode-for-sharepoint-2010.md)如果服务器将仅用作应用程序服务器和服务器将不用作WFE，则你不需要选择**Reporting Services 外接程序用于 SharePoint 产品**上：<br /><br /> **安装角色**页上，选择**SQL Server 功能安装**<br /><br /> **功能选择**页上，选择**Reporting Services-SharePoint**<br /><br /> -或-<br /><br /> **Reporting Services 配置**页验证**仅安装**选择了**Reporting Services SharePoint 模式**。|  
...
##  <a name="bkmk_additional"></a> 附加配置  
 ...
 未启用 Reporting Services Web 服务。 配置的 Reporting Services SharePoint 服务具有至少一个实例\<b l e > 设置为 true。 有关详细信息，请参阅[修改 Reporting Services 配置文件 (RSreportserver.config)](../report-server/modify-a-reporting-services-configuration-file-rsreportserver-config.md)  
