...
-   使用的高可用性功能[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] [!INCLUDE[ssDE](../includes/ssde-md.md)]来最大程度地提高 Report Server 数据库的正常运行时间。 如果您将某个 [!INCLUDE[ssDE](../includes/ssde-md.md)] 实例配置为在故障转移群集中运行，则在创建报表服务器数据库时可选择该实例。  
  
-   将[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] [!INCLUDE[ssHADR](../includes/sshadr-md.md)]与[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)]数据库和数据源一起使用。 有关详细信息，请参阅 [Reporting Services 与 AlwaysOn 可用性组 (SQL Server)](../database-engine/availability-groups/windows/reporting-services-with-always-on-availability-groups-sql-server.md)。  
