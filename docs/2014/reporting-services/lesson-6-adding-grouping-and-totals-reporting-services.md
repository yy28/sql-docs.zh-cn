...
# <a name="lesson-6-adding-grouping-and-totals-reporting-services"></a>Lesson 6: Adding Grouping and Totals (Reporting Services)
 ...
 向报表添加正在运行的总计的信息，请参阅此内容策展上 curah.microsoft.com:[将合计添加到 Reporting Services (SSRS) 报表](http://go.microsoft.com/fwlink/p/?LinkId=403698)。  
  
 **本主题内容：**  
  
-   [对在报表中的数据进行分组](#bkmk_groupdata)  
...
##  <a name="bkmk_groupdata"></a> 对在报表中的数据进行分组  
...
4.  从**报表数据**窗格中，拖动`Order`字段**行组**窗格。 并将其放置到 Date 下面和 **(Details)** 上面。  
     ...
     请注意，行控点中现在有两个方括号，用于显示两个组。 表现在还包含两个`Order`列，太。  
  
5.  删除的原始日期和订单列**右**的双线。 这将删除该单个记录值，以便仅显示组值。 选择并右键单击两个列的列句柄，然后单击“删除列”。  
...
1.  右键单击顺序单元格，指向**添加总计**，然后单击**后**。  
  
     这将添加新行的每日和标签包含的总量和美元总金额的总和"**总**"顺序列中。  
...
1.  右键单击“Date”单元，指向“添加总计”，并单击“晚于”。  
     ...
     这将添加新行包含整个报表的总量和美元总金额的总和和**总**标签英寸`Date`列。  
...
3.  在**TargetServerURL**键入报表服务器，名称的名称，例如**http://\<服务器名 > / reportserver**  
...
5.  在工具栏上，单击 **“生成”** ，然后单击 **“部署教程”**。  
    ...
    > ------ Build started: Project: tutorial, Configuration: Debug ------Skipping 'Sales Orders.rdl'. 项是最新。生成已完成--0 个错误、 0 警告---启动的部署： 项目： 教程中，配置： 调试---将部署到 http://\<服务器名称 > / reportserverDeploying 报告 / 教程/销售订单。部署完成-0 个错误、 0 的警告 === 生成： 1 的成功或最新，0 失败，0 被跳过 === 部署： 1 成功，0 失败，0 被跳过 ===  
  
     如果您看到如下错误消息，则确认您对报表服务器的权限并且已使用管理员权限启动了 [!INCLUDE[ssBIDevStudio](../includes/ssbidevstudio-md.md)] 。  
  
    > "用户授予的权限 XXXXXXXX\\< 你的用户名\>不足以执行此操作"  
  
6.  使用管理员权限，例如启动报表管理器，右键单击 Internet Explorer 的图标，单击**以管理员身份运行**。  
  
     浏览至报表管理器 URL，例如： `http://<server name>/reports`。  
  
7.  浏览到包含该报表的文件夹，然后单击报表名称`Sales Orders`以在浏览器中查看呈现的报表。  
