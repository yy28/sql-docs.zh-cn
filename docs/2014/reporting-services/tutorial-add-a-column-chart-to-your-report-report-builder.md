...
3.  [格式化并水平轴标签](#Horizontal)  
  
4.  [将图例移动](#Legend)  
  
5.  [标题图表](#ChartTitle)  
  
6.  [设置格式和垂直轴标签](#Vertical)  
...
##  <a name="Chart"></a> 1.使用图表向导创建图表报表  
 从**入门**对话框，请使用图表向导以创建嵌入数据集，选择共享的数据源，并创建一个柱形图。  
...
2.  右击水平轴标题，然后单击**显示轴标题**以删除标题。 因为水平轴显示日期，所以不需要标题。  
  
3.  右击水平轴，然后单击**水平轴属性**。  
  
4.  在**轴选项**下页上**轴范围和间隔**，类型**3**为**间隔**。 图表将每隔三个日期显示一次标签。  
  
5.  单击**标签**。  
  
6.  在**更改轴标签自动调整选项**，选择**禁用自动调整**。  
...
2.  右击图表上的图例，然后单击**图例属性**。  
  
3.  有关**布局和位置**，选择一个不同的位置。 例如，将图例位置设置为底部中间。  
  
     如果将图例置于图表的顶部或底部，则图例的布局将会从垂直改为水平。 可以从“布局”下拉列表中选择不同的布局。  
  
4.  [!INCLUDE[clickOK](../includes/clickok-md.md)]  
  
5.  （可选）因为本教程中只有一个类别，所以不需要图例。 要删除图例，请右键单击图例，然后单击**删除图例**。  
...
2.  选择的单词**图表标题**在顶部的图表，然后键入以下文本：**应用商店销售订单合计**。  
...
3.  在功能区中，在**主页**选项卡上，在**数**组中，单击**货币**按钮。 轴标签将更改以显示货币格式。  
  
4.  在功能区中，在**主页**选项卡上，在**数**组中，单击**减少小数位数**按钮两次，以显示数字舍入为最接近美元。  
  
5.  右键单击垂直轴，然后单击**垂直轴属性**。  
  
6.  单击**数**。 请注意，**货币**中已选择**类别**框中，和**小数位数**已**0** （零）。  
  
7.  在**中显示值**框中，单击**数千**。  
  
8.  [!INCLUDE[clickOK](../includes/clickok-md.md)]  
  
9. 右键单击边上的图表的垂直轴标题，然后单击**轴标题属性**。  
  
10. 替换中的文本**标题文本**字段包含以下文本：**总 （以千为单位） 上的销售**。 还可以指定与如何设置标题格式相关的多种选项。  
...
3.  右键单击 **[sum （sales)]** 中的字段**值**区域中，，然后单击**添加计算系列**。  
...
3.  类型**Sales Chart**，按 ENTER，，然后键入**到 2009 年 12 月的 1 月**，因此它如下所示：  
  
     **Sales Chart**  
  
     **到 2009 年 12 月的 1 月**  
  
4.  选择**Sales Chart**，然后单击**加粗**按钮**字体**节**主页**功能区选项卡。  
  
5.  选择**到 2009 年 12 月的 1 月**，然后在**字体**节**主页**选项卡上，将字体大小设置为**10**。  
  
6.  （可选）你可能需要进行**标题**高度以适应行文本的两行通过拉下的双向箭头，当您单击中间的下边缘的文本框。  
