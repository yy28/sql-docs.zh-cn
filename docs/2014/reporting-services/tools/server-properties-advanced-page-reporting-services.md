 ...
 **EnableExecutionLogging**  
 指示报表执行日志记录是否处于启用状态。 默认值是 `true`。 有关 Report Server 执行日志的详细信息，请参阅[报表服务器执行日志和 ExecutionLog3 视图](../report-server/report-server-executionlog-and-the-executionlog3-view.md)。  
 ...
 **名**  
 在报表管理器的页面标题上显示的报表服务器站点的名称。 默认值是 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]。 此属性可以是空字符串。 最大长度为 8,000 个字符。  
 ...
 **SQL =** 当快照存储在 Report Server 数据库中时，将对其进行压缩。 这是当前的行为。  
  
 **None** = 不压缩快照。  
  
 **全部 =** 为所有存储选项（包括 Report Server 数据库或文件系统）压缩快照。  
 ...
 **EnableIntegratedSecurity**  
 确定报表数据源连接是否支持 Windows 集成安全性。 默认为 `True`。 有效值如下：  
 ...
 **Enablereportdesignclientdownload 设置**  
