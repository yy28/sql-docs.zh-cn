...
|aggregate|由多个单独值组成的汇总值。|  
|聚合函数|对多个值执行计算并返回单个汇总值的函数。|  
|聚合的聚合 (aggregate of aggregates)|根据聚合计算的汇总值，例如一组总和中的最大值。|  
|聚合查询 (aggregate query)|通过包括聚合函数（例如 Sum 或 Avg）来汇总多行信息的查询。|  
|聚合|包含预先计算好的多维数据集数据的表或结构。|  
|alias|表达式中用来表示表或列的备用名称，通常用来缩短代码中后续引用的名称，防止出现可能的不明确引用，或在查询输出中提供更具说明性的名称。|  
|分析数据 (analytical data)|报表中可进行分析的一组数值。 在地图报表中，分析值控制各地图层上地图元素的颜色、标记类型或大小。|  
|抗锯齿 (antialiasing)|在图形中，使用某一算法以便使曲线的近似锯齿形状平滑。|  
|应用程序数据库 (application database)|用来存储一个应用程序的用户数据和系统数据的数据库。|  
...
|授权|1.对授予用户的权限和访问权进行验证的操作。 2. 将访问某些信息、服务或功能的权限授予个人、计算机进程或设备的过程。|  
|轴|图形上用于显示点的位置的垂直和水平线。|  
|轴间隔 (axis interval)|图表轴上主要刻度线之间的单位数。 间隔是自动计算的，但也可以手动设置。|  
...
|批处理|1.一起提交并作为一个组运行的一组 SQL 语句。  2. 在 Reporting Services 中单个事务内的 SOAP 方法调用的集合。|  
...
|collection|1.包含一组其他对象的对象。 2. 在 Master Data Services 中，来自显式层次结构和其他集合的成员的分层形式的分组。|  
...
|concurrency|使多个用户能够同时访问和更改共享数据的进程。 SQL Server 使用锁定功能来使多个用户能够同时访问和更改共享数据而彼此之间不发生冲突。|  
...
|credential|由主体用来确立其身份的数据，例如密码或用户名。|  
...
|dataset|1.在 OLE DB for OLAP 中，是指运行多维表达式 (MDX) SELECT 语句所得到的多维数据集。   <br />2.在 Reporting Services 中，是指一种命名规范，其中包括数据源定义、查询定义、可选参数值、计算字段以及作为报表定义 (.rdl) 文件一部分的筛选和排序规则信息。 一个 .rdl 文件可以有多个数据集。|  
|DBCS|双字节字符集 (double-byte character set)|  
|default|1.在用户未指定值、设置、排序规则或名称的情况下，系统自动分配的数据值、选项设置、排序规则或名称。 2. 发生某些事件时，在用户未指定要执行的操作的情况下自动执行的操作。|  
|默认数据库|用户登录到 SQL Server 后立即与其建立连接的数据库。|  
|默认实例 (default instance)|与安装计算机具有相同名称的 SQL Server 实例。|  
|delimiter|1.在 Transact-SQL 中，是指使用双引号 ("") 或方括号 ([]) 指示对象名称开头和结尾的字符。 2. 在 Integration Services 和 Reporting Services 中，是指用于分隔记录、字段或字符串的字符。|  
...
|域|1.在 Windows 安全性中，指为了便于查看和管理而进行分组且共享一个公共安全数据库的计算机集合。 2. 在关系数据库中，是指一列中允许使用的有效值集。|  
...
|实体|1.在 Reporting Services 中，是指模型项的逻辑集合，包括源字段、角色、文件夹和表达式，它们都是以熟悉的业务术语表示的。 2. 在 Master Data Services 中，实体是模型内的对象。 每个实体都包含由属性定义的相关成员。|  
...
|表达式|1.在 SQL 中，是指符号和运算符的组合，其计算结果为单个数据值。 2. 在 Integration Services 中，是指文本、常量、函数和运算符的组合，其计算结果为单个数据值。|  
|field|1.窗口或记录中存储单个数据值的区域。  2. 在报表生成器中，数据集字段表示来自数据连接的数值或非数值数据。 例如，销售额、总销售额、客户名称、数据库标识符、URL、图像和空间数据。 报表可具有三种类型的字段：数据集字段、数据集计算字段和内置字段。|  
...
|层次结构|一种按每个成员都有一个父成员和零个或更多个子成员的方式组织维度成员的逻辑树结构。|  
...
|实例|1.计算机上运行的 SQL Server 副本。 2. 报表项（例如报表部件、子报表、动态成员或组）的特定副本。|  
|整数|一种数字类别的数据类型，包括 bigint、int、smallint 和 tinyint 数据类型。|  
...
|latitude|作为世界大地坐标系 (WGS 84) 数据中十进制度数值的纬度坐标。 有效范围是从 -90.0 到 +90.0。|  
...
|locale|定义与语言有关的某些行为的 Windows 操作系统属性。|  
...
|longitude|作为世界大地坐标系 (WGS 84) 数据中十进制度数值的坐标。 有效范围是从 -180.0 到 +180.0。|  
...
|metadata|有关数据属性的信息，例如列中的数据类型（numeric、text 等）或列的长度；有关文件的信息，例如其标题、说明、创建日期以及最后修改日期；或者有关数据结构的信息或指定对象设计（例如多维数据集或维度）的信息。|  
...
|多个实例|同一台计算机上运行的多个 SQL Server 副本。|  
...
|ODBC 驱动程序|一种动态链接库 (DLL)，可供支持 ODBC 的应用程序（例如 Excel）用来访问 ODBC 数据源。|  
...
|parameter|报表中的一种内置全局集合，使用户能够设置可根据报表数据、外观而变化并且可以连接相关报表的值。 报表参数是根据查询参数和数据集参数自动创建的，并且可由报表作者手动更改。|  
...
|pivot|1.在交叉表格格式的数据浏览器中，将行旋转为列，将列旋转为行。 2. 从多维数据结构的一组可用维度中选择维度，以显示在交叉表格格式结构的行和列中。|  
...
|property|为控件、字段或数据库对象设置的命名属性，用来定义对象的其中一个特征（例如大小、颜色或屏幕位置）或某一方面的行为（例如是否隐藏）。|  
|protocol|使计算机之间可以交换信息的一组标准的格式和过程。|  
|provider|1.OLE DB 访问接口。 2. 提供数据库访问的进程内动态链接库 (DLL)。|  
...
|scope|用于多个上下文中。 作用域可以指定要用于计算表达式的数据、呈现的页面上的文本框集或者可基于切换显示或隐藏的报表项集。|  
|脚本|用于执行某项操作的 Transact-SQL 语句的集合。|  
...
|快照|请参阅另一术语：报表快照 (report snapshot)|  
...
|字符串|包含字母、数字和大多数字符的数据类型。 String 字段（数据类型为 String 的字段）中的数字不可用于数字计算。 必须使用数字数据类型（例如 Integer 或 Float）执行数字计算。 对于参数，String 与 Text 数据类型相同。|  
...
|可变|1.在 Integration Services 中，用来存储可在脚本、表达式和属性表达式中使用以设置列值和包对象属性的值。 <br />2.分配了值的已定义实体。 局部变量是使用 DECLARE@localvariable 语句定义的，并在使用 SELECT 或 SET@localvariable 语句声明该变量的语句批处理内为其分配初始值。|  
