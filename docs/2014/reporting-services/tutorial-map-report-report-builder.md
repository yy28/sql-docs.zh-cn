...
2.  [将地图点层添加到显示存储位置](#PointLayer)  
  
3.  [添加显示路线的地图线条层](#LineLayer)  
  
4.  [添加 Bing 地图图块背景](#TileLayer)  
  
5.  [使层显示为透明](#Transparent)  
  
6.  [改变县颜色基于销售额](#Vary)  
  
    1.  [空间数据和分析数据之间建立关系](#Relationship)  
  
    2.  [指定的多边形的颜色规则](#ColorRules)  
  
    3.  [在色阶中的数据的格式设置为货币](#ColorScale)  
  
    4.  [创建一个新的图例](#NewLegend)  
  
    5.  [相关联的图例和颜色规则](#Associate)  
  
    6.  [从所在国家/地区不含数据的清除颜色](#NoData)  
  
7.  [添加自定义点](#CustomPoint)  
  
8.  [Center 地图视图](#CenterView)  
...
5.  **选择空间数据的源**页上，确认**地图库**选择。  
  
6.  在地图库窗格中，展开**状态，通过县**下**USA**，然后单击**纽约**。  
  
     此时，“地图预览”窗格将显示纽约的县地图。  
  
7.  单击“下一步” 。  
  
8.  上**选择空间数据和地图视图选项**页上，接受默认设置。 默认情况下，来自地图库的地图元素将自动嵌入到报表定义中。  
...
15. 单击地图以选择它，然后显示**图层窗格**。 **图层窗格**显示一个多边形层的层类型**嵌入**。 每个县都是该层上的一个嵌入地图元素。  
  
    > [!NOTE]  
    >  如果看不到**地图层**窗格中，它可能会显示当前视图之外。 请使用位于“设计”视图窗口底部的滚动条来更改视图。 或者，在**视图**选项卡上，清除**属性**或**报表数据**选项，以提供更多的设计图面区域。  
  
16. 右键单击地图标题，然后单击**标题属性**。  
  
17. 将使用的标题文本替换**的应用商店销售额**。  
...
11. 上**选择空间数据和地图视图选项**页上，确认**空间字段**是`SpatialLocation`且**层类型**是**点**. 接受本页上的其他默认值。  
  
     地图视图显示圆圈，这些圆圈标记了每个商店的位置。  
  
12. 单击“下一步” 。  
  
13. 指定一个地图类型，它显示随分析数据而改变的标记。 在选择地图可视化效果页上，单击**分析标记地图**，然后单击**下一步**。  
  
14. 上**选择分析数据集**页上，单击 DataSet1。 此数据集同时包含分析数据和空间数据，它将显示在新的点层上。  
  
15. 单击“下一步” 。  
  
16. 上**选择颜色主题和数据可视化**页上，清除**使用标记颜色实现数据可视化效果**选项，然后再选中选项**标记类型用于可视化数据**.  
  
17. 在**数据字段**，选择`[Sum(SellingArea)]`通过应用商店留出要显示的产品区域的大小变化标记类型。  
...
19. 添加图例标题。 右击图例标题，然后单击**图例标题属性**。  
  
20. 删除标题，并键入**显示区域 （正方形英尺）**。  
  
21. [!INCLUDE[clickOK](../includes/clickok-md.md)]  
  
22. 查看向导设置的默认值。 在**图层窗格**，右键单击的点层，，然后单击**标记类型规则**。  
  
     上**常规**选项卡上，在图例中出现的顺序列出的标记。 上**分发**选项卡上，子范围的数量为 5。 上**图例**选项卡上，图例文本设置为每个范围中显示的开始和结束值。  
...
5.  上**选择 SQL Server 空间数据源的连接**，选择 DataSource1，第一个过程中创建的数据源。  
...
14. 单击 **“完成”**。  
 ...
 地图显示与空间数据源类型新线条层**数据集**。 在本例中，空间数据来自数据源，但没有分析数据与此线条关联。  
...
4.  单击上 TileLayer1，向下的箭头，然后单击**磁贴属性**。  
  
5.  在**类型**，选择**空中**。 空中视图不包含文本。  
...
3.  单击上 PolygonLayer1，向下的箭头，然后单击**层数据**。 将打开“地图多边形层属性”对话框。  
...
6.  在**用于匹配的字段**，单击**添加**。 将添加一个新行。  
  
7.  在**来自空间数据集**，从下拉列表中，单击 COUNTYNAME。  
  
8.  在**分析数据集从**，从下拉列表中，单击 [县]。  
...
2.  单击上 PolygonLayer1，向下的箭头，然后单击**多边形颜色规则**。 将打开“地图颜色规则属性”对话框。 请注意，已选择“使用调色板实现数据的可视化效果”颜色规则选项。 此选项已由向导进行设置。  
...
2.  右键单击在视区外的映射，然后单击**添加图例**。 将在默认位置向地图添加新图例。  
  
3.  右键单击图例中，并依次**图例属性**。  
  
4.  在**位置选项**，单击指定想要显示相对于视区的图例的位置。 设计图面上的地图将更改以显示您的选择效果。  
  
5.  [!INCLUDE[clickOK](../includes/clickok-md.md)]  
  
6.  单击**标题**上该图例以选择图例标题。  
  
7.  单击**标题**以输入的文本插入模式。 替换**标题**通过**销售 （千为单位）**，然后单击文本以外。  
...
2.  单击上 PolygonLayer1，向下的箭头，然后单击**多边形颜色规则**。 将打开“地图颜色规则属性”对话框。  
  
3.  单击 **“图例”**。  
  
4.  在**色阶选项**，清除**在色阶中显示**。  
  
5.  在**图例选项**，从下拉列表中，选择 Legend2。 将显示图例文本选项。 默认情况下使用常规 [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] 格式字符串设置图例文本的格式。 N0 中的 0 指定没有小数位数。  
  
6.  在**图例文本**，使用以下格式指定具有任何小数位数的货币： `#FROMVALUE {C0} - #TOVALUE {C0}`  
...
5.  验证是否填充样式**纯色。** 。渐变样式和图案样式应用于所有颜色。  
  
6.  在**颜色**，单击向下箭头，，然后单击**Light 钢蓝色**。  
  
7.  [!INCLUDE[clickOK](../includes/clickok-md.md)]  
  
8.  预览报表。  
  
 不具有关联数据的县显示为蓝色。 具有关联的分析数据的县出现在**红色**通过**绿色**从你指定的颜色规则的颜色。  
  
##  <a name="CustomPoint"></a> 7.添加自定义点  
 若要表示尚未生成的新存储，指定一个点，并使用**图钉**标记类型。  
...
3.  单击上 PointLayer2，向下的箭头，然后单击**添加点**。  
...
7.  选择选项**重写此层的点选项**。 其他页将显示在对话框中。 您在此处设置的值优先于层或颜色规则的显示选项。  
...
##  <a name="CenterView"></a> Center 地图视图  
 更改地图视区中心和缩放级别。  
...
## <a name="next-steps"></a>后续步骤  
 ...
 有关详细信息，请参阅[地图&#40;报表生成器和 SSRS&#41; ](report-design/maps-report-builder-and-ssrs.md)和博客文章[制图调整的空间数据的 SQL Server Reporting Services](http://go.microsoft.com/fwlink/?LinkId=152771) blogs.msdn.com 上的。  
  
 有关更多的教程，请参阅[教程&#40;报表生成器&#41;](report-builder-tutorials.md)。  
