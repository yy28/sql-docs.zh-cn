...
6.  [根据销售改变县颜色](#Vary)  
    ...
    1.  [在空间数据与分析数据之间建立关系](#Relationship)  
  
    2.  [为多边形指定颜色规则](#ColorRules)  
  
    3.  [将色阶中的数据的格式设置为“货币”](#ColorScale)  
  
    4.  [创建新图例](#NewLegend)  
  
    5.  [将图例与颜色规则关联](#Associate)  
  
    6.  [清除没有数据的县的颜色](#NoData)  
...
6.  在 "地图库" 窗格中，展开 "**美国国内****的国家/地区**"，然后单击 "**纽约**"。  
...
15. 单击地图以选择它并显示 "**地图层" 窗格**。 "**地图层" 窗格**显示第一个层类型为 "**嵌入**" 的多边形层。 每个县都是该层上的一个嵌入地图元素。  
  
    > [!NOTE]  
    >  如果看不到 "**地图层**" 窗格，则它可能显示在当前视图之外。 请使用位于“设计”视图窗口底部的滚动条来更改视图。 或者，在 "**视图**" 选项卡中，清除 "**属性**" 或 "**报表数据**" 选项以提供更多的设计图面区域。  
  
16. 右键单击地图标题，然后单击 "**标题属性**"。  
  
17. 将标题文本替换为**Sales By Store**。  
...
11. 在 "**选择空间数据和地图视图选项**" 页上，确认 "**空间**" 字段`SpatialLocation`为，"**层类型**" 为 "**点**"。 接受本页上的其他默认值。  
...
17. 在 "**数据**" 字段`[Sum(SellingArea)]`中，选择此项可根据商店设置的区域的大小改变标记类型以显示产品。  
...
19. 添加图例标题。 右键单击图例标题，然后单击 "**图例标题属性**"。  
  
20. 删除标题，然后键入**显示区域（平方英尺）**。  
  
21. [!INCLUDE[clickOK](../includes/clickok-md.md)]  
  
22. 查看向导设置的默认值。 在 "**地图层" 窗格**中，右键单击点层，然后单击 "**标记类型规则**"。  
  
     在 "**常规**" 选项卡上，标记按它们在图例中的显示顺序列出。 在 "**分发**" 选项卡上，子范围数为5。 在 "**图例**" 选项卡上，图例文本设置为显示每个范围中的起始值和结束值。  
...
6.  在 "**要匹配的字段**" 中，单击 "**添加**"。 将添加一个新行。  
  
7.  在 "**来自空间数据集**" 的下拉列表中，单击 "COUNTYNAME"。  
  
8.  在 "**来自分析数据集**" 的下拉列表中，单击 "[县]"。  
...
2.  右键单击视区外的地图，然后单击 "**添加图例**"。 将在默认位置向地图添加新图例。  
  
3.  右键单击图例，然后单击 "**图例属性**"。  
  
4.  在 "**位置选项**" 中，单击指定图例相对于视区的显示位置的位置。 设计图面上的地图将更改以显示您的选择效果。  
  
5.  [!INCLUDE[clickOK](../includes/clickok-md.md)]  
  
6.  单击图例上的**标题**以选择图例标题。  
  
7.  再次单击 "**标题**" 以进入文本插入模式。 将**标题**替换为**销售额（千）**，然后在文本外部单击。  
...
5.  在 "**图例选项**" 的下拉列表中，选择 "Legend2"。 将显示图例文本选项。 默认情况下使用常规 [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] 格式字符串设置图例文本的格式。 N0 中的 0 指定没有小数位数。  
  
6.  在 "**图例文本**" 中，使用以下格式指定没有小数位数的货币：`#FROMVALUE {C0} - #TOVALUE {C0}`  
...
5.  验证填充样式是否为**纯色。** 。渐变样式和图案样式应用于所有颜色。  
  
6.  在 "**颜色**" 中，单击向下箭头，然后单击 "**浅钢蓝色**"。  
...
7.  选择 "**覆盖此层的点选项**" 选项。 其他页将显示在对话框中。 您在此处设置的值优先于层或颜色规则的显示选项。  
...
2.  右键单击刚添加的点，然后单击 "嵌入的**点属性**"。  
  
3.  单击 "**标签**"。  
  
4.  在 "**标签文本**" 中，键入**New Store**。  
...
1.  右键单击地图视区，然后单击 "**视区属性**"。  
  
2.  单击 "**中心和缩放**"。  
  
3.  验证是否已选择 "**设置视图中心和缩放级别**" 选项。  
...
## <a name="next-steps"></a>后续步骤  
 ...
 有关详细信息，请参阅[Maps &#40;报表生成器和 SSRS&#41;](report-design/maps-report-builder-and-ssrs.md)和博客条目为 blogs.msdn.com 上的[SQL Server Reporting Services 调整空间数据的 cartographic adjustment of](https://go.microsoft.com/fwlink/?LinkId=152771) 。  
