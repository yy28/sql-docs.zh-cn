...
1.  单击 "**开始**", [!INCLUDE[ssCurrentUI](../includes/sscurrentui-md.md)]指向 "程序", 指向 "**报表生成器**", 然后单击 "**报表生成器**"。  
  
     此时将打开 "**入门**" 对话框。 如果未显示, 请在 "**报表生成器**" 按钮中单击 "**新建**"。  
...
2.  在“选择多维数据集”对话框中，依次单击“Sales”和“确定”。  
    ...
    > [!TIP]  
    >  如果不想手动生成 MDX 查询，请单击 ![切换到设计模式](https://docs.microsoft.com/analysis-services/analysis-services/media/rsqdicon-designmode.gif "Switch to Design mode") 图标，将查询设计器切换到“查询”模式，将已完成的 MDX 粘贴到查询设计器，然后继续执行[创建数据集](#DSkip)中的步骤 6。  
...
2.  在“选择多维数据集”对话框中，依次单击“Sales”和“确定”。  
    ...
    > [!TIP]  
    >  如果不想手动生成 MDX 查询，请单击 ![切换到设计模式](https://docs.microsoft.com/analysis-services/analysis-services/media/rsqdicon-designmode.gif "Switch to Design mode") 图标，将查询设计器切换到“查询”模式，将已完成的 MDX 粘贴到查询设计器，然后继续执行[创建数据集](#MSkip)中的步骤 5。  
...
1.  在“选择布局”页的“选项”下，确认已选择“显示小计和总计”。  
     ...
     向导的“预览”窗格将显示包含四行的矩阵。  当您运行报表时，每行将按以下方式显示：第一行是列组, 第二行包含列标题, 第三行包含产品类别数据 (`[Sum(Net_ QTY)]`和`[Sum(Net_Sales)]`), 第四行包含总计。  
