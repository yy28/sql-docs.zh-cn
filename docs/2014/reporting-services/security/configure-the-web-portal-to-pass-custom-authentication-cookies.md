...
## <a name="modifying-the-rsreportserverconfig-file"></a>修改 RSReportServer.Config 文件  
 可以通过将 <`PassThroughCookies`> 元素添加到 rsreportserver.config 文件中的报表管理器配置设置，使报表管理器将其他 cookie 传输到 Report Server。 在单一登录身份验证解决方案中，传输其他 Cookie 十分有用，因为此类解决方案不仅需要报表服务器身份验证 Cookie，而且还需要第三方身份验证系统中的 Cookie。  
