...
ms.locfileid: "36014961"
---
# <a name="extended-protection-for-authentication-with-reporting-services"></a>Reporting Services 针对验证的扩展保护
  扩展保护是针对最新版本的 [!INCLUDE[msCoName](../../includes/msconame-md.md)] Windows 操作系统的一组增强功能。 扩展保护增强了应用程序可用来保护凭据和身份验证的方式。 该功能本身不直接提供针对特定的攻击，如凭据转发保护，但它提供一种基础结构应用程序如[!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)]强制执行身份验证扩展保护。  
...
## <a name="reporting-services-extended-protection-overview"></a>Reporting Services 扩展保护概述  
 [!INCLUDE[ssCurrent](../../../includes/sscurrent-md.md)] [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] 支持和实行已在操作系统中启用的扩展的保护。 如果操作系统不支持扩展保护或者尚未启用操作系统中的此功能， [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] 扩展保护功能将无法进行身份验证。 [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] 扩展的保护还需要 SSL 证书。 有关详细信息，请参阅[配置本机模式报表服务器上的 SSL 连接](configure-ssl-connections-on-a-native-mode-report-server.md)。  
  
> [!IMPORTANT]  
>  默认情况下，[!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)]不启用扩展保护。 可以通过修改 `rsreportserver.config` 配置文件或使用 WMI API 来更新此配置文件，以便启用此功能。 [!INCLUDE[ssCurrent](../../../includes/sscurrent-md.md)][!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] 不会提供一个用户界面来修改或查看扩展保护设置。 有关详细信息，请参阅本主题中的 [配置设置](#ConfigurationSettings) 部分。  
...
> -   SQL Server 用作[!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)]报表数据源不能具有启用的扩展的保护或要连接到报表数据源的报表服务器尝试将失败并且返回身份验证错误。  
>   
>  用于数据访问技术的文档应该具有与支持扩展保护有关的信息。  
  
### <a name="upgrade"></a>升级  
  
-   升级[!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)]服务器到[!INCLUDE[ssCurrent](../../../includes/sscurrent-md.md)]将使用默认值为配置设置添加`rsreportserver.config`文件。 如果设置已存在，[!INCLUDE[ssCurrent](../../../includes/sscurrent-md.md)]安装将保留在`rsreportserver.config`文件。  
  
-   配置设置添加到时`rsreportserver.config`配置文件的默认行为是[!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)]扩展的保护功能为关闭，你必须启用功能，如本主题中所述。 有关详细信息，请参阅本主题中的 [配置设置](#ConfigurationSettings) 部分。  
...
### <a name="what-reporting-services-extended-protection-does-not-cover"></a>Reporting Services 扩展保护不涵盖的内容  
 不支持以下功能区和方案[!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)]扩展的保护功能：  
  
-   作者[!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)]自定义安全扩展插件必须将支持扩展保护添加到其自定义安全扩展插件。  
  
-   第三方组件添加到使用或由[!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)]安装必须更新第三方供应商，以支持扩展的保护。 有关详细信息，请与第三方供应商联系。  
...
|应用场景|方案图|保护方式|  
|--------------|----------------------|-------------------|  
|直接 SSL 通信。<br /><br /> 报表服务器将实行“客户端到报表服务器”渠道绑定。|![RS_ExtendedProtection_DirectSSL](../media/rs-extendedprotection-directssl.gif "RS_ExtendedProtection_DirectSSL")<br /><br /> 1) 客户端应用程序<br /><br /> 2) 报表服务器|服务绑定不是必需的，因为将使用 SSL 渠道来进行渠道绑定。<br /><br /> 设置`RSWindowsExtendedProtectionLevel`到`Allow`或`Require`。<br /><br /> 设置`RSWindowsExtendedProtectionScenario`到`Direct`。|  
|直接 HTTP 通信。 报表服务器将实行“客户端到报表服务器”服务绑定。|![RS_ExtendedProtection_Direct](../media/rs-extendedprotection-direct.gif "RS_ExtendedProtection_Direct")<br /><br /> 1) 客户端应用程序<br /><br /> 2) 报表服务器|没有 SSL 渠道，因此可能不实行渠道绑定。<br /><br /> 然而，可以使服务绑定生效，否则，由于没有渠道绑定，它将成为不完善的防御机制，服务绑定本身只防止基本威胁。<br /><br /> 设置`RSWindowsExtendedProtectionLevel`到`Allow`或`Require`。<br /><br /> 设置`RSWindowsExtendedProtectionScenario`到`Any`。|  
...
|应用场景|方案图|保护方式|  
|--------------|----------------------|-------------------|  
|HTTP 通信。 报表服务器将实行“客户端到报表服务器”服务绑定。|![RS_ExtendedProtection_Indirect](../media/rs-extendedprotection-indirect.gif "RS_ExtendedProtection_Indirect")<br /><br /> 1) 客户端应用程序<br /><br /> 2) 报表服务器<br /><br /> 3) 代理|没有 SSL 渠道，因此可能不实行渠道绑定。<br /><br /> 设置`RSWindowsExtendedProtectionLevel`到`Allow`或`Require`。<br /><br /> 设置`RSWindowsExtendedProtectionScenario`到`Any`。<br /><br /> 请注意，必须配置报表服务器为知道要确保服务绑定强制实施的正确的代理服务器的名称。|  
|HTTP 通信。<br /><br /> 报表服务器将实行“客户端到代理”渠道绑定和“客户端到报表服务器”服务绑定。|![RS_ExtendedProtection_Indirect_SSL](../media/rs-extendedprotection-indirect-ssl.gif "RS_ExtendedProtection_Indirect_SSL")<br /><br /> 1) 客户端应用程序<br /><br /> 2) 报表服务器<br /><br /> 3) 代理|到代理的 SSL 渠道是可用的，因此，可以与代理之间实行渠道绑定。<br /><br /> 还可以实行服务绑定。<br /><br /> 报表服务器必须了解代理名称，报表服务器管理员应使用主机标头为代理创建一个 URL 预留，或在 Windows 注册表项 `BackConnectionHostNames` 中配置代理名称。<br /><br /> `RSWindowsExtendedProtectionLevel` 到`Allow`或`Require`。<br /><br /> 设置`RSWindowsExtendedProtectionScenario`到`Proxy`。|  
|与安全代理之间的间接 HTTPS 通信。 报表服务器将实行“客户端到代理”渠道绑定和“客户端到报表服务器”服务绑定。|![RS_ExtendedProtection_IndirectSSLandHTTPS](../media/rs-extendedprotection-indirectsslandhttps.gif "RS_ExtendedProtection_IndirectSSLandHTTPS")<br /><br /> 1) 客户端应用程序<br /><br /> 2) 报表服务器<br /><br /> 3) 代理|到代理的 SSL 渠道是可用的，因此，可以与代理之间实行渠道绑定。<br /><br /> 还可以实行服务绑定。<br /><br /> 报表服务器必须了解代理名称，报表服务器管理员应使用主机标头为代理创建一个 URL 预留，或在 Windows 注册表项 `BackConnectionHostNames` 中配置代理名称。<br /><br /> `RSWindowsExtendedProtectionLevel` 到`Allow`或`Require`。<br /><br /> 设置`RSWindowsExtendedProtectionScenario`到`Proxy`。|  
...
|应用场景|方案图|保护方式|  
|--------------|----------------------|-------------------|  
|间接 HTTP 通信。<br /><br /> 网关将实行“客户端到网关”渠道绑定。 此时具有“网关到报表服务器”服务绑定。|![RS_ExtendedProtection_Indirect_SSL](../media/rs-extendedprotection-indirect-ssl.gif "RS_ExtendedProtection_Indirect_SSL")<br /><br /> 1) 客户端应用程序<br /><br /> 2) 报表服务器<br /><br /> 3) 网关设备|从客户端到报表服务器的渠道绑定是不可能的，因为网关模拟上下文，并因此创建一个新的 NTLM 标记。<br /><br /> 此时，从网关到报表服务器没有 SSL，因此，无法实行渠道绑定。<br /><br /> 可以实行服务绑定。<br /><br /> 设置`RSWindowsExtendedProtectionLevel`到`Allow`或`Require`。<br /><br /> 设置`RSWindowsExtendedProtectionScenario`到`Any`。<br /><br /> 应由管理员配置网关设备以实行渠道绑定。|  
|与安全网关之间的间接 HTTPS 通信。 网关将实行“客户端到网关”渠道绑定，报表服务器将实行“网关到报表服务器”渠道绑定。|![RS_ExtendedProtection_IndirectSSLandHTTPS](../media/rs-extendedprotection-indirectsslandhttps.gif "RS_ExtendedProtection_IndirectSSLandHTTPS")<br /><br /> 1) 客户端应用程序<br /><br /> 2) 报表服务器<br /><br /> 3) 网关设备|从客户端到报表服务器的渠道绑定是不可能的，因为网关模拟上下文，并因此创建一个新的 NTLM 标记。<br /><br /> 从网关到报表服务器的 SSL 意味着可以实行渠道绑定。<br /><br /> 不需要服务绑定。<br /><br /> 设置`RSWindowsExtendedProtectionLevel`到`Allow`或`Require`。<br /><br /> 设置`RSWindowsExtendedProtectionScenario`到`Direct`。<br /><br /> 应由管理员配置网关设备以实行渠道绑定。|  
...
|应用场景|方案图|保护方式|  
|--------------|----------------------|-------------------|  
|从客户端间接和直接访问报表服务器服务，在客户端到代理或客户端到报表服务器连接中均没有 SSL。|1) 客户端应用程序<br /><br /> 2) 报表服务器<br /><br /> 3) 代理<br /><br /> 4) 客户端应用程序|可以实行从客户端到报表服务器的服务绑定。<br /><br /> 报表服务器必须了解代理名称，报表服务器管理员应使用主机标头为代理创建一个 URL 预留，或在 Windows 注册表项 `BackConnectionHostNames` 中配置代理名称。<br /><br /> 设置`RSWindowsExtendedProtectionLevel`到`Allow`或`Require`。<br /><br /> 设置`RSWindowsExtendedProtectionScenario`到`Any`。|  
|从客户端间接和直接访问报表服务器，其中，客户端与代理或报表服务器建立 SSL 连接。|![RS_ExtendedProtection_CombinationSSL](../media/rs-extendedprotection-combinationssl.gif "RS_ExtendedProtection_CombinationSSL")<br /><br /> 1) 客户端应用程序<br /><br /> 2) 报表服务器<br /><br /> 3) 代理<br /><br /> 4) 客户端应用程序|可以使用渠道绑定<br /><br /> 代理名称必须已知的报表服务器和报表服务器管理员应为代理服务器，使用主机标头创建 URL 保留项或在 Windows 注册表项中配置的代理名称`BackConnectionHostNames`。<br /><br /> 设置`RSWindowsExtendedProtectionLevel`到`Allow`或`Require`。<br /><br /> 设置`RSWindowsExtendedProtectionScenario`到`Proxy`。|  
  
## <a name="configuring-reporting-rervices-extended-protection"></a>配置 Reporting Rervices 扩展保护  
 `rsreportserver.config`文件包含控制的行为的配置值[!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)]扩展保护。  
  
 有关详细信息使用和编辑`rsreportserver.config`文件，请参阅[RSReportServer Configuration File](../report-server/rsreportserver-config-configuration-file.md)。 还可以使用 WMI API 更改和检查扩展保护设置。 有关详细信息，请参阅[SetExtendedProtectionSettings 方法&#40;WMI MSReportServer_ConfigurationSetting&#41;](../wmi-provider-library-reference/configurationsetting-method-setextendedprotectionsettings.md)。  
