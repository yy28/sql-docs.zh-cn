...
# <a name="reporting-services-report-server-native-mode"></a>Reporting Services 报表服务器（本机模式）
 ...
 您可以使用[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)]或报表管理器来管理[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]报表。 使用 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 配置管理器可在本机模式下管理报表服务器。  
...
> [!NOTE]  
>  内容管理不同于报表服务器管理。 有关如何管理运行 Report Server 的环境的详细信息，请参阅[管理（Reporting Services）](reporting-services-report-server-native-mode.md)。  
