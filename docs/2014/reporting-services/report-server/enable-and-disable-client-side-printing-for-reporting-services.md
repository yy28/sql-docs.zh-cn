...
ms.locfileid: "36125777"
---
# <a name="enable-and-disable-client-side-printing-for-reporting-services"></a>启用和禁用 Reporting Services 的客户端打印
  [!INCLUDE[msCoName](../../includes/msconame-md.md)] ActiveX 控件， **RSClientPrint**，提供在浏览器中查看报表的客户端打印。 该控件显示一个自定义打印对话框，它支持其他打印对话框常见的功能。 这些功能包括打印预览、指定特定页和范围的页面选择、页边距和打印方向等功能。 虽然默认情况下将启用客户端打印功能，但是您也可以将其禁用，以禁止使用该功能。  
  
> [!NOTE]  
>  下载 ActiveX 控件需要管理员权限。  
  
## <a name="downloading-the-activex-control"></a>下载 ActiveX 控件  
 对于希望使用打印功能的每个用户来说，都必须下载并安装提供客户端打印功能的 ActiveX 控件。 第一次用户单击**打印机**报表工具栏，Microsoft ActiveX 控件下载到计算机上的图标。 下载控件后，请**打印**对话框中会显示在用户单击**打印机**图标。  
  
 根据浏览器设置的不同，系统可能会提示用户安装控件，阻止用户安装控件，或者在后台透明地安装控件。  
  
 有关[!INCLUDE[msCoName](../../includes/msconame-md.md)]Internet Explorer 中，设置可影响 ActiveX 控件下载和安装通过指定**ActiveX 控件和插件**中的节点**安全设置**页上的Web 内容区域。 以下设置基于 Web 区域安全首选项，确定用户是否可以下载和运行打印控件：  
...
-   运行 ActiveX 控件和插件。  
 ...
 想要使用用户**RSClientPrint**要执行客户端打印必须启用以下：  
  
-   **下载已签名的 ActiveX 控件**和**脚本的 ActiveX 控件标记对于脚本化**供安装。  
  
-   **运行 ActiveX 控件和插件**现行的打印操作。  
...
## <a name="enabling-and-disabling-client-side-printing"></a>启用和禁用客户端打印功能  
 报表服务器管理员可以通过设置报表服务器系统属性禁用打印功能选项**EnableClientPrinting**到`false`。 这将对该服务器管理的所有报表禁用客户端打印功能。 默认情况下， **EnableClientPrinting**设置为`true`。 您可以通过下列方式禁用客户端打印功能：  
...
-   编写脚本或代码来设置报表服务器系统属性**EnableClientPrinting**到 `false.`  
