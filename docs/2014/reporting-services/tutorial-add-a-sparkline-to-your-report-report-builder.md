...
# <a name="tutorial-add-a-sparkline-to-your-report-report-builder"></a>教程：向报表添加迷你图（报表生成器）
 ...
 视频[如何： 创建表 （报表生成器视频） 中的迷你图](http://technet.microsoft.com/bi/ff871942.aspx)演示如何创建带有迷你图的类似的报表。  
  
##  <a name="BackToTop"></a> 你将学习的内容  
 在本教程中，您将了解如何执行下列操作：  
  
 1. [创建带有一个表的报表](#CreateTable)  
  
 2. [表或矩阵向导中创建查询](#Query)  
  
 3. [向表中添加迷你图](#Sparkline)  
  
 4. [垂直和水平对齐迷你图](#AlignSparklines)  
  
### <a name="other-optional-steps"></a>其他可选步骤  
 5. [为货币的格式数据](#FormatCurrency)  
  
 6. [以日期形式的格式数据](#FormatDates)  
...
8.  在“连接字符串”中，键入以下文本：  
     ...
     **数据源 =\<服务器名 >**  
     ...
     表达式\<servername >，对于示例 Report001，指定在其安装的 SQL Server 数据库引擎实例的计算机。 因为报表数据不是从 SQL Server 数据库提取的，所以不需要包括数据库的名称。 指定服务器上的默认数据库用于对查询进行分析。  
...
4.  在新的列中，右击 [产品] 行中，依次指向**插入**功能区选项卡上，并依次**迷你图**。  
  
5.  请确保在第一个迷你图**列**行已选择，然后单击**确定**。  
  
6.  单击要在“图表数据”窗格中显示的迷你图。  
  
7.  单击加号 （+） 在值框中，登录，然后单击**销售**。  
  
     “Sales”字段中的值现在是迷你图的值。  
  
8.  单击加号 （+） 在类别组框中，登录，然后单击**销售日期**。  
...
2.  在单击在第二个行 （列标题行） 的单元格**销售日期**列然后拖动它以选择包含的所有单元格`[Sum(Sales)]`。  
...
3.  指向列控点之间的行，使光标变为双箭头。 拖动列，调整到所需大小。 例如，展开的列**产品**，以便在一行上显示的产品名称。  
