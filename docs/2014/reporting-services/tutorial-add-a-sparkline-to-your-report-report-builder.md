...
### <a name="other-optional-steps"></a>其他可选步骤  
 5. [将数据格式设置为货币](#FormatCurrency)  
  
 6. [将数据格式设置为日期格式](#FormatDates)  
...
8.  在“连接字符串”中，键入以下文本****：  
     ...
     **数据源 =\<servername>**  
     ...
     Expression \<servername> （例如 report001)）指定安装了 SQL Server 数据库引擎实例的计算机。 因为报表数据不是从 SQL Server 数据库提取的，所以不需要包括数据库的名称。 指定服务器上的默认数据库用于对查询进行分析。  
...
4.  在新列中，右键单击 [Product] 行，指向 "**插入**" 功能区选项卡，然后单击 "**迷你图**"。  
  
5.  请确保选择**列**行中的第一个迷你图，然后单击 **"确定"**。  
  
6.  单击要在“图表数据”窗格中显示的迷你图。  
  
7.  单击 "值" 框中的加号（+），然后单击 "**销售**"。  
  
     “Sales”字段中的值现在是迷你图的值****。  
  
8.  单击 "类别组" 框中的加号（+），然后单击 " **SalesDate**"。  
  
9. 单击 **“运行”** 以预览报表。  
  
     请注意，在表的每一行中都有迷你图，但这些迷你图不正确。 迷你图中的各条形不互连。 在第二行数据中只有四个条形，因此，这些条比第一行中的条宽，因为第一行中有六个条形。 您不能比较每天每个产品的值。 它们之间需要相互对齐。  
  
     还要注意的是，对于每一行，该行的最高的条形是行高。 这也会产生误解，因为每行的最大值不相等：预算 Movie Maker 的最大值为 $10400，但超薄数字的最大值为 $26576-大于两倍。 并且，这两行的最大的条形大约为相同高度。 此外，它还需要与其他迷你图相一致。  
...
2.  单击第二行（位于列标题行下） **SalesDate**列中的单元格，并拖动以选择包含`[Sum(Sales)]`的所有单元格。  
...
3.  指向列控点之间的行，使光标变为双箭头。 拖动列，调整到所需大小。 例如，展开**product**列，以便产品名称显示在一行上。  
