...
-   对 [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[ssCurrent](../includes/sscurrent-md.md)] 报表生成器的访问权限。 可以使用报表生成器的独立版本或 ClickOnce 版本（可从报表管理器或 SharePoint 站点获取），来运行报表生成器。 ClickOnce 版本只有第一步（如何打开报表生成器）不同。  
...
-   与 [!INCLUDE[ssRSCurrent](../includes/ssrscurrent-md.md)] Report Server 集成的 [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[ssRSCurrent](../includes/ssrscurrent-md.md)] Report Server 或 SharePoint 站点的 URL。 您必须拥有保存和查看报表、共享数据源、共享数据集、报表部件和模型的权限。 默认情况下，Report Server 的 URL 为 http://\<servername >/reportserver。 默认情况下，SharePoint 站点的 URL 是 http://\<sitename > 或 http://\<server >/site。  
  
-   [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[ssCurrent](../includes/sscurrent-md.md)] 实例的名称和凭据，足以对任何数据库进行只读访问。 各教程中的数据集查询使用文字数据，但必须由 [!INCLUDE[ssCurrent](../includes/sscurrent-md.md)] 实例处理每个查询以返回报表数据集所必需的元数据。 例如，以下连接字符串仅指定一个服务器： `data source=<servername>`。 您必须对默认数据库具有读取权限，该权限是由授予您对服务器的访问权限的系统管理员分配给您的。 您还可以指定数据库，如以下连接字符串中所示：`data source=<servername>;initial catalog=<database>`。  
  
-   对于包括地图的教程，报表服务器必须配置为支持将 Bing 地图作为背景。 有关详细信息，请参阅 msdn.microsoft.com 上 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)][联机丛书](https://go.microsoft.com/fwlink/?LinkId=154888)中 [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] 文档中[的规划地图报表支持](plan-for-map-report-support.md)。  
  
-   教程[：创建钻取报表和主报表&#40;报表生成器&#41;](tutorial-creating-drillthrough-and-main-reports-report-builder.md)教程使用 Contoso 商业智能演示数据集。 此数据集由 ContosoDW 数据仓库和 Contoso_Retail 联机分析处理 (OLAP) 数据库组成。 您将在此教程中创建的报表从 Contoso Sales 多维数据集检索报表数据。 Contoso_Retail OLAP 数据库可以从 [Microsoft 下载中心](https://www.microsoft.com/download/details.aspx?id=18279)下载。 您只需要下载文件 ContosoBIdemoABF.exe。 它包含 OLAP 数据库。  
