...
## <a name="example"></a>示例  
 对于数据源类型**Microsoft SQL Server**，下面的查询将从[!INCLUDE[ssSampleDBobject](../includes/sssampledbobject-md.md)]数据库`Person`表中检索姓氏列表。  
  
```  
SELECT LastName FROM Person.Person;  
```  
  
 对于数据源类型**Microsoft SQL Server**，下面的查询将为标识号[!INCLUDE[ssSampleDBobject](../includes/sssampledbobject-md.md)]为 1 `uspGetEmployeeManagers`的员工运行存储过程：  
