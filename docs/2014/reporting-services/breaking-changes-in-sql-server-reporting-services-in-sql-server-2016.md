...
-   [SQL Server 2012 Reporting Services 中的重大更改](#bkmk_rc0)  
...
### <a name="sharepoint-mode-server-references-require-the-sharepoint-site"></a>SharePoint 模式服务器引用要求 SharePoint 网站  
 ...
 现在要求您在 URL 路径中包括 SharePoint 网站。 例如，如果你的站点名称为 "`videos`" 并且使用了 "`sites`" 前缀，则 URL 将如下所示：  
  
 `http://<Server Name>/sites/videos/_vti_bin/ReportServer`  
  
### <a name="changes-to-sharepoint-mode-command-line-installation"></a>SharePoint 模式命令行安装的更改  
 输入设置 **/RSINSTALLMODE** 仅用于本机模式安装，不用于 SharePoint 模式安装。 例如，不支持以下[!INCLUDE[ssCurrent](../includes/sscurrent-md.md)]内容： **/RSINSTALLMODE = "DefaultSharePointMode"**。 使用 **/RSSHPINSTALLMODE="DefaultSharePointMode"** 来代替该输入设置。  
  
 以下语句是完整安装命令和参数集的一个示例： **setup/ACTION = install/FEATURES = SQL，RS/InstanceName = Denali_INST1 ..../RSSHPINSTALLMODE = "DefaultSharePointMode"**  
  
 有关命令行安装的详细信息，请参阅[命令提示符安装 Reporting Services SharePoint 模式和本机模式](install-windows/install-reporting-services-at-the-command-prompt.md)。  
