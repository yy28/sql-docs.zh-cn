...
1.  [从表或矩阵向导创建的表报表和数据集](#Table)  
  
2.  [组织数据，选择布局，并设置样式表或矩阵向导](#CompleteWizard)  
  
3.  [使用背景色显示 KPI](#BackgroundColors)  
  
4.  [通过使用仪表显示 KPI](#Gauge)  
  
5.  [使用指示器来显示 KPI](#Indicator)  
...
##  <a name="Table"></a> 1.使用表向导或矩阵向导创建表报表和数据集  
 从**入门**对话框中，选择共享的数据源、 创建嵌入数据集，并显示表中的数据。  
...
1.  单击 **“开始”**，依次指向 **“程序”**、 **Microsoft SQL Server 2012 Report Builder**，再单击 **“报表生成器”**。  
    ...
    > [!NOTE]  
    >  如果**入门**未显示对话框中，执行 （从报表生成器按钮，单击**新建**。  
...
#### <a name="to-organize-data-into-groups-choose-a-layout-and-a-style"></a>若要将数据组织到组中，选择布局和样式  
...
1.  在表中，右键单击两个单元格下从`[Sum(Sales)]`单元格 （小计行显示子类别的销售），然后单击**文本框属性**。  
  
2.  在**填充**，单击**fx**按钮旁边**填充颜色**选项并输入中的以下表达式**设置表达式： BackgroundColor**字段：  
...
2.  在表中，右键单击列处理程序在前面的过程中更改的单元格中，对于指向**插入列**，然后单击**右**。 此时将向表添加一个新列。  
  
3.  类型**KPI**在列标题中。  
  
4.  上**插入**选项卡上，在**数据区域**组中，单击**仪表**，然后单击表外位于设计图面。 此时将显示 **“选择仪表类型”** 对话框。  
  
5.  单击**线性**。 第一种线性仪表类型**水平**，已选中。  
...
10. 在**指针类型**，选择**栏**。 这会将指针从标记改为条形，从而在将仪表添加到表中时更加清晰可见。  
  
11. 单击**指针填充**。 在**辅助颜色**选取**黄色**。 渐变填充模式将从白色改为黄色。  
...
16. （可选）添加处理溢出的最大刻度格，以使超出最大刻度的任何值始终指向最大刻度格：  
    ...
    3.  在**缩放 Pin**类别中，展开**MaximumPin**节点。  
  
    4.  设置**启用**属性`True`。 随即将在刻度的最大值之后显示一个刻度格。  
...
10. 在值下拉列表中选择 **[sum （sales)]**，但不要更改任何其他选项。  
...
## <a name="next-steps"></a>后续步骤  
 您已成功完成“向报表添加 KPI”教程的学习。 有关详细信息，请参阅仪表 （报表生成器）[指示器&#40;报表生成器和 SSRS&#41;](report-design/indicators-report-builder-and-ssrs.md)。  
