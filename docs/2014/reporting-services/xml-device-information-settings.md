...
|`OmitNamespace`|指示是否从 XML 中忽略默认命名空间。<br /><br /> 如果为 true，则 XML 未指定默认命名空间。<br /><br /> 如果为 false，则 XML 使用报表的 DataSchema 属性值指定默认命名空间。 该 DataSchema 属性默认为报表名称。<br /><br /> 默认值为 `false`。|  
