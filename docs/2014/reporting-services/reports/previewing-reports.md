...
## <a name="preview-mode"></a>预览模式  
 你可以通过单击预览报表设计器中的报表**预览**。 这将在本地运行报表，使用的报表处理功能和呈现功能与报表服务器所提供的功能相同。 所显示的报表是一个交互式的图像；您可以选择参数、单击链接、查看文档结构图以及展开和折叠报表的隐藏区域。 您还可以使用所安装的任意一种呈现格式将报表导出。  
...
-   通过单击**启动**按钮上[!INCLUDE[vsprvs](../../includes/vsprvs-md.md)]标准工具栏。  
  
-   按 F5。  
  
 如果你使用的项目配置，生成报表但不部署它，报表中指定`StartItem`当前配置属性，将在单独的预览窗口中打开。 预览窗口显示报表的方式与预览模式相同，并具有相同功能。  
  
> [!NOTE]  
>  在调试报表之前，必须设置开始项。 若要设置开始项，请在解决方案资源管理器，右键单击报表项目，单击**属性**，然后在`StartItem`，选择要显示的报表的名称。  
