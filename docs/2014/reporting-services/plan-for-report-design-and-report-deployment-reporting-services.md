...
# <a name="plan-for-report-design-and-report-deployment-reporting-services-2014"></a>报表设计和报表部署的规划 (Reporting Services 2014)
  [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)][!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)]提供多种创作和部署报表的方法。 使用本主题帮助计划协同工作的报表创作环境和报表服务器。 本主题概述了 [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] 组件对报表定义的支持。 报表定义是用报表定义语言 (RDL) 或客户端报表定义语言 (RDLC) 编写的 XML 文件。 每个报表定义都符合位于该文件开头的特定架构版本的要求。  
...
-   **报表设计器：** 从中[!INCLUDE[ss_dtbi](../includes/ss-dtbi-md.md)]的报表设计器部署报表。  
  
-   **报表生成器：** 将报表保存到报表生成器中的 Report Server。  
  
-   **报表管理器：** 从报表管理器将报表上传到本机模式 Report Server。  
  
-   **SharePoint：** 将报表上传到使用 SharePoint 模式 Report Server 配置的 SharePoint 站点。  
  
-   **以编程方式：** 使用 Report Server 的 SOAP API 接口以编程方式发布报表。 有关详细信息，请参阅 [Report Server Web Service](report-server-web-service/report-server-web-service.md)。  
...
|[!INCLUDE[ssVersion2005](../includes/ssversion2005-md.md)]|2005 RDL<br /><br /> 2000 RDL|  
 ...
 如果您将报表定义上载到报表服务器或者升级包含现有报表的报表服务器，报表服务器将保留原格式的报表定义。 **第一次使用**时，Report Server 将 Report Server 数据库中的报表升级到为后续视图保留的二进制格式。 报表定义 (.rdl) 本身不升级。  
...
-   [SQL Server Data Tools &#40;SSRS 中的部署和版本支持&#41;](tools/deployment-and-version-support-in-sql-server-data-tools-ssrs.md)  
  
-   [Microsoft SQL Server Data Tools-适用于 Visual Studio 2012 的商业智能](https://www.microsoft.com/download/details.aspx?id=36843)。  
  
##  <a name="bkmk_reportviewer"></a>ReportViewer 控件  
 
  [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ReportViewer 控件可在本地预览模式或远程模式下显示 .rdlc 报表，该控件还可以显示在 [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] 报表服务器上托管的 .rdl 文件。 下表提供 ReportViewer 控件为进行本地处理 (.rdlc) 支持的 RDL 版本的列表。 
  [报表服务器和 RDL 架构支持](#bkmk_report_server_rdl_schema_support)一节中总结了服务器端 RDL 支持。  
...
-   [添加和配置 ReportViewer 控件](https://msdn.microsoft.com/library/ms252104.aspx)  
