...
|[DatabaseQueryTimeout](configurationsetting-property-databasequerytimeout.md)|指定命令失败或超时之前必须经过的秒数。Report Server 针对 SQL Server 目录（而不是报表的数据源）处理进程。|  
