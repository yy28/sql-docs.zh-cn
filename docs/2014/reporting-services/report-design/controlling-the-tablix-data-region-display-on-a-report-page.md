...
-   **有条件地隐藏数据**。 若要有条件地控制是显示还是隐藏数据区域或数据区域的各个部分，可以将 Hidden 属性设置为`True`或将其设置为表达式。 表达式可以包括对报表参数的引用。 还可以指定切换项，以使用户可以决定是否显示详细信息数据。 有关详细信息，请参阅[深化操作（报表生成器和 SSRS）](drilldown-action-report-builder-and-ssrs.md)。  
