...
-   **要作为排序依据：** 在一个表列中显示的字段？ 还是依据未显示的某一字段？  
  
-   **可以按何种上下文：** 例如，可以对具有行组; 关联的行与列组; 关联的列详细信息行;父组; 内的子组或父和子组组合在一起。  
  
-   **若要添加到排序按钮的文本框中：** 在列标题或组行标题中？  
  
-   **是否要对多个数据区域同步排序：** 您可以设计一个报表，以便在用户切换排序顺序时, 具有同一祖先的其他数据区域也进行排序。  
