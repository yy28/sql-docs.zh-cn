...
-   **地图图例**显示一个指南，以帮助解释不同于地图层上的地图元素显示的分析数据。 一个地图可以具有多个图例。 对于每一地图层，您可以指定要使用的图例。 一个图例可以为多个地图层提供指导。  
  
-   **色阶**显示帮助解释地图上的颜色的指南。 一个地图有一个色阶。 多个层可以为色阶提供数据。  
  
-   **距离刻度**显示有助于解释地图的刻度的指南。 一个地图有一个距离刻度。 当前地图视区缩放值确定距离刻度。  
...
1.  在设计视图中，右键单击图例并打开 " _ \<报表项->_**属性**" 页。  
...
2.  右键单击具有所需数据的层，然后单击_ \<"地图元素\>类型_**颜色规则**"。  
...
##  <a name="ColorRanges"></a>根据颜色范围改变地图元素颜色  
...
##  <a name="CustomColors"></a>根据自定义颜色改变地图元素颜色  
...
3.  选择 "使用\<规则类型\>实现**数据的可视化效果**" 选项。 若要使用分布选项，必须根据与层关联的分析数据，在 **“分布”** 页上创建范围。  
  
4.  单击 **“分布”**。  
  
5.  请选择以下分布类型之一：  
  
    -   **EqualInterval**。 指定将数据划分为相等范围间隔的范围。  
  
    -   **EqualDistribution**。 指定用于划分数据以使每个范围具有相同项数的范围。  
...
2.  右键单击具有所需数据的层，然后单击_ \<"地图元素类型\>_**规则**"。  
...
4.  **图例文本**显示指定哪些数据显示在图例中的关键字。 使用地图关键字和自定义格式可帮助控制图例文本的格式。 例如，#FROMVALUE {C2} 指定一个具有两个小数位的货币格式。 有关详细信息，请参阅 [按规则和分析数据更改多边形、线条和点的显示方式（报表生成器和 SSRS）](vary-polygon-line-and-point-display-by-rules-and-analytical-data.md)。  
