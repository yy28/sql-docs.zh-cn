...
# <a name="parameters-collection-references-report-builder-and-ssrs"></a>Parameters 集合引用（报表生成器和 SSRS）
 ...
 在运行时比较参数值和数据集字段值时，所比较的两个项的数据类型必须相同。 报表参数可以是下列类型之一：Boolean、DateTime、Integer、Float 或 Text，它们表示基础数据类型 String。 如有必要，您可能必须转换参数值的数据类型以与数据集值相匹配。 有关详细信息，请参阅[Data Types in Expressions&#40;报表生成器和 SSRS&#41;](expressions-report-builder-and-ssrs.md)。  
...
|示例|Description|  
|-------------|-----------------|  
|`=Parameters!` \<ParameterName> `.IsMultiValue`|返回`False`。<br /><br /> 检查参数是否是多值。 如果`True`，该参数是多值并且它是对象的集合。 如果`False`，该参数是单值，是单个对象。|  
...
|`=Join(Parameters!` \<MultivalueParameterName> `.Value,", ")`|返回`String`包含的多值参数中的值以逗号分隔列表。 可以在第二个参数中使用任何分隔符进行联接。|  
