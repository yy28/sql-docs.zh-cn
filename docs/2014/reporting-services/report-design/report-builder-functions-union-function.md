...
## <a name="return"></a>返回  
 基于表达式类型返回`SqlGeometry`或`SqlGeography`的空间对象。 有关`SqlGeometry`和`SqlGeography`空间数据类型的详细信息，请参阅[空间数据类型概述](../../relational-databases/spatial/spatial-data-types-overview.md)。  
