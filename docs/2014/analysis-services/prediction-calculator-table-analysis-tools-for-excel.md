...
3.  在中**预测计算器**对话框中，对于目标，选择你想要预测，例如采购行为的列。  
  
4.  指定目标值。 如果该值是数值，则使用选项**范围内**，然后键入所需范围的最小值和最大值。 如果离散值，请选择**完全**选项，然后从下拉列表中选择值。  
  
5.  单击**选择要用于分析的列**。  
  
6.  在中**高级列选择**对话框中，选择包含有用的信息的列。 删除与分析无关的所有列。 单击“确定” 。  
  
     为了避免使结果产生偏差，还应删除包含重复信息的列。 例如，如果具有包含数值数据的 Income 列以及包含 High、Medium 和 Low 标签的 Income Group 列，则不应在同一模型中包含这两列， 而是应为每个列创建单独的模型。  
  
7.  在中**输出选项**部分中，选择**操作计算器**创建分析和记分卡中的 Excel 工作簿。 选择**打印机就绪计算器**以创建分析并生成一个报表，可以打印并用于手动评分。  
 ...
 **预测计算器**工具使用 Microsoft 逻辑回归算法，可以使用离散值以及离散化和连续数值数据。  
  
## <a name="understanding-the-scoring-reports"></a>了解记分报表  
 如果选择这两个输出选项，则预测计算器会在当前工作簿中创建以下三个新工作表：  
  
-   一个**预测报表**包含结果的分析，交互式表和关系图，帮助您试验交互和利润情况。  
  
-   交互式**预测计算器**，可帮助你创建评分。  
  
-   一个**可打印计算器**与用于评分的指令和系数。  
  
-   本节介绍每个报表中的信息以及如何使用各种报表选项。  
  
### <a name="prediction-report-with-graphs"></a>带有图形的预测报表  
 第一个预测报表的标题为**预测计算器报表\<目标状态 > 的\<目标属性 >**。 它包含从分析得出的因素表，以及有助于评估特定分析的财务影响的工具。  
...
#### <a name="chart-for-viewing-maximum-profit"></a>用于查看最大利润的图表  
 ...
 例如，如果在左上角的表的单元格**最大化利润的建议阈值**显示值 500，在右侧图表将 500 显示为最高数据点的线形图上。 值 500 意味着若要最大化利润，应使用来自挖掘模型的前 500 条建议（按概率排序）。  
...
### <a name="interactive-prediction-calculator"></a>交互式预测计算器  
 预测计算器工具创建的第二个工作表的标题为**的预测计算器\<目标状态 > 的\<目标属性 >**。 这是可用于计算各个分数的交互式工作表。 由于此工作表使用存储在模型中的模式和统计信息，因此可以试用各值并查看它们如何影响预测分数。 此报表也有两部分：一部分是交互式的，另一部分是作为参考提供的。  
  
#### <a name="first-table"></a>第一个表  
 可以选择或键入新值中的**值**表以查看更改的值对分数的影响的列。  
...
|对“是”的预计||FALSE|  
 ...
 输入中的新值，显示在单元格，为否，预测的分数将更改为 TRUE，并且**相对影响**分数的各种特性也会更新。  
...
### <a name="printable-prediction-calculator"></a>可打印的预测计算器  
 预测计算器工具创建的第三个工作表的标题为**PrintablePrediction 计算器\<目标状态 > 的\<目标属性 >**。 此记分卡可用于打印，以便能够在没有计算机的情况下手动计算分数。  
  
##### <a name="to-print-and-use-the-scoring-report-generated-by-the-prediction-calculator"></a>打印和使用预测计算器生成的记分报表  
  
1.  单击标题为选项卡**的可打印的预测计算器\<属性 >**。  
  
2.  在 Excel 文件菜单上选择**打印预览**。  
...
5.  只为每个属性选择一个值。 值选择、 放置一个复选标记的框中，和写入相应的数字**分数**列。  
  
6.  填写尽可能多的属性以确保准确性。  
  
7.  计算总和的每个属性的分数和该数输入**总**行。  
  
8.  使用打印工作表上的条件将分数转换为预测结果后立即**总**行。  
  
## <a name="related-tools"></a>相关工具  
 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 提供的 Microsoft 逻辑回归算法可用于这种类型的分析。 如果你已熟悉逻辑回归算法，可以使用轻松创建逻辑回归模型**高级**Excel 数据挖掘客户端的选项。 有关详细信息，请参阅[高级建模&#40;数据挖掘的 Excel 外接程序&#41;](advanced-modeling-data-mining-add-ins-for-excel.md)。 有关选项和逻辑回归模型的参数的详细信息，请参阅主题"Microsoft 逻辑回归算法"中[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]联机丛书。  
