...
# <a name="create-or-edit-named-query-dialog-box-analysis-services---multidimensional-data"></a>“创建或编辑命名查询”对话框（Analysis Services - 多维数据）
  可以使用 ** 中的“创建/编辑命名查询”对话框，在数据源视图设计器中创建或编辑命名查询。**[!INCLUDE[ssBIDevStudioFull](../includes/ssbidevstudiofull-md.md)]**** 可以将命名查询视为表，基于该表可以建立其他 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 对象。 可以通过执行以下操作之一显示“创建/编辑命名查询”对话框：****  
