---
title: 购物篮分析 (for Excel 表 AnalysisTools) |Microsoft 文档
ms.custom: ''
ms.date: 03/06/2017
 ...
 **购物篮分析**工具可帮助你找到`associations`数据中。 关联可以指出哪些项经常被同时购买。 在数据挖掘，此技术是众所周知的方法称为*市场篮分析*，可用来分析中非常大的数据集的客户的购买行为。 商家可以使用该信息向客户推荐相关产品，并通过将这些产品置于网页、目录或货架中的临近位置来进行促销。  
  
 若要使用购物篮分析，要分析的项必须通过交易 ID 进行关联。 例如，如果要分析通过某个网站接收的所有订单，则每个订单都会有一个订单 ID 或交易 ID，它与一个或多个购买项相关联。  
  
 当向导完成后分析数据时，它将创建两个新工作表，**购物篮项组**和**购物篮规则**。  
  
 **购物篮项组**工作表包含经常同时出现在事务中的项的列表。 这些通用组称为*项集*。 工作表中还包含统计信息，例如*支持*和*提升*，来帮助你了解项集的重要性。 如果提供了价格信息，该工作表还将创建所有相关项的金额之和，以指示这些交易的总金额。  
  
 可以在报表中对各列进行筛选和排序。 例如，你可能想要查看仅与 2 个或多个产品，这些项集或排序的项集**购物篮平均值**。  
  
 **购物篮规则**工作表使用派生自分析统计信息来创建有关如何项相关联的规则。 例如，规则可能是，如果客户购买产品 A，它们是很有可能会购买产品 b。规则可以用于创建建议。 每个规则都有支持的统计信息，可帮助您评估该规则的潜在强度，以便仅在该规则超出了某个概率阈值时才进行推荐。  
...
3.  在**购物篮分析**对话框中，选择包含事务 ID、 的列，然后选择包含的项或你想要分析的产品的列。  
  
4.  可以选择添加包含产品金额的列。  
  
5.  单击**高级**以打开**高级参数设置**对话框。 值增加**最小支持**以减少的分组为项集的产品数量。 增加**最小规则概率**若要筛选出很常见项集。  
  
### <a name="requirements"></a>要求  
 若要使用**购物篮分析**工具，你的数据必须存储在 Excel 表，并且必须包含以下列：  
...
## <a name="specifying-minimum-support-and-rule-probability"></a>指定最低支持和规则概率  
 随着数据集的增长，可能的项分组数和规则数将会过于庞大。 但是，您可以控制由该工具输出的结果数，从而只将重点放在最有价值的项集和规则上。 在中设置这些选项**购物篮高级参数对话框**。  
  
### <a name="minimum-support"></a>最低支持  
 *最小支持*意味着必须包含项集视为重要的特定项集的事务数。 例如，除非某个项集在至少 10 个不同的交易中被购买过，否则您不会对它感兴趣。 有两种方法来控制项集重要性的阈值和都使用**最小支持**参数。  
  
 **为绝对值：** 输入一个数字，表示包含目标项的事务数。 例如，如果输入 10，则至少出现在 10 个购物篮中的任何项集都包括在结果中。  
  
 **百分比：** 输入一个数字，表示整个集合的项集的百分比。 例如，如果指定 10，将对所有项集计数，目标项集必须在项集总数中至少占 10%。 如果数据集非常大，则使用百分比（而不是计数）有助于将重点放在最重要的项分组上。  
 ...
 **购物篮分析**工具将创建两个互补的报告。 第一个报表，标题为**重要组的项标识在分析期间**，提供所有找到的项集的列表。 可以使用 Microsoft Excel 中新增的表工具对这些数据进行排序、筛选和浏览。  
  
 第二个报表，标题为**购物篮规则**，告知哪种类型的推断可以基于对进行中的第一个报表列出的项集。 项集列表对于浏览和理解数据来说更为有用，而规则列表对于预测和推荐来说更为有用。  
  
### <a name="shopping-basket-item-groups-report"></a>购物篮项组报表  
 此报表包含在数据集中找到的所有可能的项组合的列表。 例如，如果事务数据包含订单，每个订单**购物篮分析**工具计算多少次各个项已排序，然后计算该项与其他项目的所有组合。  
...
|报表中的列|它告诉你|  
|----------------------|-----------------------|  
|现有项|列出进行推荐时所需的项。<br /><br /> 在数据挖掘，这些项可认为是上*左侧*的关联规则。|  
|预测项|列出要推荐的项。<br /><br /> 在数据挖掘，这些项可认为是上*右侧*的关联规则。|  
|概率|显示此规则为正确的概率。|  
...
## <a name="related-tools"></a>相关工具  
 Excel 数据挖掘客户端是一个独立的外接程序，它提供了更高级的数据挖掘功能，还包含一个用于执行关联分析的向导。 有关详细信息，请参阅[关联向导&#40;for Excel 数据挖掘客户端&#41;](associate-wizard-data-mining-client-for-excel.md)。  
