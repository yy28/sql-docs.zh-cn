 ...
 **配置凭据选项 2，"使用以下 Windows 用户凭据连接"**  
 ...
 当前无法确定哪些目标应用程序是活动的，哪些不活动；也无法通过特定目标应用程序回溯到使用它的数据刷新计划。 通常，您应该不对这些目标应用程序执行任何操作，因为删除它们可能会破坏现有的数据刷新计划。 删除仍在使用的目标应用程序将导致数据刷新失败，同时在工作簿的数据刷新历史记录页中显示消息“未找到目标应用程序”。  
