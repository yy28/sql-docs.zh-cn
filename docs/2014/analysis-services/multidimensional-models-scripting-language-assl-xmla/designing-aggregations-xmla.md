...
-   [步骤](https://docs.microsoft.com/bi-reference/xmla/xml-elements-properties/steps-element-xmla)属性确定命令在将控制权返回`DesignAggregations`给客户端应用程序之前应执行的迭代次数。  
  
-   [Time](https://docs.microsoft.com/bi-reference/xmla/xml-elements-properties/time-element-xmla)属性确定命令在将控制权返回`DesignAggregations`给客户端应用程序之前应执行的毫秒数。  
...
-   [存储](https://docs.microsoft.com/bi-reference/xmla/xml-elements-properties/storage-element-xmla)属性用于确定`DesignAggregations`命令所使用的磁盘存储量（以字节为单位）。 如果要以迭代方式设计聚合，只需在第一条命令中发送此属性。  
