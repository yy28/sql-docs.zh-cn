---
title: 挖掘模型视图 （数据挖掘模型设计器） |Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
ms.prod: sql-server-2014
ms.reviewer: ''
ms.technology: analysis-services
ms.topic: conceptual
f1_keywords:
- sql12.dm.miningmodeleditor.models.f1
ms.assetid: f7bfc77c-e0a4-4bd0-9358-ee1d5562a7f3
author: minewiskan
ms.author: owend
manager: craigg
ms.openlocfilehash: b698a6c216d4179de414f236fb7d41d6366e470d
ms.sourcegitcommit: f40fa47619512a9a9c3e3258fda3242c76c008e6
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 05/23/2019
ms.locfileid: "66077649"
---
# <a name="mining-models-view-data-mining-model-designer"></a>“挖掘模型”视图（数据挖掘模型设计器）
  可以使用 **“挖掘模型”** 视图创建或修挖掘结构包含的挖掘模型。  
  
 **“挖掘模型”** 视图具有以下工具和对话框：  
  
-   [算法参数对话框&#40;挖掘模型视图&#41;](algorithm-parameters-dialog-box-mining-models-view.md)  
  
## <a name="see-also"></a>请参阅  
 [挖掘结构视图&#40;数据挖掘模型设计器&#41;](mining-structure-view-data-mining-model-designer.md)   
 [挖掘模型查看器（数据挖掘模型设计器）](mining-model-viewers-data-mining-model-designer.md)   
 [挖掘准确性图表设计器&#40;数据挖掘&#41;](mining-accuracy-chart-designer-data-mining.md)   
 [预测查询生成器&#40;数据挖掘&#41;](prediction-query-builder-data-mining.md)  
  
  
