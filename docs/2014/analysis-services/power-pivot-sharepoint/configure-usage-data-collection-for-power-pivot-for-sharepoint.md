...
5.  您还可以更新日志文件的位置。 有关详细信息，请参阅后续部分。  
...
##  <a name="qrh"></a>出于报告目的定义快速、中等和慢的查询响应类别  
...
##  <a name="openconfig"></a>打开 "PowerPivot 服务应用程序" 页以访问配置设置  
...
##  <a name="defaultconfig"></a>PowerPivot 使用情况数据收集的默认配置  
...
|设置|默认值|类型|有效范围|  
|-------------|-------------------|----------|-----------------|  
|**Analysis Services 使用情况事件**（连接、加载、卸载、请求）|\<已启用>|Boolean|这些值可以启用或禁用。|  
