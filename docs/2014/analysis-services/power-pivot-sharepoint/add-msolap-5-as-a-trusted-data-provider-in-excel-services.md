...
# <a name="add-msolap5-as-a-trusted-data-provider-in-excel-services"></a>将 MSOLAP.5 添加为 Excel Services 中的受信任数据访问接口
 ...
 处理 PowerPivot 数据请求的每个物理服务器（如 PowerPivot for SharePoint 服务器或 Excel Services 服务器）必须在计算机上安装了 OLE DB 访问接口。 PowerPivot for SharePoint 安装始终包含 OLE DB 访问接口，但是如果 Excel Services 运行在未安装 PowerPivot for SharePoint 的计算机上，必须手动安装该访问接口。 有关详细信息，请参阅 [在 SharePoint 服务器上安装 Analysis Services OLE DB 提供程序](../../sql-server/install/install-the-analysis-services-ole-db-provider-on-sharepoint-servers.md)。  
