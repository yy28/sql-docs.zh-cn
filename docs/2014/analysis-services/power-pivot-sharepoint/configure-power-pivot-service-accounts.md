...
##  <a name="bkmk_passwordssas"></a>更新 SQL Server Analysis Services （PowerPivot）实例的过期密码  
...
##  <a name="bkmk_passwordapp"></a>更新 PowerPivot 服务应用程序的过期密码  
...
##  <a name="bkmk_newacct"></a>更改运行每个服务所用的帐户  
...
|要求|说明|  
|-----------------|-----------------|  
|设置要求|PowerPivot 系统服务是服务器场中的共享资源，在您创建服务应用程序时变得可用。 在创建服务应用程序时，必须指定服务应用程序池。 可以通过两种方式指定应用程序池：使用 PowerPivot 配置工具或通过 PowerShell 命令。<br /><br /> 您可能已配置了应用程序池标识以便基于唯一帐户运行。 不过，如果您不想这样做，请考虑立即将其更改为在其他帐户下运行。|  
