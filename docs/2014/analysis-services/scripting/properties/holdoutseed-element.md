...
ms.locfileid: "36013906"
---
# <a name="holdoutseed-element"></a>HoldoutSeed 元素
  指定包含的测试集的可重复维持分区的种子[MiningStructure](../objects/miningstructure-element-assl.md)元素。 此种子可确保模型内容在处理过程中保持不变。 如果未指定或设置为 0，[!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)]创建挖掘结构名称使用哈希算法的种子。  
...
## <a name="remarks"></a>Remarks  
 ...
 此外，当你创建挖掘结构的副本使用`EXPORT`语句，[!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)]将保留新的挖掘结构的名称，但将自动生成新的 id。 因此，两个挖掘结构可以共享相同的名称但具有不同的 ID。 任何两个名称相同的挖掘结构都将具有相同的种子。 但是，由于数据的分区还依赖于源数据，因此每个结构中分区的实际内容可能不同。  
