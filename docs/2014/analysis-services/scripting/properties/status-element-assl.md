...
ms.locfileid: "48101927"
---
# <a name="status-element-assl"></a>Status 元素 (ASSL)
  包含返回的状态指示器的多维表达式 (MDX) 表达式[Kpi](../objects/kpi-element-assl.md)元素。  
...
## <a name="remarks"></a>备注  
 `Status` 元素包含一个 MDX 表达式。  
  
 父级对应的元素`Status`在 Analysis Management Objects (AMO) 对象模型是<xref:Microsoft.AnalysisServices.Kpi>。  
