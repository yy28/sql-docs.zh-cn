...
## <a name="drillthrough-to-structure-columns"></a>钻取到结构列  
 在[!INCLUDE[ssKatmai](../../../includes/sskatmai-md.md)]，已将一个新的权限元素添加到[添加元素&#40;ASSL&#41; ](../collections/miningstructurepermissions-element-assl.md)集合。 如果你添加`AllowDrillthrough`同时向权限[添加](../collections/miningstructurepermissions-element-assl.md)和[MiningModelPermission](miningmodelpermission-element-assl.md)集合，钻取到结构，在这种方式启用从挖掘模型具有的角色的成员`AllowDrillthrough`对模型的权限可以查询数据挖掘模型，并将返回未包含在模型中的结构列。  
