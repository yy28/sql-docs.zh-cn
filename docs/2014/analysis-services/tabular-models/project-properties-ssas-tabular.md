...
-   [Project 属性](#bkmk_proj_properties)  
...
|**Database**|**“模型”**|指定在部署时将实例化的模型对象所处的 Analysis Services 数据库的名称。 该名称将在数据连接或 .rsds 数据连接文件中指定。 建议该名称应反映将使用该模型执行的分析类型，例如 AdventureWorksSalesModel。<br /><br /> ** \* \* 重要 \* 提示 \* **若要防止已部署模型的名称重复，应更改**数据库**属性名称设置以反映模型的用途。 当用户连接到作为数据源的模型时，他们将看到此名称。|  
|**多维数据集名称**|**“模型”**|指定报告客户端数据连接中显示的数据库多维数据集名称。|  
|**Version**|**11.0**|项目将部署到的 Analysis Services 实例的版本。|  
