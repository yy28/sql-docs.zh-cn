...
2.  底部附近**表**窗格中，右键单击`Date`，然后单击**新命名的计算**。  
...
4.  单击**确定**，然后展开`Date`中**表**窗格。  
     ...
     `SimpleDate`命名的计算的日期表中的列列表中显示一个图标，指示它与命名的计算。  
...
7.  滚动到右侧，以查看“浏览 Date 表”视图中的最后一列。  
     ...
     请注意，`SimpleDate`正确列显示在数据源视图中，而无需修改原始数据源连接从来自基础数据源的多个列的数据。  
...
1.  打开 [!INCLUDE[ssBIDevStudioFull](../includes/ssbidevstudiofull-md.md)] 中的“日期”维度的“维度设计器”。 若要执行此操作，请双击`Date`中的维度**维度**节点**解决方案资源管理器**。  
...
1.  在**维度结构**选项卡上的维度设计器中为`Date`维度中，拖动**日历年**属性从**属性**到窗格**层次结构**窗格。  
  
2.  拖动**日历半期**属性从**属性**到窗格**\<新级别 >** 中的单元格**层次结构**窗格中，在**日历年**级别。  
  
3.  拖动**日历季度**属性从**属性**到窗格**\<新级别 >** 中的单元格**层次结构**窗格中，在**日历半期**级别。  
  
4.  拖动**英语月份名称**属性从**属性**到窗格**\<新级别 >** 中的单元格**层次结构**窗格中，在**日历季度**级别。  
  
5.  拖动**日期密钥**属性从**属性**到窗格**\<新级别 >** 中的单元格**层次结构**窗格下方**英语月份名称**级别。  
  
6.  在**层次结构**窗格中，右键单击的标题栏**层次结构**层次结构中，单击**重命名**，然后键入`Calendar Date`。  
  
7.  通过右键单击上下文菜单中，在`Calendar Date`层次结构中，重命名**英语月份名称**级别`Calendar Month`，然后将重命名**日期密钥**级别`Date`。  
...
1.  在**维度设计器**为`Date`维度中，单击**属性关系**选项卡。  
...
2.  在**表**窗格中，右键单击`Date`，然后单击**新命名的计算**。  
  
3.  在**创建命名计算**对话框中，键入`MonthName`中**列名**框中，然后键入或复制并粘贴中的以下语句**表达式**框：  
...
5.  在**表**窗格中，右键单击`Date`，然后单击**新命名的计算**。  
  
6.  在**创建命名计算**对话框中，键入`CalendarQuarterDesc`中**列名**框中，然后键入或复制并粘贴以下 SQL 脚本中的**表达式**框：  
...
8.  在**表**窗格中，右键单击`Date`，然后单击**新命名的计算**。  
  
9. 在**创建命名计算**对话框中，键入`CalendarSemesterDesc`中**列名**框中，然后键入或复制并粘贴以下 SQL 脚本中的**表达式**框：  
...
8.  在**名称列**对话框中，在**源列**列表中，选择`MonthName`，然后单击**确定**。  
...
6.  在**名称列**对话框中，在**源列**列表中，选择`CalendarQuarterDesc`，然后单击**确定**。  
...
6.  在**名称列**对话框中，在**源列**列表中，选择`CalendarSemesterDesc`，然后单击**确定**。  
...
2.  已收到后**成功完成部署**消息，单击**浏览器**选项卡**维度设计器**为`Date`维度，并然后单击设计器工具栏上的重新连接按钮。  
...
1.  打开**维度结构**选项卡上的维度设计器中为`Date`维度，，然后选择**日历半期**中**属性**窗格。  
...
9. 在 [!INCLUDE[ssBIDevStudio](../includes/ssbidevstudio-md.md)] 的“生成”菜单上，单击“部署 Analysis Services 教程”。 已成功完成部署后，单击**浏览器**维度设计器中的选项卡`Date`维度。  
