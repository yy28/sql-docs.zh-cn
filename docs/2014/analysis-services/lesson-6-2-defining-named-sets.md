...
3.  在**名称**框中，更改名为设置为的新名称`[Core Products]`。  
  
     在“脚本组织程序”窗格中，注意用于将命名集与脚本命令或计算成员区分开来的唯一图标。  
  
4.  上**元数据**选项卡中**计算工具**窗格中，展开**产品**，展开**类别**，展开`Members`，，然后展开**所有产品**。  
...
2.  在**名称**框中，将名为设置为此名称更改`[Large Resellers]`。  
...
7.  上**元数据**选项卡中**计算工具**窗格中，展开**员工的**在分销商维度中，展开`Members`，然后展开**所有分销商**。  
  
     注意，此属性层次结构的成员没有分组。  
  
8.  打开“分销商”维度设计器，然后在“属性”窗格中单击“雇员数目”。  
  
9. 在属性窗口中，更改`DiscretizationMethod`属性**自动**，然后将更改`DiscretizationBucketCount`属性`5`。 有关详细信息，请参阅[对属性成员分组（离散化）](multidimensional-models/attribute-properties-group-attribute-members.md)。  
...
12. 上**元数据**选项卡中**计算工具**窗格中，展开**员工的**中**分销商**维度中，展开`Members`，然后展开**所有分销商**。  
  
     请注意，此属性层次结构的成员现在包含在编号为 0 到 4 的五个组中。 若要查看组的编号，请将指针暂停在组上以查看 InfoTip。 对于范围 `2 -17`，InfoTip 应包含 `[Reseller].[Number of Employees].&[0]`。  
  
     此属性层次结构的成员进行分组，因为 DiscretizationBucketCount 属性设置为`5`和 DiscretizationMethod 属性设置为**自动**。  
...
13. 在“表达式”框中，在 Exists 集表达式中的 Members 函数之后和右括号之前添加逗号，再将 **83 - 100** 从“元数据”窗格拖放到逗号之后。  
     ...
     ![计算表达式窗格中的 [大型经销商]](../../2014/tutorials/media/l6-named-set-02.gif "[大型经销商] 的计算表达式窗格")  
...
7.  在“元数据”窗格中，展开“分销商”，将“大型分销商”添加到筛选器区域。  
     ...
     ![包含两个名为的筛选器窗格中设置](../../2014/tutorials/media/l6-named-set-05.gif "包含两个名为的筛选器窗格中设置")  
