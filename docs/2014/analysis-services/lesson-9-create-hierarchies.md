...
4.  在`Product`表中，单击 " **Product Category Name** " 列，然后将其拖`Category`到层次结构中，然后在该`Category`名称的顶部释放。  
...
8.  通过使用 "单击并拖动" 或使用上下文菜单中的 "**添加到层次结构**" 命令，添加 "**型号名称**" 和 "**产品名称**" 列（按顺序），并将其放在 "**产品子类别名称**" 列下。 分别重命名`Model`这些`Product`列和。  
