...
13. 在“成员”中，添加以下用户和组帐户：  
    ...
    2.  添加 PowerPivot 服务应用程序的应用程序池标识，以便它可以在数据刷新计划运行时检索凭据。 若要查看应用程序池标识，请前往 "**管理服务应用**程序"，通过单击名称旁边的空白区域（这将选择行）选择 PowerPivot 服务应用程序，然后单击 "**属性**"。 此页上显示的安全帐户是要作为目标应用程序的成员来添加的帐户。  
...
2.  验证是否在 "数据刷新配置" 页中启用了 "**使用 Secure Store Service （SSS）中保存的凭据登录到数据源**" 选项，然后输入目标应用程序的名称。  
...
13. 在“成员”中，添加以下用户和组帐户：  
    ...
    2.  添加将使用该目标应用程序以访问其存储的凭据的每个 PowerPivot 服务应用程序的应用程序池标识。 若要查看标识，请前往 "**管理服务应用程序**"，通过单击名称旁边的空白区域（这将选择行）选择 PowerPivot 服务应用程序，然后单击 "**属性**"。 此页上显示的安全帐户是您想要作为目标应用程序的成员来添加的帐户。  
