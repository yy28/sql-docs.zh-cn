---
title: Excel 表分析工具 |Microsoft 文档
ms.custom: ''
ms.date: 12/29/2017
...
ms.locfileid: "36026210"
---
# <a name="table-analysis-tools-for-excel"></a>Excel 表分析工具
  中的数据挖掘工具**分析**工具栏是最简单的方法，若要开始使用数据挖掘。 每个工具都会对您的数据的分布和类型自动进行分析，并且设置参数以确保结果有效。 您就无需选择某一算法或配置复杂的参数。  
...
## <a name="requirements-for-the-table-analysis-tools-for-excel"></a>Excel 表分析工具的要求  
 若要使用 Excel 表分析工具，必须首先与 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 实例建立连接。 通过此连接可以访问用于分析数据的 Microsoft 数据挖掘算法。 如果您无权访问实例，则建议您让管理员设置一个您可用于试验数据挖掘的实例。 有关详细信息，请参阅[连接到源数据&#40;for Excel 数据挖掘客户端&#41;](connect-to-source-data-data-mining-client-for-excel.md)。  
  
 若要使用表分析工具来处理数据，必须将想要使用的数据范围转换为 Excel 表。  
  
 如果看不到**分析**功能区上，尝试单击数据表内第一次。 在选择某一数据表之前将不会激活该菜单。  
