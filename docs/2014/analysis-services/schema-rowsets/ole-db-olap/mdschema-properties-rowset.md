---
title: MDSCHEMA_PROPERTIES 行集 |Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
|`PROPERTY_TYPE`|`DBTYPE_I2`||用于指定属性类型的位图：<br /><br /> -   `MDPROP_MEMBER` (`1`) 标识的成员属性。 此属性可用于 SELECT 语句的 DIMENSION PROPERTIES 子句。<br />-   `MDPROP_CELL` (`2`) 标识单元格的属性。 此属性可用于在 SELECT 语句结尾处出现的 CELL PROPERTIES 子句。<br />-   `MDPROP_SYSTEM` (`4`) 标识内部属性。<br />-   `MDPROP_BLOB` (`8`) 标识包含二进制大型对象 (blob) 的属性。|  
...
|`PROPERTY_ORIGIN`|`DBTYPE_UI2`||标识属性应用的层次结构的类型：<br /><br /> -   `MD_USER_DEFINED` (`1`) 指示相应属性位于用户定义层次结构<br />-   `MD_SYSTEM_ENABLED` (`2`) 指示相应属性位于属性层次结构<br />-   `MD_SYSTEM_DISABLED` (`4`) 指示相应属性位于未启用属性层次结构。|  
...
## <a name="restriction-columns"></a>限制列  
 `MDSCHEMA_PROPERTIES`行集可以限制下表中列出的列。  
...
|`PROPERTY_VISIBILITY`|`DBTYPE_UI2`|（可选）使用以下有效值之一位图：<br /><br /> -1 的可见性<br />-2 不可见<br /><br /> 默认限制的值为 1。|  
