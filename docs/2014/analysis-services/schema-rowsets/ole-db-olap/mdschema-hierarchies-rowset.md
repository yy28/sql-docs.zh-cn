---
title: MDSCHEMA_HIERARCHIES 行集 |Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
|`HIERARCHY_CAPTION`|`DBTYPE_WSTR`||标签或标题的层次结构相关联。 主要用于显示目的。 如果不存在标题，`HIERARCHY_NAME`返回。 如果维度不包含层次结构，或只有一个层次结构，则此列将包含相应维度的名称。|  
...
|`HIERARCHY_ORIGIN`|`DBTYPE_UI2`||确定层次结构源的位掩码：<br /><br /> -   `MD_USER_DEFINED` 标识用户定义层次结构，并且值为`0x0000001`。<br />-   `MD_SYSTEM_ENABLED` 标识属性层次结构，且具有值为`0x0000002`。<br />-   `MD_SYSTEM_INTERNAL` 与任何属性层次结构中，标识属性，且具有值为**0x0000004**。<br /><br /> 父级/子级属性层次结构为 `MD_USER_DEFINED` 和 `MD_SYSTEM_ENABLED`。|  
|`HIERARCHY_DISPLAY_FOLDER`|`DBTYPE_WSTR`||在用户界面中显示层次结构时需使用的路径。 文件夹名称将由分号 (;) 分隔。 嵌套的文件夹由反斜杠 (\\)。|  
...
## <a name="restriction-columns"></a>限制列  
 `MDSCHEMA_HIERARCHIES`行集可以限制下表中列出的列。  
...
|`HIERARCHY_VISIBILITY`|`DBTYPE_UI2`|（可选）使用以下有效值之一位图：<br /><br /> -1 的可见性<br />-2 不可见<br /><br /> 默认限制的值为 1。|  
