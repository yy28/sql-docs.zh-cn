...
|`ORDINAL_POSITION`|`DBTYPE_UI4`||列序号。 列从 1 开始编号。 如果该列没有固定的序号值，则为 `NULL`。|  
...
|`COLUMN_FLAGS`|`DBTYPE_UI4`||-A 位掩码，用于描述列的特征。 `DBCOLUMNFLAGS` 枚举的类型指定了位掩码中的位。 此列不能包含 `NULL` 值。 有效值包括：<br />-   **DBCOLUMNFLAGS_ISNULLABLE** (`0x20`)<br />-   **DBCOLUMNFLAGS_MAYBENULL** (`0x40`)<br />-   **DBCOLUMNFLAGS_ISLONG** (`0x80`)|  
...
|`CHARACTER_MAXIMUM_LENGTH`|`DBTYPE_UI4`||列中值的最大可能长度。 对于字符列、二进制值列或位列，可以是下列值之一：<br /><br /> 的中的列最大长度的字符，字节或 bits，分别，如果定义长度。 例如，SQL 表中的 `CHAR(5)` 列的最大长度为 5。<br />的数据最大长度分别在键入字符、 字节或位为单位，如果列不具有定义的长度。<br />零 (0)，如果列和数据类型都不具有定义的最大长度。<br />-   `NULL` 对于所有其他类型的列。|  
...
|`CHARACTER_SET_SCHEMA`|`DBTYPE_WSTR`||定义在其中的字符集的架构的非限定的名称。 如果访问接口不支持架构或不同的字符集，则为 `NULL`。|  
...
|`IS_RELATED_TO_KEY`|`DBTYPE_BOOL`||指示此列是否与键关联的布尔值。<br /><br /> 如果此列与键关联，则为 `VARIANT_TRUE`；否则为 `VARIANT_FALSE`。 如果键是一个列中，`RELATED_ATTRIBUTE`字段 （可选） 可以包含的列名称。|  
