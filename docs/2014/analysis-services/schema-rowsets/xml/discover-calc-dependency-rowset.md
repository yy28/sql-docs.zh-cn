---
title: DISCOVER_CALC_DEPENDENCY 行集 |Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
...
|`REFERENCED_OBJECT_TYPE`|`DBTYPE_WSTR`||返回具有对引用对象的依赖关系的对象的类型。 返回的对象可以是以下类型之一：<br /><br /> -   `CALC_COLUMN`： 计算的列<br />-   `COLUMN`： 对数据列<br />-   `MEASURE`： 度量值<br />-   `RELATIONSHIP`： 关系<br />-   `KPI`: KPI （关键绩效指标）|  
 ...
 **使用 WHERE 子句进行筛选**  
 ...
 下一个查询说明如何使用 WHERE 子句添加限制。 以下各列可以用作查询筛选器的 WHERE 子句中： `Database_Name`， `Object_Type`，和`Query`。  
...
> [!NOTE]  
>  尽管查询看起来使用了双引号，但实际上使用的是两个单引号。 有一对单引号会包含 Evaluate\<表名称 >，和表名称周围使用单引号会需要成双进行转义。 只有包含空格的表名称才需要在表名称周围加上单引号。  
