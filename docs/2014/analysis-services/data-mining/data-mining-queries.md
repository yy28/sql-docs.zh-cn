...
##  <a name="bkmk_Interfaces"></a> 查询工具和接口  
 ...
 在了解数据挖掘模型以及生成查询的方式之后，您还可以使用数据挖掘扩展插件 (DMX) 直接编写查询。 DMX 是一种类似于 Transact-SQL 的查询语言，可以从多种不同的客户端使用它。 DMX 是用来创建自定义预测和复杂查询的理想工具。 有关 DMX 的介绍，请参阅[创建和查询数据挖掘模型使用 DMX:教程&#40;Analysis Services-数据挖掘&#41;](../../tutorials/create-query-data-mining-models-dmx-tutorials.md)。  
