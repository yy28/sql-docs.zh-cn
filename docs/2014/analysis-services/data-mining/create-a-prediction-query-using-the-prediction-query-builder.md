 ...
 **设计**模式允许您通过选择输入数据，将数据映射到模型，然后将预测函数添加到使用网格生成的语句中，生成预测查询。 设计网格包含以下生成块：  
 ...
 **查询**模式提供文本编辑器，使您可以直接访问数据挖掘扩展插件（DMX）语言，并提供输入数据和模型列的视图。 选择 **“查询”** 模式时，一个基本文本编辑器将替换用于定义查询的网格。 您可使用此编辑器复制并保存编写的查询，或从剪贴板将其粘贴到现有 DMX 查询中，然后运行它们。  
