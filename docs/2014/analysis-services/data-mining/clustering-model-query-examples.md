...
##  <a name="finding-information-about-the-model"></a><a name="bkmk_top2"></a> 查找有关模型的信息  
...
###  <a name="sample-query-3-returning-a-cluster-or-list-of-clusters"></a><a name="bkmk_Query3"></a> 示例查询 3：返回分类或分类列表  
...
###  <a name="sample-query-4-returning-attributes-for-a-cluster"></a><a name="bkmk_Query4"></a> 示例查询 4：返回分类的属性  
...
###  <a name="sample-query-5-return-a-cluster-profile-using-system-stored-procedures"></a><a name="bkmk_Query5"></a> 示例查询 5：使用系统存储过程返回分类配置文件  
...
###  <a name="sample-query-6-find-discriminating-factors-for-a-cluster"></a><a name="bkmk_Query6"></a> 示例查询 6：查找分类的对比因子  
...
###  <a name="sample-query-7-returning-cases-that-belong-to-a-cluster"></a><a name="bkmk_Query7"></a> 示例查询 7：返回属于分类的事例  
...
###  <a name="sample-query-8-predicting-outcomes-from-a-clustering-model"></a><a name="bkmk_Query8"></a> 示例查询 8：从聚类分析模型中预测结果  
...
###  <a name="sample-query-9-determining-cluster-membership"></a><a name="bkmk_Query9"></a> 示例查询 9：确定分类成员身份  
...
###  <a name="sample-query-10-returning-all-possible-clusters-with-probability-and-distance"></a><a name="bkmk_Query10"></a> 示例查询 10：返回具有概率和距离的所有可能分类  
 在上一个示例中，概率分数不是非常高。 若要确定是否存在更好的分类，可以将 [PredictHistogram (DMX)](/sql/dmx/predicthistogram-dmx) 函数与 [分类 (DMX)](/sql/dmx/cluster-dmx) 函数一起使用，以返回包括所有可能的分类以及新事例属于每个分类的概率的嵌套表。 FLATTENED 关键字用于将分层行集改为平面表，以便于查看。  
