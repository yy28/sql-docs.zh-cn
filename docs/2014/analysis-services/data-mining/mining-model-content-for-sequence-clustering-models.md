 ...
 **模型根**基数值等于分类数加1。 有关详细信息，请参阅 [基数](#bkmk_cardinality)。  
  
 **群集节点**基数始终为1，因为每个群集都有一个子节点，该节点包含群集中的序列列表。  
 ...
 **群集节点**模型中分类的调整后的概率。 调整后的概率总和不等于 1，因为在顺序分析和聚类分析中使用的聚类分析方法允许在多个分类中使用部分成员身份。  
 ...
 **群集节点**对群集的原始支持，表示向此群集提供事例的定型事例数。  
  
 **序列节点**始终为0。  
  
 **过渡节点**群集中表示特定转换的事例的百分比。 它可以为 0，也可以为正值。 其计算方法是用群集节点的原始支持乘以该分类的概率。  
