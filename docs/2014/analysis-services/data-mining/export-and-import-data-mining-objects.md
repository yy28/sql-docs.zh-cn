...
## <a name="exporting-data-mining-structures"></a>导出数据挖掘结构  
 ...
 如果挖掘结构已经过处理且已缓存结果（这是默认行为），则在您导出挖掘结构时，定义中将包含该结构所基于的数据的摘要。 若要删除此摘要，则必须通过执行 `Process Clear Structure` 操作来清除与挖掘结构相关联的缓存。 有关详细信息，请参阅 [Process a Mining Structure](process-a-mining-structure.md)。  
