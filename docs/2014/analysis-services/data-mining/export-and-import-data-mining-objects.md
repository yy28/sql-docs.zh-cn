...
# <a name="export-and-import-data-mining-objects"></a>导出和导入数据挖掘对象
 ...
 如果您的数据挖掘解决方案使用关系数据，而不是多维数据库，来使用传输模型`EXPORT`和`IMPORT`是更加快速、 轻松比使用数据库还原或部署整个解决方案。  
...
## <a name="exporting-data-mining-structures"></a>导出数据挖掘结构  
 ...
 如果挖掘结构已经过处理且已缓存结果（这是默认行为），则在您导出挖掘结构时，定义中将包含该结构所基于的数据的摘要。 若要删除此摘要，您必须清除通过执行与挖掘结构关联的缓存`Process Clear Structure`操作。 有关详细信息，请参阅 [Process a Mining Structure](process-a-mining-structure.md)。  
  
### <a name="exporting-data-mining-models"></a>导出数据挖掘模型  
 你可以使用`WITH DEPENDENCIES`关键字导出的数据源和数据源视图定义以及挖掘模型和其结构。  
...
## <a name="importing-data-mining-structures-and-models"></a>导入数据挖掘结构和模型  
 ...
 你还可以导入的挖掘结构或挖掘模型使用`Restore`命令。 您的模型或结构将被还原到与其所导出的数据库同名的数据库中。 有关详细信息，请参阅 [Restore Options](../multidimensional-models/restore-options.md)。  
