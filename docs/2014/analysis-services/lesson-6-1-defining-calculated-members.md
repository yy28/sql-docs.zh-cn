...
17. 之前在计算脚本中添加新行`[Internet GPM]`计算，然后将以下文本添加到自己的行上的脚本：  
...
8.  更改到此计算成员的名称`[Reseller Sales Ratio to All Products]`。  
...
13. 更改到此计算成员的名称`[Total Sales Ratio to All Products]`。  
...
18. 之前在计算脚本中添加新行`[Internet Sales Ratio to All Products]`计算，然后将以下文本添加到自己的行上的脚本：  
