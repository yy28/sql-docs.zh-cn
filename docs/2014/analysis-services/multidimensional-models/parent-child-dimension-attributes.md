...
# <a name="attributes-in-parent-child-hierarchies"></a>父子层次结构中的属性
 ...
 数据成员仅可用于具有父子层次结构的维度，并且仅当父属性允许时才可见。 可以使用维度设计器来控制数据成员的可见性。 若要公开数据成员，设置`MembersWithData`到的父属性的属性`NonLeafDataVisible.`若要隐藏父属性所包含的数据成员，设置`MembersWithData`到对父属性的属性`NonLeafDataHidden`。  
  
 此设置不会覆盖非叶成员的正常聚合行为；为了进行聚合，将始终包括作为子成员的数据成员。 但是，可以用自定义汇总公式来覆盖正常聚合行为。 多维表达式 (MDX) [DataMember](/sql/mdx/datamember-mdx)函数使你能够访问而不考虑的值关联的数据成员的值`MembersWithData`属性。  
  
 `MembersWithDataCaption`的父属性的属性提供[!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)]与用于生成数据成员的成员名称的命名模板。  
...
## <a name="using-data-members"></a>使用数据成员  
 ...
 此外，每个销售职员成员的个人佣金可能不同。 在这种情况下，与其下属销售人员所产生的总销售额的合计相对比，将使用两种不同的计数法来计算销售经理的个人总销售额的佣金。 因此，能够访问非叶成员的基础事实数据表数据非常重要。 MDX`DataMember`函数可以用于检索单独的销售经理 1 成员的总销售量和自定义汇总表达式可用来从提供总的销售经理 1 成员的聚合值中排除的数据成员与该成员相关的销售人员的销售卷。  
