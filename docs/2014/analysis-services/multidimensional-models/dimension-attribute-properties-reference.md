...
|`AttributeHierarchyOrdered`|确定关联的属性层次结构是否已排序。 默认值是 `True`。 但是，如果属性层次结构不会用于查询，您可以节省处理时间通过更改此属性设置为值`False`。|  
|`AttributeHierarchyVisible`|确定属性层次结构是否对客户端应用程序可见。 默认值是 `True`。 但是，如果属性层次结构不会用于查询，您可以节省处理时间通过更改此属性设置为值`False`。|  
...
|`IsAggregatable`|指定是否可以聚合特性成员的值。 默认值是`True`，这意味着，属性层次结构包含 （全部） 级别。 如果该属性的值为 `False`，则此特性层次结构不包含“(全部)”级别。|  
...
|`MembersWithData`|父特性用于确定是否显示父特性中非叶成员的数据成员。 此属性的值是时，才使用的值`Usage`属性设置为 Parent。 也就是说已定义父子层次结构。 可用选项如下所示：<br /><br /> **NonLeafDataHidden** 非叶数据已隐藏。<br /><br /> **NonLeafDataVisible** 非叶数据可见。|  
|`MembersWithDataCaption`|提供父特性为其系统生成的数据成员创建标题时所用的模板字符串。 此属性的值是时，才使用的值`Usage`属性设置为 Parent。 也就是说已定义父子层次结构。|  
|`Name`|包含特性的用户友好名称。|  
...
|`NamingTemplate`|定义如何在基于父特性构造的父子层次结构中命名级别。 此属性的值是时，才使用的值`Usage`属性设置为 Parent。 也就是说已定义父子层次结构。|  
|`OrderBy`|说明如何对特性层次结构中包含的成员进行排序。 默认值为 Name，该值指定属性成员的排序基于值的`NameColumn`属性，如果有的话。 否则，按键列的值对成员进行排序。 可用选项如下所示：<br /><br /> `NameColumn` 值进行排序`NameColumn`属性。<br /><br /> **Key** 按特性成员键列值进行排序。<br /><br /> **AttributeKey** 按指定特性的成员键值进行排序，该特性必须与此特性具有特性关系。<br /><br /> **AttributeName** 按指定特性的成员名称值进行排序，该特性必须与此特性具有特性关系。|  
|`OrderByAttribute`|标识对特性层次结构成员进行排序所依据的特性。|  
|`RootMemberIf`|确定如何识别父子层次结构的根成员或最顶层成员。 此属性的值是时，才使用的值`Usage`属性设置为 Parent。 也就是说已定义父子层次结构。 默认值为 `ParentIsBlankSelfOrMissing`，表示只有符合一个或多个为 `ParentIsBlank`、`ParentIsSelf` 或 `ParentIsMissing` 所描述的条件的成员才被视为根成员。 下面是其他可用的值：<br /><br /> `ParentIsBlank` 具有 null 值、 零或空字符串键列或列中的唯一成员会被视为根成员。<br /><br /> `ParentIsSelf` 只有使用本身为父级的成员会被视为根成员。<br /><br /> `ParentIsMissing` 无法找到父级的唯一成员会被视为根成员。|  
...
|`ValueColumn`|标识提供特性值的列。 如果`NameColumn`指定的属性的元素，则相同`DataItem`值用作默认值为`ValueColumn`元素。 如果未指定特性的 `NameColumn` 元素并且特性的 `KeyColumns` 集合包含单个表示字符串数据类型键列的 `KeyColumn` 元素，则使用相同的 `DataItem` 值作为 `ValueColumn` 元素的默认值。|  
  
> [!NOTE]  
>  有关如何设置值的详细信息`KeyColumn`属性时你正在使用 null 值和其他数据完整性问题，请参阅[Analysis Services 2005 中处理数据完整性问题](http://go.microsoft.com/fwlink/?LinkId=81891)。  
  
> [!NOTE]  
>  当查询中不显式包含层次结构中的成员时，将使用特性的默认成员来计算表达式。 指定属性的默认成员`DefaultMember`特性的属性。 当查询中包括维度的层次结构时，将忽略与该层次结构中的级别相对应的特性中的所有默认成员。 如果查询中不包括维度的层次结构，则默认成员将用于维度中的所有特性。 有关默认成员的详细信息，请参阅 [定义默认成员](attribute-properties-define-a-default-member.md)。  
