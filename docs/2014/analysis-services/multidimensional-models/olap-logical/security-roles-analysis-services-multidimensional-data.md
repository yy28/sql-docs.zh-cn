...
|操作|值|解释|  
|------------|------------|-----------------|  
|处理|{`true`, `false`}<br /><br /> `false`Default=|如果值为 `true`，则成员可以处理该对象以及该对象中包含的任何对象。<br /><br /> 处理权限不适用于挖掘模型。 <xref:Microsoft.AnalysisServices.MiningModel> 权限始终从 <xref:Microsoft.AnalysisServices.MiningStructure> 继承。|  
