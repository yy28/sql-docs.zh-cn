...
# <a name="define-a-default-member"></a>定义默认成员
 ...
 特性的默认成员用于在查询中不包括特性的情况下计算表达式。 指定属性的默认成员`DefaultMember`特性的属性。 当查询中包括维度的层次结构时，将忽略与该层次结构中的级别相对应的特性中的所有默认成员。 如果查询中不包括维度的层次结构，则默认成员将用于该维度的所有特性中。  
  
## <a name="resolving-the-default-member-when-no-default-member-is-specified"></a>未指定默认成员时解析默认成员  
 如果特性层次结构，指定任何默认成员并且特性层次结构是聚合 (`IsAggregatable`属性设置为`True`)，(All) 成员将作为默认成员。 如果未指定任何默认成员并且特性层次结构是不可聚合 (`IsAggregatable`属性设置为`False`)，从属性层次结构的顶层中选择默认成员。  
...
## <a name="see-also"></a>请参阅  
 [配置&#40;所有&#41;属性层次结构的级别](database-dimensions-configure-the-all-level-for-attribute-hierarchies.md)  
