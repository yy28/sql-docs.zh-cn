...
### <a name="connecting-to-the-server-object"></a>连接服务器对象  
 ...
 下面的代码示例在连接成功时返回 <xref:Microsoft.AnalysisServices.Server> 对象，如果出现错误，则返回 `null`。 连接过程中的错误在 try/catch 构造中处理。 AMO 错误通过 <xref:Microsoft.AnalysisServices.AmoException> 异常类捕获。 在此示例中，错误显示在一个消息框中供用户查看。  
...
###  <a name="AMO"></a> AmoException 异常对象  
 AMO 将在发现各种不同的问题时引发异常。 有关异常的详细说明，请参阅[AMO 其他类和方法](amo-other-classes-and-methods.md)。 下面的示例代码演示了捕获 AMO 中的异常的正确方法：  
...
### <a name="processing-a-database"></a>处理数据库  
 ...
 Process 方法可以包含参数，但不是必需的。 如果没有指定任何参数，将用所有子对象的 `ProcessDefault` 选项处理它们。 有关处理选项的详细信息，请参阅<xref:Microsoft.AnalysisServices.Database>。  
...
1.  创建<xref:Microsoft.AnalysisServices.DataSourceView>对象。  
     ...
     先验证该 <xref:Microsoft.AnalysisServices.DataSource> 对象是否存在，如果验证结果为 `true`，则删除 <xref:Microsoft.AnalysisServices.DataSource> 然后创建它。 如果 <xref:Microsoft.AnalysisServices.DataSource> 不存在，则创建它。  
...
> [!NOTE]  
>  子句 `'WHERE 1=0'` 是为了避免查询向 `DataSet` 对象返回行。  
...
```  
 ...
 在该示例代码中，`AddTable` 和 `AddComputedColumn` 方法使用 `DataAdapter` 对象的 `FillSchema` 方法将 `DataTable` 添加到 `DataSet` 中，并对架构进行配置以使其与数据源中的架构匹配。 扩展属性添加必需的信息以配置 [!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)] 的架构。  
