...
|**#**|表示显示一个数字或不显示任何内容的数字占位符。<br /><br /> 如果表达式在格式字符串中出现数字符号（）的位置有一个数字 **#** ，则格式化值将显示该数字。 否则，格式化值在该位置什么也不显示。<br /><br /> 数字符号（ **#** ）占位符的工作方式类似于零（**0**）数字占位符，不同之处在于，如果数字的位数等于或少于 **#** 格式表达式中小数点分隔符两侧的字符，则不会显示前导零和尾随零。|  
|**.**|表示决定小数点分隔符左侧和右侧显示几位数的小数点占位符。<br /><br /> 如果格式表达式在 **#** 句点（**.**）左侧只包含数字符号（）字符，则小于1的数字以小数点分隔符开头。 若要显示用小数显示的前导零，请使用零 (0) 作为小数点分隔符左侧的第一个数字占位符。<br /><br /> 在格式化输出中用作小数点占位符的实际字符取决于计算机系统所识别的数字格式。<br /><br /> 注意：在某些区域设置中，用逗号作为小数点分隔符。|  
...
|**- + $ ( )**|显示文字字符。<br /><br /> 若要显示未列出的字符，请在该字符前加上一个反斜杠（ **\\** ），或将该字符用双引号（**""**）引起来。|  
|**\\**|显示格式字符串中的下一个字符。<br /><br /> 若要将具有特殊含义的字符显示为文字字符，请在该字符前加上一个反斜杠（ **\\** ）。 反斜杠本身不显示。 使用反斜杠与将下一个字符放在双引号中的作用是相同的。 若要显示反斜杠，请使用两个反斜杠（ **\\\\** ）。 不能显示为文字字符的字符示例包括下列字符：<br /><br /> 日期格式和时间格式字符-**a**、 **c**、 **d**、 **h**、 **m**、 **n**、 **p**、 **q**、 **s**、 **t**、 **w**、 **y**、 **/** 和 **：**<br /><br /> 数字格式字符- **#** 、 **0**、 **%** 、 **E**、 **e**、**逗号**和**句点**<br /><br /> 字符串格式字符- **@** 、 **&** 、 **\<**, **>** 和 **！**|  
