...
-   [CREATE KPI 语句&#40;MDX&#41;](/sql/mdx/mdx-data-definition-create-kpi)  
...
### <a name="mdx-extensions-and-enhancements"></a>MDX 扩展和增强功能  
 ...
 通过 `caption` 属性、`display_folder` 属性和新 `STATIC | DYNAMIC` 关键字对 CREATE SET 命令进行了增强。 *静态*意味着仅在创建时计算集。 *动态*意味着每次在查询中使用一组计算集。 如果省略关键字，默认值则为 `STATIC`。  
