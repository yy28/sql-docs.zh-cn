...
## <a name="dbstoragelocation-usage"></a>DbStorageLocation 用法  
 `DbStorageLocation`数据库属性用作的一部分`Create`数据库中的命令`Detach` / `Attach`数据库命令序列`Backup` / `Restore`数据库命令序列或在`Synchronize`数据库命令。 更改 `DbStorageLocation` 数据库属性被认为是数据库对象的结构更改。 这意味着必须重新创建所有元数据并且重新处理数据。  
