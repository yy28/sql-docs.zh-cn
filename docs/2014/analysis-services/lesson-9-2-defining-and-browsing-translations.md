...
6.  中的行**标题**对象**月份名称**属性，键入`Mes del Año`中**西班牙语 （西班牙）** 翻译列和`Mois d'Année`中**法语 （法国）** 翻译列。  
...
1.  请切换到 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 教程多维数据集的多维数据集设计器，然后切换到“翻译”选项卡。  
     ...
     ![默认语言翻译选项卡中的](../../2014/tutorials/media/l9-translations-5.gif "默认翻译选项卡中的语言")  
...
7.  中的行**标题**Internet 销售 Sales Amount 度量值，类型的对象`Cantidad de las Ventas del Internet`中**西班牙语 （西班牙）** 翻译列和`Quantité de Ventes d'Internet`中**法语 （法国）** 翻译列。  
...
5.  在工具栏上，选择“语言”列表中的“西班牙语(西班牙)”。  
     ...
     注意，“元数据”窗格中的项将重新填充。 重新填充“元数据”窗格中的项之后，注意“Internet 销售额”度量值将不再出现在“Internet 销售”显示文件夹中。 相反，它显示在一个名为的新的显示文件夹中的西班牙语`Ventas del lnternet`，如以下图像中所示。  
  
     ![Repopulated 元数据窗格](../../2014/tutorials/media/l9-translations-6.gif "Repopulated 元数据窗格")  
  
6.  在元数据窗格中，右键单击`Cantidad de las Ventas del Internet`，然后选择**向查询添加**。  
  
7.  在元数据窗格中，展开`Fecha`，展开**Fecha.Calendar 日期**，右键单击**Fecha.Calendar 日期**，然后选择**将添加到筛选器**。  
...
9. 在“元数据”窗格中，右键单击“Mes del Ano”，然后选择“添加到查询”。  
     ...
     ![在数据窗格中的月份名称的西班牙语](../../2014/tutorials/media/l9-translations-7.gif "西班牙语在数据窗格中的月份名称")  
