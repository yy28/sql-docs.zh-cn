...
ms.locfileid: "36025595"
---
# <a name="connect-to-a-data-mining-server"></a>连接到数据挖掘服务器
  ![连接按钮](media/misc-connection.gif "连接按钮")  
  
 单击**连接**按钮以选择一个现有的连接，或创建新的连接到的实例[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]。  
...
3.  在**连接到 Analysis Services**对话框框中，键入服务器名称。  
 ...
 **获取一条消息指出"未找到任何连接。"**  
 ...
 如果将显示为按钮的下半部分中的文本**无连接**，这意味着你尚未创建的连接[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]数据库，或连接失败。 您可以继续处理来自 Access 或其他数据源的 Excel 中的数据，但如果要创建数据挖掘模型或运行预测查询，则必须拥有活动连接。  
  
 **假设我没有使用服务器的权限？**  
  
 如果你没有足够的权限来将您的挖掘模型存储在服务器上，或者如果你想要使用数据挖掘进行试验，而不保存所做的工作，你可以使用表分析工具，创建的临时数据结构和临时模型。 您仍需要能在服务器上存储临时模型。 请要求管理员允许使用*会话挖掘模型*服务器上。  
 ...
 **我无法连接到模型时使用 Visio 工具**  
  
 Visio 数据挖掘模板无法使用临时挖掘结构和模型。 若要创建某个挖掘模型的关系图，该模型必须存储在服务器上。  
  
 **如何监视使用情况的连接？**  
  
 [跟踪&#40;for Excel 数据挖掘客户端&#41;](trace-data-mining-client-for-excel.md)工具创建的外接程序和指定的服务器之间的所有活动的日志。  
  
 若要启用监视的会话模型，请选择**使用会话模型**选项**跟踪**对话框。  
 ...
 **临时模型是什么？如何保存临时模型？**  
 ...
 你还可以显式可以通过创建一个临时结构或模型**数据挖掘高级查询编辑器**并选择其中一个 DMX 模板。 添加 `USE SESSION MODELS` 子句可指定对象是临时的。   
  
### <a name="creating-backups-of-mining-models-and-structures"></a>创建挖掘模型和结构的备份  
 若要创建的备份模型或结构，你可以将其导出使用[管理模型&#40;SQL Server 数据挖掘外接程序&#41;](manage-models-sql-server-data-mining-add-ins.md)，Excel 数据挖掘客户端中。  
  
 如果您创建了一个临时挖掘模型，该模型通常具有难于理解的名称，例如 Table5_593679_TS_62446。 但是，你可以使用[跟踪&#40;for Excel 数据挖掘客户端&#41;](trace-data-mining-client-for-excel.md)工具来发现的临时结构和模型的创建的表分析工具的名称，然后备份它们使用**管理模型**。  
  
##### <a name="identify-and-export-a-temporary-model"></a>标识和导出临时模型  
  
1.  在**连接**组的数据挖掘客户端 Excel，请单击**跟踪**。  
  
2.  查看连接活动日志，并且通过查看列和可预测输出定位模型（举例）。  
  
     高级用户：如果您熟悉 DMX 或 XMLA，则可以将语句复制到文件以供以后使用。  
  
3.  当已找到的临时模型和结构的名称时，打开**管理模型**并选择的模型。  
  
4.  单击“导出此挖掘模型”以便在您指定的位置中生成一个脚本文件。  
  
## <a name="see-also"></a>请参阅  
 [连接到源数据&#40;for Excel 数据挖掘客户端&#41;](connect-to-source-data-data-mining-client-for-excel.md)   
