---
title: 示例数据 （SQL Server 数据挖掘外接程序） |Microsoft 文档
ms.custom: ''
ms.date: 03/06/2017
...
ms.locfileid: "36017200"
---
# <a name="sample-data-sql-server-data-mining-add-ins"></a>示例数据（SQL Server 数据挖掘外接程序）
  ![数据挖掘功能区中的分区数据向导](media/dmc-partition.gif "数据挖掘功能区中的分区数据向导")  
  
 **示例数据**向导可以轻松地将您的源数据划分为两个集，一个用于构建 （培训） 模型，一个用于测试模型。 此向导还提供了一个选项，可用于重新抽样数据来生成能更好地表示您的目标的新数据集。  
...
## <a name="random-sampling-and-oversampling"></a>随机抽样和过度抽样  
 ...
 如果你使用的随机采样选项，**示例数据**向导自动创建训练和测试数据集，并将其输出到单独的 Excel 工作表，以供稍后参考。  
  
 如果你的数据存储在 Excel 工作簿，并不外部数据源，你还可以选择使用*过度采样*。 通过此选项，指定可能在数据中罕见的目标值，而该向导将收集均衡的一组，其中不仅包括目标值。 可让向导达到一个目标百分比或创建某个数量的行。  
  
 如果你使用过采样选项，**示例数据**向导创建的新工作表包含新平衡的示例数据。  
  
## <a name="using-the-sample-data-wizard"></a>使用示例数据向导  
  
#### <a name="to-separate-data-into-training-and-testing-sets"></a>若要将数据划分为定型集和测试集  
  
1.  在**数据挖掘**功能区上，单击**示例数据**。  
  
2.  上**选择源数据**页上，指定是否**数据**要对分区是在 Excel 范围或表，还是在外部数据源。  
  
3.  上**选择采样类型**页上，指定是否想要创建训练和测试数据集的随机采样，或创建新的数据集的过采样。  
...
5.  上**完成页**，接受默认名称为新的数据集，或键入新名称。  
 ...
 Excel 数据挖掘客户端中的多数向导还提供一个选项，用于将数据随机划入定型集和测试集。 但是，如果使用这些向导，则数据保留在同一工作表（或其他数据源）中，并在内部存储有关特定行是测试事例还是定型事例的信息。 与此相反，当使用**示例数据**向导、 测试和定型数据输出到单独的工作表，以方便参考。  
...
|“选择抽样类型”页（Excel 数据挖掘客户端）|如果您使用外部数据源，则会限制您只能使用随机抽样选项。 此外，必须指定要使用创建最终的数据集的行数**行计数**选项。 不能指定源数据的百分比。|  
...
## <a name="other-sampling-options"></a>其他抽样选项  
 ...
 有关详细信息，请参阅[Row Sampling Transformation](../integration-services/data-flow/transformations/row-sampling-transformation.md)和[Percentage Sampling Transformation](../integration-services/data-flow/transformations/percentage-sampling-transformation.md)。  
