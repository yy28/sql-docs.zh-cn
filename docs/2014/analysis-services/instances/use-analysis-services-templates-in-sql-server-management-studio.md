...
##  <a name="bkmk_usingTE"></a> 打开 Analysis Services 模板  
 ...
 ![筛选出模板资源管理器的 Analysis Services](../media/ssas-templateexplorer.gif "模板资源管理器，筛选出的 Analysis Services")  
...
> [!WARNING]  
>  不能对已在直接查询模式下部署的表格模型使用 MDX 查询。 但是，可以通过将 DAX 表查询用于 EVALUATE 命令来发送等效的查询。 有关详细信息，请参阅[用于 DAX 查询参数](https://msdn.microsoft.com/library/gg492200(v=sql.120).aspx)。  
...
4.  您可以按原样执行此查询，但您可能会想要进行某些更改，例如添加函数以便返回特定成员。 例如，键入`.members`后 **[产品类别]。 [Product Category Name]**。 有关详细信息，请参阅 [Using Member Expressions](/sql/mdx/using-member-expressions)。  
...
2.  拖动`Backup`到空查询窗口的模板。  
  
3.  双击内的文本\<DatabaseID > 元素。  
  
4.  在对象资源管理器中，选择要备份的数据库，然后将该数据库拖放到 DatabaseID 元素的括号之间。  
  
5.  双击内的文本\<文件 > 元素。 键入备份文件的名称，包括 .abf 文件扩展名。 如果您不使用默认的备份位置，则指定完整的文件路径。 有关详细信息，请参阅[备份、还原和同步数据库 (XMLA)](../multidimensional-models-scripting-language-assl-xmla/backing-up-restoring-and-synchronizing-databases-xmla.md)。  
...
3.  在模板中，替换[RequestType 元素&#40;XMLA&#41; ](../xmla/xml-elements-properties/type-element-xmla.md)元素与以下文本： `<RequestType>MDSCHEMA_INPUT_DATASOURCES</RequestType>`  
...
||连续列值|演示如何使用 DMX SELECT DISTINCT FROM *\<模型 >* DMX 语句`RangeMin`和`RangeMax`函数从连续列中检索的一组指定范围中的值指定的挖掘模型。|  
...
|MDX\表达式|变动平均值 - 固定|演示如何使用 MDX`ParallelPeriod`和`CurrentMember`具有自然排序集创建一个提供固定数量时间段由在时间维度层次结构包含的度量值的移动平均值的计算度量值的函数。|  
||变动平均值 - 可变|演示如何使用 MDX`CASE`中的语句`Avg`函数来创建提供可变数量时间段由在时间维度的层次结构包含的度量值的移动平均值的计算度量值。|  
||本期截止到现在|演示如何在计算成员内使用 MDX `PeriodsToDate` 函数。|  
||父级比率|演示如何使用 MDX`Parent`函数来创建计算度量值，表示指定层次结构中的父成员的每个子级的度量值的百分率。|  
||总计比率|演示如何使用“全部”成员来创建一个计算度量值，以便表示指定层次结构中每个成员的度量值的百分率。|  
|MDX\查询|“基本查询”|演示一个可从其构造 MDX 查询的基本 MDX SELECT 语句。|  
||KPI 查询|演示如何使用 MDX`KPIValue`和`KPIGoal`函数来检索关键性能指标 (KPI) 在 MDX 查询的信息。|  
...
|XMLA\管理|Backup|演示如何使用 XMLA`Backup`命令以备份[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]到文件的数据库。|  
||取消|演示如何使用 XMLA`Cancel`命令以取消对当前会话 （针对管理员或服务器管理员以外的用户） 的所有正在运行的操作、 （适用于管理员），数据库或实例 （适用于服务器管理员。）|  
...
||DELETE|演示如何使用 XMLA`Delete`命令以删除现有[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]数据库。|  
||处理维度|演示如何将 XMLA `Batch` 命令与 `Parallel` 元素和 `Process` 命令组合使用来通过使用并行批处理操作更新维度的属性。|  
||处理分区|演示如何使用 XMLA`Batch`命令，结合`Parallel`元素和`Process`命令，以使用并行批处理操作完全处理分区。|  
||还原|演示如何使用 XMLA`Restore`命令才能还原[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]从现有备份文件的数据库。|  
||同步|演示如何使用 XMLA`Synchronize`命令以使另一个同步[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]与当前数据库[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]数据库针对 SynchronizeSecurity 标记使用 SkipMembership 选项。|  
|XMLA\架构行集|发现架构行集|演示如何使用 XMLA `Discover` 方法检索 DISCOVER_SCHEMA_ROWSETS 架构行集的内容。|  
|XMLA\服务器状态|连接|演示如何使用 XMLA`Discover`方法来检索 DISCOVER_CONNECTIONS 架构行集的内容。|  
||中执行计划的管理任务，即“作业”|演示如何使用 XMLA`Discover`方法来检索 DISCOVER_JOBS 架构行集的内容。|  
||位置|演示如何使用 XMLA`Discover`方法来检索 DISCOVER_LOCATIONS 架构行集，指定的位置的备份文件的路径的内容。|  
||锁|演示如何使用 XMLA `Discover` 方法检索 DISCOVER_LOCKS 架构行集的内容。|  
||内存授予|演示如何使用 XMLA `Discover` 方法检索 DISCOVER_MEMORYGRANT 架构行集的内容。|  
||性能计数器|演示如何使用 XMLA`Discover`方法来检索 DISCOVER_PERFORMANCE_COUNTERS 架构行集的内容。|  
||会话|演示如何使用 XMLA `Discover` 方法检索 DISCOVER_SESSIONS 架构行集的内容。|  
||跟踪|演示如何使用 XMLA`Discover`方法来检索 DISCOVER_TRACES 架构行集的内容。|  
||中的|演示如何使用 XMLA`Discover`方法来检索 DISCOVER_TRANSACTIONS 架构行集的内容。|  
