---
title: 从客户端应用程序进行连接（Analysis Services） |Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
...
##  <a name="connect-using-sql-server-management-studio-ssms"></a><a name="bkmk_SSMS"></a> 使用 SQL Server Management Studio (SSMS) 进行连接  
...
3.  身份验证始终为 Windows 身份验证，并且用户标识始终为通过 Management Studio 进行连接的 Windows 用户。  
     ...
     为了能成功连接，您必须拥有对服务器或服务器上的数据库的访问权限。 您想要在 Management Studio 中执行的大多数任务都要求管理权限。 请确保您用来进行连接的帐户是服务器管理员角色的成员。 有关详细信息，请参阅[&#40;Analysis Services&#41;授予服务器管理员权限](grant-server-admin-rights-to-an-analysis-services-instance.md)。  
...
##  <a name="connect-using-excel"></a><a name="bkmk_excel"></a>使用 Excel 进行连接  
...
##  <a name="connect-using-sql-server-data-tools"></a><a name="bkmk_SSDT"></a>使用 SQL Server Data Tools 进行连接  
...
##  <a name="test-connections"></a><a name="bkmk_tshoot"></a>测试连接  
