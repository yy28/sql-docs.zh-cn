---
title: 交叉验证 （SQL Server 数据挖掘外接程序） |Microsoft 文档
ms.custom: ''
ms.date: 03/06/2017
...
-   指定的剖面，数或*折叠*，在其中对结构数据进行分区。  
  
 **交叉验证**向导，然后在每个折叠上创建新模型、 测试其他折叠上的模型，然后报告模型的准确性。 完成后，**交叉验证**向导将创建一个报表，显示每个折叠的度量值并提供聚合中的模型的摘要。 此信息可用于确定基础数据对模型的有益程度，或对基于同一数据的不同模型进行比较。  
...
1.  在**准确性和验证**组**数据挖掘**功能区上，单击**交叉验证**。  
  
2.  在**选择结构或模型**对话框框中，选择一个现有挖掘结构或挖掘模型。 如果选择结构，该向导将对所有基于该结构且具有相同可预测属性的模型都使用交叉验证。 如果选择模型，则该向导将只对该模型使用交叉验证。  
  
3.  在**指定交叉验证参数**对话框中，在**折叠计数**框中，选择在其中将数据集的折叠数。 折叠是随机选择的数据交叉部分。  
  
4.  （可选） 设置要通过键入的数字在交叉验证中使用的行最大数目**最大行数**文本框。  
  
    > [!NOTE]  
    >  使用的行越多，得到的结果就越精确。 但是，处理时间也会显著增加。 选择的行数取决于您的数据，但是通常情况下，应该在不牺牲性能的前提下选择最大的行数。 为了提高性能，也可以指定较少的折叠。  
  
5.  选择从列**目标属性**下拉列表中。 该列表只显示那些在最初创建该模型时配置为可预测属性的列。 该模型可能包含多个可预测属性，但是您只能选择其中的一个。  
  
6.  选择一个介于**目标状态**下拉列表中。  
  
     如果可预测列包含连续数值数据，则此选项不可用。  
  
7.  （可选） 指定要用作值**目标阈值**计数一样准确的预测。 该值表示为概率，它是一个 0 到 1 之间的数字，其中 1 表示保证该预测准确，0 表示该预测不可能正确，.5 相当于随机推测。  
  
     如果可预测列包含连续数值数据，则此选项不可用。  
  
8.  单击 **“完成”**。 创建新工作表，名为**交叉验证**。  
...
## <a name="understanding-cross-validation-results"></a>了解交叉验证结果  
 交叉验证的结果将显示在新工作表，标题为**交叉验证报表\<属性名称 >**。 新工作表包括多个部分：第一部分是一个摘要，提供有关已测试模型的重要元数据，以便您了解这些结果分别属于哪个模型或结构。  
  
 报表中的第二部分提供一个统计信息摘要，指示原始模型的有效程度。 总之，为每个折叠创建模型之间的差异分析的三个关键度量值：*均方根误差*，*平均绝对误差*，和*对数得分*. 这些是标准的统计度量值，不仅用于数据挖掘，还用在大多数类型的统计分析中。  
...
#### <a name="classification-passfail"></a>分类： 通过/失败  
 ...
 通过/失败计算由的满足以下条件的事例计数：**传递**如果具有最高概率的预测的状态是与输入状态相同并且概率大于你为指定的值**状态阈值**; 否则为**失败**。  
...
#### <a name="lift"></a>提升  
 *提升*是与可能性相关联的度量值。 如果结果更有可能是当你使用比随机推测进行时模型，则称该模型提供*正提升*。 但是，如果该模型进行预测不太可能比随机的机会，提升分数是*负*。 因此，此指标指示通过使用模型可获得的改善程度，分数越高越好。  
  
 提升的计算方法为：测试事例中实际预测概率与边缘概率的比率。  
  
#### <a name="log-score"></a>对数评分  
 *对数评分*，也称为*日志可能性分数*用于预测，表示两个概率，转换为对数刻度之间的比率。 由于概率用小数表示，因此，对数评分始终是负数。 接近 0 的分数是较好的分数。  
  
 原始分数可以具有非常不规则或扭曲的分布，而对数评分与百分比相似。  
  
#### <a name="root-mean-square-error"></a>均方根误差  
 *均方根误差*(RMSE) 是用于查看不同的数据集进行比较，并扩展输入的小数位数可以引入差异平滑统计信息中的标准方法。  
  
 RMSE 表示预测值与实际值进行比较时的平均误差。 它的计算方式为：所有分区事例的平均误差的平方根除以分区中的事例数，不包括目标属性缺失值的行。  
  
#### <a name="mean-absolute-error"></a>平均绝对误差  
 *平均绝对误差*是对实际值的预测值的平均错误。 其计算方法为：获取误差的绝对值之和，然后找出这些误差的平均值。  
