...
# <a name="configure-distributed-replay"></a>Configure Distributed Replay
  [!INCLUDE[msCoName](../../includes/msconame-md.md)] Distributed Replay 配置详细信息在 Distributed Replay 控制器、客户端以及安装有管理工具的位置的 XML 文件中[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]指定。 这些文件包括下面的文件：  
...
##  <a name="DReplayController"></a>控制器配置文件： DReplayController  
...
##  <a name="DReplayClient"></a>客户端配置文件： Dreplayclient.exe  
...
### <a name="example"></a>示例  
 此示例显示一个客户端配置文件，该文件已经过修改，以指定控制器服务正在名为 `Controller1`的另一台计算机上运行。 
  `WorkingDirectory` 和 `ResultDirectory` 元素配置为分别使用文件夹 `c:\ClientWorkingDir` 和 `c:\ResultTraceDir`。 日志记录级别已从默认值改为消除 `INFORMATION` 和 `WARNING` 日志条目。  
...
##  <a name="PreprocessConfig"></a>预处理配置文件： DReplay  
...
|最长空闲时间|`<MaxIdleTime>`|将空闲时间的上限设为某个绝对值（以秒为单位）。|一个 >=-1 的整数<br /><br /> `-1`指示原始跟踪文件中的原始值没有变化。<br /><br /> 
  `0` 表示在任意给定时间点有某个活动正在进行。|不是。 默认情况下，该值为 `-1`。|  
...
##  <a name="ReplayConfig"></a>重播配置文件： DReplay  
...
|-------------|-----------------|-----------------|--------------------|--------------|  
|
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 目标实例（测试服务器）|`<Server>`|指定要连接的服务器名和 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例名。|*server_name*[\\*instance_name*]<br /><br /> 不能使用“`localhost`”或“`.`”来表示本地主机。|否，如果已通过使用 **-s**_目标服务器_参数和管理工具的**重播**选项来指定服务器名称。|  
