...
# <a name="sqllogship-application"></a>sqllogship 应用程序
  **Sqllogship**应用程序执行日志传送配置的备份、复制或还原操作以及相关的清理任务。 对特定数据库的特定实例[!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]执行该操作。  
  
 ![主题链接图标](../../2014/database-engine/media/topic-link.gif "“主题链接”图标")有关语法约定，请参阅[命令提示实用工具参考 &#40;数据库引擎&#41;](../tools/command-prompt-utility-reference-database-engine.md)。  
 ...
 **-logintimeout** _timeout_value_  
 指定在尝试超时之前为尝试登录到服务器实例所分配的时间。默认值为15秒。 *timeout_value*是**int**_。_  
  
 **-querytimeout** _timeout_value_  
 指定在尝试超时之前分配给启动指定操作的时间量。默认值为 "无超时期限"。 *timeout_value*是**int**_。_  
 ...
 **Sqllogship**应用程序 sqllogship 安装在 X:\PROGRAM Files\Microsoft SQL Server\120\Tools\Binn 目录中。  
...
|选项|目录访问权限|权限|  
|------------|----------------------|-----------------|  
|**-备份**|需要对备份目录的读/写访问权限。|需要与 BACKUP 语句相同的权限。 有关详细信息，请参阅 [BACKUP (Transact-SQL)](/sql/t-sql/statements/backup-transact-sql)。|  
|**-复制**|需要对备份目录的读取访问权限以及对复制目录的写入访问权限。|需要与 [sp_help_log_shipping_secondary_database](/sql/relational-databases/system-stored-procedures/sp-help-log-shipping-secondary-database-transact-sql) 存储过程相同的权限。|  
