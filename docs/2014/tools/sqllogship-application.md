 ...
 **-logintimeout** _timeout_value_  
 指定在尝试超时之前为尝试登录到服务器实例所分配的时间。默认值为15秒。 *timeout_value* 为 **int** _._  
  
 **-querytimeout** _timeout_value_  
 指定在尝试超时之前分配给启动指定操作的时间量。默认值为 "无超时期限"。 *timeout_value* 为 **int** _._  
...
|**-restore**|需要对复制目录的读/写访问权限。|需要与 RESTORE 语句相同的权限。 有关详细信息，请参阅 [RESTORE (Transact-SQL)](/sql/t-sql/statements/restore-statements-transact-sql).|  
