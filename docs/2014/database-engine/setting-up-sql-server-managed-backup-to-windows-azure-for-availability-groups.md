...
-   要考虑禁用实例级别设置，如果要创建新的可用性数据库，并且不想将实例级别设置应用于数据库  
...
6.  **启用并配置[!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)]Node1 上的 AGTestDB:** 启动 SQL Server Management Studio 并连接到 Node1 上装有可用性数据库的实例。 在根据要求修改数据库名称、存储 URL、SQL 凭据和保持期的值后，从查询窗口中运行以下语句：  
...
7.  **启用并配置[!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)]Node2 上的 AGTestDB:** 启动 SQL Server Management Studio 并连接到 Node2 上装有可用性数据库的实例。 在根据要求修改数据库名称、存储 URL、SQL 凭据和保持期的值后，从查询窗口中运行以下语句：  
