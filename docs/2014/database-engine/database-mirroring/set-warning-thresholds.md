...
ms.locfileid: "65805162"
---
# <a name="set-warning-thresholds"></a>设置警告阈值
  使用此对话框可为在 "**数据库镜像监视器**" 对话框的导航树中选择的数据库启用和配置一个或多个警告阈值。  
...
## <a name="options"></a>选项  
 *服务器实例及其连接状态*  
 伙伴服务器实例的名称，格式为_SYSTEM_**\\**_INSTANCE_NAME_。 对于默认服务器实例，只显示系统名。  
  
 该字段还指示当前监视器是否连接至该服务器实例。 可能的连接状态为：  
  
-   **未连接到**  *server_instance_name*  
  
-   正在**尝试连接到***server_instance_name*    
 ...
 **连接**/**取消**  
 "**连接**/**取消**" 按钮与每个*服务器实例及其连接状态*字段相关联。 按钮状态取决于连接状态：  
