...
# <a name="table-valued-parameter-type-discovery"></a>表值参数类型发现
 ...
 对于大多数参数类型，ICommandWithParameters：： GetParameterInfo 支持过程参数的类型信息。 从开始[!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)]，由于引入了用户定义的类型和`xml`数据类型，GetParameterInfo 方法不足以实现此目的，因为不能通过 ICommandWithParameters 提供用户定义的类型信息（名称、架构和目录）。 定义了一个新接口 ISSCommandWithParameters，用于提供扩展类型信息。  
