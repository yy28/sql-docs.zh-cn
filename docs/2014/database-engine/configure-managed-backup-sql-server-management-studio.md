...
## <a name="includess_smartbackupincludesss-smartbackup-mdmd-functions-using-managed-backup-interface-in-sql-server-management-studio"></a>
  [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] 函数使用 SQL Server Management Studio 中的托管备份接口  
 ...
 **若要配置托管备份：** 若[!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)]要在 SQL Server Management Studio 中配置，您必须是系统管理员或具有以下权限：  
  ...
  `db_backupoperator` 数据库角色中的成员、具有 `ALTER ANY CREDENTIAL` 权限，以及对 `EXECUTE` 存储过程具有 `sp_delete_backuphistory` 权限。  
  
 对 `SELECT` 函数的 `smart_admin.fn_get_current_xevent_settings` 权限。  
  
 `EXECUTE`对存储过程`smart_admin.sp_get_backup_diagnostics`的权限。 此外，它还需要 `VIEW SERVER STATE` 权限，因为它在内部调用其他需要此权限的系统对象。  
  
 对 `EXECUTE` 和 `smart_admin.sp_set_instance_backup` 的`smart_admin.sp_backup_master_switch` 权限。  
