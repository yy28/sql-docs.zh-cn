...
# <a name="transparent-data-encryption-with-azure-sql-database"></a>借助 Azure SQL 数据库实现透明数据加密
 ...
 [!INCLUDE[ssSDSfull](../includes/sssdsfull-md.md)] 不支持 Azure 密钥保管库与 TDE 的集成。 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 可以使用密钥保管库的非对称密钥。 有关详细信息, 请[参阅示例 A:使用 Key Vault](../relational-databases/security/encryption/extensible-key-management-using-azure-key-vault-sql-server.md#ExampleA)中的非对称密钥透明数据加密。  
  
||  
|-|  
|**适用对象**：[!INCLUDE[sqldbesa](../includes/sqldbesa-md.md)]([在某些区域中预览](https://azure.microsoft.com/documentation/articles/sql-database-preview-whats-new/?WT.mc_id=TSQL_GetItTag))。|  
...
8.  在“数据加密预览” 边栏选项卡中，将“数据加密” 按钮移动为“开”，然后单击“保存” （在页面顶部）以应用设置。 “加密状态” 将粗略估计透明数据加密的进度。  
     ...
     此外，还可以通过使用查询工具（如 [!INCLUDE[ssSDS](../includes/sssds-md.md)] ）作为具有“查看数据库状态” [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)]**权限的数据库用户连接到** ，以便监视加密进度。 查询[sys.databases _database_encryption_keys](/sql/relational-databases/system-dynamic-management-views/sys-dm-database-encryption-keys-transact-sql)视图的列。`encryption_state`  
...
3.  若要监视[!INCLUDE[ssSDS](../includes/sssds-md.md)]加密的进度, 具有**VIEW database STATE** `encryption_state`权限的数据库用户可以查询[sys.databases _database_encryption_keys](/sql/relational-databases/system-dynamic-management-views/sys-dm-database-encryption-keys-transact-sql)视图的列。  
...
6.  在“透明数据加密预览” 边栏选项卡中，将“数据加密” 按钮移动为“关”，然后单击“保存” （在页面顶部）以应用设置。 “加密状态” 将粗略估计透明数据解密的进度。  
     ...
     此外，还可以通过使用查询工具（如 [!INCLUDE[ssSDS](../includes/sssds-md.md)] ）作为具有“查看数据库状态” [!INCLUDE[ssManStudio](../includes/ssmanstudio-md.md)]**权限的数据库用户连接到** ，以便监视解密进度。 查询[sys.databases _database_encryption_keys](/sql/relational-databases/system-dynamic-management-views/sys-dm-database-encryption-keys-transact-sql)视图的列。`encryption_state`  
