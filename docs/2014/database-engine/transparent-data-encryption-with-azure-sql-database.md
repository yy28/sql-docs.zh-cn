...
ms.locfileid: "36129044"
---
# <a name="transparent-data-encryption-with-azure-sql-database"></a>借助 Azure SQL 数据库实现透明数据加密
  [!INCLUDE[ssSDSfull](../includes/sssdsfull-md.md)] 透明数据加密 （预览版） 有助于保护计算机免受恶意活动的威胁通过执行实时加密和解密的数据库、 关联的备份和事务日志文件但无需更改应用程序。  
 ...
 [!INCLUDE[ssSDSfull](../includes/sssdsfull-md.md)] 不支持 Azure 密钥保管库与 TDE 的集成。 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 可以使用密钥保管库的非对称密钥。 有关详细信息，请参阅[示例 a： 透明数据加密，通过使用密钥保管库的非对称密钥](../relational-databases/security/encryption/extensible-key-management-using-azure-key-vault-sql-server.md#ExampleA)。  
  
||  
|-|  
|**适用于**: [!INCLUDE[sqldbesa](../includes/sqldbesa-md.md)] ([在某些区域以预览版提供](http://azure.microsoft.com/documentation/articles/sql-database-preview-whats-new/?WT.mc_id=TSQL_GetItTag))。|  
  
> [!IMPORTANT]  
>  目前这是预览功能。 我确认并同意的该实现[!INCLUDE[ssSDS](../includes/sssds-md.md)]受到许可协议 （例如企业协议、 Microsoft Azure 协议或 Microsoft 在线订阅中的预览条款我的数据库中的透明数据加密协议），以及任何适用[补充使用条款的 Microsoft Azure 预览版](http://azure.microsoft.com/support/legal/preview-supplemental-terms/)。  
...
-   若要创建数据库加密密钥必须是[!INCLUDE[ssSDS](../includes/sssds-md.md)]管理员或者您必须是的成员**dbmanager**角色在 master 数据库中，并且具有**控件**对数据库拥有权限。  
  
-   使用 SET 选项执行 ALTER DATABASE 语句仅需要 **dbmanager** 角色的成员身份。  
  
##  <a name="Preview"></a> 注册的数据库上 TDE 并启用 TDE 预览的订阅  
...
7.  阅读预览中，条款和同意这些条款，如果选择**透明数据 encryptionPreview 条款**复选框，并依次**确定**页的底部附近。 返回到**数据 encryptionPREVIEW**边栏选项卡，其中**数据加密**按钮应现已启用。  
...
8.  在“数据加密预览”  边栏选项卡中，将“数据加密”  按钮移动为“开” ，然后单击“保存”  （在页面顶部）以应用设置。 “加密状态”  将粗略估计透明数据加密的进度。  
     ...
     此外，还可以通过使用查询工具（如 [!INCLUDE[ssSDS](../includes/sssds-md.md)] ）作为具有“查看数据库状态” [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)]**权限的数据库用户连接到** ，以便监视加密进度。 查询`encryption_state`列[sys.dm_database_encryption_keys](/sql/relational-databases/system-dynamic-management-views/sys-dm-database-encryption-keys-transact-sql)视图。  
...
3.  若要监视加密进度[!INCLUDE[ssSDS](../includes/sssds-md.md)]，数据库用户与**VIEW DATABASE STATE**权限可以查询`encryption_state`列[sys.dm_database_encryption_keys](/sql/relational-databases/system-dynamic-management-views/sys-dm-database-encryption-keys-transact-sql)视图。  
...
6.  在“透明数据加密预览”  边栏选项卡中，将“数据加密”  按钮移动为“关” ，然后单击“保存”  （在页面顶部）以应用设置。 “加密状态”  将粗略估计透明数据解密的进度。  
     ...
     此外，还可以通过使用查询工具（如 [!INCLUDE[ssSDS](../includes/sssds-md.md)] ）作为具有“查看数据库状态” [!INCLUDE[ssManStudio](../includes/ssmanstudio-md.md)]**权限的数据库用户连接到** ，以便监视解密进度。 查询`encryption_state`列[sys.dm_database_encryption_keys](/sql/relational-databases/system-dynamic-management-views/sys-dm-database-encryption-keys-transact-sql)视图。  
...
##  <a name="Working"></a> TDE 使用受保护的数据库上 [!INCLUDE[ssSDS](../includes/sssds-md.md)]  
...
##  <a name="Moving"></a> 使用移动 TDE 保护的数据库。Bacpac 文件  
