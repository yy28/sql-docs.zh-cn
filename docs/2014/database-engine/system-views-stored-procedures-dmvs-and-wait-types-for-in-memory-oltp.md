---
title: 系统视图，内存中 OLTP 的存储过程、 Dmv 和等待类型 |Microsoft 文档
ms.custom: ''
ms.date: 06/13/2017
...
|-----------------|-----------------|-----------------------------|  
|[sys.data_spaces (Transact-SQL)](/sql/relational-databases/system-catalog-views/sys-data-spaces-transact-sql)|检查文件组是否包含内存优化的数据。|下面的列显示其他值：**类型**和**type_desc**。|  
|[sys.indexes (Transact-SQL)](/sql/relational-databases/system-catalog-views/sys-indexes-transact-sql)|检查内存优化表中是否存在索引。|下面的列显示其他值：**类型**和**type_desc**。|  
...
|-----------------------|-----------------|-----------------------------|  
|[OBJECTPROPERTYEX (Transact-SQL)](/sql/t-sql/functions/objectproperty-transact-sql)|检查数据库对象是否经过内存优化。|**ExecIsWithNativeCompilation**和**TableIsMemoryOptimized**属性。<br /><br /> **IsSchemaBound**属性支持 （返回 0 而不是 NULL 的过程） 的过程对象类型。|  
...
## <a name="wait-types"></a>等待类型  
 ...
 有关详细信息，请参阅等待带有前缀的类型**WAIT_XTP**，和**XTPPROC**中[sys.dm_os_wait_stats &#40;TRANSACT-SQL&#41; ](/sql/relational-databases/system-dynamic-management-views/sys-dm-os-wait-stats-transact-sql)主题。  
