...
> [!IMPORTANT]  
>  可用性数据库在 *、PowerShell 和 SQL Server 管理对象 (SMO) 名称中有时候被称作“数据库副本”*[!INCLUDE[tsql](../../../includes/tsql-md.md)]。 例如，“数据库副本”一词在返回与可用性数据库有关的信息的 AlwaysOn 动态管理视图的名称中使用：**sys.dm_hadr_database_replica_states** 和 **sys.dm_hadr_database_replica_cluster_states**。 但在 SQL Server 联机丛书中，“副本”一词通常表示可用性副本。 例如，“主副本”和“辅助副本”始终表示可用性副本。  
