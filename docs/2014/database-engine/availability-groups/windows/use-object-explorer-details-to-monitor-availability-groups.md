...
|**联机**|远程可用性副本当前连接到本地副本。|  
|**无效**|如果本地副本是一个辅助副本，则此值对于其他辅助副本均为 NULL。|  
...
|**得到**|该数据库与当前主副本（如果有）或上一个主副本上的主数据库同步。<br /><br /> 注意：在性能模式中，数据库从不处于“已同步”状态。|  
|**无效**|未知状态。 当本地服务器实例无法与 WSFC 故障转移群集通信（即本地节点不是 WSFC 仲裁的一部分）时，出现此值。|  
...
|Value|说明|  
|-----------|-----------------|  
|**Suspended**|此状态指示该数据库在本地挂起，需要手动恢复。<br /><br /> 在主副本上，该值对于辅助数据库是不可靠的。 若要确认辅助数据库是否挂起，请在承载该数据库的辅助副本上进行查询。|  
