...
# <a name="replication-change-tracking-change-data-capture-and-alwayson-availability-groups-sql-server"></a>复制、更改跟踪、更改数据捕获和 AlwaysOn 可用性组 (SQL Server)
  ...
  [!INCLUDE[ssHADR](../../../includes/sshadr-md.md)]上支持复制、更改数据捕获 (CDC) 和更改跟踪 (CT)。 
  [!INCLUDE[ssHADR](../../../includes/sshadr-md.md)] 有助于提供高可用性和附加数据库恢复功能。  
  
 
  
##  <a name="Overview"></a>AlwaysOn 可用性组上的复制概述  
...
###  <a name="Changes"></a>对复制代理的常规更改以支持 AlwaysOn 可用性组  
...
-   **访问 CDC 更改数据和 Always On 的客户端应用程序**  
...
-   **对 CDC 变更数据和域登录名的客户端访问**  
  ...
  [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 身份验证登录名将无法访问任何辅助数据库中的数据，也无法访问除定义了数据库用户的原始数据库之外的任何新的主数据库中的数据。  
...
##  <a name="Prereqs"></a>使用复制的先决条件、限制和注意事项  
