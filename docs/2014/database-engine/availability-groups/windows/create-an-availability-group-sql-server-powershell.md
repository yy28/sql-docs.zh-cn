...
|创建可用性组|首先，将 `New-SqlAvailabilityReplica` cmdlet 与 `-AsTemplate` 参数一起使用，以便为您计划包括在可用性组中的两个可用性副本中的每一个都创建内存中可用性副本对象。<br /><br /> 然后，通过使用创建可用性组`New-SqlAvailabilityGroup`cmdlet 和引用可用性副本对象。|在要承载初始主副本的服务器实例上执行。|  
 ...
 **<sup>*</sup>**  若要执行给定的任务，请将目录更改 (`cd`) 到指明的服务器实例或实例。  
...
> [!NOTE]  
>  若要查看的语法和给定 cmdlet 的示例，请使用`Get-Help`中的 cmdlet [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] PowerShell 环境。 有关详细信息，请参阅 [Get Help SQL Server PowerShell](../../../powershell/sql-server-powershell.md)。  
...
8.  或者，使用 Windows`dir`命令来验证新的可用性组的内容。  
