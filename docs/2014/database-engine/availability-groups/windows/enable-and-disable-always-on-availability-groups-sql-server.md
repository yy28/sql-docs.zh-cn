...
1.  将默认值（ `cd` ）设置为 `\SQL\NODE1\DEFAULT` 要确定是否启用的服务器实例（例如） [!INCLUDE[ssHADR](../../../includes/sshadr-md.md)] 。  
...
####  <a name="example-enable-sqlalwayson"></a><a name="ExmplEnable-SqlHadrServic"></a>示例：Enable-SqlAlwaysOn  
 以下 PowerShell 命令 [!INCLUDE[ssHADR](../../../includes/sshadr-md.md)] 在 SQL Server （*计算机* \\ *实例*）的实例上启用。  
