---
title: 活动辅助副本：辅助副本备份（Always On 可用性组） |Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
...
1.  配置可用性组以便指定要对其执行备份的可用性副本。 有关详细信息，请参阅 *CREATE AVAILABILITY GROUP (Transact-SQL)* 或 *ALTER AVAILABILITY GROUP (Transact-SQL)* 中的 AUTOMATED_BACKUP_PREFERENCE 和 BACKUP_PRIORITY 参数[](/sql/t-sql/statements/create-availability-group-transact-sql)[](/sql/t-sql/statements/alter-availability-group-transact-sql)。  
  
2.  为承载作为执行备份候选的可用性副本的每个服务器实例上的每个可用性数据库都创建编写了脚本的备份作业。 有关详细信息，请参阅 [配置可用性副本备份 (SQL Server)](configure-backup-on-availability-replicas-sql-server.md)。  
