---
title: 全文目录属性 （表和视图页） |Microsoft 文档
ms.custom: ''
ms.date: 03/08/2017
...
|**断字符语言**|显示断字符的语言。|  
|**数据类型列**|列出保存中列出的列的文档类型的表中列的名称**可用列**如果列是`varbinary(max)`或`image`列。|  
...
|**手动**|修改、 添加，或在索引的数据中，删除数据时[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]跟踪所做的更改。 当 **“手动”** 更改跟踪处于有效状态时，不会使用这些更改自动更新索引。 相反，管理员可以手动应用更改通过使用[ALTER FULLTEXT INDEX...START UPDATE POPULATION](/sql/t-sql/statements/alter-fulltext-index-transact-sql)语句。|  
