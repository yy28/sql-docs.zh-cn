...
# <a name="sql-server-managed-backup-to-azure---retention-and-storage-settings"></a>针对 Azure 的 SQL Server 托管备份 - 保留和存储设置
 ...
 有关设置 @no__t 的完整演练，请参阅[将 SQL Server 托管备份设置为 azure](../relational-databases/backup-restore/enable-sql-server-managed-backup-to-microsoft-azure.md)和[为可用性组设置 SQL Server 托管备份到 azure](../../2014/database-engine/setting-up-sql-server-managed-backup-to-windows-azure-for-availability-groups.md)。  
...
-   不要对当前正使用维护计划或日志传送的数据库启用 [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] 。 有关互操作性和与其他 SQL Server 功能共存的详细信息，请参阅 @no__t 0SQL Server 托管备份到 Azure：互操作性和共存 @ no__t-0  
...
-   将身份验证信息存储到存储帐户的 Azure 存储帐户和 SQL 凭据应该在配置 @no__t 0 之前创建。 有关详细信息，请参阅**SQL Server 备份到 URL**主题的[关键组件和概念简介](../relational-databases/backup-restore/sql-server-backup-to-url.md#intorkeyconcepts)部分和 [Lesson 2：创建 SQL Server Credential @ no__t。  
...
####  <a name="Permissions"></a> Permissions  
 若要运行启用 [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] 的存储过程，您必须是具有**ALTER ANY CREDENTIAL**权限的**db_backupoperator**数据库角色中的 @no__t 或成员，并且必须具有对**sp_delete_backuphistory**的 @no__t 权限和@no__t 6 个存储过程。  用于查看现有设置的存储过程和函数通常分别需要针对存储过程的 `Execute` 权限以及针对函数的 `Select` 权限。  
...
-   **使用 Transact-sql：**  
     ...
     如果你是第一次启用 [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)]，则所需的参数为： *\@database_name*、 *\@credential_name*、 *\@encryption_algorithm*、 *@no__t 8enable_backup* *-10storage_url*参数是可选的。 如果未提供 @storage_url 参数的值，则使用 SQL 凭据中的存储帐户信息来派生该值。 如果提供存储 URL，则只应为存储帐户的根提供该 URL，并且该 URL 必须与您指定的 SQL 凭据中的信息相符。  
...
##  <a name="InstanceConfigure"></a>为实例启用和配置默认 [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)] 设置  
 可以通过两种方式在实例级别启用和配置默认 @no__t 值设置：使用系统存储过程 `smart_admin.set_instance_backup` 或**SQL Server Management Studio**。 下面说明了这两种方法：  
...
##  <a name="DatabaseDisable"></a> 为数据库禁用 [!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)]  
 可通过使用 `sp_set_db_backup` 系统存储过程，禁用[!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)]设置。 *@No__t-1enableparameter*用于为特定数据库启用和禁用 @no__t 2 配置，其中1启用，0禁用配置设置。  
