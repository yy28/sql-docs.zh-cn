...
####  <a name="Permissions"></a> Permissions  
 若要运行启用的存储的过程[!INCLUDE[ss_smartbackup](../includes/ss-smartbackup-md.md)]，你必须是`System Administrator`中或成员**db_backupoperator**拥有数据库角色**ALTER ANY CREDENTIAL**权限和`EXECUTE`上的权限**sp_delete_backuphistory**，和`smart_admin.sp_backup_master_switch`存储过程。  用于查看现有设置的存储过程和函数通常分别需要针对存储过程的 `Execute` 权限以及针对函数的 `Select` 权限。  
...
-   **使用 TRANSACT-SQL:**  
    ...
    >  有关如何创建加密证书的详细信息，请参阅 [Create an Encrypted Backup](../relational-databases/backup-restore/create-an-encrypted-backup.md)中的“创建备份证书”步骤。  
     ...
     此存储过程的详细信息，请参阅[smart_admin.set_db_backup &#40;TRANSACT-SQL&#41;](https://msdn.microsoft.com/library/dn451013(v=sql.120).aspx)  
