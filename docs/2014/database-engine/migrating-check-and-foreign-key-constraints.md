...
## <a name="checking-constraints-after-an-insert-update-or-delete-operation"></a>在插入、更新或删除操作后检查约束  
 此解决方法是乐观的，这是因为大多数的更改都不违反约束。 在此解决方法中，先修改数据，然后再对约束进行评估。 如果违反了约束，则会检测到该约束，但不会回滚更改。  
...
## <a name="sample-code"></a>代码示例  
 下面的示例基于 AdventureWorks2012 数据库。 具体而言，这些示例基于 [Sales]。除唯一索引之外的 [SalesOrderDetail] 表及其关联的 check 和 foreign key 约束。  
  
 此处指定的存储过程仅适用于内边距运算。 用于更新和删除操作的存储过程应具有相似的结构。  
  
## <a name="table-definition-for-the-workarounds"></a>解决方法的表定义  
 转换为内存优化表之前，[Sales] 的定义。[SalesOrderDetail] 如下所示：  
