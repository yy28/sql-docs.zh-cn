...
### <a name="discontinued-sql-dmo-from-sql-server-express-installation"></a>SQL Server Express 安装中已不再使用的 SQL-DMO  
 用于 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 的 SQL-DMO 已从 [!INCLUDE[ssExpressEd10](../includes/ssexpressed10-md.md)] 中删除。 我们建议您尽快修改当前使用此功能的应用程序。 如果必须支持 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] Express 的 SQL-DMO，请从[Microsoft 下载中心](https://www.microsoft.com/download/details.aspx?id=24793)安装 [!INCLUDE[ssVersion2005](../includes/ssversion2005-md.md)] 功能包的向后兼容组件。 请使用 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 管理对象 (SMO) 进行新的开发工作。  
...
|[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)]-EnableIntegrated 安全属性|使用 [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] 中的属性设置。|  
|[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)]-"计划事件和报表传递" 和 "Web 服务和 HTTP 访问"|编辑 RSReportServer.config 配置文件。|  
|命令行选项|在此版本中不支持。|  
...
|USESYSDB|在此版本中无等效参数。|  
 ...
 <sup>2</sup>从 [!INCLUDE[ssKatmai](../includes/sskatmai-md.md)]开始，指定/Action = EditionUpgrade，以在不使用原始安装媒体的情况下随时将现有版本的 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 升级到其他版本。 有关支持的版本升级的详细信息，请参阅 [Supported Version and Edition Upgrades](../database-engine/install-windows/supported-version-and-edition-upgrades.md)。  
