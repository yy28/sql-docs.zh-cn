...
7.  在 "**字段**" 列中，选择**PredictTimeSeries**。  
    ...
    > [!NOTE]  
    >  还可以将`Predict`函数用于时序模型。 但在默认情况下，预测函数只为每个时序创建一个预测。 因此，若要指定多个预测步骤，必须使用**PredictTimeSeries**函数。  
...
2.  在 "**保存数据挖掘查询结果**" 对话框中，选择[!INCLUDE[ssSampleDBDWobject](../includes/sssampledbdwobject-md.md)]"**数据源**"。 如果您希望将数据保存到不同的关系数据库，也可以创建数据源。  
