...
|班次|指示为其记录呼叫的轮班时间。 此呼叫中心将工作日分成四个班次:AM、PM1、PM2 和午夜。<br /><br /> 班次可能影响客户服务质量，因此您将使用此列作为输入。|  
...
|ServiceGrade|指示总体服务质量的指标, 以整个班次的*放弃率*为衡量。 挂断率越高，说明客户的满意度越差，因此丢失潜在订单的可能性也就越大。|  
 ...
 但是, 我们不会在`DateKey`模型中使用, 因为它包含的唯一值太多。 与`Shift` **DayOfWeek 之间**没有直接关系, and 和DayOfWeek仅部分`WageType`相关。 如果您担心共线性，可以使用所有可用列创建结构，然后在每个模型中忽略不同的列并测试效果。  
