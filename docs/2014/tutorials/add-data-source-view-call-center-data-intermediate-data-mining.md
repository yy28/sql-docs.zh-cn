---
title: 添加数据源视图呼叫中心数据 （数据挖掘中级教程） |Microsoft 文档
ms.custom: ''
ms.date: 12/29/2017
...
1.  在**解决方案资源管理器**，右键单击**数据源视图**，然后选择**新数据源视图**。  
  
     系统将打开数据源视图向导。  
  
2.  在“欢迎使用数据源视图向导”页中，单击“下一步”。  
  
3.  上**选择数据源**页上，在**关系数据源**，选择[!INCLUDE[ssAWDWsp](../includes/ssawdwsp-md.md)]数据源。 如果你没有此数据源，请参阅[Basic Data Mining Tutorial](../../2014/tutorials/basic-data-mining-tutorial.md)。 单击“下一步” 。  
  
4.  上**选择表和视图**页上，选择下表，然后单击向右箭头以将其添加到数据源视图：  
...
6.  上**完成向导**页上，默认情况下名为数据源视图[!INCLUDE[ssAWDWsp](../includes/ssawdwsp-md.md)]。 名称更改为**CallCenter**，然后单击**完成**。  
  
     数据源视图设计器将打开以显示**CallCenter**数据源视图。  
  
7.  在数据源视图窗格中，右键单击并选择**添加/删除表**。 选择表， **DimDate**单击**确定**。  
  
     应之间自动添加关系`DateKey`中每个表的列。 将使用此关系以使列， **EnglishDayNameOfWeek**，从**DimDate**表，然后在模型中使用。  
  
8.  在数据源视图设计器中，右键单击表， **FactCallCenter**，然后选择**新命名的计算**。  
    ...
    |**表达式**|`(SELECT EnglishDayNameOfWeek AS DayOfWeek FROM DimDate where FactCallCenter.DateKey = DimDate.DateKey)`|  
     ...
     若要验证的表达式创建数据需要用鼠标右键单击表**FactCallCenter**，然后选择**浏览数据**。  
...
|WageType|指示在一天是否为工作日、 除周末之外或假日。<br /><br /> 很可能是由于客户服务质量上的有所差别工作日与周末因此你将使用此列作为输入。|  
...
|LevelTwoOperators|指示值班的二级接线员的数量。<br /><br /> 员工必须登录一定数量的 service 小时数，才会被视为一个级别 2 运算符。|  
...
|ServiceGrade|一个度量值，该值指示对常规质量的服务，以度量*挂断率*的整个差值。 挂断率越高，说明客户的满意度越差，因此丢失潜在订单的可能性也就越大。|  
  
 请注意，数据包括基于单个日期列的四个不同的列： `WageType`， **DayOfWeek**， `Shift`，和`DateKey`。 通常在数据挖掘中不要使用派生自同一数据的多个列，因为值之间的相关性太强，可能遮盖其他模式。  
  
 但是，我们将不会使用`DateKey`模型中因为它包含太多唯一值。 之间没有直接关系`Shift`和**DayOfWeek**，和`WageType`和**DayOfWeek**仅部分相关。 如果您担心共线性，可以使用所有可用列创建结构，然后在每个模型中忽略不同的列并测试效果。  
  
## <a name="next-task-in-lesson"></a>课程中的下一个任务  
 [创建神经网络结构和模型&#40;中间数据挖掘教程&#41;](../../2014/tutorials/creating-a-neural-network-structure-and-model-intermediate-data-mining-tutorial.md)  
