...
# <a name="exploring-the-market-basket-models-intermediate-data-mining-tutorial"></a>浏览市场篮模型（数据挖掘中级教程）
  现在您已经生成了`Association`模型，可以使用数据挖掘设计器的 " [!INCLUDE[msCoName](../includes/msconame-md.md)] **挖掘模型查看器**" 选项卡中的 "关联查看器" 来浏览该模型。 本教程指导您使用查看器来浏览各项之间的关系。 查看器可以帮助您快速查看哪些产品通常会一起出现，并且大致了解出现的模式。  
...
3.  单击查看**+** 器右下角的 "登录"，并按住鼠标按钮，以在图形周围平移。  
  
4.  在查看器的左侧，向下拖动滑块，将其从**所有链接**（默认值）移动到滑块控件的底部。  
...
2.  在 "**筛选**项集" 框`Touring Tire`中键入，然后在框外单击。  
...
2.  在 "**筛选规则**" 框中`Mountain-200`，输入。  
  
     清除 "**显示长名称**" 复选框。  
  
3.  从 "**显示**" 列表中，选择 "**仅显示属性名称**"。  
  
     然后，查看器将只显示包含词 "`Mountain-200`" 的规则。 规则的概率告诉你当某人购买`Mountain-200`自行车时，该人还会购买所列的其他产品。  
