---
title: 将计划的策略部署到多个实例 |Microsoft 文档
ms.custom: ''
ms.date: 03/06/2017
...
3.  在**对象资源管理器详细信息**窗格中，选择所有你想要部署到其他服务器通过已注册的服务器的策略的计划的最佳做法。  
...
5.  如果你选择多个策略，若要导出，在**浏览文件夹**对话框中，选择目标文件夹，或者创建一个新文件夹。 对于本课程中，使用的路径创建一个新文件夹**C:\Scheduled_BP_Policies**，然后单击**确定**。  
  
     如果你只选择一个策略，若要导出，在**导出策略**对话框框中，使用路径创建一个新文件夹**C:\Scheduled_BP_Policies**，单击**打开**，然后单击**保存**。  
...
2.  展开**数据库引擎**，展开**本地服务器组**或**中央管理服务器**，右键单击你想要部署的策略，该节点，然后单击**导入策略**。  
    ...
    > [!NOTE]  
    >  如果你右键单击**本地服务器组**或中央管理服务器本身，策略将部署到所有托管服务器。 如果右键单击特定的服务器组，则策略将只部署到该组中的服务器。 如果右键单击特定的已注册服务器，则策略将只部署到该服务器。  
  
3.  旁边**要导入文件**，单击省略号按钮 (**...**).  
  
4.  在**选择策略**对话框中，浏览到保存计划的策略的文件夹位置。 对于本例，请浏览到的位置**C:\Scheduled_BP_Policies**。  
  
5.  选择你想要对目标实例中，导入，然后单击策略**打开**。  
  
6.  在**导入**对话框中，在**策略状态**列表中，选择所需的策略状态。 您可以选择在导入时保留策略状态、启用或禁用策略。 请注意，必须启用策略才能根据计划运行它们。  
  
7.  单击**确定**若要将策略导入到目标的所有实例。  
  
    > [!NOTE]  
    >  如果有任何错误，**导入**对话框不会消失。 单击**日志**页后，可以查看的消息。 单击**取消**以关闭对话框。  
  
8.  若要查看从目标实例的策略，连接到的实例，打开对象资源管理器，展开**管理**，然后展开**策略**。 你应看到在导入的策略**策略**节点。 如果您双击每个策略，则可以查看计划或更改设置。  
...
## <a name="next"></a>Next  
 现已学完了本教程。 若要返回到起始位置，请参阅[教程： 通过策略的管理来评估最佳实践](../../2014/tutorials/tutorial-evaluating-best-practices-by-using-policy-based-management.md)。  
  
 若要查看的列表[!INCLUDE[ssDE](../includes/ssde-md.md)]教程中，单击[数据库引擎教程](../relational-databases/database-engine-tutorials.md)。  
