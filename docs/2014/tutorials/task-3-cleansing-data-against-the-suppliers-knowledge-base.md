...
2.  如果在清理过程完成后，请查看**统计信息**中**Profiler**选项卡。源统计信息提供处理的记录数，找到是正确的具有 DQS，建议的更改的记录数和无效的记录数 DQS 更正的记录数的记录数。 在右侧的列表框中，您可以看到清理过程中涉及的每个域的已更正的值、建议的值和值的完成度（提供数据的程度）和准确性（数据可用于目标用途的程度）。  
