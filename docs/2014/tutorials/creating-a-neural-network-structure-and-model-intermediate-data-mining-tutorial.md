---
title: 创建神经网络结构和模型 （数据挖掘中级教程） |Microsoft 文档
ms.custom: ''
ms.date: 12/29/2017
...
# <a name="creating-a-neural-network-structure-and-model-intermediate-data-mining-tutorial"></a>创建神经网络结构和模型（数据挖掘中级教程）
 ...
 由于神经网络非常灵活且可以分析输入和输出的多个组合，因此应试用多种处理数据的方式以获得最佳结果。 例如，你可能想要自定义服务质量的数字目标是方式*装箱*，或分组，以面向特定业务要求。 为此，您将向该挖掘结构中添加一个以不同的方式对数值数据进行分组的新列，然后创建一个使用这个新列的模型。 您将使用这些挖掘模型来进行一些浏览。  
 ...
 **步骤**  
 ...
 [使用离散化要装箱的可预测列](#bkmk_ColumnCopy)  
  
 [复制列并更改为不同的模型的离散化方法](#bkmk_Alias)  
  
 [创建可预测列的别名，以便你可以比较模型](#bkmk_Alias2)  
  
 [处理的所有模型](#bkmk_SeedProcess)  
...
4.  上**创建数据挖掘结构**页上，确认选项**创建具有挖掘模型的挖掘结构**选择。  
  
5.  单击下拉列表中的选项**你想要使用何种数据挖掘技术？**，然后选择**Microsoft 神经网络**。  
...
8.  上**指定表类型**页上，选择**用例**旁边的复选框**FactCallCenter**表。 未选择任何操作**DimDate**。 单击“下一步” 。  
  
9. 上**指定定型数据**页上，选择**密钥**列旁边**FactCallCenterID。**  
  
10. 选择`Predict`和**输入**复选框。  
  
11. 选择**密钥**，**输入**，和`Predict`复选框下表中所示：  
...
12. 上**指定列的内容和数据类型**页上，验证该网格包含列、 内容类型和下表中所示的数据类型，然后单击**下一步**。  
...
13. 上**创建测试设置**页上，清除选项时，文本框**的数据以供测试百分比**。 单击“下一步” 。  
  
14. 上**完成向导**页上，为**挖掘结构名称**，类型`Call Center`。  
  
15. 有关**挖掘模型名称**，类型`Call Center Default NN`，然后单击**完成**。  
  
     **允许钻取**框被禁用，因为你不能钻取到与神经网络模型的数据。  
  
16. 在解决方案资源管理器，右键单击你刚数据挖掘结构的名称创建，然后选择**过程**。  
...
## <a name="use-discretization-to-bin-the-target-column"></a>使用离散化将目标列装箱  
 ...
 ![服务级值分布](../../2014/tutorials/media/skt-service-grade-valuesc.gif "服务级值的分布")  
  
 因此，处理模型时，输出的分组方式可能会与预期的不同。 例如，如果你使用群集来标识最佳组的值，该算法将 ServiceGrade 中的值为此范围： 0.0748051948-0.09716216215。 尽管此分组在数学上很准确，但此类范围可能对业务用户并没有太大意义。  
...
> [!NOTE]  
>  可以在处理完所有数据后，从模型的边际统计信息节点获取这些统计信息。 边际统计信息节点有关的详细信息，请参阅[挖掘模型内容神经网络模型的&#40;Analysis Services-数据挖掘&#41;](../../2014/analysis-services/data-mining/mining-model-content-for-neural-network-models-analysis-services-data-mining.md)。  
...
-   **按相等区域装箱**  
 ...
 或者，而不是使用的数字值，可以添加一个单独的派生的列，将服务年级分类到预定义的目标范围，如**最佳**(ServiceGrade \<= 0.05)， **可接受**(0.10 > ServiceGrade > 0.05)，和**差**(ServiceGrade > = 0.10)。  
  
###  <a name="bkmk_newColumn"></a> 创建的列的副本和更改的离散化方法  
 你将创建包含目标属性，ServiceGrade 挖掘列的一个副本，并更改对数字进行分组的方式。 可以创建挖掘结构中任何列（包括可预测属性）的多个副本。  
...
3.  在**选择列**对话框中，从列表中选择 ServiceGrade**源列**，然后单击**确定**。  
  
     一个新的列添加到挖掘结构列的列表中。 默认情况下，这个新的挖掘列具有与现有列相同的名称，并有一个数值后缀，如 ServiceGrade 1。 您可以更改该列的名称，使其更具描述性。  
  
     您还将指定离散化方法。  
  
4.  右键单击 ServiceGrade 1 并选择**属性**。  
  
5.  在**属性**窗口中，找到**名称**属性，并将名称更改为**服务级装箱**。  
  
6.  出现一个对话框，询问您是否要对所有相关的挖掘模型列的名称进行同样的更改。 单击 **“否”**。  
  
7.  在**属性**窗口中，找到的部分**数据类型**并将其展开，如有必要。  
...
##  <a name="bkmk_NewModel"></a> 将新的挖掘模型添加到挖掘结构  
...
-   将创建一个新的挖掘模型，调用 Center 装箱 NN，使用作为其目标结果 ServiceGrade 列中，分布式成四个存储桶的大小相等的值。  
  
#### <a name="to-add-a-mining-model-based-on-the-new-discretized-column"></a>添加基于新的离散化列的挖掘模型  
  
1.  在解决方案资源管理器，右键单击你刚的挖掘结构创建，然后选择**打开**。  
  
2.  单击 **“挖掘模型”** 选项卡。  
  
3.  单击**创建相关的挖掘模型**。  
  
4.  在**新建挖掘模型**对话框中，为**模型名称**，类型`Call Center Binned NN`。 在**算法名称**下拉列表中，选择**Microsoft 神经网络**。  
...
##  <a name="bkmk_Alias2"></a> 为目标列创建别名  
 通常，您不能比较使用不同可预测属性的挖掘模型。 但是，您可以为挖掘模型列创建别名。 即，使其具有与原始列相同的名称可以重命名的列，ServiceGrade 装箱，挖掘模型中。 然后可以在一个准确性图表中直接比较这两个模型，即使数据是以不同的方式离散化的也是如此。  
  
###  <a name="bkmk_Alias"></a> 若要添加在挖掘模型的挖掘结构列的别名  
  
1.  在**挖掘模型**选项卡上，在**结构**，选择 ServiceGrade 装箱。  
  
     请注意，**属性**窗口将显示对象，ScalarMiningStructure 列的属性。  
  
2.  在挖掘模型列 ServiceGrade Binned NN 下方单击与 ServiceGrade Binned 列对应的单元格。  
  
     请注意现在**属性**窗口显示的对象，MiningModelColumn 的属性。  
  
3.  找到**名称**属性，并将值更改`ServiceGrade`。  
...
5.  中单击任一处**挖掘模型**选项卡。  
     ...
     网格将更新以显示新的临时列别名， `ServiceGrade`，列用法旁边。 包含挖掘结构及两个挖掘模型的网格应如下所示：  
...
###  <a name="bkmk_SeedProcess"></a> 若要指定的种子和处理模型  
  
1.  在**挖掘模型**选项卡上，右键单击的列，模型命名为调用中心 — LR，然后选择**设置算法参数**。  
  
2.  在为 HOLDOUT_SEED 参数行中，单击下面的空单元格中**值**，和类型`1`。 单击“确定” 。 对与此结构关联的每个模型重复此步骤。  
...
## <a name="next-task-in-lesson"></a>课程中的下一个任务  
 [浏览呼叫中心模型&#40;中间数据挖掘教程&#41;](../../2014/tutorials/exploring-the-call-center-model-intermediate-data-mining-tutorial.md)  
