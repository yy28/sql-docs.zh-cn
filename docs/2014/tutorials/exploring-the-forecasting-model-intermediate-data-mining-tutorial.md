...
-   如果某一时序的树形图包含多个分支，这表示检测到的时序太过复杂，无法表示为单个公式。 相反，树形图可能包含多个分支，每个分支标记有导致树的条件*拆分*。 当树拆分时，每个分支表示不同的时间段，这些时间段内的趋势可描述为单个公式。  
     ...
     例如，如果您看看图表图形并查看在启动一段时间内年 9 月中且一直持续到年底假期的销售量突然跳转，您可以切换到**模型**视图，以查看趋势发生的确切日期。 树中表示为“九月前”和“九月后”的分支将包含不同的公式：一个公式用算术方法描述截至拆分时的销售趋势，另一个公式描述从九月开始一直到年底假期的销售趋势。  
  
#### <a name="to-explore-the-decision-tree-for-a-time-series-model"></a>浏览时序模型的决策树  
  
1.  中**树**上列出**模型**选项卡的查看器中，选择**T1000 Europe: Amount**系列。  
...
3.  现在选择其他要查看的时序。 使用**树**上列出**模型**选项卡的查看器中，选择**M200 North America: Amount**系列。  
...
-   **一般内容树查看器**： 允许你浏览和查看所使用的数据序列的所有模型中的一次和如果创建了混合模型，这两个 ARIMA 和 ARTXP 树同一图表中显示。  
