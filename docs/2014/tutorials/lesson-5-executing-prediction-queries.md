---
title: 第 5 课： 执行预测查询 |Microsoft 文档
ms.custom: ''
ms.date: 03/06/2017
...
ms.locfileid: "36312935"
---
# <a name="lesson-5-executing-prediction-queries"></a>第 5 课：执行预测查询
  在本课程中，你将使用[SELECT FROM\<模型 > 预测联接 (DMX)](/sql/dmx/select-from-model-cases-dmx)窗体的 SELECT 语句以创建两个不同类型的预测基于决策树模型中创建[第 2 课： 将挖掘模型添加到关联的挖掘结构](../../2014/tutorials/lesson-2-adding-mining-models-to-the-market-basket-mining-structure.md)。 这些预测类型定义如下。  
 ...
 [SELECT FROM\<模型 > 预测联接 (DMX)](/sql/dmx/select-from-model-cases-dmx)的 SELECT 语句的窗体包含三个部分：  
  
-   结果中返回的挖掘模型列和预测函数列表。 结果还可以包含源数据中的输入列。  
  
-   定义用于创建预测的数据源查询。 例如，在批查询中，此部分可能是客户列表。  
...
## <a name="singleton-query"></a>单独查询  
 第一步是使用[SELECT FROM&#60;模型&#62;PREDICTION JOIN &#40;DMX&#41; ](/sql/dmx/select-from-model-cases-dmx)中单独预测查询。 下面是单独语句的一般示例：  
...
3.  将  
    ...
    ```  
     ...
     AS 语句用于查询返回的别名列。 [PredictHistogram](/sql/dmx/predicthistogram-dmx)函数返回有关预测，包括概率和支持统计信息。 有关可以在预测语句中使用的函数的详细信息，请参阅[函数&#40;DMX&#41;](/sql/dmx/functions-dmx)。  
...
7.  在**另存为**对话框中，浏览到相应的文件夹，然后将该文件`Singleton_Query.dmx`。  
...
## <a name="batch-query"></a>批查询  
 下一步是使用[SELECT FROM&#60;模型&#62;PREDICTION JOIN &#40;DMX&#41; ](/sql/dmx/select-from-model-cases-dmx)在批预测查询中。 下面是批语句的一般示例：  
...
```  
 ...
 与在单独查询中相同，代码的前两行定义查询返回的挖掘模型的列，以及用于生成预测的挖掘模型的名称。 顶部\<数 > 语句指定的数或指定的结果，查询将只返回\<编号 >。  
...
```  
 ...
 顶部结合使用 ORDER BY\<数 > 语句，以筛选返回的结果。 例如，在此预测中，您将返回前十位自行车购买者，他们按照正确的预测概率排列。 您可以使用 [DESC|ASC] 语法控制结果的显示顺序。  
...
8.  在**另存为**对话框中，浏览到相应的文件夹，然后将该文件`Batch_Prediction.dmx`。  
...
9. 在工具栏上，单击**执行**按钮。  
 ...
 若要了解如何使用 DMX 在市场篮方案中，请参阅[市场篮 DMX 教程](../../2014/tutorials/market-basket-dmx-tutorial.md)。  
