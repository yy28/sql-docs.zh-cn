...
# <a name="creating-and-querying-data-mining-models-with-dmx-tutorials-analysis-services---data-mining"></a>使用 DMX 创建和查询数据挖掘模型：教程（Analysis Services - 数据挖掘）
  [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]使用[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]创建数据挖掘解决方案后，您可以针对数据挖掘模型创建查询以预测趋势、检索数据中的模式并度量挖掘模型的准确性。  
  
 以下列表中的分步教程将帮助你了解如何使用[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]生成和运行数据挖掘查询，以便能够充分利用数据。  
