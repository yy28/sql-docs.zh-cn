...
# <a name="task-4-setting-domain-rules"></a>任务 4：设置域规则
  在此任务中，你将为**联系人电子邮件**域创建一个规则，以验证该电子邮件地址是否以**\@adventure-works.com**结束。 有关详细信息，请参阅[创建域规则](https://msdn.microsoft.com/library/hh510397.aspx)主题。  
...
9. 在 "**联系人电子邮件**" 列中键入**frank7 \@adventure-works.com** （一个有效的值）。  
  
10. 重复上述两个步骤，添加**joe2 \@adventure-work.com** （不包含 "s" 的无效值）。  
  
11. 单击工具栏上的 "最后一个" 按钮（"**测试域规则**"），根据规则测试输入数据。  
  
     !["对所有字词测试域规则" 工具栏按钮](../../2014/tutorials/media/et-settingdomainrules-05.jpg ""对所有字词测试域规则" 工具栏按钮")  
