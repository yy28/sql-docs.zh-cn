---
title: 创建预测的呼叫中心模型 （数据挖掘中级教程） |Microsoft 文档
ms.custom: ''
ms.date: 03/06/2017
...
3.  在**挖掘模型**窗格中，单击**选择模型**。  
  
4.  **选择挖掘模型**对话框中显示的挖掘结构列表。 展开挖掘结构可查看与该结构相关联的挖掘模型列表。  
  
5.  展开挖掘结构 Call Center Default，选中神经网络模型 Call Center - LR。  
  
6.  从 **“挖掘模型”** 菜单中选择 **“单独查询”**。  
  
     **单独查询输入**对话框将出现，并映射到挖掘模型中的列的列。  
  
7.  在**单独查询输入**对话框中，进行移位时，单击该行，然后选择*午夜*。  
  
8.  单击对应的行级别 2 运算符和类型`6`。  
  
9. 在底部的下半部分**挖掘模型预测**选项卡上，单击网格中的第一行。  
  
10. 在**源**列中，单击向下箭头，然后选择**预测函数**。 在**字段**列中，选择**PredictHistogram**。  
  
     你可以自动使用此预测函数的自变量列表将显示在**条件/参数**框。  
  
11. 从列表中的列中拖动 ServiceGrade 列**挖掘模型**窗格**条件/参数**框。  
  
     列的名称自动作为参数插入。 可以选择任何可预测属性列以便放入此文本框。  
  
12. 单击按钮**切换到查询结果视图**，预测查询生成器的角。  
...
6.  在**创建命名查询**对话框中，为**名称**，类型`Shifts for Call Center`。  
...
8.  在设计窗格中，右键单击表中，将在调用中心和选择**浏览数据**预览 T-SQL 查询返回的数据。  
...
2.  在**挖掘模型**窗格中，单击**选择模型**，，然后选择调用中心 — LR 从列表中的可用模型。  
  
3.  从**挖掘模型**菜单上，清除选项，**单独查询**。 将警告您单独查询输入将丢失。 单击“确定” 。  
  
     **单独查询输入**对话框中将替换**选择输入表**对话框。  
  
4.  单击“选择事例表” 。  
  
5.  在**选择表**对话框中，从列表中的数据源的 selectShifts。 在**表/视图名称**列表中，选择将在作为呼叫中心 （可能会自动选择它），然后单击**确定。**  
  
     **挖掘模型预测**设计图面将更新以显示基于输入数据中和在模型中的列的名称和数据类型创建的映射。  
...
7.  单击 LevelTwoOperators 旁的空单元格，然后选择**调用 Center.AvgOperators 的移位**。  
  
8.  单击调用中，选择旁边的空单元格**调用 Center.AvgCalls 的移位**。 然后单击**确定**。  
  
##### <a name="to-create-the-predictions-for-each-shift"></a>创建每个班次的预测  
  
1.  在底部网格中的下半部分**预测查询生成器**，单击下面的空单元格中**源，** ，然后选择呼叫中心的变化。  
  
2.  在下面的空单元格中**字段**，选择 Shift。  
  
3.  单击网格中的下一个空行并重复实数过程来为 WageType 添加另一行。  
  
4.  单击网格中的下一个空行。 在**源**列中，选择**预测函数**。 在**字段**列中，选择**预测**。  
  
5.  拖动列 ServiceGrade 从**挖掘模型**窗格以网格中，并为**条件/参数**单元格。 在**别名**字段中，键入**预测服务等级**。  
  
6.  单击网格中的下一个空行。 在**源**列中，选择**预测函数**。 在**字段**列中，选择**PredictProbability**。  
  
7.  拖动列 ServiceGrade 从**挖掘模型**窗格以网格中，并为**条件/参数**单元格。 在**别名**字段中，键入**概率**。  
...
|班次、工资和日|预测的当前平均响应时间下的服务质量|预测的响应时间缩短为 90％ 时的服务质量|预测与响应时间减少 80%的服务质量|  
|--------------------------|------------------------------------------------------------------|--------------------------------------------------------------------------|--------------------------------------------------------------------------|  
|假日 AM|0.165|0.05|0.05|  
|假日 PM1|0.05|0.05|0.05|  
|假日午夜|0.165|0.05|0.05|  
  
 可以在此模型上创建多种其他预测查询。 例如，你无法预测多少运算符或不需要以满足特定服务级别响应一定数量的传入调用。 因为您可以在逻辑回归模型中包括多个输出，所以很容易试用不同的独立变量和结果，而无需创建许多单独的模型。  
  
## <a name="remarks"></a>Remarks  
 数据挖掘外接程序 Excel 2007 提供方便地回答复杂问题，例如多少级别的两个运算符的逻辑回归向导所需提高到特定 shift 目标级别的服务等级。 数据挖掘外接程序是免费下载的，并包含基于神经网络或逻辑回归算法的向导。 有关更多信息，请参见下列链接：  
  
-   [SQL Server 2005 数据挖掘外接 Office 2007](http://www.microsoft.com/sql/technologies/dm/addins.mspx)： 目标查找和怎么办应用场景分析  
  
-   [SQL Server 2008 数据挖掘外接 Office 2007](http://go.microsoft.com/fwlink/?LinkID=117790)： 目标查找应用场景分析，如果应用场景分析和预测计算器  
