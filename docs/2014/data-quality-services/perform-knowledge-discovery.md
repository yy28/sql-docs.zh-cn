...
5.  单击 **“下一步”** 可前进到知识库发现向导的 **“发现”** 页。 还可以选择以下项：  
    ...
    -   单击 **“关闭”** 将返回到 DQS 主页并且保存您的工作。 知识库对您而言将被锁定，且该知识库在 **“打开知识库”** 屏幕的知识库表中的状态将为 **“发现 - 映射”**。 单击后**关闭**，若要执行域管理活动中，您将需要单击**知识发现**从**打开知识库**屏幕上，请继续执行**知识库管理：管理域字词**屏幕上，单击**完成**，然后单击**是**若要发布的知识库或**否**若要保存该知识库所做工作和退出。  
...
2.  在发现进程正在运行，请为每个发现步骤显示的完成状态：**预处理记录**，**运行域规则**，和**运行发现**。 将为上述每个阶段显示完成百分比和完成状态。  
...
6.  单击 **“下一步”** 可前进到知识库发现向导的 **“管理域值”** 页。 在此页上，您可以修改添加到知识库的域的知识。 还可以选择以下项：  
    ...
    -   单击 **“关闭”** 将返回到 DQS 主页并且保存您的工作。 知识库将被锁定，且该知识库在 **“打开知识库”** 屏幕的知识库表中的状态将为 **“发现 - 发现”**。 单击后**关闭**，若要执行域管理活动中，您将需要单击**知识发现**从**打开知识库**屏幕上，请继续执行**知识库管理：管理域字词**屏幕上，单击**完成**，然后单击**是**若要发布的知识库或**否**若要保存该知识库所做工作和退出。  
...
4.  若要更改某个值的状态，请继续执行以下操作：  
    ...
    1.  **将所选的域值设为已更正**:若要从错误或无效的值的状态更改为正确，选择该值，然后依次**所选域值设为已更正**（检查） 从图标栏中的向下箭头或从类型下拉列表。 如果有错误或无效的值与某一正确值组合在一起，则在该操作后删除该值。  
  
    2.  **将所选的域值设为错误**:要从正确或无效的值的状态更改为错误，请选择该值，，然后单击**将所选域值设为错误**（叉号） 图标从图标栏中的向下箭头或从类型下拉列表。 您可以在 **“更正为”** 列中输入更正值，或者将其保留为空白。  
  
    3.  **将所选的域值设为无效**:若要从正确或错误的值的状态更改为无效，选择该值，然后依次**将所选域值设为无效**（三角形） 图标从图标栏中的向下箭头或从类型下拉列表。 您可以在 **“更正为”** 列中输入更正值，或者将其保留为空白。  
  
    4.  **正确的**:设置错误或无效的值后, 输入的新值**更正为**列。 DQS 将为更换值添加一个新行，将其指定为正确，然后组合这两个值。 这个新值将显示为前导值，前导值为粗体，而有错误或无效值是缩进的。  
  
5.  若要将值指定为一组同义词，则选择正确的多个值，然后继续如下操作：  
  
    -   **将所选的域值设为同义词**:单击此项可将所选的值设为同义词。 DQS 会将这些值之一指定为将用来替换其他值的前导值。  
    ...
    -   **将所选的域值设为其组的前导值**:更改组的前导值通过未指定为前导值，在组中选择一个值，然后单击**将所选域值设为其组的前导值**按钮。  
  
6.  **拼写检查器**:如果已启用了拼写检查器在域属性页中的，查找有红色的波浪下划线，拼写检查器正在建议的更正所指示的任何值。 右键单击带下划线的值，然后选择一个更正值（如果有适用的更正）。 值类型变为（或仍保持为）错误，并且更正将被添加到 **“更正为”** 列。 单击向下箭头可查看其他建议的更正。 手动输入一个更正并且将其添加到拼写检查器字典，并且能够将其作为更正选择。 有关详细信息，请参阅 [Use the DQS Speller](../../2014/data-quality-services/use-the-dqs-speller.md) 和 [Set Domain Properties](../../2014/data-quality-services/set-domain-properties.md)。  
  
    > [!NOTE]  
    >  若要使用拼写检查器，您或者可以在 **“域属性”** 页中启用它，或者如果已在 **“域属性”** 页中禁用它，则可以在 **“管理数据发现结果”** 页中单击 **“启用/禁用拼写检查器”** 图标以便在该页上启用它。  
  
7.  **添加新的域值**:通过单击将新值添加到域**添加新的域值**按钮以在表的末尾添加一行。 在输入某个值后，将按字母顺序重新定位该行。  
  
8.  **从 Excel 导入域值**:从 Excel 电子表格添加新值，通过单击向下的箭头**导入值**图标，，然后选择**从 Excel 导入域值**。 输入文件名，根据需要选择 **“将第一行用作标头”** ，然后单击 **“确定”**。 有关详细信息，请参阅 [将值从 Excel 文件导入到域](../../2014/data-quality-services/import-values-from-an-excel-file-into-a-domain.md)。  
  
9. **导入项目值**:从数据质量项目添加新值，通过单击向下的箭头**导入值**图标，并选择**导入项目值**。 输入文件名，根据需要选择 **“将第一行用作标头”** ，然后单击 **“确定”**。 选择您从中导入值的项目，然后单击 **“确定”**。 将显示导入的值。 单击 **“完成”**。 有关详细信息，请参阅“将项目值导入到域中”。  
  
10. **删除所选的域值**:从域中删除一个或多个现有值，通过选择值，然后单击**删除所选域值**按钮。 无法删除 DQS_NULL 的项，因此，如果您选择要删除的多个值，并且 DQS_NULL 的项是其中之一，则操作将失败。  
...
12. 还可以单击以下项：  
    ...
    -   单击 **“返回”** 可返回到 **“发现”** 。 单击后**关闭**，若要执行域管理活动中，您将需要单击**知识发现**从**打开知识库**屏幕上，请继续执行**知识库管理：管理域字词**屏幕上，单击**完成**，然后单击**是**若要发布的知识库或**否**若要保存该知识库所做工作和退出。  
...
-   **错误：** 这是一个值，属于域，但不正确的值。 例如，City 域中的“Shicago”（而非“Chicago”）是有错误的。 DQS 将其检测到有语法错误的值指定为有错误并且在发现过程中指定关联的更正。 语法错误包括拼写错误。  
  
-   **无效：** 这是不属于域，并且没有更正的值。 例如，City 域中的“12345”无效。 DQS 在某个值未能通过域规则时将其指定为无效。  
...
-   **值总计**:找到了多少总值的每个字段以及总共  
  
-   **新值**:自上次发现过程和值的总数及其占多少个值的总数为每个字段和所有映射的字段是新  
  
-   **唯一值**:对于每个字段和所有映射的字段是唯一的多少个值的总数及其占值的总数的百分比  
  
-   **新的唯一值**:自上次发现过程和值的总数及其占多少的唯一值的每个字段和所有映射的字段是新  
  
-   **在域值中有效**:对于每个字段和所有映射的字段都无效，多少个值的总数及其占值的总数的百分比  
...
-   **新**:的新值数以及新值的字段中的现有值与相比的百分比  
  
-   **唯一**:中的字段和及其占总数的百分比的唯一记录的数目  
  
-   **在域中有效**:有效的域值和及其占总数的百分比的数量  
  
-   **完整性**:匹配操作映射的每个源字段的完整性  
