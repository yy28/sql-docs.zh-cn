...
3.  活动监视屏幕显示有关活动网格中各活动的信息。 活动网格显示与各 DQS 活动有关的以下信息：  
    ...
    |**处于活动状态**|指示活动当前是否处于活动状态。 该选项可具有以下值：<br /><br /> **Active**:活动当前正在运行。<br /><br /> **结束**:活动已完成。<br /><br /> **终止**:活动已被终止使用活动监视屏幕 DQS 管理员或活动用户中的相应功能区域中运行时已取消[!INCLUDE[ssDQSClient](../includes/ssdqsclient-md.md)]。|  
    ...
    |**当前状态**|指示活动的当前状态。 活动状态取决于最后的计算过程。 该选项可具有以下值：<br /><br /> **运行**:计算过程正在运行。<br /><br /> **已成功**:任何计算过程运行之前，将状态设置为**Succeeded**。 同样，在计算过程成功结束后，状态也设置为 **“成功”**。<br /><br /> **失败**:计算过程已失败。<br /><br /> **停止**:计算过程已停止。<br /><br /> <br /><br /> 注意：一个活动内可以有多个计算过程，例如在知识发现活动内多次运行发现过程。 因此，在整个活动生命周期中状态可以更改多次。|  
    |**DQKB**|用于活动的知识库的名称。|  
...
1.  确定筛选条件：是否要基于活动网格（基于值的）中某一列的值和/或基于日期范围筛选活动记录。  
    ...
    1.  **基于值的筛选**:选择在某一筛选条件**筛选依据**列表，并选择要作为筛选依据中的相应值**值**列表。 在 **“筛选依据”** 列表中选择某一选项后， **“值”** 列表将更新为可能的值。 您可以筛选按活动记录中的以下字段：**处于活动状态**，**类型**，**子类型**，**当前状态**， **DQKB**，并**用户**。  
  
    2.  **日期基于范围的筛选**:选择适当的日期中**从日期**并**到日期**日期控件。 默认情况下， **“开始日期”** 中显示的日期是当前日期的前两天， **“结束日期”** 中显示的日期是当前日期。 筛选并不依据“开始日期”和“结束日期”执行，而是依据范围执行。 这意味着将显示在选定的日期范围内运行的各项活动。  
...
2.  下部窗格在以下两个选项卡之下显示所选活动的活动详细信息：  
    ...
    -   **活动步骤**:显示一个网格，与选定活动关联的计算进程 （活动步骤）。 对于此选项卡之下的某一活动，可以显示若干活动步骤。这可能会在用户在活动中运行若干次相同活动步骤的情况下发生。 例如，停止活动步骤后再次开始该步骤。 此选项卡下的网格显示每个活动步骤与活动关联的以下信息：**类型**，**当前状态**，**开始时间**，**已用时间**，并且**结束时间**。  
  
    -   **Profiler**:显示当前和历史活动的事件探查信息。 对于当前活动，它包含部分但一致的信息。 在您将相应的活动详细信息导出到某一 Excel 文件时，活动的事件探查信息将导出到该 Excel 文件。 导出的 Excel 文件的“事件探查器 - 源”和“事件探查器 - 字段”工作表中提供这些信息。  
...
3.  系统会提示您指定要保存的 Excel 文件的名称和位置。 导出的 Excel 文件包含以下工作表：  
    ...
    |事件探查器 - 源|有关**清理**子类型，包含与活动有关的以下信息： 记录、 更正记录、 已更正的记录和无效记录。<br /><br /> 有关**知识发现**，**域管理**，**匹配策略**，以及**匹配**子类型，包含以下与活动有关的信息： 记录、 值总计、 新值、 唯一值和新的唯一值。|  
    |事件探查器 - 字段|有关**清理**并**SSIS 清理**子类型，包含与活动有关的以下信息： 字段、 域、 更正的值，建议的值、 完整性和准确性。<br /><br /> 有关**知识发现**，**域管理**，**匹配策略**，以及**匹配**子类型，包含以下与活动有关的信息： 字段中，域中，新的、 唯一、 在域和完整性的考虑中有效。|  
