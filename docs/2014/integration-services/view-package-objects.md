...
# <a name="view-package-objects"></a>查看包对象
 ...
 可执行文件（包中的容器和任务）可以包含事件处理程序、优先约束和变量。 [!INCLUDE[ssISnoversion](../includes/ssisnoversion-md.md)]支持容器的嵌套层次结构，而 For 循环容器、Foreach 循环容器和序列容器可以包含其他可执行文件。  
