...
## <a name="task-names"></a>任务名称  
 虽然您可以通过名称或 ID 指定任务，但 `STOCK` 名字对象是 <xref:Microsoft.SqlServer.Dts.Runtime.Executables.Add%2A> 方法中最常用的参数。 若要向 `STOCK` 名字对象所标识的可执行文件添加任务，请使用下面的语法：  
