...
|`CheckpointUsage`|指定何时可以重新启动包。 值是`Never`， `IfExists`，和`Always`。 此属性的默认值是`Never`，指示包无法重新启动。 有关详细信息，请参阅<xref:Microsoft.SqlServer.Dts.Runtime.DTSCheckpointUsage>。|  
...
|`FailPackageOnFailure`|指定如果包组件中出现错误时，包是否失败。 此属性的唯一有效值是`False`。|  
...
|`Name`|包的名称。|  
|`PackageType`|包类型。 值是`Default`， `DTSDesigner`， `DTSDesigner100`， `DTSWizard`， `SQLDBMaint`，和`SQLReplication`。 此属性的默认值是`Default`。 有关详细信息，请参阅<xref:Microsoft.SqlServer.Dts.Runtime.DTSPackageType>。|  
...
|`Expressions`|单击浏览按钮 **(…)** 可以为包属性创建表达式。<br /><br /> 注意： 你可以创建对象模型包括，而不仅仅是在属性窗口中列出的属性的所有包属性的属性表达式。<br /><br /> 有关详细信息，请参阅 [在包中使用属性表达式](expressions/use-property-expressions-in-packages.md)。<br /><br /> 若要查看现有的属性表达式，请展开 `Expressions`。 单击表达式文本框中的浏览按钮 **(…)** 可以修改和计算表达式。|  
...
|`UpdateObjects`|指示当包所含对象的更新版本可用时，是否更新包以使用更新版本的对象。 例如，如果此属性设置为`True`，包括大容量插入任务的程序包更新为使用较新版本的大容量插入任务[!INCLUDE[ssISnoversion](../includes/ssisnoversion-md.md)]提供。 此属性的默认值是`False`。|  
...
|“属性”|Description|  
|--------------|-----------------|  
|`PackagePassword`|包保护级别的密码 (`EncryptSensitiveWithPassword`和`EncryptAllWithPassword`) 需要密码。|  
|`ProtectionLevel`|包的保护级别。 值是`DontSaveSensitive`， `EncryptSensitiveWithUserKey`， `EncryptSensitiveWithPassword`， `EncryptAllWithPassword`，和**ServerStorage**。 此属性的默认值是`EncryptSensitiveWithUserKey`。 有关详细信息，请参阅<xref:Microsoft.SqlServer.Dts.Runtime.DTSProtectionLevel>。|  
...
|“属性”|Description|  
|--------------|-----------------|  
|`IsolationLevel`|包事务的隔离级别。  此属性的默认值是`Serializable`。 有效值为 <br />`Unspecified`<br />`Chaos`<br />`ReadUncommitted`<br />`ReadCommitted`<br />`RepeatableRead`<br />`Serializable`<br />`Snapshot`的用户。<br /><br /> 仅当 `IsolationLevel` 属性的值设为 `TransactionOption` 时，系统才将 `Required` 属性应用到包事务。<br /><br /> 值`IsolationLevel`满足以下条件时，将忽略子容器请求的属性：<br /><br /> 子容器的 `TransactionOption` 属性的值为 `Supported`。<br />子容器联接父容器的事务。<br /><br /> 只有在容器开始新的事务时，才遵从该容器请求的 `IsolationLevel` 属性的值。 在以下条件成立时，容器将开始新的事务：<br /><br /> 容器的值`TransactionOption`属性是`Required`。<br />父级已开始一个事务。<br /><br /> <br /><br /> 注意：`Snapshot`值`IsolationLevel`属性是与包事务不兼容。 因此，不能使用`IsolationLevel`属性设置为包事务的隔离级别`Shapshot`。 相反，使用 SQL 查询将包事务设置为`Snapshot`。 有关详细信息，请参阅 [SET TRANSACTION ISOLATION LEVEL (Transact-SQL)](/sql/t-sql/statements/set-transaction-isolation-level-transact-sql)。<br /><br /> 有关 `IsolationLevel` 属性的详细信息，请参阅<xref:Microsoft.SqlServer.Dts.Runtime.DtsContainer.IsolationLevel%2A>。|  
