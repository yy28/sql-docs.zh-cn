...
> [!NOTE]  
>  变更数据捕获的每个版本中不可[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]。 有关的各版本支持的功能列表[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]，请参阅[SQL Server 2014 各个版本支持的功能](../../getting-started/features-supported-by-the-editions-of-sql-server-2014.md)。  
...
-   计算的开始和结束`datetime`你想要检索的源数据更改间隔的值。  
  
     若要计算这些值，请使用执行 SQL 任务或[!INCLUDE[ssISnoversion](../../../includes/ssisnoversion-md.md)]使用的表达式`datetime`函数。 然后将这些端点存储在包变量中以供以后在包中使用。  
  
     **有关详细信息：**[指定变更数据的时间间隔  ](specify-an-interval-of-change-data.md)  
...
-   确定所选间隔内的变更数据是否已准备就绪。 此步骤是必需的，因为异步捕获进程可能尚未到达所选端点。  
     ...
     **有关详细信息：**[确定是否更改数据是否已准备就绪  ](determine-whether-the-change-data-is-ready.md)  
...
-   将插入、删除和更新应用到目标。  
     ...
     **有关详细信息：**[所做的更改应用到目标  ](apply-the-changes-to-the-destination.md)  
...
## <a name="samples-of-change-data-capture-packages"></a>变更数据捕获包的示例  
 [!INCLUDE[ssISnoversion](../../../includes/ssisnoversion-md.md)] 提供了两个示例，演示如何使用变更数据捕获在包中。 有关详细信息，请参阅以下主题：  
