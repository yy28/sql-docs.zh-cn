...
> [!NOTE]  
>  变更数据捕获不可用的每个版本[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]。 有关支持的版本的功能的列表[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]，请参阅[支持的 SQL Server 2014 的版本功能](../../getting-started/features-supported-by-the-editions-of-sql-server-2014.md)。  
...
-   计算的起始和结束`datetime`你想要检索的源数据更改间隔的值。  
  
     若要计算这些值，使用执行 SQL 任务或[!INCLUDE[ssISnoversion](../../../includes/ssisnoversion-md.md)]使用表达式`datetime`函数。 然后将这些端点存储在包变量中以供以后在包中使用。  
  
     **有关详细信息：**[指定间隔的变更数据  ](specify-an-interval-of-change-data.md)  
...
-   确定所选间隔内的变更数据是否已准备就绪。 此步骤是必需的，因为异步捕获进程可能尚未到达所选端点。  
     ...
     **有关详细信息：**[确定是否更改数据可  ](determine-whether-the-change-data-is-ready.md)  
...
-   将更改拆分为插入、更新和删除进行处理。  
     ...
     **有关详细信息：**[进程还会插入、 更新和删除  ](process-inserts-updates-and-deletes.md)  
...
-   将插入、删除和更新应用到目标。  
     ...
     **有关详细信息：**[将更改应用于目标  ](apply-the-changes-to-the-destination.md)  
...
## <a name="samples-of-change-data-capture-packages"></a>变更数据捕获包的示例  
 [!INCLUDE[ssISnoversion](../../../includes/ssisnoversion-md.md)] 提供演示如何使用的两个样本中的变更数据捕获包。 有关详细信息，请参阅以下主题：  
