...
## <a name="configuring-a-single-transaction"></a>配置单个事务  
 在此选择中，程序包自身启动单个事务。 通过将包的 TransactionOption 属性设置为，可以将包配置为启动此事务 `Required` 。  
...
## <a name="configuring-multiple-transactions"></a>配置多个事务  
 在此选择中，包自身支持但不启动事务。 通过将包的 TransactionOption 属性设置为，可以将包配置为支持事务 `Supported` 。  
