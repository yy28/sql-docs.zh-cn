...
# <a name="excel-connection-manager"></a>Excel 连接管理器
 ...
 `ConnectionManagerType`的连接管理器的属性设置为`EXCEL`。  
...
-   指示所选工作表或范围中的第一行被访问数据是否包含列名称。  
 ...
 Excel 连接管理器使用[!INCLUDE[msCoName](../../includes/msconame-md.md)]OLE DB Provider for Jet 4.0 和其支持的 Excel ISAM （索引顺序访问方法） 驱动程序连接和读取和将数据写入到 Excel 数据源。 此提供程序和驱动程序与 Excel 源和 Excel 目标一起使用时的行为的详细信息，请参阅[Excel 源](../data-flow/excel-source.md)和[Excel 目标](../data-flow/excel-destination.md)。  
