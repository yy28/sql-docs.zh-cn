...
## <a name="options"></a>“常规”  
 **复制一个或多个表或视图的数据**  
 将字段从所选的源表和视图复制到指定的目标或目标，通过使用**选择源表和源视图**对话框。 如果希望在不对记录进行筛选或排序的情况下复制源中的所有数据，请使用此选项。  
...
-   System.Data.Odbc  
 ...
 若要使**将数据从一个或多个表或视图复制**其他提供程序选项可用，第三方可以将其自己的 ProviderDescriptor 部分添加到 ProviderDescriptors.xml 文件。 默认情况下，此文件位于\<*驱动器*>: files\microsoft SQL Server\100\DTS\ProviderDescriptors。 若要查看 ProviderDescriptor 部分的要求，请参阅 ProviderDescriptors.xsd 架构文件，默认情况下，该文件位于 ProviderDescriptors.xml 文件所在的文件夹中。  
  
 **编写查询以指定要传输的数据**  
 生成 SQL 语句以便检索行，通过使用**提供源查询**对话框。 如果希望在复制操作中修改或限制源数据，请使用此选项。 只有符合选择条件的行才可用于复制。  
