 ...
 **目标**  
 选择与目标的数据存储格式相匹配的数据访问接口。 可用于数据源的访问接口可能不止一个。 例如，对于[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]可以使用[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]本机客户端、 SQL Server.NET Framework 数据提供程序或 Microsoft OLE DB Provider for SQL Server。  
  
> [!NOTE]  
>  若要将数据保存到 ODBC 目标，请选择用于 ODBC 的 .NET Framework 数据访问接口。  
  
 **数据源**属性具有可变数量的选项，具体取决于计算机上安装的提供程序更改。 下表列出了某些常用的目标的选项。 有关其他访问接口的信息，请参阅访问接口特定的文档。  
 ...
 **“数据库”**  
 从指定的实例上的数据库列表中进行选择[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]，或通过单击创建新的数据库**新建**。  
  
 **“刷新”**  
 通过单击“刷新”，还原可用数据库的列表。  
  
 **新建**  
 通过使用创建新的目标数据库**Create Database**对话框。  
...
> [!NOTE]  
>  选择**Microsoft Excel**仅当你想要连接到数据源使用 Excel 2003 或更早版本。 若要连接到数据源使用 Excel 2007，选择**Microsoft Office 12.0 Access 数据库引擎 OLE DB 提供程序**，单击**属性**，然后在**所有**选项卡**数据链接属性**对话框中，为**扩展属性**，输入`Excel 12.0`。  
  
 **Excel 文件路径**  
 指定要在其中存储数据的工作簿的路径和文件名称 (例如，C:\MyData.xls， \\\Sales\Database\Northwind.xls)。 或者，单击 **“浏览”** 定位工作簿。  
...
> [!NOTE]  
>  当你将数据导出到[!INCLUDE[ofprexcel](../../includes/ofprexcel-md.md)]目标，则向导将使用[!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]Excel 目标组件。 某些使用注意事项和已知的问题的信息，请参阅[Excel 目标](../data-flow/excel-destination.md)。  
  
### <a name="destination--microsoft-access"></a>目标 = Microsoft Access  
  
> [!NOTE]  
>  选择**Microsoft Access**仅当你想要连接到数据库使用 Access 2003 或更早版本。 若要连接到使用 Access 2007 的数据库，选择**Microsoft Office 12.0 Access 数据库引擎 OLE DB 提供程序**。  
  
 **文件名**  
 指定要在其中存储数据的数据库文件的路径和文件名称 (例如，C:\MyData.mdb， \\\Sales\Database\Northwind.mdb)。 或者，单击 **“浏览”** 定位数据库文件。  
