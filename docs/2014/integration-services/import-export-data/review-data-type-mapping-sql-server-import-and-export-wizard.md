...
ms.locfileid: "36014592"
---
# <a name="review-data-type-mapping-sql-server-import-and-export-wizard"></a>查看数据类型映射（SQL Server 导入和导出向导）
  使用**查看 Data Type Mapping**页后，可以查看有关该向导必须执行使源数据与目标兼容的数据类型转换的详细的信息。 此信息包括可视提示，用于区分应成功的转换与可能导致错误或截断的转换。 对于每个转换，您都可以决定是否接受向导建议的转换，也可以指定如何处理发生的错误。  
...
### <a name="table-list"></a>表列表  
 上半部分**查看数据类型问题**页**表**列出要从源传输到目标的表的列表。 下表对列表中的列进行了说明。  
...
### <a name="data-type-mapping-list"></a>数据类型映射列表  
 下半部分**查看数据类型问题**页**数据类型映射**列表。 此网格提供有关在 **“表”** 列表中选择的表中的列的详细转换信息。 下表对列表中的列进行了说明。  
...
|**出错时**|指定向导处理错误的方式：<br /><br /> 使用**On Error （全局）** 设置。<br /><br /> 失败时出现一个错误，并停止导入或导出过程。<br /><br /> 忽略此错误。|  
|**截断时**|指定向导处理截断的方式：<br /><br /> 使用**上截断 （全局）** 设置。<br /><br /> 失败并出现错误，并停止导入或导出过程<br /><br /> 忽略截断。|  
