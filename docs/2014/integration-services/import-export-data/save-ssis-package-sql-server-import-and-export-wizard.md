...
# <a name="save-ssis-package-sql-server-import-and-export-wizard"></a>保存 SSIS 包（SQL Server 导入和导出向导）
  使用 "**保存 SSIS 包**" 页可以命名、描述[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Integration Services （[!INCLUDE[ssIS](../../includes/ssis-md.md)]）包并将其保存到[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] `msdb`数据库中，也可以保存到扩展名为 .dtsx 的文件中。  
