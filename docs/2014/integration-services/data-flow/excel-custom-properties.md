...
|OpenRowset|String|当 AccessMode 为`OpenRowset`时，为 Excel 目标访问的表或视图的名称。|  
|OpenRowsetVariable|String|当 AccessMode 为`OpenRowset from Variable`时，为包含 Excel 目标访问的表或视图名称的变量的名称。|  
