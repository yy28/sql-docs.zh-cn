...
|“值”|说明|  
|-----------|-----------------|  
|**注册表项**|键入包含配置信息的注册表项。 格式为 \<registry key>。<br /><br /> 该注册表项必须已经存在于 HKEY_CURRENT_USER 中并且具有一个名为 Value 的值。 该值可以是 DWORD 或一个字符串。<br /><br /> 如果要使用的注册表项不在 HKEY_CURRENT_USER 的根目录，请使用格式 \<Registry key\registry key\\...> 来标识该密钥。|  
