...
-   当发生错误或警告时，或者当任务失败时，发送电子邮件。  
 ...
 只有包具有事件处理程序，用于处理其 `OnError` 事件。 如果执行 SQL 任务运行时，发生错误`OnError`包的事件处理程序运行。 下图显示导致的调用序列`OnError`要执行的包的事件处理程序。  
...
|**OnVariableValueChanged**|**OnVariableValueChanged** 事件的事件处理程序。 此事件在变量值更改时由可执行文件引发。 此事件由此变量的可执行文件引发。 如果你设置，则不会引发此事件**RaiseChangeEvent**变量属性`False`。 有关详细信息，请参阅 [Integration Services (SSIS) 变量](integration-services-ssis-variables.md)。|  
