...
## <a name="getting-started-with-a-synchronous-transformation-component"></a>开始一个同步转换组件  
 向 [!INCLUDE[ssIS](../../includes/ssis-md.md)] 设计器的“数据流”窗格添加脚本组件时，“选择脚本组件类型”**** 对话框将打开，并提示选择源、目标或转换组件类型。 在此对话框中选择“转换”****。  
...
### <a name="configuring-input-columns"></a>配置输入列  
 ...
 在 "**脚本转换编辑器**" 的 "**输入列**" 页中，列列表显示数据流中上游组件的输出中可用的列。 选择要转换或传递的列。 将要就地转换的所有列标记为读/写。  
...
2.  在 [!INCLUDE[ssIS](../../includes/ssis-md.md)] 设计器中将源或另一转换的输出连接到新转换组件。 此`AdventureWorks`输出应提供示例数据库的**Person**表中包含**AddressID**和**City**列的数据。  
...
6.  创建并配置目标组件，如 ** 目标或 **使用脚本组件创建目标** 中演示的示例目标组件，该目标组件需要 AddressID**[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 和 City[](../extending-packages-scripting-data-flow-script-component-types/creating-a-destination-with-the-script-component.md) 列。 然后，将转换的输出连接到目标组件。 在 [!INCLUDE[tsql](../../includes/tsql-md.md)] 数据库中运行以下 `AdventureWorks` 命令，以创建目标表：  
...
2.  在 [!INCLUDE[ssIS](../../includes/ssis-md.md)] 设计器中将源或另一转换的输出连接到新转换组件。 此`AdventureWorks`输出应提供示例数据库的**Person**表中的数据，其中至少包含**AddressID**和**City**列。  
...
7.  创建并配置两个目标组件，如 ** 目标、平面文件目标或在**使用脚本组件创建目标**中演示的示例目标组件，这两个目标组件需要 AddressID**[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 和 City[](../extending-packages-scripting-data-flow-script-component-types/creating-a-destination-with-the-script-component.md) 列。 然后，将转换的每个输出连接到任一目标组件。 可通过在 [!INCLUDE[tsql](../../includes/tsql-md.md)] 数据库中运行与下列命令（具有唯一表名）类似的 `AdventureWorks` 命令来创建目标表：  
