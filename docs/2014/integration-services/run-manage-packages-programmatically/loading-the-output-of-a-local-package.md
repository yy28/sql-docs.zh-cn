...
2.  在开发项目中，将引用设置为`Microsoft.SqlServer.Dts.DtsClient`命名空间中查找程序集**Microsoft.SqlServer.Dts.DtsClient.dll**。 默认情况下，此程序集安装在 C:\Program Files\Microsoft SQL Server\100\DTS\Binn 中。 导入到你的代码通过使用 C# 命名空间`Using`或[!INCLUDE[vbprvb](../../includes/vbprvb-md.md)]`Imports`语句。  
  
3.  在代码中，创建类型的对象`DtsClient.DtsConnection`使用连接字符串包含所需的命令行参数**dtexec.exe**运行包。 有关详细信息，请参阅 [dtexec Utility](../packages/dtexec-utility.md)。 然后，使用此连接字符串打开连接。 还可以使用 dtexecui 实用工具直观地创建所需的连接字符串。  
...
6.  单击`Parameters`并在**设置查询参数**对话框框中，在查询中，Parameter0，将单个输入的参数映射到 DtsClient::Country 变量。  
