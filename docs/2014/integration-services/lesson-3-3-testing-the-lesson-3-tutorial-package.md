...
-   尽管为生成的信息的语义`PipelineExecutionPlan`和`PipelineExecutionTrees`事件不在此教程的范围之内，你可以看到的第一行列出中指定的信息字段**详细信息**选项卡**配置 SSIS 日志**对话框。 此外，可以验证已为 Foreach 循环的每个迭代记录了所选择的两个事件：PipelineExecutionPlan 和 PipelineExecutionTrees。  
