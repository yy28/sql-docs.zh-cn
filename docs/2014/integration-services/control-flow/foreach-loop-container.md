...
-   Foreach ADO 枚举器，用于枚举表中的行。 例如，可以获取 ADO 记录集中的行。  
     ...
     记录集目标将数据保存在内存中存储的包变量中的记录集`Object`数据类型。 您通常使用具有 Foreach ADO 枚举器的 Foreach 循环容器来每次处理记录集的一行。 为 Foreach ADO 枚举器指定的变量必须为 Object 数据类型。 有关记录集目标的详细信息，请参阅 [Use a Recordset Destination](../data-flow/recordset-destination.md)。  
