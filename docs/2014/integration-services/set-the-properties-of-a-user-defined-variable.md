---
title: 设置用户定义的变量的属性 |Microsoft 文档
ms.custom: ''
ms.date: 06/13/2017
...
-   可通过 Value 属性或 Expression 属性来设置变量的值。 默认情况下，EvaluateAsExpression 属性设置为`False`和变量的值属性设置的值。 若要使用表达式来设置的值，你必须首先设置 EvaluateAsExpression `True`，然后提供的 Expression 属性中的表达式。 Value 属性自动设置为该表达式的计算结果。  
  
-   ValueType 属性包含 Value 属性中的值的数据类型。 通过表达式设置 Value 时，ValueType 将自动更新为与该表达式的计算结果兼容的数据类型。 例如，如果值包含 0 和 ValueType 属性包含**Int32**然后将表达式设置为 getdate （） 函数，值包含当前日期和时间和 ValueType 将设置为`DateTime`。  
...
-   若要使用表达式设置变量值，首先确认变量数据类型是与该表达式的计算结果兼容，然后提供中的表达式`Expression`列**变量**窗口。 中的 EvaluateAsExpression 属性**属性**窗口将自动设置为`True`。  
...
-   通过变量的 **“变量”** 窗口，可以访问 **“表达式生成器”** 对话框。 使用该工具可以生成、验证和计算表达式。 有关详细信息，请参阅[表达式生成器](expressions/expression-builder.md)和 [Integration Services (SSIS) 表达式](expressions/integration-services-ssis-expressions.md)。  
 ...
 在这两**变量**和**属性**窗口中，如果将表达式分配给变量，将和`EvaluateAsExpression`设置为`True`，无法更改变量数据类型。  
  
 **设置 Namespace 和 Name 属性**  
  
 值`Name`和`Namespace`属性必须以字母字符字母开头，如 Unicode 标准 2.0 中或下划线 (_) 所定义。 后续字符可以是在 Unicode 标准 2.0 中定义的字母或数字，或是下划线 (\_)。  
...
5.  在列表中，选择的变量，然后更新中的值`Name`，**数据类型**， `Value`， `Namespace`，**引发更改事件**，**说明、** 和`Expression`列。  
