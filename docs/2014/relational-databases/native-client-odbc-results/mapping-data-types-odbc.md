...
### <a name="retrieving-data-from-the-server"></a>从服务器检索数据  
 ...
 如果未不指定任何绑定， [SQLGetData](../native-client-odbc-api/sqlgetdata.md)函数可以用于确定实例中的数据类型**sql_variant**列。  
...
3.  调用[SQLColAttribute](../native-client-odbc-api/sqlcolattribute.md)通过指定**SQL_CA_SS_VARIANT_TYPE**作为其属性值。 此函数将返回的实例中的 C 数据类型**sql_variant**到客户端的列。  
...
```  
 ...
 如果用户创建绑定使用[SQLBindCol](../native-client-odbc-api/sqlbindcol.md)，驱动程序读取的元数据和数据。 然后，驱动程序将这些数据转换为在绑定中指定的适当 ODBC 类型。  
  
### <a name="sending-data-to-the-server"></a>将数据发送到服务器  
 **SQL_SS_VARIANT**、 新数据类型特定于[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Native Client ODBC 驱动程序，用于数据发送到**sql_variant**列。 使用参数向服务器发送数据时 (例如，INSERT INTO TableName VALUES (？，？))， [SQLBindParameter](../native-client-odbc-api/sqlbindparameter.md)用于指定的参数信息，包括 C 类型和相应[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]类型。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client ODBC 驱动程序会将 C 数据类型转换为相应之一**sql_variant**子类型。  
