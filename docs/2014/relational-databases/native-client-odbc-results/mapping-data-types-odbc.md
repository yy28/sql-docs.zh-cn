...
### <a name="retrieving-data-from-the-server"></a>从服务器检索数据  
 ...
 如果未指定绑定， [SQLGetData](../native-client-odbc-api/sqlgetdata.md)函数可以用于确定实例中的数据类型**sql_variant**列。  
...
3.  调用[SQLColAttribute](../native-client-odbc-api/sqlcolattribute.md)通过指定**SQL_CA_SS_VARIANT_TYPE**作为其属性值。 此函数将返回中的实例的 C 数据类型**sql_variant**到客户端的列。  
...
```  
 ...
 如果用户创建绑定使用[SQLBindCol](../native-client-odbc-api/sqlbindcol.md)，驱动程序读取元数据和数据。 然后，驱动程序将这些数据转换为在绑定中指定的适当 ODBC 类型。  
