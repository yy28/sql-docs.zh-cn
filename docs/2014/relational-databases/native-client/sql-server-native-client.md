...
-   C 数据类型扩展能力。 有关详细信息，请参阅[ODBC 中的 C 数据类型](https://go.microsoft.com/fwlink/?LinkID=191495)。  
     ...
     若要在 Native Client [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]中支持此功能，SQLGetDescField `SQL_C_SS_TIME2`可以返回`time` （对于类型`SQL_C_SS_TIMESTAMPOFFSET` ）或`datetimeoffset`（对于） `SQL_C_BINARY`而不是（如果应用程序使用 ODBC 3.8）。 有关详细信息，请参阅[对 ODBC 日期和时间改进的数据类型支持](features/date-and-time-improvements.md)。  
