...
-   用小缓冲区多次调用 `SQLGetData` 来检索一个大型参数值。 有关详细信息，请参阅[使用 SQLGetData 检索输出参数](http://go.microsoft.com/fwlink/?LinkID=191494)。  
