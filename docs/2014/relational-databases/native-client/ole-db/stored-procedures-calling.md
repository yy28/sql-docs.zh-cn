...
```  
 ...
 有关演示 RPC 转义序列的示例应用，请参阅[执行存储过程&#40;使用 RPC 语法&#41;过程返回代码和输出参数和&#40;OLE DB&#41;](../../native-client-ole-db-how-to/results/execute-stored-procedure-with-rpc-and-process-output.md)。  
  
### <a name="transact-sql-execute-statement"></a>Transact-SQL EXECUTE 语句  
 ODBC 调用转义序列和 RPC 转义序列是首选的方法调用存储的过程而不是[执行](/sql/t-sql/language-elements/execute-transact-sql)语句。 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client OLE DB 提供程序使用的 RPC 机制[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]以优化命令处理。 此 RPC 协议通过避免在服务器上进行大量参数处理和语句分析来提高性能。  
