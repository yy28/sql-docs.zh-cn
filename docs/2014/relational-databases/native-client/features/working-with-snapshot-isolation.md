...
|属性 ID|描述|  
|-----------------|-----------------|  
|DBPROP_SESS_AUTOCOMMITISOLEVELS|键入：VT_I4<br /><br /> R/W:只读<br /><br /> 说明:指定一个位掩码，指示在自动提交模式下的事务隔离级别。 可在此位掩码中设置的值与可针对 DBPROP_SUPPORTEDTXNISOLEVELS 设置的值相同。|  
...
## <a name="sql-server-native-client-odbc-driver"></a>SQL Server Native Client ODBC 驱动程序  
 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client ODBC 驱动程序提供了支持快照隔离但对所做的增强功能[SQLSetConnectAttr](../../native-client-odbc-api/sqlsetconnectattr.md)并[SQLGetInfo](../../native-client-odbc-api/sqlgetinfo.md)函数。  
