...
> [!NOTE]  
>  如果可能，请在运行时提示用户输入他们的凭据，并避免用持久化格式存储他们的凭据。 如果您必须保留其凭据，则应加密它们使用[Win32 加密 API](http://go.microsoft.com/fwlink/?LinkId=64532)。 有关使用密码的详细信息，请参阅[强密码](../../security/strong-passwords.md)。  
...
>  不得保留 SSPROP_AUTH_OLD_PASSWORD，因为只有在密码过期时才使用它。  
 ...
 有关 dbpropset_sqlserverdbinit 限属性集的详细信息，请参阅[初始化和授权属性](../../native-client-ole-db-data-source-objects/initialization-and-authorization-properties.md)。  
...
### <a name="odbc-user-interface-password-expiration"></a>ODBC 用户界面密码过期  
 ...
 如果[SQLDriverConnect](../../native-client-odbc-api/sqldriverconnect.md)称为和的值**DriverCompletion**如果密码已过期设置为 SQL_DRIVER_NOPROMPT，则初始连接尝试将失败。 通过后续调用返回的 SQLSTATE 值 28000 和本机错误代码值 18487 **SQLError**或**SQLGetDiagRec**。  
...
### <a name="odbc-programmatic-password-expiration"></a>ODBC 编程密码过期  
 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client ODBC 驱动程序还支持通过在添加 SQL_COPT_SS_OLDPWD 属性在连接到服务器使用之前设置密码过期[SQLSetConnectAttr](../../native-client-odbc-api/sqlsetconnectattr.md)函数。  
  
 连接句柄的 SQL_COPT_SS_OLDPWD 属性是指已过期的密码。 对于此属性，没有连接字符串属性，原因是如果有的话会影响连接池。 如果登录成功，则驱动程序将清除此属性。  
  
 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client ODBC 驱动程序，对此功能的四个情况下均返回 SQL_ERROR： 密码过期、 密码策略冲突、 帐户锁定，并且旧的密码属性时使用 Windows 身份验证的设置时。 该驱动程序向用户返回相应的错误消息时[SQLGetDiagField](../../native-client-odbc-api/sqlgetdiagfield.md)调用。  
