...
> [!NOTE]  
>  此外可以指定**varchar （max)**， **nvarchar （max)**，并**varbinary （max)** 数据类型作为输入和输出参数类型的存储过程、 函数返回类型或在[CAST 和 CONVERT](/sql/t-sql/functions/cast-and-convert-transact-sql)函数。  
...
-   绑定为 DBTYPE_IUNKNOWN 并使用流处理。  
 ...
 在报告列的最大小时[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]Native Client OLE DB 访问接口将报告：  
...
```  
 ...
 详细了解如何[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]Native Client OLE DB 访问接口公开大值数据类型，请参阅[Blob 和 OLE 对象](../../native-client-ole-db-blobs/blobs-and-ole-objects.md)。  
  
## <a name="sql-server-native-client-odbc-driver"></a>SQL Server Native Client ODBC 驱动程序  
 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client ODBC 驱动程序公开**varchar （max)**， **varbinary （max)** 并**nvarchar （max)** 为 SQL_VARCHAR、 SQL_VARBINARY 和 SQL_ 类型WVARCHAR 中接受或返回 ODBC SQL 数据类型的 ODBC API 函数。  
  
 在报告列的最大大小时，该驱动程序将报告以下两个值之一：  
  
-   定义的最大大小，例如，即 2000年有关**varchar(2000)** 列，或  
