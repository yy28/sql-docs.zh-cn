...
```  
 ...
 有时某个命令长时间未完成。 如果应用程序需要取消命令而无需等待答复，则可采用通过调用**SQLCancel**使用相同的语句作为未完成命令的处理。 这是唯一一次**SQLCancel**应使用。 某些程序员使用**SQLCancel**时处理这些情况下方式通过结果设置并且想要取消的结果集的其余部分。 [SQLMoreResults](../../native-client-odbc-api/sqlmoreresults.md)或[SQLCloseCursor](../../native-client-odbc-api/sqlclosecursor.md)应该用于不取消未完成的结果集的其余部分**SQLCancel**。  
