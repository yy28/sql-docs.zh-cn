...
-   主机策略：这是由 CLR（在本例中为 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]）的主机设置的策略，对该主机中运行的托管代码有效。  
 ...
 在中运行时授予托管代码的代码访问安全权限集向中[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]加载的程序集授予一组权限，用户和计算机级别策略可能会进一步限制授予用户代码的最终权限集。  
...
### <a name="safe"></a>SAFE  
 只允许内部计算和本地数据访问。 
  `SAFE` 是最具限制性的权限集。 由具有 `SAFE` 权限的程序集执行的代码无法访问外部系统资源，例如文件、网络、环境变量或注册表。  
...
|`SqlClientPermission`|
  `Context connection = true`、`context connection = yes`：只能使用上下文连接并且连接字符串只能指定值“context connection=true”或“context connection=yes”。<br /><br /> **AllowBlankPassword = false：** 不允许使用空白密码。|  
  
### <a name="external_access"></a>EXTERNAL_ACCESS  
 EXTERNAL_ACCESS 程序集具有与`SAFE`程序集相同的权限，还可以访问外部系统资源，例如文件、网络、环境变量和注册表。  
...
|`SecurityPermission`|
  `Assertion:` 能够断言此代码的所有调用方都具有执行该操作所需的权限。<br /><br /> 
  `ControlPrincipal:` 能够操作主体对象。<br /><br /> 
  `Execution:` 用于执行托管代码的权限。<br /><br /> 
...
> [!IMPORTANT]  
>  对于执行计算和数据管理任务而无需访问 `SAFE` 外部资源的程序集，[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 是推荐的权限设置。 `EXTERNAL_ACCESS`默认情况下，程序集[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]作为服务帐户执行，只`EXTERNAL_ACCESS`应将执行权限提供给受信任的登录名，以作为服务帐户运行。 从安全角度来看，`EXTERNAL_ACCESS` 和 `UNSAFE` 程序集是等同的。 但是，`EXTERNAL_ACCESS` 程序集提供了 `UNSAFE` 程序集所不具备的各种可靠性和健壮性保护。 指定`UNSAFE`将允许程序集中的代码对执行非法操作[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]。 有关在中[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]创建 clr 程序集的详细信息，请参阅[管理 Clr 集成程序集](../../../relational-databases/clr-integration/assemblies/managing-clr-integration-assemblies.md)。  
