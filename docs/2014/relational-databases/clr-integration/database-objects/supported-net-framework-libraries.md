...
> [!NOTE]  
>  如果服务或升级不受支持的程序集在全局程序集缓存 (GAC) 中，你[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]。 如果程序集中均存在[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]CLR 集成。 如果在 GAC 中提供服务或升级的任何程序集（包括不支持的 .NET Framework 程序集）同时已在数据库中注册，请确保使用 `ALTER ASSEMBLY` 语句对 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 数据库中的程序集副本同时提供相应服务或升级。 有关详细信息，请参阅[知识库文章 949080](http://support.microsoft.com/kb/949080)。  
  
## <a name="supported-libraries"></a>支持的库  
 开头[!INCLUDE[ssVersion2005](../../../includes/ssnoversion-md.md)]具有受支持的.NET Framework 库，已经过测试以确保它们满足可靠性和安全性的标准，用于与交互的列表[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]加载它们直接从全局程序集缓存 (GAC)。  
...
## <a name="unsupported-libraries"></a>不支持的库  
 ...
 例如，不支持 `System.DirectoryServices` 命名空间。 您必须使用 `UNSAFE` 权限注册 System.DirectoryServices.dll 程序集，然后才能从代码中调用它。 `UNSAFE` 权限是必需的，这是因为 `System.DirectoryServices` 命名空间中的类不满足 `SAFE` 或 `EXTERNAL_ACCESS` 要求。 有关详细信息，请参阅[CLR 集成编程模型限制](clr-integration-programming-model-restrictions.md)和[CLR Integration Code Access Security](../security/clr-integration-code-access-security.md)。  
