...
-   Db-library 应用程序调用**BCP_SETL**之前连接到启用 DBPROCESS 上的大容量复制操作。 ODBC 应用程序改为调用[SQLSetConnectAttr](../native-client-odbc-api/sqlsetconnectattr.md)连接以启用连接句柄上的大容量操作之前：  
...
-   指定 Db-library 值[bcp_bind](../native-client-odbc-extensions-bulk-copy-functions/bcp-bind.md)*varlen*参数解释不同于 ODBC **bcp_bind * * * cbData*参数。  
...
-   Db-library  **bcp_colfmt * * * file_collen*和 ODBC [bcp_colfmt](../native-client-odbc-extensions-bulk-copy-functions/bcp-colfmt.md)* cbUserData * 具有同样的问题作为 **bcp_bind * * * varlen*和*cbData*参数如上所示。 更改任何 Db-library *file_collen* -1 到 SQL_VARLEN_DATA 及任何规范*file_collen* 0 到 SQL_NULL_DATA 的规范。  
  
-   *IValue* ODBC 参数[bcp_control](../native-client-odbc-extensions-bulk-copy-functions/bcp-control.md)函数是 void 指针。 在 DB 库， *iValue*为整数。 将值转换为 ODBC *iValue*为 void *。  
