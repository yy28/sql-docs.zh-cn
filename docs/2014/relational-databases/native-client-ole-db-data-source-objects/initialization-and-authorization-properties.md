...
|DBPROP_INIT_ASYNCH|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client OLE DB 访问接口支持异步启动。<br /><br /> 如果设置 DBPROP_INIT_ASYNCH 属性中的 DBPROPVAL_ASYNCH_INITIALIZE 位，将导致 IDBInitialize::Initialize 成为非阻止调用。 有关详细信息，请参阅[正在执行异步操作](../native-client/features/performing-asynchronous-operations.md)。|  
...
|属性 ID|描述|  
|-----------------|-----------------|  
|SSPROP_AUTH_OLD_PASSWORD|键入：VT_BSTR<br /><br /> R/W:写入<br /><br /> 默认值：VT_EMPTY<br /><br /> 说明:当前或过期的密码。 有关详细信息，请参阅[更改密码以编程方式](../native-client/features/changing-passwords-programmatically.md)。|  
...
|SSPROP_INIT_DATATYPECOMPATIBILITY|键入：VT_UI2<br /><br /> R/W:读/写<br /><br /> 默认值：0<br /><br /> 说明:使数据类型之间的兼容性[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]和 ActiveX 数据对象 (ADO) 应用程序。 如果使用默认值 0，则数据类型处理将默认采用由访问接口使用的方案。 如果使用值 80，则数据类型处理仅使用 [!INCLUDE[ssVersion2000](../../includes/ssversion2000-md.md)] 数据类型。 有关详细信息，请参阅[使用 SQL Server Native Client 使用 ADO](../native-client/applications/using-ado-with-sql-server-native-client.md)。|  
...
|SSPROP_INIT_MARSCONNECTION|键入：VT_BOOL<br /><br /> R/W:读/写<br /><br /> 默认值：VARIANT_FALSE<br /><br /> 说明:指定是否连接启用多个活动结果集 (MARS)。 必须在与数据库建立连接之前将该选项设置为 true。 有关详细信息，请参阅[使用多个活动的结果集 (MARS)](../native-client/features/using-multiple-active-result-sets-mars.md)。|  
...
|SSPROP_INIT_NETWORKLIBRARY|键入：VT_BSTR<br /><br /> R/W:读/写<br /><br /> 说明:用于与的实例进行通信的网络库 (DLL) 名称[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。 该名称不应当包含路径或 .dll 文件扩展名。<br /><br /> 可以使用 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 客户端配置实用工具来自定义其默认值。 **注意：** 此属性支持只有 TCP 和命名管道。 如果该属性在使用时带有前缀，最后将得到导致错误的双前缀，因为该属性用于在内部生成前缀。|  
