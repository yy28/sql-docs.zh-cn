...
|属性 ID|Description|  
|-----------------|-----------------|  
|SSPROP_ENABLEFASTLOAD|R/W：读/写 默认值：VARIANT_FALSE<br /><br /> 说明：若要从内存启用大容量复制，应将 SSPROP_ENABLEFASTLOAD 属性设置为 VARIANT_TRUE。 通过将数据源上设置此属性，新创建的会话允许使用者访问[IRowsetFastLoad](../native-client-ole-db-interfaces/irowsetfastload-ole-db.md)接口。<br /><br /> 如果该属性设置为 VARIANT_TRUE， **IRowsetFastLoad**接口是可通过**IOpenRowset::OpenRowset**通过请求**IID_IRowsetFastLoad**接口或通过设置**SSPROP_IRowsetFastLoad**到 VARIANT_TRUE。|  
