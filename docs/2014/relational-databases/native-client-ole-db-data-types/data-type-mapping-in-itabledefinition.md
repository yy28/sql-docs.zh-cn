...
|DBTYPE_UDT|**UDT**|在使用以下信息`DBCOLUMNDESC`结构通过**ITableDefinition::CreateTable**需要 UDT 列时：<br /><br /> -   *pwSzTypeName*将被忽略。<br />-   *rgPropertySets*必须包括`DBPROPSET_SQLSERVERCOLUMN`属性设置在上一部分中所述`DBPROPSET_SQLSERVERCOLUMN`中[使用用户定义类型](../native-client/features/using-user-defined-types.md)。|  
|DBTYPE_UI1|**tinyint**||  
