...
-   **更改时阻止**-如果为，则在 `True` 数据库架构不同于在之前的 DAC 中定义的架构时，升级将终止。 如果为 `False`，则即使检测到更改，升级也将继续。 默认设置为 `False`。  
  
-   **失败时回滚**-如果为 `True` ，则在事务中包含升级，如果遇到错误，则将尝试回滚。 如果为 `False`，则在进行更改时所有更改都将提交，并且在出错时，您可能要还原数据库的以前的备份。 默认设置为 `False`。  
  
-   **跳过策略验证**-如果为 `True` ，则不评估 DAC 服务器选择策略。 如果为 `False`，将评估策略，并且在存在验证错误时升级将终止。 默认设置为 `False`。  
