...
# <a name="diagnostic-records-and-fields"></a>诊断记录和字段
 ...
 标题记录中的字段包含有关函数执行的一般信息，包括返回代码、行计数、状态记录数和所执行的语句的类型。 除非 ODBC 函数返回 SQL_INVALID_HANDLE，否则，总是会创建标题记录。 标头记录中的字段的完整列表，请参阅[SQLGetDiagField](../native-client-odbc-api/sqlgetdiagfield.md)。  
