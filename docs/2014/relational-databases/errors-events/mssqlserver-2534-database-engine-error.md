...
|消息正文|表错误：页 P_ID 的页头指示它分配给对象 ID O_ID，索引 ID I_ID，分区 ID PN_ID，分配单元 ID A_ID （类型为 TYPE），另一个对象分配。|  
...
> [!CAUTION]  
>  对匹配的 [MSSQLSERVER_2533](mssqlserver-2533-database-engine-error.md) 错误运行 REPAIR 将在重新生成之前释放页。  
