...
|消息正文|订阅服务器 (%s)（属于原始发布服务器“%s”）在重定向的发布服务器“%s”中不显示为远程服务器。 运行`sp_addlinkedserver`重定向的发布服务器添加为远程服务器的订阅者。|  
  
## <a name="explanation"></a>解释  
 `sp_validate_redirected_publisher` 使用在远程服务器的发布服务器数据库的订阅元数据表来标识其关联的订阅服务器并验证中的订阅服务器的 master.dbo.sysservers 有关联的项。 如果任何标识的订阅服务器不存在，则会返回此错误。  
