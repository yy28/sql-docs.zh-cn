...
> [!NOTE]  
>  您必须拥有相应级别的权限才能在服务器上创建管理数据仓库。 有关详细信息，请参阅 [CREATE DATABASE (SQL Server Transact-SQL)](/sql/t-sql/statements/create-database-sql-server-transact-sql)。 您还必须拥有为管理数据仓库角色创建登录名的相应级别权限。  
