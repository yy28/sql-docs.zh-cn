 ...
 **注意**每个表只允许有一个聚集索引。 每个表只允许创建一个 xVelocity 内存优化的列存储索引。  
 ...
 **添加**  
 向索引键添加列。 在单击 "**添加**" 时显示的 "从* \<表名***中选择列**>" 对话框中选择表列。 对于空间索引，在选择一列后，该按钮将呈灰色。  
 ...
 **包含列**  
 在非聚集索引中包含非键列。 使用此选项，您可以将列作为非键列添加到非聚集索引的叶级别中，从而跳过当前对索引键总大小的索引限制以及对构成索引键的最大列数的索引限制。 有关详细信息，请参阅[创建包含列的索引](create-indexes-with-included-columns.md)  
  
##  <a name="Columns"></a>"选择（索引）列" 对话框  
...
### <a name="grids"></a>网格  
 此面板显示分割方案的每个级别上网格的密度。 密度可指定为 **“低”**、 **“中”** 或 **“高”** 三个级别。 默认值为 **“中”**。 "**低**" 表示4x4 网格（16个单元格），"**中**" 表示8x8 网格（64个单元），"**高**" 表示16x16 网格（256单元格）。 在选择 **“几何自动网格”** 或 **“地理自动网格”** 分割选项时，这些选项将不可用。  
