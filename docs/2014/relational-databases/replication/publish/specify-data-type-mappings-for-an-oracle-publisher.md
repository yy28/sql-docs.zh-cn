...
4.  在分发服务器上，执行 [sp_changearticlecolumndatatype](/sql/relational-databases/system-stored-procedures/sp-changearticlecolumndatatype-transact-sql)。 指定发布** \@服务器**的 Oracle 发布服务器的名称，以及** \@发布**、 ** \@项目**和** \@列**以定义已发布的列。 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]为 " ** \@类型**" 指定要映射到的数据类型的名称，以及 " ** \@长度**"、 ** \@"精度**" 和** \@"小数位数**" （如果适用）。  
