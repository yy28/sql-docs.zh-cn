...
1.  在发布服务器上，对发布数据库执行 [sp_addmergearticle](/sql/relational-databases/system-stored-procedures/sp-addmergearticle-transact-sql)。 指定项目所属的** \@发布的名称、项目** ** \@的名称、要**为** \@source_object**发布的数据库对象，以及[|（位或）](/sql/t-sql/language-elements/bitwise-or-transact-sql)schema_option 的一个或多个架构选项的结果。 ** \@** 有关详细信息，请参阅 [定义项目](define-an-article.md)。  
