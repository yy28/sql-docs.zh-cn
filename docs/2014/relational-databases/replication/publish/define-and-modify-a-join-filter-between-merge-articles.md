...
4.  指定联接选项：  
    ...
    -   默认情况下，合并复制在同步过程中会逐行处理更改。 若要将筛选的表和联接的表行中的相关更改作为一个单元进行处理，则选择 **“逻辑记录”** ([!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)] 和更高版本）。 只有满足使用逻辑记录的项目和发布要求，此选项才可用。 有关详细信息，请参阅[通过逻辑记录对相关行的更改进行分组](../merge/group-changes-to-related-rows-with-logical-records.md)中的“使用逻辑记录的注意事项”部分。  
