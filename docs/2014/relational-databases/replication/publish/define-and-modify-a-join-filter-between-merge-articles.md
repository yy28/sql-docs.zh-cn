...
3.  创建联接语句：选择 **“使用生成器创建语句”** 或 **“手动编写联接语句”**。  
    ...
    -   如果选择使用生成器，请使用网格中的列（**“连接”**、 **“筛选的表列”**、 **“运算符”**、和 **“联接的表列”**）生成联接语句。  
         ...
         网格中的每一列都包含一个下拉组合框，使你可以选择两个列和一个运算符**=**（ **<>**、 **<=**、 **\<**、 **>=**、 **>**、和**等）。** 结果显示在 **“预览”** 文本区域中。 如果联接中涉及多个列对，则从 **“连接词”** 列中选择一个连接词（AND 或 OR），然后再输入两列和一个运算符。  
...
4.  指定联接选项：  
    ...
    -   默认情况下，合并复制在同步过程中会逐行处理更改。 若要将筛选的表和已联接表的行中的相关更改作为一个单元进行处理，请选择[!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)] "**逻辑记录**" （仅限和更高版本）。 只有满足使用逻辑记录的项目和发布要求，此选项才可用。 有关详细信息，请参阅[通过逻辑记录对相关行的更改进行分组](../merge/group-changes-to-related-rows-with-logical-records.md)中的“使用逻辑记录的注意事项”部分。  
...
3.  在发布服务器上，对发布数据库执行 [sp_addmergefilter &#40;Transact-SQL&#41;](/sql/relational-databases/system-stored-procedures/sp-addmergefilter-transact-sql)。 指定**@publication**、的此筛选器的唯一名称**@filtername**、在步骤2中创建的子项目的名称**@article**、要联接到的**@join_articlename**父项目的名称，以及以下值之一： **@join_unique_key**  
    ...
    > [!CAUTION]  
    >  仅当**@join_unique_key**对父项目的基础表中的联接列具有保证唯一性的约束时，才设置为**1** 。 如果**@join_unique_key**错误地设置为**1** ，则可能无法收敛数据。  
