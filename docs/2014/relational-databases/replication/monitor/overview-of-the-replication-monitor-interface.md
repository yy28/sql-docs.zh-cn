...
# <a name="overview-of-the-replication-monitor-interface"></a>复制监视器界面概述
  [!INCLUDE[msCoName](../../../includes/msconame-md.md)][!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]复制监视器以两个窗格的形式显示了以发布服务器为中心的视图或以分发服务器为中心的所有复制活动。 在监视器的左窗格中添加发布服务器后，监视器的右窗格中即显示发布服务器、其发布、对这些发布的订阅和各种复制代理的相关信息。 除了显示有关复制拓扑的信息以外，复制监视器还可用于执行多种任务，如启动和停止代理以及验证数据。  
...
-   "**所有订阅" 选项卡**和 **"订阅监视列表" 选项卡。**  
  
     本主题的前面已对这两个选项卡进行了说明。  
  
-   **发布服务器到分发服务器的历史记录**选项卡（仅适用于事务复制）  
