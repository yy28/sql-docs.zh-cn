...
|快照代理|[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]设置用户帐户 ：复制快照|快照：Delivered Cmds/sec|每秒传递到分发服务器的命令数。|  
|快照代理|[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]设置用户帐户 ：复制快照|快照：传递的事务/秒|每秒传递到分发服务器的事务数。|  
|日志读取器代理|[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]设置用户帐户 ：复制日志读取器|日志读取器：Delivered Cmds/sec|每秒传递到分发服务器的命令数。|  
|日志读取器代理|[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]设置用户帐户 ：复制日志读取器|日志读取器：传递的事务/秒|每秒传递到分发服务器的事务数。|  
|日志读取器代理|[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]设置用户帐户 ：复制日志读取器|日志读取器：传递延迟|从事务应用于发布服务器起，到传递给分发服务器为止所经过的时间（以毫秒为单位）。|  
|分发代理|[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]设置用户帐户 ：复制程序分发|Dist:Delivered Cmds/sec|每秒传递到订阅服务器的命令数。|  
|分发代理|[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]设置用户帐户 ：复制程序分发|Dist:传递的事务/秒|每秒传递到订阅服务器的事务数。|  
|分发代理|[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]设置用户帐户 ：复制程序分发|Dist:传递延迟|从事务传递给分发服务器起，到应用于订阅服务器为止所经过的时间（以毫秒为单位）。|  
