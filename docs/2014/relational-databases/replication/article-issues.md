...
|要求视图所引用的表。|[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 要求已发布的视图和索引视图引用的所有表在订阅服务器上均应可用。 如果没有将被引用表作为此发布中的项目发布，则必须在订阅服务器上手动创建这些对象。|使用 **“上一步”** 按钮导航到 **“项目”** 页。 添加所需的所有对象。|  
