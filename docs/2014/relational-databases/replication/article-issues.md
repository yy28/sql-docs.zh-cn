...
ms.locfileid: "63032021"
---
# <a name="article-issues"></a>项目问题
  "**项目问题**" 页列出了为项目找到的条件，以及这些条件所需的任何更改。 下表列出了可能出现的问题以及确保复制和现有应用程序正常运行所必需的操作：  
...
|要求视图所引用的表。|[!INCLUDE[msCoName](../../includes/msconame-md.md)][!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]要求已发布的视图和索引视图引用的所有表在订阅服务器上都可用。 如果没有将被引用表作为此发布中的项目发布，则必须在订阅服务器上手动创建这些对象。|使用 **“上一步”** 按钮导航到 **“项目”** 页。 添加所需的所有对象。|  
|要求存储过程引用的对象。|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]要求已发布存储过程引用的所有对象（例如，表和用户定义函数）在订阅服务器上都可用。 如果没有将被引用的对象发布为该发布中的项目，则必须在订阅服务器上手动创建这些对象。|使用 **“上一步”** 按钮导航到 **“项目”** 页。 添加所需的所有对象。|  
