...
# <a name="configure-iis-7-for-web-synchronization"></a>配置 IIS 7 以实现 Web 同步
 ...
 配置 IIS 7 是启用 Web 同步所需的三个步骤中的第一步。  
...
4.  在 **“Internet 信息服务(IIS)管理器”** 中创建应用程序池：  
    ...
    > [!NOTE]  
    >  如果您预计并发同步客户端超过两个，则最好创建 Web 园。 有关详细信息，请参阅[配置 Web 同步](configure-web-synchronization.md)中的“创建 Web 园”。  
