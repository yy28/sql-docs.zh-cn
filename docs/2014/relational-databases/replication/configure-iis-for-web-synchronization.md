...
-   使用基本身份验证。 建议使用基本身份验证，因为它不需要 Kerberos 委托就可以在单独的计算机上运行 IIS 和 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 发布服务器/分发服务器（建议配置）。 将 SSL 与基本身份验证一起使用可以确保登录名、密码和所有数据都在传输中加密。 （无论使用何种类型的身份验证，都需要 SSL。）有关 Web 同步的最佳做法的详细信息，请参阅[配置 Web 同步](configure-web-synchronization.md)中的“Web 同步的最佳安全配置”。  
