...
```  
 ...
 在此注册表项之下有一个项列表，每个项对应于计算机上安装的每个 LocalDB。 每个这些密钥的名称中有格式的 LocalDB 版本号*\<主要版本 >*。*\<次要版本 >* (例如，为项[!INCLUDE[ssSQL14](../../includes/sssql14-md.md)]名为 12.0)。 在每个版本项之下都有一个 `InstanceAPIPath` 名称-值对，用于定义指向随该版本一起安装的 SQLUserInstance.dll 文件的完全路径。 下面的示例显示一个安装了 LocalDB 版本 11.0 和 12.0 的计算机的注册表项：  
