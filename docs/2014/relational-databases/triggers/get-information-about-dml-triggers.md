...
4.  在 "**对象依赖关系**" 窗口中，若要查看依赖于 DML 触发器的对象，请选择 **" \<DML trigger name> 依赖于的对象**"。 将在 **“依赖关系”** 区域显示这些对象。  
  
     若要查看 DML 所依赖的对象，请选择 " ** \<DML trigger name> 依赖于的对象**"。 将在 **“依赖关系”** 区域显示这些对象。 展开每个节点以查看所有对象。  
