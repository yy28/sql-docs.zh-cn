 ...
 **ISSAsynchStatus::GetStatus**方法的行为完全相同**IDBAsynchStatus::GetStatus**方法处在于如果初始化的数据源对象被中止，E_UNEXPECTED 返回而不是比 DB_E_CANCELED (尽管[ISSAsynchStatus::WaitForAsynchCompletion](issasynchstatus-waitforasynchcompletion-ole-db.md)将返回 DB_E_CANCELED)。 这是因为在中止后，数据源对象不会照常处于僵停状态，以便进一步尝试初始化操作。  
