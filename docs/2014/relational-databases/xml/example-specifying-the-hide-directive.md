...
```  
 ...
 此查询使用 [xml](/sql/t-sql/xml/query-method-xml-data-type) 数据类型的 **query() 方法（xml 数据类型）** 来查询 **xml** 类型的 CatalogDescription 列，以便检索摘要说明。 此查询还使用 [xml](/sql/t-sql/xml/value-method-xml-data-type) 数据类型的 **value() 方法（xml 数据类型）** 从 CatalogDescription 列中检索 ProductModelID 值。 所得到的 XML 中不需要此值，但对生成的行集进行排序时需要此值。 因此，列名称 `[Summary!2!ProductModelID!HIDE]`包括 **HIDE** 指令。 如果 SELECT 语句中不包括此列，您将不得不按 `[ProductModel!1!ProdModelID]` 和 `[Summary!2!SummaryDescription]` xml **类型的** 对行集进行排序，并且不能在 ORDER BY 中使用 **xml** 类型列。 因此，将另外添加一个 `[Summary!2!ProductModelID!HIDE]` 列，然后在 ORDER BY 子句中指定该列。  
