...
|**\<xsd:choice>**|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]拒绝具有不具有子级的粒子的架构 **\<xsd:choice>** ，除非该粒子的**minOccurs**属性值定义为零。|  
|**\<xsd:include>**|当前， [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 不支持此元素。 服务器拒绝包含此元素的 XML 架构。<br /><br /> 作为解决方案，可对包含指令的 XML 架构进行 **\<xsd:include>** 预处理，以将任何包含的架构的内容复制并合并到单个架构中以上载到服务器。 有关详细信息，请参阅 [预处理架构以便合并包括的架构](preprocess-a-schema-to-merge-included-schemas.md)。|  
...
|**xs： QName**|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]不支持使用 XML 架构限制元素从**xs： QName**派生的类型。<br /><br /> [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]不支持将**xs： QName**作为成员元素的联合类型。<br /><br /> 有关详细信息，请参阅 [The xs:QName Type](the-xs-qname-type.md)。|  
...
|ID 属性|每个 XML 架构组件可在其上具有 ID 属性。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]对 **\<xsd:attribute>** **ID**类型的声明强制唯一性，但不存储这些值。 唯一性的强制的作用范围是 {CREATE &#124; ALTER} XML SCHEMA COLLECTION 语句。|  
...
|本地命名空间|必须为元素显式指定本地命名空间 **\<xsd:any>** 。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 拒绝使用空字符串 ("") 作为命名空间属性的值的架构。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 而是要求显式使用“##local”以指示作为通配符实例的未限定元素或属性。|  
...
|主题|说明|  
|-----------|-----------------|  
|[规范格式和模式限制](canonical-forms-and-pattern-restrictions.md)|说明规范格式和模式限制。|  
|[通配符组成部分和内容验证](wildcard-components-and-content-validation.md)|介绍了使用通配符、宽松验证和任何带有 XML 架构集合的类型元素的限制。|  
|[<xsd: redefine> 元素](the-xsd-redefine-element.md)|介绍使用元素的限制 \<xsd:redefine> ，并介绍了解决方法。|  
|[xs:QName 类型](the-xs-qname-type.md)|介绍与 xs:QName 类型有关的限制。|  
|[<xsd:simpleType> 声明的值](values-for-xsd-simpletype-declarations.md)|描述应用于声明的限制 \<xsd:simpleType> 。|  
|[枚举方面](enumeration-facets.md)|介绍与枚举方面有关的限制。|  
|[混合类型和简单内容](mixed-type-and-simple-content.md)|介绍将混合类型限制为简单内容的限制。|  
