...
# <a name="canonical-forms-and-pattern-restrictions"></a>规范格式和模式限制
 ...
 因此，你应小心时将模式方面应用到从以下基元类型派生的类型： `boolean`， `decimal`， `float`， `double`， `dateTime`， `time`， `date`， `hexBinary`和`base64Binary`。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 将发出警告。  
