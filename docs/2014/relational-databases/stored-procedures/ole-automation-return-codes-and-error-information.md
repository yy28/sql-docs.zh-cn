...
ms.locfileid: "85062789"
---
# <a name="ole-automation-return-codes-and-error-information"></a>OLE 自动化返回代码和错误信息
  OLE 自动化系统存储过程返回一个 `int` 返回代码，该代码是基础 OLE 自动化操作返回的 HRESULT。 HRESULT 为 0 表示成功。 非零 HRESULT 是十六进制形式 0x800*nnnnn*的 OLE 错误代码，但在 `int` 存储过程返回代码中作为值返回时，HRESULT 的形式为 214*nnnnnnn*。  
  
 例如，传递无效对象名称（SQLDMO）。Sqldmo.xyzzy）到 sp_OACreate 会导致过程返回2147221005的 `int` HRESULT，这是0x800401f3 的十六进制。  
  
 可以使用 `CONVERT(binary(4), @hresult)` 将 `int` HRESULT 转换为 `binary` 值。 但是，使用 `CONVERT(char(10), CONVERT(binary(4), @hresult))` 将产生一个不可读的字符串，因为 HRESULT 的每个字节都被转换成一个 ASCII 字符。 您可以使用以下示例 HexToChar 存储过程将 `int` HRESULT 转换为 `char` 包含可读十六进制字符串的值。  
