...
ms.locfileid: "36129453"
---
# <a name="fetch-and-update-rowsets-odbc"></a>提取和更新行集 (ODBC)
    
### <a name="to-fetch-and-update-rowsets"></a>提取和更新行集  
  
1.  （可选） 调用[SQLSetStmtAttr](../../native-client-odbc-api/sqlsetstmtattr.md)与 SQL_ROW_ARRAY_SIZE 若要更改的行集中的行 (R) 数。  
...
3.  如果使用绑定列，则在行集的绑定列缓冲区中现在可以使用数据值和数据长度。  
    ...
    -   调用[SQLGetData](../../native-client-odbc-api/sqlgetdata.md)最后一个绑定的行集列之后，若要获取的数据的一个或多个时间未绑定列。 调用[SQLGetData](../../native-client-odbc-api/sqlgetdata.md)应递增列号的顺序。  
...
6.  （可选） 执行的定位的 UPDATE 或 DELETE 语句，指定游标名称 (可从[SQLGetCursorName](../../native-client-odbc-api/sqlgetcursorname.md)) 和同一个连接上使用不同的语句句柄。  
