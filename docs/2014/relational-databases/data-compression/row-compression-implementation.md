...
-   它对于数值类型使用可变长度存储格式 (例如`integer`， `decimal`，和`float`) 和基于数值类型 (例如`datetime`和`money`)。  
...
|`float`|是|不存储带零的最低有效字节。 `float` 压缩是主要适用于尾数中的值。|  
|`real`|是|不存储带零的最低有效字节。 `real` 压缩是主要适用于尾数中的值。|  
...
|`datetimeoffset`|是|类似于`datetime2`，只不过有 2 个字节的时区的格式 (hh: mm)。<br /><br /> 就像 `datetime2`，压缩可以节省 2 个字节。<br /><br /> 对于时区值，在大多数情况下 MM 值都可能是 0。 因此，压缩可能会节省 1 个字节。<br /><br /> 进行行压缩时存储不做任何更改。|  
