...
## <a name="conversions"></a>转换  
 本主题介绍在客户端上执行的转换。 如果客户端指定的参数的秒的小数部分精度不同于服务器上定义的精度，那么，客户端转换将在该服务器允许成功执行该操作时导致失败。 具体而言，客户端视为任何截断秒的小数部分的错误，而[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]将时间舍入为最接近的整数秒数的值。  
...
|12|转换语义[!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)]为 DBTYPE_DATE 维护。 秒的小数部分被截断为零。|  
|13|转换语义[!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)]维护之前的 DBTYPE_FILETIME。 如果使用 Windows FileTimeToSystemTime API，秒的小数部分精度则限制为 1 毫秒。|  
|14|转换语义[!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)]为`smalldatetime`进行维护。 秒设置为零。|  
|15|转换语义[!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)]为`datetime`进行维护。 秒舍入为最接近的 1/300 秒。|  
