---
title: ClientNetworkProtocolProperty 类 |Microsoft 文档
ms.custom: ''
ms.date: 06/13/2017
ms.prod: sql-server-2014
ms.reviewer: ''
ms.suite: ''
ms.technology:
- database-engine
- docset-sql-devref
ms.tgt_pltfrm: ''
ms.topic: reference
api_name:
- ClientNetworkProtocolProperty Class
api_location:
- sqlmgmproviderxpsp2up.mof
topic_type:
- apiref
helpviewer_keywords:
- ClientNetworkProtocolProperty class
ms.assetid: a6e8adb5-2ffd-4c49-b9e4-9a1e459ff7b4
caps.latest.revision: 28
author: JennieHubbard
ms.author: jhubbard
manager: jhubbard
ms.openlocfilehash: efa9a28a319a215b6b77af09701a266e11cee629
ms.sourcegitcommit: 5dd5cad0c1bbd308471d6c885f516948ad67dfcf
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 06/19/2018
ms.locfileid: "36127666"
---
# <a name="clientnetworkprotocolproperty-class"></a>ClientNetworkProtocolProperty 类
  [ClientNetworkProtocolProperty 类](clientnetworkprotocolproperty-class.md)表示的客户端网络协议的属性。  
  
 与[ClientNetworkProtocolProperty 类](clientnetworkprotocolproperty-class.md)，你可以执行以下任务：  
  
-   设置一个用于从属性列表选择客户端网络协议属性的索引值。  
  
-   检查当前属性所属的协议。  
  
-   检查当前属性的名称、类型和值。  
  
-   修改当前属性的属性字符串值和数值。  
  
-   设置当前属性的标志。  
  
## <a name="see-also"></a>请参阅  
 [配置客户端协议](http://technet.microsoft.com/library/ms181035.aspx)  
  
  