...
-   [将逐个字段数据存储在非 XML 格式化文件中](#FieldByFieldNonXmlFF)  
...
-   文件存储类型  
     ...
     *文件存储类型*描述如何将数据存储在数据文件中。 数据可以按其数据库表类型（本机格式）、字符表示形式（字符格式）或支持隐式转换的任何数据类型导出到数据文件中；例如，以 `smallint` 形式复制 `int`。 用户定义的数据类型将按其基类型导出。 有关详细信息，请参阅[使用 bcp 指定文件存储类型 (SQL Server)](specify-file-storage-type-by-using-bcp-sql-server.md)。  
...
##  <a name="FieldSpecificPrompts"></a>特定于字段的提示概述  
...
```  
 ...
 对于每列，bcp 都提示输入字段特定的值。 以下示例显示了针对表中 `EmployeeID` 和 `Name` 两个字段的提示，并为每列提供了建议的默认文件存储类型（本机格式）。 
  `EmployeeID` 和 `Name` 列的前缀长度分别为 0 和 2。 用户指定英文逗号 (`,`) 作为每个字段的终止符。  
...
##  <a name="FieldByFieldNonXmlFF"></a>将字段中的数据存储在非 XML 格式化文件中  
