...
###  <a name="number"></a> 点数  
 所有非空`geometry`实例由组成*点*。 这些点表示在其上绘制几何图形的面的 X 和 Y 坐标。 `geometry` 提供许多用于查询实例的点的内置方法。  
...
```  
 ...
 边界`Polygon`或`MultiPolygon`实例是其环的集合。  
...
##  <a name="rel"></a> 确定几何图形实例之间的关系  
 `geometry`数据类型提供了许多内置方法可用于确定两个之间的关系`geometry`实例。  
