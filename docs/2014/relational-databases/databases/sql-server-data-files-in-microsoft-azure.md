...
-   在此功能的当前版本中，不支持在 Azure 存储中存储 `FileStream` 数据。 可以将 `Filestream` 数据存储在 Azure 存储集成的本地数据库中，但不能使用 Azure 存储在计算机之间移动 Filestream 数据。 对于 `FileStream` 数据，建议继续使用传统方法在不同计算机之间移动与 Filestream 关联的文件（.mdf，.ldf）。  
  
-   目前，此新增强功能不支持多个 SQL Server 实例同时访问 Azure 存储中的相同数据库文件。 如果 ServerA 处于联机状态并且有一个活动数据库文件，并且如果 ServerB 是意外启动的，并且它还具有指向相同数据文件的数据库，则第二个服务器将无法启动该数据库，错误代码为**5120 无法打开物理文件 "%。\*ls "。操作系统错误% d： "% ls"** 。  
...
-   无法使用“Azure 存储中的 SQL Server 数据文件”功能在 Azure Blob 中存储内存 OLTP 数据。 这是因为内存中 OLTP 依赖于 `FileStream`，在此功能的当前版本中，不支持在 Azure 存储中存储 `FileStream` 的数据。  
  
-   使用 Azure 功能中的 SQL Server 数据文件时，SQL Server 将使用 `master` 数据库中的排序规则集执行所有 URL 或文件路径比较。  
...
3.  *错误代码 5120 无法打开物理文件 "%.\*ls"。操作系统错误 %d：“%ls”*    
    解决方法：目前，此新增强功能不支持多个 SQL Server 实例同时访问 Azure 存储中的相同数据库文件。 如果 ServerA 处于联机状态并且有一个活动数据库文件，并且如果 ServerB 是意外启动的，并且它还具有指向相同数据文件的数据库，则第二个服务器将无法启动该数据库，错误*代码为5120无法打开物理文件 "%。\*ls "。操作系统错误% d： "% ls"* 。  
