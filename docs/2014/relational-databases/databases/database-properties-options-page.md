 ...
 **ANSI NULLS 已启用**  
 指定等于 (`=`) 和不等于 (`<>`) 比较运算符在与 Null 值一起使用时的行为。 可能的值为 `True` （on）和 `False` （off）。 如果为 `True`，所有与空值的比较的结果值都为 UNKNOWN。 `False` `True` 如果这两个值都为 null，则非 UNICODE 值与 null 值的比较结果为。 有关详细信息，请参阅 [SET ANSI_NULLS (Transact-SQL)](/sql/t-sql/statements/set-ansi-nulls-transact-sql)。  
  
 **ANSI 填充已启用**  
 指定 ANSI 填充状态是开还是关。 允许的值为 `True` （开）和 `False` （off）。 有关详细信息，请参阅 [SET ANSI_PADDING (Transact-SQL)](/sql/t-sql/statements/set-ansi-padding-transact-sql)。  
  
 **ANSI 警告已启用**  
 对于几种错误条件指定 ISO 标准行为。 `True`如果为，则在聚合函数（如 SUM、AVG、MAX、MIN、STDEV、STDEVP、VAR、VARP 或 COUNT）中出现 null 值时，将生成警告消息。 如果 `False` 为，则不会发出任何警告。 有关详细信息，请参阅 [SET ANSI_WARNINGS (Transact-SQL)](/sql/t-sql/statements/set-ansi-warnings-transact-sql)。  
