 ...
 **递归触发器已启用**  
 指定触发器是否可以由其他触发器激发。 可能的值为 `True` 和 `False`。 当设置为`True`时，将启用触发器的递归激发。 当设置为`False`时，仅阻止直接递归。 若要禁用间接递归，请使用 sp_configure 将 nested triggers 服务器选项设置为 0。 有关详细信息，请参阅 [创建嵌套触发器](../triggers/create-nested-triggers.md)。  
