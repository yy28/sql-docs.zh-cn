...
|**EventSubClass** <sup>1</sup>|21|事件子类的类型，提供有关每个事件类的详细信息。 例如， **Execution Warning** 事件类的事件子类值代表执行警告的类型：<br /><br /> **1** = 查询等候。 查询执行之前必须等待资源；例如，内存。<br /><br /> **2** = 查询超时。查询等待执行所需资源时超时。 所有事件类都不填充此数据列。|  
...
|介绍如何使用存储过程创建只检索有关所需跟踪事件信息的筛选器。|[设置跟踪筛选器 (Transact-SQL)](../../ssms/agent/set-sql-server-alias-for-sql-server-agent-service-ssms.md)|  
|介绍如何使用存储过程修改现有跟踪。|[修改现有跟踪 (Transact-SQL)](../sql-trace/modify-an-existing-trace-transact-sql.md)|  
|介绍如何使用内置函数查看保存的跟踪。|[查看保存的跟踪 (Transact-SQL)](../sql-trace/view-a-saved-trace-transact-sql.md)|  
|介绍如何使用内置函数查看跟踪筛选器信息。|[查看筛选器信息 (Transact-SQL)](../sql-trace/view-filter-information-transact-sql.md)|  
|介绍如何使用存储过程删除跟踪。|[删除跟踪 (Transact-SQL)](../sql-trace/delete-a-trace-transact-sql.md)|  
