...
3.  也可以执行 **sp_trace_setfilter** 以设置任意筛选或筛选组合。  
    ...
    > [!IMPORTANT]  
    >  与常规存储过程不同参数的所有 SQL Server Profiler 存储过程 (<strong>sp_trace_*xx*</strong>) 已严格类型化和不支持自动数据类型转换。 如果这些参数不是使用正确的输入参数数据类型（正如参数说明中指定的一样）调用的，则存储过程会返回错误。  
