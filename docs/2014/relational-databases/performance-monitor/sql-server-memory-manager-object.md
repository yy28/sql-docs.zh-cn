...
# <a name="sql-server-memory-manager-object"></a>SQL Server Memory Manager 对象
  Microsoft [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]中的**Memory Manager**对象提供了计数器，用于监视服务器的总体内存使用情况。 监视总体的服务器内存使用情况，以估计用户活动和资源使用，有助于查明性能瓶颈。 监视 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的实例使用的内存有助于确定：  
...
|**锁定所有者块**|指定当前正在服务器上使用的锁拥有者块的数目（定期进行刷新）。 一个锁拥有者块代表一个独立线程对某一对象上的一个锁的拥有权。 因此，若三个线程在一个页上各有一个共享 (S) 锁，就会有三个锁拥有者块。|  
