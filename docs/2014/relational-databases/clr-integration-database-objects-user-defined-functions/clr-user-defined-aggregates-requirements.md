...
|方法|语法|Description|  
|------------|------------|-----------------|  
|`Init`|公共 void init （);|查询处理器使用此方法初始化聚合的计算。 对于查询处理器正在聚合的每个组调用此方法一次。 查询处理器可以选择重用聚合类的同一实例来计算多个组的聚合。 `Init` 方法应在上一次使用此实例后根据需要执行清除，并允许重新启动新的聚合计算。|  
|`Accumulate`|公共 void Accumulate （输入类型值 [，输入类型值，...]）;|表示该函数的参数的一个或多个参数。 *input_type*应托管[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]等效于本机数据类型[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]由指定的数据类型*input_sqltype*中`CREATE AGGREGATE`语句。 有关详细信息，请参阅[映射 CLR 参数数据](../clr-integration-database-objects-types-net-framework/mapping-clr-parameter-data.md)。<br /><br /> 对于用户定义类型 (UDT)，input-type 与 UDT 类型相同。 查询处理器使用此方法累计聚合值。 对于正在聚合的组中的每个值调用此方法一次。 查询处理器仅在为聚合类的指定实例调用 `Init` 方法之后才调用此方法。 此方法的实现应更新实例的状态以反映正在传递的参数值的累计。|  
|`Merge`|公共 void 合并 （udagg_class 值）;|此方法可以将此聚合的另一实例与当前实例合并。 查询处理器使用此方法合并聚合的多个部分计算。|  
|`Terminate`|公共 return_type terminate （);|此方法完成聚合计算并返回聚合的结果。 *Return_type*应托管[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]数据，它是类型的托管等效项*return_sqltype*中指定`CREATE AGGREGATE`语句。 *Return_type*也可以是用户定义的类型。|  
...
## <a name="see-also"></a>请参阅  
 [CLR 用户定义的类型](../clr-integration-database-objects-user-defined-types/clr-user-defined-types.md)   
