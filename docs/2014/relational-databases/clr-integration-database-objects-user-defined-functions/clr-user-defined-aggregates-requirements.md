...
|`Terminate`|public return_type Terminate （）;|此方法完成聚合计算并返回聚合的结果。 *Return_type*应为托管 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 数据类型，该类型是语句中指定的*return_sqltype*的托管等效项 `CREATE AGGREGATE` 。 *Return_type*也可以是用户定义的类型。|  
