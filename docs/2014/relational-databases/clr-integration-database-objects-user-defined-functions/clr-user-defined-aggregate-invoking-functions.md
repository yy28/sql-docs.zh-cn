...
>  不支持在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 中执行已使用 /clr:pure 编译器选项编译的 Visual C++ 数据库对象，例如标量值函数。  
 ...
 与大多数聚合相同，大部分逻辑位于 `Accumulate` 方法中。 在此，作为参数传入到 `Accumulate` 方法的字符串将追加到已在 `StringBuilder` 方法中初始化的 `Init` 对象中。 假如这不是第一次调用 `Accumulate` 方法，则在追加传入字符串之前还会将逗号追加到 `StringBuilder`。 在计算任务结束时会调用 `Terminate` 方法，该方法以字符串的形式返回 `StringBuilder`。  
...
## <a name="example-2"></a>示例 2  
 下面的示例演示 `Accumulate` 方法中有两个参数的聚合。  
