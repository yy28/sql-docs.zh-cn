...
## <a name="node-test-example-1"></a>节点测试：示例1  
 ...
 在本示例中，`child` 是轴，`Customer` 是节点测试。 `child`轴的主要节点类型是** \<元素>**。 因此，如果** \<Customer>** 节点是>节点的** \<元素**，则节点测试为 TRUE。 如果上下文节点没有** \<客户>** 子级，则返回一组空节点。  
  
## <a name="node-test-example-2"></a>节点测试：示例 2  
 位置路径`attribute::CustomerID`选择上下文节点的**CustomerID**属性。  
  
 在此示例中`attribute` ，是轴， `CustomerID`是节点测试。 `attribute`轴的主要节点类型为** \<attribute>**。 因此，如果**CustomerID**是>节点的** \<属性**，则节点测试为 TRUE。 如果上下文节点没有**CustomerID**，则返回一组空节点。  
