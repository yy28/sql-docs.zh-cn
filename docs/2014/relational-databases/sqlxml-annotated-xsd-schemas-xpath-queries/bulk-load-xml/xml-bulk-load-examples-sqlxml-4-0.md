...
5.  执行 VBScript 代码。  
     ...
     架构必须指定相应`sql:datatype`有关**CustomerID**属性时的值**CustomerID**作为指定为包含大括号 （{和}），此类的 GUID:  
    ...
    ```  
     ...
     时`sql:datatype`指定为将列类型标识`uniqueidentifier`，则大容量负载操作将删除大括号 （{和}） 从**CustomerID**前将其插入列中值。  
...
## <a name="j-bulk-loading-in-xml-data-type-columns"></a>J. 在 xml 数据类型列中执行大容量加载  
 如果映射架构指定了[xml 数据类型](/sql/t-sql/xml/xml-transact-sql)通过使用列`sql:datatype="xml"`批注，XML 大容量加载可以将 XML 子元素映射字段的源文档中复制到此列。  
  
 请看以下 XSD 架构，它映射 AdventureWorks 示例数据库中 Production.ProductModel 表的视图。 在此表中，CatalogDescription 字段`xml`数据类型映射到 **\<Desc >** 元素使用`sql:field`和`sql:datatype="xml"`批注。  
