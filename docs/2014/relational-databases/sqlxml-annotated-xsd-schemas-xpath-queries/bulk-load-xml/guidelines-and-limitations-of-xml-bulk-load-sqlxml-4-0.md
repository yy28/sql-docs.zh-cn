...
-   如果指定 SchemaGen 属性 (例如，SchemaGen = true)，则创建在映射架构中标识的表。 但 SchemaGen 不会创建这些表上的任何约束 （例如，PRIMARY KEY/FOREIGN KEY 约束中） 有一个例外：如果构成关系中的主键的 XML 节点被定义为具有 XML 类型的 ID (即， `type="xsd:ID"` xsd) 和 SGUseID 属性设置为 True SchemaGen，则不仅主键从创建 ID 类型的节点但从映射架构关系创建主键/外键关系。  
