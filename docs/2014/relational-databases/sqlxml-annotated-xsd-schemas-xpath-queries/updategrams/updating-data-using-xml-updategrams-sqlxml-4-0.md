...
```  
 ...
 `mapping-schema` 属性用于标识要由 Updategram 使用的映射架构。 如果属的 updategram 指定的映射架构，元素和属性名称中指定**\<之前 >** 和**\<后 >** 块必须与架构中的名称匹配。 该映射架构将这些元素或属性名称映射到数据库表和列名称。  
...
-   为 Updategram 中的键字段提供了一个或多个特定值。  
 ...
 如果是这种情况，属的 updategram 使用中指定的键列`sql:key-fields`以中的元素配对**\<之前 >** 和**\<后 >** 块。  
 ...
 如果指定中的有一个元素**\<之前 >** 块和中的相应元素只有一个**\<后 >** 一直阻止，请使用`updg:id`不需要。 但是，建议指定 `updg:id` 以避免多义性。  
...
```  
 ...
 请注意如何`updg:id`属性对的第一个实例\<HumanResources.Shift > 中的元素**\<之前 >** 第二个实例的块\<HumanResources.Shift > 中的元素**\<后 >** 块。  
...
3.  创建并使用 SQLXML 4.0 测试脚本 (Sqlxml4test.vbs) 以执行 updategram。  
 ...
 有关使用映射架构的 updategram 的更多示例，请参阅[指定批注映射架构中属的 Updategram &#40;SQLXML 4.0&#41;](specifying-an-annotated-mapping-schema-in-an-updategram-sqlxml-4-0.md)。  
