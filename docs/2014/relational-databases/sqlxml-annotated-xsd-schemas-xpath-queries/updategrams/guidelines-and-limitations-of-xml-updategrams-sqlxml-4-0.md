...
-   如果使用 updategram 修改二进制列中的数据（ [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] `image`例如数据类型），则必须提供一个映射架构，在该架构中，必须指定[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]数据类型（例如`sql:datatype="image"`）和 XML 数据类型（例如， `dt:type="binhex"`或`dt:type="binbase64`）。 必须在 updategram 中指定二进制列的数据；updategram 将忽略在映射架构中指定的 `sql:url-encode` 批注。  
...
-   当使用 updategram 时，在中>`text/ntext`块** \<之前**，不应在中使用二进制大型对象（BLOB）类型（如和），因为这将包含它们以用于并发控制。 由于 BLOB 类型的比较限制，这可能导致 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 出现问题。 例如，在 WHERE 子句中使用 LIKE 关键字比较 `text` 数据类型的列；不过，对于数据大小大于 8K 的 BLOB 类型，比较将失败。  
  
-   由于 BLOB 类型的比较限制，`ntext` 数据中的特殊字符可能导致 SQLXML 4.0 出现问题。 例如，在对`ntext`类型的列进行并发检查时，在 updategram 的** \<before>** 块中使用 "[Serializable]" 会失败，并出现以下 SQLOLEDB 错误说明：  
