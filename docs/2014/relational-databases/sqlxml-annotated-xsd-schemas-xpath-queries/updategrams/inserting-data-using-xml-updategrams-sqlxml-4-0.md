 ...
 **\<之前 >** 为插入操作，可以省略块。 如果可选`mapping-schema`属性未指定，  **\<ElementName >** ，指定在属的 updategram 映射到数据库表和子元素，或者属性映射到表中的列。  
...
## <a name="updgid-attribute"></a>updg:id 属性  
 如果 updategram 只是要插入记录，则 updategram 不需要 `updg:id` 属性。 有关详细信息`updg:id`，请参阅[更新数据使用 XML Updategram &#40;SQLXML 4.0&#41;](updating-data-using-xml-updategrams-sqlxml-4-0.md)。  
...
## <a name="updgguid-attribute"></a>updg:guid 属性  
 `updg:guid` 属性是一个生成全局唯一标识符的可选属性。 此值保持在范围内为整个**\<同步 >** 块中指定它。 你可以使用此值中的任意位置**\<同步 >** 块。 属性调用`NEWGUID()`[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]函数生成的唯一标识符。  
...
-   大多数示例使用默认映射（即，未在 updategram 中指定任何映射架构）。 有关使用映射架构的 updategram 的更多示例，请参阅[指定批注映射架构中属的 Updategram &#40;SQLXML 4.0&#41;](specifying-an-annotated-mapping-schema-in-an-updategram-sqlxml-4-0.md)。  
...
### <a name="d-using-the-at-identity-attribute-to-retrieve-the-value-that-has-been-inserted-in-the-identity-type-column"></a>D. 使用 at-identity 属性检索已在 IDENTITY 类型的列中插入的值  
 ...
 首先，updategram 向 Sales.SalesOrderHeader 表中添加一条记录。 在该表中，SalesOrderID 列为 IDENTITY 类型的列。 因此，在向该表添加此记录时，updategram 使用 `at-identity` 属性将已赋值的 SalesOrderID 值捕获为“x”（占位符值）。 Updategam 然后指定此`at-identity`变量中的 SalesOrderID 属性的值作为\<Sales.SalesOrderDetail > 元素。  
...
```  
 ...
 指定的映射架构的 updategram 的更多示例，请参阅[指定批注映射架构中属的 Updategram &#40;SQLXML 4.0&#41;](specifying-an-annotated-mapping-schema-in-an-updategram-sqlxml-4-0.md)。  
