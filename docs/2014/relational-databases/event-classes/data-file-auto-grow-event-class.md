...
ms.locfileid: "48229937"
---
# <a name="data-file-auto-grow-event-class"></a>Data File Auto Grow 事件类
  **Data File Auto Grow** 事件类指明数据文件已自动增长。 如果使用 ALTER DATABASE 语句显式增长数据文件，则不会触发此事件。  
...
|**Duration**|**bigint**|扩展文件所需的时间长度（毫秒）。|13|用户帐户控制|  
|**EndTime**|**datetime**|数据文件自动增长结束时间。|18|用户帐户控制|  
|**EventClass**|**int**|事件类型 = 92。|27|否|  
|**EventSequence**|**int**|**CursorClose** 事件类在批处理中的顺序。|51|否|  
|**Filename**|**nvarchar**|扩展的文件的逻辑名称。|36|用户帐户控制|  
|**HostName**|**nvarchar**|正在运行客户端的计算机的名称。 如果客户端提供了主机名，则填充此数据列。 若要确定主机名，请使用 HOST_NAME 函数。|8|用户帐户控制|  
|**IntegerData**|**int**|作为文件增量的 8 KB 页数。|25|用户帐户控制|  
...
|**NTDomainName**|**nvarchar**|[!INCLUDE[msCoName](../../includes/msconame-md.md)] 用户所属的 Windows 域。|7|用户帐户控制|  
