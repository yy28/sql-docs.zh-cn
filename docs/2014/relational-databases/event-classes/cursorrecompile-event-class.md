...
# <a name="cursorrecompile-event-class"></a>CursorRecompile 事件类
  **Cursorrecompile 事件类**事件类介绍了应用程序编程接口（API）游标中发生的游标重新编译事件。 当由于架构更改而重新[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssDE](../../includes/ssde-md.md)]编译 transact-sql 游标时，将发生游标重新编译事件。  
...
|**EventClass**|**int**|记录的事件类型 = 75。|27|否|  
|**EventSequence**|**int**|批处理中 **CursorRecompile** 事件类的顺序。|51|否|  
