...
ms.locfileid: "62662221"
---
# <a name="showplan-all-for-query-compile-event-class"></a>Showplan All for Query Compile 事件类
  当编译 SQL 语句时[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ，将发生 "显示计划所有" 查询编译事件类。 包括此事件类以标识 Showplan 运算符。 此事件类所包括的信息是 Showplan XML For Query Compile 事件类所包括信息的一部分。  
...
|SPID|`int`|
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 为客户端的相关进程分配的服务器进程 ID。|12|是|  
