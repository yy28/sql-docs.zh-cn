...
|EventSubClass|`nvarchar`|事件子类的类型，提供有关每个事件类的进一步信息。 此列可能包含下列值：<br /><br /> 注册的订阅：指示查询通知订阅在数据库中成功注册的时间。<br /><br /> 倒带订阅：指示[!INCLUDE[ssDE](../../includes/ssde-md.md)]收到完全符合现有订阅的订阅请求的时间。 在这种情况下， [!INCLUDE[ssDE](../../includes/ssde-md.md)] 将现有订阅的超时值设置为新订阅请求中指定的超时值。<br /><br /> 而激发订阅：指示通知订阅生成通知消息的时间。<br /><br /> 触发失败，出现代理错误：指示通知消息因 [!INCLUDE[ssSB](../../includes/sssb-md.md)] 错误而失败的时间。<br /><br /> 触发失败且不会 broker 错误：指示通知消息不因 [!INCLUDE[ssSB](../../includes/sssb-md.md)] 错误而失败的时间。<br /><br /> Broker 截获错误：指示 [!INCLUDE[ssSB](../../includes/sssb-md.md)] 已在查询通知使用的会话中传递错误。<br /><br /> 订阅删除尝试：指示[!INCLUDE[ssDE](../../includes/ssde-md.md)]已尝试删除过期的订阅以释放资源。<br /><br /> 订阅删除失败：指示尝试删除过期的订阅已失败。 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 将自动重新计划删除订阅以释放资源。<br /><br /> 销毁的订阅：指示[!INCLUDE[ssDE](../../includes/ssde-md.md)]已成功删除过期的订阅。|21|用户帐户控制|  
...
|LoginSID|`image`|已登录用户的安全标识号 (SID)。 您可以在 sys.server_principals 目录视图中找到此信息。 服务器中的每个登录名都具有唯一的 SID。|41|用户帐户控制|  
