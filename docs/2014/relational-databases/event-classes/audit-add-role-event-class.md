...
# <a name="audit-add-role-event-class"></a>Audit Add Role 事件类
  在数据库中添加或删除数据库角色时，会发生**Audit Add Role**事件类。 此事件类用于 **sp_addrole** 和 **sp_droprole** 存储过程。  
  
 此事件类可能会从的[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]未来版本中删除。 建议使用 **Audit Database Principal Management** 事件类。  
