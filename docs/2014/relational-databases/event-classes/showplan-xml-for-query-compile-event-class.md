...
ms.locfileid: "62650400"
---
# <a name="showplan-xml-for-query-compile-event-class"></a>Showplan XML For Query Compile 事件类
  当编译 SQL 语句时[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ，将发生显示计划 XML For Query Compile 事件类。 包括 thisevent 类，用于标识上[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]的显示计划运算符。  
...
|LoginSID|`image`|已登录用户的安全标识号 (SID)。 您可以在 sys.server_principals 目录视图中找到此信息。 服务器中的每个登录名都具有唯一的 SID。|41|否|  
