...
ms.locfileid: "63012536"
---
# <a name="audit-schema-object-take-ownership-event-class"></a>Audit Schema Object Take Ownership 事件类
  当检查更改架构对象（如表、过程或函数）的所有者的权限时，会发生**Audit Schema Object Take 所有权**事件类。 使用 ALTER AUTHORIZATION 语句指定对象所有者时会出现这种情况。  
...
|**ObjectType**|**int**|表示事件中涉及的对象类型的值。 为此列返回的值结合了 **sys.objects** 目录视图中的 **type** 列中的对应值以及 [ObjectType Trace Event Column](objecttype-trace-event-column.md)中列出的值。|28|是|  
