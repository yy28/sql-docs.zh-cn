...
|ObjectType|`int`|调用的存储过程类型。 此值对应于 sys.objects 目录视图中的类型列。 有关值的信息，请参阅 [ObjectType 跟踪事件列](objecttype-trace-event-column.md)。|28|用户帐户控制|  
...
|TextData|`ntext`|存储过程调用的文本。|1|用户帐户控制|  
