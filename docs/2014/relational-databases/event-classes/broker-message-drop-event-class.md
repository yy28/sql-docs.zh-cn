...
|**EventSubClass**|`nvarchar`|指明无法送达消息是否为排序的消息。 两个值之一：<br /><br /> **序列化消息**。 无法送达消息是排序的消息。<br /><br /> **未编序消息**。 无法送达消息不是排序的消息。|21|是|  
|**GUID.EMPTY**|`uniqueidentifier`|无法送达消息所属会话的会话 ID。 此标识符将作为消息的一部分进行传输，并在会话双方之间共享。|54|否|  
...
|**TextData**|`ntext`|
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 无法送达消息的原因。|1|是|  
