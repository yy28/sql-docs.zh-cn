...
-   计划指南定义中的提示（包括 USE PLAN 提示）未成功应用于查询或批处理。 也就是说，编译的查询计划无法遵守指定的提示，并且该计划将在不使用计划指南的情况下进行编译。  
 ...
 无效的计划指南可能导致此事件激发。 请使用 [sys.fn_validate_plan_guide](/sql/relational-databases/system-functions/sys-fn-validate-plan-guide-transact-sql) 函数来验证查询或批处理所使用的计划指南，然后更正此函数报告的错误。  
