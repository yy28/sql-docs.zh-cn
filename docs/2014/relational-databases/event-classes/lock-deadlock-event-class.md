...
|DatabaseID|`int`|该锁被获取时所在的数据库的 ID。 [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)]如果在跟踪中捕获到 ServerName 数据列并且服务器可用，则将显示数据库的名称。 可使用 DB_ID 函数来确定数据库的值。|3|是|  
...
|TextData|`ntext`|与正在获取的锁类型相关的文本值。|1|是|  
