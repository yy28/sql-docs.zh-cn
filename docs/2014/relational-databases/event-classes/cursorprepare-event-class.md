...
# <a name="cursorprepare-event-class"></a>CursorPrepare 事件类
  **Cursorprepare 事件类**事件类描述应用程序编程接口（API）游标中发生的游标准备事件。 当将与游标关联的[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssDE](../../includes/ssde-md.md)] SELECT 语句编译到执行计划中但不创建该游标时，就会发生游标准备事件。  
...
|**EventSequence**|**int**|
  **CursorPrepare** 事件类在批处理中的顺序。|51|否|  
