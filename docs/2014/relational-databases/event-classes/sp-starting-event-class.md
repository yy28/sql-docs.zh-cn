...
|----------------------|---------------|-----------------|---------------|----------------|  
|ApplicationName|`nvarchar`|创建与实例的连接的客户端应用程序的名称 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 。 此列由应用程序传递的值填充，而不是由所显示的程序名填充。|10|是|  
...
|LineNumber|`int`|显示调用此存储过程的 execute 语句的行号。|5|是|  
...
|ObjectName|`nvarchar`|将要开始的存储过程的名称。|34|是|  
|ObjectType|`int`|将要开始的存储过程的类型。 此值对应于 sys.objects 目录视图中的类型列。 有关值的信息，请参阅 [ObjectType 跟踪事件列](objecttype-trace-event-column.md)。|28|是|  
