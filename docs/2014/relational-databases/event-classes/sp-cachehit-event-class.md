...
ms.locfileid: "52816549"
---
# <a name="spcachehit-event-class"></a>SP:CacheHit 事件类
  SP:CacheHit 事件类指示存储过程位于计划缓存中。  
...
|EventSubClass|`int`|事件子类的类型。<br /><br /> 1 = 执行上下文命中：在计划缓存中找到了可用执行计划。<br /><br /> 2 = Compplan 命中：在计划缓存中找到了编写计划。|21|用户帐户控制|  
...
|ObjectID|`int`|在缓存中找到的存储过程的系统分配的 ID。|22|用户帐户控制|  
|ObjectName|`nvarchar`|在缓存中找到的对象的名称。 如果填充了 ObjectName，则不会填充 TextData。|34|用户帐户控制|  
...
|TextData|`ntext`|在缓存中找到的 SQL 代码的文本。 如果填充了 TextData，则不会填充 ObjectName。|1|用户帐户控制|  
