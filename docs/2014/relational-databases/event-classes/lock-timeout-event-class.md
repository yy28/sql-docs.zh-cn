...
|DatabaseID|`int`|发生锁超时的数据库的 ID。 [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)] 如果在跟踪中捕获 ServerName 数据列而且服务器可用，则将显示数据库名。 可使用 DB_ID 函数来确定数据库的值。|3|用户帐户控制|  
|DatabaseName|`nvarchar`|发生超时的数据库的名称。|35|用户帐户控制|  
|Duration|`bigint`|从发出锁请求到锁超时之间的时间长度（微秒）。|13|用户帐户控制|  
|EndTime|`datetime`|事件结束的时间。|15|用户帐户控制|  
|EventClass|`int`|事件类型 = 27。|27|否|  
...
|ObjectID|`int`|超时对象的 ID（如果可用且适用）。|22|用户帐户控制|  
...
|TextData|`ntext`|取决于超时发生时获取的锁类型的文本值。|1|用户帐户控制|  
