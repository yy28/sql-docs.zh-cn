...
1.  在对象资源管理器中，右键单击要对其创建全文索引的表，指向  “全文索引”，然后单击  “定义全文索引”。  
     ...
     **断字符语言**  
     从下拉列表中选择语言。 
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 将使用此选项为索引标识正确的断字符。 
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 使用断字符在全文索引的数据中标识词的边界。  
  
     **类型列**  
     选择存储作为全文索引列的文档类型的列名称。  
  
     只有当 "**可用列**" 列中命名的列为或`varbinary(max)` `image`类型时，才会启用**type 列**。  
...
3.  选择目录、索引文件组和非索引字表。  
    ...
    |*用户定义-非索引字表名称*|该列表显示已对数据库创建的用户定义的每个非索引字表（如果有）的名称。 选择任何要用于新全文索引的用户定义的非索引字表。|  
