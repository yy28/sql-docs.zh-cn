...
-   标注字符设置  
     ...
     对于给定的同义词库，所有搜索模式都是敏感的或不区分等式标记，如波形符（ **~** ）、锐音符号（**？？**）或元音变音符（**？？**）（即区分*重音*或不*区分重音*）。 例如，假设指定了 "caf？" 模式。 替换为全文查询中的其他模式。 如果同义词库不区分重音，则全文搜索将替换模式 "caf？"。 和“cafe”。 如果同义词库区分重音，则全文搜索仅替换模式 "caf？"。 默认情况下，同义词库不区分重音。  
...
##  <a name="understanding-the-structure-of-a-thesaurus-file"></a><a name="structure"></a>了解同义词库文件的结构  
 每个同义词库文件都定义了一个 ID 为 `Microsoft Search Thesaurus` 的 XML 容器，以及一个包含示例同义词库的注释 `<!--`...`-->`。 同义词库是在元素中定义 \<thesaurus> 的，该元素包含定义音调符号设置、扩展集和替换集的子元素的示例，如下所示：  
...
##  <a name="editing-a-thesaurus-file"></a><a name="editing"></a>编辑同义词库文件  
 可通过编辑给定语言的同义词库文件（XML 文件）来配置该语言的同义词库。 在安装过程中，将安装空的同义词库文件，其中仅包含 \<xml> 容器和注释掉的示例 \<thesaurus> 元素。 为了使查找同义词的全文搜索查询正常工作，必须创建 \<thesaurus> 定义一组同义词的实际元素。 可以定义两种形式的同义词，即扩展集和替换集。  
