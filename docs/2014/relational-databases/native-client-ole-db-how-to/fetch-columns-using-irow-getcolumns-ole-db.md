...
-   如何两次访问某一列。 第一次获取实际列宽度，稍后访问实际数据。 在 DBCOLUMNACCESS 结构中，如果**pData**为 NULL 和**cbMaxLen**为 0，对的调用`IRow` - `>GetColumns()`只返回实际的列长度。 在这种情况下，可以再次对同一列调用 `IRow->GetColumns()`，以检索实际数据。  
