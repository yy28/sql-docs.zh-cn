...
# <a name="allocating-an-environment-handle"></a>分配环境句柄
 ...
 分配环境句柄之后，应用程序必须设置环境属性以指示它将使用哪一个版本的 ODBC 函数调用。 若要使用 ODBC 3。*x*函数调用[SQLSetEnvAttr](../native-client-odbc-api/sqlsetenvattr.md)与*属性*参数设置为 SQL_ATTR_ODBC_VERSION 和*ValuePtr*设置为 SQL_OV_ODBC3。  
