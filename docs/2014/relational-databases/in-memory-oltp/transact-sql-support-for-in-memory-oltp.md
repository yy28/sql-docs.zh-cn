...
-   [&#40;transact-sql&#41;创建过程](/sql/t-sql/statements/create-procedure-transact-sql) `NATIVE_COMPILATION`（请参阅`SCHEMABINDING`、 `EXECUTE AS`、和） `BEGIN ATOMIC`  
  
-   [&#40;transact-sql&#41;CREATE TABLE](/sql/t-sql/statements/create-table-transact-sql) （请`MEMORY_OPTIMIZED`参阅、 `DURABILITY` `BUCKET_COUNT` `INDEX`、、和） `HASH`  
  
-   [CREATE TYPE &#40;transact-sql&#41;](/sql/t-sql/statements/create-type-transact-sql) `MEMORY_OPTIMIZED`（请参阅`BUCKET_COUNT`、 `INDEX`、和） `HASH`  
