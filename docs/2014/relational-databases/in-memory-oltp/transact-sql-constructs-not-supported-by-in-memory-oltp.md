...
|操作|TRUNCATE TABLE|内存优化的表不支持 TRUNCATE 操作。 若要从表中删除所有行，删除使用的所有行`DELETE FROM`*表*或删除并重新创建该表。|  
...
|运算符|OUTER JOIN|不支持此运算符。 从本机编译的存储过程中删除 `OUTER JOIN`。 有关详细信息，请参阅[实现 Outer Join](implementing-an-outer-join.md)。|  
...
|功能|Views|无法从本机编译的存储过程中访问视图。 不引用视图，改为引用底层基表。|  
