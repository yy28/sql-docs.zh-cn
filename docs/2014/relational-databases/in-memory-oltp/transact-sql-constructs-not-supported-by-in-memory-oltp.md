...
|功能|计算列|内存优化的表不支持计算列。 删除计算的列从`CREATE TABLE`语句。|  
|功能|复制|内存优化表不支持复制。|  
...
|运算|CREATE INDEX|必须使用 `CREATE TABLE` 语句内联指定内存优化的表的索引。 要将索引添加到内存优化的表，请删除并重新创建该表，包括指定新索引。|  
|运算|ALTER TABLE|不支持更改内存优化的表。 删除表并使用更新的表定义重新创建它。|  
...
|功能|ANSI_PADDING OFF|会话选项`ANSI_PADDING`创建内存优化表或本机编译存储的过程时，必须为 ON。 在运行 CREATE 语句前执行 `SET ANSI_PADDING ON`。|  
...
|运算|内存优化的表作为 MERGE 的目标|内存优化的表不能是 `MERGE` 操作的目标。 使用`INSERT`， `UPDATE`，或`DELETE`语句相反。|  
...
|类型|“属性”|解决方法|  
|----------|----------|----------------|  
|功能|筛选索引|内存优化的表不支持筛选的索引。 省略`WHERE`从索引定义的子句。|  
...
|运算|ALTER INDEX|不支持更改内存优化的表的索引。 请删除该表并使用更新的索引定义重新创建它。|  
|运算|DROP INDEX|不支持删除内存优化的表的索引。 请删除该表并使用所需的索引重新创建它。|  
...
|功能|多行 INSERT... VALUES 语句|在本机编译的存储过程中无法使用同一 `INSERT` 语句插入多行。 创建`INSERT`语句为每个行。|  
...
|功能|EXECUTE WITH RECOMPILE|选项`WITH RECOMPILE`不支持本机编译的存储过程。|  
...
|运算|ALTER PROCEDURE|不能更改本机编译的存储过程。 要更改该过程定义，请删除并重新创建存储过程。|  
|运算|保存点|不能从具有活动保存点的事务调用本机编译的存储过程。 从事务中删除该保存点。|  
|运算|ALTER AUTHORIZATION|不支持更改现有内存优化的表或本机编译的存储过程的所有者。 请删除并重新创建该表或过程来更改所有权。|  
|运算符|OPENROWSET|不支持此运算符。 删除`OPENROWSET`从本机编译的存储过程。|  
|运算符|OPENQUERY|不支持此运算符。 删除`OPENQUERY`从本机编译的存储过程。|  
|运算符|OPENDATASOURCE|不支持此运算符。 删除`OPENDATASOURCE`从本机编译的存储过程。|  
|运算符|OPENXML|不支持此运算符。 删除`OPENXML`从本机编译的存储过程。|  
|运算符|CONTAINSTABLE|不支持此运算符。 删除`CONTAINSTABLE`从本机编译的存储过程。|  
|运算符|FREETEXTTABLE|不支持此运算符。 删除`FREETEXTTABLE`从本机编译的存储过程。|  
|功能|表值函数|不能从本机编译的存储过程引用表值函数。 有可能解决此限制的一种方法是将表值函数中的逻辑添加到过程主体。|  
|运算符|CHANGETABLE|不支持此运算符。 删除`CHANGETABLE`从本机编译的存储过程。|  
|运算符|GOTO|不支持此运算符。 使用其他过程构造，如 WHILE。|  
|运算符|EXECUTE, INSERT EXEC|不支持嵌套本机编译的存储过程。 所需的操作可以内联指定作为存储过程定义的一部分。|  
|运算符|OFFSET|不支持此运算符。 删除`OFFSET`从本机编译的存储过程。|  
|运算符|UNION|不支持此运算符。 删除`UNION`从本机编译的存储过程。 可以使用表变量将几个结果集合并为单个结果集。|  
|运算符|INTERSECT|不支持此运算符。 删除`INTERSECT`从本机编译的存储过程。 在某些情况下，可以使用 INNER JOIN 获得相同的结果。|  
|运算符|EXCEPT|不支持此运算符。 删除`EXCEPT`从本机编译的存储过程。|  
|运算符|OUTER JOIN|不支持此运算符。 删除`OUTER JOIN`从本机编译的存储过程。 有关详细信息，请参阅[实现 Outer Join](implementing-an-outer-join.md)。|  
|运算符|APPLY|不支持此运算符。 删除`APPLY`从本机编译的存储过程。|  
|运算符|PIVOT|不支持此运算符。 删除`PIVOT`从本机编译的存储过程。|  
|运算符|UNPIVOT|不支持此运算符。 删除`UNPIVOT`从本机编译的存储过程。|  
|运算符|OR, IN|本机编译的存储过程中查询的 WHERE 子句中不支持 Disjunction (OR, IN)。 为每种情况创建查询。|  
|运算符|CONTAINS|不支持此运算符。 删除`CONTAINS`从本机编译的存储过程。|  
|运算符|FREETEXT|不支持此运算符。 删除`FREETEXT`从本机编译的存储过程。|  
|运算符|NOT|不支持此运算符。 删除`NOT`从本机编译的存储过程。 在某些情况下，可以使用不相等替换 `NOT`。 例如，可以使用 `NOT a=b` 替换 `a!=b`。|  
|运算符|TSEQUAL|不支持此运算符。 删除`TSEQUAL`从本机编译的存储过程。|  
|运算符|LIKE|不支持此运算符。 删除`LIKE`从本机编译的存储过程。|  
...
|操作数|TABLESAMPLE|不支持此运算符。 删除`TABLESAMPLE`从本机编译的存储过程。|  
...
|选项|FOR XML|不支持此选项。 删除`FOR XML`从本机编译的存储过程。|  
|选项|FOR BROWSE|不支持此选项。 删除`FOR BROWSE`从本机编译的存储过程。|  
...
|功能|ROLLUP|在本机编译的存储过程中，`ROLLUP` 不能与 `GROUP BY` 子句一起使用。 删除`ROLLUP`从过程定义。|  
|功能|CUBE|在本机编译的存储过程中，`CUBE` 不能与 `GROUP BY` 子句一起使用。 删除`CUBE`从过程定义。|  
|功能|GROUPING SETS|在本机编译的存储过程中，`GROUPING SETS` 不能与 `GROUP BY` 子句一起使用。 删除`GROUPING SETS`从过程定义。|  
