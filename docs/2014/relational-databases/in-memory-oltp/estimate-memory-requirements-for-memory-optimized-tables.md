...
# <a name="estimate-memory-requirements-for-memory-optimized-tables"></a>估算内存优化表的内存需求
  无论是创建新[!INCLUDE[hek_2](../../includes/hek-2-md.md)]的内存优化表还是将现有的基于磁盘的表迁移到内存优化表, 都一定要合理估计每个表的内存需求, 以便能够将服务器设置为具有足够的记忆. 本节介绍如何估算使用内存优化表存放数据时所需的内存大小。  
 ...
 **设置哈希索引数组大小**  
 ...
 哈希数组大小由`(bucket_count= <value>)` \<值 > 的值设置为大于零的整数值。 如果\<value > 不是2的幂, 实际的 bucket_count 将舍入为下一个最近的2次幂。  在我们的示例表中, (bucket_count = 5000000), 因为5000000不是2的幂, 所以实际 bucket 计数向上舍入为 8388608 (2<sup>23</sup>)。  在计算此哈希数组所需的内存时，必须使用该值，而非 5,000,000。  
 ...
 **非聚集索引的内存**  
 ...
 非聚集索引是以 B 树的形式实现的, 其中包含索引值和指向后续节点的指针。  叶节点包含索引键和指向内存中表行的指针。  
...
-   **叶节点占用的内存**   
    叶节点对于表中的每个唯一键都具有 1 行，并且指向具有该唯一键的数据行。  如果有多个行具有相同的键 (即, 您有一个非唯一的非聚集索引), 则索引叶节点中只有一行指向某个行, 其中的其他行彼此链接。  因此，可通过下面的公式估算所需的总内存：   
    非聚集索引所需内存 = （指针大小 + Sum (键列数据类型大小))* 唯一键行数  
  
 在用于范围查找时, 非聚集索引的最佳使用方式举例说明, 如下查询所示:  
