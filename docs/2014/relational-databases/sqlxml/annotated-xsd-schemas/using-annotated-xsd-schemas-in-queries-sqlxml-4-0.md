 ...
 **\<sql:xpath-query>** 元素允许你为带有批注的架构所定义的 XML 视图指定 XPath 查询。 使用元素的属性标识要对其执行 XPath 查询的带批注的架构 `mapping-schema` **\<sql:xpath-query>** 。  
...
## <a name="using-inline-mapping-schemas"></a>使用内联映射架构  
 ...
 模板中可包含多个内联架构。 若要使用模板中包含的内联架构，请在元素上指定具有唯一值的**id**属性 **\<xsd:schema>** ，然后使用 **#idvalue**引用内联架构。 **Id**属性在行为上与 XDR 架构中使用的**sql： id** （{urn：架构-microsoft com： xml} id）的行为相同。  
...
```  
 ...
 该模板还指定了两个 XPath 查询。 每个 **\<xpath-query>** 元素都通过指定属性来唯一标识映射架构 `mapping-schema` 。  
  
 在模板中指定内联架构时， `sql:is-mapping-schema` 还必须在元素上指定批注 **\<xsd:schema>** 。 `sql:is-mapping-schema` 取布尔值（0=false，1=true）。 带有 sql：的内联架构**为-映射-schema = "1"** 被视为内联批注的架构，不在 XML 文档中返回。  
 ...
 除了在 `mapping-schema` 模板中的元素上指定属性 **\<sql:xpath-query>** （在存在 XPath 查询时），或在 **\<updg:sync>** updategram 中的元素上指定，您可以执行以下操作：  
  
-   在 `mapping-schema` **\<ROOT>** 模板中的元素（全局声明）上指定属性。 这样，该映射架构就成为默认架构，并将由无显式 `mapping-schema` 批注的所有 XPath 和 updategram 节点使用。  
  
-   使用 ADO `mapping schema` 对象指定 `Command` 属性。  
  
 `mapping-schema`或元素上指定的特性 **\<xpath-query>** **\<updg:sync>** 具有最高优先级; ADO `Command` 对象的优先级最低。  
