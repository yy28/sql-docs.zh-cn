...
## <a name="sql_copt_ss_perf_data"></a>SQL_COPT_SS_PERF_DATA  
 SQL_COPT_SS_PERF_DATA 属性返回包含当前驱动程序性能统计信息的 SQLPERF 结构的指针。 `SQLGetConnectAttr`如果未启用性能日志记录，将返回 NULL。 驱动程序不会动态更新 SQLPERF 结构中的统计信息。 每`SQLGetConnectAttr`次需要刷新性能统计信息时调用。  
