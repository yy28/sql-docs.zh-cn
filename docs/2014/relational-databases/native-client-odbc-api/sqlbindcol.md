...
## <a name="sqlbindcol-support-for-enhanced-date-and-time-features"></a>SQLBindCol 对日期和时间增强功能的支持  
 结果列的日期/时间类型的值将转换中所述[从 SQL 转换到 C](../native-client-odbc-date-time/datetime-data-type-conversions-from-sql-to-c.md)。请注意，若要检索作为其相应的结构的时间和 datetimeoffset 列 (`SQL_SS_TIME2_STRUCT`和**SQL_SS_TIMESTAMPOFFSET_STRUCT**)， *TargetType*必须指定为`SQL_C_DEFAULT`或`SQL_C_BINARY`。  
