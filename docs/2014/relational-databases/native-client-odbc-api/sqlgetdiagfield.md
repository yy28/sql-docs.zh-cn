...
|SQL_DIAG_SS_PROCNAME|根据需要生成错误的存储过程的名称。 该值作为字符串返回。 字符串的长度（以字符为单位）取决于 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的版本。 可通过调用来确定[SQLGetInfo](sqlgetinfo.md)请求 SQL_MAX_PROCEDURE_NAME_LEN 值。|  
