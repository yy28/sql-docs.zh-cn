 ...
 **SQLBrowseConnect**忽略的 ODBC 数据源定义中存储的数据库和语言关键字的值。 如果数据库或连接字符串中指定的语言传递给**SQLBrowseConnect**是无效的**SQLBrowseConnect**返回 SQL_NEED_DATA 和等级 3 连接属性。  
 ...
 以下属性，通过调用设置[SQLSetConnectAttr](sqlsetconnectattr.md)，确定返回的结果集**SQLBrowseConnect**。  
  
|Attribute|Description|  
|---------------|-----------------|  
|SQL_COPT_SS_BROWSE_CONNECT|如果设置为 SQL_MORE_INFO_YES， **SQLBrowseConnect**返回扩展的服务器属性字符串。<br /><br /> 以下是返回一个扩展字符串的示例**SQLBrowseConnect**: ServerName\InstanceName;群集： 没有;版本： 8.00.131<br /><br /> 在此字符串中，分号用于分隔与服务器有关的各部分信息， 逗号用于分隔不同的服务器实例。|  
|SQL_COPT_SS_BROWSE_SERVER|如果指定服务器名称，则**SQLBrowseConnect**将返回指定的服务器的信息。 如果 SQL_COPT_SS_BROWSE_SERVER 设置为 NULL， **SQLBrowseConnect**在域中返回的所有服务器的信息。<br /><br /> 由于网络问题而**SQLBrowseConnect**可能未收到及时的响应来自所有服务器。 因此，每个请求所返回的服务器列表都可能不同。|  
...
## <a name="sqlbrowseconnect-support-for-high-availability-disaster-recovery"></a>对高可用性、灾难恢复的 SQLBrowseConnect 支持  
 有关详细信息使用**SQLBrowseConnect**连接到[!INCLUDE[ssHADR](../../includes/sshadr-md.md)]群集，请参阅[高可用性、 灾难恢复的 SQL Server Native Client Support](../native-client/features/sql-server-native-client-support-for-high-availability-disaster-recovery.md)。  
