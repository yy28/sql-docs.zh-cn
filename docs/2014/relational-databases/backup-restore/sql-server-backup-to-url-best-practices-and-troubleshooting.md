...
# <a name="sql-server-backup-to-url-best-practices-and-troubleshooting"></a>SQL Server 备份到 URL 最佳实践和故障排除
 ...
 有关使用 Azure Blob 存储服务进行 SQL Server 备份或还原操作的详细信息, 请参阅:  
...
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]备份操作使用多个线程来优化到 Azure Blob 存储服务的数据传输。  但是性能取决于各种因素，如 ISV 带宽和数据库的大小。 如果您计划从内部 SQL Server 数据库备份大型数据库或文件组，建议您首先执行某些吞吐量测试。 [Azure 存储 SLA](https://go.microsoft.com/fwlink/?LinkId=271619)具有可考虑的 blob 的最大处理时间。  
...
## <a name="troubleshooting-backup-to-or-restore-from-url"></a>备份到 URL 或从 URL 还原故障排除  
 下面是在备份到 Azure Blob 存储服务或从中进行还原时, 解决错误的一些快速方法。  
...
-   验证存储帐户名称和密钥值。 凭据中存储的信息必须与您在备份和还原操作中使用的 Azure 存储帐户的属性值匹配。  
...
-   从压缩备份中还原时，您可能看到以下错误：  
    ...
    -   **发生 SqlException 3284。对应16状态:5**  
        **在设备上的消息文件标记 'https://mystorage.blob.core.windows.net/mycontainer/TestDbBackupSetNumber2_0.bak ' 未对齐。使用用于创建 backupset 的相同块大小重新发出 Restore 语句:"65536" 看起来像一个可能的值。**  
...
-   由于 blob 具有活动租约，备份期间出错：失败的备份活动可能导致 blob 产生活动租约。  
     ...
     **“备份到 URL”收到来自远程端点的异常。异常消息:远程服务器返回了错误：(412) 当前 blob 上有租约, 但请求**中未指定任何租约 ID。  
...
-   在 "http://storageaccount.blob.core.windows.net/container/BackupAzurefile.bak " 上写入失败:"备份到 URL" 收到来自远程终结点的异常。 异常消息:无法从传输连接中读取数据:连接已关闭。  
...
-   BackupIoRequest:: ReportIoError: 备份设备 "http://storageaccount.blob.core.windows.net/container/BackupAzurefile.bak " 上的写入失败。 操作系统错误，“备份到 URL”收到来自远程端点的异常。 异常消息:无法从传输连接中读取数据:连接已关闭。  
 ...
 HTTP 状态代码 502，HTTP 状态消息代理错误 (每分钟的 HTTP 请求数超出配置限制。 请与 ISA Server 管理员联系。  “应用程序适配器” 区域）  
