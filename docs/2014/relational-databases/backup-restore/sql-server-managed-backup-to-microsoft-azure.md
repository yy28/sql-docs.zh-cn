...
-   目前，自动备份多个数据库需要制定备份策略、编写自定义代码并安排备份。 通过使用[!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)]，只需提供保持期设置和存储位置。 [!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)]计划、执行并维护备份。  
     ...
     [!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)]可以在数据库级别配置，或以 SQL Server 实例的默认设置来配置。 使用[!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)]自动备份具有以下优势:  
...
-   有了[!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)]，您便无须为数据库指定备份类型或备份频率。  指定保持期, 并[!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)]确定数据库在 Azure Blob 存储服务上存储备份的类型和频率。 有关用于创建备份策略的一组条件[!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)]的详细信息, 请参阅本主题中的[组件和概念](#Concepts)部分。  
  
-   配置为使用加密后，即提高备份数据的安全性。 有关详细信息, 请参阅[备份加密](backup-encryption.md)  
  
 有关使用 azure blob 存储进行[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]备份的好处的更多详细信息, 请参阅[使用 azure blob 存储服务进行备份和还原 SQL Server](sql-server-backup-and-restore-with-microsoft-azure-blob-storage-service.md)  
...
###  <a name="Security"></a> Permissions  
 Transact-SQL 是用于配置和监控[!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)]的主要界面。 通常, 要运行配置存储过程, 需要具有**ALTER ANY CREDENTIAL**权限的**db_backupoperator**数据库角色, 以及`EXECUTE`对**sp_delete_backuphistory**存储过程的权限。  用于查看信息的存储过程和函数通常分别需要针对存储过程的 `Execute` 权限以及针对函数的 `Select` 权限。  
  
###  <a name="Prereqs"></a> 先决条件  
 **先决条件：**  
  
 使用[!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)] **Azure 存储服务**来存储备份文件。    有关创建 Azure 存储帐户的概念、结构和要求详细说明, 请查看**SQL Server 备份到 URL** "主题的[关键组件和概念](sql-server-backup-to-url.md#intorkeyconcepts)部分。  
...
|[smart_admin.set_instance_backup &#40;Transact-SQL&#41;](https://msdn.microsoft.com/library/dn451009(v=sql.120).aspx)|系统存储过程, 用于启用和配置 SQL Server [!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)]实例的默认设置。|  
|[smart_admin backup_master_switch &#40;transact-sql&#41;](/sql/relational-databases/system-stored-procedures/managed-backup-sp-backup-master-switch-transact-sql)|用于暂停和继续运行[!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)]的系统存储过程。|  
...
|[smart_admin.sp_backup_on_demand &#40;Transact-SQL&#41;](/sql/relational-databases/system-stored-procedures/managed-backup-sp-backup-on-demand-transact-sql)|系统存储过程, 用于对启用[!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)]的数据库执行即席备份, 而无需中断日志链。|  
 ...
 **使用的[!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)]备份策略:**  
 ...
 **备份容器和文件命名约定:**  
 ...
 使用以下约定命名非可用性数据库的备份文件:使用数据库名称的前 40 个字符、不含“-”的数据库 GUID 以及时间戳创建名称。 在各段之间插入下划线字符作为分隔符。 完整备份使用 **.bak** 文件扩展名，日志备份使用 **.log** 。 对于可用性组数据库，除了上方所述的文件命名约定之外，还在 40 个字符的数据库名称后添加可用性组数据库 GUID。 可用性组数据库 GUID 值为 sys.databases 中 group_database_id 的值。  
...
-   [!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)]代理仅支持数据库备份:完整备份和日志备份。  不支持文件备份自动化。  
...
-   恢复模式:仅支持设置为完整或大容量日志模式的数据库。  不支持设置为简单恢复模式的数据库。  
  
-   [!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)] 可能有一些限制。 有关详细信息, 请[参阅 SQL Server 托管备份到 Azure:互操作性](../../database-engine/sql-server-managed-backup-to-windows-azure-interoperability-and-coexistence.md)和共存。  
...
|**指导**有关配置和监视[!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)]的分步说明。|[设置 SQL Server 托管备份到 Azure](enable-sql-server-managed-backup-to-microsoft-azure.md)|  
|**指导**有关配置和监视[!INCLUDE[ss_smartbackup](../../includes/ss-smartbackup-md.md)]可用性组中的数据库的分步说明。|[为可用性组设置 SQL Server 托管备份到 Azure](../../database-engine/setting-up-sql-server-managed-backup-to-windows-azure-for-availability-groups.md)|  
