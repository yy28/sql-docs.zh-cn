...
1.  创建一个用于创建低优先级压缩备份的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 登录名。  
    ...
    -   [创建登录名](../security/authentication-access/create-a-login.md)  
  
    -   [CREATE LOGIN &#40;Transact-SQL&#41;](/sql/t-sql/statements/create-login-transact-sql)  
  
2.  还可以选择对此登录名授予 VIEW SERVER STATE。  
  
    -   [GRANT 系统对象权限 (Transact-SQL)](/sql/t-sql/statements/grant-system-object-permissions-transact-sql)  
  
     有关详细信息，请参阅 [GRANT 数据库主体权限 (Transact-SQL)](/sql/t-sql/statements/grant-database-principal-permissions-transact-sql)。  
  
3.  为此登录名创建一个 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 用户。  
  
     **创建用户**  
  
    -   [创建数据库用户](../security/authentication-access/create-a-database-user.md)  
...
> [!NOTE]  
>  有关 Resource Governor 资源池、工作负荷组以及分类的信息，请参阅 [Resource Governor](../resource-governor/resource-governor.md)。  
...
-   [使用模板配置资源调控器](../resource-governor/configure-resource-governor-using-a-template.md)  
  
-   [创建资源池](../resource-governor/create-a-resource-pool.md)  
  
-   [创建工作负荷组](../resource-governor/create-a-workload-group.md)  
  
### <a name="to-configure-resource-governor-for-limiting-cpu-usage-transact-sql"></a>配置用于限制 CPU 使用的资源调控器 (Transact-SQL)  
  
1.  发出 [CREATE RESOURCE POOL](/sql/t-sql/statements/create-resource-pool-transact-sql) 语句以创建资源池。 此过程的示例使用以下语法：  
...
2.  发出一个 [CREATE WORKLOAD GROUP](/sql/t-sql/statements/create-workload-group-transact-sql) 语句，为你要调控其 CPU 使用的低优先级操作创建一个工作负荷组。 此过程的示例使用以下语法：  
  
     CREATE WORKLOAD GROUP *group_name* USING *pool_name*;  
  
3.  发出一个 [CREATE FUNCTION](/sql/t-sql/statements/create-function-transact-sql) 语句，以创建一个分类器函数，此函数将上一步创建的工作负荷组映射到低优先级登录名的用户。 此过程的示例使用以下语法：  
    ...
    -   [SUSER_SNAME (Transact-SQL)](/sql/t-sql/functions/suser-sname-transact-sql)  
        ...
        > [!IMPORTANT]  
        >  SUSER_NAME 仅仅是可用在分类器函数中的几个系统函数中的一个。 有关详细信息，请参阅 [创建和测试分类器用户定义函数](../resource-governor/create-and-test-a-classifier-user-defined-function.md)。  
  
    -   [SET @local_variable (Transact-SQL)](/sql/t-sql/language-elements/set-local-variable-transact-sql)。  
  
4.  发出一个 [ALTER RESOURCE GOVERNOR](/sql/t-sql/statements/alter-resource-governor-transact-sql) 语句，将该分类器函数注册到资源调控器中。 此过程的示例使用以下语法：  
...
## <a name="see-also"></a>请参阅  
 [创建和测试分类器用户定义函数](../resource-governor/create-and-test-a-classifier-user-defined-function.md)   
