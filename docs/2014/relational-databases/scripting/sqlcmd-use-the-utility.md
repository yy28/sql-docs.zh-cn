...
-   身份验证选项（ **-E**、 **-U**和 **-P**），用于指定 `sqlcmd` 用来连接到 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]实例的凭据。  
  
    > [!NOTE]  
    >  **-E** 选项为默认选项，毋须指定。  
  
-   输入选项（ **-q**、 **-q**和 **-i**），用于标识要 `sqlcmd`的输入的位置。  
  
-   输出选项（ **-o**），用于指定在其中放置其输出的 `sqlcmd` 文件。  
...
## <a name="running-transact-sql-statements-interactively-by-using-sqlcmd"></a>使用 sqlcmd 以交互方式运行 Transact-SQL 语句  
 您可以使用 `sqlcmd` 实用工具以交互方式在命令提示符窗口中执行 [!INCLUDE[tsql](../../includes/tsql-md.md)] 语句。 若要通过使用 `sqlcmd`以交互方式执行 [!INCLUDE[tsql](../../includes/tsql-md.md)] 语句，请在不使用 **-q**、 **-q**、 **-Z**或 **-i**选项指定任何输入文件或查询的情况下运行实用工具。 例如：  
 ...
 若要清除语句缓存，请键入 `:RESET`。 键入 `^C` 会导致 `sqlcmd` 退出。 在发出 `^C` 命令后，还可以用 `GO` 停止语句缓存的执行。  
  
 可以通过输入 **： ED**命令和 `sqlcmd` 提示符来编辑在交互式会话中输入 [!INCLUDE[tsql](../../includes/tsql-md.md)] 语句。 编辑器将打开，编辑 [!INCLUDE[tsql](../../includes/tsql-md.md)] 语句并关闭编辑器后，修改后的 [!INCLUDE[tsql](../../includes/tsql-md.md)] 语句将显示于命令窗口中。 输入 `GO` [!INCLUDE[tsql](../../includes/tsql-md.md)] 语句中运行 therevised。  
...
## <a name="interactive-sqlcmd-example"></a>交互式 sqlcmd 示例  
 ...
 这表示您已连接到 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的实例，并且 `sqlcmd` 现在已可以接受 [!INCLUDE[tsql](../../includes/tsql-md.md)] 语句和 `sqlcmd` 命令。 `1>` 后闪烁的下划线是 `sqlcmd` 提示符，它标明了所键入语句和命令的显示位置。 现在，键入 `USE AdventureWorks2012`，然后按 ENTER，然后键入 `GO` 并按 ENTER。 命令提示符窗口的内容如下：  
...
### <a name="g-using-sqlcmd-to-set-encryption-on-azure-sql-database"></a>G. 使用 sqlcmd 在 Azure SQL 数据库上设置加密  
 可以在连接上执行 `sqlcmd`以 [!INCLUDE[ssSDS](../../includes/sssds-md.md)] 上的数据以指定加密和证书信任。 提供两个 "sqlcmd" 选项：  
