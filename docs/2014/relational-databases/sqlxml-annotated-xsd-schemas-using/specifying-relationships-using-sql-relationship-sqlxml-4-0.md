...
-   组成父表与子表间关系的一列或多列。 例如，CustomerID 列，该列同时出现在父表和子表中。  
 ...
 为了提供表名和必需的联接信息，应针对 `sql:relationship` 批注指定以下属性。 这些特性是仅对有效 **\<sql:relationship >** 元素：  
 ...
 **反函数**  
 此属性上指定 **\<sql:relationship >** updategram 使用。 有关详细信息，请参阅[sql:relationship 上指定 sql:inverse 属性](specifying-the-sql-inverse-attribute-on-sql-relationship-sqlxml-4-0.md)。  
  
 `sql:key-fields`批注必须在包含子元素具有的元素中指定 **\<sql:relationship >** 定义元素和子，之间并不提供的主键父元素中指定的表。 即使未指定架构 **\<sql:relationship >**，必须指定`sql:key-fields`以生成适当的层次结构。 有关详细信息，请参阅[使用 sql:key 标识键列的字段](identifying-key-columns-using-sql-key-fields-sqlxml-4-0.md)。  
...
### <a name="c-specifying-the-relationship-annotation-on-an-attribute"></a>C. 针对属性指定关系批注  
 在此示例中的架构包括\<客户 > 具有元素\<CustomerID > 子元素和 OrderIDList IDREFS 类型的属性。 \<客户 > 元素映射到 Sales.Customer 表 AdventureWorks 数据库中。 默认情况下，此映射的作用域适用于所有子元素或特性，除非`sql:relation`上指定的子元素或属性，在这种情况下，必须使用定义相应的主键/外键关系\<关系 > 元素。 并且，子元素或属性（使用 `relation` 批注指定不同的表）还必须指定 `relationship` 批注。  
