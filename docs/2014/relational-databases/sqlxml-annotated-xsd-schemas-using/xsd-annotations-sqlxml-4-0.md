...
|--------------------|-----------------|----------------|--------------------|  
|`sql:encode`|在将某一 XML 元素或属性映射到 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] BLOB 列时，允许请求某一引用 URI。 此 URI 可以在以后使用，以便返回 BLOB 数据。|[请求 URL 引用 BLOB 数据使用 sql： 进行编码&#40;SQLXML 4.0&#41;](requesting-url-references-to-blob-data-using-sql-encode-sqlxml-4-0.md)|`url-encode`|  
...
|`sql:inverse`|指示 updategram 逻辑颠倒已使用指定的父-子关系的解释 **\<sql: relationship >**。|[Sql: relationship 上指定 sql: inverse 属性&#40;SQLXML 4.0&#41;](specifying-the-sql-inverse-attribute-on-sql-relationship-sqlxml-4-0.md)|不支持|  
...
|`sql:limit-field`<br /><br /> `sql:limit-value`|允许限制基于限制值返回的值。|[筛选值使用 sql:-字段和 sql： 的值&#40;SQLXML 4.0&#41;](../sqlxml-annotated-xsd-schemas-xpath-queries/bulk-load-xml/annotation-interpretation-sql-limit-field-and-sql-limit-value.md)|相同|  
|`sql:mapped`|允许从结果中排除架构项。|[从生成 XML 文档中使用 sql 中排除架构元素： 映射&#40;SQLXML 4.0&#41;](excluding-schema-elements-from-the-xml-document-using-sql-mapped.md)|`map-field`|  
...
|`sql:prefix`|创建有效的 XML ID、IDREF 和 IDREFS。 将某一字符串置于 ID、IDREF 和 IDREFS 的值之前。|[有效的 ID、 IDREF 和 IDREFS 类型属性使用 sql: prefix 创建&#40;SQLXML 4.0&#41;](creating-valid-id-idref-and-idrefs-type-attributes-using-sql-prefix-sqlxml-4-0.md)|相同|  
|`sql:relationship`|指定 XML 元素之间的关系。 `parent`、`child`、`parent-key` 和 `child-key` 属性用于建立该关系。|[关系使用 sql: relationship 指定&#40;SQLXML 4.0&#41;](specifying-relationships-using-sql-relationship-sqlxml-4-0.md)|属性名称不同：<br /><br /> `key-relation`<br /><br /> `foreign-relation`<br /><br /> `key`<br /><br /> `foreign-key`|  
