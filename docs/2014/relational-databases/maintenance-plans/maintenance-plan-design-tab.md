...
# <a name="maintenance-plan-design-tab"></a>维护计划（“设计”选项卡）
  使用 "**维护计划" （"设计" 选项卡）** 可以指定维护计划及其子的属性。 将任务从工具箱拖到计划设计器中。 右键单击任务组以创建分支执行路径。 维护计划将另存为 [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 包，它们由 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 代理作业执行。  
