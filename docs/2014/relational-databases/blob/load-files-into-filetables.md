...
|文件当前存储在文件系统中。<br /><br /> [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 包含一个元数据表，该表中包含指向文件的指针。|第一步是使用上文所述的方法之一移动或复制文件。<br /><br /> 第二步是更新现有元数据的表以指向该文件的新位置。<br /><br /> 有关详细信息，请参阅[示例：文件迁移到 FileTable 的文件系统](#HowToMigrateFiles)本主题中。|  
  
###  <a name="HowToLoadNew"></a> 如何：将文件加载到 FileTable  
...
###  <a name="HowToMigrateFiles"></a> 示例：文件迁移到 FileTable 中的文件系统  
 ...
 此示例中，假设没有现有的数据库表`PhotoMetadata`，其中包含有关照片的数据。 该表具有 `varchar`(512) 类型的 `UNCPath` 列，其中包含指向 .jpg 文件的实际 UNC 路径。  
...
###  <a name="HowToBulkLoad"></a> 如何：大容量加载文件到 FileTable  
...
###  <a name="disabling"></a> 如何：禁用针对大容量加载的 FileTable 约束  
