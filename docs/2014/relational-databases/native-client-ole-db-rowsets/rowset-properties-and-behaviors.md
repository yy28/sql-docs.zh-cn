...
|DBPROP_ACCESSORDER|R/W:读/写<br /><br /> 默认值：DBPROPVAL_AO_RANDOM<br /><br /> 说明:访问顺序。 访问行集中的列时必须遵照的顺序。<br /><br /> DBPROPVAL_AO_RANDOM:可以按任意顺序访问列。<br /><br /> DBPROPVAL_AO_SEQUENTIALSTORAGEOBJECTS:列绑定为存储对象只能在由列序号确定的顺序依次访问。<br /><br /> DBPROPVAL_AO_SEQUENTIAL:必须按顺序由列序号确定访问的所有列。|  
...
|DBPROP_CHANGEINSERTEDROWS|R/W:读/写<br /><br /> 默认值：VARIANT_FALSE<br /><br /> 说明:此属性仅可设置为 VARIANT_TRUE，如果行集使用由键集驱动的游标。|  
...
|DBPROP_QUICKRESTART|R/W:读/写<br /><br /> 默认值：VARIANT_FALSE<br /><br /> 说明:如果设置为 VARIANT_TRUE， [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client OLE DB 提供程序将尝试使用服务器游标用于行集。|  
...
|DBPROP_RETURNPENDINGINSERTS|R/W:只读<br /><br /> 默认值：VARIANT_FALSE<br /><br /> 说明:当调用提取行的方法时， [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client OLE DB 提供程序不返回挂起的插入行。|  
...
|DBPROP_ROWSET_ASYNCH|R/W:读/写<br /><br /> 默认值：0<br /><br /> 说明:提供用于异步行集处理。 此属性位于行集属性组和 DBPROPSET_ROWSET 属性集中。 类型为 VT_14。<br /><br /> [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client 支持的位掩码中的唯一值是 `DBPROPVAL_ASYNCH_INITIALIZE`。|  
...
|DBPROP_SERVERCURSOR|R/W:读/写<br /><br /> 默认值：VARIANT_FALSE<br /><br /> 说明:设置时，[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]游标用于支持行集。 有关详细信息，请参阅[行集和 SQL Server 游标](rowsets-and-sql-server-cursors.md)。|  
|DBPROP_SERVERDATAONINSERT|R/W:读/写<br /><br /> 默认值：VARIANT_FALSE<br /><br /> 说明:服务器在插入数据。<br /><br /> VARIANT_TRUE:在将插入传输到服务器时，该提供程序从服务器以更新本地行缓存检索数据。<br /><br /> VARIANT_FALSE:提供程序不检索服务器值的新插入的行。|  
...
|DBPROP_UNIQUEROWS|R/W:读/写<br /><br /> 默认值：VARIANT_FALSE<br /><br /> 说明:唯一行。<br /><br /> VARIANT_TRUE:由其列值唯一标识每个行。 唯一标识此行的列集在 GetColumnInfo 方法返回的 DBCOLUMNINFO 结构中设置了 DBCOLUMNFLAGS_KEYCOLUMN  。<br /><br /> VARIANT_FALSE:可能或不可能由其列值唯一标识行。 键列可能（也可能不）由 DBCOLUMNFLAGS_KEYCOLUMN 进行标记。|  
...
|属性 ID|Description|  
|-----------------|-----------------|  
|SSPROP_COLUMN_ID|列：ColumnID<br /><br /> R/W:只读<br /><br /> 键入：VT_U12 &#124; VT_ARRAY<br /><br /> 默认值：VT_EMPTY<br /><br /> 说明:一个表示在当前 COMPUTE 子句结果列的序号位置 （从 1 开始） 的整数值数组[!INCLUDE[tsql](../../includes/tsql-md.md)]SELECT 语句。 这是[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Native Client OLE DB 访问接口中等效的 ODBC sql_ca_ss_column_id 属性的属性。|  
|SSPROP_DEFERPREPARE|列：否<br /><br /> R/W:读/写<br /><br /> 键入：VT_BOOL<br /><br /> 默认值：VARIANT_TRUE<br /><br /> 说明:VARIANT_TRUE:在准备好的执行，推迟命令准备，直到**icommand:: Execute**调用或执行元属性操作。 如果此属性设置为<br /><br /> VARIANT_FALSE:准备语句时**icommandprepare:: Prepare**执行。|  
|SSPROP_IRowsetFastLoad|列：否<br /><br /> R/W:读/写<br /><br /> 键入：VT_BOOL<br /><br /> 默认值：VARIANT_FALSE<br /><br /> 说明:将此属性设置为 variant_true，则可以打开通过一个快速加载行集**iopenrowset:: Openrowset**。 无法在 ICommandProperties::SetProperties 中设置此属性  。|  
|SSPROP_ISSAsynchStatus|列：否。<br /><br /> R/W:读/写<br /><br /> 键入：VT_BOOL<br /><br /> 默认值：VARIANT_FALSE<br /><br /> 说明:将此属性设置为 variant_true，则可以使用的异步操作[ISSAsynchStatus](../../relational-databases/native-client-ole-db-interfaces/issasynchstatus-ole-db.md)接口。|  
|SSPROP_MAXBLOBLENGTH|列：否<br /><br /> R/W:读/写<br /><br /> 键入：VT_I4<br /><br /> 默认值：提供程序不限制由服务器返回的文本的大小和属性值设置为其最大值。 例如，2147483647。<br /><br /> 说明:[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client OLE DB 访问接口执行 SET TEXTSIZE 语句，以限制在 SELECT 语句中返回的二进制大型对象 (BLOB) 数据的长度。|  
|SSPROP_NOCOUNT_STATUS|列：NoCount<br /><br /> R/W:只读<br /><br /> 键入：VT_BOOL<br /><br /> 默认值：VARIANT_FALSE<br /><br /> 说明:表示 SET NOCOUNT on/off 中的状态的布尔值[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]:<br /><br /> VARIANT_TRUE：当 SET NOCOUNT 为 ON 时<br /><br /> VARIANT_FALSE：当 SET NOCOUNT 为 OFF 时|  
|SSPROP_QP_NOTIFICATION_MSGTEXT|列：否<br /><br /> R/W:读/写<br /><br /> 键入：VT_BSTR （允许 1-2000年个字符）<br /><br /> 默认值：空字符串<br /><br /> 说明:查询通知消息文本。 这是用户定义的文本，没有确定的格式。|  
|SSPROP_QP_NOTIFICATION_OPTIONS|列：否<br /><br /> R/W:读/写<br /><br /> 键入：VT_BSTR<br /><br /> 默认值：空字符串<br /><br /> 说明:查询通知选项。 在具有 `name=value` 的字符串中指定这些内容。 用户负责创建服务并从队列中读取通知。 查询通知选项字符串的语法为：<br /><br /> `service=<service-name>[;(local database=<database>&#124;broker instance=<broker instance>)]`<br /><br /> 例如：<br /><br /> `service=mySSBService;local database=mydb`|  
|SSPROP_QP_NOTIFICATION_TIMEOUT|列：否<br /><br /> R/W:读/写<br /><br /> 键入：VT_UI4<br /><br /> 默认值：432000 秒 （5 天）<br /><br /> 最小值：1 秒<br /><br /> 最大值：2 ^31-1 秒<br /><br /> 说明:查询通知保持为活动状态的秒数。|  
