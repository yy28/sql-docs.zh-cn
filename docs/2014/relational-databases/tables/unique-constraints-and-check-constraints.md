...
### <a name="limitations-of-check-constraints"></a>CHECK 约束的限制  
 CHECK 约束不接受计算结果为 FALSE 的值。 因为空值的计算结果为 UNKNOWN，所以表达式中存在这些值可能会覆盖约束。 例如，假设上放置约束`int`列**MyColumn**指定**MyColumn**可以包含值 10 (**MyColumn = 10**)。 如果将值 NULL 插入到 **MyColumn**， [!INCLUDE[ssDE](../../includes/ssde-md.md)] 将插入 NULL 且不返回错误。  
...
|任务|主题|  
|----------|-----------|  
|介绍如何创建唯一约束。|[创建唯一约束](../tables/create-unique-constraints.md)|  
|介绍如何修改唯一约束。|[修改唯一约束](../tables/modify-unique-constraints.md)|  
|介绍如何删除唯一约束。|[删除唯一约束](../tables/delete-unique-constraints.md)|  
|介绍当复制代理在表中插入或更新数据时如何禁用 CHECK 约束。|[对复制禁用 CHECK 约束](../tables/disable-check-constraints-for-replication.md)|  
|介绍在表中添加、更新或删除数据时如何禁用 CHECK 约束。|[对 INSERT 和 UPDATE 语句禁用 CHECK 约束](../tables/disable-check-constraints-with-insert-and-update-statements.md)|  
|介绍如何更改约束表达式或更改对特定条件启用或禁用约束的选项。|[修改 CHECK 约束](../tables/modify-check-constraints.md)|  
|介绍如何删除 CHECK 约束。|[删除 CHECK 约束](../tables/delete-check-constraints.md)|  
|介绍如何查看 CHECK 约束的属性。|[唯一约束和 CHECK 约束](../tables/unique-constraints-and-check-constraints.md)|  
