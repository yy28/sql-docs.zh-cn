...
|布尔表达式 (Boolean expression)|生成布尔值（true 或 false）的表达式。 此类表达式可以涉及比较（测试值是否相等，或对于非布尔值， \< [小于] 或 > [大于] 关系）和逻辑组合（使用布尔运算符，如 AND、OR 和 XOR）和布尔表达式。|  
...
|列|数据库表的每一行中的区域，它为该表建模的对象的某个属性存储数据值。|  
...
|条件表达式 (conditional expression)|生成布尔值（true 或 false）的表达式。 此类表达式可以涉及比较（测试值是否相等，或对于非布尔值， \< [小于] 或 > [大于] 关系）和逻辑组合（使用布尔运算符，如 AND、OR 和 XOR）和布尔表达式。|  
...
|key|标识缓存中的对象的字符串。 该字符串在区域中必须唯一。 在添加对象时对象与某个键相关联，然后使用相同键检索对象。|  
...
|页面|用更小的数据子集返回查询结果，这使用户可通过查看数据“页”来在结果集中导航。|  
|页面|在虚拟存储系统中，一个固定长度的连续虚拟地址块，在分页操作过程中它作为一个单元在内存和磁盘之间来回复制。|  
...
|路径|将一个数据流组件的输出连接到另一个数据流组件的输入的数据流元素。|  
...
|发布者|使数据可供复制到其他服务器的一种服务器。 发布服务器还检测更改的数据并且维护与站点上的所有发布有关的信息。|  
...
|SDK 中 IsInRole 中的声明|一组例程（通常在一个或多个库中），旨在使开发人员能够更轻松地为给定的计算机、操作系统或用户界面编写程序。|  
