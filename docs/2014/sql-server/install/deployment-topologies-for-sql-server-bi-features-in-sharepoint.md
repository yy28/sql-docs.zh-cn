---
title: 在 SharePoint 中的 SQL Server BI 功能的部署拓扑 |Microsoft 文档
ms.custom: ''
ms.date: 03/06/2017
 ...
 **[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] SharePoint 模式下**要求 SharePoint server 2013，并利用 SharePoint 服务应用程序体系结构。  
  
 下面的部分说明了典型的部署拓扑：  
  
###  <a name="bkmk_bi_Sharepoint2013_3tier"></a> PowerPivot for SharePoint 2013 和 Reporting Services 三服务器部署  
 在以下三服务器部署中，SQL Server 数据库引擎，[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]在 SharePoint 模式和 SharePoint，每次运行的独立服务器上运行服务器。 [!INCLUDE[ssGeminiShortvnext](../../includes/ssgeminishortvnext-md.md)] 2013年安装程序包 (**spPowerPivot.msi**) 必须在 SharePoint 服务器上运行。  
...
###  <a name="bkmk_powerpivot_sharepoint2013_1server"></a> PowerPivot for SharePoint 2013 单服务器部署  
 ...
 下图说明了单个服务器的一部分的组件[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]部署。  
...
###  <a name="bkmk_powerpivot_sharepoint2013_2server"></a> PowerPivot for SharePoint 2013 双服务器部署  
 在以下两个服务器部署中，SQL Server 数据库引擎和[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]在 SharePoint 模式下运行 SharePoint 的单独服务器上。 对于 SharePoint 2013，[!INCLUDE[ssGeminiLongvnext](../../includes/ssgeminilongvnext-md.md)]安装程序包 (**spPowerPivot.msi**) 在 SharePoint 服务器上安装。  
 ...
 该安装程序包作为 [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] 功能包的一部分提供。 可以从下载功能包[!INCLUDE[msCoName](../../includes/msconame-md.md)]下载中心[for Microsoft® SharePoint® 的 Microsoft® SQL Server® 2014 PowerPivot®](http://go.microsoft.com/fwlink/?LinkID=296473) (超链接"http://go.microsoft.com/fwlink/?LinkID=296473"\t"_blank" http://go.microsoft.com/fwlink/?LinkID=296473)。  
...
###  <a name="bkmk_powerpivot_sharepoint2013_3server"></a> PowerPivot for SharePoint 2013 三服务器部署  
 在以下三服务器部署中，SQL Server 数据库引擎，[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]在 SharePoint 模式和 SharePoint，每次运行的独立服务器上运行服务器。 [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)] 2013 安装程序包 (spPowerPivot.msi) 必须安装在 SharePoint 服务器上。  
  
 ![为 PowerPivot Mode3 服务器部署](../../../2014/sql-server/install/media/as-powerpivot-mode-3server-deployment.gif "作为 PowerPivot Mode3 服务器部署")  
...
###  <a name="bkmk_powerpivot_ssrs_sharepoint2013_1server"></a> PowerPivot for SharePoint 2013 和 Reporting Services 单服务器部署  
 单服务器部署用于测试目的，不建议在生产部署中采用。  
...
##  <a name="bkmk_sharepoint2010_1server"></a> 单服务器部署  
 ...
 若要在一台服务器上安装所有功能，请依次在同一物理服务器上安装 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 和 [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)]。 独立服务器配置上的说明，请参阅[部署清单： Reporting Services、 Power View 和 PowerPivot for SharePoint](deployment-checklist-reporting-services-power-view-power-pivot-for-sharepoint.md)。  
  
##  <a name="bkmk_sharepoint2010_2server"></a> 两层部署  
 两层部署通常是指在一台计算机上安装 SharePoint Server 2010，而在第二台计算机上安装 SQL Server 数据库引擎。 将数据层移到专用服务器是由 2 台计算机组成的场的最常见配置。 在两层场中，你安装[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]和[!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)]SharePoint 服务器上。 前端上的所有 Web 服务和应用程序层中的共享服务均在同一物理服务器上运行。 2 层部署的安装步骤与独立部署非常类似，都是在同一物理服务器上依次安装 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 和 [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)]。  
  
##  <a name="bkmk_sharepoint2010_3server"></a> 三层部署  
 通常，三层部署会将 Web 前端服务从处理功能或占用大量内存的应用程序中分离出来。 在此拓扑中，也安装[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]和[!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)]只是应用程序服务器上。 运行于 Web 前端的 Web 服务将作为安装后任务，在服务器配置期间通过部署到场中应用程序的解决方案进行安装。 下图说明了 3 层部署。  
...
##  <a name="bkmk_sharepoint2010_scaleserver"></a> 三层向外扩展部署  
 ...
 部署多服务器场时，请务必按照以下 SharePoint 文章中的说明执行操作： [三层服务器场的多个服务器 (SharePoint Server 2010)](http://go.microsoft.com/fwlink/?linkID=219834)。  
