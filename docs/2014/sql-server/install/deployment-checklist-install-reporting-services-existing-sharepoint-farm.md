...
|实例上确定[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]来承载报表服务器数据库。 可以使用本地或远程实例的[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。 您选择的实例所在计算机的存储容量应能够容纳报表。||  
...
## <a name="installation-scenarios"></a>安装方案  
 下表描述了可能的方案，当你在安装[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]到现有 SharePoint 场。 本地模式允许从 SharePoint 文档库中，而无需与集成从本地呈现报表[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]报表服务器。 用于 SharePoint 产品的 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 外接程序是必需的，但 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 报表服务器不是。 有关本地模式的详细信息，请参阅[报表查看器中的本地模式和连接模式下的报表在报表查看器中&#40;SharePoint 模式下的 Reporting Services&#41; ](../../../2014/reporting-services/local-vs-connected-mode-report-viewer-reporting-services-sharepoint-mode.md)并[在哪里可以找到 Reporting Services 外接程序用于 SharePoint 产品](../../reporting-services/install-windows/where-to-find-the-reporting-services-add-in-for-sharepoint-products.md)。  
