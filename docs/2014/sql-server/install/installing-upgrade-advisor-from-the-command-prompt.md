---
title: 从命令提示符安装升级顾问 |Microsoft 文档
ms.custom: ''
ms.date: 03/06/2017
...
|/L|指定日志文件选项。 若要记录到的所有消息*log_file_name*，使用 **-L\*v * * * log_file_name*。 若要记录仅错误消息，使用`-Le` *log_file_name*。|  
|ADDLOCAL = 所有&AMP;#124;删除 = 所有&AMP;#124;REINSTALL = ALL|指定安装 (ADDLOCAL)、删除 (REMOVE) 或重新安装 (REINSTALL) 升级顾问。|  
