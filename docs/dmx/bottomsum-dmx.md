...
## <a name="remarks"></a>备注  
 **BottomSum**函数以升序顺序返回最底层的行。 排名基于每行> 参数的\<排名表达式的计算值，因此， \<排名表达式> 值的总和至少是\<sum> 参数指定的给定总计。 当仍满足指定的 sum 值时， **BottomSum**将返回尽可能少数量的元素。  
...
|Road Bottle Cage|1195|0.080314537|0.077173962|  
 ...
 BottomSum 函数获取此查询的结果，并返回其值与指定计数之和的最小值的行。  
...
```  
 ...
 BottomSum 函数的第一个参数是表列的名称。 在此示例中，通过调用 Predict 函数并使用 INCLUDE_STATISTICS 参数来返回嵌套表。  
  
 BottomSum 函数的第二个参数是用于对结果进行排序的嵌套表中的列。 在此示例中，INCLUDE_STATISTICS 选项返回 $SUPPORT、$PROBABILTY 和 $ADJUSTED PROBABILITY 列。 此示例使用 $PROBABILITY 返回概率总值至少为 50% 的行。  
  
 BottomSum 函数的第三个参数将目标 sum 指定为 double。 若要获取概率总值为 10% 的最小计数的产品行，请键入 .1。  
 ...
 **注意**提供此示例只是为了说明 BottomSum 的用法。 运行此查询可能需要很长时间，具体取决于数据集的大小。  
