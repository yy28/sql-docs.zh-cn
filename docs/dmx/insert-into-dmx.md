...
[!INCLUDE[ssas-appliesto-sqlas](../includes/ssas-appliesto-sqlas.md)]
  ...
  处理指定的数据挖掘对象。 有关处理挖掘模型和挖掘结构的详细信息, 请参阅[处理要求和&#40;注意事项数据&#41;挖掘](https://docs.microsoft.com/analysis-services/data-mining/processing-requirements-and-considerations-data-mining)。  
...
## <a name="remarks"></a>备注  
 ...
 通过使用第二种语法形式, 将插入到 *\<对象 >* 中。COLUMN_VALUES, 您可以将数据直接插入模型列中, 而无需定型模型。 该方法以一种简练、有序的方式向模型提供列数据，在处理包含层次结构或有序列的数据集时，该方法很有用。  
  
 如果对挖掘模型或挖掘结构使用**INSERT INTO** , 并离开\<映射的模型列 > 和\<源数据查询 > 参数, 则该语句的行为类似于**ProcessDefault**, 使用绑定已存在。 如果绑定不存在，则语句将返回错误。 有关**ProcessDefault**的详细信息, 请参阅[处理选项和&#40;设置&#41;Analysis Services](https://docs.microsoft.com/analysis-services/multidimensional-models/processing-options-and-settings-analysis-services)。 下例说明了该语法：  
...
|插入包含源查询的挖掘模型 *\<模型 >*<br /><br /> 或<br /><br /> 插入包含源查询 *\<>* 挖掘结构结构中|结构或模型已包含内容。|进程失败。 在执行此操作之前, 必须通过使用[DELETE &#40;DMX&#41;](../dmx/delete-dmx.md)清除这些对象。|  
...
-   返回行集的任意 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 查询  
 ...
 有关数据源类型的详细信息, 请参阅[ &#60;源数据&#62;查询](../dmx/source-data-query.md)。  
