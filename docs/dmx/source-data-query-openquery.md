...
## <a name="remarks"></a>备注  
 **OPENQUERY**通过支持数据源权限提供了一种更安全的访问外部数据的方法。 由于连接字符串存储在数据源中，因此管理员可以使用数据源的属性来管理对数据的访问。 有关数据源的详细信息, 请参阅[支持的&#40;数据源 SSAS&#41;-多维](https://docs.microsoft.com/analysis-services/multidimensional-models/supported-data-sources-ssas-multidimensional)。  
...
## <a name="examples"></a>示例  
 下面的示例使用[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]数据库中已定义的 MyDS 数据源来创建[!INCLUDE[ssSampleDBDWobject](../includes/sssampledbdwobject-md.md)]与数据库的连接, 并查询**vTargetMail**视图。  
