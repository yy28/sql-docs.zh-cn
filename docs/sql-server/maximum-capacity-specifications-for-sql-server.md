...
>
>* [下载 SQL Server](https://www.microsoft.com/sql-server/sql-server-downloads_)
...
|内存优化表中的每行字节数||8,060|启动 [!INCLUDE[ssSQL15](../includes/sssql15-md.md)] 内存优化表支持行外存储。 如果表中的所有列的最大大小超过 8060 个字节，则长度可变的列被推送到行外；此操作是编译时的决定。 存储于行外的列仅有 8 字节的引用存储于行内。 有关详细信息，请参阅 [内存优化表中的表和行大小](../relational-databases/in-memory-oltp/table-and-row-size-in-memory-optimized-tables.md)。|
