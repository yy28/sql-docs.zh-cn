...
|功能|Enterprise|Standard|Web|Express with Advanced Services|Express|  
|-------------|----------------|--------------|---------|------------------------------------|------------------------|  
|Server Core 支持 <sup>1</sup>|用户帐户控制|是|是|是|用户帐户控制|  
|日志传送|用户帐户控制|是|是|否|否|  
|数据库镜像|用户帐户控制|用户帐户控制<br /><br /> 仅完全安全|仅见证服务器|仅见证服务器|仅见证服务器| 
|备份压缩|用户帐户控制|是|否|否|否| 
|数据库快照|用户帐户控制|是 <sup>3</sup>|是 <sup>3</sup>|是 <sup>3</sup>|是 <sup>3</sup>|
|AlwaysOn 故障转移群集实例|用户帐户控制<br /><br /> 节点数是操作系统支持的最大值|用户帐户控制<br /><br /> 支持 2 个节点|否|否|否|  
|AlwaysOn 可用性组|用户帐户控制<br /><br /> 支持最多 8 个次要副本，包括 2 个同步次要副本|否|否|否|否|
|基本可用性组 <sup>2</sup>|否|用户帐户控制<br /><br /> 支持 2 个节点|否|否|否|
|联机页面和文件还原|用户帐户控制|否|否|否|否|
|联机索引|用户帐户控制|否|否|否|否|
|联机架构更改|用户帐户控制|否|否|否|否|
|快速恢复|用户帐户控制|否|否|否|否|
|镜像备份|用户帐户控制|否|否|否|否|
|热插拔内存和 CPU|用户帐户控制|否|否|否|否|
|数据库恢复顾问|用户帐户控制|是|是|是|用户帐户控制|
|加密备份|用户帐户控制|是|否|否|否|
|Microsoft Azure 的混合备份（URL 的备份）|用户帐户控制|是|否|否|否|  
...
|功能|Enterprise|Standard|Web|Express with Advanced Services|Express|  
|-------------|----------------|--------------|---------|------------------------------------|------------------------| 
|列存储 <sup>1</sup>|用户帐户控制|是 <sup>2</sup>|是 <sup>2</sup>|是<sup>2</sup>|是<sup>2</sup>|  
|内存中 OLTP <sup>1</sup>|用户帐户控制|是 <sup>2</sup>|是 <sup>2</sup>|是 <sup>2</sup>， <sup>3</sup>|是 <sup>2</sup>|
|Stretch Database|用户帐户控制|是|是|是|用户帐户控制|
|永久性主内存|用户帐户控制|是|是|是|用户帐户控制|
|多实例支持|50|50|50|50|50|
|表和索引分区|用户帐户控制|是 <sup>2</sup>|是 <sup>2</sup>|是 <sup>2</sup>|是 <sup>2</sup>|  
|数据压缩|用户帐户控制|是 <sup>2</sup>|是 <sup>2</sup>|是 <sup>2</sup>|是 <sup>2</sup>|
|Resource Governor|用户帐户控制|否|否|否|否|  
|已分区表并行度|用户帐户控制|否|否|否|否|
|多个 Filestream 容器|用户帐户控制|是 <sup>2</sup>|是 <sup>2</sup>|是 <sup>2</sup>|是 <sup>2</sup>|
|NUMA 感知、大型页内存和缓冲区数组分配|用户帐户控制|否|否|否|否|
|缓冲池扩展|用户帐户控制|是|否|否|否|
|IO 资源调控|用户帐户控制|否|否|否|否|  
|延迟持续性|用户帐户控制|是|是|是|用户帐户控制|
...
|-------------|----------------|--------------|---------|-------------|------------------------------------| 
|行级安全性|用户帐户控制|用户帐户控制|是 <sup>1</sup>|是 <sup>1</sup>|是 <sup>1</sup>|  
|始终加密|用户帐户控制|是 <sup>1</sup>|是 <sup>1</sup>|是 <sup>1</sup>|是 <sup>1</sup>| 
|动态数据屏蔽|用户帐户控制|用户帐户控制|是 <sup>1</sup>|是 <sup>1</sup>|是 <sup>1</sup>|   
|基本审核|用户帐户控制|是|是|是|用户帐户控制| 
|精细审核|用户帐户控制|是 <sup>1</sup>|是 <sup>1</sup>|是 <sup>1</sup>|是 <sup>1</sup>| 
|透明数据库加密|用户帐户控制|否|否|否|否|   
|可扩展的密钥管理|用户帐户控制|否|否|否|否| 
|用户定义的角色|用户帐户控制|是|是|是|用户帐户控制| 
|包含的数据库|用户帐户控制|是|是|是|用户帐户控制| 
|备份加密|用户帐户控制|是|否|否|否|  
...
|功能|Enterprise|Standard|Web|Express with Advanced Services|Express|   
|-------------|----------------|--------------|---------|------------------------------------|------------------------| 
|异类订阅服务器|用户帐户控制|是|否|否|否|  
|合并复制|用户帐户控制|是|支持（仅订阅服务器）|支持（仅订阅服务器）|支持（仅订阅服务器）|   
|Oracle 发布|用户帐户控制|否|否|否|否| 
|对等事务复制|用户帐户控制|否|否|否|否|   
|快照复制|用户帐户控制|是|支持（仅订阅服务器）|支持（仅订阅服务器）|支持（仅订阅服务器）|   
|SQL Server 变更跟踪|用户帐户控制|是|是|是|用户帐户控制| 
|事务复制|用户帐户控制|是|支持（仅订阅服务器）|支持（仅订阅服务器）|支持（仅订阅服务器）|   
|事务复制到 Azure|用户帐户控制|是|否|否|否|   
|事务复制可更新的订阅|用户帐户控制|否|否|否|否|  
  
##  <a name="SSMS"></a> Management Tools  
  
|功能|Enterprise|Standard|Web|Express with Advanced Services|Express| 
|-------------|----------------|--------------|---------|------------------------------------|------------------------|  
|SQL 管理对象 (SMO)|用户帐户控制|是|是|是|用户帐户控制|  
|SQL 配置管理器|用户帐户控制|是|是|是|用户帐户控制|   
|SQL CMD（命令提示工具）|用户帐户控制|是|是|是|用户帐户控制|      
|Distributed Replay - 管理工具|用户帐户控制|是|是|是|否|  
|Distributed Replay - 客户端|用户帐户控制|是|是|否|否|  
|分布式重播 - 控制器|支持（最多 16 个客户端）|支持（1 个客户端）|支持（1 个客户端）|否|否|   
|SQL Profiler|用户帐户控制|用户帐户控制|否 <sup>1</sup>|否 <sup>1</sup>|否 <sup>1</sup>|  
|SQL Server 代理|用户帐户控制|是|是|否|否| 
|Microsoft System Center Operations Manager 管理包|用户帐户控制|是|是|否|否|  
|数据库优化顾问 (DTA)|用户帐户控制|是 <sup>2</sup>|是 <sup>2</sup>|否|否|      
...
|PowerShell 脚本支持|用户帐户控制|是|是|是|用户帐户控制| 
|SysPrep 支持 <sup>1</sup>|用户帐户控制|是|是|是|用户帐户控制| 
|支持数据层应用程序组件操作 - 提取、部署、升级、删除|用户帐户控制|是|是|是|用户帐户控制| 
|策略自动执行（检查计划和更改）|用户帐户控制|是|是|否|否|   
|性能数据收集器|用户帐户控制|是|是|否|否| 
|能够作为多实例管理中的托管实例注册|用户帐户控制|是|是|否|否|   
|标准性能报表|用户帐户控制|是|是|否|否| 
|计划指南和计划指南的计划冻结|用户帐户控制|是|是|否|否|   
|使用 NOEXPAND 提示的索引视图的直接查询|用户帐户控制|是|是|是|用户帐户控制| 
|自动索引视图维护|用户帐户控制|是|是|否|否| 
|分布式分区视图|用户帐户控制|否|否|否|否| 
|并行索引操作|用户帐户控制|否|否|否|否|  
|查询优化器自动使用索引视图|用户帐户控制|否|否|否|否| 
|并行一致性检查|用户帐户控制|否|否|否|否| 
|SQL Server 实用工具控制点|用户帐户控制|否|否|否|否|    
|缓冲池扩展|用户帐户控制|是|否|否|否| 
...
|功能|Enterprise|Standard|Web|Express with Advanced Services|Express| 
|-------------|----------------|--------------|---------|------------------------------------|------------------------| 
|Microsoft Visual Studio 集成|用户帐户控制|是|是|是|用户帐户控制| 
|Intellisense（Transact-SQL 和 MDX）|用户帐户控制|是|是|是|用户帐户控制| 
|SQL Server Data Tools (SSDT)|用户帐户控制|是|是|是|否|    
|MDX 编辑、调试和设计工具|用户帐户控制|是|否|否|否|   
...
|R Server (Standalone)|用户帐户控制|否|否|否|否|   
|Polybase 计算节点|用户帐户控制|是 <sup>1</sup>|是 <sup>1</sup>, <sup>2</sup>|是 <sup>1</sup>, <sup>2</sup>|是 <sup>1</sup>, <sup>2</sup>| 
|Polybase 头节点|用户帐户控制|否|否|否|否| 
|JSON|用户帐户控制|是|是|是|用户帐户控制|   
|查询存储|用户帐户控制|是|是|是|用户帐户控制|   
|临时|用户帐户控制|是|是|是|用户帐户控制|   
|公共语言运行时 (CLR) 集成|用户帐户控制|是|是|是|用户帐户控制|   
|本机 XML 支持|用户帐户控制|是|是|是|用户帐户控制| 
|XML 索引|用户帐户控制|是|是|是|用户帐户控制| 
|MERGE 和 UPSERT 功能|用户帐户控制|是|是|是|用户帐户控制|   
|FILESTREAM 支持|用户帐户控制|是|是|是|用户帐户控制| 
|FileTable|用户帐户控制|是|是|是|用户帐户控制| 
|日期和时间数据类型|用户帐户控制|是|是|是|用户帐户控制|  
|国际化支持|用户帐户控制|是|是|是|用户帐户控制| 
...
|功能|Enterprise|Standard|Web|Express with Advanced Services|Express|   
|-------------|----------------|--------------|---------|------------------------------------|------------------------| 
|在无需数据库的情况下创建多维数据集|用户帐户控制|是|否|否|否 |   
|自动生成临时和数据仓库架构|用户帐户控制|是|否|否|否| 
|变更数据捕获|用户帐户控制|是 <sup>1</sup>|否|否|否| 
|星型联接查询优化|用户帐户控制|否|否|否|否| 
|可扩展的只读 Analysis Services 配置|用户帐户控制|否|否|否|否| 
|针对已分区表和索引的并行查询处理|用户帐户控制|否|否|否|否|   
|全局批处理集成|用户帐户控制|否|否|否|否| 
...
|------------------|----------------|--------------|---------|------------------------------------|------------------------|
|空间索引|用户帐户控制|是|是|是|用户帐户控制|   
|平面和大地测量数据类型|用户帐户控制|是|是|是|用户帐户控制| 
|高级空间库|用户帐户控制|是|是|是|用户帐户控制|   
|导入/导出业界标准的空间数据格式|用户帐户控制|是|是|是|用户帐户控制|   
  
##  <a name="ADS"></a> Additional Database Services  
  
|功能名称|Enterprise|Standard|Web|Express with Advanced Services|Express|   
|------------------|----------------|--------------|---------|------------------------------------|------------------------| 
|[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 迁移助手|用户帐户控制|是|是|是|用户帐户控制|   
|数据库邮件|用户帐户控制|是|是|否|否| 
