...
|![R Server](../sql-server/media/r-server.png "R Server")|**[机器学习服务](../advanced-analytics/r-services/r-services.md)**<br /><br /> Microsoft 机器学习服务支持使用常用 R 和 Python 语言，将机器学习集成到企业工作流中。<br /><br /> 机器学习服务（数据库内）将 R 和 Python 与 SQL Server 集成，方便用户通过调用存储过程，轻松生成、重新定型模型，并对模型评分。<br /><br /> Microsoft 机器学习服务器对 R 和 Python 提供企业级支持，用户无需使用 SQL Server。|    
