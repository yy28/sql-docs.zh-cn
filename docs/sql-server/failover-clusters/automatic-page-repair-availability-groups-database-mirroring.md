...
helpviewer_keywords: 
  - "自动页修复"
  - "可用性组 [SQL Server], 自动页修复"
  - "数据库镜像 [SQL Server], 自动页修复"
  - "可疑页 [SQL Server]"
...
##  <a name="ErrorTypes"></a> 会导致自动页修复尝试的错误类型  
 数据库镜像自动页修复只尝试修复特定数据文件中的页，此数据文件是指对其执行的操作由于下表中列出的某一错误而失败的数据文件。  
  
|错误号|说明|导致自动页修复尝试的实例|  
|------------------|-----------------|---------------------------------------------------------|  
|[823](../Topic/MSSQLSERVER_823.md)|仅当操作系统对数据执行循环冗余检查 (CRC) 失败时才执行此操作。|ERROR_CRC。 此错误的操作系统值为 23。|  
|[824](../Topic/MSSQLSERVER_824.md)|逻辑错误。|逻辑数据错误，例如残缺写或错误的页校验和。|  
|829|页已标记为还原已挂起。|全部。|  
  
 若要查看最近的 823 CRC 错误和 824 错误，请参阅 [msdb](../../relational-databases/databases/msdb-database.md) 数据库中的 [suspect_pages](../../relational-databases/system-tables/suspect-pages-transact-sql.md) 表。  
  
 [[返回页首]](#Top)  
  
##  <a name="UnrepairablePageTypes"></a> 无法自动修复的页类型  
 页自动修复不能修复以下控制页类型：  
...
##  <a name="ViewAPRattempts"></a> 如何查看自动页修复尝试  
