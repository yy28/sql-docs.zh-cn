...
-   所有迁移策略都需要 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] sysadmin 角色。 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] 服务所用的所有 Windows 用户（即运行复制代理的 Windows 帐户）都必须具有新环境中每台计算机的 OS 级别权限。
...
适用的迁移策略取决于原始 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] 群集拓扑的某些参数，即 [!INCLUDE[sshadrc-md](../../../includes/sshadrc-md.md)]的使用情况和 SQL 故障转移群集实例。 所选策略还取决于目标环境的需求。 如果新环境要求每台计算机或 SQL FCI 保留原始虚拟网络名称 (VNN)，或如果 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] 拓扑所依赖的新实例继承了原始实例的所有系统对象，则必须选择可迁移这些对象的策略。
...
1.  根据需要将任意实例升级到 [!INCLUDE[sssql15-md](../../../includes/sssql15-md.md)] 或更高版本。 并行实例应运行相同版本的 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)]。
...
1.  进行完整备份，并停止通向原始 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] 群集的流量。
...
8.  在故障转移群集管理器的原始环境中，更改每个 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] FCI 角色的“服务器名称”资源的名称。

9.  现在，只需将每个 SQL FCI 角色已重命名的服务器名称资源重新联机。

10. 现在，在故障转移群集管理器中的目标群集上，将每个 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] FCI 角色的“服务器名称”资源重命名为原始群集以前保留的名称。
...
9.  在故障转移群集管理器的原始环境中，更改每个 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] FCI 角色的“服务器名称”资源的名称。

10. 现在，只需将每个 SQL FCI 角色已重命名的服务器名称资源重新联机。

11. 现在，在故障转移群集管理器中的目标群集上，将每个 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] FCI 角色的“服务器名称”资源重命名为原始群集以前保留的名称。

12. 重命名所有 FCI 后，在新群集中重启每台计算机。

13. 随着计算机重启后恢复联机，在故障转移群集管理器中启动每个 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] FCI 角色。
...
5.  在故障转移群集管理器中的原始群集上，关闭每个 SQL FCI 群集角色，并在 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] 配置管理器中停止 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] 独立实例。
...
9.  在故障转移群集管理器的原始环境中，将每个 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] FCI 角色的“服务器名称”资源更改为唯一的新名称。
...
13. 现在，在故障转移群集管理器中的目标群集上，将每个 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] FCI 角色的“服务器名称”资源重命名为原始群集以前保留的名称。
...
6.  在故障转移群集管理器中的原始群集上，关闭每个 SQL FCI 群集角色，并在 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] 配置管理器中停止 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] 独立实例。
...
10. 在故障转移群集管理器的原始环境中，将每个 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] FCI 角色的“服务器名称”资源更改为唯一的新名称。
...
14. 现在，在故障转移群集管理器中的目标群集上，将每个 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] FCI 角色的“服务器名称”资源重命名为原始群集以前保留的名称。

15. 重命名所有 FCI 后，在新群集中重启每台计算机。

16. 随着计算机重启后恢复联机，在故障转移群集管理器中启动每个 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] FCI 角色。
...
-   **数据库****镜像****终结点**
    ...
    从 SQL 的角度来看，数据库镜像终结点与系统表一起迁移到新的 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] 实例。 在迁移之前，请确保已在防火墙中应用正确的规则，并且没有其他进程在侦听同一端口。
...
-   **远程****分发服务器**、**发布服务器**、**订阅服务器**
    ...
    分发服务器和发布服务器之间的关系只依赖于承载这二者的计算机的 VNN，该 VNN 可正确解析为新计算机。 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] 代理作业也会随系统表一起正确地迁移，因此各种复制代理能够照常继续执行。 在迁移之前，任何运行 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] 代理本身或任何 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] 代理作业的 Windows 帐户必须在目标环境中拥有相同的权限。 与发布服务器和订阅服务器的通信将照常执行。

-   **快照****文件夹**

    在迁移之前，任何 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] 功能所用的任何网络共享需要可供目标环境中的计算机（具有与原始环境相同的权限）访问。 需要先确保满足此条件才可进行迁移。

### <a name="service-broker"></a>Service broker

-   **Service** **Broker** **端点**

    从 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] 的角度来看，没有与终结点相关的问题。 在迁移之前，必须确保没有进程正在侦听同一端口，并且没有任何防火墙规则阻止该端口，或者存在专门允许该端口的防火墙规则。
...
### <a name="includessnoversionincludesssnoversionmd-agent"></a>[!INCLUDE[ssNoVersion](../../../includes/ssnoversion.md)] 代理
