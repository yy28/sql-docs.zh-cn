...
- [SQL Server Reporting Services 发行说明](../reporting-services/reporting-services-发行说明.md)发布于 Reporting Services 部分。
...
**问题和对客户的影响：**在 [!INCLUDE[ssMDSshort_md](../includes/ssmdsshort-md.md)] 中将实体事务日志类型设置为“属性”（默认值为“成员”）时，以下方案会失败：
...
具体而言，虽然以下方案可能适用于你，但尚未完全通过测试且在 [!INCLUDE[ssSQLv14_md](../includes/sssqlv14-md.md)] 中**不**受支持：
- 卸载 CTP 1.1。
