---
title: 设备管理任务
description: 本指南介绍如何为分析平台系统（AP）执行设备管理任务。
author: mzaman1
ms.prod: sql
ms.technology: data-warehouse
ms.topic: conceptual
ms.date: 04/17/2018
ms.author: murshedz
ms.reviewer: martinle
ms.custom: seo-dt-2019
ms.openlocfilehash: 51925c5657d0381b9e107dc5ce3cf6fd88366bdf
ms.sourcegitcommit: e042272a38fb646df05152c676e5cbeae3f9cd13
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 04/27/2020
ms.locfileid: "74401428"
---
# <a name="appliance-management-tasks-for-analytics-platform-system"></a>用于分析平台系统的设备管理任务
本指南介绍如何为分析平台系统（AP）执行设备管理任务。  
  
## <a name="guide-sections"></a>指南部分  
  
-   [设备安装和配置概述 &#40;分析平台系统&#41;](appliance-installation-and-configuration-overview.md)  
  
-   [&#40;分析平台系统连接到设备节点&#41;](connect-to-appliance-nodes.md)  
  
-   [防病毒软件 &#40;分析平台系统&#41;](antivirus-software.md)  
  
-   [设备配置 &#40;分析平台系统&#41;](appliance-configuration.md)  
  
-   [软件服务 &#40;分析平台系统&#41;](software-servicing.md)  
  
-   [PDW 服务状态 &#40;分析平台系统&#41;](pdw-services-status.md)  
  
-   [&#40;Analytics Platform System&#41;打开或关闭 AP 设备](power-the-aps-appliance-on-or-off.md)  
  
-   [设备监视 &#40;分析平台系统&#41;](appliance-monitoring.md)  
  
