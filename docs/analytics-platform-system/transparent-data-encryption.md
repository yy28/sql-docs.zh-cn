---
title: 透明数据加密的并行数据仓库 |Microsoft 文档
description: 透明数据加密 (TDE) 并行数据仓库 (PDW) 执行实时 I/O 加密和解密的数据和事务日志文件和特殊 PDW 日志文件。"
author: mzaman1
manager: craigg
ms.prod: sql
...
ms.locfileid: "31539147"
---
# <a name="transparent-data-encryption"></a>透明数据加密
您可以采取一些预防措施来帮助保护数据库的安全，如设计一个安全系统、加密机密资产以及在数据库服务器的周围构建防火墙。 但是，对于在其中的物理媒体 （如驱动器或备份磁带） 被盗的情况下，恶意方可以只需还原或附加该数据库并浏览的数据。 一种解决方案是加密数据库中的敏感数据，并通过证书保护用于加密数据的密钥。 这可以防止任何没有密钥的人使用这些数据，但这种保护必须事先计划。  
  
*透明数据加密*(TDE) 执行实时 I/O 加密和解密的数据和事务日志文件和特殊 PDW 日志文件。 这种加密使用数据库加密密钥 (DEK)，该密钥存储在数据库引导记录中以供恢复时使用。 DEK 是使用 SQL Server PDW 的 master 数据库中存储的证书保护的对称密钥。 TDE 保护“处于休眠状态”的数据，即数据和日志文件。 它提供了遵从许多法律、法规和各个行业建立的准则的能力。 此功能允许软件开发人员而无需更改现有应用程序使用 AES 和 3DES 加密算法来加密数据。  
...
3.  使用[sp_pdw_add_network_credentials](../relational-databases/system-stored-procedures/sp-pdw-add-network-credentials-sql-data-warehouse.md)创建可以进行身份验证并写入存储证书的备份的共享的凭据。 如果已存在目标的存储服务器的凭据，则可以使用现有的凭据。  
...
**第一步： 在上启用 TDE 的 SQL Server PDW。** 此操作一次才有必要。  
...
**第二个： 创建并备份 master 数据库中的证书。** 此操作才必须使用一次。 你可以为每个数据库 （推荐），一个单独的证书，或可以保护多个数据库使用一个证书。  
...
**上次： 创建 DEK 和使用 ALTER DATABASE 来加密用户数据库。** 为受 TDE 的每个数据库重复此操作。  
...
加密和解密操作由 SQL Server 安排在后台线程中执行。 你可以查看使用更高版本在本文中显示的列表中的目录视图和动态管理视图这些操作的状态。  
...
你可以找到的数据库使用的加密状态**sys.dm_pdw_nodes_database_encryption_keys**动态管理视图。 有关详细信息，请参阅*目录视图和动态管理视图*本文前面部分。  
...
在 PDW 中手动执行这些语句后，重启设备，以便恢复 DMK:  
...
以下链接包含有关 SQL Server 如何管理加密的一般信息。 这些文章可帮助你了解 SQL Server 加密，但这些文章不具有特定于 SQL Server PDW 的信息，并且他们会讨论在 SQL Server PDW 中不存在的功能。  
