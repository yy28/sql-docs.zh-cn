---
title: "基本 ODBC 应用程序步骤 |Microsoft 文档"
ms.custom: 
ms.date: 01/19/2017
ms.prod: sql-non-specified
ms.prod_service: drivers
ms.service: 
ms.component: reference
ms.reviewer: 
ms.suite: sql
ms.technology:
- drivers
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- application process [ODBC]
- application process [ODBC], about application process
ms.assetid: a92d1f78-c669-47ad-88c4-0b1a93503dfc
caps.latest.revision: 7
author: MightyPen
ms.author: genemi
manager: jhubbard
ms.workload: Inactive
ms.translationtype: MT
ms.sourcegitcommit: f7e6274d77a9cdd4de6cbcaef559ca99f77b3608
ms.openlocfilehash: fbafffd1922dabbb49bec1cae59d6bce099bd5d7
ms.contentlocale: zh-cn
ms.lasthandoff: 09/09/2017

---
# <a name="basic-odbc-application-steps"></a>基本 ODBC 应用程序步骤
本部分介绍了的 ODBC 应用程序的常规流程。 不太可能任何应用程序按此顺序完全调用所有这些函数。 但是，大多数应用程序使用这些步骤的某些变体。 基本应用程序步骤下图所示。  
  
 ![ODBC 应用程序的基本步骤](../../../odbc/reference/develop-app/media/pr10.gif "pr10")  
  
 本部分包含以下主题。  
  
-   [步骤 1： 连接到数据源](../../../odbc/reference/develop-app/step-1-connect-to-the-data-source.md)  
  
-   [步骤 2： 初始化应用程序](../../../odbc/reference/develop-app/step-2-initialize-the-application.md)  
  
-   [步骤 3： 生成并执行的 SQL 语句](../../../odbc/reference/develop-app/step-3-build-and-execute-an-sql-statement.md)  
  
-   [步骤 4a： 提取结果](../../../odbc/reference/develop-app/step-4a-fetch-the-results.md)  
  
-   [步骤 4b： 提取的行计数](../../../odbc/reference/develop-app/step-4b-fetch-the-row-count.md)  
  
-   [步骤 5： 提交事务](../../../odbc/reference/develop-app/step-5-commit-the-transaction.md)  
  
-   [步骤 6： 从数据源断开连接](../../../odbc/reference/develop-app/step-6-disconnect-from-the-data-source.md)

