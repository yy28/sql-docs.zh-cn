...
|SQL_ATTR_TXN_ISOLATION|第1级/第2级 [2]|  
 ...
 [1] 支持级别 1) 所需的连接级异步 (的应用程序必须支持通过调用 **SQLSetConnectAttr**将此特性设置为 SQL_TRUE;不需要通过 **SQLSetStmtAttr**将该属性设置为其默认值以外的值。 支持级别 2) 所需的语句级别异步 (的应用程序必须支持使用任一函数将此特性设置为 SQL_TRUE。  
  
 [2] 对于级别1接口一致性，驱动程序必须支持一个值，除了使用 SQL_DEFAULT_TXN_ISOLATION 选项) 调用 **SQLGetInfo** 时，驱动程序定义的默认值 (可用。 对于级别2接口一致性，驱动程序还必须支持 SQL_TXN_SERIALIZABLE。  
