---
title: 步骤 4a： 提取结果 |Microsoft Docs
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
...
下一步是提取的结果，如以下插图所示。  
 ...
 如果在"步骤 3:: 生成和执行 SQL 语句"中执行的语句**选择**语句或目录函数，该应用程序首先调用**SQLNumResultCols**来确定的列数在结果集中。 此步骤不是结果的必需的如果应用程序已知道设置列，例如，当 SQL 语句为垂直或自定义应用程序中硬编码数的。  
 ...
 应用程序现在返回到"步骤 3:: 生成和执行 SQL 语句"来执行另一条语句在同一个事务;或继续到"步骤 5:: Commit Transaction"来提交或回滚事务。
