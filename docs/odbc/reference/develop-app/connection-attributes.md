...
连接属性是连接的特征。 例如，因为事务发生在连接级别，所以事务隔离级别就是一个连接属性。 同样，在超时前尝试连接时的登录超时值或等待的秒数是连接属性。  
 ...
 连接属性可以在连接之前或之后设置，也可以根据属性和驱动程序设置。 登录超时值（SQL_ATTR_LOGIN_TIMEOUT）适用于连接过程，并且仅在连接前设置时有效。 指定是否使用 ODBC 游标库（SQL_ATTR_ODBC_CURSORS）和网络数据包大小（SQL_ATTR_PACKET_SIZE）的属性必须在连接前设置，因为 ODBC 游标库驻留在驱动程序管理器和驱动程序之间，因此必须在驱动程序之前加载。  
