 ...
 **SQLDriverConnect**中的关键字/值对必须遵循某些语法规则。 关键字及其值不应包含 **[] {} ( # A1，;？ \*=！ @** 个字符。 **DSN**关键字的值不能只包含空格，并且不能包含前导空格。 由于注册表语法的原因，关键字和数据源名称不能包含反斜杠 (\\) 字符。 关键字-值对中的等号前后不允许有空格。  
  
 **FILEDSN**关键字可在对**SQLDriverConnect**的调用中使用，以指定包含数据源信息的文件的名称 (参阅本部分后面的[使用文件数据源进行连接](../../../odbc/reference/develop-app/connecting-using-file-data-sources.md)) 。 **SAVEFILE**关键字可用于指定 .dsn 文件的名称，在该文件中，将保存通过调用**SQLDriverConnect**进行的成功连接的关键字值对。 有关文件数据源的详细信息，请参阅 [SQLDriverConnect](../../../odbc/reference/syntax/sqldriverconnect-function.md) 函数说明。
