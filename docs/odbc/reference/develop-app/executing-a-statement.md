...
ms.locfileid: "67901347"
---
# <a name="executing-a-statement"></a>执行语句
有四种方法可以执行语句，具体取决于它们是由数据库引擎编译（准备）的还是定义这些语句的人员：  
...
-   **目录函数**驱动程序编写器创建一个返回预定义结果集的函数。 通常，此函数会提交预定义的 SQL 语句，或调用为此目的而创建的过程。 函数在运行时在一次或多次执行。  
 ...
 可以多次执行特定语句（由其语句句柄标识）。 语句可以使用各种不同的 SQL 语句执行，也可以使用相同的 SQL 语句重复执行。 例如，下面的代码使用同一个语句句柄（*hstmt1*）来检索和显示 Sales 数据库中的表。 然后重新使用此句柄来检索用户选择的表中的列。  
...
```  
 ...
 应用程序可以使用 SQL_ATTR_QUERY_TIMEOUT 语句特性来限制为语句分配的时间量。 如果超时时间已过，则在数据源返回结果集之前，执行 SQL 语句的函数返回 SQLSTATE HYT00 （超时已过期）。 默认情况下没有任何超时。  
