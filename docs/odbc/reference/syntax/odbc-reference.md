---
title: "ODBC 引用 |Microsoft 文档"
ms.custom: 
ms.date: 01/19/2017
ms.prod: sql-non-specified
ms.reviewer: 
ms.suite: 
ms.technology:
- drivers
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- API reference [ODBC]
- ODBC [ODBC], API reference
ms.assetid: 1ba0f6cc-dfa7-4fe8-8bc2-f862b386156d
caps.latest.revision: 8
author: MightyPen
ms.author: genemi
manager: jhubbard
ms.translationtype: MT
ms.sourcegitcommit: f7e6274d77a9cdd4de6cbcaef559ca99f77b3608
ms.openlocfilehash: 310395e70e4a18aa1c39a9b615a8edbc3031bb5d
ms.contentlocale: zh-cn
ms.lasthandoff: 09/09/2017

---
# <a name="odbc-reference"></a>ODBC 引用
以下主题包含的语法和语义信息适用于所有 ODBC 功能。  
  
-   [函数摘要](../../../odbc/reference/syntax/function-summary.md)  
  
-   [ODBC API 参考](../../../odbc/reference/syntax/odbc-api-reference.md)  
  
-   [Setup DLL API 函数](../../../odbc/reference/syntax/setup-dll-api-reference.md)  
  
-   [安装程序 DLL API 参考](../../../odbc/reference/syntax/installer-dll-api-reference-function.md)  
  
-   [转换 DLL 的函数参考](../../../odbc/reference/syntax/translation-dll-api-reference.md)  
  
-   [ODBC 服务提供程序接口 (SPI) 参考](../../../odbc/reference/syntax/odbc-service-provider-interface-spi-reference.md)
