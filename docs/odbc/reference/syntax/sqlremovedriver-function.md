 ...
 **摘要**  
 **SQLRemoveDriver**更改或删除驱动程序有关的信息从 Odbcinst.ini 条目中的系统信息。  
 ...
 *fRemoveDSN*  
 ...
 TRUE:删除与驱动程序中指定的 Dsn *lpszDriver*。 FALSE:不删除与驱动程序中指定的 Dsn *lpszDriver*。  
...
|ODBC_ERROR_REQUEST_FAILED|请求失败|*FRemoveDSN*参数为 TRUE; 但是，无法删除一个或多个 Dsn。 在调用**SQLConfigDriver** ODBC_REMOVE_DRIVER 请求失败。|  
|ODBC_ERROR_OUT_OF_MEM|内存不足|由于内存不足，安装程序无法执行该函数。|  
  
## <a name="comments"></a>注释  
 **SQLRemoveDriver**进行了补充[SQLInstallDriverEx](../../../odbc/reference/syntax/sqlinstalldriverex-function.md)函数和更新组件使用情况的系统信息中的数。 只能从安装程序应用程序应调用此函数。  
  
 **SQLRemoveDriver**将按 1 递减的组件使用情况计数值。 如果组件使用情况计数达到 0，将执行以下操作：  
...
2.  系统信息中的驱动程序条目将被删除。 Driver 项是在以下系统信息位置，在驱动程序名称下：  
 ...
 **SQLRemoveDriver**不会实际删除任何文件。 调用程序负责删除文件和维护文件使用情况计数。 组件使用情况计数和文件使用率计数已达到仅后零是以物理方式已删除的文件。 可以删除一个组件中的某些文件，并且其他人不会删除，具体取决于是否使用具有递增文件使用情况计数的其他应用程序的文件。  
  
 **SQLRemoveDriver**也称为升级过程的一部分。 如果应用程序检测到它必须执行升级和它以前已安装的驱动程序，应删除并再重新安装该驱动程序。 **SQLRemoveDriver**首先应调用要递减的组件使用计数，然后**SQLInstallDriverEx**应调用来增加的组件使用情况计数。 应用程序安装程序必须将新文件替换为旧的文件。 文件使用情况计数将保持不变，并使用较旧版本文件的其他应用程序现在将使用较新版本。  
