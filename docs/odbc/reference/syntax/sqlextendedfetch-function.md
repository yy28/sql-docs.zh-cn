...
> [!NOTE]
>  在 ODBC 3.x*中，* **SQLExtendedFetch**已被**SQLFetchScroll**取代。 ODBC 2.x*应用程序不*应调用**SQLExtendedFetch**;应改为调用**SQLFetchScroll**。 当使用 ODBC*2.x 驱动程序*时，驱动程序管理器会将**SQLFetchScroll**映射到**SQLExtendedFetch** 。 如果 ODBC*2.x 驱动程序*要使用 odbc 1.x 应用程序调用它，则应该** 支持**SQLExtendedFetch** 。 有关详细信息，请参阅附录 G：驱动程序准则中的 "注释" 和 "注释" 和 "[阻止游标" 和向后兼容性](../../../odbc/reference/appendixes/block-cursors-scrollable-cursors-and-backward-compatibility.md)，以实现向后兼容性。  
