...
-   SQL_PARAM_UNUSED：未使用此参数集，这可能是由于某些先前的参数集导致了导致进一步处理终止的错误，或者是因为在指定的数组中为该参数集设置了 SQL_PARAM_IGNORE SQL_DESC_ARRAY_APD 的 STATUS_PTR 字段。  
 ...
 **SQL_DESC_BIND_OFFSET_PTR [应用程序描述符]**  
 此 SQLLEN * 标头字段指向绑定偏移量。 默认情况下，它设置为 null 指针。 如果此字段不是 null 指针，则驱动程序将取消引用指针，并将取消引用的值添加到描述符记录中具有非 null 值的每个延迟字段（SQL_DESC_DATA_PTR、SQL_DESC_INDICATOR_PTR 和 SQL_DESC_OCTET_LENGTH_PTR）在提取时，并在绑定时使用新的指针值。  
 ...
 **SQL_DESC_CASE_SENSITIVE [实现描述符]**  
 如果列或参数被视为区分大小写和比较区分大小写，则此只读 SQLINTEGER 记录字段包含 SQL_TRUE; 如果该列不是针对排序规则和比较区分大小写，或者是非字符，则为 SQL_FALSE。该列.  
 ...
 **SQL_DESC_PRECISION [All]**  
 此 SQLSMALLINT 记录字段包含精确数值类型的位数、近似数值类型的尾数（二进制精度）中的位数或 SQL_TYPE_TIME 的秒小数部分中的数字个数 SQL_TYPE_TIMESTAMP 或 SQL_INTERVAL_SECOND 数据类型。 对于所有其他数据类型，此字段未定义。  
