...
|DiagIdentifier|返回类型|返回|  
|--------------------|-----------------|-------------|  
|SQL_DIAG_CLASS_ORIGIN|SQLCHAR|一个字符串，指示定义此记录中 SQLSTATE 值的类部分的文档。 对于开放组和 ISO 调用级别接口定义的所有 SQLSTATEs，其值为 "ISO 9075"。 对于特定于 ODBC 的 SQLSTATEs (其 SQLSTATE 类为 "IM" ) 的所有项，其值为 "ODBC 3.0"。|  
