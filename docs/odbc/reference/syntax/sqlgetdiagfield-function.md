...
> [!NOTE]
>  如果 odbc 2.x*驱动程序返回*了 SQLSTATE 01S01 （行中的错误），或者 odbc*1.x 驱动程序*返回 SQLSTATE 01S01 （行中的错误），则 Odbc 1.X 驱动程序管理器不会在诊断队列中对状态记录进行 *.x*排序，**或在已**与**SQLExtendedFetch**定位的游标上调用**SQLSetPos** 。  
