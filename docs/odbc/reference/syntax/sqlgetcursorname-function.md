 ...
 *NameLengthPtr*  
 输出一个指向内存的指针，将在此内存中返回 (不包括 null 终止字符) 可在 CursorName 中返回的字符总数 \* *CursorName*。 如果可返回的字符数大于或等于*BufferLength*，则 CursorName 中的游标名称将 \* *CursorName*被截断为*BufferLength*减去 null 终止字符的长度。  
