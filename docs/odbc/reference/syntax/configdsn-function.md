...
## <a name="modifying-a-data-source"></a>修改数据源  
 ...
 如果*hwndParent*为 null，则 ConfigDSN 将使用*lpszAttributes*中的信息来修改**ConfigDSN**文件中的信息（或注册表）。 如果*hwndParent*不为 null，则**ConfigDSN**将使用*lpszAttributes*中的信息显示一个对话框;对于不在*lpszAttributes*中的信息，它使用系统信息中的信息。 在**ConfigDSN**将信息存储在系统信息中之前，用户可以修改这些信息。  
