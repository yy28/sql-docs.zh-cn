...
ODBC C 数据类型指示用于在应用程序中存储数据的 C 缓冲区的数据类型。  
 ...
 在带有*TargetType*参数的**SQLBindCol**和**SQLGetData**函数中指定 C 数据类型，并在**SQLBindParameter**函数中指定带有*ValueType*参数的。 还可以通过调用**SQLSetDescField**来指定 ARD 或 APD 的 SQL_DESC_CONCISE_TYPE 字段，或**通过使用***类型*参数（和*子类型*参数，如需要）和*DescriptorHandle*参数设置为 ARD 或 APD 的句柄来指定。  
...
|SQL_C_USHORT [j]|SQLUSMALLINT|无符号短整型|  
|SQL_C_SLONG [j]|SQLINTEGER|long int|  
|SQL_C_ULONG [j]|SQLUINTEGER|无符号长整型|  
|SQL_C_FLOAT|SQLREAL|FLOAT|  
|SQL_C_DOUBLE|SQLDOUBLE、SQLFLOAT|double|  
|SQL_C_BIT|SQLCHAR|无符号字符|  
|SQL_C_STINYINT [j]|SQLSCHAR|signed char|  
|SQL_C_UTINYINT [j]|SQLCHAR|无符号字符|  
...
```  
 ...
 [a] datetime C 数据类型中的 year、month、day、hour、minute 和 second 字段的值必须符合公历的约束。 （请参阅本附录后面的[公历的约束](../../../odbc/reference/appendixes/constraints-of-the-gregorian-calendar.md)。）  
  
 [b] "分数" 字段的值是十亿分之一的数目，范围为0到999999999（1小于1000000000）。 例如，半秒的 "分数" 字段的值为500000000，每秒（1毫秒）的值为1000000，第二个秒的值（1微秒）为1000，为第二个十亿分之一（一个毫微秒）为1。  
 ...
 [e] 在 little endian 模式下，数字以缩放整数的形式存储在 SQL_NUMERIC_STRUCT 结构的*val*字段中（最左边的字节是最小有效字节）。 例如，数字10.001 基数10，小数位数为4，将扩展为100010的整数。 因为这是十六进制格式的186AA，SQL_NUMERIC_STRUCT 中的值将是 "AA 86 01 00 00 .。。00 "，包含 SQL_MAX_NUMERIC_LEN **#define**定义的字节数。  
  
 有关**SQL_NUMERIC_STRUCT**的详细信息，请参阅[如何： SQL_NUMERIC_STRUCT 检索数值数据](retrieve-numeric-data-sql-numeric-struct-kb222831.md)。  
  
 [f] SQL_C_NUMERIC 数据类型的精度和小数位数字段进行，用于从应用程序进行输入以及从驱动程序输出到应用程序。 当驱动程序将数值写入 SQL_NUMERIC_STRUCT 时，它将使用自己的特定于驱动程序的默认值作为*精度*字段的值，并且它将使用 "*缩放*" 字段的应用程序描述符的 "SQL_DESC_SCALE" 字段中的值（默认值为0）。 应用程序可以通过设置应用程序描述符的 "SQL_DESC_PRECISION" 和 "SQL_DESC_SCALE" 字段为精度和小数位数提供自己的值。  
