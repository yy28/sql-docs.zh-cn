...
|**INSERT(** _string_exp1_, *start*, *length*, _string_exp2_**)**  (ODBC 1.0)|返回一个字符的字符串的位置*长度*已从删除字符*string_exp1*，起点*启动*，以及在何处*string_exp2*是否已插入*string_exp，* 开始*启动*。|  
|**LCASE(** _string_exp_ **)** (ODBC 1.0)|返回一个字符串等于的中*string_exp*，所有大写字符转换为小写字符。|  
|**LEFT(** _string_exp_, _count_**)** (ODBC 1.0)|返回最左侧*计数*个字符*string_exp*。|  
...
|**RIGHT(** _string_exp_, _count_**)** (ODBC 1.0)|返回最右侧*计数*个字符*string_exp*。|  
...
|**SPACE(** _count_ **)** (ODBC 2.0)|返回一个字符串组成*计数*空格。|  
