...
-   **数据的字节长度**-数字可发送给数据源的 SQL 数据的字节，该值指示是否将会截断数据发送到数据源之前。 对于字符串数据，这不包括 null 终止字符的空间。  
  
-   **列的字节长度**-存储在数据源的数据所需的字节数。  
  
-   **字符字节长度**-最大的字符窗体中显示数据所需的字节数。 这是为每个 SQL 数据类型中定义的一样[显示大小](../../../odbc/reference/appendixes/display-size.md)，只是字符字节长度是以字节为单位，而显示大小是以字符为单位。  
  
-   **数字位数**-数字用来表示一个数字的字符，包括负号、 小数点和指数 （如果需要）。  
