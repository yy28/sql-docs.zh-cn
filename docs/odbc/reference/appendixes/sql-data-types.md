...
|SQL_INTEGER|整数|精确数值精度为 10 和小数位数为 0 (有符号: – 2 [31] < = *n* < = 2 [31] – 1，无符号： 0 < = *n* < = 2 [32] – 1) [3]。|  
...
|SQL_BIGINT|bigint|精确数值，其精度为 19 （有符号） 或 20 （如果无符号） 和小数位数为 0 (有符号: – 2 [63] < = *n* < = 2 [63] – 1，无符号： 0 < = *n* < = 2 [64] – 1) [3] [9]。|  
...
|SQL_INTERVAL_DAY_TO_HOUR [7]|间隔一天 (*p*) 为 1 小时|天/之间的小时数两个日期/时间;*p*为间隔起始精度。|  
