...
> [!NOTE]  
>  间隔文本的完整 BNF 语法节所示[间隔文本语法](../../../odbc/reference/appendixes/interval-literal-syntax.md)附录 c: SQL 语法中。  
...
|文字的转义子句|指定时间间隔|  
|---------------------------|------------------------|  
|{间隔"326"YEAR(4)}|指定 326 年的时间间隔。 间隔起始精度为 4。|  
|{间隔"326"MONTH(3)}|指定 326 几个月的时间间隔。 间隔起始精度为 3。|  
|{间隔"3261"DAY(4)}|指定 3261 天的间隔。 间隔起始精度为 4。|  
...
|{间隔"163 12' 为 1 小时 DAY(3)}|指定 163 天 12 小时的间隔。 间隔起始精度为 3。|  
|{间隔"163 12:39 为分钟 DAY(3)}|指定 163 天，12 小时和 39 分钟的间隔。 间隔起始精度为 3。|  
|{到 SECOND(3) 间隔"163 12:39:59.163"DAY(3)}|指定 163 天，12 小时，39 分和 59.163 秒的间隔。 时间间隔前导精度为 3，并且秒精度为 3。|  
...
|{间隔"223.16"SECOND(2,2)}<br /><br /> {间隔"223.16"SECOND(3,1)}|在第一个示例中，前导精度为太小，并且在第二个示例中，秒的精度太小。|  
|{间隔"223.16"第二个}<br /><br /> {间隔"223"年}|前导精度未指定，因为它默认为 2，这是太小而无法保存指定的文本。|  
|{间隔"22.1234567"第二个}|秒的精度是未指定的因此它将默认为 6。 文本在小数点后具有 7 位。|  
|{为月间隔"163-13' YEAR(3)}<br /><br /> {INTERVAL"163 65 到小时 DAY(3)}<br /><br /> {为分钟的时间间隔"163 62:39"DAY(3)}<br /><br /> {到 SECOND(3) 间隔"163 12:125:59.163"DAY(3)}<br /><br /> {间隔"163:144"为分钟 HOUR(3)}<br /><br /> {间隔"163:567:234.163"到 SECOND(4) HOUR(3)}<br /><br /> {间隔"163:591.163"到 SECOND(5) MINUTE(3)}|尾随的字段不遵循公历日历以来的规则。|
