 ...
 **总结**  
 ...
 驱动程序不会将**SQLAsyncNotificationCallback**与函数名称**SQLAsyncNotificationCallback**一起调用。 相反，驱动程序管理器会将指向驱动程序的函数指针传递为相应连接句柄或语句句柄的 SQL_ATTR_ASYNC_DBC_NOTIFICATION_CALLBACK 或 SQL_ATTR_ASYNC_STMT_NOTIFICATION_CALLBACK 属性的值，分别. 可以为不同的句柄分配不同的函数指针值。 函数指针的类型定义为 SQL_ASYNC_NOTIFICATION_CALLBACK。  
