...
|开关|Description|  
|------------|-----------------|  
|/A {*action*}|指定的操作。<br /><br /> 如果仅指定一个操作/A 是可选的。|  
...
|/H|显示使用情况 （帮助）。 此开关等同于 /？。|  
|/L[*mode*] *filename*|发送到三种模式之一中的文件的程序输出： 标准 (n)、 详细 (v) 和调试 (d)。 调试模式下记录由 odbcconf.exe 加载的 Dll。<br /><br /> 如果没有一种模式指定 /L，日志文件将为空。<br /><br /> 例如， **/Lv log.txt**。|  
|/R|在重新启动后，将执行的操作。|  
|/S|静默模式。 不显示错误消息。|  
...
|CONFIGDSN *driver_name* DSN=*name* &#124; *attributes*|添加或修改系统数据源。<br /><br /> 等效于[SQLConfigDataSource 函数](../odbc/reference/syntax/sqlconfigdatasource-function.md)。<br /><br /> 例如：<br /><br /> /A {CONFIGDSN "SQL Server" "DSN=name &#124; Server=srv"}|  
|CONFIGSYSDSN *driver_name* DSN=*name* &#124; *attributes*|添加或修改系统数据源。<br /><br /> 等效于[SQLConfigDataSource 函数](../odbc/reference/syntax/sqlconfigdatasource-function.md)。<br /><br /> 例如：<br /><br /> /A {CONFIGSYSDSN "SQL Server" "DSN=name &#124; Server=srv"}|  
