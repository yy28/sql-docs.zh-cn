...
ODBC 文本驱动程序支持带分隔符和固定宽度的文本文件。 文本文件由可选的标头行和零个或多个文本行组成。  
 ...
 分隔的文本行包含一个或多个以分隔符分隔的数据值：逗号、制表符或自定义分隔符。 必须在整个文件中使用相同的分隔符。 Null 数据值由行中没有数据的行中的两个分隔符表示。 带分隔符的文本行中的字符串可以用双引号引起来， ( "" ) 。 分隔值前后不能出现空白。  
...
> [!NOTE]  
>  固定宽度文本文件中每一列的宽度是在 Schema.ini 文件中指定的。  
...
> [!NOTE]  
>  在 Schema.ini 文件中指定以自定义分隔的文本文件中的分隔符。  
