...
|MAXBUFFERSIZE|Microsoft Access 用于将数据传输到磁盘或从磁盘传输数据的内部缓冲区大小（kb）。 默认缓冲区大小为 2048 KB （显示为2048）。 可以使用任何可被256整除的整数值。 这会在 "设置" 对话框中设置与**缓冲区大小**相同的选项。|  
...
|PAGETIMEOUT|指定页（如果未使用）在被删除之前保留在缓冲区中的时间段（以毫秒为单位）。 默认值为 5-十分之一秒（0.5 秒）。 请注意，此选项适用于使用 ODBC 驱动程序的所有数据源。<br /><br /> 这会设置与 "设置" 对话框中**页面超时**相同的选项。|  
|PWD|密码。|  
