...
|**RelationshipType**|必须<br /><br /> 正在设置的关系的类型。|可能的值为：<br /><br /> **1**：父级<br /><br /> **2**：同级（同一级）|  
|**ImportStatus_ID**|必须<br /><br /> 导入过程的状态。|可能的值为：<br /><br /> **0**- 指定此值表示记录已经准备好临时存储。<br /><br /> **1**- 自动分配的值，表示记录的暂存过程已成功。<br /><br /> **2**- 自动分配的值，表示记录的临时过程已失败。|  
