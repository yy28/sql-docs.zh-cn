...
1. [安装和设置 Windows Server 故障转移群集（WSFC）](#windows-server-failover-cluster-wsfc)。
...
**同步提交**：这可确保在数据库的主副本上提交事务时，还会在所有其他同步副本上提交事务。 异步提交则不能保证这一点，并且会落后于主要副本。

通常只有当两个节点位于同一数据中心时，才能启用同步提交。 如果它们处于不同的数据中心，同步提交会降低数据库的性能。 如果未选中此复选框，则使用异步提交。

**自动故障转移：** 主副本关闭后，当选择 "自动故障转移" 时，AG 会自动故障转移到其辅助副本。 只有在使用同步提交的副本上才能够启用。

**可读辅助副本：** 默认情况下，用户无法连接到任何辅助副本。 用户只能连接到使用只读访问的次要副本。
...
8. 在“指定副本”页上，单击“侦听器”选项卡，执行以下操作********。 请参阅图 18。
   ...
   d.单击“下一步”。 在“网络模式”文本框中输入 DHCP，然后单击“下一步”继续********。
...
3. 在“数据库”页中，键入在**创建可用性组**部分所创建的数据库名称，然后单击“下一步”[](#create-an-availability-group)****。 请参阅图 22。
