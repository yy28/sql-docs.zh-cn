---
title: "创建网站对话框 （Master Data Services 配置管理器） |Microsoft 文档"
ms.custom: 
ms.date: 03/20/2017
...
|**主机标头**|可选值。 键入主机标头名称。 在您想要将一个主机名（也称作域名）分配给使用单个 IP 地址或端口的计算机时，使用此主机标头。 在您指定主机名后，客户端必须使用该名称而非 IP 地址来访问网站。 在您配置主机名后，在您的 DNS 服务器具有针对该主机名的条目之前，不能在 Web 浏览器中打开该网站。<br /><br /> 例如，如果你希望用户访问网站`http://www.contoso.com/`，你必须指定主机名称 www.contoso.com 和 DNS 服务器必须具有它的条目。<br /><br /> 如果你的网站在 intranet 上可用，你不必指定主机名，如果用户在浏览器中，键入服务器名称，如`http://server_name`。 但是，如果您的环境中的 DNS 服务器被配置为存储此 Web 服务器的其他名称，则您可以为每个主机名创建单独的绑定，以便用户可以使用 DNS 服务器存储的其他名称。 如果您必须为您的网站配置多个主机名，则应使用 IIS 管理器之类的 IIS 工具添加附加的网站绑定。|  
