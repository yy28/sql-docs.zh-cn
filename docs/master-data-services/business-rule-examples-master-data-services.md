...
有关如何部署示例模型的说明，请参阅 [Master Data Services](../sql-server/media/master-data-services.png#deploySample)。  
  
  
## 业务规则示例  
示例模型 |实体  |业务规则名称| Description  
---------|---------|---------|-----------|  
...
在下面的业务规则中，如果 Color 和 Class 属性不满足 `is equal` 规则条件，则 `defaults to` [[规则操作](../master-data-services/business-rule-actions-master-data-services.md)](Business%20Rule%20Conditions%20(Master%20Data%20Services).xml)会应用于 Name 属性。  
```  
If  
...
有关设置网站的说明，请参阅 [Master Data Services](../sql-server/media/master-data-services.png)。  
2. 单击包含业务规则的示例模型（如以上表所列），然后单击“实体”。  
3. 单击应用规则的实体（如以下表所列），然后单击“业务规则”。  
4. 单击要查看的业务规则的名称。 UI 将展开，以显示 **If**、**Then** 和 **Else** 语句。  
  
## 本文是否对你有帮助？ 我们洗耳恭听   
你正在查找哪些信息，是否已经找到？ 我们不断听取你的反馈来改进内容。 请将你的评论提交到 [sqlfeedback@microsoft.com](mailto:sqlfeedback@microsoft.com)。   
