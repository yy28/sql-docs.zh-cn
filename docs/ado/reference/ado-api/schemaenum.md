...
|**adSchemaDBInfoKeywords**|30|返回提供程序特定的关键字的列表。<br /><br /> （IDBInfo：： GetKeywords）|\<无>|  
|**adSchemaDBInfoLiterals**|31|返回在文本命令使用的提供程序特定的文本的列表。<br /><br /> （IDBInfo：： GetLiteralInfo）|\<无>|  
...
|**adSchemaSQLLanguages**|18|返回在目录中定义的 SQL 实现处理数据所支持的一致性级别、选项和语言。<br /><br /> （SQL_LANGUAGES 行集）|\<无>|  
