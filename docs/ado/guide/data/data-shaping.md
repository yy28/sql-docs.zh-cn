---
title: "数据成型 |Microsoft 文档"
ms.prod: sql-non-specified
ms.technology:
- drivers
ms.custom: 
ms.date: 01/19/2017
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- data shaping [ADO]
ms.assetid: 62bd7dc9-45b5-4ca9-8b52-457325e0ce9e
caps.latest.revision: 9
author: MightyPen
ms.author: genemi
manager: jhubbard
ms.workload: Inactive
ms.translationtype: MT
ms.sourcegitcommit: f7e6274d77a9cdd4de6cbcaef559ca99f77b3608
ms.openlocfilehash: 1ad3b257166308fe2c5a4bd94da5427dc0ca2649
ms.contentlocale: zh-cn
ms.lasthandoff: 09/09/2017

---
# <a name="data-shaping"></a>调整数据
本部分包含以下主题。  
  
-   [数据调整概述](../../../ado/guide/data/data-shaping-overview.md)  
  
-   [数据 ShapingExample](../../../ado/guide/data/data-shaping-example.md)  
  
-   [重新调整](../../../ado/guide/data/reshaping.md)  
  
-   [孙聚合](../../../ado/guide/data/grandchild-aggregates.md)  
  
-   [与中间的参数化的命令计算命令](../../../ado/guide/data/parameterized-commands-with-intervening-compute-commands.md)  
  
-   [持久化分层记录集](../../../ado/guide/data/persisting-hierarchical-recordsets.md)  
  
-   [所需的提供程序，供你调整数据](../../../ado/guide/data/required-providers-for-data-shaping.md)  
  
-   [在常规的形状命令](../../../ado/guide/data/shape-commands-in-general.md)  
  
-   [形状 APPEND 子句](../../../ado/guide/data/shape-append-clause.md)  
  
-   [形状 COMPUTE 子句](../../../ado/guide/data/shape-compute-clause.md)  
  
-   [构造分层记录集](../../../ado/guide/data/fabricating-hierarchical-recordsets.md)  
  
-   [访问在分层记录集中的行](../../../ado/guide/data/accessing-rows-in-a-hierarchical-recordset.md)  
  
-   [正式形状语法](../../../ado/guide/data/formal-shape-grammar.md)  
  
-   [Visual Basic 应用程序函数](../../../ado/guide/data/visual-basic-for-applications-functions.md)

