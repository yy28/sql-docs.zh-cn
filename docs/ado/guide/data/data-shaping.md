---
title: 数据定形 |Microsoft Docs
ms.prod: sql
ms.prod_service: connectivity
ms.technology: connectivity
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
ms.topic: conceptual
helpviewer_keywords:
- data shaping [ADO]
ms.assetid: 62bd7dc9-45b5-4ca9-8b52-457325e0ce9e
author: MightyPen
ms.author: genemi
ms.openlocfilehash: 1811d46bb99b7cdb38bf6508d709826167f787f7
ms.sourcegitcommit: e042272a38fb646df05152c676e5cbeae3f9cd13
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 04/27/2020
ms.locfileid: "67925603"
---
# <a name="data-shaping"></a>数据整理
本部分包含以下主题。  
  
-   [数据整理概述](../../../ado/guide/data/data-shaping-overview.md)  
  
-   [数据 ShapingExample](../../../ado/guide/data/data-shaping-example.md)  
  
-   [重新整理](../../../ado/guide/data/reshaping.md)  
  
-   [孙级聚合](../../../ado/guide/data/grandchild-aggregates.md)  
  
-   [参数化命令与中间 COMPUTE 命令](../../../ado/guide/data/parameterized-commands-with-intervening-compute-commands.md)  
  
-   [保留分层记录集](../../../ado/guide/data/persisting-hierarchical-recordsets.md)  
  
-   [数据整理所需的提供程序](../../../ado/guide/data/required-providers-for-data-shaping.md)  
  
-   [常用 Shape 命令](../../../ado/guide/data/shape-commands-in-general.md)  
  
-   [Shape APPEND 子句](../../../ado/guide/data/shape-append-clause.md)  
  
-   [Shape COMPUTE 子句](../../../ado/guide/data/shape-compute-clause.md)  
  
-   [构造分层记录集](../../../ado/guide/data/fabricating-hierarchical-recordsets.md)  
  
-   [访问分层记录集中的行](../../../ado/guide/data/accessing-rows-in-a-hierarchical-recordset.md)  
  
-   [正式 Shape 语法](../../../ado/guide/data/formal-shape-grammar.md)  
  
-   [Visual Basic for Applications 函数](../../../ado/guide/data/visual-basic-for-applications-functions.md)
