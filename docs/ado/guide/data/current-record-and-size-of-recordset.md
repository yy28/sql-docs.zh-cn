...
## <a name="current-record"></a>当前记录  
 数据集中的当前记录对应于**Recordset**对象的游标位置所指向的。 当从数据源返回**recordset**对象作为调用 recordset 的结果时 **。打开**、**执行命令**或**连接**（包括**NamedCommand**和**StoredProcedure**），光标设置为指向第一条记录。 在示例数据集中，初始当前记录是 "叔叔 Bob 的随机是 Pears" 项。  
