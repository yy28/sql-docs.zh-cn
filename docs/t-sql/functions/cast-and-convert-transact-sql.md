...
> [!TIP]  
>  可从 [Microsoft 下载中心](http://www.microsoft.com/download/details.aspx?id=35834)将此图表下载为 PDF 文件。  
...
从 GETDATE() 值开始，此示例显示当前日期和时间，使用 `CAST` 将当前日期和时间更改为字符数据类型，然后使用 `CONVERT` 以 `ISO 8901` 格式显示日期和时间。
