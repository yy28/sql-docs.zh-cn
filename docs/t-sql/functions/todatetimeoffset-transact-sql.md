...
### <a name="d-changing-the-time-zone-offset-of-the-current-date-and-time"></a>D. 更改当前日期和时间的时区偏移量  
 下面的示例将当前日期和时间的时区偏移量更改为时区 `-07:00`。  
...
### <a name="e-changing-the-time-zone-offset-in-minutes"></a>E. 更改时区偏移量（以分钟为单位）  
 下面的示例将当前时区更改为 `-120` 分钟。  
...
### <a name="f-adding-a-13-hour-time-zone-offset"></a>F. 添加 13 小时时区偏移量  
 下面的示例将 13 小时时区偏移量添加到日期和时间。  
