---
title: "创建过程 (Transact SQL) |Microsoft 文档"
ms.custom: 
ms.date: 09/06/2017
 ...
 *schema_name*  
 该过程所属的架构的名称。 过程是绑定到架构的。 如果在创建过程时未指定架构名称，则自动分配正在创建过程的用户的默认架构。  
  
 *过程名称*  
 过程的名称。 过程名称必须符合的规则[标识符](../../relational-databases/databases/database-identifiers.md)和在架构中必须唯一。  
  
 避免使用**sp_**前缀命名过程时。 此前缀由 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 用来指定系统过程。 如果存在同名的系统过程，则使用前缀可能导致应用程序代码中断。  
  
 可以通过使用一个数字符号 （#） 之前创建本地或全局临时过程*procedure_name* (*#procedure_name*) 本地临时过程，以及用于全局临时的两个数字符号过程 (*# # procedure_name*)。 局部临时程序仅对创建了它的连接可见，并且在关闭该连接后将被删除。 全局临时程序可用于所有连接，并且在使用该过程的最后一个会话结束时将被删除。 对于 CLR 过程，不能指定临时名称。  
...
>  [!INCLUDE[ssNoteDepFutureAvoid](../../includes/ssnotedepfutureavoid-md.md)]  
 ...
 编号的过程不能使用**xml**或 CLR 用户定义类型和不能在计划指南。  
  
 **@***参数*  
 在过程中声明的参数。 使用指定参数名称 at 符号 (**@**) 作为第一个字符。 参数名称必须符合的规则[标识符](../../relational-databases/databases/database-identifiers.md)。 每个过程的参数仅用于该过程本身；其他过程中可以使用相同的参数名称。  
  
 可声明一个或多个参数；最大值是 2,100。 除非定义了参数的默认值或者将参数设置为等于另一个参数，否则用户必须在调用过程时为每个声明的参数提供值。 如果一个过程包含[表值参数](../../relational-databases/tables/use-table-valued-parameters-database-engine.md)，和参数是调用中缺少，传入一个空表。 参数只能代替常量表达式，而不能用于代替表名、列名或其他数据库对象的名称。 有关详细信息，请参阅 [EXECUTE (Transact-SQL)](../../t-sql/language-elements/execute-transact-sql.md)。  
...
**准则[!INCLUDE[tsql](../../includes/tsql-md.md)]过程**:  
  
-   所有[!INCLUDE[tsql](../../includes/tsql-md.md)]数据类型可以用作参数。  
  
-   您可以使用用户定义的表类型创建表值参数。 表值参数只能是 INPUT 参数，并且这些参数必须带有 READONLY 关键字。 有关详细信息，请参阅[使用表值参数 &#40; 数据库引擎 &#41;](../../relational-databases/tables/use-table-valued-parameters-database-engine.md)  
  
-   **光标**数据类型只能是输出参数和必须附带由 VARYING 关键字。  
  
**有关 CLR 过程的指导原则**:  
  
-   在托管代码中具有等效值的所有本机 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 数据类型都可以用作参数。 有关 CLR 类型之间的对应关系的详细信息和[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]系统数据类型，请参阅[映射 CLR 参数数据](../../relational-databases/clr-integration-database-objects-types-net-framework/mapping-clr-parameter-data.md)。 有关详细信息[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]系统数据类型和其语法，请参阅[数据类型 &#40;Transact SQL &#41;](../../t-sql/data-types/data-types-transact-sql.md).  
...
VARYING  
 指定作为输出参数支持的结果集。 该参数由过程动态构造，其内容可能发生改变。 仅适用于**光标**参数。 该选项对于 CLR 过程无效。  
  
*默认值*  
 参数的默认值。 如果为参数定义了默认值，而无需指定该参数的值执行过程。 默认值必须是常量或者为 NULL。 该常量值可以采用通配符的形式，这使其可以在将该参数传递到过程时使用 LIKE 关键字。   
  
 默认值记录在**sys.parameters.default**列仅对 CLR 过程。 该列为 NULL[!INCLUDE[tsql](../../includes/tsql-md.md)]过程参数。  
  
OUT | OUTPUT  
 指示参数是输出参数。 使用 OUTPUT 参数将值返回给过程的调用方。 **文本**， **ntext**，和**映像**参数不能用作输出参数，除非过程是 CLR 过程。 OUTPUT 参数可以为游标占位符，CLR 过程除外。 不能将表值数据类型指定为过程的 OUTPUT 参数。  
  
READONLY  
 指示无法更新或在过程的正文内修改参数。 如果参数类型为表值类型，则必须指定 READONLY。  
  
RECOMPILE  
 指示[!INCLUDE[ssDE](../../includes/ssde-md.md)]不会缓存此过程中，并强制对它进行编译每次执行时它的查询计划。 有关强制执行重新编译的原因的详细信息，请参阅[重新编译存储过程](../../relational-databases/stored-procedures/recompile-a-stored-procedure.md)。 在指定了 FOR REPLICATION 或者用于 CLR 过程时不能使用此选项。  
  
 若要指示[!INCLUDE[ssDE](../../includes/ssde-md.md)]要丢弃过程内的单个查询的查询计划，请在查询的定义中使用 RECOMPILE 查询提示。 有关详细信息，请参阅[查询提示 (Transact-SQL)](../../t-sql/queries/hints-transact-sql-query.md)。  
  
ENCRYPTION  
 **适用于**: SQL Server ([!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)]通过[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)])， [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]。  
  
 指示[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]将 CREATE PROCEDURE 语句的原始文本转换为经过模糊处理的格式。 模糊处理的输出直接在中不可见的目录视图中的任何[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。 对系统表或数据库文件没有访问权限的用户不能检索模糊文本。 但是，文本可供特权用户也可以通过访问系统表[DAC 端口](../../database-engine/configure-windows/diagnostic-connection-for-database-administrators.md)或直接访问数据库文件。 此外，能够向服务器进程附加调试器的用户可在运行时从内存中检索已解密的过程。 有关访问系统元数据的详细信息，请参阅[元数据可见性配置](../../relational-databases/security/metadata-visibility-configuration.md)。  
...
EXECUTE AS*子句*  
 ...
 对于本机编译存储过程，启动[!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]并在[!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]，没有任何限制在 EXECUTE AS 子句。 在[!INCLUDE[ssSQL14](../../includes/sssql14-md.md)]自助，所有者，和*user_name*子句支持本机编译的存储过程。  
 ...
 **适用于**: SQL Server ([!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)]通过[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)])， [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]。  
 ...
 A`FOR REPLICATION`过程有一个对象类型**RF**中**sys.objects**和**sys.procedures**。  
 ...
 **适用于**:[!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)]通过[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]， [!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)]。  
 ...
 指定 [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] 程序集的方法，以便 CLR 过程引用。 *class_name*必须为有效[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]标识符，并且为程序集中的类必须存在。 如果类具有使用句点的命名空间限定名称 (**。**) 来分隔命名空间部分，类名必须由使用方括号分隔 (**[]**) 或引号 (**""**). 指定的方法必须为该类的静态方法。  
...
BEGIN、ROLLBACK 和 COMMIT 操作无法在原子块内使用。  
 ...
 每个本机编译存储过程的外层作用域都有一个 ATOMIC 块。 这些块不能嵌套。 有关原子块的详细信息，请参阅[Natively Compiled Stored Procedures](../../relational-databases/in-memory-oltp/natively-compiled-stored-procedures.md)。  
  
**NULL** |不为 NULL  
 确定参数中是否允许 Null 值。 默认值为 NULL。  
  
NATIVE_COMPILATION  
 **适用于**:[!INCLUDE[ssSQL14](../../includes/sssql14-md.md)]通过[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]和[!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]。  
  
 指示过程已本机编译。 NATIVE_COMPILATION、SCHEMABINDING 和 EXECUTE AS 可以按任意顺序指定。 有关详细信息，请参阅[Natively Compiled Stored Procedures](../../relational-databases/in-memory-oltp/natively-compiled-stored-procedures.md)。  
  
SCHEMABINDING  
 **适用于**:[!INCLUDE[ssSQL14](../../includes/sssql14-md.md)]通过[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]和[!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]。  
  
 确保过程引用的表不能删除或修改。 SCHEMABINDING 在本机编译存储过程中是必需的。 (有关详细信息，请参阅[Natively Compiled Stored Procedures](../../relational-databases/in-memory-oltp/natively-compiled-stored-procedures.md)。)SCHEMABINDING 限制与其对用户定义的函数的限制是相同的。 有关详细信息，请参阅中的 SCHEMABINDING 部分[CREATE FUNCTION &#40;Transact SQL &#41;](../../t-sql/statements/create-function-transact-sql.md).  
  
语言 = [N] 语言  
 **适用于**:[!INCLUDE[ssSQL14](../../includes/sssql14-md.md)]通过[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]和[!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]。  
  
 等效于[设置语言 &#40;Transact SQL &#41;](../../t-sql/statements/set-language-transact-sql.md)会话选项。 LANGUAGE = [N] 'language' 是必须的。  
...
REPEATABLE READ  
 指定语句不能读取已修改但是尚未由其他事务提交的数据。 如果另一个事务修改由当前事务已读取的数据，当前事务将失败。  
  
SERIALIZABLE  
 请指定下列内容：  
-   语句不能读取已由其他事务修改但尚未提交的数据。  
-   如果另一个事务修改由当前事务已读取的数据，当前事务将失败。  
-   如果另一个事务在当前事务中任何语句读取的密钥的范围中插入新行的键值的将落入，当前事务将失败。  
  
SNAPSHOT  
 指定在事务中任何语句读取的数据是在事务开始时存在的数据的事务处理方式一致版本。  
...
```   
调用具有语句的存储过程：`EXEC What_DB_is_this;`   

略显复杂，旨在提供一个输入的参数，以便使过程更为灵活。 例如：  
```sql   
CREATE PROC What_DB_is_that @ID int   
AS    
SELECT DB_NAME(@ID) AS ThatDB;   
```   
在调用过程时，请提供数据库 id 号。 例如，`EXEC What_DB_is_that 2;`返回`tempdb`。   

请参阅[示例](#Examples)许多的更多示例本主题末尾。     
...
-   当在过程中创建或引用数据库对象时使用架构名称。 需要更少的处理时间[!INCLUDE[ssDE](../../includes/ssde-md.md)]来解析对象名称，如果它不必搜索多个架构。 它还可以防止权限和访问问题引起的而无需指定架构创建对象时分配的用户的默认架构。  
...
-   避免使用`SELECT *`。 而是应指定所需的列名。 这样做可以避免停止过程执行的[!INCLUDE[ssDE](../../includes/ssde-md.md)]错误。 例如，`SELECT *`首次成功的语句将数据从一个 12 列的表返回，然后将该数据插入到 12 的列的临时表或任一表中的列的顺序更改。  
  
-   避免处理或返回过多的数据。 尽可能在过程代码中缩小结果的范围，这样，该过程执行的任何后续操作都将使用可能的最小数据集完成。 仅将基本数据发送到客户端应用程序。 它比跨网络发送多余的数据并且强制客户端应用程序处理不必要的大结果集更高效。  
  
-   通过使用 BEGIN/提交事务使用显式事务，使事务尽可能短。 更长的事务意味着更长的记录锁定和更高的死锁风险。  
...
-   在过程主体中对 CREATE TABLE 或 ALTER TABLE [!INCLUDE[tsql](../../includes/tsql-md.md)] 语句引用的所有表列使用 DEFAULT 关键字。 这样可以防止将 NULL 传递给不允许空值的列。  
...
-   使用将转换 Null 的修改语句并且包括从查询中删除含 Null 值的行的逻辑。 请注意，在[!INCLUDE[tsql](../../includes/tsql-md.md)]、 NULL 不是一个空或者是"nothing"的值。 它是针对未知值的占位符并且可能导致意外的行为，特别是在查询结果集或使用 AGGREGATE 函数时。  
...
## <a name="general-remarks"></a>一般备注  
 ...
 在过程中指定的变量可以是用户定义或系统变量，如 @@SPID。  
  
 第一次执行某个过程时，将编译该过程以确定检索数据的最优访问计划。 如果已经生成的计划仍保留在[!INCLUDE[ssDE](../../includes/ssde-md.md)]计划缓存中，则该过程随后执行的操作可能重新使用该计划。  
  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 启动时可以自动执行一个或多个过程。 必须由系统管理员在创建过程**master**数据库中，并且下执行**sysadmin**作为后台进程的固定的服务器角色。 这些过程不能有任何输入或输出参数。 有关详细信息，请参阅[执行存储过程](../../relational-databases/stored-procedures/execute-a-stored-procedure.md)。  
 ...
 试图超过最高级的嵌套将导致整个调用链失败。 你可以使用 @@NESTLEVEL函数以返回当前存储的过程执行的嵌套级别。  
...
> [!NOTE]  
>  在过程和用户定义函数中传递参数，或者在批处理语句中声明和设置变量时，不执行 SET ANSI_WARNINGS。 例如，如果变量指**char**(3)，然后设置为值大于三个字符，数据将剪裁为定义的大小和插入或更新语句会成功。  
...
|CREATE RULE|SET SHOWPLAN_ALL||  
 ...
 过程可以引用尚不存在的表。 在创建时，只进行语法检查。 直到第一次执行该过程时才对其进行编译。 只有在编译过程中才解析过程中引用的所有对象。 因此，成功，则创建引用不存在的表的语法正确过程但是，如果不存在引用的表，在执行时将失败过程。  
...
-   使用的参数类型与 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 过程的相应参数的数据类型兼容。 有关匹配信息[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]数据类型到[!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)]数据类型，请参阅[映射 CLR 参数数据](../../relational-databases/clr-integration-database-objects-types-net-framework/mapping-clr-parameter-data.md)。  
...
|视图|Description|  
|----------|-----------------|  
|[sys.sql_modules](../../relational-databases/system-catalog-views/sys-sql-modules-transact-sql.md)|返回 [!INCLUDE[tsql](../../includes/tsql-md.md)] 过程的定义。 具有 ENCRYPTION 选项创建过程的文本不能通过查看**sys.sql_modules**目录视图。|  
...
### <a name="permissions"></a>Permissions  
 需要**CREATE PROCEDURE**数据库中的权限和**ALTER**过程正在创建，或要求的成员身份的架构权限**db_ddladmin**固定的数据库角色。  
  
 有关 CLR 存储过程，需要在 EXTERNAL NAME 子句中引用的程序集的所有权或**引用**上该程序集的权限。  
  
##  <a name="mot"></a>创建过程和内存优化表  
 可以通过传统和本机编译存储过程访问内存优化表。 本机过程是在大多数情况下的更高效的方式。
有关详细信息，请参阅[Natively Compiled Stored Procedures](../../relational-databases/in-memory-oltp/natively-compiled-stored-procedures.md)。  
  
 下面的示例演示如何创建访问内存优化表的本机编译存储的过程`dbo.Departments`:  
...
```  
 ...
 有关本机编译存储过程中的可编程性的讨论，支持的查询外围应用，并查看运算符[的本机编译的 T-SQL 模块支持的功能](../../relational-databases/in-memory-oltp/supported-features-for-natively-compiled-t-sql-modules.md)。  
...
|[传递参数](#Parameters)|@parameter <br> &nbsp;&nbsp;• = 默认值 <br> &nbsp;&nbsp;• 输出 <br> &nbsp;&nbsp;• 表值参数类型 <br> &nbsp;&nbsp;• 游标不同|  
|[通过使用存储的过程修改数据](#Modify)|UPDATE|  
|[错误处理](#Error)|TRY…CATCH|  
|[对进行模糊处理的过程定义](#Encrypt)|WITH ENCRYPTION|  
|[强制过程重新编译](#Recompile)|WITH RECOMPILE|  
|[设置的安全上下文](#Security)|EXECUTE AS|  
...
#### <a name="c-creating-a-clr-stored-procedure"></a>C. 创建 CLR 存储过程  
 下面的示例创建`GetPhotoFromDB`引用的过程`GetPhotoFromDB`方法`LargeObjectBinary`类`HandlingLOBUsingCLR`程序集。 创建该过程之前，`HandlingLOBUsingCLR`本地数据库中注册程序集。  
  
**适用于**:[!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)]通过[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]， [!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)] (如果使用程序集从创建*assembly_bits。*  
...
#### <a name="e-using-a-procedure-with-wildcard-parameters"></a>E. 使用带有通配符参数的过程  
 以下示例将创建一个存储过程，此过程通过传递雇员的名和姓的完整值或部分值来返回雇员的信息。 此过程模式匹配传递的参数，或如果未提供，将使用预设的默认值 (以字母开头的姓氏`D`)。  
...
```  
 ...
 `uspGetEmployees2`可以在许多组合中执行过程。 此处只列出了几个可能的组合。  
...
#### <a name="f-using-output-parameters"></a>F. 使用输出参数  
...
```  
 ...
 执行 `uspGetList`，返回价格低于 `$700` 的 [!INCLUDE[ssSampleDBCoShort](../../includes/sssampledbcoshort-md.md)] 产品（自行车）的列表。 `OUTPUT`参数`@Cost`和`@ComparePrices`控制流语言中用于返回中的消息**消息**窗口。  
  
> [!NOTE]  
>  OUTPUT 变量必须在创建过程时或使用变量时定义。 参数名称和变量名称不需要匹配;但是，数据类型和参数定位必须匹配，除非`@ListPrice`  = *变量*使用。  
...
**适用于**:[!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)]通过[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]，SQL 数据库。  
...
```  
 ...
 `WITH ENCRYPTION`选项对进行模糊处理过程的定义时查询系统目录或使用元数据函数，如下面的示例所示。  
...
#### <a name="l-using-the-with-recompile-option"></a>L. 使用 WITH RECOMPILE 选项  
 `WITH RECOMPILE`时提供给过程的参数不是正常现象，以及在新的执行计划不应缓存或存储在内存中时，子句也很有帮助。  
...
###  <a name="Security"></a>设置的安全上下文  
 此节中的示例使用 EXECUTE AS 子句设置执行存储过程的安全上下文。  
  
#### <a name="m-using-the-execute-as-clause"></a>M. 使用 EXECUTE AS 子句  
 下面的示例演示使用[EXECUTE AS](../../t-sql/statements/execute-as-clause-transact-sql.md)子句，以指定可以在其中执行的过程的安全上下文。 在示例中，选项`CALLER`指定，可以调用它的用户上下文中执行该过程。  
...
### <a name="o-create-a-stored-procedure-that-runs-a-select-statement"></a>O. 创建运行 SELECT 语句的存储过程  
 此示例演示用于创建和运行过程的基本语法。 当运行时一批，CREATE PROCEDURE 必须是第一个语句。 例如，若要创建以下存储中的过程[!INCLUDE[ssawPDW](../../includes/ssawpdw-md.md)]，首先，设置数据库上下文，然后运行 CREATE PROCEDURE 语句。  
...
## <a name="see-also"></a>另请参阅  
 ...
 [sys.numbered_procedure &#40;Transact SQL &#41;](../../relational-databases/system-catalog-views/sys-numbered-procedures-transact-sql.md)   
