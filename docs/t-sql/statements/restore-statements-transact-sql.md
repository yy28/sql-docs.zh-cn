...
-   联机还原  
    ...
    > **注意：**仅在 Enterprise edition 中允许联机还原[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。  
...
> **注意：**对于使用完整或大容量日志恢复模式数据库，在大多数情况下必须备份日志尾部在还原数据库之前。 还原数据库而不首先备份日志的末尾将导致错误，除非 RESTORE DATABASE 语句包含 WITH REPLACE 或 WITH STOPAT 子句，此子句必须指定数据备份的结束时间或在数据备份结束之后发生的事务。 有关结尾日志备份的详细信息，请参阅[结尾日志备份 (SQL Server) ](../../relational-databases/backup-restore/tail-log-backups-sql-server.md)。  
...
-   RECOVERY（默认值）表示，应在完成当前备份前滚之后执行回滚。  
     ...
     数据库恢复，需要将整个设置正在还原的数据 (*回滚前滚集*) 与数据库一致。 如果前滚集尚未前滚到与数据库保持一致的地步，并且指定了 RECOVERY，则[!INCLUDE[ssDE](../../includes/ssde-md.md)]将发出错误。  
  
## <a name="compatibility-support"></a>兼容性支持  
 备份的**master**，**模型**和**msdb**由使用的早期版本创建[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]无法还原[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。  
  
> **注意：**否[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]备份还原到早期版本的[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]比在其创建备份的版本。  
  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的每个版本使用的默认路径与早期版本不同。 因此，若要还原在早期版本备份的默认位置创建的数据库，必须使用 MOVE 选项。 有关新的默认路径的信息，请参阅[文件位置的默认实例和 Named Instances of SQL Server](../../sql-server/install/file-locations-for-default-and-named-instances-of-sql-server.md)。  
...
## <a name="general-remarks"></a>一般备注  
 ...
 还原损坏**master**数据库使用的特殊过程来执行。 有关详细信息，请参阅[备份和还原系统数据库 (SQL Server)](../../relational-databases/backup-restore/back-up-and-restore-of-system-databases-sql-server.md)。  
...
## <a name="reverting-a-database-to-a-database-snapshot"></a>恢复到数据库快照数据库  
 A*还原数据库操作*（指定使用 DATABASE_SNAPSHOT 选项） 采用完整的源数据库返回时，它将它恢复到数据库快照的时间，从点的数据覆盖源数据库在指定的数据库快照中维护的时间。 当前只能存在可以恢复到的快照。 然后，恢复操作重新生成日志（因此，以后无法将已恢复的数据库前滚到用户错误点）。  
...
-   B. [还原完整和差异数据库备份](#restoring_full_n_differential_db_backups)  
  
-   C. [使用重新启动语法还原数据库](#restoring_db_using_RESTART)  
  
-   D. [还原数据库并移动文件](#restoring_db_n_move_files)  
  
-   E. [使用备份和还原复制数据库](#copying_db_using_bnr)  
  
-   F. [使用 STOPAT 还原到时间点](#restoring_to_pit_using_STOPAT)  
  
-   G. [还原到一个标记的事务日志](#restoring_transaction_log_to_mark)  
  
-   H. [还原使用磁带语法](#restoring_using_TAPE)  
  
-   I. [还原使用文件和文件组的语法](#restoring_using_FILE_n_FG)  
...
> **注意：**恢复执行单独的步骤，以减少过早恢复你的可能性之前的所有日志, 已都还原备份。  
