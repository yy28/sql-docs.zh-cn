---
title: "集 REMOTE_PROC_TRANSACTIONS (Transact SQL) |Microsoft 文档"
ms.custom: 
ms.date: 07/26/2017
...
> [!IMPORTANT]  
>  [!INCLUDE[ssNoteDepNextDontUse](../../includes/ssnotedepnextdontuse-md.md)] 提供此选项的目的在于为使用远程存储过程的应用程序提供向后兼容性。 不发出远程存储过程调用，而是使用引用链接服务器的分布式查询。 通过使用这些定义[sp_addlinkedserver](../../relational-databases/system-stored-procedures/sp-addlinkedserver-transact-sql.md)。  
...
## <a name="remarks"></a>注释  
 ...
 REMOTE_PROC_TRANSACTIONS 是连接级别设置，可以使用重写实例级别**sp_configure 远程 proc trans**选项。  
 ...
 REMOTE_PROC_TRANSACTIONS 选项是一种兼容性选项，将影响对的实例所做的仅远程存储的过程调用[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]定义为远程服务器使用**sp_addserver**。 选项不适用于定义为链接的服务器使用的实例执行存储的过程的分布式查询**sp_addlinkedserver**。  
