...
在 CTP 3.0 中，不支持“PARAMETERS”和“ENVIRONMENT_VARIABLES”   。
...
SQL Server 2019 中引入了 EXECUTE EXTERNAL SCRIPT 权限，以便可以授予对特定语言的外部脚本执行权限。 以前只有 EXECUTE ANY EXTERNAL SCRIPT 数据库权限，无法授予对特定语言的执行权限。
