 ...
 *file_format_name*  
 ...
 RCFILE (结合 SERDE_METHOD = *SERDE_method*)  
 指定记录纵栏表文件格式 (RcFile)。 此选项需要你指定的 Hive 序列化程序和反序列化程序 (SerDe) 方法。 如果在 Hadoop 中使用 Hive/HiveQL 查询 RC 文件，此要求是相同的。 请注意，SerDe 方法是区分大小写。  
...
-   FORMAT_TYPE = RCFILE，SERDE_METHOD = org.apache.hadoop.hive.serde2.columnar.ColumnarSerDe  
 ...
 DELIMITEDTEXT  
 指定以文本格式使用列分隔符，也称为字段终止符。  
  
 FIELD_TERMINATOR = *field_terminator*  
 仅适用于带分隔符的文本文件。 这将文本分隔文件中指定一个或多个标记每个字段 （列） 的末尾的字符。 默认值为管道字符 ꞌ | ꞌ。 要获得有保证的支持，我们建议使用一个或多个 ascii 字符。
...
-   年、 月和日可以具有多种格式和订单。 下表仅显示 ymd。 月可以有 1 个或 2 的数字，或者 3 个字符。 天可以有 1 或 2 个数字。 年份可具有 2 个或 4 位数。  
...
|日期类型|示例|Description|  
|---------------|-------------|-----------------|  
|DateTime|DATE_FORMAT = yyyy-月-日 HH:mm:ss.fff|除了年、 月和日，这包括 00-24 小时，00-59 分钟，00-59 秒和毫秒的 3 个数字。|  
|DateTime|DATE_FORMAT = yyyy-月-日 hh:mm:ss.ffftt|除了年、 月和日，这包括 00-12 小时，00-59 分钟，00-59 秒、 毫秒，和 AM，3 个数字 am，PM 或 pm。|  
|SmallDateTime|DATE_FORMAT = yyyy-月-日 hh: mm|除了年、 月和日，这包括 00-23 小时数，00-59 分钟。|  
|SmallDateTime|DATE_FORMAT = yyyy-月-日 hh:mmtt|这包括 00-11 除了年、 月和日，小时数，00-59 分钟，没有秒，和 AM、 am、 PM，或 pm。|  
...
|datetime2|DATE_FORMAT = yyyy-月-日 HH:mm:ss.fffffff|除了年、 月和日，这包括 00-23 小时数，00-59 分钟，00-59 秒和毫秒的 7 位。|  
|datetime2|DATE_FORMAT = yyyy-月-日 hh:mm:ss.ffffffftt|除了年、 月和日，这包括 00-11 小时数，00-59 分钟，00-59 秒、 毫秒，和 AM 7 位 am，PM 或 pm。|  
|DateTimeOffset|DATE_FORMAT = yyyy-月-日 HH:mm:ss.fffffff zzz|除了年、 月和日，这包括 00-23 小时数，00-59 分钟，00-59 秒和毫秒为单位，和的时区偏移量作为输入文件中放置的 7 位 {+ &#124;-} HH:ss。 例如，而无需夏时制的洛杉矶以来节省了 8 小时后面 UTC，值为-08:00 输入文件中的指定洛杉矶的时区。|  
|DateTimeOffset|DATE_FORMAT = yyyy-月-日 hh:mm:ss.ffffffftt zzz|除了年、 月和日，这包括 00-11 小时数，00-59 分钟，00-59 秒、 毫秒、 （上午、 am、 PM，或下午），7 位和时区偏移量。 请参阅前一行中的说明。|  
...
-   字母 zzz 指定系统的格式的当前时区的时区偏移量 {+ |-} HH:ss]。  
 ...
 USE_TYPE_DEFAULT = {TRUE |**FALSE** }  
 指定如何处理缺失值分隔的文本文件中，当 PolyBase 从文本文件中检索数据。  
  
 TRUE  
 当从文本文件中检索数据，存储每个缺失值针对的外部表定义中的相应列的数据类型使用默认值。 例如，将缺失值替换：  
...
-   1900-01-01 如果列是日期列。  
 ...
 FALSE  
 将所有缺失值存储为 NULL。 通过使用带分隔符的文本文件中的单词 NULL 存储任何 NULL 值会作为字符串 NULL 导入。  
  
   编码 = {'UTF8' |UTF16}   
 在 Azure SQL 数据仓库中，PolyBase 可以读取 UTF8 和 UTF16 LE 编码带分隔符的文本文件。 在 SQL Server 和 PDW 中，PolyBase 不支持读取 UTF16 编码文件。
  
 DATA_COMPRESSION = *data_compression_method*  
 指定外部数据的数据压缩方法。 如果不指定 DATA_COMPRESSION，默认值为未压缩的数据。  
...
## <a name="general-remarks"></a>一般备注  
 ...
 当数据存储在压缩的格式之一时，PolyBase 在返回的数据记录之前将首先解压缩数据。  
  
## <a name="limitations-and-restrictions"></a>限制和局限   
  
 必须支持行分隔符分隔文本文件中的 Hadoop 的 LineRecordReader 即它必须是 '\r'、 \n 或 \r\n。 这些不是用户可配置的。  
 ...
 对外部表中的文件数的限制，因此我们建议将小于 30000 文件存储在根和外部文件位置的子文件夹。 此外，我们建议你保留成数量较少的根目录下的子文件夹的数量。 在引用的文件太多时可能会出现 Java 虚拟机的内存不足异常。  
...
### <a name="a-create-a-delimitedtext-external-file-format"></a>A. 创建 DELIMITEDTEXT 外部文件格式  
 此示例将创建名为外部文件格式*textdelimited1*文本分隔文件。 FORMAT_OPTIONS 指定将用竖线字符分隔文件中的字段 |。 文本文件也是使用 Gzip 编解码器压缩的。 如果不指定 DATA_COMPRESSION，则是未压缩的文本文件。  
