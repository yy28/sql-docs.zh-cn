---
title: "ALTER FULLTEXT INDEX (Transact SQL) |Microsoft 文档"
ms.custom: 
ms.date: 04/27/2017
...
[!INCLUDE[tsql-appliesto-ss2008-asdb-xxxx-xxx-md](../../includes/tsql-appliesto-ss2008-asdb-xxxx-xxx-md.md)]
  ...
  更改中的全文索引的属性[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。  
 ...
 *table_name*  
 ...
 ENABLE | DISABLE  
 告知[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]是否收集的全文索引数据*table_name*。 ENABLE 激活全文索引；DISABLE 关闭全文索引。 禁用索引时，表不支持全文查询。  
 ...
 SET CHANGE_TRACKING {MANUAL | AUTO | OFF}  
 指定对涵盖的全文索引的表列所做的更改 （更新、 删除或插入） 是否将传播由[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]到全文索引。 通过 WRITETEXT 和 UPDATETEXT 所做的数据更改不会反映到全文索引中，也不能使用更改跟踪方法拾取。  
...
>  有关更改跟踪交互和 WITH NO POPULATION 的信息，请参阅本主题后面的“备注”。  
 ...
 添加 |删除*column_name*  
 指定要添加到全文索引或从全文索引中删除的列。 列的类型必须是**char**， **varchar**， **nchar**， **nvarchar**，**文本**， **ntext**，**映像**， **xml**， **varbinary**，或**varbinary （max)**。  
  
 仅对先前为全文索引启用的列使用 DROP 子句。  
  
 使用类型列和语言与添加子句来设置这些属性上*column_name*。 添加列后，必须重新填充该表的全文索引，才能使针对此列进行的全文查询生效。  
  
> [!NOTE]  
>  在全文索引中添加或删除某个列后，是否填充全文索引取决于是否启用了更改跟踪以及是否指定了 WITH NO POPULATION。 有关详细信息，请参阅本主题后面的“备注”。  
  
 类型列*type_column_name*  
 指定一个表列的名称*type_column_name*，也就是说，用来保存的文档类型**varbinary**， **varbinary （max)**，或**映像**文档。 此列（称为类型列）包含用户提供的文件扩展名（.doc、.pdf、.xls 等）。 类型列必须是 **char**、 **nchar**、 **varchar**或 **nvarchar**类型。  
  
 指定类型列*type_column_name*才*column_name*指定**varbinary**， **varbinary （max)**或**映像**列中，在此情况下，数据未存储为二进制数据; 否则为[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]返回错误。  
 ...
 *language_term*是可选的可以指定为字符串、 整数或与区域设置标识符 (LCID) 的一种语言对应的十六进制值。 如果*language_term*指定，它表示的语言将应用到的搜索条件的所有元素。 如果未不指定任何值，默认全文语言的[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]使用实例。  
  
 使用**sp_configure**存储过程来访问有关的默认全文语言的信息[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]实例。  
  
 当指定为一个字符串， *language_term*对应于**别名**中的列值**syslanguages**系统表。 在情况下，字符串必须括在单引号中，*language_term*。 如果为一个整数，指定*language_term*是实际标识的语言的 LCID。 当指定为十六进制值， *language_term* 0x 后跟 LCID 的十六进制值。 十六进制值不能超过八位（包括前导零在内）。  
 ...
 对于包含多种语言的文本数据的非 BLOB 和非 XML 列，或者列中所存储文本的语言未知时，请使用非特定 (0x0) 语言资源。 对于存储在 XML 或 BLOB 类型列中的文档，在创建索引时，将使用文档内的语言编码。 例如，在 XML 列中，XML 文档中的 xml:lang 属性将标识语言。 在查询时，以前中指定的值*language_term*将成为用于全文查询，除非的默认语言*language_term*指定为全文查询的一部分。  
 ...
 **适用范围**： [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] 到 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。  
 ...
 如果启用了 CHANGE_TRACKING 并指定了 WITH NO POPULATION，[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 将返回一个错误。 如果启用 CHANGE_TRACKING 且未指定 WITH NO POPULATION，[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]对索引执行完全填充。  
 ...
 **适用范围**： [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] 到 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。  
 ...
 START {FULL|INCREMENTAL|UPDATE} POPULATION  
 告知[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]开始填充全文索引的*table_name*。 如果全文索引填充已在进行，[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]返回一条警告，并且不启动新的填充。  
  
 FULL  
 指定检索表中的每一行以进行全文索引，即使已对这些行进行了索引。  
  
 INCREMENTAL  
 指定仅检索自上次填充以来修改的行以进行全文索引。 仅当表包含一个类型为 **timestamp**的列时，才能应用 INCREMENTAL。 如果全文目录中的表不包含类型的列**时间戳**，表进行完全填充。  
 ...
 *stoplist_name*  
 ...
 设置搜索属性列表 {OFF |*property_list_name* } [WITH NO POPULATION]  
 **适用范围**： [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] 到 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。  
...
## <a name="changing-the-search-property-list-causes-rebuilding-the-index"></a>更改搜索属性列表将导致重新生成索引  
 ...
 但是，如果将全文索引与不同的属性列表相关联，则会重新生成索引。 立即重新生成索引将截断全文索引，从而删除所有现有的数据，并且必须重新填充索引。 尽管填充进行时，在基表搜索仅在已填充按建立索引的表行上的全文查询。 重新填充的索引数据将包括新添加的搜索属性列表的注册属性中的元数据。  
...
> [!NOTE]  
>  有关如何为全文搜索使用搜索属性列表的工作原理的详细信息，请参阅[使用搜索属性列表搜索文档属性](../../relational-databases/search/search-document-properties-with-search-property-lists.md)。 有关完全填充的信息，请参阅[填充全文索引](../../relational-databases/search/populate-full-text-indexes.md)。  
  
### <a name="scenario-a-switching-directly-to-a-different-search-property-list"></a>情况 A：直接切换到不同的搜索属性列表  
  
1.  创建全文索引`table_1`与搜索属性列表`spl_1`:  
...
1.  创建全文索引`table_1`与搜索属性列表`spl_1`后, 跟自动的完全填充 （默认行为）：  
...
3.  全文索引将再次与相同或不同的搜索属性列表相关联。  
     ...
     例如以下语句重新将关联的全文索引与原始的搜索属性列表， `spl_1`:  
    ...
    > [!NOTE]  
    >  重新生成还需要为不同的搜索属性列表，如`spl_2`。  
  
## <a name="permissions"></a>Permissions  
 用户必须具有 ALTER 权限表或索引的视图，或者是属于**sysadmin**固定服务器角色或**db_ddladmin**或**db_owner**固定数据库角色的成员.  
...
**适用范围**： [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] 到 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。  
 ...
 下面的示例将`DocumentPropertyList`属性列表与全文索引`Production.Document`表。 此 ALTER FULLTEXT INDEX 语句启动完全填充，这是 SET SEARCH PROPERTY LIST 子句的默认行为。  
...
### <a name="d-starting-a-full-population"></a>D. 启动完全填充  
 下面的示例启动完全填充的全文索引上`JobCandidate`表。  
