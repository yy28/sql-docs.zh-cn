...
> 可以对执行完整数据库备份**master**数据库。  
 ...
 你可以通过指定日志备份还原到特定时间或备份内的事务`WITH STOPAT`， `STOPATMARK`，或`STOPBEFOREMARK`中你[RESTORE LOG](../../t-sql/statements/restore-statements-transact-sql.md)语句。  
  
> [!NOTE]  
>  典型的日志备份后，某些事务日志记录变为非活动状态，除非另行指定，`WITH NO_TRUNCATE`或`COPY_ONLY`。 一个或多个虚拟日志文件中的所有记录变为不活动状态后，日志将被截断。 如果日志在常规日志备份后未被截断，则可能是某些操作延迟了日志截断。 有关详细信息，请参阅[因素可能延迟日志截断](../../relational-databases/logs/the-transaction-log-sql-server.md#FactorsThatDelayTruncation)。  
  
 { *database_name* | **@ * * * database_name_var* } 是从其事务日志、 部分数据库或完整数据库备份的数据库。如果为变量提供 (**@***database_name_var*)，此名称可以是指定为字符串常量 (**@***database_name_var***= * * * 数据库名称*) 或字符变量字符串数据类型，除**ntext**或**文本**数据类型。  
 ...
 *n*  
 ...
 有关详细信息，请参阅"介质簇中镜像媒体集"[备注](#general-remarks)本主题后面的部分。  
...
> 默认情况下，`BACKUP DATABASE`创建完整备份。  
 ...
 ENCRYPTION  
 用于指定将备份加密。 你可以指定要与备份加密或指定的加密算法`NO_ENCRYPTION`没有加密的备份。 建议进行加密以帮助保护备份文件的安全。 可指定的算法的列表如下：  
...
> [!WARNING]  
> 当结合使用加密`FILE_SNAPSHOT`使用指定的加密算法加密参数，该元数据文件本身和系统验证[透明数据加密 (TDE)](../../relational-databases/security/encryption/transparent-data-encryption.md)已完成对于数据库中。 无其他加密对于数据本身。 如果未加密数据库，或如果之前未完成加密颁发备份语句，备份将失败。  
...
> [!NOTE]  
> 若要指定备份集还原操作，请使用`FILE = <backup_set_file_number>`选项。 有关如何指定备份集的详细信息，请参阅"指定备份集"中[RESTORE 参数 &#40;Transact SQL &#41;](../../t-sql/statements/restore-statements-arguments-transact-sql.md).
...
-   如果用于`BACKUP DATABASE`、`COPY_ONLY`选项创建不能用作差异基准的完整备份。 差异位图不会被更新，因此差异备份的表现就像仅复制备份不存在一样。 后续差异备份将最新的常规完整备份用作它们的基准。  
  
    > [!IMPORTANT]  
    > 如果`DIFFERENTIAL`和`COPY_ONLY`一起，使用`COPY_ONLY`将被忽略，并创建差异备份。  
  
-   如果用于`BACKUP LOG`、`COPY_ONLY`选项创建*仅复制日志备份*，其中不截断事务日志。 仅复制日志备份对日志链没有任何影响，因此其他日志备份的表现就像仅复制备份不存在一样。  
...
有关使用与备份压缩信息[透明数据加密 (TDE)](../../relational-databases/security/encryption/transparent-data-encryption.md)启用的数据库，请参阅[备注](#general-remarks)部分。
...
> [!NOTE]  
> 璝惠之间的交互 { **NOINIT** |INIT} 和 { **NOSKIP** |跳过}，请参阅[备注](#general-remarks)本主题中更高版本。  
...
> [!IMPORTANT]  
> 使用`FORMAT`仔细。 格式化介质集的任何一个卷都将使整个介质集不可用。 例如，如果初始化现有条带介质集中的单个磁带，则整个介质集都将变得不可用。  
  
指定格式意味着`SKIP`;`SKIP`不需要显式声明。  
  
MEDIADESCRIPTION **=** { *text* | **@***text_variable* }  
指定介质集的自由格式文本说明，最多为 255 个字符。  
  
MEDIANAME  **=**  { *media_name* | **@ * * * media_name_variable* }  
指定整个备份介质集的介质名称。 介质名称必须是长度不得超过 128 个字符，如果`MEDIANAME`指定，则它必须与备份的卷上已有以前指定的介质名称匹配。 如果未指定该选项或指定了 SKIP 选项，将不会对介质名称进行验证检查。  
...
> [!NOTE]  
> 有关使用的重要信息`BUFFERCOUNT`选项，请参阅[不正确 BufferCount 数据传输选项可能会导致 OOM 情况](http://blogs.msdn.com/b/sqlserverfaq/archive/2010/05/06/incorrect-buffercount-data-transfer-option-can-lead-to-oom-condition.aspx)博客。  
  
MAXTRANSFERSIZE  **=**  { *maxtransfersize*  |   ***@**  maxtransfersize_variable*} 指定的最大传输单元 （以字节） 之间使用[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]和备份媒体。 可能的值是 65536 字节 (64 KB) 的倍数，最多可到 4194304 字节 (4 MB)。  

> [!NOTE]  
> 如果已配置的数据库使用 SQL 编写器服务，创建备份时[FILESTREAM](../../relational-databases/blob/filestream-sql-server.md)，或包括[内存优化文件组](../../relational-databases/in-memory-oltp/the-memory-optimized-filegroup.md)，则`MAXTRANSFERSIZE`在还原时应为大于或等于`MAXTRANSFERSIZE`创建备份时使用。 

> [!NOTE]  
> 有关[透明数据加密 (TDE)](../../relational-databases/security/encryption/transparent-data-encryption.md)一个单个数据文件，默认启用了数据库`MAXTRANSFERSIZE`为 65536 (64 KB)。 非 TDE 加密数据库默认值`MAXTRANSFERSIZE`1048576 (1 MB) 使用时，是备份到磁盘和 65536 (64 KB) 时使用 VDI 或磁带。
> 有关用于 TDE 的备份压缩的详细信息加密数据库，请参阅[备注](#general-remarks)部分。
...
> [!NOTE]  
> 如果你使用`NOREWIND`，实例[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]之前相同的进程中运行的备份或还原语句使用一直保留磁带驱动器的所有权`REWIND`或`UNLOAD`选项或服务器实例已关闭。 磁带保持打开将防止其他进程访问磁带。 有关如何显示打开的磁带列表以及如何关闭打开的磁带的信息，请参阅[备份设备 &#40;SQL server&#41;](../../relational-databases/backup-restore/backup-devices-sql-server.md).  
  
{**卸载**|NOUNLOAD}    

> [!NOTE]  
> `UNLOAD`和`NOUNLOAD`是保留的整个生命周期的会话或它通过指定其他重置之前的会话设置。  
...
> [!NOTE]  
> 备份到磁带备份设备，`BLOCKSIZE`选项会影响备份操作的性能。 通常，只有写入磁带设备时，此选项才会影响性能。  
...
{NORECOVERY |备用 **=**  *undo_file_name* }  
  ...
  若要执行跳过日志截断，且然后使数据库进入 RESTORING 状态以原子方式的最大努力日志备份，使用`NO_TRUNCATE`和`NORECOVERY`选项组合在一起。  
...
NO_TRUNCATE  
指定是不会被截断的日志，将导致[!INCLUDE[ssDE](../../includes/ssde-md.md)]尝试备份而不考虑数据库的状态。 因此，与执行的备份`NO_TRUNCATE`可能具有不完整元数据。 该选项允许在数据库损坏时备份日志。  
  
BACKUP LOG 的 NO_TRUNCATE 选项相当于同时指定 COPY_ONLY 和 CONTINUE_AFTER_ERROR。  
  
而无需`NO_TRUNCATE`选项，该数据库必须处于联机状态。 如果数据库处于挂起状态，你可以通过指定创建备份`NO_TRUNCATE`。 但如果数据库处于脱机或紧急状态，即使不允许备份`NO_TRUNCATE`。 有关数据库状态的信息，请参阅[Database States](../../relational-databases/databases/database-states.md)。  
  
## <a name="about-working-with-sql-server-backups"></a>有关使用 SQL Server 备份  
 本节简要说明了以下基本备份概念：  
...
> [!NOTE]  
> `BACKUP LOG WITH NO_LOG`和`WITH TRUNCATE_ONLY`已停用选项。 使用完整恢复模式或大容量日志恢复模式时，如果必须删除数据库中的日志备份链，请切换至简单恢复模式。 有关详细信息，请参阅[查看或更改数据库的恢复模式 (SQL Server)](../../relational-databases/backup-restore/view-or-change-the-recovery-model-of-a-database-sql-server.md)。  
...
#### <a name="backup-devices-in-a-striped-media-set-a-stripe-set"></a>条带的介质集 （带区集） 中的备份设备  
 A*带区集*是一套数据划分为块和分布式的固定顺序在其的磁盘文件。 条带集中使用的备份设备的数目必须保持不变 (除非媒体重新初始化`FORMAT`)。  
...
#### <a name="working-with-a-mirrored-media-set"></a>使用一个镜像的介质集  
 ...
 若要备份到镜像介质集，则所有的镜像服务器必须存在。 若要备份到镜像的介质集中，指定`TO`子句来指定第一个镜像数据库，并指定`MIRROR TO`每个其他镜像的子句。  
  
 获取镜像的介质集，每个`MIRROR TO`子句必须列出相同数量和类型的设备作为的 TO 子句。 下面的示例写入到包含两个镜像并在每个镜像中使用三个设备的镜像介质集：  
...
##### <a name="media-families-in-mirrored-media-sets"></a>在镜像的介质集的介质簇  
 中指定每个备份设备`TO`备份语句子句对应于介质簇。 例如，如果`TO`子句列出三个设备，备份将数据写入三个介质簇。 在镜像介质集中，每个镜像都必须包含各个介质簇的副本。 这正是各个镜像中的设备数量必须相同的原因。  
...
||NOINIT|INIT|  
|------|------------|----------|  
|NOSKIP|如果该卷包含一个有效介质标头，验证媒体名称匹配给定`MEDIANAME`，如果任何。 如果匹配，则追加备份集，同时保留所有现有的备份集。<br /> 如果卷中不含有效的介质标头，则会发生错误。|如果卷中包含有效的介质标头，将执行以下检查：<br /><ul><li>如果`MEDIANAME`指定了，验证指定的介质名称是否匹配介质标头的介质名称。<sup>1</sup></li><li>确保介质上没有未过期的备份集。 如果有，则终止备份。</li></ul><br />如果这些检查都通过了，则覆盖该介质上的所有备份集，只保留介质标头。<br /> 如果卷不包含有效介质标头，生成一个使用指定密钥`MEDIANAME`和`MEDIADESCRIPTION`，如果任何。|  
|SKIP|如果卷中包含有效的介质标头，则追加备份集，并保留所有现有备份集。|如果卷中会包含一个有效<sup>2</sup>介质标头，将覆盖在媒体上，并保留介质标头的任何备份集。<br /> 如果介质为空，将生成一个使用指定的介质标头`MEDIANAME`和`MEDIADESCRIPTION`，如果任何。|  

<sup>1</sup>用户必须属于适当固定数据库或服务器角色以执行备份操作。    
...
备份支持`RESTART`选项以提供与早期版本的向后兼容性[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。 但是，RESTART 不起作用。  
  
## <a name="general-remarks"></a>常规备注  
...
使用与备份压缩时[透明数据加密 (TDE)](../../relational-databases/security/encryption/transparent-data-encryption.md)启用了数据库使用单个数据文件时，建议使用`MAXTRANSFERSIZE`设置**65536 (64 KB) 大于**。   
从开始[!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]，则此启用 TDE 的优化的压缩算法加密首先解密页压缩，然后再次加密的数据库。 如果使用`MAXTRANSFERSIZE = 65536`(64 KB)，使用 TDE 加密数据库的备份压缩直接压缩的加密的页面，并可能不会产生较好的压缩率。 有关详细信息，请参阅[已启用 TDE 的数据库的备份压缩](http://blogs.msdn.microsoft.com/sqlcat/2016/06/20/sqlsweet16-episode-1-backup-compression-for-tde-enabled-databases/)。

> [!NOTE]  
> TDE 加密数据库的优化的压缩算法自动时使用：
> * 备份到 URL 时，在此情况下默认`MAXTRANSFERSIZE`更改为 1048576 (1 MB)，并不强制为低的值。
> * 该数据库有多个数据文件，这种情况下默认`MAXTRANSFERSIZE`更改为 65536 的倍数 (64 KB) 不会更改为较低的值 (如`MAXTRANSFERSIZE = 65536`)。 
...
-   文件管理操作如`ALTER DATABASE`语句使用`ADD FILE`或`REMOVE FILE`选项。  
...
## <a name="security"></a>Security  
 开头[!INCLUDE[ssSQL11](../../includes/sssql11-md.md)]、`PASSWORD`和`MEDIAPASSWORD`选项不再可用于创建备份。 就仍可以还原使用密码创建的备份。  
