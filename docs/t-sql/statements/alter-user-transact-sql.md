...
> [!IMPORTANT]
> SQL 数据库托管实例的 Azure AD 登录名当前为公共预览版  。 Azure SQL 数据库托管实例应用于具有 Azure AD 登录名的用户时，仅支持以下选项：`DEFAULT_SCHEMA = { schemaName | NULL }` 和 `DEFAULT_LANGUAGE = { NONE | lcid | language name | language alias }`
