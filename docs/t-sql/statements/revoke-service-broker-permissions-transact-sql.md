---
title: "撤消 Service Broker 权限 (Transact SQL) |Microsoft 文档"
ms.custom: 
ms.date: 03/03/2017
...
## <a name="arguments"></a>参数  
 GRANT OPTION FOR  
 指示要撤消用于向其他主体授予指定权限的权限。 权限本身不会撤消。  
 ...
 *database_principal*  
 指定要从中撤消权限的主体。 *database_principal*可以是以下之一：  
...
>  如果对授予了 WITH GRANT OPTION 权限的权限执行级联撤消，将同时撤消该权限的 GRANT 和 DENY 权限。  
 ...
 AS *revoking_principal*  
 指定一个主体，执行该查询的主体从该主体获得撤消该权限的权利。 *revoking_principal*可以是以下之一：  
...
## <a name="service-broker-contracts"></a>Service Broker 约定  
 A[!INCLUDE[ssSB](../../includes/sssb-md.md)]协定是数据库级安全所包含的数据库，是指其父权限层次结构中的。 最特定和受限权限，可以上吊销[!INCLUDE[ssSB](../../includes/sssb-md.md)]协定列出以下表，以及将其包含是通过默示的更多常规权限中。  
...
## <a name="service-broker-routes"></a>Service Broker 路由  
 [!INCLUDE[ssSB](../../includes/sssb-md.md)] 路由是权限层次结构中其父级数据库包含的数据库级安全对象。 最特定和受限权限，可以上吊销[!INCLUDE[ssSB](../../includes/sssb-md.md)]路由列示在以下表中，以及将其包含是通过默示的更多常规权限。  
...
### <a name="service-broker-services"></a>Service Broker 服务  
 [!INCLUDE[ssSB](../../includes/sssb-md.md)] 服务是权限层次结构中其父级数据库包含的数据库级安全对象。 最特定和受限权限，可以上吊销[!INCLUDE[ssSB](../../includes/sssb-md.md)]服务将在下表中，以及将其包含是通过默示的更多常规权限中列出。  
...
## <a name="see-also"></a>另请参阅  
 [授予 Service Broker 权限 &#40;Transact SQL &#41;](../../t-sql/statements/grant-service-broker-permissions-transact-sql.md)   
 [拒绝 Service Broker 权限 &#40;Transact SQL &#41;](../../t-sql/statements/deny-service-broker-permissions-transact-sql.md)   
