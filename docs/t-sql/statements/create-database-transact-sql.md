...
PRIMARY 指定关联的 \<filespec> 列表定义主文件。 在主文件组的 \<filespec> 项中指定的第一个文件将成为主文件。 一个数据库只能有一个主文件。 有关详细信息，请参阅 [Database Files and Filegroups](../../relational-databases/databases/database-files-and-filegroups.md)。
...
FILENAME { 'os\_file\_name' | 'filestream\_path' } 指定操作系统（物理）文件名。
...
SERVICE_OBJECTIVE 指定性能级别。 有关 [!INCLUDE[ssSDW](../../includes/sssdw-md.md)] 的服务目标的详细信息，请参阅[性能层](https://azure.microsoft.com/documentation/articles/performance-tiers/)。
