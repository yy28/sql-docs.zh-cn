...
REQUEST_MAX_MEMORY_GRANT_PERCENT = value  
指定单个请求可以从池中获取的最大内存量。 value 是相对于 MAX_MEMORY_PERCENT 指定的资源池大小的百分比。 value 是 [!INCLUDE[ssSQL17](../../includes/sssql17-md.md)] 以下版本的整数，并且自 [!INCLUDE[sql-server-2019](../../includes/sssqlv15-md.md)] 起上下浮动（默认设置为 25）。 value 的允许范围是 1 到 100。  
...
-   将 value 设置为 0 可以阻止在用户定义的工作负荷组中运行具有 SORT 和 HASH MATCH 操作的查询。  
...
MAX_DOP = value  
指定并行请求的最大并行度 (DOP)。 value 必须为 0 或一个正整数。 value 的允许范围为 0 到 64。 value 的默认设置为 0，表示使用全局设置。 按如下方式处理 MAX_DOP：  
...
### <a name="1-shows-how-to-create-a-workload-group-named-newreports"></a>1.演示如何创建名为 newReports 的工作负荷组    
