---
title: "STPolyFromWKB (geometry 数据类型) |Microsoft 文档"
ms.custom: 
ms.date: 08/03/2017
...
返回**geometryPolygon**开放地理空间联盟 (OGC) 熟知二进制 (WKB) 表示形式的实例。
 ...
 *WKB_polygon*  
 是的 WKB 表示形式**geometryPolygon**您希望返回的实例。 *WKB_polygon*是**varbinary （max)**表达式。  
  
 *SRID*  
 是**int**表达式表示空间引用标识符 (SRID) 的**geometryPolygon**您希望返回的实例。  
