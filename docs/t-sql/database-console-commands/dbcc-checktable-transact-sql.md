...
REPAIR_REBUILD  
 ...
 > [!NOTE]  
 > 仅将 REPAIR 选项作为最后手段使用。 若要修复错误，建议您通过备份进行还原。 修复操作不会考虑表本身或表之间可能存在的任何约束。 如果一个或多个约束涉及到指定的表，我们建议运行`DBCC CHECKCONSTRAINTS`后修复操作。
 > 如果必须使用修复，运行`DBCC CHECKTABLE`没有提供用于查找要使用的修复级别修复选项。 如果想要使用 REPAIR_ALLOW_DATA_LOSS 级别，我们建议你备份数据库，在运行之前`DBCC CHECKTABLE`使用此选项。  
...
MAXDOP  
 **适用于**: [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] (开头[!INCLUDE[ssSQL14](../../includes/sssql14-md.md)]通过 SP2 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)])。  
...
- 从开始[!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]，对持久化计算的列、 UDT 列和筛选的索引的其他检查不会默认情况下，若要避免昂贵的表达式计算。 此更改大大减少了对包含这些对象的数据库 CHECKDB 的持续时间。 但是，这些对象的物理一致性检查是始终会完成。 仅当指定 EXTENDED_LOGICAL_CHECKS 选项时将表达式计算执行除了已存在逻辑检查 （索引的视图、 XML 索引和空间索引） 作为 EXTENDED_LOGICAL_CHECKS 选项的一部分。
-  如果兼容性级别为 90 ([!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)]) 或更短，除非指定 NOINDEX DBCC CHECKTABLE 执行物理和逻辑一致性检查的单个表或索引的视图和上所有其非聚集和 XML 索引。 不支持空间索引。
...
小型转储文件 (`SQLDUMP*nnnn*.txt`) 中创建[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]每当 DBCC CHECKTABLE 检测到损坏的错误的日志目录。 当*功能使用情况*数据收集和*错误报告*的实例启用功能[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]，该文件自动转发到[!INCLUDE[msCoName](../../includes/msconame-md.md)]。 收集的数据将用于改进 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 功能。
