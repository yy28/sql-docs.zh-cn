...
> **注意：** DBCC CHECKDB 支持包含内存优化表的数据库时，但在基于磁盘的表中时只执行验证。 但是，作为数据库备份和恢复的一部分，将对内存优化文件组中的文件完成 CHECKSUM 验证。    
>     
...
> [!WARNING]
> - REPAIR_ALLOW_DATA_LOSS 选项是一个受支持的功能，但它可能不是始终都将数据库添加到以物理方式一致状态的最佳选项。 
> -如果成功，REPAIR_ALLOW_DATA_LOSS 选项可能导致丢失某些数据。 实际上，它可能导致的数据丢失多于用户从上次已知成功备份还原数据库导致的数据丢失。 
>
...
在 SP2 以前的 [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] 版本中，执行 DBCC CHECKDB 将清除 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例的计划高速缓存。 清除计划缓存将导致对所有后续执行计划进行重新编译，并可能会导致查询性能暂时性地突然降低。 在 SP2 及更高版本中，执行 DBCC CHECKDB 不会清除计划缓存。
...
转储文件 (SQLDUMP*nnnn*.txt) 中创建[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]日志目录每当 DBCC CHECKDB 检测到损坏错误。 如果为 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例启用了“功能使用情况数据收集”和“错误报告”功能，该文件将被自动转发给 [!INCLUDE[msCoName](../../includes/msconame-md.md)]。 收集的数据将用于改进 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 功能。
转储文件包含 DBCC CHECKDB 命令的结果以及其他诊断输出数据。 访问仅限于[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]服务帐户和 sysadmin 角色的成员。 默认情况下，sysadmin 角色包含 Windows builtin\ 管理员组和本地管理员组的所有成员。 如果数据收集进程失败，DBCC 命令不会失败。
