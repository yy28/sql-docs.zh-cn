...
-   运行[DBCC CHECKALLOC](../../t-sql/database-console-commands/dbcc-checkalloc-transact-sql.md)数据库上。    
-   运行[DBCC CHECKTABLE](../../t-sql/database-console-commands/dbcc-checktable-transact-sql.md)上每个表和数据库中的视图。    
-   运行[DBCC CHECKCATALOG](../../t-sql/database-console-commands/dbcc-checkcatalog-transact-sql.md)数据库上。    
-   验证数据库中每个索引视图的内容。    
-   存储时验证表元数据和文件系统目录和文件之间的链接级一致性**varbinary （max)**使用 FILESTREAM 的文件系统中的数据。    
 ...
 *database_name* | *database_id* | 0  
 要为其运行完整性检查的数据库的名称或 ID。 如果未指定，或者指定为 0，则使用当前数据库。 数据库名称必须符合的规则[标识符](../../relational-databases/databases/database-identifiers.md)。  
...
> [!WARNING]
> REPAIR_ALLOW_DATA_LOSS 选项是一个受支持的功能，但它可能不是始终都将数据库添加到以物理方式一致状态的最佳选项。 如果成功，REPAIR_ALLOW_DATA_LOSS 选项可能会导致一些数据丢失。 实际上，它可能导致的数据丢失多于用户从上次已知成功备份还原数据库导致的数据丢失。 
>
...
EXTENDED_LOGICAL_CHECKS  
 ...
 有关详细信息，请参阅*执行逻辑一致性检查对索引*中[备注](#remarks)本主题中后面的部分。  
...
ESTIMATEONLY  
 显示的估计的 tempdb 运行 DBCC CHECKDB 与所有其他指定选项所需的空间量。 不执行实际数据库检查。  
...
此 argment 始终意味着 NO_INFOMSGS，并且不允许使用任何一个的修复选项。  
    
> [!WARNING] 
> 指定 PHYSICAL_ONLY 会使 DBCC CHECKDB 跳过对 FILESTREAM 数据的所有检查。
    
DATA_PURITY  
 使 DBCC CHECKDB 检查数据库中是否存在无效或越界的列值。 例如，DBCC CHECKDB 检测到具有都大于或小于的可接受范围的日期和时间值的列**datetime**数据类型; 或**十进制**或近似数字数据类型具有无效的小数位数或精度值的列。  
...
> 如果指定了 PHYSICAL_ONLY，则不执行列完整性检查。
 ...
 MAXDOP  
 **适用于**: [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ([!INCLUDE[ssSQL14](../../includes/sssql14-md.md)]通过 SP2 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)])。  
...
从开始[!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)]SP2，执行的 DBCC CHECKDB**不再**清除计划缓存的实例的[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。 之前[!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)]SP2，执行的 DBCC CHECKDB 清除计划缓存。 清除计划缓存将导致对所有后续执行计划进行重新编译，并可能会导致查询性能暂时性地突然降低。 
...
对于执行这些检查所需要的事务一致性，DBCC CHECKDB 使用内部数据库快照。 这样可以防止在执行这些命令时出现阻塞和并发问题。 有关详细信息，请参阅[查看数据库快照 &#40; 的稀疏文件的大小Transact SQL &#41;](../../relational-databases/databases/view-the-size-of-the-sparse-file-of-a-database-snapshot-transact-sql.md)和中的 DBCC 内部数据库快照的使用情况部分[DBCC &#40;Transact SQL &#41;](../../t-sql/database-console-commands/dbcc-transact-sql.md). 如果无法创建快照或指定了 TABLOCK，DBCC CHECKDB 将获取锁以获得所需一致性。 在这种情况下，需要排他数据库锁才能执行分配检查，需要共享表锁才能执行表检查。
如果无法创建内部数据库快照，针对主机运行时，DBCC CHECKDB 无法正常工作。
对 tempdb 运行 DBCC CHECKDB 不执行任何分配或目录的检查，必须获取共享的表锁来执行表检查。 这是因为，为了提高性能，不允许对 tempdb 使用数据库快照。 这意味着，无法获得所需的事务一致性。
在 Microsoft SQL Server 2012 或 SQL Server 的早期版本中，运行具有其文件位于 ReFS 格式化的卷上的数据库的 DBCC CHECKDB 命令时可能遇到的错误消息。 有关详细信息，请参阅知识库文章 2974455:[时 SQL Server 数据库位于 ReFS 卷上的 DBCC CHECKDB 行为。](https://support.microsoft.com/kb/2974455)    
    
## <a name="checking-and-repairing-filestream-data"></a>检查和修复 FILESTREAM 数据    
在数据库和表启用 FILESTREAM，你可以根据需要存储**varbinary （max)**文件系统中的二进制大型对象 (Blob)。 对在文件系统中存储 BLOB 的数据库使用 DBCC CHECKDB 时，DBCC 将检查文件系统和数据库之间的链接级一致性。
例如，如果表包含**varbinary （max)**使用 FILESTREAM 属性，DBCC CHECKDB 的列将检查是否存在文件系统目录和文件和表行、 列和列之间的一对一映射值。 如果指定 REPAIR_ALLOW_DATA_LOSS 选项，DBCC CHECKDB 便可修复损坏。 为修复 FILESTREAM 损坏，DBCC 将删除缺少文件系统数据的任何表行。
...
转储文件 (`SQLDUMP*nnnn*.txt`) 中创建[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]日志目录每当 DBCC CHECKDB 检测到损坏错误。 当*功能使用情况*数据收集和*错误报告*的实例启用功能[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]，该文件自动转发到[!INCLUDE[msCoName](../../includes/msconame-md.md)]。 收集的数据将用于改进 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 功能。
转储文件包含 DBCC CHECKDB 命令的结果以及其他诊断输出数据。 访问仅限于[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]服务帐户和 sysadmin 角色的成员。 默认情况下，sysadmin 角色中包含的 windows 的所有成员`BUILTIN\Administrators`组和本地管理员组。 如果数据收集进程失败，DBCC 命令不会失败。
...
当数据库已设置为紧急模式下使用[ALTER DATABASE](../../t-sql/statements/alter-database-transact-sql.md)语句，DBCC CHECKDB 可以执行一些特殊修复数据库上如果指定了 REPAIR_ALLOW_DATA_LOSS 选项。 这些修复可能允许那些在普通情况下无法恢复的数据库在物理一致状态下重新联机。 这些修复应当是最后手段，并且只有在无法从备份还原数据库时才采用。 当数据库设置为紧急模式下时，数据库标记为 READ_ONLY，禁用日志记录，并访问仅限于 sysadmin 固定的服务器角色的成员。
...
如果 DBCC CHECKDB 命令成功，则数据库将在物理上是一致的，并且数据库状态将设置为 ONLINE。 但是，数据库可能包含一种或多种事务不一致的情况。 我们建议你运行[DBCC CHECKCONSTRAINTS](../../t-sql/database-console-commands/dbcc-checkconstraints-transact-sql.md)以确定任何业务逻辑缺陷，并立即备份数据库。
