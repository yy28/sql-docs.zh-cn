---
title: "声明游标 (Transact SQL) |Microsoft 文档"
ms.custom: 
ms.date: 03/14/2017
 ...
 *cursor_name*  
 是的名称[!INCLUDE[tsql](../../includes/tsql-md.md)]定义的服务器游标。 *cursor_name*必须符合标识符的规则。  
  
 INSENSITIVE  
 定义一个游标，以创建将由该游标使用的数据的临时副本。 此临时表中回答到光标处的所有请求**tempdb**; 因此，要基础表所做的修改不会反映在返回对此游标所做的提取的数据，并且此游标不允许修改。 使用 ISO 语法时，如果省略 INSENSITIVE，则已提交的（任何用户）对基础表的删除和更新则会反映在后面的提取操作中。  
 ...
 *select_statement*  
 ...
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]隐式转换为另一种类型的游标，如果中的子句*select_statement*与请求的游标类型的功能发生冲突。  
  
 READ ONLY  
 禁止通过该游标进行更新。 在 UPDATE 或 DELETE 语句的 WHERE CURRENT OF 子句中不能引用游标。 该选项优先于要更新的游标的默认功能。  
  
 更新 [OF *column_name* [**，**...*n*]]  
 定义游标中可更新的列。 如果的*column_name* [**，**...*.n*] 指定，仅列出的列进行修改。 如果指定了 UPDATE，但未指定列的列表，则可以更新所有列。  
...
> [!NOTE]  
>  如果指定既不全局或本地，则默认值由的设置进行控制**默认为局部游标**数据库选项。  
 ...
 STATIC  
 定义一个游标，以创建将由该游标使用的数据的临时副本。 此临时表中回答到光标处的所有请求**tempdb**; 因此，要基础表所做的修改不会反映在返回对此游标所做的提取的数据，并且此游标不允许修改。  
  
 KEYSET  
 指定当游标打开时，游标中行的成员身份和顺序已经固定。 中的表中内置的唯一标识行的密钥集**tempdb**称为**键集**。  
  
> [!NOTE]  
>  如果查询引用了至少一个无唯一索引的表，则键集游标将转换为静态游标。  
  
 对基表中的非键值所做的更改（由游标所有者更改或由其他用户提交）可以在用户滚动游标时看到。 其他用户执行的插入是不可见的（不能通过 [!INCLUDE[tsql](../../includes/tsql-md.md)] 服务器游标执行插入）。 如果删除了某行，尝试读取的行返回 @@FETCH_STATUS为-2。 从游标外部更新键值类似于删除旧行后再插入新行。 具有新值的行不可见，并且尝试提取具有旧值的行返回 @@FETCH_STATUS为-2。 如果通过指定 WHERE CURRENT OF 子句来通过游标执行更新，则新值可见。  
...
>  FAST_FORWARD 和 FORWARD_ONLY 可以同时用在同一个 DECLARE CURSOR 语句中。  
 ...
 SCROLL_LOCKS  
 指定通过游标进行的定位更新或删除一定会成功。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]在读取到光标以确保其可用性的更高版本的修改会锁定行。 如果还指定了 FAST_FORWARD 或 STATIC，则不能指定 SCROLL_LOCKS。  
  
 OPTIMISTIC  
 指定如果行自读入游标以来已得到更新，则通过游标进行的定位更新或定位删除不成功。 当将行读入游标时，[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 不锁定行。 它改为使用的比较**时间戳**列的值或如果表未包含任何校验和值**时间戳**列，来确定是否它已读入游标后已修改行。 如果已修改该行，尝试进行的定位更新或定位删除将失败。 开放式不能指定是否还指定 FAST_FORWARD。  
  
 TYPE_WARNING  
 指定如果游标从所请求的类型隐式转换为另一种类型，则向客户端发送警告消息。  
  
 *select_statement*  
 定义游标结果集的标准 SELECT 语句。 内不允许使用计算、 COMPUTE BY，用于浏览和 INTO 关键字*select_statement*游标声明。  
  
> [!NOTE]  
>  你可以使用在游标声明; 内的查询提示但是，如果你还使用有关更新的子句，指定选项 (*query_hint*) 之后的更新。  
  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]隐式转换为另一种类型的游标，如果中的子句*select_statement*与请求的游标类型的功能发生冲突。 有关详细信息，请参阅“隐式游标转换”。  
  
 更新 [OF *column_name* [**，**...*n*]]  
 定义游标中可更新的列。 如果的*column_name* [**，**... *n* ] 提供，仅列出的列进行修改。 如果指定了 UPDATE，但未指定列的列表，则除非指定了 READ_ONLY 并发选项，否则可以更新所有的列。  
...
## <a name="remarks"></a>注释  
 ...
 不能混淆这两种格式。 如果你指定滚动或之前光标关键字不区分大小写的关键字，则无法使用光标之间和任何关键字*select_statement*关键字。 如果你指定光标之间和任何关键字*select_statement*关键字，不能在光标关键字之前指定滚动或 INSENSITIVE。  
