...
## <a name="a-using-select-to-retrieve-rows-and-columns"></a>A. 使用 SELECT 检索行和列  
 ...
 [!code-sql[选择 #SelectExamples1](../../t-sql/queries/codesnippet/tsql/select-examples-transact_1.sql)]  
 ...
 [!code-sql[选择 #SelectExamples2](../../t-sql/queries/codesnippet/tsql/select-examples-transact_2.sql)]  
  
 该示例仅返回 `Product` 表中产品系列为 `R` 且生产天数少于 `4` 的那些行。  
  
 [!code-sql[选择 #SelectExamples3](../../t-sql/queries/codesnippet/tsql/select-examples-transact_3.sql)]  
...
## <a name="b-using-select-with-column-headings-and-calculations"></a>B. 将 SELECT 与列标题和列计算一起使用  
 ...
 [!code-sql[选择 #SelectExamples4](../../t-sql/queries/codesnippet/tsql/select-examples-transact_4.sql)]  
  
 该查询将计算每个销售订单中每种产品的收入。  
  
 [!code-sql[选择 #SelectExamples5](../../t-sql/queries/codesnippet/tsql/select-examples-transact_5.sql)]  
  
## <a name="c-using-distinct-with-select"></a>C. 将 DISTINCT 与 SELECT 一起使用  
 以下示例使用 `DISTINCT` 以避免检索重复标题。  
  
 [!code-sql[选择 #SelectExamples6](../../t-sql/queries/codesnippet/tsql/select-examples-transact_6.sql)]  
  
## <a name="d-creating-tables-with-select-into"></a>D. 使用 SELECT INTO 创建表  
 以下第一个示例将在 `#Bicycles` 中创建一个名为 `tempdb` 的临时表。  
  
 [!code-sql[选择 #SelectExamples7](../../t-sql/queries/codesnippet/tsql/select-examples-transact_7.sql)]  
  
 第二个示例创建永久表 `NewProducts`。  
  
 [!code-sql[选择 #SelectExamples8](../../t-sql/queries/codesnippet/tsql/select-examples-transact_8.sql)]  
...
## <a name="e-using-correlated-subqueries"></a>E. 使用相关子查询  
 ...
 [!code-sql[选择 #SelectExamples9](../../t-sql/queries/codesnippet/tsql/select-examples-transact_9.sql)]  
 ...
 [!code-sql[选择 #SelectExamples10](../../t-sql/queries/codesnippet/tsql/select-examples-transact_10.sql)]  
 ...
 [!code-sql[选择 #SelectExamples11](../../t-sql/queries/codesnippet/tsql/select-examples-transact_11.sql)]  
  
 此示例使用两个相关子查询查找售出过某种特定产品的雇员的姓名。  
  
 [!code-sql[选择 #SelectExamples12](../../t-sql/queries/codesnippet/tsql/select-examples-transact_12.sql)]  
  
## <a name="f-using-group-by"></a>F. 使用 GROUP BY  
 以下示例查找数据库中各销售订单的总额。  
  
 [!code-sql[选择 #SelectExamples13](../../t-sql/queries/codesnippet/tsql/select-examples-transact_13.sql)]  
...
## <a name="g-using-group-by-with-multiple-groups"></a>G. 对多个组使用 GROUP BY  
 ...
 [!code-sql[选择 #SelectExamples14](../../t-sql/queries/codesnippet/tsql/select-examples-transact_14.sql)]  
  
## <a name="h-using-group-by-and-where"></a>H. 使用 GROUP BY 和 WHERE  
 以下示例在只检索标价大于 `$1000` 的行后，将结果进行分组。  
  
 [!code-sql[选择 #SelectExamples15](../../t-sql/queries/codesnippet/tsql/select-examples-transact_15.sql)]  
  
## <a name="i-using-group-by-with-an-expression"></a>I. 将 GROUP BY 与表达式一起使用  
 以下示例按表达式进行分组。 如果表达式不包含聚合函数，则可以按表达式进行分组。  
  
 [!code-sql[选择 #SelectExamples16](../../t-sql/queries/codesnippet/tsql/select-examples-transact_16.sql)]  
  
## <a name="j-using-group-by-with-order-by"></a>J. 将 GROUP BY 与 ORDER BY 一起使用  
 以下示例查找每种产品的平均价格并按平均价格将结果排序。  
  
 [!code-sql[选择 #SelectExamples18](../../t-sql/queries/codesnippet/tsql/select-examples-transact_17.sql)]  
...
## <a name="k-using-the-having-clause"></a>K. 使用 HAVING 子句  
 ...
 [!code-sql[选择 #SelectExamples19](../../t-sql/queries/codesnippet/tsql/select-examples-transact_18.sql)]  
...
## <a name="l-using-having-and-group-by"></a>L. 使用 HAVING 和 GROUP BY  
 ...
 [!code-sql[选择 #SelectExamples21](../../t-sql/queries/codesnippet/tsql/select-examples-transact_19.sql)]  
...
## <a name="m-using-having-with-sum-and-avg"></a>M. 将 HAVING 与 SUM 和 AVG 一起使用  
 ...
 [!code-sql[选择 #SelectExamples22](../../t-sql/queries/codesnippet/tsql/select-examples-transact_20.sql)]  
  
 若要查看总销售额超过 `$2000000.00` 的产品，请使用以下查询：  
  
 [!code-sql[选择 #SelectExamples23](../../t-sql/queries/codesnippet/tsql/select-examples-transact_21.sql)]  
  
 若要确保在计算中至少为每种产品包含 1500 项，请使用 `HAVING COUNT(*) > 1500` 消除返回总数少于 `1500` 售出项的产品。 该查询如下所示：  
  
 [!code-sql[选择 #SelectExamples24](../../t-sql/queries/codesnippet/tsql/select-examples-transact_22.sql)]  
...
## <a name="n-using-the-index-optimizer-hint"></a>N. 使用 INDEX 优化器提示  
 ...
 [!code-sql[选择 #SelectExamples45](../../t-sql/queries/codesnippet/tsql/select-examples-transact_23.sql)]  
  
## <a name="m-using-option-and-the-group-hints"></a>M. 使用 OPTION 和 GROUP 提示  
 以下示例说明了如何将 `OPTION (GROUP)` 子句与 `GROUP BY` 子句一起使用。  
  
 [!code-sql[选择 #SelectExamples46](../../t-sql/queries/codesnippet/tsql/select-examples-transact_24.sql)]  
  
## <a name="o-using-the-union-query-hint"></a>O. 使用 UNION 查询提示  
 以下示例使用 `MERGE UNION` 查询提示。  
  
 [!code-sql[选择 #SelectExamples47](../../t-sql/queries/codesnippet/tsql/select-examples-transact_25.sql)]  
  
## <a name="p-using-a-simple-union"></a>P. 使用简单 UNION  
 在下面的示例中，结果集同时包含 `ProductModelID` 和 `Name` 表中的 `ProductModel` 与 `Gloves` 列中的内容。  
  
 [!code-sql[选择 #SelectExamples48](../../t-sql/queries/codesnippet/tsql/select-examples-transact_26.sql)]  
...
## <a name="q-using-select-into-with-union"></a>Q. 将 SELECT INTO 与 UNION 一起使用  
 ...
 [!code-sql[选择 #SelectExamples49](../../t-sql/queries/codesnippet/tsql/select-examples-transact_27.sql)]  
...
## <a name="r-using-union-of-two-select-statements-with-order-by"></a>R. 将 ORDER BY 与两个 SELECT 语句的 UNION 一起使用  
 ...
 [!code-sql[选择 #SelectExamples50](../../t-sql/queries/codesnippet/tsql/select-examples-transact_28.sql)]  
...
## <a name="s-using-union-of-three-select-statements-to-show-the-effects-of-all-and-parentheses"></a>S. 使用三个 SELECT 语句的 UNION 来说明 ALL 和括号的作用  
 ...
 [!code-sql[选择 #SelectExamples51](../../t-sql/queries/codesnippet/tsql/select-examples-transact_29.sql)]  
