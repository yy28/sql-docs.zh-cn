...
## <a name="arguments"></a>参数  
 other_geography   
 与对其调用  **的实例进行比较的其他 geography 实例**`STEquals()`。  
...
## <a name="examples"></a>示例  
 下面的示例使用 `geography` 创建两个等效但又不完全相等的 `STGeomFromText()` 实例，并使用 `STEquals()` 测试它们的等效性。 因为 `LINESTRING` 和 `POINT` 包含在 `POLYGON` 中，所以这两个实例相等。  
