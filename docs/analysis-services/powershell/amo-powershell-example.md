---
title: "AMO PowerShell 示例 | Microsoft Docs"
ms.custom: ""
ms.date: "03/01/2017"
...
---
# AMO PowerShell 示例
  下面的 PowerShell 脚本示例调用 Analysis Services Management Object (AMO) 类型，以根据 SQL Server 关系数据库创建表格数据库（兼容性级别 1200）。  
  
 此示例只为了方便演示。 它由于未定义表关系而停止运行，且不包括数据导入。  这让脚本看起来更像是代码片段，而不是可执行的代码。 若要使其可运行，它需要使用其他 Helper 函数和更多命令来弥补缺陷。  
  
## DatabaseGenerate.ps1 示例  
...
## 另请参阅  
 ...
 [Analysis Services 管理对象 (AMO)](../Topic/Analysis%20Services%20Management%20Objects%20\(AMO\).md)   
