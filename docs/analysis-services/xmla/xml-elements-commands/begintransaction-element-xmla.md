 ...
 **BeginTransaction**命令开始在当前会话上的一个活动事务。 如果存在活动事务，则 [!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)] 实例会递增当前会话的事务引用计数。 如果不存在活动事务，该实例将开始新的事务，并将当前会话的引用计数设置为 1。 如果使用显式指定一个活动事务**BeginTransaction**命令，在显式指定事务内执行所有后续命令。  
