 ...
 **备份**命令备份[!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)]数据库中指定[对象](../../../analysis-services/xmla/xml-elements-properties/object-element-xmla.md)到备份文件，还可以选择将远程分区备份到远程备份文件的元素。 如果**对象**元素而不引用的对象[!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)]数据库，就会出错。  
...
|关系 OLAP (ROLAP)|元数据|  
 ...
 期间**备份**命令时，共享的锁置于[!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)]数据库中指定**对象**元素。 **Backup** 命令完成后释放共享锁。  
  
 多个**备份**可以并行情况下，运行命令，如果命令包含在[并行](../../../analysis-services/xmla/xml-elements-properties/parallel-element-xmla.md)的集合[批处理](../../../analysis-services/xmla/xml-elements-commands/batch-element-xmla.md)命令。 **Parallel** 集合可以使数据库同时备份到多个备份文件中。  
  
 有关备份和还原数据库的详细信息，请参阅[备份、 还原和同步数据库&#40;XMLA&#41;](../../../analysis-services/multidimensional-models-scripting-language-assl-xmla/backing-up-restoring-and-synchronizing-databases-xmla.md)。  
