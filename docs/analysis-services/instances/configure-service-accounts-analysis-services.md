...
ms.topic: "article"
helpviewer_keywords: 
  - "安全性 [Analysis Services], 登录帐户"
  - "登录帐户 [Analysis Services]"
  - "帐户 [Analysis Services]"
  - "登录帐户 [Analysis Services], 关于登录帐户"
...
-   运行 compmgmt.msc | **本地用户和组** | **组** | **SQLServerMSASUser$**\<server-name>**$MSSQLSERVER**（用于默认实例）。  
...
3.  若要在 GPEDIT 中添加帐户特权，右键单击“增加进程工作集”，然后选择“属性”。  
...
>  有关与 IIS 配置和 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 相关的文件权限信息，请参阅[在 Internet Information Services (IIS) 8.0 上配置对 Analysis Services 的 HTTP 访问](../../analysis-services/instances/configure http access to analysis services on iis 8.0.md)。  
 ...
 有一个安全组是为你安装的每个实例而创建的。 安全组是以实例命名的，默认实例命名为 **SQLServerMSASUser$MSSQLSERVER** 或者命名实例命名为 **SQLServerMSASUser$**\<servername>$\<instancename>。 该安装程序为此安全组配置执行服务器操作所需的文件权限。 如果你检查 \MSAS13.MSSQLSERVER\OLAP\BIN 目录上的安全权限，则会看到该安全组（而非服务帐户或其 per-service SID）是该目录的权限持有者。  
