---
title: "适用于 Analysis Services 表格数据库和多维数据库的数据库一致性检查器 (DBCC)。 | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "03/04/2017"
...
manager: "erikre"
---
# 适用于 Analysis Services 表格数据库和多维数据库的数据库一致性检查器 (DBCC)。
...
-   位于兼容级别 1200 的新表格模型数据库的元数据包括 **TableName** 和 **PartitionName**等描述符。  
...
## 适用于表格 1200 数据库的命令语法  
 位于 1200 兼容级别的表格数据库使用对象定义的表格元数据。 以下示例演示了在 SQL Server 2016 功能级别创建的表格数据库的完整 DBCC 语法。  
  
 两种语法之间的主要差别包括更新的 XMLA 命名空间、没有 \<Object> 元素、没有 \<Model> 元素（每个数据库仍只有一个模型）。  
...
### 在 Management Studio 中运行 DBCC 命令  
...
### 在 SQL Server Profiler 2016 中跟踪 DBCC 输出  
...
4.  在 SQL Server Profiler 中，DBCC 活动是通过事件子类为 DBCC 的 **Command** 事件指示的：  
     ...
     状态消息以“正在检查 \<对象> 的一致性”、“已开始检查 \<对象>”或“已完成检查 \<对象>”开头。  
  
    > [!NOTE]  
    >  在 CTP 3.0 中，对象由内部名称标识。 例如，Categories 层次结构写成 H$Categories-\<objectID>。 在以后 CTP 中，内部名称应替换为用户友好的名称。  
  
     下面列出了错误消息。  
  
### 在 SSMS 的 xEvent 会话中跟踪 DBCC 输出  
...
|DBCC 命令|DBCC 命令在处理 DBCC 操作过程中将报告多条状态消息。  在开始之前，它会报告包括数据库、表或对象列名的状态消息，在完成每个对象检查后再次报告一条消息。|正在检查 \<对象名称> \<对象类型> 的一致性。 阶段: 前期检查。<br /><br /> 正在检查 \<对象名称> \<对象类型> 的一致性。 阶段: 后期检查。|  
...
|**元数据管理器出错**<br /><br /> ID 为“\<parentobjectID>”的 \<parentobject> 中不存在 ID 为“\<objectID>”的 \<object>，或用户没有访问该对象的权限。|索引损坏（多维）|重新处理对象和所有依赖对象。|  
|**对分区执行一致性检查期间出错**<br /><br /> 检查 \<database-name> 数据库中 \<cube-name> 多维数据集的 \<measure-group-name> 度量值组的 \<partition-name> 分区一致性时出错。 请重新处理分区或索引以修复损坏。|索引损坏（多维）|重新处理对象和所有依赖对象。|  
...
|**表元数据损坏**<br /><br /> 表 \<table-name> 元数据文件已损坏。 在 DataFileList 节点下找不到主表。|元数据损坏（仅限表格）|删除并重新部署项目，或者从备份还原，然后重新处理。<br /><br /> 有关说明，请参阅博客[如何处理 Analysis Services 数据库中存在损坏](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx)。|  
|**存储层损坏**<br /><br /> 存储层损坏: \<parent-name> \<parent-type> 中的 \<type-name> 集合损坏。|元数据损坏（仅限表格）|删除并重新部署项目，或者从备份还原，然后重新处理。<br /><br /> 有关说明，请参阅博客[如何处理 Analysis Services 数据库中存在损坏](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx)。|  
|**缺少系统表**<br /><br /> 缺少系统表 \<table-name>。|对象损坏（仅限表格）|重新处理对象和所有依赖对象|  
|**表统计信息损坏**<br /><br /> 缺失表系统表 \<table-name> 的统计信息。|元数据损坏（仅限表格）|删除并重新部署项目，或者从备份还原，然后重新处理。<br /><br /> 有关说明，请参阅博客[如何处理 Analysis Services 数据库中存在损坏](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx)。|  
  
## 通过 msmdsrv.ini 配置文件禁用对数据库加载操作的自动一致性检查  
