...
[!INCLUDE[ssas-appliesto-sqlas](../../../includes/ssas-appliesto-sqlas.md)]支持以下的 OLE DB 架构行集[!INCLUDE[msCoName](../../../includes/msconame-md.md)]XML for Analysis (XMLA) 提供。 使用**DISCOVER_ENUMERATORS**行集[发现](../../../analysis-services/xmla/xml-elements-methods-discover.md)方法来检查特定数据源提供程序是否支持行集。  
