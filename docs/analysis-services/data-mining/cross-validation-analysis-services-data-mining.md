...
### 设置分区的数目  
 ...
 ![交叉验证如何对数据进行分段](../../analysis-services/data-mining/media/xvoverviewmain.gif "交叉验证如何对数据进行分段")  
...
### 选择要验证的模型和列  
 ...
 若要选择可预测属性，请单击 **“目标属性”** 并从列表中选择列。 如果目标属性是嵌套列或嵌套表中的列，则必须使用“\<嵌套表名称>(键).\<嵌套列>”格式键入该嵌套列的名称。 如果仅使用嵌套表中的键列，则可以使用“\<嵌套表名称>(键)”。  
...
|主题|链接|  
|------------|-----------|  
|介绍如何在 SQL Server Development Studio 中设置交叉验证参数。|[“交叉验证”选项卡（“挖掘准确性图表”视图）](../Topic/Cross-Validation%20Tab%20\(Mining%20Accuracy%20Chart%20View\).md)|  
