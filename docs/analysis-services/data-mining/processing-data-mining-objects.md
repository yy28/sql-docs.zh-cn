...
-   针对模型创建数据挖掘扩展插件 (DMX) 查询，并钻取到结构中。 有关详细信息，请参阅 [SELECT FROM \<模型> SAMPLE_CASES (DMX)](../Topic/SELECT%20FROM%20%3Cmodel%3E.CASES%20\(DMX\).md)。  
...
-   针对结构事例创建 DMX 查询。 有关详细信息，请参阅 [SELECT FROM \<结构> CASES](../Topic/SELECT%20FROM%20%3Cstructure%3E.CASES.md)。  
