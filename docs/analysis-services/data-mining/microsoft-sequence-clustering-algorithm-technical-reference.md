...
## Microsoft 顺序分析和聚类分析算法的实现  
 ...
 与典型的聚类分析模型相比，顺序分析和聚类分析模型通常会创建更多的分类。 因此，Microsoft 顺序分析和聚类分析算法会执行“分类分解” ，以基于序列属性和其他属性分解分类。  
...
|群集|兴趣性分数|尽管聚类分析算法可能使用离散算法或离散化算法，但每个属性的分数仍将作为间距进行计算，并且是连续的，因此使用兴趣性分数。|  
 ...
 有关详细信息，请参阅 [Feature Selection](../Topic/Feature%20Selection.md)。  
