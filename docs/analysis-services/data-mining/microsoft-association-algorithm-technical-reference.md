...
ms.topic: "article"
helpviewer_keywords: 
  - "MINIMUM_ITEMSET_SIZE 参数"
  - "MAXIMUM_SUPPORT 参数"
  - "关联算法 [Analysis Services]"
  - "MINIMUM_SUPPORT 参数"
  - "OPTIMIZED_PREDICTION_COUNT 参数"
  - "关联 [Analysis Services]"
  - "MAXIMUM_ITEMSET_COUNT 参数"
  - "MAXIMUM_ITEMSET_SIZE 参数"
  - "MINIMUM_PROBABILITY 参数"
...
### 设置算法参数  
 在任何时候均可使用 [!INCLUDE[ssBIDevStudioFull](../../includes/ssbidevstudiofull-md.md)]中的数据挖掘设计器来更改挖掘模型的参数。 也可使用 AMO 中的 <xref:Microsoft.AnalysisServices.MiningModel.AlgorithmParameters%2A> 集合或使用 XMLA 中的 [MiningModels 元素 (ASSL)](../../analysis-services/scripting/collections/miningmodels-element-assl.md) 来通过编程方式更改参数。 下表对各参数进行了说明：  
