---
title: 设置的安全性存储过程 |Microsoft Docs
ms.date: 05/02/2018
ms.prod: sql
ms.technology: analysis-services
ms.custom: olap
ms.topic: conceptual
ms.author: owend
ms.reviewer: owend
author: minewiskan
manager: kfile
ms.openlocfilehash: 12f848571fb5529229d6966ee5b6303a60ea3439
ms.sourcegitcommit: 3026c22b7fba19059a769ea5f367c4f51efaf286
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 06/15/2019
ms.locfileid: "68181073"
---
# <a name="setting-security-for-stored-procedures"></a>设置存储过程的安全性
[!INCLUDE[ssas-appliesto-sqlas](../../includes/ssas-appliesto-sqlas.md)]
  设置存储过程的安全性**PermissionSet**属性的实例对存储过程[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]（服务器级别），[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]数据库，或[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]项目。  
  
## <a name="see-also"></a>请参阅  
 [多维模型程序集管理](../../analysis-services/multidimensional-models/multidimensional-model-assemblies-management.md)   
 [定义存储过程](../../analysis-services/multidimensional-models-extending-olap-stored-procedures/defining-stored-procedures.md)  
  
  
