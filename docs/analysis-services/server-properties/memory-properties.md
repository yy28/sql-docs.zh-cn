...
ms.locfileid: "35239102"
---
# <a name="memory-properties"></a>内存属性
[!INCLUDE[ssas-appliesto-sqlas-all-aas](../../includes/ssas-appliesto-sqlas-all-aas.md)]

  [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 预分配一个适度在启动时的内存量，以便可以立即处理请求。 可随着查询和处理工作负荷增加而分配更多内存。 
...
若要了解有关每个 Analysis Services 实例利用由版本的最大内存的详细信息，请参阅[版本和 SQL Server 支持的功能](../../sql-server/editions-and-components-of-sql-server-2017.md#Cross-BoxScaleLimits)。
 ...
 以下设置适用于这两个表格和多维服务器模式，除非另外说明。  
 
## <a name="default-memory-configuration"></a>默认内存配置

在默认配置下，每个 Analysis Services 实例分配内存 (40 MB 到 50 MB) 的量在启动时，即使实例处于空闲状态。 

请记住，配置设置是基于每个实例的。 如果在相同硬件上运行 Analysis Services 的多个实例（如表格和多维实例），则每个实例都独立于其他实例来分配自己的内存。

下表简要介绍使用频率较高的内存设置（在参考部分中提供了详细信息）。 你应配置这些设置仅当 Analysis Services 争夺与同一服务器上的其他应用程序的内存：
 ...
 **LowMemoryLimit**  
 一个有符号的 64 位双精度浮点数字属性，定义 Analysis Services 开始释放低优先级对象，例如不常使用的缓存内存的第一个阈值。 一旦分配了内存，服务器便不会释放低于此限制的内存。 默认值为 65；这指示最低内存限制为物理内存或虚拟地址空间的 65%（以二者中少者计）。  
 ...
 **HardMemoryLimit**  
 指定一个内存阈值，达到该阈值后实例会主动终止活动用户会话以减少内存的使用量。 所有终止的会话将收到一条有关被取消内存压力错误。 默认值为零 (0)，这意味着 **HardMemoryLimit** 将设置为 **TotalMemoryLimit** 和系统的物理总内存之间的中间值；如果系统的物理内存大于进程的虚拟地址空间，则改用虚拟地址空间来计算 **HardMemoryLimit**。  

**QueryMemoryLimit**   
仅 azure Analysis Services。 要控制在查询过程的临时结果都可以使用的内存量的高级的属性。 仅适用于 DAX 度量值和查询。 对多维模式服务器的 MDX 查询不使用此限制。 而不考虑查询所使用的常规内存分配。 中指定的百分比。 默认值为 0 表示无限制。
...
-   没有足够的内存来用于所有字典。 在处理期间，server 会为每个列的字典锁在内存中，和所有这些不能为指定的值超过**VertiPaqMemoryLimit**。  
