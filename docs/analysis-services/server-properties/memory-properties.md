...
**QueryMemoryLimit**   
仅 azure Analysis Services 中。 高级的属性来控制在查询期间的临时结果可以使用的内存量。 仅适用于 DAX 度量值和查询。 它不考虑查询所使用的常规内存分配。 指定最多 100 个的百分比形式表示。 除此之外，它是以字节为单位。 指定设置值为 0 表示没有限制。 对于 Azure Analysis 的默认值确定由你的计划。 
