...
**QueryMemoryLimit**   
仅 azure Analysis Services 中。 高级的属性来控制在查询期间的临时结果可以使用的内存量。 仅适用于 DAX 度量值和查询。 它不考虑查询所使用的常规内存分配。 指定以百分比表示，是由你的计划确定默认值。 
