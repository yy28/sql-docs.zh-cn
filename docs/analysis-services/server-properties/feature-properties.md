...
[!INCLUDE[ssas-appliesto-sqlas](../../includes/ssas-appliesto-sqlas.md)]功能属性与产品功能有关，大多数是高级，包括控制服务器实例之间的链接的属性。  
...
|“属性”|，则“默认”|Description|  
|--------------|-------------|-----------------|  
|**ManagedCodeEnabled**|@shouldalert|布尔值属性，指示是否启用 CLR 存储过程。|  
|**LinkInsideInstanceEnabled**|@shouldalert|布尔值属性，指示是否可在同一个服务器实例内创建链接对象。|  
|**LinkToOtherInstanceEnabled**|0|布尔值属性，指示是否可链接到远程服务器上的对象。|  
|**LinkFromOtherInstanceEnabled**|0|布尔值属性，指示是否可从其他服务器实例链接到对象。|  
|**ConnStringEncryptionEnabled**|@shouldalert|布尔值属性，指示是否在服务器配置文件中对连接字符串加密。|  
|**UseCachedPageAllocators**|0|布尔值属性，指示是否启用缓存页分配器。|  
|**ComUdfEnabled**|0|布尔值属性，指示是否启用定义为 COM 对象的用户定义函数。|  
|**SQMSupportEnabled**|@shouldalert|布尔值属性，指示是否自动将错误和功能使用情况报告发送给 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 。|  
