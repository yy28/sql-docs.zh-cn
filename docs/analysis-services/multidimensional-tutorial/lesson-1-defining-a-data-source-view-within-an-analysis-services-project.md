...
在本任务中，指定**AdventureWorksDW**数据库的数据源作为[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]维度和多维数据集，在后续课程中定义。  
  
[定义数据源视图](lesson-1-3-defining-a-data-source-view.md)  
在本任务中，定义中所选表中的元数据的单个统一的视图**AdventureWorksDW**数据库。  
  
[修改默认表名](lesson-1-4-modifying-default-table-names.md)  
在本任务中，您将修改数据源视图中的表名，以便后续的名称[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]的更加友好的用户定义的对象。  
