...
12. 查看“英语月份名称”属性层次结构的成员。  
    ...
    注意，层次结构成员首先按年份然后按月份的字母顺序排序。 这是因为“数据源”视图中 EnglishCalendarMonth 列的数据类型是字符串列，它基于基础关系数据库中的 nvarchar 数据类型。 有关如何对每年内的月份按时间顺序进行排序的信息，请参阅[根据辅助属性对属性成员进行排序](../analysis-services/sorting-attribute-members-based-on-a-secondary-attribute.md)。  
