---
title: "为 Power View 报表配置表行为属性（SSAS 表格） | Microsoft Docs"
ms.custom: ""
ms.date: "12/09/2016"
...
manager: "erikre"
---
# 为 Power View 报表配置表行为属性（SSAS 表格）
...
-   从矩阵报表中删除多余的小计。 在字段级别进行默认分组会为每个字段创建小计。 如果您只需要在行级别计算的单个小计，则设置行标识符将生成此结果。  
 ...
 您不能为标记为日期表的表设置行标识符。 对于日期表，当您标记表时指定行标识符。 有关详细信息，请参阅[“标记为日期表”对话框 (SSAS)](../Topic/Mark%20as%20Date%20Table%20Dialog%20Box%20\(SSAS\).md)。  
...
### 图像缺失  
 ...
 ![在报表中图像 URL 显示为文本](../../analysis-services/tabular-models/media/ssas-rptprop-noimageurl.gif "在报表中图像 URL 显示为文本")  
...
### 表缺少一个或多个行  
 ...
 ![默认组将 2 合并为 1](../../analysis-services/tabular-models/media/ssas-jonyang-norowid.gif "默认组将 2 合并为 1")  
 ...
 ![包含基于行 ID 的重复项的行数据](../../analysis-services/tabular-models/media/ssas-jonyang.gif "包含基于行 ID 的重复项的行数据")  
 ...
 **之前：默认分组基于图表中的字段**  
 ...
 ![基于字段级别默认分组的图表](../../analysis-services/tabular-models/media/ssas-rptprop-chartfieldgroup.gif "基于字段级别默认分组的图表")  
  
 **之后：基于行标识符进行分组（行标识符变成轴）**  
  
 ![基于行 ID 分组的图表](../../analysis-services/tabular-models/media/ssas-rptprop-chartrowid.gif "基于行 ID 分组的图表")  
