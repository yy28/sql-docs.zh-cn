---
title: 双向交叉筛选器在 Analysis Services 表格模型 |Microsoft Docs
ms.date: 05/07/2018
ms.prod: sql
...
[!INCLUDE[ssas-appliesto-sqlas-aas](../../includes/ssas-appliesto-sqlas-aas.md)]
 ...
 使用双向交叉筛选器可以轻松解决表格和 Power Pivot 开发人员过去经常遇到的多对多查询设计难题。 如果您使用过 DAX 解决方法的多对多关系中表格或 Power Pivot 模型，您可以尝试应用双向筛选器以确定是否能够产生预期的结果。  
...
>  默认情况下，在数据导入期间，将在派生自事实数据表与相关维度表之间的外键和主键关系的多对一配置中为你创建表关系。  
 ...
 让我们尝试从非重复计数添加**DimProducts**到数据透视表。 请注意“产品计数”的重复值。  从表面看，似乎缺少一种表关系，但在我们的模型中可以看到，所有关系都已完全定义并处于活动状态。 在这种情况下，之所以出现重复值，是因为没有针对产品表中的行设置日期筛选器。  
...
## <a name="learn-step-by-step"></a>了解分步  
 可以通过逐步学习本演练来尝试使用双向交叉筛选器。 若要跟着介绍一起操作，你将需要：  
...
## <a name="next-steps"></a>后续步骤  
 了解何时与如何使用双向交叉筛选器来试错，以及双向交叉筛选器在你方案中的工作方式。 有时，您会发现内置行为是不足够，需要故障回复 DAX 计算才能顺利完成工作。 在中**另请参阅**部分中，您将有关此主题查找其他资源的多个链接。  
