...
> [!NOTE]  
>  使用集成工作区模式时，本地 Analysis Services 实例为 64 位，而 SSDT 表格在 Visual Studio 的 32 位环境中运行。 如果正连接到特殊数据源，请确保工作站上同时安装了相应数据提供程序的 32 位和 64 位版本。 64 位 Analysis Services 实例需要 64 位的提供程序，而 SSDT 表格中的表导入向导需要 32 位版本。
...
-   在使用“从 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 导入”项目模板创建新的表格模型项目时，不能从 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 工作簿导入数据。  
  ...
  > [!IMPORTANT]  
>  模型的兼容级别和工作区服务器必须必须相对应。 对于 **SQL Server 2016 RTM (1200)** 兼容级别的表格模型，工作区服务器必须是 [!INCLUDE[ssASCurrent](../../includes/ssascurrent-md.md)] 表格实例。  
