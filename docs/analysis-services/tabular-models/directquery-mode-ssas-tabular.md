...
# DirectQuery 模式（SSAS 表格）
  本主题介绍兼容级别 1200 的 Analysis Services 表格模型的 *DirectQuery 模式*。 你可以为正在 SSDT 中设计的模型打开 DirectQuery 模式；对于已部署的表格模型，你可以在 SSMS 中更改为 DirectQuery 模式。 在选择 DirectQuery 模式之前，务必先了解其优势和限制。
...
|**公式一致性**|在某些情况下，与仅使用关系数据存储的 DirectQuery 模型相比，相同的公式可能在缓存模型中返回不同的结果。 这些差异是由内存中分析引擎和 SQL Server 之间的语义差异造成的。<br /><br /> 有关兼容性问题的完整列表，包括在实时部署模型时可能返回不同结果的函数，请参阅 [DirectQuery 模式下的 DAX 公式兼容性 (SQL Server Analysis Services)](http://msdn.microsoft.com/zh-cn/981b6a68-434d-4db6-964e-d92f8eb3ee3e)。|  
...
## DirectQuery 支持的数据源
兼容级别 1200 的 DirectQuery 表格模型与以下数据源和提供程序兼容：
