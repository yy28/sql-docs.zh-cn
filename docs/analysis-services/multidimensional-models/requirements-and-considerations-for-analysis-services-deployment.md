...
helpviewer_keywords: 
  ...
  - "可伸缩性 [Analysis Services]"
  - "空间 [Analysis Services]"
  - "Analysis Services 部署, 要求"
  - "部署 [Analysis Services], 要求"
  - "磁盘空间 [Analysis Services]"
  - "要求 [Analysis Services]"
  - "处理器 [Analysis Services]"
  - "系统要求 [Analysis Services]"
...
### 当处理结构更改时提供可用性  
 ...
 若要透明处理对源数据的增量更新，请启用主动缓存。 主动缓存将以新的源数据更新多维数据集，而不需要手动处理，并且不会影响多维数据集的可用性。 有关详细信息，请参阅[主动缓存（分区）](../Topic/Proactive%20Caching%20\(Partitions\).md)。  
