...
[!INCLUDE[ssas-appliesto-sqlas](../../includes/ssas-appliesto-sqlas.md)]性能和可用性的解决方案取决于许多因素，包括基础硬件、 服务器部署拓扑、 你的解决方案 （例如，具有分区分布的特征的功能多个服务器或使用 ROLAP 存储需要直接访问关系引擎），服务级别协议和你的数据模型的复杂性。  
