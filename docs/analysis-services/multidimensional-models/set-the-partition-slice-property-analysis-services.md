...
ms.topic: "article"
helpviewer_keywords: 
  - "分区 [Analysis Services], 数据切片"
  - "数据切片 [Analysis Services]"
...
> [!NOTE]  
>  请注意，动态 MDX 函数（如 [Generate (MDX)](../../mdx/generate-mdx.md) 或 [Except (MDX)](../Topic/Except%20\(MDX\).md)）在分区的切片属性中不受支持。 你必须通过使用显式元组或成员引用定义切片。  
>   
