...
使用说明和本主题中提供的链接安装 Analysis Services 教程中使用的数据和项目文件。  如果正在完成多维教程，你仅需要安装的示例项目，如果你想比较替换为你在本教程中创建完全完成的项目。
...
-   Analysis Services  
    ...
    默认情况下，作为表格的实例，你可以通过在服务器中的安装向导的配置页选择多维服务器模式替代情况下安装 Analysis Services 2016 及更高版本。 如果要同时运行两种服务器模式，请在同一台计算机上重新运行 SQL Server 安装程序，以在另一模式中再安装一个 Analysis Services 实例。  
...
## <a name="step-2-download-sql-server-data-tools-for-visual-studio"></a>步骤 2： 下载 SQL Server Data Tools for Visual Studio 
在此版本中，SQL Server Data Tools 和其他 SQL Server 功能将分开下载与安装。 设计器和用于创建 BI 模型和报表的项目模板中包括 SSDT Visual Studio 2015 或作为[Nuget 包](https://marketplace.visualstudio.com/items?itemName=ProBITools.MicrosoftAnalysisServicesModelingProjects)为 Visual Studio 2017。  
  
-   [下载 SQL Server Data Tools](http://go.microsoft.com/fwlink/?LinkID=827542)。 文件将保存到 Downloads 文件夹。 运行安装程序以安装此工具。  
  
## <a name="step-3-install-databases"></a>步骤 3：安装数据库  
Analysis Services 多维模型使用您从关系数据库管理系统导入的事务数据。 为了实现本教程教学目的，您将使用以下关系数据库作为数据源。  
  
-   **2012 或更高版本的 AdventureWorksDW** – 这是一个关系数据仓库的数据库引擎实例上运行。 它提供了将由您在本教程中生成和部署的 Analysis Services 数据库和项目使用的原始数据。  
...
示例项目使用数据源模拟设置，这些设置可指定导入或处理数据所用的安全上下文。 默认情况下，模拟设置指定用于访问数据的 Analysis Services 服务帐户。 若要使用此默认设置，你必须确保在其下运行 Analysis Services 的服务帐户上具有数据读取器权限**AdventureWorksDW2014**数据库。  
...
6.  选中的复选框旁边**AdventureWorksDW2014**数据库。 角色成员身份应自动包括 **db_datareader** 和 **public**。 单击“确定”接受默认值。  
  
## <a name="step-5-install-projects"></a>步骤 5：安装项目  

示例项目才需要与多维建模教程中创建的内容进行比较。 这些不是完成本教程所必需的。
  
1.  下载[adventure 的工作原理的多维-模型-project.zip](https://github.com/Microsoft/sql-server-samples/releases/tag/adventureworks-analysis-services)从 Adventure Works for GitHub 上的 Analysis Services 示例页。  
  
    此项目适用于 SSAS 2014 和更高版本。  
  
2.  将 .zip 文件移到根驱动器下一级的文件夹（例如 C:\Tutorial）。 如果您尝试在 Downloads 文件夹中解压缩这些文件，则此步骤将会缓解有时候会发生的“路径过长”错误。  
  
3.  解压缩示例项目：右键单击该文件，然后选择“全部提取”。 
  
4.  取消对这些文件的只读权限。 右键单击父文件夹，选择**属性**，然后清除的复选框**只读**。 单击 **“确定”**。 将更改应用至此文件夹、子文件夹和文件。  

5.  打开解决方案 (.sln) 文件中的[!INCLUDE[ssBIDevStudioFull](../includes/ssbidevstudiofull-md.md)]。  
  
6.  部署解决方案以验证是否正确设置了数据库权限和服务器位置信息。  
  
    如果 Analysis Services 和数据库引擎是作为默认实例 (MSSQLServer) 安装的，并且所有软件在同一台计算机上运行，则可以单击“生成”菜单上的“部署解决方案”来生成示例项目并将其部署到本地 Analysis Services 实例。 在部署期间，数据将进行处理 （或导入） 从**AdventureWorksDW**本地数据库引擎实例上的数据库。 将在包含从数据库引擎检索的数据的 Analysis Services 实例上创建新的 Analysis Services 数据库。  
