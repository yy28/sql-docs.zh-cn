...
#### 隐藏 Product Category 表和 Product Subcategory 表  
...
有关默认字段集的详细信息，请参阅 SQL Server 联机丛书中的[配置 Power View 报表的默认字段集（SSAS 表格）](../analysis-services/tabular-models/configure-default-field-set-for-power-view-reports-ssas-tabular.md)。  
...
有关表行为属性的详细信息，请参阅 SQL Server 联机丛书中的[为 Power View 报表配置表行为属性（SSAS 表格）](../analysis-services/tabular-models/configure-table-behavior-properties-for-power-view-reports-ssas-tabular.md)。  
...
因为您更改了模型，所以必须重新部署此模型。 必须重复在[第 14 课：部署](../analysis-services/lesson-14-deploy.md)中执行的任务。  
