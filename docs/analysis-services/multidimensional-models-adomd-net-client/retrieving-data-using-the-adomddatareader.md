...
2.  **检索数据。**  
     ...
     ADOMD.NET 命令一运行查询，返回中的结果**Resultset**格式，以表格格式以平展的数据分析规范的 XML 中所述<xref:Microsoft.AnalysisServices.AdomdClient.AdomdDataReader>对象。 在考虑分析数据的可变维数的情况下对该数据进行查询时，表格格式并不常用。  
    ...
    -   调用 <xref:Microsoft.AnalysisServices.AdomdClient.AdomdDataReader.Read%2A> 对象的 <xref:Microsoft.AnalysisServices.AdomdClient.AdomdDataReader> 方法。  
         ...
         <xref:Microsoft.AnalysisServices.AdomdClient.AdomdDataReader.Read%2A> 方法可从查询结果中获取行。 您然后可以将传递的名称或到列的序号引用[项](https://msdn.microsoft.com/en-us/library/ms131793(v=sql.130).aspx)属性来访问返回的行的每一列。 例如，当前行中的第一列名为 ColumnName。 然后，`reader[0].ToString()` 或 `reader["ColumnName"].ToString()` 将返回当前行中第一列的内容。  
