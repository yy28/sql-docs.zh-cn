...
[!INCLUDE[ssas-appliesto-sqlas](../../../includes/ssas-appliesto-sqlas.md)]在表格模型中，表是基数据的表示形式。  
  
## <a name="tables-representation"></a>表表示形式  
  
### <a name="tables-in-amo"></a>AMO 中的表  
 使用 AMO 管理表时，不存在一对一的对象匹配。 在 AMO 中，表由 <xref:Microsoft.AnalysisServices.Dimension> 和 <xref:Microsoft.AnalysisServices.MeasureGroup> 表示。 为了提供度量值组，必须定义 <xref:Microsoft.AnalysisServices.Cube> 来承载度量值组。 为了提供维度、度量值组和多维数据集，必须定义数据源视图对象来保存对数据源的绑定定义。  
...
```  
 ...
 一旦创建并更新了数据源视图，就需要创建多维数据集对象，但直到创建第一个表后才能在服务器更新多维数据集对象。 多维数据集对象不能创建为空。 下面的代码段演示如何创建多维数据集；该代码段假定您具有非空字符串 newCubeName，而且该多维数据集的名称已经过验证，不与其他名称重名。  
...
2.  创建 RowNumber 属性，并将其定义为该维度的键属性。  
  
3.  创建维度属性，并将其标记为与 RowNumber 具有一对多关系。  
...
7.  在该度量值组中创建 AMO 默认度量值对象。 请注意，这是唯一一次需要使用 AMO 度量值对象；表格模型中的计算度量值在 AMO MdxScripts[“MdxScript”] 对象中定义。  
