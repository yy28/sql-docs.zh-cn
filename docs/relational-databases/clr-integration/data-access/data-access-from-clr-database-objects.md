...
> [!NOTE]  
>  默认情况下，不允许用户定义类型方法和用户定义函数执行数据访问。 必须将**SqlMethodAttribute**或**SqlFunctionAttribute**的**DataAccess**属性设置为**dataaccesskind.read** ，才能启用用户定义类型 (UDT) 方法或用户定义函数的只读数据访问。 不允许从 UDT 或用户定义函数执行数据修改操作，否则，将在执行时引发异常。  
 ...
 [从 CLR 数据库对象进行 XML 序列化](https://docs.microsoft.com/dotnet/standard/serialization/introducing-xml-serialization)  
