...
[!INCLUDE[appliesto-ss-asdbmi-xxxx-xxx-md](../../includes/appliesto-ss-asdbmi-xxxx-xxx-md.md)]
 ...
 CLR 和 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 作为运行时环境以不同的方式处理内存、线程和同步。 本文介绍了这两个运行时的集成方式，以便统一管理所有系统资源。 本文还介绍了如何集成 CLR 代码访问安全性（CAS）和 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 安全机制，以便为用户代码提供可靠且安全的执行环境。  
...
###### <a name="scalability-common-threading-scheduling-and-synchronization"></a>可伸缩性：公共线程化、计划和同步  
 ...
 不执行上述任何操作的代码（例如，仅包含计算的紧凑循环）不会自动生成计划程序，这可能导致系统中的其他工作负荷长时间等待。 在这些情况下，由开发人员通过调用 .NET Framework 的 preemtive （）函数，或使用 BeginThreadAffinity （）在任何代码部分中通过（）显式进入模式来显式生成。预计会长时间运行。 下面的代码示例演示如何使用这些方法中的每种方法手动生成。
...
### <a name="programming-model-restrictions"></a>编程模型限制  
 ...
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 还不允许对用**SharedState**、**同步**和**ExternalProcessMgmt**主机保护属性进行批注的 .NET Framework api 进行调用。 这将防止 SAFE 和 EXTERNAL_ACCESS 程序集调用能够启用共享状态、执行同步和影响 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 进程的完整性的任何 API。 有关详细信息，请参阅[CLR 集成编程模型限制](../../relational-databases/clr-integration/database-objects/clr-integration-programming-model-restrictions.md)。  
