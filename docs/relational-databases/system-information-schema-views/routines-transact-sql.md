...
|ROUTINE_SCHEMA|**nvarchar(** 128 **)**|包含此函数的架构的名称。<br /><br /> **\*\* 重要\* \*** 请勿使用 INFORMATION_SCHEMA 视图来确定对象的架构。 查找对象架构的唯一可靠方法是查询 sys.objects 目录视图。|  
...
|DATETIME_PRECISION|**smallint**|如果返回值是类型的一秒的小数部分精度**datetime**。 否则，返回 NULL。|  
