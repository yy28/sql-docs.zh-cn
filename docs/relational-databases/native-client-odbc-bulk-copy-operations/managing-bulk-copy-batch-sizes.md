...
-   如果不指定 TABLOCK，请将批大小限制为小于 1,000 行。  
 ...
 批处理的另一个副作用是将每个批处理视为未完成的结果集，直到它完成**bcp_batch**。 如果在批处理未完成时尝试对某个连接句柄进行任何其他操作，则 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client ODBC 驱动程序将发出错误，其中包含 SQLState = "HY000" 和错误消息字符串：  
