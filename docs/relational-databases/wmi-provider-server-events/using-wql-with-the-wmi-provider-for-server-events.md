...
```  
 ...
 WQL 查询 (`FROM`) 的 `DDL_DATABASE_LEVEL_EVENTS` 子句中的参数可以为可对其创建事件通知的任何有效事件。 `SELECT` 和 `WHERE` 子句中的参数可以指定与某一事件或其父事件关联的任何事件属性。 有关有效事件和事件属性的列表，请参阅[事件通知 （数据库引擎）](http://technet.microsoft.com/library/ms182602.aspx)。  
 ...
 *event_property*  
 事件的属性。 示例包括**PostTime**， **SPID**，并**LoginName**。 查找中列出的每个事件[WMI Provider for Server Events 类和属性](../../relational-databases/wmi-provider-server-events/wmi-provider-for-server-events-classes-and-properties.md)以确定它所持有的属性。 例如，DDL_DATABASE_LEVEL_EVENTS 事件具有**DatabaseName**并**用户名**属性。 它还继承**SQLInstance**， **LoginName**， **PostTime**， **SPID**，并**ComputerName**从其父事件的属性。  
 ...
 *event_type*  
 可针对其创建事件通知的任何事件。 有关可用事件的列表，请参阅[WMI Provider for Server Events 类和属性](http://technet.microsoft.com/library/ms186449.aspx)。 请注意，*事件类型*名称对应于相同*event_type* | *event_group* ，可以手动创建事件通知时指定通过使用 CREATE EVENT NOTIFICATION。 示例*事件类型*包括 CREATE_TABLE、 LOCK_DEADLOCK、 DDL_USER_EVENTS 和 TRC_DATABASE。  
  
> [!NOTE]  
>  执行 DDL 式操作的某些系统存储过程也可以激发事件通知。 测试您的事件通知以确定它们是否响应运行的系统存储过程。 例如，CREATE TYPE 语句和**sp_addtype**存储的过程都将激发针对 CREATE_TYPE 事件创建事件通知。 但是， **sp_rename**存储的过程不会激发任何事件通知。 有关详细信息，请参阅[DDL 事件](../../relational-databases/triggers/ddl-events.md)。  
 ...
 *where_condition*  
 ...
 仅`=`操作数可连同**DatabaseName**， **SchemaName**，和**ObjectName**。 其他表达式不能与这些事件属性一起使用。  
...
```  
 ...
 请注意，并非所有事件均可在任何特定的范围内进行查询。 例如，针对跟踪事件（例如 Lock_Deadlock）或跟踪事件组（例如 TRC_LOCKS）的 WQL 查询仅可在服务器级别进行注册。 与此类似，CREATE_ENDPOINT 事件和 DDL_ENDPOINT_EVENTS 事件组也只能在服务器级别进行注册。 用于注册事件的适当范围的详细信息，请参阅[设计事件通知](http://technet.microsoft.com/library/ms175854\(v=sql.105\).aspx)。 尝试注册 WQL 查询它的*event_type*仅可以注册在服务器级别建立时将始终在服务器级别。 如果 WMI 客户端拥有权限，则注册成功。 否则，将向客户端返回一个错误。 不过，在某些情况下，仍可根据与服务器级别事件对应的属性将 WHERE 子句用作服务器级别事件的筛选器。 例如，许多跟踪事件具有**DatabaseName**可以用作筛选器的 WHERE 子句中的属性。  
  
 在创建服务器范围事件通知**主**数据库，并且可以通过查询元数据[sys.server_event_notifications](../../relational-databases/system-catalog-views/sys-server-event-notifications-transact-sql.md)目录视图。  
  
 数据库范围或对象范围事件通知在指定的数据库中创建并可通过查询元数据[sys.event_notifications](../../relational-databases/system-catalog-views/sys-event-notifications-transact-sql.md)目录视图。 （您必须为目录视图加上对应的数据库名前缀。）  
