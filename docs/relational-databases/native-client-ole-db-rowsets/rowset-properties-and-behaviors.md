...
|DBPROP_LOCKMODE|R/w： 读/写<br /><br /> 默认值：DBPROPVAL_LM_NONE<br /><br /> 说明：由行集执行的锁定级别（DBPROPVAL_LM_NONE、DBPROPVAL_LM_SINGLEROW）。<br /><br /> 注意： 当在事务中，使用快照隔离，如果通过使用键集游标或动态服务器游标打开行集和锁模式设置为 DBPROPVAL_LM_SINGLEROW，将会出错提取行，如果其他人已自该事务更新该行时已启动。 对于其他游标类型和锁定模式，如果在自启动此事务之后其他人已更新了该行，则在该用户尝试更新此行之前，不会发生错误。 在这两种情况下，服务器将生成这些错误。|  
...
|DBPROP_ROWSET_ASYNCH|R/w： 读/写<br /><br /> 默认值： 0<br /><br /> 说明：为异步行集处理而提供。 此属性位于行集属性组和 DBPROPSET_ROWSET 属性集中。 类型为 VT_14。<br /><br /> 支持的位掩码中的唯一值[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Native Client 是**DBPROPVAL_ASYNCH_INITIALIZE**。|  
...
|DBPROP_TRANSACTEDOBJECT|R/w： 只读<br /><br /> 默认值：VARIANT_FALSE<br /><br /> 说明： [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client OLE DB 访问接口只支持事务的对象。 有关详细信息，请参阅[事务](../../relational-databases/native-client-ole-db-transactions/transactions.md)。|  
