...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]TM: Rollback Tran Completed 事件类指示 ROLLBACK TRANSACTION 请求已完成。 该请求是通过事务管理界面从客户端发送的。 EventSubClass 列指示在当前事务回滚后是否启动新事务。  
...
|错误|**int**|给定事件的错误号。 通常是 sys.messages 表中存储的错误号。|31|是|  
|EventClass|**int**|事件类型 = 188。|27|是|  
