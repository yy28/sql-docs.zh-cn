...
-   计划指南定义中的提示（包括 USE PLAN 提示）未成功应用于查询或批处理。 也就是说，编译的查询计划无法遵守指定的提示，并且该计划将在不使用计划指南的情况下进行编译。  
  
 无效的计划指南可能导致此事件激发。 请使用 [sys.fn_validate_plan_guide](../../relational-databases/system-functions/sys-fn-validate-plan-guide-transact-sql.md) 函数来验证查询或批处理所使用的计划指南，然后更正此函数报告的错误。  
  
 此事件包括在 [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)] 优化模板中。  
  
## <a name="plan-guide-unsuccessful-event-class-data-columns"></a>Plan Guide Unsuccessful 事件类的数据列  
...
|DatabaseID|**int**|由 USE *database* 语句指定的数据库的 ID；如果未对指定实例发出 USE *database* 语句，则为默认数据库的 ID。 [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)] 如果在跟踪中捕获 ServerName 数据列而且服务器可用，则将显示数据库名。 可使用 DB_ID 函数来确定数据库的值。|3|用户帐户控制|  
|DatabaseName|**nvarchar**|正在其中运行用户语句的数据库的名称。|35|用户帐户控制|  
|EventClass|**int**|事件类型 = 218。|27|否|  
...
|LoginSid|**图像**|登录用户的安全标识号 (SID)。 你可以在 [sys.server_principals](../../relational-databases/system-catalog-views/sys-server-principals-transact-sql.md) 或 [sys.sql_logins](../../relational-databases/system-catalog-views/sys-sql-logins-transact-sql.md) 目录视图中找到此信息。 服务器中的每个登录名都具有唯一的 SID。|41|用户帐户控制|  
|NTDomainName|**nvarchar**|用户所属的 Windows 域。|7|用户帐户控制|  
|NTUserName|**nvarchar**|Windows 用户名。|6|用户帐户控制|  
|ObjectID|**int**|应用计划指南时正在编译的模块的对象 ID。 如果计划指南未应用到模块，则此列将设置为 NULL。|22|用户帐户控制|  
...
## <a name="see-also"></a>另请参阅  
 [Plan Guide Successful 事件类](../../relational-databases/event-classes/plan-guide-successful-event-class.md)   
