...
|数据列|类型|描述|列号|可筛选|  
|-----------------|----------|-----------------|-------------------|----------------|  
|**应用程序名称**|**nvarchar**|客户端应用程序的名称，该客户端应用程序创建了指向 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]实例的连接。 此列由应用程序传递的值填充，而不是由所显示的程序名填充。|10|用户帐户控制|  
...
|**EventSubClass**|**nvarchar**|指明无法送达消息是否为排序的消息。 两个值之一：<br /><br /> **已编序的消息**。 无法送达消息是排序的消息。<br /><br /> **未编序的消息**。 无法送达消息不是排序的消息。|21|用户帐户控制|  
...
|**TextData**|**ntext**|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 无法送达消息的原因。|@shouldalert|用户帐户控制|  
