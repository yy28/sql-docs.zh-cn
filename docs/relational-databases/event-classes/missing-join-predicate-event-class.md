...
---
# Missing Join Predicate 事件类
  Missing Join Predicate 事件类表示正在执行无联接谓词的查询。 这可能导致长时间运行查询。 有关联接谓词的详细信息，请参阅[谓词 (Transact-SQL)](../Topic/Predicates%20\(Transact-SQL\).md)。  
  
## Missing Join Predicate 事件类的数据列  
