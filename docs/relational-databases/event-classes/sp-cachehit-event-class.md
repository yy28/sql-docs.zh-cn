...
|EventSubClass|**int**|1 = 执行上下文命中：在计划缓存中找到了可用执行计划。<br /><br /> 2 = Compplan 命中：在计划缓存中找到了编写计划。|21|用户帐户控制|  
...
|TextData|**ntext**|在缓存中找到的 SQL 代码的文本。 如果填充了 TextData，则不会填充 ObjectName。|@shouldalert|用户帐户控制|  
