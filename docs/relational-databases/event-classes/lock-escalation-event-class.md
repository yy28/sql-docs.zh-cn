...
|**DatabaseName**|**nvarchar**|发生升级的数据库的名称。|35|用户帐户控制|  
|**EventClass**|**int**|事件类型 = 60。|27|否|  
|**EventSubClass**|**int**|锁升级的原因：<br /><br /> **0 - LOCK_THRESHOLD** 指示语句超出了锁阈值。<br /><br /> **1 - MEMORY_THRESHOLD** 指示语句超出了内存阈值。|21|用户帐户控制|  
...
|**IntegerData**|**int**|HoBT 锁的计数。 锁升级时 HoBT 锁的数目。|25|用户帐户控制|  
|**IntegerData2**|**int**|已升级的锁的计数。 已转换的锁的总数。 因为这些锁结构已计入升级的锁中，所以释放这些锁结构。|55|用户帐户控制|  
|**IsSystem**|**int**|指示事件是发生在系统进程中还是发生在用户进程中。 1 = 系统，0 = 用户。|60|用户帐户控制|  
|**LineNumber**|**int**|[!INCLUDE[tsql](../../includes/tsql-md.md)] 语句的行号。|5|用户帐户控制|  
...
|**Exchange Spill**|**int**|为其触发了锁升级的表的系统分配 ID。|22|用户帐户控制|  
|**ObjectID2**|**bigint**|相关对象或实体的 ID。 （触发了锁升级的 HoBT ID。）|56|用户帐户控制|  
|**Offset**|**int**|[!INCLUDE[tsql](../../includes/tsql-md.md)] 语句的起始偏移量。|61|用户帐户控制|  
|**OwnerID**|**int**|1 = TRANSACTION<br /><br /> 2 = CURSOR<br /><br /> 3 = SESSION<br /><br /> 4 = SHARED_TRANSACTION_WORKSPACE<br /><br /> 5 = EXCLUSIVE_TRANSACTION_WORKSPACE<br /><br /> 6 = WAITFOR_QUERY|58|用户帐户控制|  
...
|**TextData**|**ntext**|导致锁升级的 [!INCLUDE[tsql](../../includes/tsql-md.md)] 语句的文本。|@shouldalert|用户帐户控制|  
|**TransactionID**|**bigint**|系统分配的事务 ID。|4|用户帐户控制|  
|**类型**|**int**|锁升级粒度：<br /><br /> 1 = NULL_RESOURCE<br /><br /> 2 = DATABASE<br /><br /> 3 = FILE<br /><br /> 5 = OBJECT（表级别）<br /><br /> 6 = PAGE<br /><br /> 7 = KEY<br /><br /> 8 = EXTENT<br /><br /> 9 = RID<br /><br /> 10 = APPLICATION<br /><br /> 11 = METADATA<br /><br /> 12 = HOBT<br /><br /> 13 = ALLOCATION_UNIT|57|用户帐户控制|  
