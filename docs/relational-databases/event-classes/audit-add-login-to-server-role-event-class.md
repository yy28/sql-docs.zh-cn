...
---
# <a name="audit-add-login-to-server-role-event-class"></a>Audit Add Login to Server Role 事件类
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]每当向固定服务器角色添加登录名或从其删除登录名时，都会发生 Audit Add Login to Server Role 事件类。 此事件类用于 **sp_addsrvrolemember** 和 **sp_dropsrvrolemember** 存储过程。  
  
## <a name="audit-add-login-to-server-role-event-class-data-columns"></a>Audit Add Login to Server Role 事件类的数据列  
...
|**DBUserName**|**nvarchar**|添加或删除登录名的数据库用户名。|40|是|  
|**EventClass**|**int**|事件类型 = 108。|27|是|  
...
|**RoleName**|**nvarchar**|成员身份正在修改的固定服务器角色的名称。|38|是|  
