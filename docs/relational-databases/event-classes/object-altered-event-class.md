...
|IntegerData|**int**|对应的“开始”事件的事件序列号。 此列只用于“提交”或“回滚”类型的事件类型。 子类。|25|用户帐户控制|  
...
|ObjectID2|**bigint**|分区方案更改时的分区函数 ID、服务更改时的队列 ID 或 XML 架构更改时的集合架构 ID。|56|用户帐户控制|  
...
|RequestID|**int**|包含该语句的批处理请求的 ID。|49|用户帐户控制|  
