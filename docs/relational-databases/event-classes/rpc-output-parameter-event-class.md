...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]RPC Output Parameter 事件类跟踪远程过程调用 (RPC) 在执行之后的输出参数值。  
  
 此类用于检查存储过程返回的输出值。 例如，如果应用程序在执行远程调用之后没有生成预期的输出值，可以使用此事件类来帮助找出客户端代码和服务器代码之间的问题。  
  
## <a name="rpc-output-parameter-event-class-data-columns"></a>RPC Output Parameter 事件类的数据列  
...
|ObjectName|**nvarchar**|被引用参数的名称。|34|是|  
