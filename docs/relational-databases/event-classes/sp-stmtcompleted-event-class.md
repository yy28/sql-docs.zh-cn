...
|Reads|**bigint**|服务器代表事件所执行的逻辑磁盘读取次数。|16|用户帐户控制|  
|RequestID|**int**|包含该语句的请求的 ID。|49|用户帐户控制|  
|RowCounts|**bigint**|受事件影响的行数。|48|用户帐户控制|  
...
|SourceDatabaseID|**int**|对象所在的数据库的 ID。|62|用户帐户控制|  
...
|Writes|**bigint**|服务器代表事件所执行的物理磁盘写入次数。|17|用户帐户控制|  
