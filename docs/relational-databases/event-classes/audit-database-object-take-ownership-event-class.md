...
|**ObjectType**|**int**|表示事件中涉及的对象类型的值。 为此列返回的值结合了 **sys.objects** 目录视图中的 **type** 列中的对应值以及 [ObjectType Trace Event Column](../../relational-databases/event-classes/objecttype-trace-event-column.md)中列出的值。 例如，如果返回 8277-U，则对象类型为用户定义表。|28|用户帐户控制|  
