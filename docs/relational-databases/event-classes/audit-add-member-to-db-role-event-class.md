...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]在数据库角色中添加或删除登录名时，会发生 Audit Add Member to DB Role 事件类。 此事件类和 **sp_addrolemember**、 **sp_changegroup**以及 **sp_droprolemember** 存储过程一起使用。  
...
|**DatabaseName**|**nvarchar**|已修改角色成员身份的数据库的名称。|35|是|  
|**DBUserName**|**nvarchar**|颁发者在数据库中的用户名。|40|是|  
|**EventClass**|**int**|事件类型 = 110。|27|是|  
|**EventSequence**|**int**|给定事件在请求中的顺序。|51|是|  
|**EventSubClass**|**int**|事件子类的类型。<br /><br /> 1 = 添加<br /><br /> 2 = 删除<br /><br /> 3 = 更改组|21|是|  
...
|**TargetUserName**|**nvarchar**|正在修改其角色成员身份的用户的名称。|39|是|  
