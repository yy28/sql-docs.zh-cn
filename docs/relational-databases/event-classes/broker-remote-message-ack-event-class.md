...
## Broker:Remote Message Ack 事件类的数据列  
...
|**IntegerData**|**int**|包含确认的消息的片段号。|25|是|  
|**IntegerData2**|**int**|正在进行确认的消息的片段号。|55|是|  
...
|**RoleName**|**nvarchar**|正在确认消息的实例的角色。 这可以是 **initiator** 或 **target**。|38|是|  
