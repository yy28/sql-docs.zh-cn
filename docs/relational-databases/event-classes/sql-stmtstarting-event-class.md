...
## <a name="sqlstmtstarting-event-class-data-columns"></a>SQL:StmtStarting 事件类的数据列  
...
|NestLevel|**int**|如果曾在存储过程中运行 SQL 语句，则为存储过程的嵌套级别。|29|用户帐户控制|  
...
|State|**int**|指示在重新编译后是否执行语句。<br /><br /> 1 = 已重新编译|30|用户帐户控制|  
|TextData|**ntext**|将要执行的语句文本。|@shouldalert|用户帐户控制|  
