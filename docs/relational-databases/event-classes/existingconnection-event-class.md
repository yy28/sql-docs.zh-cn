...
|BinaryData|**图像**|二进制转储的选项标记（例如会话级别设置），包括 ANSI NULLS、ANSI Padding、cursor close on commit、Null 串联和带引号的标识符。|2|用户帐户控制|  
|ClientProcessID|**int**|主机为运行该客户端应用程序的进程分配的 ID。 如果客户端提供了客户端进程 ID，则填充此数据列。|9|用户帐户控制|  
|DatabaseID|**int**|用户连接的当前数据库 ID。 由 USE *database* 语句指定的数据库的 ID；如果未对给定实例发出 USE *database* 语句，则为默认数据库的 ID。 可使用 DB_ID 函数来确定数据库的值。|3|用户帐户控制|  
|DatabaseName|**nvarchar**|正在其中运行用户语句的数据库的名称。|35|用户帐户控制|  
|EventClass|**int**|事件类型 = 17。|27|否|  
|EventSequence|**int**|此事件在跟踪中的顺序。|51|否|  
...
|IntegerData|**int**|连接中使用的网络数据包大小。|25|用户帐户控制|  
|IsSystem|**int**|指示事件是发生在系统进程中还是发生在用户进程中。 1 = 系统，NULL = 用户。 对于此事件，始终为 NULL。|60|用户帐户控制|  
...
|StartTime|**datetime**|用户打开连接的时间（登录时间）。|14|用户帐户控制|  
|TextData|**ntext**|设置与连接相关的选项。|@shouldalert|用户帐户控制|  
