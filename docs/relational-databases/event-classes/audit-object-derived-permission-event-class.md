...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)] 当为某个指定的对象发出 CREATE、ALTER 或 DROP 命名时，会记录 Audit Object Derived Permission 事件类。 只有在对象没有与其直接关联的权限或所有者时，才会发生此事件。  
