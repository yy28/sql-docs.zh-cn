...
ms.locfileid: "33263030"
---
# <a name="system-base-tables"></a>系统基表
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-xxx-md](../../includes/tsql-appliesto-ss2008-xxxx-xxxx-xxx-md.md)]

  系统基表是基础表，用于实际存储特定数据库的元数据。 **Master**数据库是特殊这个方面来说，因为它包含在任何其他数据库中找不到某些其他表。 这些表包含服务器范围内的持久化元数据。  
...
## <a name="system-base-table-metadata"></a>系统基表元数据  
 已在数据库的控件、 ALTER 或 VIEW DEFINITION 权限被授权者可以查看系统基表中的元数据**sys.objects**目录视图。 被授权者还可以解析的名称，然后使用内置函数类似于对象 Id 的系统基表[OBJECT_NAME](../../t-sql/functions/object-name-transact-sql.md)和[OBJECT_ID](../../t-sql/functions/object-id-transact-sql.md)。  
...
|**sys.sysidxstats**|存在于每个数据库中。 表和索引视图的每个索引或统计信息均存在对应的一行<br /><br /> 注意: （除外堆） 的每个索引都与具有相同的名称作为索引的统计信息关联。|  
|**sys.sysiscols**|存在于每个数据库中。 每个持久化索引和统计信息列均存在对应的一行。|  
|**sys.sysscalartypes**|存在于每个数据库中。 每个用户定义类型或系统类型均存在对应的一行。|  
|**sys.sysdbreg**|中存在**master**仅数据库。 每个注册数据库均存在对应的一行。|  
|**sys.sysxsrvs**|中存在**master**仅数据库。 每个本地服务器、链接服务器或远程服务器均存在对应的一行。|  
|**sys.sysrmtlgns**|此系统的基础表中存在**master**仅数据库。 每个远程登录映射均存在对应的一行。 这用于将声明来自对应服务器的传入登录映射到实际本地登录。|  
|**sys.syslnklgns**|中存在**master**仅数据库。 每个链接登录映射均存在对应的一行。 链接登录映射由远程过程调用和分布式查询使用，从本地服务器发出到对应的链接服务器。|  
|**sys.sysxlgns**|中存在**master**仅数据库。 每个服务器主体均存在对应的一行。|  
|**sys.sysdbfiles**|存在于每个数据库中。 如果列**dbid**为零，行表示一个属于此数据库的文件。 在**master**数据库列**dbid**可以是非零值。 如果是这样，该行表示主文件。|  
|**sys.sysusermsg**|中存在**master**仅数据库。 每一行表示用户定义的错误消息。|  
|**sys.sysprivs**|存在于每个数据库中。 每个数据库或服务器级权限均存在对应的一行。<br /><br /> 注意： 将服务器级权限存储在**master**数据库。|  
...
|**sys.syslogshippers**|中存在**master**仅数据库。 每个数据库镜像见证服务器均存在对应的一行。|  
...
|**sys.sysendpts**|中存在**master**仅数据库。 服务器中创建的每个端点均存在对应的一行。|  
|**sys.syswebmethods**|中存在**master**仅数据库。 对服务器中创建的启用 SOAP 的 HTTP 端点定义的每个 SOAP 方法均存在对应的一行。|  
