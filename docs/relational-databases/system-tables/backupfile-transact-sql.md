...
|**file_number**|**numeric （10，0）**|数据库中的唯一文件标识号（对应于**sys. database_files**。**file_id**）。|  
...
|**状态**|**tinyint**|文件的状态，可以是下列值之一：<br /><br /> 0 = ONLINE <br /><br /> 1 = RESTORING<br /><br /> 2 = RECOVERING<br /><br /> 3 = RECOVERY PENDING <br /><br /> 4 = SUSPECT <br /><br /> 6 = OFFLINE <br /><br /> 7 = DEFUNCT<br /><br /> 8 = 删除<br /><br /> 注意：将跳过值5，以便这些值与数据库状态的值相对应。|  
...
|**create_lsn**|**数值（25，0）**|创建文件时的日志序列号。|  
|**drop_lsn**|**数值（25，0）**|删除文件时的日志序列号。 可以为 NULL。<br /><br /> 如果文件尚未删除，该值为 NULL。|  
|**file_guid**|**uniqueidentifier**|文件的唯一标识符。|  
|**read_only_lsn**|**数值（25，0）**|包含该文件的文件组从读写属性更改为只读属性（最新更改）时的日志序列号。 可以为 NULL。|  
|**read_write_lsn**|**数值（25，0）**|包含该文件的文件组从只读属性更改为读写属性（最新更改）时的日志序列号。 可以为 NULL。|  
|**differential_base_lsn**|**数值（25，0）**|差异备份的基准 LSN。 差异备份仅包括日志序列号等于或大于**differential_base_lsn**的数据区。<br /><br /> 对于其他备份类型，该值为 NULL。|  
