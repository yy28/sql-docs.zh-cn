...
|**状态值**|**tinyint**|状态：<br /><br /> **0** = 非活动。<br /><br /> **1** = 活动。|  
...
|**queue_type**|**int**|指定所使用的队列类型。 可以是以下值之一：<br /><br /> **1** = msmq，使用[!INCLUDE[msCoName](../../includes/msconame-md.md)]消息队列存储事务。<br /><br /> **2** = 使用[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]来存储事务的 sql。<br /><br /> 注意：使用[!INCLUDE[msCoName](../../includes/msconame-md.md)]消息队列已弃用，不再可用。|  
