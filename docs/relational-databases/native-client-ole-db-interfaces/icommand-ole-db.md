...
## <a name="icommandexecute"></a>ICommand::Execute  
 如果插入的数据大于列的大小，通常会导致错误。 但是，可能会出现将返回 S_OK、但 dwStatus 将设置为 DBSTATUS_S_TRUNCATED 的情况**。 通常，当使用参数插入数据时，通常会出现这种情况，其中列不够大，无法保存数据，并且尚未调用**ICommand 与参数：：：set参数Info。**  
