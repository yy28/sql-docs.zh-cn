...
## <a name="remarks"></a>备注  
 ...
 BCPReadFmt 方法在读取格式值之后，会相应调用 **IBCPSession::BCPColumns** 和 [IBCPSession::BCPColFmt](../../relational-databases/native-client-ole-db-interfaces/ibcpsession-bcpcolumns-ole-db.md) 方法[](../../relational-databases/native-client-ole-db-interfaces/ibcpsession-bcpcolfmt-ole-db.md)。 用户不必分析格式化文件并发出上述调用。  
 ...
 [IBCPSession：： BCPControl](../../relational-databases/native-client-ole-db-interfaces/ibcpsession-bcpcontrol-ole-db.md)的*eOption*参数的**BCP_OPTION_DELAYREADFMT**值将修改 IBCPSession：： BCPReadFmt 的行为。  
