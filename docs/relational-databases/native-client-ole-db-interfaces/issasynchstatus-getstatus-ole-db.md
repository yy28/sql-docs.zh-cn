...
## <a name="remarks"></a>备注  
 ISSAsynchStatus::GetStatus 方法的行为与 IDBAsynchStatus::GetStatus 方法完全类似，不同之处在于如果中止对数据源对象的初始化，前者将返回 E_UNEXPECTED，而不是 DB_E_CANCELED（但是 **ISSAsynchStatus::WaitForAsynchCompletion** 将返回 DB_E_CANCELED）****[](../../relational-databases/native-client-ole-db-interfaces/issasynchstatus-waitforasynchcompletion-ole-db.md)。 这是因为在中止后，数据源对象不会照常处于僵停状态，以便进一步尝试初始化操作。  
