...
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]表值函数是用户定义的函数，返回一个表。  
...
> [!NOTE]  
>  对于表值函数，返回的表类型的列不能包含时间戳列或非 Unicode 字符串数据类型列 (如**char**， **varchar**，和**文本**). 不支持 NOT NULL 约束。  
...
### <a name="defining-a-table-valued-function-in-transact-sql"></a>在 Transact-SQL 中定义表值函数  
 用于定义 CLR 表值函数的语法是类似于[!INCLUDE[tsql](../../includes/tsql-md.md)]表值函数，通过添加**外部名称**子句。 例如：  
 ...
 **请注意**表值函数只能执行数据访问通过[!INCLUDE[tsql](../../includes/tsql-md.md)]查询中的**InitMethod**方法，而不是在**FillRow**方法。 **InitMethod**应标记为**SqlFunction.DataAccess.Read**特性属性，如果[!INCLUDE[tsql](../../includes/tsql-md.md)]执行查询。  
