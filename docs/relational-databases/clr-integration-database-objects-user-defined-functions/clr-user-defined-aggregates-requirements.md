...
|**终止**|`public return_type Terminate();`|此方法完成聚合计算并返回聚合的结果。 *Return_type*应是托管[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]数据，它是类型的托管的等价物*return_sqltype*中指定**CREATE AGGREGATE**语句。 *Return_type*也可以是用户定义类型。|  
