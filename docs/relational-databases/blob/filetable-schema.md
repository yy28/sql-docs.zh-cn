...
description: 了解 Filetable 的预定义固定架构，Filetable 是一项使用目录结构存储文件的 SQL Server 功能。
ms.custom: ''
ms.date: 03/01/2017
ms.prod: sql
...
|**file_stream**|**varbinary(max)**<br /><br /> **文件流 (filestream)**|可变|Null|包含 FILESTREAM 数据。|不适用。|  
|**file_type**|**nvarchar(255)**|可变|NULL。<br /><br /> 文件系统中的创建或重命名操作将从名称填充文件扩展名值。|表示文件的类型。<br /><br /> 在你创建全文索引时，可将此列用作 **TYPE COLUMN** 。<br /><br /> **file_type** 是持久化计算列。|自动计算。 无法设置。|  
