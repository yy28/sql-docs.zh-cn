...
#### Parameters  
 *FilestreamPath*  
 [in] 是由 [PathName](../../relational-databases/system-functions/pathname-transact-sql.md) 函数返回的 **nvarchar(max)** 路径。 必须从具有 FILESTREAM 表和列的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] SELECT 或 UPDATE 权限的帐户的上下文中调用 PathName。  
