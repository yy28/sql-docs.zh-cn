...
ms.locfileid: "73787105"
---
# <a name="sqldescribeparam"></a>SQLDescribeParam
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]

  若要描述任何 SQL 语句的参数，[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client ODBC 驱动程序在已准备好的 ODBC 语句句柄上调用 SQLDescribeParam 时生成并执行 [!INCLUDE[tsql](../../includes/tsql-md.md)] SELECT 语句。 结果集的元数据确定预定义语句中的参数的特征。 SQLDescribeParam 可以返回 SQLExecute 或 SQLExecDirect 可能返回的任何错误代码。  
...
-   SQLExecDirect 之后，对包含 FROM 子句的任何 [!INCLUDE[tsql](../../includes/tsql-md.md)] UPDATE 或 DELETE 语句执行。  
...
-   如果在 [!INCLUDE[tsql](../../includes/tsql-md.md)] 命令中有注释（/* \*/），则为。  
  
 处理一批 [!INCLUDE[tsql](../../includes/tsql-md.md)] 语句时，驱动程序也不支持在批处理中第一个语句后的语句中的参数标记 SQLDescribeParam 调用。  
...
## <a name="sqldescribeparam-and-table-valued-parameters"></a>SQLDescribeParam 和表值函数  
 ...
 有关通常情况下的表值参数的详细信息，请参阅[表值&#40;参数&#41;ODBC](../../relational-databases/native-client-odbc-table-valued-parameters/table-valued-parameters-odbc.md)。  
 ...
 **SQLDescribeParam**支持大型 CLR 用户定义类型（udt）。 有关详细信息，请参阅[大型 CLR 用户定义类型&#40;ODBC&#41;](../../relational-databases/native-client/odbc/large-clr-user-defined-types-odbc.md)。  
