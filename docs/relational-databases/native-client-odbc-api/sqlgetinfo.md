...
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]
 ...
 本机客户端中的[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] **SQLGetInfo**与 ODBC[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]驱动程序 （SQLSRV32） 中的**SQLGetInfo**不同。使用SQL_KEYWORDS和 0 缓冲区长度调用**SQLGetInfo**时 DLL）。  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client 驱动程序返回 SQL_SUCCESS，而 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ODBC 驱动程序返回 SQL_SUCCESS_WITH_INFO。  但是，当调用非零缓冲区长度小于输出关键字字符串时，本机客户端中的[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] **SQLGetInfo**将返回SQL_SUCCESS_WITH_INFO和 SQLState 01004。  
  
|fInfoType|rgbInfoValue|  
|---------------|------------------|  
|SQL_ACCESSIBLE_PROCEDURES|"Y"|  
|SQL_ACCESSIBLE_TABLES|"Y"|  
|SQL_ACTIVE_CONNECTIONS|连接数受 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 限制。 驱动程序返回 0 此**SQLGetInfo**请求。|  
|SQL_ACTIVE_ENVIRONMENTS|环境数不受驱动程序限制。 驱动程序返回 0 此**SQLGetInfo**请求。|  
|SQL_ACTIVE_STATEMENTS|驱动程序返回 1 此**SQLGetInfo**请求。 可用于应用程序的语句句柄数不受驱动程序限制，但默认执行某个语句句柄将阻止执行其他任何句柄。|  
...
|SQL_CONVERT_BIGINT|不支持转换 ODBC SQL_BIGINT 数据类型。 本机[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]客户端 ODBC 驱动程序支持[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]**十进制 （19，0）** 数据类型作为 ODBC 类型SQL_DECIMAL。 请参阅下文中的 SQL_CONVERT_DECIMAL。|  
...
|SQL_CONVERT_DATE|不支持转换 ODBC SQL_TYPE_DATE 数据类型。 本机[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]客户端 ODBC 驱动程序支持[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]**日期时间**数据类型，作为 ODBC 类型SQL_TYPE_TIMESTAMP。 请参阅下文中的 SQL_CONVERT_TIMESTAMP。|  
...
|SQL_CONVERT_DOUBLE|不支持转换 ODBC SQL_DOUBLE 数据类型。 本机[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]客户端 ODBC 驱动程序支持 ODBC SQL_DOUBLE数据类型，如SQL_FLOAT。 请参阅下文中的 SQL_CONVERT_FLOAT。|  
...
|SQL_CONVERT_TIME|不支持转换 ODBC SQL_TYPE_TIME 数据类型。 本机[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]客户端 ODBC 驱动程序支持[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]**日期时间**数据类型，作为 ODBC 类型SQL_TYPE_TIMESTAMP。 请参阅下文中的 SQL_CONVERT_TIMESTAMP。|  
...
|SQL_DATA_SOURCE_NAME|当前数据源的名称。 如果连接未指定数据源名称，则将*StringLengthPtr*指向的值集为 0。|  
...
|SQL_IDENTIFIER_CASE|SQL_IC_MIXED如果连接到运行大小写排序顺序的服务器。<br /><br /> SQL_IC_SENSITIVE如果连接到运行区分大小写排序顺序的服务器。|  
|SQL_IDENTIFIER_QUOTE_CHAR|"（双引号）|  
|SQL_INDEX_KEYWORDS|SQL_IK_ASC SQL_IK_DESC|  
|SQL_INFO_SCHEMA_VIEWS|驱动程序不支持请求。|  
|SQL_INFO_SS_NETLIB_NAME|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client ODBC 驱动程序特定的属性。 连接使用的网络库的名称。<br /><br /> 默认情况下，返回 DBNETLIB。  在此情况下，DBNETLIB 是指网络库，并且与 dbnetlib.dll 无关。|  
|SQL_INTEGRITY|"Y"|  
