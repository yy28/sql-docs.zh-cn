...
### 目标  
 目标是指事件使用者。 目标在触发事件的线程中同步处理事件或在系统提供的线程中异步处理事件。 扩展事件提供了多个目标，您可以根据需要将其用于定向事件输出。 有关详细信息，请参阅 [SQL Server Extended Events Targets](../Topic/SQL%20Server%20Extended%20Events%20Targets.md)。  
