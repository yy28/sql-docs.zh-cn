...
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]
 ...
 下表中介绍的转换适用于在客户端上所进行的转换。 如果客户端为参数指定与服务器上定义的参数不同的分数秒精度，则客户端转换可能会成功，但在调用 SQLExecute 或**SQLExecuteDirect**时，服务器将返回错误**SQLExecuteDirect**。 特别是，ODBC 将小秒的任何截断视为错误，而[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]行为是舍入;例如，当您从**日期时间 2（6） 转到 datetime2（2）** 时，会发生舍入。 **datetime2(2)** Datetime 列值舍入为 1/300 秒，服务器将 smalldatetime 列的秒数设置为零。  
...
-   **1**： 如果提供的数据无效，则使用 SQLSTATE 22007 和消息"无效日期时间格式"生成诊断记录。  
  
-   **2：** 时间字段必须为零，或者使用 SQLSTATE 22008 和消息"分数截断"生成诊断记录。  
  
-   **3**： 分数秒数必须为零，或者使用 SQLSTATE 22008 和消息"分数截断"生成诊断记录。  
...
-   **9**： 字符串被解析并转换为日期、日期时间、日期时间偏移或时间值，具体取决于遇到的第一个标点符号字符和剩余组件的存在。 然后，根据上表中针对此过程发现的源类型的规则，此字符串将转换为目标类型。 如果在分析数据时检测到错误，则生成具有 SQLSTATE 22018 和消息“为转换指定的字符值无效”的诊断记录。 对于 datetime 和 smalldatetime 参数，如果年份超出这些类型支持的范围，则生成具有 SQLSTATE 22007 和消息“日期时间格式无效”的诊断记录。  
...
-   **10**： 如果发生数据丢失的截断，则会使用 SQLSTATE 22008 和消息"无效时间格式"生成诊断记录。 如果值处于服务器使用的 UTC 范围可表示的范围外，也会发生此错误。  
  
-   **11：** 如果数据的字节长度不等于 SQL 类型所需的结构大小，则使用 SQLSTATE 22003 和消息"数值不在范围"中生成诊断记录。  
  
-   **12**： 如果数据的字节长度为 4 或 8，则数据以原始 TDS 小日期或日期时间格式发送到服务器。 如果数据的字节长度与 SQL_TIMESTAMP_STRUCT 大小完全匹配，则将该数据转换为 datetime2 的 TDS 格式。  
  
-   **13**： 如果发生数据丢失的截断，则会使用 SQLSTATE 22001 和消息"字符串数据，右截断"生成诊断记录。  
  
     根据下表，根据目标列的大小确定小数秒位数（比例）：  
...
-   **不适用**：保持现有[!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)]和早期行为。  
