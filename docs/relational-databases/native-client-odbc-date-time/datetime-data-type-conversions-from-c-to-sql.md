...
-   **1**： 如果提供的数据不是有效的使用 SQLSTATE 22007 和消息"无效的日期时间格式"生成的诊断记录。  
  
-   **2**： 时间字段必须为零，或使用 SQLSTATE 22008 和消息"小数截断"生成诊断记录。  
  
-   **3**： 秒的小数部分必须为零，或使用 SQLSTATE 22008 和消息"小数截断"生成诊断记录。  
...
-   **9**： 分析字符串并将其转换为日期、 datetime、 datetimeoffset 或时间值，具体取决于遇到的第一个标点字符和剩余的组件的状态。 然后，根据上表中针对此过程发现的源类型的规则，此字符串将转换为目标类型。 如果在分析数据时检测到错误，则生成具有 SQLSTATE 22018 和消息“为转换指定的字符值无效”的诊断记录。 对于 datetime 和 smalldatetime 参数，如果年份超出这些类型支持的范围，则生成具有 SQLSTATE 22007 和消息“日期时间格式无效”的诊断记录。  
...
-   **10**： 如果发生数据丢失的截断，使用 SQLSTATE 22008 和消息"无效的时间格式"生成的诊断记录。 如果值处于服务器使用的 UTC 范围可表示的范围外，也会发生此错误。  
  
-   **11**： 的字节长度的数据所需的 SQL 类型的结构的大小不相等，如果使用 SQLSTATE 22003 和消息"数值超出范围"生成诊断记录。  
  
-   **12**： 如果数据的字节长度为 4 或 8，则数据发送到服务器原始 TDS smalldatetime 或日期时间格式。 如果数据的字节长度与 SQL_TIMESTAMP_STRUCT 大小完全匹配，则将该数据转换为 datetime2 的 TDS 格式。  
  
-   **13**： 如果发生数据丢失的截断，使用 SQLSTATE 22001 和消息"字符串数据，右截断"生成的诊断记录。  
  
     从下表根据目标列的大小确定的秒的小数部分位数 （缩放）：  
