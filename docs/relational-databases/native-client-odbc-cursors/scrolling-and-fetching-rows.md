 ...
 **SQLFetchScroll**用于解决游标滚动。 **SQLFetchScroll**支持提取下一行，之前，第一个和最后一个行集除了相对提取 (提取行集 *n* 从开始的当前行集的行) 和绝对提取 （提取从行开始的行集 *n* )。 如果 *n* 是中的绝对读取负，行进行计数从结果集的末尾。 绝对提取行 -1 表示提取从结果集中最后一行开始的行集。  
