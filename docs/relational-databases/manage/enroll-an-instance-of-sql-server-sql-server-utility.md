...
description: 将 SQL Server 实例注册到现有 SQL Server 实用工具来监视它的性能和配置。
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
-   要注册的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例不能是 Azure SQL 数据库。  
...
|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 版本必须支持实例注册。|有关 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]各版本支持的功能列表，请参阅 [SQL Server 2016 各个版本支持的功能](~/sql-server/editions-and-supported-features-for-sql-server-2016.md)。|  
...
|WMI 必须正确配置。|若要排查 WMI 配置问题，请参阅 [SQL Server 实用工具故障排除](/previous-versions/sql/sql-server-2016/ee210592(v=sql.130))。|  
...
>   
>  有关排除此故障的详细信息，请参阅 [SQL Server 实用工具故障排除](/previous-versions/sql/sql-server-2016/ee210592(v=sql.130))。  
