...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]此 Active Server Pages (ASP) 应用程序允许你更新 Person.Contact 表在 Microsoft 中的 AdventureWorks 示例数据库中的客户信息[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]。 应用程序执行以下操作：  
...
-   通过使用 HTML 表单显示此信息。  
 ...
 以下模板为第一个模板 (GetContact.xml)。 将此模板保存在该目录中的虚拟名称与关联**模板**类型。  
...
```  
 ...
 以下模板为第二个模板 (UpdateContact.xml)。 将此模板保存在该目录中的虚拟名称与关联**模板**类型。  
