...
|SQL_LONGVARBINARY、SQL_LONGVARCHAR、SQL_WLONGVARCHAR|当连接到实例6时，完全支持 SQL_LONGVARBINARY、SQL_LONGVARCHAR 或 SQL_WLONGVARCHAR 数据类型（使用 WHERE 子句）的列的更新 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 。*x*及更高版本。 当连接到 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 4.2*x*的实例时，S1000 错误 "部分插入/更新"。 文本或图像列的插入/更新失败”。<br /><br /> 注意： [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native CLIENT ODBC 驱动程序支持连接到 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 6.5 及更早版本。|  
...
|大值类型|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Native CLIENT ODBC 驱动程序将**varchar （max）**、 **varbinary （max）** 和**nvarchar （max）** 类型在接受或返回 ODBC SQL 数据类型的 api 中分别公开为 SQL_VARCHAR、SQL_VARBINARY 和 SQL_WVARCHAR。|  
