---
title: 访问ADO.NET的用户定义类型 |微软文档
description: 用 .NET 框架 CLR 语言编写的 UDT 允许 SQL Server 数据库存储对象和自定义数据结构。 在ADO.NET，提供程序公开 UDT。
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.reviewer: ''
...
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]
  用户定义的类型 （UDT） 使用[!INCLUDE[msCoName](../../includes/msconame-md.md)].NET Framework 通用语言运行时 （CLR） 支持的任何语言编写，这些语言生成可验证的代码。 这包括 [!INCLUDE[msCoName](../../includes/msconame-md.md)] Visual C# 和 [!INCLUDE[msCoName](../../includes/msconame-md.md)] Visual Basic。 使用 UDT 可在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 数据库中存储对象和自定义数据结构。 数据公开为 .NET Framework 类或结构的公共成员，行为则由类或结构的方法来定义。 UDT 可用作表的列定义、[!INCLUDE[tsql](../../includes/tsql-md.md)] 批处理中的变量，还可用作 [!INCLUDE[tsql](../../includes/tsql-md.md)] 函数或存储过程的参数。  
  
 在**ADO.NET，System.Data.SqlClient**提供程序以以下方式公开 UDT：  
  
-   通过**系统.Data.SqlClient.SqlDataReader**作为一个对象。  
  
-   通过**SqlDataReader**作为原始字节。  
  
-   作为**系统.Data.SqlClient.Sql参数对象的参数**。  
...
## <a name="in-this-section"></a>本节内容  
 ...
 介绍如何使用**DataSet**中的 UDT 以及如何使用**DataAdapter**更新 UDT 数据。  
