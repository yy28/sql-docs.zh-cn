...
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]进行编码时你的用户定义类型 (UDT) 定义，则必须实现各种功能，具体取决于是否要实现用户定义的类型作为类或结构，以及你已选择的格式和序列化选项。  
