...
### <a name="example"></a>示例  
 以下 [!INCLUDE[tsql](../../includes/tsql-md.md)] 必须按以下顺序执行。 第一个表即在引用**点**UDT 必须被删除，然后的类型和最后程序集。  
 ...
 **货币**UDT **AdventureWorks**示例数据库提供构建 UDT 和其关联的函数的建议方法的有用示例。 **货币**UDT 用于处理基于特定区域性的货币的系统上的资金，并允许不同的货币类型，如资金、 欧元和等的存储。 UDT 类公开的区域性名称为字符串，而作为 money 大量**十进制**数据类型。 所有必需的序列化方法都包括在定义该类的程序集中。 实现到另一个区域性中的货币换算的函数使用与外部函数名为**ConvertCurrency**，并且此函数位于单独的程序集。 **ConvertCurrency**函数完成其工作，通过检索的表中的转换率**AdventureWorks**数据库。 如果应发生变化的转换率的源，或如果应该与现有代码的任何其他更改，该程序集可以轻松地修改而不会影响**货币**UDT。  
