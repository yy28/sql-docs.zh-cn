...
-   如果 UDT 是在类中定义的，而不是在结构中定义的，则必须将**InteropServices**指定为**StructLayout。** 此属性控制数据字段的物理布局，并用来按照成员的出现顺序对它们进行布局。 
 ...
 **IsByteOrdered**  
 确定**** 如何[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]对 UDT 执行二进制比较的布尔值。  
