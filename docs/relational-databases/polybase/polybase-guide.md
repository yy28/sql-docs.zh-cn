...
helpviewer_keywords: 
  ...
  - "Hadoop 导入 ×"
  - "Hadoop 导出"
  - "Hadoop 导出, PolyBase 概述"
  - "Hadoop 导入, PolyBase 概述"
...
[!INCLUDE[tsql-appliesto-ss2016-xxxx-asdw-pdw_md](../../includes/tsql-appliesto-ss2016-xxxx-asdw-pdw-md.md)]
  ...
  PolyBase 是一种访问和合并所有来自 SQL Server 中的非关系和关系数据的技术。   它允许对 Hadoop 或 Azure blob 存储中的外部数据运行查询。   查询会进行优化以将计算推送到 Hadoop  
  
 仅使用 Transact-SQL (T-SQL) 语句，你就可以在 SQL Server 中的关系表和存储在 Hadoop 或 Azure Blob 存储中的非关系数据之间来回导入和导出。 你还可以从 T-SQL 查询内查询外部数据并将其与关系数据联接。  
...
## 为什么要用 PolyBase？  
 若要作出正确决策，商业决策者需要同时分析关系数据和其他未构建到表中的数据 - 尤其是 Hadoop 和其他 NoSQL 技术。 除非有方法能够在不同数据存储类型之间传输数据，否则这将很难执行。 PolyBase 通过处理 SQL Server 外部的数据填补了这一差距。  
...
-   **从 Hadoop 或 Azure blob 存储中导入数据。** 通过从 Hadoop 或 Azure blob 存储导入数据到关系表中，利用 Microsoft SQL 的列存储技术和分析功能的速度。 不需要单独的 ETL 或导入工具。  
  
-   **将数据导出到 Hadoop 或 Azure blob 存储** 将数据存档到 Hadoop 或 Azure blob 存储，以获得高效存储并使其在线，便于访问。  
