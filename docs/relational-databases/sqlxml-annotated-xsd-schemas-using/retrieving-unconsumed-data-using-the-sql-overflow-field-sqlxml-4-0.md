...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]通过使用在 XML 文档中的数据库中插入记录时[!INCLUDE[tsql](../../includes/tsql-md.md)]OPENXML 函数，所有源 XML 文档中未用完的数据可以存储在列中。 当通过使用带批注的架构，可以从数据库检索数据时，你可以指定**sql:overflow-字段**属性用于标识在其中存储的溢出数据的表中的列。 **Sql:overflow-字段**上指定属性**\<元素 >**。  
