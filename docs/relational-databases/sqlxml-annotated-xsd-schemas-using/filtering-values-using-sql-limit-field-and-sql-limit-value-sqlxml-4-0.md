...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]你可以限制从某些限制的值根据数据库查询返回的行。 **Sql:limit-字段**和**sql:limit-值**批注用于标识包含限制值的数据库列并指定特定的限制值要用于筛选的数据返回。  
