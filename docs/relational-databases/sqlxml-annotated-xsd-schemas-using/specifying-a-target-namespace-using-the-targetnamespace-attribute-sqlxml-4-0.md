---
title: 使用 targetNamespace 指定目标命名空间（SQLXML）
...
### <a name="a-specifying-a-target-namespace"></a>A. 指定目标命名空间  
 以下 XSD 架构使用**XSD： targetNamespace**属性指定目标命名空间。 该架构还会将**elementFormDefault**和**attributeFormDefault**属性值设置为 **"未限定"** （这些属性的默认值）。 这是全局声明，影响架构中的所有本地元素（ **\<Order>** 在架构中）和属性（**CustomerID**、"**联系人姓名**" 和 "**订单 id** "）。  
...
```  
 ...
 此实例文档定义了 urn： MyNamespace 命名空间，并将前缀（y0）与该命名空间相关联。 前缀仅应用于 **\<Customer>** 全局元素。 （元素是全局性的，因为它在架构中声明为元素的子元素 **\<xsd:schema>** 。）  
  
 前缀不适用于本地元素和属性，因为**elementFormDefault**和**attributeFormDefault**属性的值在架构中设置为 **"非限定"** 。 请注意， **\<Order>** 元素是局部的，因为其声明显示为 **\<complexType>** 定义元素的元素的子元素 **\<CustomerType>** 。 同样，属性（**CustomerID**、**订单 Id**和**联系人姓名**）是本地的，而不是全局的。  
