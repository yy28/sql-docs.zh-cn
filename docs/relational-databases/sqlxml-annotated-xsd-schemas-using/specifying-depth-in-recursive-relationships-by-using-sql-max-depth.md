...
```  
 ...
 在架构中，在**CustomerBaseType**复杂类型上指定了**sql： max 深度**。 该架构还指定了 **\<Customer>** **CustomerType**类型的元素，该元素派生自**CustomerBaseType**。 在此类架构上指定的 XPath 查询将生成一个错误，因为在限制基类型中定义的元素上不支持**sql：最大深度**。  
