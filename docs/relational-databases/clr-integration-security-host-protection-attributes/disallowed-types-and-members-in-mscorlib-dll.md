---
title: 在 mscorlib.dll 中禁止的类型和成员 |微软文档
description: SQL Server CLR 编程不允许具有主机保护资源枚举某些值的类型或成员。 本文列出了 mscorlib.dll 不允许的值。
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.reviewer: ''
...
ms.locfileid: "81486863"
---
# <a name="disallowed-types-and-members-in-mscorlibdll"></a>mscorlib.dll 中禁用的类型和成员
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]通用语言集成 （CLR） 编程不允许使用具有**指定** **System.Security.security.security.Host 保护资源**（具有**外部进程Mgmt、****外部线程****、MayLeakOnAbort、****安全基础结构**、**自影响进程Mgmnt、****自影响线程**、**共享状态**、**同步**或**UI）** 的值的类型或成员。 下表列出了宿主保护属性 (HPA) 值被禁用的 mscorlib.dll 程序集的成员和类型。  
