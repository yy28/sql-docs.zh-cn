...
当检测到[!INCLUDE[sssqlv14-md](../../includes/sssqlv14-md.md)]潜在性能问题时，自动优化就会通知你，并使你能够应用纠正措施， [!INCLUDE[ssde_md](../../includes/ssde_md.md)]或让你能够自动修复性能问题。
使用中的[!INCLUDE[sssqlv14-md](../../includes/sssqlv14-md.md)]自动优化，可以确定并修复**查询执行计划选择回归**导致的性能问题。 自动优化[!INCLUDE[ssazure_md](../../includes/ssazure_md.md)]还会创建必要的索引并删除未使用的索引。 有关查询执行计划的详细信息，请参阅[执行计划](../../relational-databases/performance/execution-plans.md)。
...
[!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)]监视在数据库上执行的查询，并自动提高工作负荷的性能。 [!INCLUDE[ssde_md](../../includes/ssde_md.md)]具有内置的智能机制，可通过动态调整数据库以适应工作负荷，自动优化和提高查询的性能。 有两种自动优化功能可用：
 ...
 -  **自动索引管理**标识应在数据库中添加的索引以及应删除的索引。 **适用**于：[!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]

## <a name="why-automatic-tuning"></a>为什么启用自动优化？

经典数据库管理中的三个主要任务是监视工作负荷、识别关键[!INCLUDE[tsql_md](../../includes/tsql-md.md)]查询、应该添加的索引以提高性能并识别很少使用。 [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)]提供对需要监视的查询和索引的详细见解。 然而，持续监视数据库是一种很繁琐的任务，尤其是在处理多个数据库时。 可能无法高效管理大量数据库。 你可以考虑[!INCLUDE[ssde_md](../../includes/ssde_md.md)]使用自动优化功能将某些监视和优化操作委派给，而不是手动监视和优化数据库。
...
此过程使数据库能够通过查找哪些索引和计划可提高工作负荷的性能和影响工作负荷的索引，动态适应工作负荷。 基于这些发现，自动优化将应用可提高工作负荷性能的优化操作。 此外，自动优化后，数据库会持续监视性能，以确保它可提高工作负荷的性能。 不会提高性能的任何操作都将自动还原。 此验证过程非常重要，可确保自动优化所做的任何更改都不会降低工作负荷性能。

## <a name="automatic-plan-correction"></a>自动计划更正

自动计划更正是一种自动优化功能，用于识别**执行计划选择回归**，并通过强制执行最后一个已知良好的计划来自动解决问题。 有关查询执行计划和查询优化器的详细信息，请参阅[查询处理体系结构指南](../../relational-databases/query-processing-architecture-guide.md)。

### <a name="what-is-execution-plan-choice-regression"></a>执行计划的选择回归是什么？

[!INCLUDE[ssdenoversion_md](../../includes/ssdenoversion_md.md)]可能使用不同的执行计划来执行[!INCLUDE[tsql_md](../../includes/tsql-md.md)]查询。 查询计划取决于统计信息、索引和其他因素。 用于执行某些[!INCLUDE[tsql_md](../../includes/tsql-md.md)]查询的最佳计划可能会随时间而改变。 在某些情况下，新计划可能不比上一个计划好，新计划可能会导致性能下降。

 ![查询执行计划选择回归](media/plan-choice-regression.png "查询执行计划选择回归") 

每当您注意到计划选择回归时，您都应该找到以前的一个良好计划，并强制执行此计划`sp_query_store_force_plan` ，而不是使用过程。
[!INCLUDE[ssde_md](../../includes/ssde_md.md)]中[!INCLUDE[sssqlv14-md](../../includes/sssqlv14-md.md)]提供了有关回归计划和建议的纠正措施的信息。
此外， [!INCLUDE[ssde_md](../../includes/ssde_md.md)]还可让你完全自动执行此过程[!INCLUDE[ssde_md](../../includes/ssde_md.md)] ，并修复发现的与计划更改相关的任何问题。
...
[!INCLUDE[ssde_md](../../includes/ssde_md.md)]自动检测任何潜在计划选择回归，包括应使用的计划而不是错误的计划。
当[!INCLUDE[ssde_md](../../includes/ssde_md.md)]应用最后一个已知良好的计划时，它会自动监视强制计划的性能。 如果强制计划不优于回归计划，则新计划将为 unforced，并且[!INCLUDE[ssde_md](../../includes/ssde_md.md)]将编译新计划。 如果[!INCLUDE[ssde_md](../../includes/ssde_md.md)]验证强制计划是否优于回归计划，则会保留强制计划（如果它比回归计划好），直到重新编译（例如，在下一个统计信息更新或架构更改时）。
...
若不使用自动优化，用户必须定期监视系统并查找回归的查询。 如果有任何计划回归，用户应找到以前的某个良好计划，并强制执行此计划， `sp_query_store_force_plan`而不是使用过程。 最佳做法是强制执行最后一个已知良好的计划，因为较旧的计划可能会因统计信息或索引更改而无效。 强制执行最后一个已知良好计划的用户应监视使用强制计划执行的查询的性能，并验证强制计划是否按预期方式工作。 根据监视和分析的结果，应强制执行计划，或用户应找到其他方法来优化查询。
...
| `FORCE_LAST_GOOD_PLAN` | CPU 时间从 37 ms 更改为84毫秒 | 3/16/2017 | 26 | `queryId` `recommendedPlanId` `regressedPlanId` `T-SQL` |   |

下面的列表中介绍了此视图中的某些列：
 - 建议的操作类型-`FORCE_LAST_GOOD_PLAN`
 - 说明，其中包含的[!INCLUDE[ssde_md](../../includes/ssde_md.md)]信息认为此计划的更改是潜在的性能回归
 - 检测到潜在回归时的日期时间
 - 此建议的分数
...
`estimated_gain`表示在执行建议的计划而不是当前计划时要保存的预计秒数。 如果增益大于10秒，则建议使用建议的计划，而不是当前计划。 如果当前计划中的错误（例如超时或中止执行）比建议计划中的多，则将列`error_prone`设置为值。 `YES` 易出错的计划是指应强制实施建议计划而不是当前计划的另一个原因。

尽管[!INCLUDE[ssde_md](../../includes/ssde_md.md)]提供标识计划选择回归所需的所有信息，但持续监视和修复性能问题可能是一种单调乏味的过程。 自动优化可使此过程变得更加容易。
...
在[!INCLUDE[ssazure_md](../../includes/ssazure_md.md)]中，索引管理非常简单[!INCLUDE[ssazure_md](../../includes/ssazure_md.md)] ，因为了解工作负荷并确保始终以最佳方式为数据编制索引。 正确的索引设计对于优化工作负荷性能至关重要，而自动索引管理也有助于优化索引。 自动索引管理可修复未正确编制索引的数据库中的性能问题，或者维护并改进现有数据库架构上的索引。 中[!INCLUDE[ssazure_md](../../includes/ssazure_md.md)]的自动优化执行以下操作：
 ...
 - 标识索引，这些索引可以提高从[!INCLUDE[tsql_md](../../includes/tsql-md.md)]表中读取数据的查询的性能。
...
[!INCLUDE[ssazure_md](../../includes/ssazure_md.md)]使用内置智能和高级规则来分析查询，确定最适合当前工作负荷的索引，并且索引可能会被删除。 对于可优化读取数据的查询的索引，Azure SQL 数据库可确保将此类索引数量降至最低，同时将对其他查询的影响降至最低。
...
在中创建必要索引所需[!INCLUDE[ssazure_md](../../includes/ssazure_md.md)]的操作可能会消耗资源，暂时会影响工作负荷性能。 若要将索引创建对工作负荷性能的影响[!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]降到最低，请为任何索引管理操作查找适当的时间范围。 如果数据库需要资源才能执行工作负荷，优化操作将被推迟，并在数据库有足够可用于维护任务的未使用资源时被启动。 验证操作是自动索引管理中的一项重要功能。 创建[!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]或删除索引时，监视过程会分析工作负荷的性能，以验证操作是否提高了性能。 如果未显著提高，则会立即还原操作。 这样， [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]就可以确保自动操作不会对工作负荷的性能产生负面影响。 自动优化创建的索引对在基础架构上进行的维护操作没有任何影响。 自动创建的索引不会阻止对架构进行更改（如删除或重命名列）。 当删除相关的表或[!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]列时，自动创建的索引会立即被删除。

### <a name="alternative---manual-index-management"></a>备选-手动索引管理

如果没有自动索引管理，用户需要手动查询[sys.databases dm_db_missing_index_details &#40;transact-sql&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-db-missing-index-details-transact-sql.md)查看或使用中[!INCLUDE[ssManStudio](../../includes/ssManStudio-md.md)]的 "性能仪表板" 报表查找可能会提高性能的索引，使用此视图中提供的详细信息创建索引，并手动监视查询的性能。 为了找到应删除的索引，用户应监视索引的操作使用情况统计信息，以查找很少使用的索引。
