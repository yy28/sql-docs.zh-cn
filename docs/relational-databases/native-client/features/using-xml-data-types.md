...
> [!NOTE]  
>  [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]本机客户端 OLE 数据库提供程序不包括 SAX 读取器，但**I顺序流**可以轻松地传递到 MSXML 中的 SAX 和 DOM 对象。  
  
 **I顺序流**应用于检索大型 XML 文档。 用于其他大值类型的相同技术也适用于 XML。 有关详细信息，请参阅[使用大型数据类型](../../../relational-databases/native-client/features/using-large-value-types.md)。  
  
 还可以通过常见接口（如 IRow::GetColumns、IRowChange::SetColumns 和 ICommand::Execute）检索、插入或更新行集中类型为 XML 的列中存储的数据************。 与检索情况类似，应用程序可以将文本字符串或**I顺序流**传递给[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]本机客户端 OLE 数据库提供程序。  
  
> [!NOTE]  
>  要通过 ISequentialStream 接口以字符串格式发送 XML 数据，必须通过指定 DBTYPE_IUNKNOWN 获取 ISequentialStream 并在绑定中将其 pObject 参数设置为 Null**********。  
  
 当由于使用者缓冲区过小而导致已检索的 XML 数据被截断时，长度可能作为 0xffffffff 返回，这意味着长度为未知的。 这与将它实现为针对客户端进行流处理的数据类型而未在发送实际数据之前发送长度信息是一致的。 在某些情况下，当提供程序缓冲整个值（如**IRowset：：GetData）** 以及执行数据转换的位置时，可能会返回实际长度。  
...
|DBTYPE_VARIANT (VT_BSTR)|传递<sup>6，10</sup>|N/A <sup>2</sup>|成功<sup>3</sup>|N/A <sup>2</sup>|  
 ...
 <sup>1</sup>如果使用 ICommandWith参数指定了 DBTYPE_XML 以外的服务器类型 **：：setparameterInfo**并且访问器类型DBTYPE_XML，则执行语句时将发生错误（DB_E_ERRORSOCCURRED，参数状态为DBSTATUS_E_BADACCESSOR）;否则，数据将发送到服务器，但服务器返回一个错误，指示没有从 XML 到参数的数据类型的隐式转换。  
 ...
 <sup>12</sup>如果缓冲区长度小于两个字符（即，没有足够的空间用于 null 终止），则报告溢出错误。  
  
> [!NOTE]  
>  对于 NULL XML 值，不返回值。  
  
 XML 标准要求 UTF-16 编码的 XML，以便以字节顺序标记 (BOM) UTF-16 字符代码 0xFEFF 开头。 使用 WSTR 和 BSTR 绑定[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]时，本机客户端不需要或添加 BOM，因为绑定隐含了编码。 当使用 BYTES、XML 或 IUNKNOWN 绑定时，目的是为了在处理其他 XML 处理器和存储系统时提供简便性。 在此情况下，应向 UTF-16 编码的 XML 提供 BOM，并且应用程序无需关心实际编码，因为绝大多数的 XML 处理器（包括 SQL Server）会通过检查该值的前几个字节推导出编码。 使用 BYTES、XML 或 IUNKNOWN 绑定从[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]本机客户端接收的 XML 数据始终在 UTF-16 中使用 BOM 进行编码，并且没有嵌入的编码声明。  
...
### <a name="ole-db-rowset-additions-and-changes"></a>OLE DB 行集的添加和更改内容  
 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]本机客户端向许多核心 OLE DB 架构行集添加新值或更改。  
...
### <a name="ole-db-property-set-additions-and-changes"></a>OLE DB 属性集的添加和更改内容  
 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]本机客户端向许多核心 OLE DB 属性集添加新值或更改。  
  
#### <a name="the-dbpropset_sqlserverparameter-property-set"></a>DBPROPSET_SQLSERVERPARAMETER 属性集  
 为了通过 OLE DB 支持**xml**数据类型[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]，本机客户端实现了新的DBPROPSET_SQLSERVERPARAMETER属性集，其中包含以下值。  
...
#### <a name="the-dbpropset_sqlservercolumn-property-set"></a>DBPROPSET_SQLSERVERCOLUMN 属性集  
 为了支持在**ITable定义**界面中创建表，[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]本机客户端向DBPROPSET_SQLSERVERCOLUMN属性集添加了三个新列。  
...
### <a name="ole-db-interface-additions-and-changes"></a>OLE DB 接口的添加和更改内容  
 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]本机客户端向许多核心 OLE DB 接口添加新值或更改。  
  
#### <a name="the-isscommandwithparameters-interface"></a>ISSCommandWithParameters 接口  
 为了通过 OLE DB 支持**xml**数据类型[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]，本机客户端实现了许多更改，包括添加[ISSCommand 与参数接口](../../../relational-databases/native-client-ole-db-interfaces/isscommandwithparameters-ole-db.md)。 这一新接口继承自核心 OLE DB 接口 ICommandWithParameters****。 除了从**ICommand 与参数**继承的三种方法外，**获取参数信息**、**映射参数名称**和**设置参数信息**;**ISSCommand 与参数**提供用于处理服务器特定数据类型的[Get 参数属性](../../../relational-databases/native-client-ole-db-interfaces/isscommandwithparameters-getparameterproperties-ole-db.md)和[设置参数属性](../../../relational-databases/native-client-ole-db-interfaces/isscommandwithparameters-setparameterproperties-ole-db.md)方法。  
  
> [!NOTE]  
>  ISSCommandWithParameters 接口也利用新的 SSPARAMPROPS 结构****。  
  
#### <a name="the-icolumnsrowset-interface"></a>IColumnsRowset 接口  
 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]本机客户端将以下[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]特定于列添加到 IColumnRowset 返回的行**集：：getColumnRowset**方法。 这些列包含 XML 架构集合的由三个部分组成的名称。 对于非 XML 列或非类型化 XML 列，所有这三列均取默认值 NULL。  
...
#### <a name="the-irowsetchange-interface"></a>IRowsetChange 接口  
 ...
 在DBTYPE_IUNKNOWN/I顺序流的情况下，如果使用者不指定任何存储对象，使用者必须提前创建**I顺序流**对象，将 XML 文档与对象绑定，然后通过**IRowsetChange：setData**方法将对象传递给提供程序。 使用者还可以创建存储对象，将 pObject 参数设置为 IID_ISequentialStream，创建 ISequentialStream 对象，然后将 ISequentialStream 对象传递给 IRowsetChange::SetData 方法************。 在这两种情况下，提供程序都可以通过 ISequentialStream 对象检索 XML 对象，然后将其插入适当的列中****。  
  
#### <a name="the-irowsetupdate-interface"></a>IRowsetUpdate 接口  
 IRowsetUpdate 接口提供了用于延迟更新的功能****。 在使用者调用**IRowsetUpdate：Update**方法之前，提供给行集的数据不会提供给其他事务。  
...
## <a name="sql-server-native-client-odbc-driver"></a>SQL Server Native Client ODBC 驱动程序  
 在[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]本机客户端 ODBC 驱动程序中，对各种函数进行了许多更改，以支持**xml**数据类型。  
  
### <a name="sqlcolattribute"></a>SQLColAttribute  
 [SQLColAttribute](../../../relational-databases/native-client-odbc-api/sqlcolattribute.md)函数具有三个新的字段标识符，包括SQL_CA_SS_XML_SCHEMACOLLECTION_CATALOG_NAME、SQL_CA_SS_XML_SCHEMACOLLECTION_SCHEMA_NAME和SQL_CA_SS_XML_SCHEMACOLLECTION_NAME。  
  
 本机[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]客户端 ODBC 驱动程序报告SQL_DESC_DISPLAY_SIZE和SQL_DESC_LENGTH列SQL_SS_LENGTH_UNLIMITED。  
  
### <a name="sqlcolumns"></a>SQLColumns  
 [SQLColumns](../../../relational-databases/native-client-odbc-api/sqlcolumns.md)函数有三个新列，包括SS_XML_SCHEMACOLLECTION_CATALOG_NAME、SS_XML_SCHEMACOLLECTION_SCHEMA_NAME和SS_XML_SCHEMACOLLECTION_NAME。 现有的 TYPE_NAME 列用于指示 XML 类型的名称，而 XML 类型列或参数的 DATA_TYPE 为 SQL_SS_XML。  
  
 本机[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]客户端 ODBC 驱动程序报告COLUMN_SIZE和CHAR_OCTET_LENGTH值SQL_SS_LENGTH_UNLIMITED。  
  
### <a name="sqldescribecol"></a>SQLDescribeCol  
 当[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] [SQLDescribeCol](../../../relational-databases/native-client-odbc-api/sqldescribecol.md)函数中无法确定列大小时，本机客户端 ODBC 驱动程序SQL_SS_LENGTH_UNLIMITED报告。  
  
### <a name="sqlgettypeinfo"></a>SQLGetTypeInfo  
 本机[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]客户端 ODBC 驱动程序报告SQL_SS_LENGTH_UNLIMITED为[SQLGetTypeInfo](../../../relational-databases/native-client-odbc-api/sqlgettypeinfo.md)函数中**xml**数据类型的最大COLUMN_SIZE。  
  
### <a name="sqlprocedurecolumns"></a>SQLProcedureColumns  
 [SQL程序列函数](../../../relational-databases/native-client-odbc-api/sqlprocedurecolumns.md)具有与**SQLColumn**函数相同的列添加功能。  
  
 本机[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]客户端 ODBC 驱动程序报告SQL_SS_LENGTH_UNLIMITED为**xml**数据类型的最大COLUMN_SIZE。  
...
-   SQL_C_CHAR：数据在客户端上转换为 UTF-16 并发送到服务器，就像 SQL_C_WCHAR 一样（包括添加 BOM）。 如果未在客户端代码页中对 XML 进行编码，则这可能导致数据损坏。  
 ...
 XML 标准要求 UTF-16 编码的 XML，以便以字节顺序标记 (BOM) UTF-16 字符代码 0xFEFF 开头。 使用SQL_C_BINARY绑定时，[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]本机客户端不需要或添加 BOM，因为绑定隐含了编码。 其目的是为了在处理其他 XML 处理器和存储系统时提供简便性。 在此情况下，应向 UTF-16 编码的 XML 提供 BOM，并且应用程序无需关心实际编码，因为绝大多数的 XML 处理器（包括 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]）会通过检查该值的前几个字节推导出编码。 使用SQL_C_BINARY绑定从[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]本机客户端接收的 XML 数据始终使用 BOM 在 UTF-16 中编码，并且没有嵌入的编码声明。  
