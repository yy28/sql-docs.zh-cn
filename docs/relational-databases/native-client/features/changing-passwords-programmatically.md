...
> [!NOTE]  
>  如果可能，请在运行时提示用户输入他们的凭据，并避免用持久化格式存储他们的凭据。 如果必须保存其凭据，您还应该加密这些使用[Win32 crypto API](http://go.microsoft.com/fwlink/?LinkId=64532)。 有关使用密码的详细信息，请参阅[强密码](../../../relational-databases/security/strong-passwords.md)。  
