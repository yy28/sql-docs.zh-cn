...
## <a name="sql-server-native-client-ole-db-provider"></a>SQL Server Native Client OLE DB 访问接口  
 ...
 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client OLE DB 提供程序还支持将输出参数绑定为大值数据类型的 DBTYPE_IUNKNOWN，以促进存储过程返回大值类型作为返回值（作为 DBTYPE_IUNKNOWN 公开给机.  
...
## <a name="sql-server-native-client-odbc-driver"></a>SQL Server Native Client ODBC 驱动程序  
 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client ODBC 驱动程序在接受或返回 ODBC SQL 数据类型的 ODBC API 函数中将**varchar （max）** 、 **varbinary （max）** 和**nvarchar （max）** 类型公开为 SQL_VARCHAR、SQL_VARBINARY 和 SQL_WVARCHAR。  
