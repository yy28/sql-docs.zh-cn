...
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]
 ...
 有关日期/时间改进的详细信息，请参阅[&#40;OLE DB&#41;的日期和时间改进](../../../relational-databases/native-client-ole-db-date-time/date-and-time-improvements-ole-db.md)以及[ODBC&#41;&#40;日期和时间改进](../../../relational-databases/native-client-odbc-date-time/date-and-time-improvements-odbc.md)。  
...
### <a name="use-time-as-a-distinct-data-type"></a>将时间用作非重复数据类型  
 ...
 新的 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 时间数据类型具有秒的小数形式，其准确度可达 100 纳秒。 这需要本机客户端中[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]的新类型：DBTYPE_DBTIME2 （OLE DB） 和SQL_SS_TIME2 （ODBC）。 已编写的所用时间不带秒的小数部分的现有应用程序可以使用 time(0) 列。 现有的 OLE DB DBTYPE_TIME 和 ODBC SQL_TYPE_TIME 类型及其对应的结构应正常工作，除非应用程序依赖于元数据中返回的类型。  
...
### <a name="use-datetime-with-extended-fractional-seconds-precision"></a>使用具有扩展的秒的小数部分精度的日期时间  
 ...
 ODBC 还定义了一个精度高达 1 纳秒的类型。 但是，此类型已由现有的 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 应用程序使用，并且此类应用程序预计只需 3 毫秒的精度。 新的**日期时间2（3）** 类型与现有**日期时间**类型不直接兼容。 **日期时间2（3）** 的精度为1毫秒，**日期时间**的精度为1/300秒。 在 ODBC 中，应用程序可以确定哪一个服务器类型用于描述符字段 SQL_DESC_TYPE_NAME。 因此，现有类型 SQL_TYPE_TIMESTAMP（对于 ODBC 2.0 应用程序为 SQL_TIMESTAMP）可用于这两个类型。  
