...
>   
>  此外，远程处理代理/存根 dll （在 MDAC 2.8 中）无法调用 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 中的**ISSAsynchStatus**接口。 ISSAsynchStatus 接口不通过远程公开。  
>   
...
-   调用 ICommand::Execute，并请求 IMultipleResults。  
 ...
 执行完命令后，可以将**IMultipleResults**用作正常情况，只需要同步事例中的一个例外：可以返回 DB_S_ASYNCHRONOUS，在这种情况下，可以使用**IDBAsynchStatus**或**ISSAsynchStatus**来确定操作何时完成。  
