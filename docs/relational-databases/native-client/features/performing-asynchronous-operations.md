...
## <a name="opening-and-closing-a-database-connection"></a>打开和关闭数据库连接  
 使用[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]本机客户端 OLE DB 提供程序时，设计以异步初始化数据源对象的应用程序可以在调用**IDB初始化：：初始化**之前在DBPROP_INIT_ASYNCH属性中设置DBPROPVAL_ASYNCH_INITIALIZE位。 设置此数据时，提供程序立即通过对 Initialize 的调用返回值。如果操作立即完成，则返回 S_OK；如果初始化以异步方式继续进行，则返回 DB_S_ASYNCHRONOUS****。 应用程序可以查询数据源对象上的**IDBAsynch 身份**或[ISSAsynchStatus 接口](../../../relational-databases/native-client-ole-db-interfaces/issasynchstatus-ole-db.md)，然后调用**IDBAsynch 状态：获取状态**或[ISSAsynch状态：：WaitForsynch 完成](../../../relational-databases/native-client-ole-db-interfaces/issasynchstatus-waitforasynchcompletion-ole-db.md)以获取初始化的状态。  
...
> [!NOTE]  
>  用于连接池的数据源对象不能调用本机客户端 OLE 数据库提供程序中的[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] **ISSAsynchStatus**接口。 ISSAsynchStatus 接口不对入池数据源对象公开****。  
>   
>  如果应用程序显式强制使用游标引擎，则 IOpenRowset::OpenRowset 和 IMultipleResults::GetResult 不支持异步处理********。  
>   
>  此外，远程代理/存根 dll（在 MDAC 2.8 中）不能调用本机客户端中的[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] **ISSAsynchStatus**接口。 ISSAsynchStatus 接口不通过远程公开****。  
>   
