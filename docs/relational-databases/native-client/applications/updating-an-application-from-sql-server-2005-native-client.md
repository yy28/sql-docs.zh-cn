...
|------------------------------------------------------------------------------------|-----------------|  
|OLE DB 仅填充到定义的小数位数。|对于将转换的数据发送到服务器的转换，[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client （从 [!INCLUDE[ssKatmai](../../../includes/sskatmai-md.md)]开始）填充数据中的尾随零，最大长度为**日期时间**值的最大长度。 SQL Server Native Client 9.0 则填充到 9 位数。|  
|验证 ICommandWithParameter：： SetParameterInfo DBTYPE_DBTIMESTAMP。|[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client （从 [!INCLUDE[ssKatmai](../../../includes/sskatmai-md.md)]开始）实现了 ICommandWithParameter：： SetParameterInfo 中的*bScale*的 OLE DB 要求，将其设置为 DBTYPE_DBTIMESTAMP 的秒小数部分。|  
|**Sp_columns**存储过程现在为 IS_NULLABLE 列返回 **"no"** ，而不是 **"no** "。|从 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 10.0 （[!INCLUDE[ssKatmai](../../../includes/sskatmai-md.md)]）开始， **sp_columns**存储过程现在为 IS_NULLABLE 列返回 **"no"** 而不是 **"no"** 。|  
...
|SQLCopyDesc 现在执行描述符一致性检查。|在 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 10.0 之前，如果对特定记录设置了 SQL_DESC_DATA_PTR 字段，则 SQLCopyDesc 不执行一致性检查。|  
...
|日期超出范围时返回其他错误。|对于**日期时间**类型，将 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client （从 [!INCLUDE[ssKatmai](../../../includes/sskatmai-md.md)]开始）返回超出了早期版本中所返回日期的错误号。<br /><br /> 具体而言，[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 9.0 在字符串转换为**datetime**时，将所有超出范围的年份值返回22007，并且当日期在支持的范围内时，从版本10.0 （[!INCLUDE[ssKatmai](../../../includes/sskatmai-md.md)]）开始 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native client 将返回22008**datetime**或**smalldatetime**支持的范围之外的**datetime2** 。|  
