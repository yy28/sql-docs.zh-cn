...
## <a name="remarks"></a>备注
`sys.dm_db_page_info`动态管理函数将返回页面信息， `page_id`如`file_id`、 `index_id`、 `object_id`等。 此信息可用于故障排除和调试各种性能（锁和闩锁争用）和损坏问题。
...
的一个重要用例`sys.dm_db_page_info`是将它与公开页信息的其他 dmv 结合起来。  为了便于此用例，添加了一个名`page_resource`为的新列，它以8字节十六进制格式公开页面信息。 此列已添加到`sys.dm_exec_requests`中， `sys.sysprocesses`并将根据需要添加到将来的其他 dmv。

`sys.fn_PageResCracker`新函数`page_resource`将采用作为输入，并输出包含`database_id`、 `file_id`和`page_id`的单个行。  然后，可以使用此函数来简化或`sys.dm_exec_requests` `sys.sysprocesses`之间的联接`sys.dm_db_page_info`。
...
下面的查询将返回一行，其中包含给定`database_id`的`file_id`、、 `page_id`与默认模式（"受限"）组合的所有页面信息
