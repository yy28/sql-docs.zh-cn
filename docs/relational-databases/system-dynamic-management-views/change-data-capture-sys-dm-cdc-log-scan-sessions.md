...
|**start_time**|**datetime**|会话的开始时间。<br /><br /> 当**session_id** = 0，聚合的数据收集开始的时间。|  
|**end_time**|**datetime**|会话已结束的时间。<br /><br /> NULL = 会话处于活动状态。<br /><br /> 当**session_id** = 0，最后一个会话已结束的时间。|  
|**duration**|**bigint**|会话的持续时间（以秒为单位）。<br /><br /> 0 = 会话不包含变更数据捕获事务。<br /><br /> 当**session_id** = 0，与更改数据捕获事务的所有会话的持续时间 （以秒为单位） 的总和。|  
|**scan_phase**|**nvarchar(200)**|会话的当前阶段。 以下是可能的值以及及其说明：<br /><br /> 1： 读取配置<br />2： 第一次扫描，生成哈希表<br />3： 第二个扫描<br />4： 第二个扫描<br />5： 第二个扫描<br />6： 架构版本管理<br />7： 上次扫描<br />8： 完成<br /><br /> 当**session_id** = 0，此值始终为"聚合"。|  
|**error_count**|**int**|遇到的错误数。<br /><br /> 当**session_id** = 0，在所有会话中的错误总数。|  
|**start_lsn**|**nvarchar(23)**|会话的起始 LSN。<br /><br /> 当**session_id** = 0，最后一个会话的开始 LSN。|  
|**current_lsn**|**nvarchar(23)**|当前正在扫描的 LSN。<br /><br /> 当**session_id** = 0，当前 LSN 为 0。|  
|**end_lsn**|**nvarchar(23)**|会话的结束 LSN。<br /><br /> NULL = 会话处于活动状态。<br /><br /> 当**session_id** = 0，最后一个会话的结束 LSN。|  
|**tran_count**|**bigint**|已处理的变更数据捕获事务数。 此计数器在第 2 阶段中填充。<br /><br /> 当**session_id** = 0，在所有会话中处理事务数。|  
|**last_commit_lsn**|**nvarchar(23)**|已处理的最后一个提交日志记录的 LSN。<br /><br /> 当**session_id** = 0，任何会话的最新提交日志记录 LSN。|  
|**last_commit_time**|**datetime**|最后一个提交日志记录的处理时间。<br /><br /> 当**session_id** = 0，上次提交的任何会话日志记录的时间。|  
|**log_record_count**|**bigint**|扫描的日志记录数。<br /><br /> 当**session_id** = 0，对所有会话所扫描的记录数。|  
|**schema_change_count**|**int**|检测到的数据定义语言 (DDL) 操作数。 此计数器在第 6 阶段填充。<br /><br /> 当**session_id** = 0，在所有会话中处理的 DDL 操作的数目。|  
|**command_count**|**bigint**|已处理的命令数。<br /><br /> 当**session_id** = 0，在所有会话中处理的命令数。|  
