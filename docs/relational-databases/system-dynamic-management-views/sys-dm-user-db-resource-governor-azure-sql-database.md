...
|**dtu_limit**|INT|数据库 (对于 vCore 为 NULL) 的 DTU 限制。|
|**cpu_limit**|INT|vCore 的数据库 (为 NULL 的 DTU 数据库) 的限制。|
...
|**min_memory**|INT|可由用户工作负荷的最小内存百分比。|
|**max_memory**|INT|可由用户工作负荷的最大内存百分比。|
|**max_sessions**|INT|用户组的会话数限制。|
|**max_memory_grant**|INT|最大内存授予的每个查询中用户的工作负荷，以百分比表示。|
...
|**instance_cap_cpu**|INT|在实例级别的 CPU 上限。|
|**instance_max_log_rate**|BIGINT|日志生成速率在实例级别，每秒字节数的上限。|
|**instance_max_worker_threads**|INT|在实例级别的工作线程限制。|
|**replica_type**|INT|辅助副本类型，其中 0 是主键和 1。|
|**max_transaction_size**|BIGINT|使用的任何事务，以 kb 为单位的最大日志空间。|
|**checkpoint_rate_mbps**|INT|检查点带宽 （以 mbps 为单位）。|
|**checkpoint_rate_io**|INT|检查点 IO 在 IOs 中每秒速率。|
|**last_updated_date_utc**|DATETIME|日期和时间的最后一个设置的更改或重新配置。|
|**primary_group_id**|INT|主要用户工作负荷组 id。|
|**primary_group_max_workers**|INT|主要用户的工作负荷组级别的辅助角色限制。|
|**primary_min_log_rate**|BIGINT|主要用户的工作负荷组级别的最小日志速率 （字节数 / 秒）。|
|**primary_max_log_rate**|BIGINT|主要用户的工作负荷组级别的最大日志速率 （字节数 / 秒）。|
|**primary_group_min_io**|INT|主要用户的工作负荷组级别的最小 IO。|
|**primary_group_max_io**|INT|主要用户的工作负荷组级别的最大 IO。|
|**primary_group_min_cpu**|FLOAT|最小 CPU 百分比限制在主要用户的工作负荷组级别。|
|**primary_group_max_cpu**|FLOAT|最大 CPU 百分比限制在主要用户的工作负荷组级别。|
|**primary_log_commit_fee**|INT|主要用户的工作负荷组级别的日志速率调控提交费用。|
|**primary_pool_max_workers**|INT|在主用户池级别的辅助角色限制。
|**pool_max_io**|INT|在主用户池级别的最大 IO 限制。|
|**govern_db_memory_in_resource_pool**|bit|指示是否在资源池级别控制的缓冲池的最大大小。 通常设置为弹性池内的数据库。|
|**volume_local_iops**|INT|每个本地卷 （例如 c:，d:） 的第二个 cap IOs。|
|**volume_managed_xstore_iops**|INT|每个远程存储帐户的第二个上限的 IOs。|
|**volume_external_xstore_iops**|INT|每个远程存储帐户由 Azure SQL DB 备份和遥测数据的第二个上限的 IOs。|
|**volume_type_local_iops**|INT|每个的所有本地卷的第二个上限的 IOs。|
|**volume_type_managed_xstore_iops**|INT|每个实例使用的所有远程存储帐户的第二个上限的 IOs。|
|**volume_type_external_xstore_iops**|INT|每个实例使用的 Azure SQL DB 备份和遥测数据的所有远程存储帐户的第二个上限的 IOs。|
|**volume_pfs_iops**|INT|每个高级文件存储的第二个上限的 IOs。|
|**volume_type_pfs_iops**|INT|每个实例使用的所有高级文件存储的第二个上限的 IOs。|
|||
