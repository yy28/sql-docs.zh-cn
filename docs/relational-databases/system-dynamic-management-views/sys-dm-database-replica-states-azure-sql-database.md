...
ms.locfileid: "68418946"
---
# <a name="sysdmdatabasereplicastates-azure-sql-database"></a>sys.dm_database_replica_states（Azure SQL 数据库）
...
|**synchronization_health**|**tinyint**|反映联接到可用性副本上的可用性组的数据库的同步状态与可用性副本的可用性模式 (同步提交或异步提交模式) 的交集, 其中一种是以下值。<br /><br /> 0 = 不正常。 数据库的**synchronization_state**为 0 (未同步)。<br /><br /> 1 = 部分正常。 如果**synchronization_state**为 1 (正在同步), 则将同步提交可用性副本上的数据库视为部分正常。<br /><br /> 2 = 正常。 如果**synchronization_state**为 2 (已同步), 则会将同步提交可用性副本上的数据库视为正常, 如果**synchronization_state**为 1 (正在同步)。|  
