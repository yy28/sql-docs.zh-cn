---
title: sys.databases _database_replica_states （Azure SQL 数据库） |Microsoft Docs
ms.custom: ''
ms.date: 05/22/2019
ms.prod: sql
ms.reviewer: ''
...
|列名|“名称”|说明（针对主副本）|  
|-----------------|---------------|----------------------------------------|  
|database_id|**smallint**|数据库的标识符。|  
|**group_id**|**ssNoversion**|数据库所属的可用性组的标识符。|  
|**replica_id**|**ssNoversion**|可用性组内可用性副本的标识符。|  
|**group_database_id**|**ssNoversion**|可用性组内数据库的标识符。 在此数据库联接到的每个副本上，该标识符都是相同的。|  
...
|**is_primary_replica**|**bit**|如果副本是主副本，则返回 1; 如果副本是数据库所属的可用性组中的辅助副本，则返回0。 这不是指分布式可用性组或活动异地复制关系中的主数据库或辅助数据库。<br /><br />适用范围：[!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] 到 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。|  
...
|**synchronization_health**|**tinyint**|反映联接到可用性副本上的可用性组的数据库的同步状态与可用性副本的可用性模式（同步提交或异步提交模式）的交集，其中一种是以下值。<br /><br /> 0 = 不正常。 数据库的**synchronization_state**为0（未同步）。<br /><br /> 1 = 部分正常。 如果**synchronization_state**为1（正在同步），则将同步提交可用性副本上的数据库视为部分正常。<br /><br /> 2 = 正常。 如果**synchronization_state**为2（已同步），则会将同步提交可用性副本上的数据库视为正常，如果**synchronization_state**为1（正在同步）。|  
|**synchronization_health_desc**|**nvarchar(60)**|可用性数据库的**synchronization_health**说明。<br /><br /> NOT_HEALTHY<br /><br /> PARTIALLY_HEALTHY<br /><br /> HEALTHY|  
...
|**database_state_desc**|**nvarchar(60)**|可用性副本的**database_state**说明。<br /><br /> ONLINE<br /><br /> RESTORING<br /><br /> RECOVERING<br /><br /> RECOVERY_PENDING<br /><br /> SUSPECT<br /><br /> EMERGENCY<br /><br /> OFFLINE<br /><br /> **注意：** 与 sys.databases 中的**state_desc**列相同。|  
...
|**last_hardened_time**|**datetime**|在辅助数据库上，最后强化的 LSN （**last_hardened_lsn**）的日志块标识符的时间。 在主数据库上，反映了与最小强制写入的 LSN 相对应的时间。|  
...
|**secondary_lag_seconds**|**bigint**|在同步期间，辅助副本在主副本后的秒数。<br /><br />适用范围：[!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] 到 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。|  
|**quorum_commit_lsn**|**numeric(25,0)**|标识为仅供参考。 不提供支持。 不保证以后的兼容性。|
|**quorum_commit_time**|**datetime**|标识为仅供参考。 不提供支持。 不保证以后的兼容性。|
