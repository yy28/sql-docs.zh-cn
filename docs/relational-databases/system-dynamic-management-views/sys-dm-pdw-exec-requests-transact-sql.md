...
|command|**恩瓦尔查尔 （4000）**|保存用户提交的请求的全文。|任何有效的查询或请求文本。 超过 4000 字节的查询将被截断。|  
...
|importance|**恩瓦尔查尔 （128）**|在"执行"时设置请求的重要性。  这是此工作负荷组和跨工作负荷组中共享资源的请求的相对重要性。  分类器中指定的重要性将覆盖工作负载组重要性设置。</br>适用于：Azure SQL 数据仓库|Null</br>low</br>below_normal</br>正常（默认）</br>above_normal</br>high|
...
|-**0 x 40**|由于估计结果集大小过大（> 100 万行），结果集缓存被禁用。|  
