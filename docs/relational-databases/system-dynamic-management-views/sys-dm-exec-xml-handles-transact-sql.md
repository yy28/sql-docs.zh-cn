...
|**dormant_duration_ms**|**bigint**|自上次 OPENXML 调用以来经过的时间（毫秒）。 如果尚未调用 OPENXML，返回的毫秒数**sp_xml_preparedocumen**t 调用。|  
