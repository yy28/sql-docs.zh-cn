...
> [!NOTE] 
> 若要从 **[!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)] 或 [!INCLUDE[ssPDW](../../includes/sspdw-md.md)]** 中调用此名称，请使用名称**sys.databases. dm_pdw_nodes_os_wait_stats**。  
  
|列名|数据类型|描述|  
|-----------------|---------------|-----------------|  
|wait_type|**nvarchar(60)**|等待类型的名称。 有关详细信息，请参阅本主题后面的 "[等待类型](#WaitTypes)"。|  
...
-   外部进程完成。  
 ...
 在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 等待时间计数器是**bigint**值，因此在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]早期版本中不像等效的计数器那样容易作为计数器滚动更新。  
...
|BACKUP_OPERATOR |当任务正在等待磁带装入时出现。 若要查看磁带状态，请查询 sys.databases. dm_io_backup_tapes。 如果装入操作没有挂起，则该等待类型可能指示磁带机发生硬件问题。| 
...
|BMPALLOCATION |在同步在批处理模式运算符内进入不同处理阶段的线程时出现。 <br />**适用于**：[!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] 及更高版本。| 
|BMPBUILD |在同步在批处理模式运算符内进入不同处理阶段的线程时出现。 <br />**适用于**：[!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] 及更高版本。| 
|BMPREPARTITION |在同步在批处理模式运算符内进入不同处理阶段的线程时出现。 <br /> **适用于**：[!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] 及更高版本。| 
|BMPREPLICATION |在同步在批处理模式运算符内进入不同处理阶段的线程时出现。 <br /> **适用于**：[!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] 及更高版本。| 
|BPSORT |在同步在批处理模式运算符内进入不同处理阶段的线程时出现。 <br /> **适用于**：[!INCLUDE[ssSQL15_md](../../includes/sssql15-md.md)] 及更高版本。| 
...
|CXCONSUMER |当使用者线程等待制造者线程发送行时，在并行查询计划中发生。 这是并行查询执行的正常部分。 <br /> **适用于**： [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] （从 [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] SP2 开始，[!INCLUDE[ssSQL17](../../includes/sssql17-md.md)] CU3） [!INCLUDE[ssSDS](../../includes/sssds-md.md)]|
|CXPACKET |在同步查询处理器交换迭代器时，以及在生成和使用行时，在并行查询计划中发生。 如果等待太久，无法通过优化查询（如添加索引）来减少等待时间，请考虑调整并行度的开销阈值或降低并行度。<br /> **注意：** 从 [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] SP2、[!INCLUDE[ssSQL17](../../includes/sssql17-md.md)] CU3 和 [!INCLUDE[ssSDS](../../includes/sssds-md.md)]开始，CXPACKET 仅指同步查询处理器交换迭代器，并为使用者线程生成行。 在 CXCONSUMER 等待类型中单独跟踪使用者线程。| 
...
|EXTERNAL_SCRIPT_NETWORK_IO |仅限内部使用。 <br /> **适用范围**： [!INCLUDE[ssSQLv14_md](../../includes/sssqlv14-md.md)] 至当前。| 
...
|FOREIGN_REDO |仅限内部使用。 <br /> **适用范围**： [!INCLUDE[ssSQLv14_md](../../includes/sssqlv14-md.md)] 至当前。| 
...
|HTBUILD |在同步在批处理模式运算符内进入不同处理阶段的线程时出现。 <br /> **适用于**：[!INCLUDE[ssSQL12](../../includes/sssql11-md.md)] 及更高版本。| 
|HTDELETE |在同步在批处理模式运算符内进入不同处理阶段的线程时出现。 <br /> **适用于**：[!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] 及更高版本。| 
|HTMEMO |在同步在批处理模式运算符内进入不同处理阶段的线程时出现。 <br /> **适用于**：[!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] 及更高版本。| 
|HTREINIT |在同步在批处理模式运算符内进入不同处理阶段的线程时出现。 <br /> **适用于**：[!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] 及更高版本。| 
|HTREPARTITION |在同步在批处理模式运算符内进入不同处理阶段的线程时出现。 <br /> **适用于**：[!INCLUDE[ssSQL12](../../includes/sssql11-md.md)] 及更高版本。| 
...
|LATCH_DT |等待 DT（破坏）闩锁时出现。 它不包括缓冲区闩锁或事务标记闩锁。 Dm_os_latch_stats 中提供了闩锁\_\* 等待列表。 请注意，sys.dm_os_latch_stats 将 LATCH_NL、LATCH_SH、LATCH_UP、LATCH_EX 以及 LATCH_DT 等待分到一组。| 
|LATCH_EX |等待 EX（排他）闩锁时出现。 它不包括缓冲区闩锁或事务标记闩锁。 Dm_os_latch_stats 中提供了闩锁\_\* 等待列表。 请注意，sys.dm_os_latch_stats 将 LATCH_NL、LATCH_SH、LATCH_UP、LATCH_EX 以及 LATCH_DT 等待分到一组。| 
|LATCH_KP |等待 KP（保持）闩锁时出现。 它不包括缓冲区闩锁或事务标记闩锁。 Dm_os_latch_stats 中提供了闩锁\_\* 等待列表。 请注意，sys.dm_os_latch_stats 将 LATCH_NL、LATCH_SH、LATCH_UP、LATCH_EX 以及 LATCH_DT 等待分到一组。| 
|LATCH_NL |标识为仅供参考。 不提供支持。 不保证以后的兼容性。| 
|LATCH_SH |等待 SH（共享）闩锁时出现。 它不包括缓冲区闩锁或事务标记闩锁。 Dm_os_latch_stats 中提供了闩锁\_\* 等待列表。 请注意，sys.dm_os_latch_stats 将 LATCH_NL、LATCH_SH、LATCH_UP、LATCH_EX 以及 LATCH_DT 等待分到一组。| 
|LATCH_UP |等待 UP（更新）闩锁时出现。 它不包括缓冲区闩锁或事务标记闩锁。 Dm_os_latch_stats 中提供了闩锁\_\* 等待列表。 请注意，sys.dm_os_latch_stats 将 LATCH_NL、LATCH_SH、LATCH_UP、LATCH_EX 以及 LATCH_DT 等待分到一组。| 
...
|METADATA_LAZYCACHE_RWLOCK |仅限内部使用。 <br /> **适用**于：仅 [!INCLUDE[ssKilimanjaro_md](../../includes/sskilimanjaro-md.md)]。 |  
...
|PREEMPTIVE_AUDIT_ACCESS_EVENTLOG |在 [!INCLUDE[ssNoVersion_md](../../includes/ssnoversion-md.md)] 操作系统 (SQLOS) 计划程序切换到抢先模式时发生，以便将审核事件写入 Windows 事件日志。 <br /> **适用**于：仅 [!INCLUDE[ssKilimanjaro_md](../../includes/sskilimanjaro-md.md)]。 |  
|PREEMPTIVE_AUDIT_ACCESS_SECLOG |在 SQLOS 计划程序切换到抢先模式时发生，以便将审核事件写入 Windows 安全日志。 <br /> **适用**于：仅 [!INCLUDE[ssKilimanjaro_md](../../includes/sskilimanjaro-md.md)]。 |  
...
|PREEMPTIVE_OS_SQMLAUNCH |仅限内部使用。 <br /> **适用于**： [!INCLUDE[ssKilimanjaro_md](../../includes/sskilimanjaro-md.md)] 到 [!INCLUDE[ssSQL15_md](../../includes/sssql15-md.md)]。 |  
...
|QUERY_ERRHDL_SERVICE_DONE |标识为仅供参考。 不提供支持。 <br /> **适用**于：仅 [!INCLUDE[ssKilimanjaro_md](../../includes/sskilimanjaro-md.md)]。 |  
|QUERY_WAIT_ERRHDL_SERVICE |标识为仅供参考。  不提供支持。 <br /> **适用**于：仅 [!INCLUDE[ssKilimanjaro_md](../../includes/sskilimanjaro-md.md)]。  |  
...
|RESOURCE_SEMAPHORE_SMALL_QUERY |当由于存在其他并发查询而无法立即批准较小查询的内存请求时出现。 等待时间不应超过几秒钟，因为如果服务器无法在几秒钟内给予请求的内存，则会将请求传输到主查询内存池中。 等待时间较长可能指示当主内存池被等待的查询阻塞时并发小查询的数量过多。 <br /> **适用**于：仅 [!INCLUDE[ssKilimanjaro_md](../../includes/sskilimanjaro-md.md)]。 |  
...
|SOS_LOCALALLOCATORLIST |在 [!INCLUDE[ssNoVersion_md](../../includes/ssnoversion-md.md)] 内存管理器中进行内部同步期间出现。 <br /> **适用**于：仅 [!INCLUDE[ssKilimanjaro_md](../../includes/sskilimanjaro-md.md)]。 |  
...
|SOS_RESERVEDMEMBLOCKLIST |在 SQL Server 内存管理器中进行内部同步期间出现。 <br /> **适用**于：仅 [!INCLUDE[ssKilimanjaro_md](../../includes/sskilimanjaro-md.md)]。 |  
...
|SQLTRACE_BUFFER_FLUSH |当某任务正在等待后台任务将跟踪缓冲区每隔四秒刷新到磁盘时出现。 <br /> **适用**于：仅 [!INCLUDE[ssKilimanjaro_md](../../includes/sskilimanjaro-md.md)]。 |  
...
|SQLTRACE_LOCK |仅限内部使用。 <br /> **适用**于：仅 [!INCLUDE[ssKilimanjaro_md](../../includes/sskilimanjaro-md.md)]。 |  
...
|WAIT_ON_SYNC_STATISTICS_REFRESH |在等待同步统计信息更新完成后，查询编译和执行才能恢复之前发生。<br /> 适用范围：从  **开始**[!INCLUDE[sql-server-2019](../../includes/sssqlv15-md.md)]|
...
|XE_PACKAGE_LOCK_BACKOFF |标识为仅供参考。 不提供支持。 <br /> **适用**于：仅 [!INCLUDE[ssKilimanjaro_md](../../includes/sskilimanjaro-md.md)]。 |  
