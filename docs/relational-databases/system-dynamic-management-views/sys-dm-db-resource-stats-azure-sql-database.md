...
|avg_log_write_percent|**小数 (5,2)**|平均事务日志写入百分比表示的服务层限制的次数 （以 MBps)。|  
|avg_memory_usage_percent|**小数 (5,2)**|平均内存使用率（以服务层限制的百分比表示）。<br /><br /> 这包括用于缓冲池页数和存储内存中 OLTP 对象的内存。|  
...
|avg_instance_cpu_percent|**小数 (5,2)**|以百分比表示的 CPU 使用率的平均数据库。|
|avg_instance_memory_percent|**小数 (5,2)**|以百分比表示的平均数据库内存使用量。|
|avg_login_rate_percent|**小数 (5,2)**|标识为仅供参考。 不提供支持。 不保证以后的兼容性。|
