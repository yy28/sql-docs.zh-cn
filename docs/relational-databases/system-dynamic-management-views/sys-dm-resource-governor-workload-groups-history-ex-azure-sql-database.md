...
|**group_id**| ssNoversion |工作负荷组的 ID。 不可为 null。|
|**名称**| nvarchar(256) |工作负荷组的名称。 不可为 null。|
|**snapshot_time**| DATETIME |拍摄的资源组统计信息快照的日期时间。|
|**duration_ms**| ssNoversion |当前和以前的快照之间的持续时间。|
|**active_worker_count**| ssNoversion |当前快照中的总工作线程。|
|**active_request_count**| ssNoversion |当前请求计数。 不可为 null。|
|**active_session_count**| ssNoversion |当前快照中的活动会话总数。|
|**total_request_count**| BIGINT |工作负荷组中已完成请求的累计计数。 不可为 null。|
|**delta_request_count**| ssNoversion |自上一个快照以来工作负荷组中的已完成请求的计数。 不可为 null。|
|**total_cpu_usage_ms**| BIGINT |此工作负荷组的累计 CPU 使用情况，以毫秒为单位。 不可为 null。|
|**delta_cpu_usage_ms**| ssNoversion |自上一个快照以来的毫秒的 CPU 使用率。 不可为 null。|
|**delta_cpu_usage_preemptive_ms**| ssNoversion |抢先式的 win32 调用不管理方法是 SQL CPU RG 自上次快照。|
|**delta_reads_reduced_memgrant_count**| ssNoversion |自上一个快照以来达到的最大查询大小限制的内存授予数。 不可为 null。|
|**reads_throttled**| ssNoversion |限制读取的总次数。|
|**delta_reads_queued**| ssNoversion |自上一个快照以来的读取 IOs 排入队列的总数。 可以为 Null。 如果没有为 IO 调控资源组，则为 null。|
|**delta_reads_issued**| ssNoversion |读取自上一个快照以来发出的 Io 总数。 可以为 Null。 如果没有为 IO 调控资源组，则为 null。|
|**delta_reads_completed**| ssNoversion |读取自上一个快照以来完成的 Io 总数。 不可为 null。|
|**delta_read_bytes**| BIGINT |自上一个快照以来读取的字节总数。 不可为 null。|
|**delta_read_stall_ms**| ssNoversion |读的 IO 到达和完成自上一个快照之间 （以毫秒为单位） 的总时间。 不可为 null。|
|**delta_read_stall_queued_ms**| ssNoversion |读 IO 到达和发出自上一个快照之间 （以毫秒为单位） 的总时间。 可以为 Null。 如果没有为 IO 调控资源组，则为 null。 非零 delta_read_stall_queued_ms 意味着 IO 受 RG。|
|**delta_writes_queued**| ssNoversion |自上一个快照以来的写入 IOs 排入队列的总数。 可以为 Null。 如果没有为 IO 调控资源组，则为 null。|
|**delta_writes_issued**| ssNoversion |写入自上一个快照以来发出的 Io 总数。 可以为 Null。 如果没有为 IO 调控资源组，则为 null。|
|**delta_writes_completed**| ssNoversion |写入自上一个快照以来完成的 Io 总数。 不可为 null。|
|**delta_writes_bytes**| BIGINT |自上一个快照以来写入的字节总数。 不可为 null。|
|**delta_write_stall_ms**| ssNoversion |写 IO 到达和完成自上一个快照之间 （以毫秒为单位） 的总时间。 不可为 null。|
|**delta_background_writes**| ssNoversion |自上一个快照以来所执行的后台任务的总写入。|
...
|**max_concurrent_request**| ssNoversion |并发请求最大数的当前设置。 不可为 null。|
|**max_io**| ssNoversion |组的的最大 IO 限制。|
|**max_global_io**| ssNoversion |标识为仅供参考。 不提供支持。 不保证以后的兼容性。
|**max_queued_io**| ssNoversion |标识为仅供参考。 不提供支持。 不保证以后的兼容性。|
|**max_log_rate_kb**| BIGINT |在资源组级别的最大日志速率 （每秒千字节）。|
|**max_session**| ssNoversion |会话组的限制。|
|**max_worker**| ssNoversion |辅助角色组的限制。|
|||
