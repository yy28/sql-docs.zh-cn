...
> [!NOTE]  
>  若要从我们称之为[!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)]或[!INCLUDE[ssPDW](../../includes/sspdw-md.md)]，使用名称**sys.dm_pdw_nodes_db_file_space_usage**。  
  
|列名|数据类型|Description|  
|-----------------|---------------|-----------------|  
|database_id|**int**|数据库 ID。|  
|file_id|**int**|文件 ID。<br /><br /> file_id 映射到在 file_id [sys.dm_io_virtual_file_stats](../../relational-databases/system-dynamic-management-views/sys-dm-io-virtual-file-stats-transact-sql.md)和中 fileid [sys.sysfiles](../../relational-databases/system-compatibility-views/sys-sysfiles-transact-sql.md)。|  
|filegroup_id|**int**|**适用范围**： [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] 到 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。<br /><br /> 文件组 ID。|  
...
|modified_extent_page_count|**bigint**|**适用范围**：[!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] SP2 到 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。<br /><br />在中修改的页的总数分配自上次完整数据库备份后的文件的范围。 已修改的页计数可以用于跟踪自上次完整备份，以决定是否需要差异的备份后的数据库中的差异更改量。|
...
### <a name="determing-the-amount-of-free-space-in-tempdb"></a>确定 tempdb 中的可用空间量  
 下面的查询返回的可用页和总可用空间总数以兆字节 (MB) 可用中的所有文件中**tempdb**。  
