...
|request_id|**ssNoversion**|确定目标请求。 引用 dm_exec_sessions.request_id。|  
...
|node_id|**ssNoversion**|标识查询树中的运算符节点。|  
|thread_id|**ssNoversion**|区分属于同一个查询运算符节点的线程（针对并行查询）。|  
...
|object_id|**ssNoversion**|要对其进行读写的对象的标识符。 引用 sys.objects.object_id。|  
|index_id|**ssNoversion**|打开其行级的索引（如果有）。|  
...
|segment_read_count|**ssNoversion**|迄今为止的段预读数。|  
|segment_skip_count|**ssNoversion**|迄今为止跳过的段数。| 
