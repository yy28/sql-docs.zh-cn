...
## <a name="remarks"></a>备注  
 如果数据库不使用特定版本可能会限制任何功能，则视图返回任何行。  
...
-   **对分区。** 指示数据库包含已分区表、已分区索引、分区方案或分区函数。 若要使数据库能够移动到 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Enterprise 或 Developer 以外的版本，将表修改到单个分区上是不够的。 必须删除相应的已分区表。 如果该表包含数据，请使用 SWITCH PARTITION 将每个分区转换成无分区表。 然后删除已分区表、分区方案和分区函数。  
...
> [!NOTE]
> 从开始[!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]Service Pack 1，这些功能都可跨多个[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]版本，并不只局限于 Enterprise 或 Developer Edition。
