...
|**row_overflow_fetch_in_pages**|**bigint**|从 ROW_OVERFLOW_DATA 分配单元检索到的行溢出数据页数的累积计数。<br /><br /> 这些页包含存储在类型为**varchar （n）**、 **nvarchar （n）**、 **varbinary （n）** 和**sql_variant**的数据，这些列已被推送到行外。|    
