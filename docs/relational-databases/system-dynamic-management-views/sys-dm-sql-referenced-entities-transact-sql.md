...
|referenced_id|**int**|被引用的实体的 ID。 当 referenced_minor_id 不为 0 时，referenced_id 是该列被定义时所在的实体。<br /><br /> 对于跨服务器的引用，此列始终为 NULL。<br /><br /> 对于跨数据库的引用，当由于数据库脱机或实体无法绑定而无法确定 ID 时，此列为 NULL。<br /><br /> 对于数据库内的引用，如果无法确定 ID，则为 NULL。 对于非绑定到架构的引用，在数据库中或依赖于调用方的名称解析时，被引用的实体不存在时，无法解析 ID。  在后一种情况下，is_caller_dependent 设置为 1。<br /><br /> 对于绑定到架构的引用，此列永远不会为 NULL。|  
