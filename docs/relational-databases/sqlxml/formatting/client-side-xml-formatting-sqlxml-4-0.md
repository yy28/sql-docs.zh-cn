...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]本主题提供有关客户端的 XML 格式设置信息。 客户端格式化是指对中间层上的 XML 的格式化。  
 ...
 **重要**若要使用与新的客户端 FOR XML 功能**xml**数据类型，应始终使用客户端[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]本机客户端 (SQLNCLI11) 数据提供程序，而不是 SQLOLEDB 访问接口。 SQLNCLI11 为 SQL Server 访问接口的最新版本，且完全识别 [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)] 中引入的数据类型。 FOR XML 与 SQLOLEDB 访问接口将处理的客户端的行为**xml**以字符串形式的数据类型。  
...
> [!NOTE]  
>  当使用 SQLXMLOLEDB Provider 或 SQLXML 托管类时，你可以使用**ClientSideXml**属性来请求客户端的 XML 格式。  
