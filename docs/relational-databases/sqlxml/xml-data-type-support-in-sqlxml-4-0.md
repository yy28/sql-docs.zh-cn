...
-   使用**xml**中不支持其中的批注的数据类型列 (如**sql:relationship**和**sql:key-字段**) 或允许的将导致[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]实现 SQLXML 4.0 的中间层组件将无法捕获的错误。 其原因在于 SQLXML 不要求 SQL 类型信息。 这类似于其他数据类型（如 BLOB 和二进制类型）的 SQLXML 的行为。  
  
-   映射**xml**列仅支持 XSD 架构。 XDR 架构不支持映射**xml**列。  
