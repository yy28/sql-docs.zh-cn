...
**批模式执行**  
批模式执行是一种查询处理方法，用于统一处理多个行。 批模式执行与列存储存储格式紧密集成，并且围绕列存储存储格式进行了优化。 批模式执行有时候称作基于向量或向量化的执行。 对列存储索引的查询使用批处理模式执行，这通常可将查询性能提高 2-4 倍。 有关执行模式的详细信息，请参阅[查询处理体系结构指南](../query-processing-architecture-guide.md#execution-modes)。 
