...
[!INCLUDE[tsql-appliesto-ss2012-all_md](../../includes/tsql-appliesto-ss2012-all-md.md)]
  ...
  *列存储索引* 是存储和查询大型数据仓库事实数据表的标准。 它使用基于列的数据存储和查询处理，与传统的面向行的存储相比，可对数据仓库最多提高 **10 倍查询性能**，与使用非压缩数据大小相比，可提供多达 **10 倍数据压缩率**。 从 [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] 开始，列存储索引支持操作分析，这是一种针对事务工作负荷运行高性能实时分析的功能。  
...
-   使用聚集列存储索引来存储数据仓库工作负载的事实数据表和大型维度表。 这样可将查询性能和数据压缩率最多提高 10 倍。 请参阅 [Columnstore Indexes for Data Warehousing](../Topic/Columnstore%20Indexes%20for%20Data%20Warehousing.md)。  
...
## 相关任务  
 所有关系表（除非指定为非聚集列存储索引）使用行存储作为基础数据格式。 除非指定了 WITH CLUSTERED COLUMNSTORE INDEX 选项，否则 CREATE TABLE 将创建行存储表。  
  
 当你使用 CREATE TABLE 语句创建表时，可以通过指定 WITH CLUSTERED COLUMNSTORE INDEX 选项将表创建为列存储。 如果你已有一个行存储表并想要将其转换为列存储，可以使用 CREATE COLUMNSTORE INDEX 语句。 有关示例，请参阅。  
...
|为数据仓库创建高性能列存储索引。|[针对数据仓库的列存储索引](../Topic/Columnstore%20Indexes%20for%20Data%20Warehousing.md)|介绍如何使用列存储表上的 btree 索引来创建高性能数据仓库查询。|  
|使用 btree 索引对列存储索引强制主键约束。|[针对数据仓库的列存储索引](../Topic/Columnstore%20Indexes%20for%20Data%20Warehousing.md)|演示如何合并 btree 和列存储索引，以便对列存储索引强制主键约束。|  
...
|将数据载入列存储索引|[列存储索引数据加载](../Topic/Columnstore%20Indexes%20Data%20Loading.md)||  
