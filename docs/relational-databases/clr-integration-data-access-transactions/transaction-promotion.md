...
## <a name="distributed-transactions"></a>分布式事务  
 分布式事务通常会使用大量的系统资源。 
  [!INCLUDE[msCoName](../../includes/msconame-md.md)] 分布式事务处理协调器 (MS DTC) 会管理此类事务，并集成在这些事务中访问的所有资源管理器。 另一方面，事务升级是一种特殊形式的**系统**事务事务，它可以有效地将工作委托给简单[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]的事务。 **SqlClient** [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] **和协调**处理事务时涉及到的工作，根据需要将其提升为完整的分布式事务。  
