...
|None|**Cache**|**Cache** 是一个专门的 **Spool** 运算符。 它仅存储一行数据。 **Cache** 是一个逻辑运算符。 **中不使用** Cache [!INCLUDE[ssCurrent](../includes/sscurrent-md.md)]。| 
...
|![RRemote Index Seek Showplan 运算符](../relational-databases/media/remote-index-scan-32x.gif "Remote Index Seek Showplan 运算符")|**Remote Index Scan**|**Remote Index Scan** 运算符可以扫描在 Argument 列中指定的远程索引。 **Remote Index Scan** 既是一个逻辑运算符，也是一个物理运算符。| 
