...
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]
  ...
  在 sqlncli.msi 中定义的**SSVARIANT**结构对应于 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] NATIVE Client OLEDB 访问接口中的 DBTYPE_SQLVARIANT 值。  
...
|cyMoneyVal|DBTYPE_CY|**LARGE_INTEGER**|**VT_SS_MONEY VT_SS_SMALLMONEY**|支持**货币**和**smallmoney**[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 数据类型。|  
...
|Time2Val|DBTYPE_DBTIME2|**DBTIME2**|**VT_SS_TIME2**|支持[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 数据类型的**时间**。<br /><br /> 包括以下成员：<br /><br /> tTime2Val (DBTIME2)<br /><br /> *bScale* （**BYTE**）指定*tTime2Val*值的小数位数。|  
...
|NCharVal|没有对应的 OLE DB 类型指示器。|**struct _NCharVal**|**VT_SS_WVARSTRING、**<br /><br /> **VT_SS_WSTRING**|支持[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 数据类型的**nchar**和**nvarchar** 。<br /><br /> 包括以下成员：<br /><br /> *sActualLength* （**SHORT**）指定*pwchNCharVal*指向的字符串的实际长度。 不包括尾零。<br /><br /> *sMaxLength* （**SHORT**）指定*pwchNCharVal*指向的字符串的最大长度。<br /><br /> *pwchNCharVal* （**WCHAR** \*）指向字符串的指针。<br /><br /> 未使用的成员： *rgbReserved*、 *dwReserved*和*pwchReserved*。|  
