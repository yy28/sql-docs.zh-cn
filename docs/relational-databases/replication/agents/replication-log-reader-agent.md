 ...
 **-Distributor** *server_name*[**\\***instance_name*]  
 分发服务器名称。 指定 *server_name* 的默认实例为 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 在该服务器上。 指定 *server_name***\\***instance_name* 对于命名实例的 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 在该服务器上。  
 ...
 **-DistributorSecurityMode** [ **0**| **1**]  
 指定分发服务器的安全模式。 值为 **0** 指示 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 身份验证模式 （默认值），并将值 **1** 指示 [!INCLUDE[msCoName](../../../includes/msconame-md.md)] Windows 身份验证模式。  
 ...
 **-ReadBatchThreshold** *number_of_commands*  
 在复制命令由分发代理发送给订阅服务器之前，从事务日志读取的复制命令的数目。 默认值为 0。 如果未指定此参数，日志读取器代理将读取到日志的结尾或中指定的数字 **-ReadBatchSize** （事务的数目）。  
  
 **-RecoverFromDataErrors**  
 指定日志读取器代理在从非 SQL Server 发布服务器发布的列数据中遇到错误时应继续运行。 默认情况下，这类错误可导致日志读取器代理失败。 当您使用 **-RecoverFromDataErrors**, 、 出错的列数据被复制为 NULL 或者适当的非空值，并且警告消息记录到 [MSlogreader_history](../../../relational-databases/system-tables/mslogreader-history-transact-sql.md) 表。 仅 Oracle 发布服务器支持此参数。  
