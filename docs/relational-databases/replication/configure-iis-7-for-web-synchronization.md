...
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-xxx_md](../../includes/tsql-appliesto-ss2008-xxxx-xxxx-xxx-md.md)]
  ...
  本主题中的过程将指导您完成手动配置的过程 [!INCLUDE[msCoName](../../includes/msconame-md.md)] Internet 信息服务 (IIS) 版本 7 及更高的合并复制使用 Web 同步。 
  
 配置 IIS 7 或更高版本是启用 Web 同步所需的三个步骤的第一个。  
  
 整个配置过程的概述，请参阅 [配置 Web 同步](../../relational-databases/replication/configure-web-synchronization.md)。  
...
从版本 5.0 开始，在 IIS 上支持 web 同步。 配置 Web 同步向导的 IIS 版本 5 和 6，不可用于 IIS 7.0 和更高版本。 **开始使用 SQL Server 2012、 IIS 服务器上使用 web 同步组件应在复制中安装 SQL Server。 这可能是免费的 SQL Server Express edition。**
  
#### 安装和配置 SQL Server 复制侦听器  
  
1.  在 IIS 计算机上安装 SQL Server 复制。

2. 在正在运行 IIS 的计算机上为 replisapi.dll 创建一个新的文件目录。 只要您希望，但我们建议您创建的目录下，您可以创建目录 \<*驱动器*>: \Inetpub 目录。 例如，创建目录 \<*驱动器*>: \Inetpub\SQLReplication\\。  
...
6.  在 IIS 上创建一个虚拟目录。 应在步骤 4 所创建的网站下创建虚拟目录，并将其映射到步骤 1 中创建的目录。 建议尽量限制为此目录分配权限。 您必须至少选择 **“读取”** 权限和 **“执行”** 权限。  
    ...
    1.  在 **Internet 信息服务 (IIS) 管理器**, ，请在 **连接** 窗格中，右键单击 **Default Web Site**, ，然后选择 **添加虚拟目录**。  
  
    2.  对于“别名” ，请输入 **SQLReplication**。  
  
    3.  有关 **物理路径**, ，输入 **\< 驱动器>: \Inetpub\SQLReplication\\**, ，然后单击 **确定**。  
  
7.  配置 IIS 以允许执行 replisapi.dll。  
  
    1.  在 **Internet 信息服务 (IIS) 管理器**, ，请单击 **Default Web Site**。  
    ...
    6.  有关 **可执行文件**, ，输入 **\< 驱动器>: \Inetpub\SQLReplication\replisapi.dll**。  
    ...
    10. 检查 Replisapi.dll 是否列在 **“已启用”** 处理程序映射下。 如果该文件在 **禁用** 列表，请右键单击 Replisapi 条目，然后单击 **编辑功能权限**。 选中 **“执行”** 框，然后单击 **“确定”**。  
...
1.  在 **Internet 信息服务 (IIS) 管理器**, ，展开本地服务器节点，然后单击 **Default Web Site** （或您的 Web 同步站点如果不同于默认网站）。  
...
1.  在 **Internet 信息服务 (IIS) 管理器**, 、 单击本地服务器节点，然后在中心窗格中，双击 **服务器证书**。  
  
2.  在 **操作** 窗格中，单击 **创建自签名证书**。  
  
3.  在 **创建自签名证书** 对话框中，输入该证书的名称，然后单击 **确定**。  
  
###### 将证书绑定到网站  
  
1.  在 **连接** 窗格中，单击 **Default Web Site** （或 Web 同步站点，如果不同于默认网站）。  
  
2.  在 **“操作”** 窗格中，依次单击 **“绑定”**和 **“添加”**。 **“添加网站绑定”** 对话框将出现。  
  
3.  从 **类型** 下拉列表中，选择 **https**。 保留 **“IP 地址”** 和 **“端口”**的默认设置。  
  
4.  从 **SSL 证书** 下拉列表中，选择"以创建用于测试，自签名的证书"中创建该证书单击 **确定**, ，然后单击 **关闭**。  
  
###### 测试证书  
  
1.  在 **Internet 信息服务 (IIS) 管理器**, ，请单击 **默认 Web 站点。**  
  
2.  从 **操作** 窗格中，单击 **浏览 \*: 443(https)**。  
  
3.  Internet Explorer 随即打开并显示一条消息“此网站的安全证书有问题。”。 此警告向您表明关联的证书不是由认可的 CA 颁发的，因此可能不值得信任。 这是预期的警告，因此请单击 **继续访问此网站 （不推荐）**。  
...
1.  在运行 IIS 的计算机上创建一个本地帐户：  
    ...
    1.  打开 **“服务器管理器”**。 从开始菜单中，右键单击 **计算机**, ，然后单击 **管理**。  
...
2.  将帐户添加到 IIS_IUSRS 组：  
    ...
    2.  用鼠标右键单击 **IIS_IUSRS**, ，然后单击 **将添加到组**。  
  
    3.  在 **IIS_IUSRS 属性** 对话框中，单击 **添加**。  
  
    4.  在 **“选择用户、计算机或组”** 对话框中，添加在步骤 1 中创建的帐户。  
  
    5.  验证 **从此位置** 显示本地计算机 （而不是域） 的名称。 如果此字段不显示本地计算机名称，请单击 **“位置”**。 在 **“位置”** 对话框中，选择本地计算机，然后单击 **“确定”**。  
  
    6.  在 **选择用户** 对话框和 **IIS_IUSRS 属性** 对话框中，单击**确定**。  
  
3.  对包含 replisapi.dll 的文件夹授予最低帐户权限：  
  
    1.  在 Windows 资源管理器中，右键单击为 replisapi.dll 创建的文件夹，然后单击 **属性**。  
  
    2.  在 **“安全性”** 选项卡上，单击 **“编辑”**。  
  
    3.  在 **权限 \< 文件夹名称>** 对话框中，单击 **添加** 添加在步骤 1 中创建该帐户。  
  
    4.  验证 **从此位置** 显示本地计算机 （而不是域） 的名称。 如果此字段不显示本地计算机名称，请单击 **“位置”**。 在 **“位置”** 对话框中，选择本地计算机，然后单击 **“确定”**。  
  
    5.  验证是否仅授予该帐户 **读取**, ，**读取和执行**, ，和 **列出文件夹内容** 权限。  
  
    6.  选择所有不需要访问该目录的用户或组，然后依次单击 **“删除”**和 **“确定”**。  
  
4.  创建应用程序池在 **Internet 信息服务 (IIS) 管理器**:  
  
    1.  在 **Internet 信息服务 (IIS) 管理器**, ，请在 **连接** 窗格中，展开本地服务器节点。  
  
    2.  用鼠标右键单击 **应用程序池**, ，然后单击 **添加应用程序池**。  
  
    3.  输入应用程序池的名称，保留其余字段中的默认值，然后单击 **“确定”**。  
  
    > [!NOTE]  
    >  如果您预计并发同步客户端超过两个，则最好创建 Web 园。 有关详细信息，请参阅"创建 Web 园" [配置 Web 同步](../../relational-databases/replication/configure-web-synchronization.md)。  
  
5.  将帐户与应用程序池关联：  
  
    1.  在 **Internet 信息服务 (IIS) 管理器**, ，展开本地服务器节点，然后单击 **应用程序池**。  
  
    2.  右键单击该应用程序池，您创建的然后单击 **设置应用程序池默认设置**。  
...
6.  将应用程序池与复制网站关联：  
    ...
    4.  从 **应用程序池** 下拉列表中，选择在步骤 4 中创建的应用程序池，然后单击 **确定**。  
  
    5.  再次单击 **“确定”** ，关闭“高级设置”。  
  
## 测试到 replisapi.dll 的连接  
 在诊断模式下，若要测试与运行 IIS 的计算机的连接，以确保正确安装安全套接字层 (SSL) 证书运行 Web 同步。 若要在诊断模式下运行 Web 同步，您必须是运行 IIS 的计算机中的管理员。  
...
2.  在订阅服务器上的 Internet Explorer 中，用向 replisapi.dll 的地址追加 `?diag` 的方法以诊断模式连接到服务器。 例如︰ **https://server.domain.com/directory/replisapi.dll?diag**。  
    ...
    > [!NOTE]  
    >  在上例中， **server.domain.com** 应使用的确切替换 **颁发给** 下会列出名称 **服务器证书** 部分在 IIS 管理器。  
