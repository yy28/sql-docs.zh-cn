...
-   若要禁用发布和分发，所有分发数据库和发布数据库都必须联机。 如果存在分发数据库或发布数据库的“数据库快照”  ，则在禁用发布和分发前，必须先删除这些数据库快照。 数据库快照是数据库的只读脱机副本，与复制快照无关。 有关详细信息，请参阅 [数据库快照 （&) #40;SQL Server 和 #41;](../../relational-databases/databases/database-snapshots-sql-server.md)。  
...
2.  在订阅数据库上的每个订阅服务器上，执行 [sp_removedbreplication](../../relational-databases/system-stored-procedures/sp-removedbreplication-transact-sql.md) 以从数据库删除复制对象。 此存储过程不会删除分发服务器上的复制作业。  
  
3.  在发布服务器上对发布数据库中，执行 [sp_removedbreplication](../../relational-databases/system-stored-procedures/sp-removedbreplication-transact-sql.md) 以从数据库删除复制对象。  
  
4.  如果发布服务器上使用远程分发服务器上，执行 [sp_dropdistributor](../../relational-databases/system-stored-procedures/sp-dropdistributor-transact-sql.md)。  
...
7.  在分发服务器上，执行 [sp_dropdistributor](../../relational-databases/system-stored-procedures/sp-dropdistributor-transact-sql.md) 若要从服务器中删除分发服务器指定。  
  
    > [!NOTE]  
    >  如果在执行前，将不会删除所有复制发布和分发对象 [sp_dropdistpublisher](../../relational-databases/system-stored-procedures/sp-dropdistpublisher-transact-sql.md) 和 [sp_dropdistributor](../../relational-databases/system-stored-procedures/sp-dropdistributor-transact-sql.md), ，这些过程将返回错误。 若要删除的发布服务器或分发服务器后，删除所有与复制相关的对象 **@no_checks** 参数必须设置为 **1**。 如果发布服务器或分发服务器脱机或无法访问， **@ignore_distributor** 参数可以设置为 **1** ，以便可以删除它们; 但是，任何发布和分发留在原地的对象，必须手动删除。  
...
3.  通过使用创建连接到分发服务器 <xref:Microsoft.SqlServer.Management.Common.ServerConnection> 类。  
  
4.  创建的实例 <xref:Microsoft.SqlServer.Replication.DistributionPublisher> 类。 指定 <xref:Microsoft.SqlServer.Replication.DistributionPublisher.Name%2A> 属性，并传递 <xref:Microsoft.SqlServer.Management.Common.ServerConnection> 步骤 3 中的对象。  
  
5.  （可选）调用 <xref:Microsoft.SqlServer.Replication.ReplicationObject.LoadProperties%2A> 方法获取对象的属性，并验证发布服务器上是否存在。 如果此方法返回 **false**，则步骤 4 中设置的发布服务器名称不正确，或者此分发服务器未使用该发布服务器。  
...
7.  创建的实例 <xref:Microsoft.SqlServer.Replication.ReplicationServer> 类。 传递 <xref:Microsoft.SqlServer.Management.Common.ServerConnection> 步骤 3 中的对象。  
