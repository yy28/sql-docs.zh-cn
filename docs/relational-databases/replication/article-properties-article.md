...
## 用于所有发布的选项  
 **复制表分区方案** 和 **复制索引分区方案**  
 [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] 引入了的表分区和索引分区功能，它们是不相关的分区复制到提供通过行和列筛选器。 **“复制表分区方案”** 选项和 **“复制索引分区方案”** 选项指定了是否应将分区方案复制到订阅服务器。 有关分区的详细信息，请参阅 [Partitioned Tables and Indexes](../../relational-databases/partitions/partitioned-tables-and-indexes.md)。  
 ...
 **在订阅服务器上创建架构**  
 [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] 引入了使用 CREATE SCHEMA 语句定义的架构。 架构是一个对象; 的所有者它用于多个部分组成的名称，如 \< 数据库>。 \< 架构>。 \< 对象>。 如果数据库中有非 DBO 架构拥有的对象，则复制功能可以在订阅服务器上创建这些架构，从而可以创建发布的对象。  
 ...
 **自动管理标识范围**  
 默认情况下，复制将管理发布服务器和各个订阅服务器上的所有标识列。 每个复制节点分配的标识值范围 (与指定 **发布服务器范围大小** 和 **订阅服务器范围大小** 选项) 以确保在只有一个节点使用给定的值。 有关详细信息，请参阅 [复制标识列](../../relational-databases/replication/publish/replicate-identity-columns.md)。  
  
## 用于事务发布的选项  
 **复制 INSERT、UPDATE 和 DELETE 存储过程**  
 如果在 **语句传递** 部分中的此对话框中，您选择要使用存储的过程将更改传播到订阅服务器 （默认值），请选择是否要将这些过程复制到每个订阅服务器。 如果选择 **False**，则必须手动复制这些过程，否则在尝试传递更改时，分发代理将失败。  
  
 **语句传递**  
 此部分中的选项适用于所有表，包括以表的形式复制的索引视图。 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 建议使用默认选项。 默认情况下，事务复制通过安装在每个订阅服务器上的一组存储过程，将更改传播到订阅服务器。 对发布服务器上的表进行插入、更新或删除操作时，该操作将转换为对订阅服务器上的存储过程的调用。  
...
-   **双向** （默认值）︰ 可以下载到订阅服务器上的更改，并将其上载到发布服务器。  
  
-   **仅下载到订阅服务器，禁止订阅服务器更改**︰ 更改可以下载到订阅服务器，但它们不能上载到发布服务器。 使用触发器可以防止在订阅服务器上进行更改。  
  
-   **仅下载到订阅服务器，允许订阅服务器更改**︰ 更改可以下载到订阅服务器，但它们不能上载到发布服务器。  
  
 有关详细信息，请参阅 [对 Download-Only 项目优化合并复制性能](../../relational-databases/replication/merge/optimize-merge-replication-performance-with-download-only-articles.md)。  
  
 **分区选项**  
 指定参数化筛选器创建的分区的类型。 有关详细信息，请参阅 [Parameterized Row Filters](../../relational-databases/replication/merge/parameterized-row-filters.md)的“设置‘分区选项’”部分。  
  
 **跟踪级别**  
 确定是否将同一行或同一列的更改视为冲突。  
  
 **验证 INSERT 权限**, ，**验证 UPDATE 权限**, ，和 **验证 DELETE 权限**  
 确定在同步期间是否对订阅服务器登录进行检查，以验证其是否具有对发布数据库中已发布表的 INSERT、UPDATE 或 DELETE 权限。 默认值是 **False** 因为合并复制不需要要向其授予; 这些权限控制对已发布表的访问通过发布访问列表 (PAL)。 有关 PAL 的详细信息，请参阅 [安全发布服务器](../../relational-databases/replication/security/secure-the-publisher.md)。  
...
### “冲突解决程序”选项卡  
 **使用默认冲突解决程序**  
 如果选择默认冲突解决程序，则解决冲突时会基于分配给每个订阅服务器的优先级，或基于写入到发布服务器的更改的先后次序，具体取决于所使用的订阅类型。 有关详细信息，请参阅 [Detect and Resolve Merge Replication Conflicts](../../relational-databases/replication/merge/detect-and-resolve-merge-replication-conflicts.md)。  
 ...
 **使用自定义冲突解决程序(已在分发服务器上注册)**  
 ...
 如果冲突解决程序需要任何输入信息，请在 **“输入冲突解决程序所需的信息”** 文本框中指定该信息。 输入所需的有关详细信息 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 自定义冲突解决程序，请参阅 [Microsoft 基于 COM 的冲突解决程序](../../relational-databases/replication/merge/microsoft-com-based-resolvers.md)。  
  
 **允许订阅服务器在按需同步时交互式解决冲突**  
 如果订阅服务器将使用按需同步（合并复制时的默认设置），并且您希望交互式地解决冲突，请选择此选项。 指定按需同步 **同步计划** 新建订阅向导页。 若要交互式地解决冲突，请使用交互式冲突解决程序用户界面。 有关详细信息，请参阅 [Interactive Conflict Resolution](../../relational-databases/replication/merge/interactive-conflict-resolution.md)。  
...
### “属性”选项卡  
 **复制 INSERT、UPDATE 和 DELETE 存储过程**  
 项目是否是在事务发布，且在 **语句传递** 部分中的此对话框中，您选择要使用存储的过程将更改传播到订阅服务器 （默认值），请选择是否要将这些过程复制到每个订阅服务器。 如果选择 **False**，则必须手动复制这些过程，否则在尝试传递更改时，分发代理将失败。  
  
 **目标对象所有者**  
 如果不输入值 **dbo**:  
  
-   如果运行 [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] 或更高版本的订阅服务器，则必须确保在订阅服务器上创建一个与所输入值同名的架构。 有关详细信息，请参阅 [CREATE SCHEMA & #40;Transact SQL & #41;](../../t-sql/statements/create-schema-transact-sql.md)。  
 ...
 **语句传递**  
 此部分中的选项适用于事务发布中的所有表。 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 建议使用默认选项。 默认情况下，事务复制通过安装在每个订阅服务器上的一组存储过程，将更改传播到订阅服务器。 对发布服务器上的表进行插入、更新或删除操作时，该操作将转换为对订阅服务器上的存储过程的调用。  
