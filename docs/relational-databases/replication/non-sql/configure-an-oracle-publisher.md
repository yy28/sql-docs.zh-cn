...
4.  在 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 分发服务器上将 Oracle 数据库配置为发布服务器。  
 ...
 可以从 Oracle 数据库复制的对象的列表，请参阅 [设计注意事项和限制为 Oracle 发布服务器](../../../relational-databases/replication/non-sql/design-considerations-and-limitations-for-oracle-publishers.md)。  
  
> [!NOTE]  
>  您必须是属于 **sysadmin** 固定的服务器角色以启用的发布服务器或分发服务器并创建 Oracle 发布或订阅从 Oracle 发布。  
...
> [!NOTE]  
>  删除 **MSSQLSERVERDISTRIBUTOR** 公共同义词和用配置的 Oracle 复制用户 **CASCADE** 选项从 Oracle 发布服务器中删除所有复制对象。  
  
 有一个示例脚本可以帮助建立 Oracle 复制用户架构。 脚本位于以下目录在安装后 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]: *\< 驱动器>*:\\files\microsoft SQL Server\\*\< 实例名>*\MSSQL\Install\oracleadmin.sql。 [Script to Grant Oracle Permissions](../../../relational-databases/replication/non-sql/script-to-grant-oracle-permissions.md)主题中也包括了此脚本。  
  
 使用具有 DBA 权限的帐户连接到 Oracle 数据库并执行此脚本。 此脚本将提示输入复制管理用户架构的用户名和密码以及用于创建对象的默认表空间（此表空间必须已存在于 Oracle 数据库中）。 有关指定对象的其他表空间的信息，请参阅 [管理 Oracle 表空间](../../../relational-databases/replication/non-sql/manage-oracle-tablespaces.md)。 可以任选用户名和强密码，但要将它们记下来，因为以后将 Oracle 数据库配置为发布服务器时会提示您输入此信息。 建议只将此架构用于复制所需的对象，而不要在此架构下创建要发布的表。  
...
5.  如果您遇到连接到 Oracle 数据库的问题，请参阅部分" [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 分发服务器无法连接到 Oracle 数据库实例"中 [排除 Oracle 发布服务器](../../../relational-databases/replication/non-sql/troubleshooting-oracle-publishers.md)。  
...
## 在 SQL Server 分发服务器上将 Oracle 数据库配置为发布服务器  
 Oracle 发布服务器总是使用远程分发服务器。您必须配置一个 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 实例以作为 Oracle 发布服务器的分发服务器（一个 Oracle 发布服务器只能使用一个分发服务器，但单个分发服务器可以为多个 Oracle 发布服务器提供服务）。 配置分发服务器后，通过 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]、Transact-SQL 或复制管理对象 (RMO)，在 [!INCLUDE[ssManStudioFull](../../../includes/ssmanstudiofull-md.md)] 分发服务器上将 Oracle 数据库实例标识为发布服务器。 有关配置分发服务器的详细信息，请参阅 [配置分发](../../../relational-databases/replication/configure-distribution.md)。  
...
>  Oracle 发布服务器不能与它的 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 分发服务器同名，也不能与任何使用同一分发服务器的 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 发布服务器同名。  
 ...
 在 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 分发服务器上标识 Oracle 发布服务器后，复制创建一个链接服务器，其名称与 Oracle 数据库的 TNS 服务名相同。 此链接服务器只能由复制使用。 如果您需要通过链接的服务器的连接以连接到 Oracle 发布服务器，创建另一个 TNS 服务名称，并在调用时，然后使用此名称 [sp_addlinkedserver & #40;Transact SQL & #41;](../../../relational-databases/system-stored-procedures/sp-addlinkedserver-transact-sql.md)。  
