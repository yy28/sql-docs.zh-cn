...
ms.topic: "article"
helpviewer_keywords: 
  - "Oracle 发布 [SQL Server 复制], 管理注意事项"
  - "管理复制, Oracle 发布"
...
### Oracle 导入  
 用 Oracle 导入，您可以设置的选项 **忽略** 为 y 或 n (默认值为 ' n ')。 如果 **忽略** 设置为 ' n '，表会删除并重新导入过程中创建。 这会删除复制触发器并禁用复制。 如果将 **ignore** 设置为“y”，导入将尝试将行加载到现有表中，这会激发复制触发器。 因此，用导入工具向复制的表导入时，确保将 **ignore** 设置为“y”。  
  
### SQL*Loader  
 使用 SQL\*加载程序，您可以设置的选项 **直接** 为 'true' 或 'false' （默认值为 'false'）。 如果将 **direct** 设置为“false”，则用常规 INSERT 语句插入行，这些语句会激发复制触发器。 如果将 **direct** 设置为“true”，则优化加载而不激发触发器。 因此，应确保 **直接** 设置为 'false' 时加载到复制的表使用 SQL * Loader 工具。  
...
## 删除或修改复制对象  
 如果删除或修改了任何发布服务器级的跟踪表、触发器、序列或存储过程，必须删除并重新配置发布服务器。 有关这些对象的部分列表，请参阅 [Oracle 发布服务器上创建对象](../../../relational-databases/replication/non-sql/objects-created-on-the-oracle-publisher.md)。  
