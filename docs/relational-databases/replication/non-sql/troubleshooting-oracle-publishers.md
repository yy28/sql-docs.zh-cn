...
ms.topic: "article"
helpviewer_keywords: 
  - "Oracle 发布 [SQL Server 复制], 故障排除"
  - "故障排除 [SQL Server 复制], Oracle 发布"
...
## 引发关于 Oracle 客户端和网络软件的错误  
 所用的帐户 [!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 分发服务器上运行的必须被授予读取和执行权限目录 （及其所有子目录） 中安装 Oracle 客户端网络软件。 如果未授予权限或者未正确安装 Oracle 客户端组件，您将接收到下列错误消息：  
...
-   已使用正确的 Oracle 主目录和路径。 即使 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 分发服务器上只安装了一组 Oracle 二进制文件，也要确保正确设置与 Oracle 主目录相关的环境变量。 如果更改了环境变量值，必须停止并重新启动 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 才能使更改生效。  
 ...
 有关配置和测试连接性的详细信息，请参阅"安装和配置 Oracle 客户端网络软件上的 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 分发服务器"中 [配置 Oracle 发布服务器](../../../relational-databases/replication/non-sql/configure-an-oracle-publisher.md)。  
  
## Oracle 发布服务器与另一分发服务器相关联  
...
-   "Oracle 服务器实例 ' \<*OraclePublisherName*>' 以前已配置为使用 \<*SQLServerDistributorName*>' 作为其分发服务器。 若要开始使用 ' \<*NewSQLServerDistributorName*>' 作为其分发服务器，必须删除在 Oracle 服务器实例，这将删除该服务器实例上的所有发布的当前复制配置。"  
  
-   "Oracle 服务器 '\<*OracleServerName*> 已被定义为发布服务器' \<*OraclePublisherName*> 在分发服务器上 \<*SQLServerDistributorName*>。*\< DistributionDatabaseName>*。 删除发布服务器或删除公共同义词 '*\< SynonymName>*以重新创建。"  
...
## 引发关于缺少主键的 SQL Server 错误 21663  
 事务发布中的项目必须具备有效的主键。 如果它们不具备有效的主键，则在尝试添加项目时会收到以下错误消息：  
  
 "源表找到任何有效的主键 [\<*TableOwner*>]。 [\<*TableName*>]"  
...
## 引发关于重复链接服务器登录名的 SQL Server 错误 21642  
 在初始配置 Oracle 发布服务器时，会为发布服务器和分发服务器之间的连接创建一个链接服务器项。 该链接服务器的名称与 Oracle TNS 服务名称相同。 如果尝试创建具有相同名称的链接服务器，则会显示以下错误消息：  
  
 “异类发布服务器需要链接服务器。 链接的服务器名为*\< LinkedServerName>*' 已存在。 请删除链接服务器或另选一个发布服务器名称。”  
  
 如果尝试直接创建链接服务器，或者预先删除了 Oracle 发布服务器和 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 分发服务器之间的关系，而当前在尝试重新配置它，就会出现此错误。 如果在尝试重新配置发布服务器时收到此错误，删除与链接的服务器 [sp_dropserver & #40;Transact SQL & #41;](../../../relational-databases/system-stored-procedures/sp-dropserver-transact-sql.md)。  
  
 如果您需要通过链接的服务器的连接以连接到 Oracle 发布服务器，创建另一个 TNS 服务名称，并在调用时，然后使用此名称 [sp_addlinkedserver & #40;Transact SQL & #41;](../../../relational-databases/system-stored-procedures/sp-addlinkedserver-transact-sql.md)。 有关创建 TNS 服务名称的信息，请参阅 Oracle 文档。  
  
## 引发 SQL Server 错误 21617  
 Oracle 发布操作使用 Oracle 应用程序 SQL*PLUS 将发布服务器支持代码包下载到 Oracle 数据库。 在尝试配置 Oracle 发布服务器， [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 验证该 SQL\*PLUS 就是通过分发服务器上的系统路径可访问。 如果 SQL\*加上不能加载，则显示以下错误消息︰  
  
 “无法运行 SQL*PLUS。 请确保分发服务器上安装了最新版本的 Oracle 客户端代码。”  
  
 尝试找到 SQL\*加上分发服务器上。 对于 Oracle 10g 客户端安装，此可执行文件的名称为 sqlplus.exe。 它通常安装在 %ORACLE_HOME%/bin 中。 若要验证的 SQL 的路径\*加上将显示在系统路径中，检查系统变量的值 **路径**:  
  
1.  用鼠标右键单击 **我的电脑**, ，然后单击 **属性**。  
...
## 引发 SQL Server 错误 21624 或错误 21629  
...
## 引发 SQL Server 错误 21626 或错误 21627  
...
3.  导航到 HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Microsoft SQL 服务器\\*\< 实例名>*\Providers。  
...
## 遇到复制用户架构的权限问题  
 复制用户架构必须具有在"创建用户架构 Manually"中所述的权限 [配置 Oracle 发布服务器](../../../relational-databases/replication/non-sql/configure-an-oracle-publisher.md)。  
...
## Oracle 错误 ORA-01000  
 ...
 若要避免此问题，确保 **max_open_cursors** Oracle 数据库中的设置设为足够大的数字 (至少为 1000)。 有关此设置的详细信息，请参阅 Oracle 文档。  
...
-   执行 **sp_dropdistpublisher**。 有关详细信息，请参阅 [sp_dropdistpublisher & #40;Transact SQL & #41;](../../../relational-databases/system-stored-procedures/sp-dropdistpublisher-transact-sql.md)。  
