...
---
# 发布信息，警告（快照发布，SQL Server 2005 及更高版本）
...
## 警告、阈值和警报  
 默认情况下，复制监视器会为未初始化的订阅显示警告：在包含订阅信息的页的“状态”  列中，显示“未初始化的订阅”  状态作为警告。 对于快照发布，您也可以通过设置选项 **“如果订阅将在阈值内过期，则发出警告”**，指定在订阅即将过期时发出警告。 如果达到或超过指定的阈值，订阅状态将显示为 **即将过期/已过期** （除非需要显示具有较高优先级的问题）。  
