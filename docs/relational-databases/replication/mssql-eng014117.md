...
-   禁用分发，注册实例，然后重新建立分发。 如果 @@SERVERNAME 的值对于某个非群集实例是不正确的，请执行下列步骤：  
    ...
    ```  
     ...
     执行后 [sp_addserver & #40;Transact SQL & #41;](../../relational-databases/system-stored-procedures/sp-addserver-transact-sql.md) 存储的过程，必须重新启动 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 服务对 @@SERVERNAME 才会生效的更改。  
  
     如果 @@SERVERNAME 的值对于某个群集实例是不正确的，则必须使用群集管理器更改该名称。 有关详细信息，请参阅 [AlwaysOn 故障转移群集实例 （&) #40;SQL Server 和 #41;](../Topic/AlwaysOn%20Failover%20Cluster%20Instances%20\(SQL%20Server\).md)。  
...
2.  禁用分发，然后重新启用它。 有关详细信息，请参阅 [配置分发](../../relational-databases/replication/configure-distribution.md)。  
