...
> [!NOTE]  
>  编辑现有发布中的筛选器需要为该发布创建新的快照。 如果发布有订阅，则必须重新初始化这些订阅。 有关更改属性的详细信息，请参阅 [更改发布和项目属性](../../relational-databases/replication/publish/change-publication-and-article-properties.md)。  
...
1.  **验证筛选的表并选择联接的表**  
    ...
    -   如果要添加新的联接，请验证中的表 **筛选的表** 文本框中是否正确 (如果不正确，请单击 **取消**, ，选择适当的表上 **筛选表行** 页，然后单击 **添加联接** 以返回到此对话框中)。 然后选择从一个表 **联接的表** 下拉列表框。  
...
2.  **创建联接语句**  
    ...
    -   如果要添加新的联接，则可以选择 **“使用生成器创建语句”** 或者 **“手动编写联接语句”**。 如果您开始手动编写联接，那么将无法使用生成器。  
         ...
         如果您选择使用生成器，则使用网格中的列 (**一起**, ，**筛选的表列**, ，**运算符**, ，和 **联接的表列**) 来生成联接语句。 在网格中的每列都包含下拉列表框中，从而允许您选择两个列和一个运算符 (**=**, ，**<>**, ，**<=**, ，**\<**, ，**>=**, ，**>**, ，**如**)。 结果显示在 **“预览”** 文本区域中。 如果联接中涉及的列的多个对，选择一个连接词 (**AND** 或 **或**) 从 **一起** 列中，然后输入两个更多的列和另一个运算符。  
...
3.  **指定联接选项**  
    ...
    -   [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] 及更高版本。 默认情况下，合并复制在同步过程中会逐行处理更改。 若要按单位处理相关更改，请选择 **“逻辑记录”**。 只有满足使用逻辑记录的项目和发布要求，此选项才可用。 详细信息，请参阅"使用逻辑记录的注意事项"部分中 [通过逻辑记录对相关行组更改](../../relational-databases/replication/merge/group-changes-to-related-rows-with-logical-records.md)。  
