...
# [新建订阅向导（用户界面参考）](new-subscription-wizard-ui-reference.md)
...
# [设置历史记录保持期 (SQL Server Management Studio)](set-the-history-retention-period-sql-server-management-studio.md)
...
# [设置事务发布的分发保持期 (SQL Server Management Studio)](set distribution retention period for transactional publications.md)
...
# [将数据大容量加载到合并发布中的表（复制 Transact-SQL 编程）](bulk-load data into tables in a merge publication.md)
