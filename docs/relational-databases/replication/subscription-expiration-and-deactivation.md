...
## <a name="transactional-replication"></a>事务复制  
 事务复制使用最大的分发保持期（`@max_distretention`sp_adddistributiondb &#40;Transact-SQL&#41;[ 的 ](../../relational-databases/system-stored-procedures/sp-adddistributiondb-transact-sql.md) 参数）和发布保持期（`@retention`sp_addpublication &#40;Transact-SQL&#41;[ 的 ](../../relational-databases/system-stored-procedures/sp-addpublication-transact-sql.md) 参数）：  
...
## <a name="merge-replication"></a>合并复制  
 合并复制使用发布保持期（`@retention`sp_addmergepublication &#40;Transact-SQL&#41;`@retention_period_unit` 的 [ 和 ](../../relational-databases/system-stored-procedures/sp-addmergepublication-transact-sql.md) 参数）。 订阅过期后必须将其重新初始化，因为订阅的元数据已被删除。 未重新初始化的订阅将由运行在发布服务器上的 **“过期的订阅清除”** 作业删除。 默认情况下，此作业每天运行；它删除在两倍的发布保持期内尚未同步的所有推送订阅。 例如：  
