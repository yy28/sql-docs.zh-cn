...
ms.topic: "article"
helpviewer_keywords: 
  - "自定义错误解决 [SQL Server 复制]"
  - "自定义更改处理 [SQL Server 复制]"
  - "错误 [SQL Server 复制], 业务逻辑处理程序"
...
-   如何实现业务逻辑处理程序的说明︰ [为合并项目实现业务逻辑处理程序](../../../relational-databases/replication/implement-a-business-logic-handler-for-a-merge-article.md)  
...
### 自定义冲突解决  
 合并复制提供了冲突检测和冲突解决选项，从而允许针对冲突接受默认的解决策略或选择自定义解决。 有关详细信息，请参阅 [Advanced Merge Replication Conflict Detection and Resolution](../../../relational-databases/replication/merge/advanced-merge-replication-conflict-detection-and-resolution.md)。 在处理冲突数据更改期间可以调用业务逻辑处理程序，它可以执行下列两种操作之一：  
