...
-   [!包括 [ssManStudioFull] (.../ Token/ssManStudioFull_md.md)]: [Enable Initialization with a Backup for Transactional Publications &#40;SQL Server Management Studio&#41;](../../relational-databases/replication/enable initialization with backup for transactional publications.md)  
  
-   复制 TRANSACT-SQL 编程︰ [初始化事务订阅从备份和 #40;复制 TRANSACT-SQL 编程 & #41;](../../relational-databases/replication/initialize a transactional subscription from a backup.md)  
...
-   在还原的数据库的订阅时间戳列必须转换为 **binary （8)** 列︰ 将包含到具有匹配除具有架构的新表的时间戳列的表的内容复制 **binary （8)** 列替代时间戳列中，删除原始表，并将具有与原始表相同的名称的新表重命名。  
