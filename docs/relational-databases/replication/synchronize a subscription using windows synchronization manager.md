...
ms.topic: "article"
helpviewer_keywords: 
  - "同步 [SQL Server 复制], Windows 同步管理器"
  - "Windows 同步管理器"
...
# 使用 Windows 同步管理器同步订阅（Windows 同步管理器）
  [!INCLUDE[msCoName](../../includes/msconame-md.md)] Windows 同步管理器仅可用于同步订阅的 Microsoft [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 发布如果 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 在同一台计算机与同步管理器 （它还可用来同步脱机文件和网页） 上运行。 若要使用同步管理器：  
  
1.  启用使用 Windows 同步管理器中的请求订阅的同步 **订阅属性-\< 订阅服务器上>: \< 订阅数据库>** 对话框。 有关访问此对话框中的详细信息，请参阅 [查看和修改请求订阅属性](../../relational-databases/replication/view-and-modify-pull-subscription-properties.md)。  
...
### 启用使用 Windows 同步管理器对请求订阅进行同步的选项  
  
1.  在 **常规** 页 **订阅属性-\< 订阅服务器上>: \< 订阅数据库>** 对话框中，选择一个值的 **启用** 为 **使用 Windows 同步管理器** 选项。  
...
1.  使用下列方法之一启动同步管理器：  
    ...
    -   单击 **“开始”**，再单击 **“运行”** 在 **运行** 对话框中，键入 **mobsync.exe** 中 **打开** 字段，，然后单击 **确定**。  
...
2.  在下列选项卡上查看和修改属性：  
    ...
    -   **订阅服务器登录名**, ，**分发服务器登录名**, ，和 **发布服务器登录名** （对于合并复制）  
  
    -   **Web 服务器信息** （适用于订阅服务器上运行 SQL Server 2005 或更高版本的合并订阅）  
...
1.  启用要使用交互式解决方法的项目和订阅。 有关详细信息，请参阅 [为合并项目指定交互式冲突解决](../../relational-databases/replication/publish/specify-interactive-conflict-resolution-for-merge-articles.md)。  
