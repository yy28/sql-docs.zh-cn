...
helpviewer_keywords: 
  ...
  - "性能 [SQL Server 复制]，事务复制"
  - "设计数据库 [SQL Server]，复制性能"
  - "性能 [SQL Server 复制]，快照复制"
  - "快照复制 [SQL Server]，性能"
...
-   适当调整分发数据库的大小。  
     ...
     在系统承受典型负载的情况下对复制进行测试，以确定存储命令需要多少空间。 确保数据库足以存储命令，而无需频繁地自动增大。 有关更改数据库的大小的详细信息，请参阅 [ALTER DATABASE & #40;Transact SQL & #41;](../../../t-sql/statements/alter-database-transact-sql.md)。  
...
-   将项目分布在多个发布上。  
     ...
     如果不能使用 **-SubscriptionStreams** 参数 （在本主题后面介绍），请考虑创建多个发布。 将项目分布在这些发布上使复制可以并行地将更改应用到订阅服务器。  
...
-   连续运行而不是按非常频繁的计划运行代理。  
    ...
    -   [!包括 [ssManStudioFull] (.../ Token/ssManStudioFull_md.md)]: [Specify Synchronization Schedules](../../../relational-databases/replication/specify-synchronization-schedules.md)  
  
## 分发代理和日志读取器代理参数  
  
-   若要解决意外、 一次性瓶颈，请使用 **– MaxCmdsInTran** 日志读取器代理程序的参数。  
...
-   为分发代理使用 **–SubscriptionStreams** 参数。  
     ...
     **–SubscriptionStreams** 参数可以显著提高聚合复制吞吐量。 它使到一台订阅服务器的多个连接可以并行应用批量更改，同时在使用单线程时保持多个事务特征的存在。 如果有一个连接无法执行或提交，则所有连接将中止当前批处理，而且代理将用单独的流重试失败的批处理。 在重试阶段完成之前，订阅服务器上会存在临时事务不一致。 失败的批处理成功提交后，订阅服务器将恢复到事务一致状态。  
     ...
     可以指定此代理参数的值，使用 **@subscriptionstreams** 的 [sp_addsubscription & #40;Transact SQL & #41;](../../../relational-databases/system-stored-procedures/sp-addsubscription-transact-sql.md)。  
  
-   增加的价值 **-ReadBatchSize** 日志读取器代理程序的参数。  
  
     日志读取器代理和分发代理支持事务读取和提交操作的批大小。 批大小的默认值为 500 个事务。 日志读取器代理从日志中读取特定数量的事务，而不管这些事务是否标记为复制。 当大量事务写入发布数据库，但只有其中一小部分标记为要复制时，应使用 **-ReadBatchSize** 参数增加日志读取器代理的读取的批大小。 此参数不适用于 Oracle 发布服务器。  
  
-   增加的价值 **-CommitBatchSize** 分发代理程序的参数。  
...
-   减小值 **-PollingInterval** 日志读取器代理程序的参数。  
