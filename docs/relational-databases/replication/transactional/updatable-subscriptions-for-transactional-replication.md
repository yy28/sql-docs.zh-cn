...
-   排队更新。不必连接发布服务器和订阅服务器即可在订阅服务器中更新数据。 可以在订阅服务器或发布服务器脱机时进行更新。  
 ...
 若要创建事务发布的可更新订阅，请参阅 [创建事务发布 (Management Studio) 的可更新订阅](../../../relational-databases/replication/publish/create-an-updatable-subscription-to-a-transactional-publication.md) 
  
## 在更新模式之间切换  
 使用可更新订阅时，可以指定订阅使用一种更新模式，然后在应用程序需要时再切换到另一种更新模式。 例如，可以指定订阅使用立即更新，但如果因系统故障而导致网络连接断开，则切换到排队更新。  
  
> [!NOTE]  
>  复制不会自动切换更新模式。 必须设置通过 SQL Server Management Studio 的更新模式，否则您的应用程序必须调用 [sp_setreplfailovermode &#40;Transact SQL &#41;](../../../relational-databases/system-stored-procedures/sp-setreplfailovermode-transact-sql.md) 模式之间切换。  
...
[可更新事务性订阅的更新模式之间切换](../../../relational-databases/replication/administration/switch-between-update-modes-for-an-updatable-transactional-subscription.md)。  
...
-   复制会添加 **msrepl_tran_version** 对已发布表以便进行跟踪的列。 由于添加的这个列，所有 **INSERT** 语句都应包括一个列列表。  
...
-   带有排队更新订阅服务器的事务复制可能会遇到性能降低时使用的是唯一的筛选的索引。 如果冲突发生在具有唯一的筛选的索引，则冲突解决方法会导致一些其他的项目删除，并插入唯一筛选的索引均未覆盖的行在订阅服务器上。
...
-   订阅服务器无法更新或插入 **文本**, ，**ntext** 或 **图像** 值，因为不能从在复制更改跟踪触发器内部插入或删除表中读取。 同样，订阅服务器不能使用 **WRITETEXT** 或 **UPDATETEXT** 更新或插入 **text** 或 **image** 值，因为这些数据会被发布服务器覆盖。 但可以将 **text** 和 **image** 列分区到单独的表中，并在一个事务中修改这两个表。  
  
     若要更新订阅服务器上的大型对象，使用的数据类型 **varchar （max)**, ，**nvarchar （max)**, ，**varbinary （max)** 而不是 **文本**, ，**ntext**, ，和 **图像** 分别数据类型。  
  
-   更新对唯一键 （包括主键），生成重复项 (例如，窗体的更新 `UPDATE <column> SET <column> =<column>+1` 不允许，并且将被拒绝，因为违反唯一性。 这是因为复制会将订阅服务器中所做的设置更新作为单独的 **UPDATE** 语句为每个受影响的行进行传播。  
...
-   如果应用程序需要在订阅服务器中使用触发器，则应在发布服务器和订阅服务器中使用 `NOT FOR REPLICATION` 选项来定义触发器。 这可以确保触发器只在原始数据更改时激发，而在复制更改时不激发。  
     ...
     确保复制触发器更新表时不会激发用户定义的触发器。 这通过调用过程实现 **sp_check_for_sync_trigger** 用户定义触发器的正文中。 有关详细信息，请参阅 [sp_check_for_sync_trigger &#40;Transact SQL &#41;](../../../relational-databases/system-stored-procedures/sp-check-for-sync-trigger-transact-sql.md)。  
...
-   对于数据类型的列 **SQL_VARIANT**︰ 当插入或更新订阅服务器上的数据，它映射以下列方式由队列读取器代理从订阅服务器复制到队列时︰  
