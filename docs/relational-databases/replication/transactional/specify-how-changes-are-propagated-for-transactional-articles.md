...
-   **sp_MSdel_ \<** *tablename* **>**, ，用于处理删除。  
  ...
  **\<***Tablename***>** 中使用的过程取决于如何将项目添加到发布和订阅数据库是否包含具有不同的所有者的相同名称的表。  
...
-   如果对已发布表进行架构更改，则必须重新生成自定义过程。 有关详细信息，请参阅 [重新生成自定义事务性过程来反映架构更改](../../../relational-databases/replication/transactional/regenerate-custom-transactional-procedures-to-reflect-schema-changes.md)。  
  
-   如果您使用的值大于 1 的 **-SubscriptionStreams** 分发代理程序的参数，则必须确保是否成功对主键列的更新。 例如：  
...
#### SCALL 语法  
 UPDATE 存储过程  
 处理 UPDATE 语句的存储的过程将传递更新后的值仅对那些已更改，跟主键列的原始值后, 跟一个位屏蔽的列 (**binary(n)**) 的参数，指示已更改的列。 在下面的示例中，列 2 (c2) 未更改：  
...
#### MCALL 语法  
 UPDATE 存储过程  
 处理 UPDATE 语句的存储的过程将传递的文章，跟主键列后, 跟一个位屏蔽的原始值中定义的所有列的更新的值 (**binary(n)**) 的参数，指示已更改的列︰  
...
> [!NOTE]  
>  使用 XCALL 时，前像值 **文本** 和 **图像** 列应为 NULL。  
