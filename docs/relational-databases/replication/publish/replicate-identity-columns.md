...
---
# 复制标识列
  为列分配 IDENTITY 属性后， [!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 将为在包含标识列的表中插入的新行自动生成顺序编号。 有关详细信息，请参阅 [标识 &#40;属性和 #41;&#40;Transact SQL &#41;](../Topic/IDENTITY%20\(Property\)%20\(Transact-SQL\).md)。 因为包含的标识列可能是主键的一部分，所以请务必避免在标识列中出现重复值。 若要在多个节点上都有更新的复制拓扑中使用标识列，复制拓扑中的每个节点都必须使用不同范围的标识值，以避免出现重复。  
...
## 分配标识范围  
 ...
 如果发布服务器上插入后用尽了其标识范围，它可以自动将分配新范围的成员执行插入 **db_owner** 固定的数据库角色。 如果用户不在该角色、 日志读取器代理、 合并代理或成员的用户执行插入的 **db_owner** 角色必须运行 [sp_adjustpublisheridentityrange &#40;Transact SQL &#41;](../../../relational-databases/system-stored-procedures/sp-adjustpublisheridentityrange-transact-sql.md)。 对于事务发布，必须运行日志读取器代理，以便自动分配新范围（默认设置是使代理连续运行）。  
...
### 合并复制  
 标识范围由发布服务器管理，并由合并代理传播到订阅服务器（在重新发布的层次结构中，范围由根发布服务器和重新发布服务器管理）。 从发布服务器上的池中分配标识值。 在将具有标识列的项目添加到发布在新建发布向导或使用 [sp_addmergearticle &#40;Transact SQL &#41;](../../../relational-databases/system-stored-procedures/sp-addmergearticle-transact-sql.md), ，指定的值︰  
  
-   **@Identity_range** 参数，它控制初始分配到发布服务器和订阅服务器，与客户端订阅的标识范围大小。  
  
    > [!NOTE]  
    >  对于运行以前版本的订阅服务器 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)], ，此参数 (而不是 **@pub_identity_range** 参数) 还控制重新发布订阅服务器的标识范围大小。  
  
-    **@Pub_identity_range** 参数，它控制的标识范围大小重新发布到订阅服务器分配具有服务器订阅 （所需的重新发布数据）。 所有带有服务器订阅的订阅服务器都会接收到重新发布的范围，即使它们实际上并不重新发布数据。  
  
-   **@threshold** 参数，用于确定 [!INCLUDE[ssEW](../../../includes/ssew-md.md)] 或早期版本的 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]订阅何时需要新范围的标识。  
  
 例如，可以指定为 10000 **@identity_range** 500000 **@pub_identity_range**。 为运行 [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)] 或更高版本的发布服务器和所有订阅服务器（包括具有服务器订阅功能的订阅服务器）分配主范围 10000。 具有服务器订阅的订阅服务器也被分配主范围 500000，与重新发布订阅服务器同步的订阅服务器可以使用 (您还必须指定 **@identity_range**, ，**@pub_identity_range**, ，和 **@threshold** 重新发布订阅服务器上的发布中的项目)。  
...
### 带有排队更新订阅的事务复制  
 标识范围由分发服务器管理，并由分发代理传播到订阅服务器。 标识值从分发服务器上的池中分配。 池的大小根据标识列所使用的数据类型和增量大小而定。 在将具有标识列的项目添加到发布在新建发布向导或使用 [sp_addarticle &#40;Transact SQL &#41;](../../../relational-databases/system-stored-procedures/sp-addarticle-transact-sql.md), ，指定的值︰  
  
-    **@Identity_range** 参数，它控制初始分配给所有订阅服务器的标识范围大小。  
  
-    **@Pub_identity_range** 参数，它控制分配给发布服务器的标识范围大小。  
  
-   **@threshold** 参数，它用于确定订阅何时需要新范围的标识。  
  
 例如，可以指定为 10000 **@pub_identity_range**, 为 1000年 **@identity_range** （假定订阅服务器上的更新较少），80% **@threshold**。 在订阅服务器上进行 800 次插入后（即 1000 的 80%），将为订阅服务器分配一个新范围。 在发布服务器上进行 8000 次插入后，将为发布服务器分配一个新范围。 分配新范围后，表中的标识范围值之间将有一定的间隔。 指定较高的阈值会产生较小的间隔，但这会降低系统的容错能力：如果分发代理由于某种原因无法运行，订阅服务器就更容易用完标识。  
...
## 在数据库还原之后处理标识范围  
 如果正在使用自动标识范围管理，则从备份还原订阅服务器后，它将自动请求标识值的新范围。 如果发布服务器是从备份还原的，则必须确保为发布服务器分配了适当的范围。 对于合并复制，将分配新范围使用 [sp_restoremergeidentityrange &#40;Transact SQL &#41;](../../../relational-databases/system-stored-procedures/sp-restoremergeidentityrange-transact-sql.md)。 对于事务复制，请先确定其使用过的最大值，然后设置新范围的起点。 还原发布数据库后请使用下列过程：  
