...
helpviewer_keywords: 
  ...
  - "已分区表 [SQL Server], 复制"
  - "复制 [SQL Server], 已分区表"
  - "发布 [SQL Server replication], 已分区表"
  - "事务复制, 已分区表"
...
-   在新建发布向导的 **“项目属性”** 页或者“发布属性”对话框中。 若要复制上一个表中列出的对象，请为属性 **“复制表分区方案”** 和 **“复制索引分区方案”** 指定 **true**值。 有关如何访问 **项目属性** 页上，请参阅 [查看和修改发布属性](../../../relational-databases/replication/publish/view-and-modify-publication-properties.md)。  
  
-   通过使用 *schema_option* 之一的以下存储过程的参数︰  
  
    -   [sp_addarticle](../../../relational-databases/system-stored-procedures/sp-addarticle-transact-sql.md) 或 [sp_changearticle](../../../relational-databases/system-stored-procedures/sp-changearticle-transact-sql.md) 对于事务复制  
  
    -   [sp_addmergearticle](../../../relational-databases/system-stored-procedures/sp-addmergearticle-transact-sql.md) 或 [sp_changemergearticle](../../../relational-databases/system-stored-procedures/sp-changemergearticle-transact-sql.md) 用于合并复制  
...
-   如果订阅服务器和发布服务器具有不同的已分区表定义，当分发代理尝试在订阅服务器上应用更改时将失败。  
 ...
 当分区具有完全相同分区方案时在发布服务器和订阅服务器可以开启 *allow_partition_switch* 连同 *replication_partition_switch* 这只会将分区切换语句复制到订阅服务器。 您还可以打开 *allow_partition_switch* 而不复制 DDL。 这在以下情况下非常有用：您希望从分区中淘汰已经过的月份，但为另一年保留复制的分区，以便在订阅服务器上进行备份。  
...
-   **@replicate_partition_switch** 确定 SWITCH PARTITION DDL 语句是否应复制到订阅服务器。 此选项是仅当 **@allow_partition_switch** 设置为 **true**。  
 ...
 可以通过设置这些属性 [sp_addpublication](../../../relational-databases/system-stored-procedures/sp-addpublication-transact-sql.md) 创建发布时，或通过使用 [sp_changepublication](../../../relational-databases/system-stored-procedures/sp-changepublication-transact-sql.md) 创建发布后。 如上所述，合并复制不支持分区切换。 若要对已启用合并复制的表执行 SWITCH PARTITION，请从发布中删除该表。  
