...
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]本主题说明如何使用 [!INCLUDE[ssManStudioFull](../../../includes/ssmanstudiofull-md.md)] 或 [!INCLUDE[tsql](../../../includes/tsql-md.md)] 在 [!INCLUDE[ssCurrent](../../../includes/sscurrent-md.md)] 中优化参数化行筛选器。  
...
1.  （可选）在发布服务器上，对发布数据库执行 [sp_helpmergepublication](../../../relational-databases/system-stored-procedures/sp-helpmergepublication-transact-sql.md)，并指定 **@publication**中优化参数化行筛选器。 请注意结果集中 **keep_partition_changes** 和 **use_partition_groups** 的值。  
  
2.  （可选）在发布服务器上，对发布数据库执行 [sp_changemergepublication](../../../relational-databases/system-stored-procedures/sp-changemergepublication-transact-sql.md)。 将值 **use_partition_groups** 指定给 **@property** ，并将 **true** 在 **false** 指定给 **@value**中优化参数化行筛选器。  
  
3.  （可选）在发布服务器上，对发布数据库执行 [sp_changemergepublication](../../../relational-databases/system-stored-procedures/sp-changemergepublication-transact-sql.md)。 将值 **keep_partition_changes** 指定给 **@property** ，并将 **true** 在 **false** 指定给 **@value**中优化参数化行筛选器。  
  
    > [!NOTE]  
    >  当启用 **keep_partition_changes**时，必须首先禁用 **use_partition_groups** ，并将值 **1** 指定给 **@force_reinit_subscription**中优化参数化行筛选器。  
  
4.  （可选）在发布服务器上，对发布数据库执行 [sp_changemergearticle](../../../relational-databases/system-stored-procedures/sp-changemergearticle-transact-sql.md)。 将值 **partition_options** 指定给 **@property** ，并为 **@value**中优化参数化行筛选器。 有关以上筛选选项的定义，请参阅 [sp_addmergearticle](../../../relational-databases/system-stored-procedures/sp-addmergearticle-transact-sql.md) 。  
