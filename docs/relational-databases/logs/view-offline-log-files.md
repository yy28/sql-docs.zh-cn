...
-   对包含错误日志文件的文件夹的读取权限。 默认情况下，错误日志文件位于下面的路径中（其中 \<*驱动器>* 表示你安装 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的驱动器，\<*实例名称*> 是 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例的名称）：  
