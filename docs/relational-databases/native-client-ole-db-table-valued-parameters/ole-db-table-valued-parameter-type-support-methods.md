...
|ISSCommandWithParameters::SetParameterInfo|若要指定表值参数命令参数，使用者指定的参数类型为"table"或"DBTYPE_TABLE"中*pwszName*应在 DBPARAMBINDINFO 结构的成员。 *UlParamSize*设置为 ~ 0。 详细信息，请参阅"指定表值参数"中[Executing Commands Containing Table-Valued 参数](../../relational-databases/native-client-ole-db-table-valued-parameters/executing-commands-containing-table-valued-parameters.md)。|  
