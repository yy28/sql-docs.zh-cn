...
## SQL Server 实用工具  
 如果您将 DAC 导入到数据库引擎的托管实例，则在下次将实用工具收集组从该实例发送到实用工具控制点时，导入的 DAC 将合并到 SQL Server 实用工具中。 然后，该 DAC 将出现在[!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)]“实用工具资源管理器”的“已部署的数据层应用程序”节点下，并且在“已部署的数据层应用程序”详细信息页中报告。  
