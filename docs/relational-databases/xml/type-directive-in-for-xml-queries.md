...
# FOR XML 查询中的 TYPE 指令
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 支持 [xml (Transact-SQL)](../../t-sql/xml/xml-transact-sql.md) 使你可以通过指定 TYPE 指令请求将 FOR XML 查询结果作为 **xml** 数据类型返回。 这样您便可以在服务器上处理 FOR XML 查询的结果。 例如，可以对其指定 Xquery，将结果分配给 **xml** 类型变量，或编写 [嵌套 FOR XML 查询](../../relational-databases/xml/use-nested-for-xml-queries.md)。  
...
### 将 FOR XML 查询结果赋给 xml 类型变量  
 在下面的示例中，一个 FOR XML 结果被赋给一个 **xml** 类型变量 `@x`。 该查询从 **xml**`TYPE` 的 `AdditionalContactInfo` 列检索联系信息（如 `BusinessEntityID`、`FirstName`、`LastName` 和其他电话号码）。 由于 `FOR XML` 子句指定了 `TYPE` 指令，因此 XML 返回为 **xml** 类型并赋给某个变量。  
