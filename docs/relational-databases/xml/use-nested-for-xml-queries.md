...
```  
 ...
 还可以使用 `@x`数据类型方法之一，处理在变量 **xml** 中返回的 XML。 例如，可以使用 [value() 方法](../../t-sql/xml/value-method-xml-data-type.md) 检索 `ProductModelID` 属性值。  
...
## 将内部 FOR XML 查询的结果以 xml 类型实例的形式返回到外部查询  
