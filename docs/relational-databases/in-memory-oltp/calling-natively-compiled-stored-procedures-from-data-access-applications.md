...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]本主题提供有关从数据访问应用程序中调用本机编译的存储过程的指导。  
 ...
 SqlClient 不支持检索有关本机编译存储过程返回的结果集的纯架构信息（元数据发现）(CommandType.SchemaOnly)。 请改为使用 [sp_describe_first_result_set (Transact-SQL)](../../relational-databases/system-stored-procedures/sp-describe-first-result-set-transact-sql.md)。  
  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 之前的 [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] Native Client 版本不支持检索有关本机编译存储过程返回的结果集的纯架构信息（元数据发现）。 请改为使用 [sp_describe_first_result_set (Transact-SQL)](../../relational-databases/system-stored-procedures/sp-describe-first-result-set-transact-sql.md)。  
  
 以下建议适用于在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client 中使用 ODBC 驱动程序调用本机编译的存储过程。  
  
 只调用存储过程一次的最高效方式是使用 **SQLExecDirect** 和 ODBC CALL 子句发出直接的 RPC 调用。 请勿使用 [!INCLUDE[tsql](../../includes/tsql-md.md)]**EXECUTE** 语句。 如需多次调用存储过程，则准备好的执行最为高效。  
