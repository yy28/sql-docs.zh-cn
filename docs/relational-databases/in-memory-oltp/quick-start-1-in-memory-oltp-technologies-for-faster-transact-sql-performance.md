---
title: "快速入门 1：可提高 Transact SQL 性能的内存中 OLTP 技术 | Microsoft Docs"
ms.custom: ""
ms.date: "03/04/2017"
...
manager: "jhubbard"
---
# 快速入门 1：可提高 Transact SQL 性能的内存中 OLTP 技术
...
- [列存储索引指南](Columnstore%20Indexes%20Guide.xml)  
  
  
> [AZURE.NOTE] 有关内存中功能的一个两分钟视频可在 [Azure SQL 数据库 - 内存中技术](http://channel9.msdn.com/Blogs/Windows-Azure/Azure-SQL-Database-In-Memory-Technologies)中找到。 该视频的发布日期为 2015 年 12 月。  
...
1. [Real-Time Operational Analytics Using In-Memory Technology](https://blogs.technet.microsoft.com/dataplatforminsider/2015/12/09/real-time-operational-analytics-using-in-memory-technology/)（使用内存中技术的实时运行分析）
2. [Real-Time Operational Analytics – Overview nonclustered columnstore index (NCCI)](https://blogs.msdn.microsoft.com/sqlserverstorageengine/2016/02/29/real-time-operational-analytics-using-nonclustered-columnstore-index/)（实时运行分析 - 非聚集列存储索引 (NCCI) 概述）
3. [Real-Time Operational Analytics: Simple example using nonclustered clustered columnstore index (NCCI) in SQL Server 2016](https://blogs.msdn.microsoft.com/sqlserverstorageengine/2016/02/29/real-time-operational-analytics-simple-example-using-nonclustered-clustered-columnstore-index-ncci/)（实时运行分析：SQL Server 2016 中使用非聚集列存储索引 (NCCI) 的简单示例）
4. [Real-Time Operational Analytics: DML operations and nonclustered columnstore index (NCCI) in SQL Server 2016](https://blogs.msdn.microsoft.com/sqlserverstorageengine/2016/03/04/real-time-operational-analytics-dml-operations-and-nonclustered-columnstore-index-ncci-in-sql-server-2016/)（实时运行分析：SQL Server 2016 中的 DML 操作和非聚集列存储索引 (NCCI)）
5. [Real-Time Operational Analytics: Filtered nonclustered columnstore index (NCCI)](https://blogs.msdn.microsoft.com/sqlserverstorageengine/2016/03/06/real-time-operational-analytics-filtered-nonclustered-columnstore-index-ncci/)（实时运行分析 - 筛选的非聚集列存储索引 (NCCI)）
6. [Real-Time Operational Analytics: Compression Delay Option for Nonclustered Columnstore Index (NCCI)](https://blogs.msdn.microsoft.com/sqlserverstorageengine/2016/03/06/real-time-operational-analytics-compression-delay-option-for-nonclustered-columnstore-index-ncci/)（实时运行分析 - 非聚集列存储索引 (NCCI) 的压缩延迟选项）
7. [Real-Time Operational Analytics: Compression Delay option with NCCI and the performance](https://blogs.msdn.microsoft.com/sqlserverstorageengine/2016/03/06/real-time-operational-analytics-compression-delay-option-with-ncci-and-the-performance/)（实时运行分析：NCCI 及性能的压缩延迟选项）
8. [Real-Time Operational Analytics: Memory-Optimized Tables and Columnstore Index](https://blogs.msdn.microsoft.com/sqlserverstorageengine/2016/03/07/real-time-operational-analytics-memory-optimized-table-and-columnstore-index/)（实时运行分析：内存优化表和列存储索引）

#### <a name="defragment-a-columnstore-index"></a>对列存储索引进行碎片整理

1. [Columnstore Index Defragmentation using REORGANIZE Command](https://blogs.msdn.microsoft.com/sqlserverstorageengine/2016/03/07/columnstore-index-defragmentation-using-reorganize-command/)（使用 REORGANIZE 命令的列存储索引）
2. [Columnstore Index Merge Policy for REORGANIZE](https://blogs.msdn.microsoft.com/sqlserverstorageengine/2016/03/08/columnstore-index-merge-policy-for-reorganize/)（REORGANIZE 的列存储索引合并策略）

#### <a name="bulk-importation-of-data"></a>批量导入数据

1. [Clustered Column Store: Bulk Load](https://blogs.msdn.microsoft.com/sqlserverstorageengine/2014/07/27/clustered-column-store-index-bulk-loading-the-data/)（聚集列存储：大容量加载）
2. [Clustered Columnstore Index: Data Load Optimizations – Minimal Logging](https://blogs.msdn.microsoft.com/sqlserverstorageengine/2016/01/10/clustered-columnstore-index-data-load-optimizations-minimal-logging/)（聚集列存储索引：数据加载优化 - 最小日志记录）
3. [Clustered columnstore Index: Data Load Optimization – Parallel Bulk Import](https://blogs.msdn.microsoft.com/sqlserverstorageengine/2016/02/28/clustered-columnstore-index-parallel-bulk-import/)（聚集列存储索引：数据加载优化 - 并行批量导入）
...
若要可靠地对跨容器事务中的内存优化表强制执行此级别，请通过执行以下 T-SQL 来[变更数据库设置](ALTER%20DATABASE%20SET%20Options%20%28Transact-SQL%29.xml)。  
...
- [管理内存中 OLTP 的内存](Managing%20Memory%20for%20In-Memory%20OLTP.xml)  
...
- 本机编译的[表值函数](../../relational-databases/user-defined-functions/create-user-defined-functions-database-engine.md)。  
  - [Improving temp table and table variable performance using memory optimization](https://blogs.msdn.microsoft.com/sqlserverstorageengine/2016/03/21/improving-temp-table-and-table-variable-performance-using-memory-optimization/)（使用内存优化改进临时表和表变量性能）  
...
有关本机 UDF 性能的测试数据和说明，请参阅：  
  ...
  - [Soften the RBAR impact with Native Compiled UDFs in SQL Server 2016](https://blogs.msdn.microsoft.com/sqlcat/2016/02/17/soften-the-rbar-impact-with-native-compiled-udfs-in-sql-server-2016/)（使用 SQL Server 2016 中的本机编译 UDF 减轻 RBAR 影响）  
...
- [内存优化表的备份、还原和恢复](Backup,%20Restore,%20and%20Recovery%20of%20Memory-Optimized%20Tables.xml)  
