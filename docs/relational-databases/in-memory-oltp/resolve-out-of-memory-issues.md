...
-   增加 MAX_MEMORY_PERCENT 值。   
    ...
    > 有关详细信息，请参阅主题[最佳做法：在 VM 环境下使用内存中 OLTP](../Topic/Using%20In-Memory%20OLTP%20in%20a%20VM%20Environment.md)。  
...
##### 增加资源池的 MAX_MEMORY_PERCENT 值  
 如果尚未为内存中表创建命名资源池，应该创建一个，并将[!INCLUDE[hek_2](../../includes/hek-2-md.md)] 数据库与其绑定。 有关如何创建并将 [!INCLUDE[hek_2](../../includes/hek-2-md.md)] 数据库与资源池绑定的指南，请参阅主题 [将具有内存优化表的数据库绑定至资源池](../../relational-databases/in-memory-oltp/bind-a-database-with-memory-optimized-tables-to-a-resource-pool.md)。  
...
## 另请参阅  
 ...
 [最佳做法：在虚拟机环境中使用内存中 OLTP](../Topic/Using%20In-Memory%20OLTP%20in%20a%20VM%20Environment.md)  
