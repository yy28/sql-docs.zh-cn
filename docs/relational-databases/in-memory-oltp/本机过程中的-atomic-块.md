...
---
# 本机过程中的 ATOMIC 块
  **BEGIN ATOMIC** 属于 ANSI SQL 标准。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 支持顶级本机编译存储过程的原子块，以及本机编译的标量用户定义函数。 有关这些函数的详细信息，请参阅[内存中 OLTP 的标量用户定义函数](../../relational-databases/in-memory-oltp/scalar-user-defined-functions-for-in-memory-oltp.md)。  
