...
3.  将以下示例复制并粘贴到查询窗口中，然后单击“执行”  。 该示例在购买  边缘表上创建边缘约束。  
...
3.  将以下示例复制并粘贴到查询窗口中，然后单击“执行”  。 该示例使用 ALTER TABLE 将边缘约束添加到购买  边缘表。
...
3.  将以下示例复制并粘贴到查询窗口中，然后单击“执行”  。 该示例使用 ALTER TABLE 在购买  边缘表上添加含有其他边缘约束子句的新边缘约束。
 ...
 GO
 ```  
 请注意，在 EC_BOUGHT1  约束中有两个边缘约束子句，一个子句将客户  连接到产品  ，另一个子句将供应商  连接到产品  。 这两个子句都可应用于析取。 即，给定的边缘必须满足这两个子句之一，才能在边缘表中使用。 
...
3.  将以下示例复制并粘贴到查询窗口中，然后单击“执行”  。 该示例使用 ALTER TABLE 在购买  边缘表上添加含有新边缘约束子句的新边缘约束。
 ...
 在此示例中，我们在购买  边缘表中创建了两个单独的边缘约束：EC_BOUGHT  和 EC_BOUGHT1  。 这两个边缘约束都具有不同的边缘约束子句。 如果一个边缘表在其上具有多个边缘约束，则给定的边缘表必须满足所有  边缘约束，才能在边缘表中使用它。 因为此处没有能够同时满足 EC_BOUGHT  和 EC_BOUGHT1  的任何边缘，所以购买  边缘表必须保持为空。 
 ...
 为了能够在购买  边缘中使用给定的边缘，它必须满足 EC_BOUGHT_NEW  约束中的边缘约束子句之一。 因此，将允许使用尝试将有效的客户  连接到产品  或将供应商  连接到产品  节点的任何边缘。 
