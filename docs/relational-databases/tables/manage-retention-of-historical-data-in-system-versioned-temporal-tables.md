...
-   **延伸整个历史记录表：** 如果主要方案是在环境中审核数据，并且数据更改较为频繁，而对历史数据的查询相对很少，则可以为整个历史记录表配置 Stretch Database。  换而言之，如果临时查询的性能并不重要，则使用此方法。 在这种情况下，Azure 提供的成本效益可能极具吸引力。   
