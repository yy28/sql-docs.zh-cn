...
### 常规  
 **Vardecimal 存储格式已启用**  
 当为 **True** 时，此只读值表示使用 vardecimal 存储格式存储 **decimal** 和 **numeric** 数据类型。 若要更改此选项，请使用 [sp_tableoption](../../relational-databases/system-stored-procedures/sp-tableoption-transact-sql.md) 的“vardecimal 存储格式”选项。 不推荐使用 Vardecimal 存储格式。 请改用 ROW 压缩。  
