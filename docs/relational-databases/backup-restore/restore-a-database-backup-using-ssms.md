...
helpviewer_keywords: 
  - "完整备份 [SQL Server]"
  - "数据库还原 [SQL Server], 完整部分"
  - "备份数据库 [SQL Server], 完整备份"
  - "数据库备份 [SQL Server], 完整备份"
  - "还原数据库 [SQL Server], 完整备份"
...
### 重要说明！    
在完整恢复模式或大容量日志恢复模式下，可能需要先备份活动事务日志（称为[日志尾部](https://msdn.microsoft.com/library/ms179314.aspx)），然后才能还原数据库。 有关详细信息，请参阅[备份事务日志 (SQL Server)](../../relational-databases/backup-restore/back-up-a-transaction-log-sql-server.md)。  
...
8. 若要查看或选择高级选项，在 **“选项”** 页的 **“还原选项”** 面板中，可以根据您的实际情况选择下列任意选项：    
    ...
    3.  **还原前进行结尾日志备份。** 并非所有还原方案都要求执行结尾日志备份。  有关详细信息，请参阅[《Tail-Log Backups (SQL Server)》](../../relational-databases/backup-restore/tail-log-backups-sql-server.md)（结尾日志备份 (SQL Server)）中的 **Scenarios That Require a Tail-Log Backup**（需要结尾日志备份的场景）。
...
### **C.  使用新的数据库名称还原以前的磁盘备份，并且原始数据库仍然存在**
...
9. [!INCLUDE[clickOK](../../includes/clickok-md.md)] 
    ...
    > [!NOTE] 
    > 如果收到以下错误消息：“System.Data.SqlClient.SqlError: 数据库“`Sales`”的日志结尾尚未备份。 如果该日志包含您不希望丢失的工作，请使用 BACKUP LOG WITH NORECOVERY 备份该日志。 使用 RESTORE 语句的 WITH REPLACE 或 WITH STOPAT 子句覆盖该日志的内容。 (Microsoft.SqlServer.SmoExtended)”。  
...
#### **F.   将本地备份还原到 Microsoft Azure 存储 (URL)**
