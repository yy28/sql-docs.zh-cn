...
### 优化还原  
 ...
 在执行还原操作时，也可开启跟踪标志 3051，以生成详细日志。 日志文件放置在日志目录中，以下面的格式命名：BackupToUrl-\<instancename>-\<dbname>-action-\<PID>.日志。 日志文件包含每次往返 Windows Azure 存储的相关信息（包括计时），这在诊断问题时可能非常有用。  
