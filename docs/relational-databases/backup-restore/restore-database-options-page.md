...
> [!NOTE]  
>  使用 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] 指定还原任务时，您可以为此还原操作生成一个包含 RESTORE 语句的对应的 [!INCLUDE[tsql](../../includes/tsql-md.md)] 脚本。 若要生成该脚本，请单击 **“脚本”** ，然后为脚本选择一个目标。 有关 RESTORE 语法的信息，请参阅 [RESTORE (Transact-SQL)](../Topic/RESTORE%20\(Transact-SQL\).md)。  
 ...
 **覆盖现有数据库 [WITH REPLACE]**  
 还原操作将覆盖当前使用你指定的数据库名称（在“还原数据库”对话框中“常规”[](../../relational-databases/backup-restore/restore-database-general-page.md "Restore Database (General Page)")页上“还原到”字段中指定）的任何数据库文件。 即使将备份从其他数据库还原到现有的数据库名称，现有数据库的文件也将被覆盖。 选择此选项等效于在 [RESTORE](../Topic/RESTORE%20Arguments%20\(Transact-SQL\).md) 语句 ([!INCLUDE[tsql](../../includes/tsql-md.md)]) 中使用 REPLACE 选项。  
  
> [!CAUTION]  
>  只有在仔细考虑后，才能使用此选项。 有关详细信息，请参阅 [RESTORE Arguments (Transact-SQL)](../Topic/RESTORE%20Arguments%20\(Transact-SQL\).md)。  
 ...
 **保留复制设置 [WITH KEEP_REPLICATION]**  
 ...
 选择此选项等效于在 [RESTORE](../Topic/RESTORE%20\(Transact-SQL\).md) 语句中使用 KEEP_REPLICATION 选项。  
 ...
 **RESTORE WITH RECOVERY**  
 在还原了在“常规”[](../../relational-databases/backup-restore/restore-database-general-page.md "Restore Database (General Page)")页的“用于还原的备份集”网格中选中的最后一个备份之后，恢复数据库。 这是默认选项，等效于在 [RESTORE](../Topic/RESTORE%20Arguments%20\(Transact-SQL\).md) 语句 ([!INCLUDE[tsql](../../includes/tsql-md.md)]) 中指定 WITH RECOVERY。  
...
> [!NOTE]  
>  如果你在“备份时间线”[](../../relational-databases/backup-restore/backup-timeline.md "Backup Timeline")对话框中选择的时间点要求结尾日志备份，则将选择此框并且你将不能对其进行编辑。  
