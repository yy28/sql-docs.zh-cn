...
-   联机段落还原方案  
     ...
     联机段落还原可能引起事务延迟。 如果仅还原了一部分文件组，则数据库中依赖联机文件组的事务可能会延迟。 这是正常现象，因为整个数据库必须一致。 有关详细信息，请参阅[延迟的事务 (SQL Server)](../../relational-databases/backup-restore/deferred-transactions-sql-server.md)。  
