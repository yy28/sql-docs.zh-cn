...
ms.topic: "article"
helpviewer_keywords: 
  - "体系结构 [SQL Server], 数据库邮件"
  - "数据库邮件 [SQL Server], 体系结构"
  - "数据库邮件 [SQL Server], 组件"
...
-   [数据库邮件体系结构](#VisualElement)  
  
-   [数据库邮件组件简介](#ComponentsAndConcepts)  
...
-   进程隔离。 若要最大程度上减小对 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的影响，传递电子邮件的组件必须在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 外部的单独进程中运行。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 即使外部进程停止或失败，也会继续对电子邮件进行排队。 队列中的邮件将在外部进程或 SMTP 服务器联机时发送。  
