...
|file_guid|**uniqueidentifier**|文件的唯一标识符。<br /><br /> NULL = 从 @no__t 的早期版本升级了数据库（对 SQL Server 2005 及更早版本有效）。|  
...
|大小|**int**|当前文件大小（以 8 KB 为单位的页数）。 对于数据库快照来说，size 表示该快照可以一直用于文件的最大空间。<br /><br /> 注意:对于 FILESTREAM 容器，此字段填充为零。 查询*database_files*目录视图以了解 FILESTREAM 容器的实际大小。|  
|max_size|**int**|最大文件大小（以 8 KB 为单位的页数）：<br /><br /> 0 = 不允许增长。<br /><br /> -1 = 文件将一直增长到磁盘充满为止。<br /><br /> 268435456 = 日志文件将增长到最大大小 2 TB。<br /><br /> 注意:使用无限制日志文件大小升级的数据库将报告-1，以表示日志文件的最大大小。|  
...
|credential_id|**int**|用于存储文件的 @no__t 中的 @no__t 0。 例如，如果在 Azure 虚拟机上运行 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]，并且数据库文件存储在 Azure blob 存储中，则使用存储位置的访问凭据配置凭据。|  
