...
|type_desc|**nvarchar （60）**|文件类型的说明：<br /><br /> ROWS<br /><br /> 日志<br /><br /> FILESTREAM<br /><br /> FULLTEXT（[!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 之前的全文目录。）|  
...
|size|**int**|当前文件大小（以 8 KB 为单位的页数）。 对于数据库快照来说，size 表示该快照可以一直用于文件的最大空间。<br /><br /> 注意：对于 FILESTREAM 容器，此字段填充为零。 查询 sys.databases 容器的*database_files*目录视图的实际大小。|  
