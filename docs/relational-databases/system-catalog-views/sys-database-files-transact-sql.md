...
|**file_guid**|**ssNoversion**|文件的 GUID。<br /><br /> NULL = 数据库已从 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的早期版本升级（对于 SQL Server 2005 及更早版本有效）。|  
...
|**differential_base_guid**|**ssNoversion**|差异备份所基于的基准备份的唯一标识符。|  
...
|**redo_start_fork_guid**|**ssNoversion**|恢复分叉的唯一标识符。 还原的下一个日志备份的 first_fork_guid 必须与此值匹配。 这表示文件的当前状态。|  
|**redo_target_lsn**|**numeric(25,0)**|对此文件的联机前滚可以停止时的 LSN。<br /><br /> 除非 state = RESTORING 或 state = RECOVERY_PENDING，否则为 NULL。|  
|**redo_target_fork_guid**|**ssNoversion**|可以在其上恢复文件的恢复分叉。 与 redo_target_lsn 成对使用。|  
