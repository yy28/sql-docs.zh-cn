---
title: sys.event_log （Azure SQL 数据库） |Microsoft 文档
ms.custom: ''
ms.date: 06/10/2016
ms.prod: ''
...
> [!CAUTION]  
>  对于具有大量的数据库或大量的登录名的安装，sys.event_log 中的活动可能会限制导致的性能，CPU 使用率过高，并可能导致登录失败。 Sys.event_log 查询会导致问题。 Microsoft 致力于解决此问题。 在此期间，若要减少此问题的影响，将查询限制的 sys.event_log。 NewRelic SQL Server 插件的用户应访问[Microsoft Azure SQL Database 插件优化和性能优化](https://discuss.newrelic.com/t/microsoft-azure-sql-database-plugin-tuning-performance-tweaks/30729)有关其他配置信息。  
...
|**end_time**|**datetime2**|聚合间隔结束的 UTC 日期和时间。 对于聚合事件， **End_time**始终是比相应之后恰好 5 分钟**start_time**同一行中。 事件未聚合， **start_time**和**end_time**等于的实际 UTC 日期和时间的事件。|  
...
> [!NOTE]  
>  某些事件（如死锁）不会聚集。 对于这些事件， **event_count**将是 1 和**start_time**和**end_time**将等于实际 UTC 日期和时间发生该事件时。  
...
-   此视图不包括所有[!INCLUDE[ssSDS](../../includes/sssds-md.md)]数据库无法发生，只有在指定的错误[事件类型](../../relational-databases/system-catalog-views/sys-event-log-azure-sql-database.md#EventTypes)本主题中。  
...
### <a name="simple-examples"></a>简单示例  
 以下查询将返回在 9/25/2011 与 9/28/2011 (UTC) 之间发生的所有事件。 默认情况下，查询结果按排序**start_time** （升序）。  
