...
|**owning_principal_id**|**int**|拥有此主体的主体的 ID。 除数据库角色以外的所有主体都必须归**dbo**所有。|  
...
### <a name="d-listing-permissions-on-schema-objects-within-a-database"></a>D：列出数据库中架构对象的权限  
 下面的查询将 `sys.database_principals` 和 `sys.database_permissions` 联接到 `sys.objects` 和 `sys.schemas` 以列出向特定架构对象授予或拒绝的权限。  
