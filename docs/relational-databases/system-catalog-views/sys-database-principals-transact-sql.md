...
|**owning_principal_id**|**int**|拥有此主体的主体的 ID。 除数据库角色的所有主体必须都拥有的**dbo**。|  
...
|**authentication_type**|**int**|**适用范围**： [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] 到 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。<br /><br /> 指示身份验证类型。 以下是可能的值以及及其说明。<br /><br /> 0： 无身份验证<br />1： 实例身份验证<br />2： 数据库身份验证<br />3: Windows 身份验证|  
...
### <a name="d-listing-permissions-on-schema-objects-within-a-database"></a>D： 列出在数据库中的架构对象的权限  
 下面的查询联接`sys.database_principals`和`sys.database_permissions`到`sys.objects`和`sys.schemas`列表权限授予或拒绝对特定的架构对象。  
