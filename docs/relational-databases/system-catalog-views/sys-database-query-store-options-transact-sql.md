...
|**actual_state_desc**|**nvarchar(60)**|查询存储的实际操作模式的文本说明。<br />OFF<br />READ_ONLY<br />READ_WRITE<br />error<br /><br /> 当实际状态为不同于所需的状态时，有些情况下：<br />-如果将数据库设置为只读模式，或如果查询存储大小超过其配置的配额，即使用户已指定读写，Query Store 可能会在只读模式下操作。<br />-在极端情况下查询存储可以因内部错误而进入错误状态。 如果发生这种情况，SQL 2017 及更高版本，可以通过执行恢复查询存储`sp_query_store_consistency_check`受影响的数据库中的存储过程。 如果运行`sp_query_store_consistency_check`不起作用，适用于 SQL 2016，您需要通过运行清除数据 `ALTER DATABASE [YourDatabaseName] SET QUERY_STORE CLEAR ALL;`|  
...
|**size_based_cleanup_mode**|**smallint**|控制当数据总量接近最大大小时是否自动激活清除：<br /><br /> 0 = OFF-不会自动激活基于大小的清除。<br /><br /> **1** = AUTO-基于大小的清理将自动激活时磁盘上的大小达到**90%** 的*max_storage_size_mb*。 这是默认的配置值。<br /><br />基于大小的清除首先会删除成本最低和最旧的查询。 它将停止时大约**百分之八十**的*max_storage_size_mb*为止。|  
