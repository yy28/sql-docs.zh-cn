...
|**max_storage_size_mb**|**bigint**|查询存储的最大磁盘大小，以兆字节（MB）为单位。 默认值为**100** MB。<br />对于[!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)]高级版，默认值为 1 GB， [!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)]对于基本版，默认值为 10 MB。<br /><br /> 使用`ALTER DATABASE <database> SET QUERY_STORE (MAX_STORAGE_SIZE_MB = <size>)`语句进行更改。|  
