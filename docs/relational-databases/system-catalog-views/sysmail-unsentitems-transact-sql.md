...
ms.locfileid: "33221884"
---
# <a name="sysmailunsentitems-transact-sql"></a>sysmail_unsentitems (Transact-SQL)
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-xxx-md](../../includes/tsql-appliesto-ss2008-xxxx-xxxx-xxx-md.md)]

  每个数据库邮件消息中对应一行**未发送**或**重试**状态。 具有“未发送”或“正在重试”状态的消息仍位于邮件队列中，并可能随时发送。 消息可以具有**未发送**下列原因造成的状态：  
...
-   数据库邮件已尝试发送邮件，但无法与 SMTP 邮件服务器联系。 数据库邮件将继续尝试使用其他数据库邮件帐户（分配给发送消息的配置文件）发送消息。 如果没有帐户可以发送邮件，数据库邮件将等待的总时间为配置**帐户重试延迟时间**参数，然后尝试再次发送消息。 数据库邮件使用**帐户重试次数**参数，以确定多少次尝试发送消息。 消息保留**重试**只要数据库邮件尝试发送消息的状态。  
...
|**sent_status**|**varchar(8)**|将**未发送**如果不是数据库邮件尝试发送邮件。 将**重试**如果数据库邮件无法发送消息，但重试。|  
