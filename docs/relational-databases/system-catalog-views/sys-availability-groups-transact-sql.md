...
|**failure_condition_level**|**int**|用户定义的失败条件级别，在此级别下必须触发自动故障转移，其中一个整数值是此表下表中显示的一个整数值。<br /><br /> 失败条件级别的范围 (1-5) 是从最少限制的级别 1 到最多限制的级别 5。 给定的条件级别包含所有限制较少的级别。 因此，最严格的条件级别 5 包含四个限制较少的级别 (1-4)，级别 4 包含级别 1-3，依此类推。<br /><br /> 若要更改此值，请使用[ALTER AVAILABILITY GROUP](../../t-sql/statements/alter-availability-group-transact-sql.md) [!INCLUDE[tsql](../../includes/tsql-md.md)]语句的 FAILURE_CONDITION_LEVEL 选项。|  
|**health_check_timeout**|**int**|[Sp_server_diagnostics](../../relational-databases/system-stored-procedures/sp-server-diagnostics-transact-sql.md)系统存储过程返回服务器运行状况信息的等待时间（以毫秒为单位），假定服务器实例速度较慢或未响应。 默认值为 30000 毫秒（30 秒）。<br /><br /> 若要更改此值，请使用[ALTER AVAILABILITY GROUP](../../t-sql/statements/alter-availability-group-transact-sql.md) [!INCLUDE[tsql](../../includes/tsql-md.md)]语句的 HEALTH_CHECK_TIMEOUT 选项。|  
|**automated_backup_preference**|**tinyint**|用于对此可用性组中的可用性数据库执行备份的首选位置。 下面是可能的值及其说明。<br /><br /> <br /><br /> 0第. 备份应该始终在主副本上发生。<br /><br /> 2仅辅助副本。 首选是对辅助副本执行备份。<br /><br /> pps-2首选辅助。 首选是对辅助副本执行备份，但如果没有可用于备份操作的辅助副本，对主副本执行备份是可接受的。 这是默认行为。<br /><br /> 三维空间任何副本。 没有是对主副本执行备份还是对辅助副本执行备份的优先选择。<br /><br /> <br /><br /> 有关详细信息，请参阅[活动次要副本：次要副本备份（Always On 可用性组）](../../database-engine/availability-groups/windows/active-secondaries-backup-on-secondary-replicas-always-on-availability-groups.md)。|  
|**automated_backup_preference_desc**|**nvarchar(60)**|**Automated_backup_preference**的说明，其中之一：<br /><br /> PRIMARY<br /><br /> SECONDARY_ONLY<br /><br /> SECONDARY<br /><br /> 无|  
...
|ReplTest1|失败条件|  
|-----------|-----------------------|  
|1|指定在发生以下任何情况时应启动自动故障转移：<br /><br /> <br /><br /> [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] -服务已关闭。<br /><br /> -用于连接到 WSFC 故障转移群集的可用性组的租约过期，因为没有从服务器实例接收到 ACK。 有关详细信息，请参阅[工作原理：SQL Server Always On 租约超时](https://blogs.msdn.com/b/psssql/archive/2012/09/07/how-it-works-sql-server-Always%20On-lease-timeout.aspx)。|  
