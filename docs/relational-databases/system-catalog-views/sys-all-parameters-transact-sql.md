...
|**路径名**|**sysname**|参数的名称， 在对象中是唯一的。 如果对象是标量函数，则参数名称为表示返回值的行中的空字符串。|  
