...
-   如果数据库数据类型的长度可变（例如， **varbinary （22）** ）或数据库列可以包含 null 值，则数据文件中的数据将以长度/空指示器作为前缀。 指示符的宽度因数据类型和大容量复制的版本而异。  
 ...
 如果未使用任何数据文件，则必须调用[bcp_bind](../../relational-databases/native-client-odbc-extensions-bulk-copy-functions/bcp-bind.md)来指定每列的数据的格式和位置，然后使用[bcp_sendrow](../../relational-databases/native-client-odbc-extensions-bulk-copy-functions/bcp-sendrow.md)将数据行复制到 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。  
