...
-   如果某个数据库数据类型的长度的变量 (例如， **varbinary(22)**) 或数据库列可以包含 null 值，如果数据文件中的数据由长度/null 指示符的前缀。 指示符的宽度因数据类型和大容量复制的版本而异。  
 ...
 如果使用没有数据文件，则必须调用[bcp_bind](../../relational-databases/native-client-odbc-extensions-bulk-copy-functions/bcp-bind.md)若要指定的格式和位置的数据 fsor 内存中的每个列，然后向其中复制数据行[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]使用[bcp_sendrow](../../relational-databases/native-client-odbc-extensions-bulk-copy-functions/bcp-sendrow.md)。  
