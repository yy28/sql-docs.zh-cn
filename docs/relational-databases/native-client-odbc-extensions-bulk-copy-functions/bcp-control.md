 ...
 *eOption*  
 ...
 BCPABORT  
 停止正在进行的大容量复制操作。 调用**bcp_control**与*eOption* BCPABORT 从另一个线程，以停止正在运行的大容量复制操作。 *IValue*参数将被忽略。  
 ...
 BCPDELAYREADFMT  
 布尔值，如果设置为 true，将导致[bcp_readfmt](../../relational-databases/native-client-odbc-extensions-bulk-copy-functions/bcp-readfmt.md)读取在执行。 如果为 false （默认值），bcp_readfmt 立即将读取格式化文件。 如果 BCPDELAYREADFMT 为 true，并且你调用 bcp_columns 或 bcp_setcolfmt，将发生序列错误。  
  
 如果调用，也会出现序列错误`bcp_control(hdbc,`BCPDELAYREADFMT`, (void *)FALSE)`之后调用`bcp_control(hdbc,`BCPDELAYREADFMT`, (void *)TRUE)`和 bcp_writefmt。  
 ...
 *IValue*参数应为一个带符号的 64 位整数，包含的值的地址。 可以传递给 BCPFIRSTEX 的最大值为 2 ^63-1。  
  
 BCPFMTXML  
 指定生成的格式化文件应采用 XML 格式。 默认情况下将禁用此选项。  
  
 XML 格式化文件提供更大的灵活性，但具有某些添加约束。 例如，你不能指定的前缀和字段的终止符同时，这是可能在较旧的格式文件中。  
 ...
 *iValue*包含 SQLTCHAR 字符字符串指针。 寻址的字符串指定 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 大容量复制处理提示或返回结果集的 Transact-SQL 语句。 如果指定的 Transact-SQL 语句返回多个结果集，则忽略第一个结果集之后的所有结果集。 有关大容量复制处理提示的详细信息，请参阅[bcp 实用工具](../../tools/bcp-utility.md)。  
 ...
 BCPKEEPIDENTITY  
 当*iValue*为 TRUE，指定大容量复制函数插入为提供的数据值[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]使用 identity 约束定义的列。 输入文件必须提供标识列的值。 如果未进行设置，则为插入的行生成新标识值。 忽略文件的标识列中所存在的任何数据。  
 ...
 *IValue*参数应为一个带符号的 64 位整数，包含的值的地址。 可以传递到 BCPLASTEX 的最大值为 2^63-1。  
 ...
 *iValue*  
 为指定的值*eOption*。 *iValue*一个整数 (LONGLONG) 值强制转换为 void 的指针，以便将来扩展为 64 位值。  
...
## <a name="remarks"></a>注释  
 ...
 从 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 大容量复制 SELECT 的结果集时，此函数还可用于指定 SELECT 语句。 设置*eOption*到 BCPHINTS 和集*iValue*要有一个指针指向包含 SELECT 语句的 SQLTCHAR 字符串。  
  
 仅当在用户文件和 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 表之间进行复制时，这些控制参数才有意义。 控制参数设置对行复制到没有影响[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]与[bcp_sendrow](../../relational-databases/native-client-odbc-extensions-bulk-copy-functions/bcp-sendrow.md)。  
