...
[!INCLUDE[SNAC_Deprecated](../../includes/snac-deprecated.md)]
 ...
 该函数提供在大容量复制操作中指定列格式的灵活方法。 它用于设置单独的列格式属性。 对**bcp_setcolfmt**的每次调用都将设置一个列格式特性。  
  
 **Bcp_setcolfmt**函数指定用户文件中数据的源或目标格式。 用作源格式时， **bcp_setcolfmt**指定现有数据文件的格式，该数据文件在大容量复制中用作数据源数据源到 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的表中。 作为目标格式使用时，将使用**bcp_setcolfmt**指定的列格式创建数据文件。  
...
## <a name="remarks"></a>备注  
 此函数取代了**bcp_colfmt**函数。 **Bcp_setcolfmt**函数提供了**bcp_colfmt**的所有功能。 此外，还提供对列排序规则的支持。 建议按照下面给出的顺序设置以下列格式属性：  
...
-   可选终止字节序列的长度。  
 ...
 对**bcp_setcolfmt**的每次调用都会指定一个用户文件列的格式。 例如，若要更改五行用户数据文件中三列的默认设置，请首先调用[bcp_columns](../../relational-databases/native-client-odbc-extensions-bulk-copy-functions/bcp-columns.md) **（5）** ，然后调用**bcp_setcolfmt**五次，其中三个调用设置您的自定义格式。 对于剩余的两次调用，将 BCP_FMT_TYPE 设置为0，并将 BCP_FMT_INDICATOR_LENGTH、BCP_FMT_DATA_LEN 和*cbValue*分别设置为0、SQL_VARLEN_DATA 和0。 此过程复制全部五列，其中的三列采用您的自定义格式，另两列采用默认格式。  
  
 调用**bcp_setcolfmt**之前，必须先调用**bcp_columns**函数。  
  
 对于用户文件中每个列的每个属性，都必须调用**bcp_setcolfmt**一次。  
