...
>  为区分非 XML 格式化文件，我们建议使用 .fmt 作为文件扩展名，例如 MyTable.fmt。  
 ...
 有关非 XML 格式化文件结构和字段的信息，请参阅[非 XML 格式化文件 (SQL Server)](../../relational-databases/import-export/non-xml-format-files-sql-server.md)。  
...
#### F. 使用具有代码页选项的格式化文件  
 如果使用 bcp 命令创建格式化文件（例如 使用“`bcp forma`t …” 有关排序规则/代码页的信息将写入格式化文件中。   
...
>  为区分 XML 格式化文件，我们建议使用 .xml 作为文件扩展名，例如 MyTable.xml。  
 ...
 有关 XML 格式化文件结构和字段的信息，请参阅 [XML 格式化文件 (SQL Server)](../../relational-databases/import-export/xml-format-files-sql-server.md)。  
