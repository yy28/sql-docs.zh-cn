...
helpviewer_keywords: 
  ...
  - "数据 [SQL Server]，批量导出和导入"
  - "传输数据"
  - "导入数据，（还请参阅批量导入 [SQL Server]）"
  - "复制数据 [SQL Server]"
  - "大容量导出 [SQL Server]"
  - "导入数据，批量导入"
  - "复制数据 [SQL Server]，批量导出和导入"
  - "导出数据，（还请参阅批量导出 [SQL Server]）"
  - "批量导出 [SQL Server]，关于批量导出"
  - "大容量导入 [SQL Server]"
  - "导入数据"
...
# 大容量导入和导出数据 (SQL Server)
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 支持从 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 表大容量（“大容量数据”）导出数据以及将大容量数据导入 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 表或未分区的视图。 
...
|[在大容量导入期间保留 Null 或使用默认值 (SQL Server)](../../relational-databases/import-export/keep-nulls-or-use-default-values-during-bulk-import-sql-server.md)|
