...
description: 只能使用 bcp 命令、BULK INSERT 语句或 OPENROWSET(BULK) 函数从数据文件批量导入数据。
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
-   为用户帐户获取所需的权限。  
     ...
     用户帐户，你在其中使用 **bcp** 实用工具、BULK INSERT 语句或 INSERT ...SELECT * FROM OPENROWSET(BULK...) 语句的用户帐户必须具有表的所需权限，这些权限由表所有者分配。 有关每个方法所需的权限的详细信息，请参阅 [bcp 实用工具、](../../tools/bcp-utility.md)[OPENROWSET (Transact SQL)](../../t-sql/functions/openrowset-transact-sql.md) 和 [BULK INSERT (Transact SQL)](../../t-sql/statements/bulk-insert-transact-sql.md)。  
...
-   大容量导入数据后进行备份。  
     ...
     对于使用简单恢复模式的数据库，建议您在大容量导入操作完成后执行完整备份或差异备份。 有关详细信息，请参阅[创建完整数据库备份 (SQL Server)](../../relational-databases/backup-restore/create-a-full-database-backup-sql-server.md) 或[创建差异数据库备份 (SQL Server)](../../relational-databases/backup-restore/create-a-differential-database-backup-sql-server.md)。  
  
     对于大容量日志恢复模式或完整恢复模式，只需执行日志备份就足够了。 有关详细信息，请参阅[事务日志备份 (SQL Server)](../../relational-databases/backup-restore/transaction-log-backups-sql-server.md)。  
