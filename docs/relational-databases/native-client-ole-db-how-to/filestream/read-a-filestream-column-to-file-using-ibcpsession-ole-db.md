---
description: '使用 SQL Server Native Client (OLE DB 中的 IBCPSession 将 FILESTREAM 列读入文件) '
title: 'FILESTREAM 到文件，IBCPSession (OLE DB) '
ms.custom: ''
ms.date: 03/07/2017
ms.prod: sql
...
ms.locfileid: "88407263"
---
# <a name="read-a-filestream-column-to-file-using-ibcpsession-in-sql-server-native-client-ole-db"></a>使用 SQL Server Native Client (OLE DB 中的 IBCPSession 将 FILESTREAM 列读入文件) 
...
-   [使用绑定到 ICommandText 参数的 ISequentialStream 向 FILESTREAM 列发送数据 (OLE DB)](../../../relational-databases/native-client-ole-db-how-to/filestream/send-data-to-filestream-isequentialstream-bound-to-icommandtext.md)  
  
-   [使用 IRowsetFastUpload 向 FILESTREAM 列发送数据 (OLE DB)](../../../relational-databases/native-client-ole-db-how-to/filestream/send-data-to-a-filestream-column-using-irowsetfastupload-ole-db.md)  
  
 复制第一个代码列表，并将其粘贴到名为 ISSHelper.h 的文件中。  
  
 复制第二个代码列表，并将其粘贴到名为 ISSHelper.cpp 的文件中。  
  
 复制第三个代码列表，并将其粘贴到名为 IBCPSession.cpp 的文件中。  
  
 编译 IBCPSession.cpp、ISSHelper.cpp、ole32.lib 和 oleaut32.lib。  
  
 运行该示例时，必须传递一个服务器名称（或 server\instance_name）以及一个将创建的格式化文件的名称。  
