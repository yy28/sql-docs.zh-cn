---
description: '使用 SQL Server Native Client (OLE DB 中的 ISequentialStream 从 FILESTREAM 列检索数据) '
...
ms.locfileid: "88455710"
---
# <a name="retrieve-data-from-a-filestream-column-using-isequentialstream-in-sql-server-native-client-ole-db"></a>使用 SQL Server Native Client (OLE DB 中的 ISequentialStream 从 FILESTREAM 列检索数据) 
...
-   [使用 IRowsetFastUpload 向 FILESTREAM 列发送数据 (OLE DB)](../../../relational-databases/native-client-ole-db-how-to/filestream/send-data-to-a-filestream-column-using-irowsetfastupload-ole-db.md)  
 ...
 复制第三个代码列表，并将其粘贴到名为 ICommandDownload.cpp 的文件中。  
  
 编译 ICommandDownload.cpp、ISSHelper.cpp、ole32.lib 和 oleaut32.lib。  
  
 运行此示例时，必须传递一个服务器名称（或 server\instance_name）。  
