...
2.  如果你正在创建一个新数据库，则右键单击“数据库”，然后单击“新建数据库”。 如果不希望使用默认排序规则，则单击“选项”页，然后从“排序规则”下拉列表中选择某一排序规则。  
...
3.  将以下示例复制并粘贴到查询窗口中，然后单击 **“执行”**。 此示例演示如何使用 [COLLATE](../Topic/COLLATE%20\(Transact-SQL\).md) 子句来指定排序规则名称。 此示例创建使用 `MyOptionsTest` 排序规则的数据库 `Latin1_General_100_CS_AS_SC` 。 在创建数据库后，执行 `SELECT` 语句以验证设置。  
...
3.  将以下示例复制并粘贴到查询窗口中，然后单击 **“执行”**。 此示例说明如何在 [ALTER DATABASE](../Topic/COLLATE%20\(Transact-SQL\).md) 语句中使用 [COLLATE](../../t-sql/statements/alter-database-transact-sql.md) 子句来更改排序规则名称。 执行 `SELECT` 语句以验证更改。  
