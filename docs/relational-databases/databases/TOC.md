# [将数据库设置为单用户模式](set-a-database-to-single-user-mode.md)
# [辅助数据库设置](secondary-database-settings.md)
# [分离数据库](detach-a-database.md)
# [数据库属性（查询存储页）](database-properties-query-store-page.md)
# [Resource 数据库](resource-database.md)
# [数据库分离和附加](database-detach-and-attach-sql-server.md)
# [“扩展属性的值”对话框](value-for-extended-property-dialog-box.md)
# [收缩文件](shrink-a-file.md)
# [tempdb 数据库](tempdb-database.md)
# [数据库对象（“扩展属性”页）](database-object-extended-properties-page.md)
# [显示数据库的数据和日志空间信息](display-data-and-log-space-information-for-a-database.md)
# [更改数据库的配置设置](change-the-configuration-settings-for-a-database.md)
# [估计非聚集索引的大小](estimate-the-size-of-a-nonclustered-index.md)
# [通过备份和还原来复制数据库](copy-databases-with-backup-and-restore.md)
# [数据库状态](database-states.md)
# [附加数据库](attach-a-database.md)
# [文件状态](file-states.md)
# [创建用户定义的数据类型别名](create-a-user-defined-data-type-alias.md)
# [重新生成系统数据库](rebuild-system-databases.md)
# [移动用户数据库](move-user-databases.md)
# [删除数据库快照 (Transact-SQL)](drop-a-database-snapshot-transact-sql.md)
# [数据库属性（“选项”页）](database-properties-options-page.md)
# [经过修改的功能（包含数据库）](modified-features-contained-database.md)
# [查看或更改数据库的属性](view-or-change-the-properties-of-a-database.md)
# [数据库文件和文件组](database-files-and-filegroups.md)
# [数据库属性（“更改跟踪”页）](database-properties-changetracking-page.md)
# [日志传送事务日志备份设置](log-shipping-transaction-log-backup-settings.md)
# [使用分离和附加来升级数据库 (Transact-SQL)](upgrade-a-database-using-detach-and-attach-transact-sql.md)
# [发布数据库 (SQL Server Management Studio)](publish-a-database-sql-server-management-studio.md)
# [将数据库复制到其他服务器](copy-databases-to-other-servers.md)
# [迁移到部分包含的数据库](migrate-to-a-partially-contained-database.md)
# [将数据库恢复到数据库快照](revert-a-database-to-a-database-snapshot.md)
# [向数据库中添加数据文件或日志文件](add-data-or-log-files-to-a-database.md)
# [数据库属性（“文件组”页）](database-properties-filegroups-page.md)
# [移动数据库文件](move-database-files.md)
# [收缩数据库](shrink-a-database.md)
# [估计堆的大小](estimate-the-size-of-a-heap.md)
# [查看 SQL Server 实例的数据库列表](view-a-list-of-databases-on-an-instance-of-sql-server.md)
# [使用复制数据库向导](use-the-copy-database-wizard.md)
# [数据库属性（“事务日志传送”页）](database-properties-transaction-log-shipping-page.md)
# [移动系统数据库](move-system-databases.md)
# [通过分离和附加来移动数据库 (Transact-SQL)](move-a-database-using-detach-and-attach-transact-sql.md)
# [master 数据库](master-database.md)
# [将 SQL Server 数据库部署到 Microsoft Azure 虚拟机](deploy-a-sql-server-database-to-a-microsoft-azure-virtual-machine.md)
# [当数据库在其他服务器实例上可用时管理元数据](manage metadata when making a database available on another server.md)
# [数据库属性（“镜像”页）](database-properties-mirroring-page.md)
# [估计数据库的大小](estimate-the-size-of-a-database.md)
# [查看或更改数据库的兼容级别](view-or-change-the-compatibility-level-of-a-database.md)
# [msdb 数据库](msdb-database.md)
# [model 数据库](model-database.md)
# [创建数据库](create-a-database.md)
# [包含数据库的排序规则](contained-database-collations.md)
# [日志传送监视器设置](log-shipping-monitor-settings.md)
# [重命名数据库](rename-a-database.md)
# [数据库属性（“常规”页）](database-properties-general-page.md)
# [数据库属性（“文件”页）](database-properties-files-page.md)
# [Microsoft Azure 中的 SQL Server 数据文件](sql-server-data-files-in-microsoft-azure.md)
# [包含的数据库](contained-databases.md)
# [数据库（数据库引擎）](databases.md)
# [系统数据库](system-databases.md)
# [估计聚集索引的大小](estimate-the-size-of-a-clustered-index.md)
# [删除数据库](delete-a-database.md)
# [数据库实例文件初始化](database-instant-file-initialization.md)
# [创建数据库快照 (Transact-SQL)](create-a-database-snapshot-transact-sql.md)
# [查看数据库快照的稀疏文件大小 (Transact-SQL)](view-the-size-of-the-sparse-file-of-a-database-snapshot-transact-sql.md)
# [数据库标识符](database-identifiers.md)
# [估计表的大小](estimate-the-size-of-a-table.md)
# [增加数据库的大小](increase-the-size-of-a-database.md)
# [查看数据库快照 (SQL Server)](view-a-database-snapshot-sql-server.md)
# [删除数据库中的数据文件或日志文件](delete-data-or-log-files-from-a-database.md)
# [针对包含数据库的安全性最佳方法](security-best-practices-with-contained-databases.md)
# [数据库快照](database-snapshots-sql-server.md)
