...
description: 了解包含的数据库、部分包含的数据库的优势和局限性，以及 SQL Server 2019 隔离数据库的方式。
ms.custom: ''
ms.date: 08/24/2016
ms.prod: sql
...
-   **Windows 主体**  
 ...
 基于 **master** 数据库中登录名的用户可被授予对包含数据库的访问权限，但将在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例上创建依赖关系。 因此，基于登录创建用户需要部分包含。
