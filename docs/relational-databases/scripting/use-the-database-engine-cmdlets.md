...
|任务说明|主题|  
|----------------------|-----------|  
|介绍使用 **Invoke-Sqlcmd** 运行包含 [!INCLUDE[tsql](../../includes/tsql-md.md)] 或 XQuery 语句的 **sqlcmd** 脚本或命令。 它可以接受 **sqlcmd** 输入作为字符串输入参数或要打开的脚本文件的文件名。|[Invoke-Sqlcmd cmdlet](../../powershell/invoke-sqlcmd-cmdlet.md)|  
