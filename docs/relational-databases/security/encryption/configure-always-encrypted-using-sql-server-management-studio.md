...
如果你还没有为 Always Encrypted 预配任何密钥，向导会自动为你生成密钥。 只需为列主密钥选择密钥存储：Windows 证书存储或 Azure Key Vault。 向导会自动在数据库中为密钥生成名称及其元数据对象。 如果你需要更好地控制密钥预配方式（并对包含列主密钥的密钥存储有更多选择），可以使用“新建列主密钥”  和“新建列加密密钥”  对话框（如下所述）在启动该向导之前预配密钥。 然后就可以在始终加密向导中选择现有的列加密密钥。
...
1.  确保已为其中运行 `SELECT` 查询的“查询编辑器”窗口的数据库连接禁用 Always Encrypted。 请参阅下面的 [为数据库连接启用和禁用 Always Encrypted](#en-dis) 。      
...
从加密列中检索值作为纯文本（用于解密值）：   
1.  确保已为其中运行 `SELECT` 查询的“查询编辑器”窗口的数据库连接启用 Always Encrypted。 这将指示用于 SQL Server（ SSMS 使用）的 .NET Framework 数据提供程序对从加密列中检索到的数据进行解密。 请参阅下面的 [为数据库启用和禁用 Always Encrypted](#en-dis) 。
...
1. 确保已为其中运行 `SELECT` 查询的“查询编辑器”窗口的数据库连接启用 Always Encrypted。 这将指示用于 SQL Server 的 .NET Framework 数据提供程序（用于 SSMS）对针对加密列的参数化 Transact-SQL 变量（见下文）进行加密。 请参阅下面的 [为数据库启用和禁用 Always Encrypted](#en-dis) 。
2. 请确保可以访问所有为加密列配置的列主密钥。 例如，如果列主密钥是证书，请确保在运行 SSMS 的计算机上部署此证书。 或者，如果列主密钥是存储在 Azure 密钥保管库中的密钥，请确保你有权访问此密钥，此外，可能会提示你登录到 Azure。
3. 请确保已为“查询编辑器”窗口启用 Always Encrypted 参数化。 （要求至少为 SSMS 版本 17.0。）声明 Transact-SQL 变量，并使用要发送（插入、更新或筛选）到数据库的值对它进行初始化。 有关详细信息，请参阅下面的 [Always Encrypted 参数化](#param) 。
...
示例  ：假设 `SSN` 是 `Patients` 表中的加密 `char(11)` 列，如果已为数据库连接启用 Always Encrypted，并已为“查询编辑器”窗口启用 Always Encrypted 参数化，同时你有权访问为 `SSN` 列配置的列主密钥，那么下面的脚本会尝试在 SSN 列中查找包含 `'795-73-9838'` 的行，并返回 `LastName` 列的值。   
...
若要为数据库连接禁用 Always Encrypted，请指定 `Column Encryption Setting=Disabled`，或从“连接到服务器”  对话框的“其他属性”  选项卡中删除“列加密设置”  设置（它的默认值为“已禁用”  ）。   
...
#### <a name="permissions-for-querying-against-encrypted-columns"></a>对加密列运行查询所需的权限
...
4.  对于使用旧列加密密钥进行加密的每个列，将“加密密钥”  设置为自动生成的新密钥。 **注意：** 或者，你也可以在运行该向导之前创建新的列加密密钥 - 请参阅上面的“预配列加密密钥”部分  。
...
- 数据库包含一个具有数据的列。
- DACPAC 中存在相同的列。
- 数据库中的列的加密配置不同于 DACPAC 中相应列的配置。 有关详细信息，请参阅下表。
...
### <a name="permissions-for-dac-upgrade-if-always-encrypted-is-set-up"></a>在已设置 Always Encrypted 的情况下执行 DAC 升级所需的权限

若要在 DACPAC 或目标数据库中设置了 Always Encrypted 的情况下执行 DAC 升级操作，可能需要以下部分或所有权限，具体视 DACPAC 架构与目标数据库架构的差异而定。
...
如果应用程序配置为修改或检索源数据库（已导出的数据库）中存储的加密数据，无需执行任何特殊操作，即可让应用程序查询目标数据库中的加密数据，因为这两个数据库中的密钥相同。
...
| 迁移时解密数据（数据存储在数据源的加密列中，并以纯文本形式迁移到数据目标；如果数据目标是数据库，则不会加密目标列）。<br><br>**注意：** 在迁移之前，必须存在包含加密列的目标表。|数据提供程序/驱动程序： *用于 SQL Server 的 .Net Framework 数据提供程序* （需要 .NET Framework 4.6 或更高版本）<br><br>列加密设置=已启用|数据提供程序/驱动程序： *任意*<br><br>列加密设置 = 已禁用<br><br>（如果使用用于 SQL Server 的 .Net Framework 数据提供程序和 .NET Framework 4.6 或更高版本。）
|迁移时重新加密数据（数据存储在数据源的加密列中，并以纯文本形式迁移到数据目标中使用不同加密类型列加密密钥的列）。<br><br>**注意：** 在迁移之前，必须存在包含加密列的目标表。| 数据提供程序/驱动程序： *用于 SQL Server 的 .Net Framework 数据提供程序* （需要 .NET Framework 4.6 或更高版本）<br><br>列加密设置=已启用|数据提供程序/驱动程序： *用于 SQL Server 的 .Net Framework 数据提供程序* （需要 .NET Framework 4.6 或更高版本）<br><br>列加密设置=已启用
...
### <a name="permissions-for-database-migration-using-sql-server-import-and-export-wizard"></a>使用 SQL Server 导入和导出向导执行数据库迁移所需的权限

若要 **加密** 或 **解密** 数据源中存储的数据，需要源数据库中的 *查看任意列主密钥定义* 和 *查看任意列加密密钥定义* 权限。

还必须有权访问为列配置的列主密钥，这些列用于存储要加密或解密的数据：
