---
title: 使用 Azure Key Vault 的可扩展密钥管理
description: 使用 SQL Server 连接器在 SQL Server 中通过 Azure Key Vault 实现可扩展性密钥管理。
