---
title: "迁移通过“始终加密”保护的敏感数据 | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "11/04/2015"
...
---
# 迁移通过“始终加密”保护的敏感数据
  若要在大容量复制操作期间加载加密数据，而不用在服务器上执行元数据检查，请使用 **ALLOW_ENCRYPTED_VALUE_MODIFICATIONS** 选项创建用户。 此选项旨在由早于 [!INCLUDE[ssSQL15](../../../includes/sssql15-md.md)]（如 bcp.exe）的 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 版本的旧式工具使用，或由不能使用“始终加密”的第三方提取-转换-加载 (ETL) 工作流使用。 这使用户可以安全地将加密数据从包含加密列的一组表中移动到具有加密列的另一组表中（在相同或不同的数据库中）。  
