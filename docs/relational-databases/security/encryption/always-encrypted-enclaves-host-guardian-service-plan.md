---
title: 规划主机保护者服务证明 | Microsoft Docs
ms.custom: ''
ms.date: 10/12/2019
ms.prod: sql
ms.reviewer: vanto
...
运行 [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] 的计算机无需加入到域中，但如果它已加入，则它应与 HGS 服务器使用的域不同。
...
SQL 客户端和 [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] 都需要能够通过 HTTP 与 HGS 通信。 可以选择使用 TLS 证书配置 HGS 以支持加密 HTTPS 连接。
...
TPM 证明需要安全启动，以确保 UEFI 加载了合法的 Microsoft 签名引导加载程序，并且没有 rootkit 或 bootkit 截获了虚拟机监控程序启动过程。
...
| RAM | 8 GB（最少） |
| NIC | 建议使用 2 个 NIC（1 个用于群集流量，1 个用于 HGS 服务） |
