...
- **PolyBase：** 仅支持对 Azure SQL 数据仓库的 PolyBase 外部表使用 RLS。
...
通过已创建的 Sales 表，创建 Azure SQL 数据仓库外部表。
...
将会话 A 中的函数用作筛选谓词，对外部表创建安全策略。 状态必须设置为 ON 以启用该策略。
