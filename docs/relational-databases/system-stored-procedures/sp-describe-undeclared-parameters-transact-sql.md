 ...
 **sp_describe_undeclared_parameters**返回始终返回状态为成功的零。 如果该过程将引发错误，并且作为 RPC 调用该过程，sys.dm_exec_describe_first_result_set error_type 列中所述的错误的类型由填充返回的状态。 如果该过程是从 [!INCLUDE[tsql](../../includes/tsql-md.md)] 中调用的，则返回值始终为零，甚至在出现错误情况时也不例外。  
...
|**suggested_assembly_qualified_type_name**|**nvarchar (4000) NULL**|对于 CLR 类型，返回的程序集和定义的类型的类的名称。 否则为 NULL。|  
...
|**formal_parameter_name**|**sysname NULL**|如果参数是存储过程或用户定义的函数的参数，则返回相应形参的名称。 否则返回 NULL。|  
|**suggested_tds_type_id**|**int NOT NULL**|供内部使用。|  
|**suggested_tds_length**|**int NOT NULL**|供内部使用。|  
  
## <a name="remarks"></a>注释  
 **sp_describe_undeclared_parameters**返回始终返回状态为零。  
  
 最常见的用途是，为应用程序提供的 [!INCLUDE[tsql](../../includes/tsql-md.md)] 语句可能包含一些参数，并且必须以某种方式处理这些参数。 示例是用户提供具有 ODBC 参数语法的查询的其中一个用户界面 （如 ODBCTest 或 RowsetViewer）。 应用程序必须动态查找参数数目，并提示用户输入每个参数。  
  
 另一个例子是，在没有用户输入时，应用程序必须循环访问这些参数，并从某个其他位置（例如，表）获取这些参数的数据。 在这种情况下，应用程序不必同时传递所有参数信息。 相反，应用程序可以从提供程序中获取所有参数信息，并从表中获取数据本身。 使用代码**sp_describe_undeclared_parameters**是更泛型，并就很难如果更高版本的数据结构发生更改，则需要修改。  
  
 **sp_describe_undeclared_parameters**在任何以下情况下将返回错误。  
...
-   如果该语句将创建任何临时表。  
 ...
 如果@tsql没有参数，而非中声明@params，该过程返回空结果集。  
 ...
 **步骤 2**  
 ...
 为给定的未声明参数@p，类型推导算法查找最里面的表达式 E (@p)，该字符串包含@p而是以下之一：  
...
-   自变量**强制转换**或**转换**。  
 ...
 类型参数的类型推导算法找到目标数据类型 TT (@p) e (@p)。 上述示例的目标数据类型如下所示：  
...
-   要将语句转换到的数据类型。  
 ...
 例如，请考虑 `SELECT * FROM t WHERE @p1 = dbo.tbl(@p2 + c1)` 查询。 然后 E (@p1) = @p1，E (@p2) = @p2 + c1，TT (@p1) 是 dbo.tbl 和 TT 的声明的返回数据类型 (@p2) 是 dbo.tbl 的声明的参数数据类型。  
  
 如果@p中的步骤 2，开头列出的类型推导算法确定的任何表达式不包含该 E (@p) 是包含最大标量表达式@p，并不执行类型推导算法计算目标数据类型 TT (@p) e (@p)。 例如，如果查询选择`@p + 2`然后 E (@p) = @p + 2，并且没有任何 TT (@p)。  
  
 **步骤 3**  
  
 现在该 E (@p) 和 TT (@p) 是标识，类型推导算法会推导的数据类型@p中的以下两种方法之一：  
  
-   简单推断  
  
     如果 E (@p) =@p和 TT (@p) 存在，即，如果@p是直接在步骤 2，类型推导算法开头列出的表达式之一的自变量推导的数据类型@p要 TT (@p). 例如：  
  
    ```  
    SELECT * FROM t WHERE c1 = @p1 AND @p2 = dbo.tbl(@p3)  
    ```  
  
     数据类型@p1， @p2，和@p3将分别为 c1 的数据类型、 dbo.tbl，返回数据类型和 dbo.tbl 的参数数据类型。  
  
     作为一种特殊情况，如果@p是到的自变量\<，>， \<=、 或 > = 运算符，规则不适用于简单推导。 类型推断算法使用下一节介绍的一般推断规则。 例如，如果 c1 是 char(30) 数据类型的列，请考虑下面两个查询：  
...
-   一般推断  
    ...
    -   CLR 系统定义的类型 (**hierarchyid**，**几何图形**， **geography**)  
...
1.  生成最少数量的 E 中的隐式转换的数据类型 (@p) 选择。 如果特定的数据类型将为 E 生成的数据类型 (@p) 不同于 TT (@p)，类型推导算法将此视为额外隐式转换 E 的数据类型 (@p) 到 TT (@p)。  
    ...
    ```  
     ...
     在此情况下，E (@p) 是 Col_Int +@p和 TT (@p) 是**int**。**int**为选择@p因为它会不生成任何隐式转换。 选择任何其他数据类型都会产生至少一次隐式转换。  
...
4.  就规则 1 而言，类型推断算法倾向于将某些转换视为比其他转换好。 转换从最好到最坏依次为：  
    ...
    2.  具有相同的数据类型的固定长度和可变长度版本之间的转换 (例如， **char**到**varchar**)。  
