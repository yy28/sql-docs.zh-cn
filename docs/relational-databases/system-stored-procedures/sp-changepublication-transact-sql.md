...
|**allow_partition_switch**|**true**|ALTER TABLE...可以对已发布数据库执行 SWITCH 语句。 有关详细信息，请参阅[复制已分区表和索引](../../relational-databases/replication/publish/replicate-partitioned-tables-and-indexes.md)。|  
...
||**false**|ALTER TABLE...SWITCH 语句不应复制到订阅服务器中。|  
|**retention**||**int**表示的保持期，以小时为单位，订阅活动。 如果订阅在保持期内不活动，则将其删除。|  
