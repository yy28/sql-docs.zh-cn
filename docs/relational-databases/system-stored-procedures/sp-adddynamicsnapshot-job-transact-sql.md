...
`[ @suser_sname = ] 'suser_sname'`为订阅创建筛选数据快照时使用的值, 该订阅按订阅服务器上的[SUSER_SNAME](../../t-sql/functions/suser-sname-transact-sql.md)函数的值进行筛选。 *suser_sname*的值为**sysname**, 无默认值。 如果此函数未用于动态筛选发布, 则*suser_sname*应为 NULL。  
  
`[ @host_name = ] 'host_name'`为订阅创建筛选数据快照时使用的值, 该订阅按订阅服务器上[HOST_NAME](../../t-sql/functions/host-name-transact-sql.md)函数的值进行筛选。 *host_name*的值为**sysname**, 无默认值。 如果不使用此函数对发布进行动态筛选, 则*host_name*应为 NULL。  
  
`[ @dynamic_snapshot_jobname = ] 'dynamic_snapshot_jobname'`创建的筛选数据快照作业的名称。 *dynamic_snapshot_jobname*的值为**sysname**, 默认值为 NULL, 是一个可选的 OUTPUT 参数。 如果指定, 则*dynamic_snapshot_jobname*必须在分发服务器上解析为唯一作业。 如果不指定，则将自动生成并在结果集中返回一个作业名，该名称的创建方式如下：  
...
`[ @dynamic_snapshot_jobid = ] 'dynamic_snapshot_jobid'`是已创建的筛选数据快照作业的标识符。 *dynamic_snapshot_jobid*的默认值为**uniqueidentifier**, 默认值为 NULL, 是一个可选的 OUTPUT 参数。  
  
`[ @frequency_type = ] frequency_type`用于计划筛选的数据快照作业的频率。 *frequency_type*的数据值为**int**, 可以是下列值之一。  
...
`[ @frequency_interval = ] frequency_interval`执行筛选数据快照作业的时间段 (以天计)。 *frequency_interval*的值为**int**, 默认值为 1, 具体取决于*frequency_type*的值。  
  
|*Frequency_type*的值|对*frequency_interval*的影响|  
|--------------------------------|-------------------------------------|  
|**1**|*frequency_interval*未使用。|  
|**4** (默认值)|每*frequency_interval*天, 默认为每天。|  
|**8**|*frequency_interval*是以下一个或多个 (与[ &#124; &#40;按位&#41; &#40;&#41; or transact-sql](../../t-sql/language-elements/bitwise-or-transact-sql.md)逻辑运算符组合):<br /><br /> **1** = 星期日&#124; **2** = 星期一&#124; **4** = 星期二&#124; **8** = 星期三&#124; **16** = 星期四&#124; **32** = 星期五&#124; **64** = 星期六|  
|**16**|在每月的*frequency_interval*日期。|  
|**32**|*frequency_interval*是下列其中一项:<br /><br /> **1** = 星期日&#124; **2** = 星期一&#124; **3** = 星期二&#124; **4** = 星期三&#124; **5** = 星期四&#124; **6** = 星期五&#124; **7** = 星期六&#124; **8** = 天&#124;**9** = 工作日&#124; **10** = 周末|  
|**64**|*frequency_interval*未使用。|  
|**128**|*frequency_interval*未使用。|  
  
`[ @frequency_subday = ] frequency_subday`指定*frequency_subday_interval*的单位。 *frequency_subday*的数据值为**int**, 可以是下列值之一。  
...
`[ @frequency_subday_interval = ] frequency_subday_interval`每次执行作业之间出现的*frequency_subday*时间段数。 *frequency_subday_interval*的值为**int**, 默认值为5。  
  
`[ @frequency_relative_interval = ] frequency_relative_interval`每月筛选的数据快照作业的发生次数。 当*frequency_type*设置为**32** (每月相对) 时, 使用此参数。 *frequency_relative_interval*的数据值为**int**, 可以是下列值之一。  
...
`[ @active_start_time_of_day = ] active_start_time_of_day`第一次安排筛选数据快照作业的时间, 格式为 HHMMSS。 *active_start_time_of_day* 是 **int**，默认值为 NULL。  
...
|**dynamic_snapshot_jobid**|**uniqueidentifier**|唯一标识分发[!INCLUDE[msCoName](../../includes/msconame-md.md)]服务器上的[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]代理作业。|  
