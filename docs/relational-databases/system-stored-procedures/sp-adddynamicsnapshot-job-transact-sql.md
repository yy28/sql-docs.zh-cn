...
ms.locfileid: "32993794"
---
# <a name="spadddynamicsnapshotjob-transact-sql"></a>sp_adddynamicsnapshot_job (Transact-SQL)
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-xxx-md](../../includes/tsql-appliesto-ss2008-xxxx-xxxx-xxx-md.md)]

  创建一个代理作业，该代理作业可为具有参数化行筛选器的发布生成筛选数据快照。 在发布服务器的发布数据库上执行此存储的过程。 管理员使用此存储过程可手动为订阅服务器创建筛选数据快照作业。  
...
|**128**|重复执行|  
 ...
 [  **@frequency_interval =** ] *frequency_interval*  
 执行筛选数据快照作业的时间段（以天计）。 *frequency_interval*是**int**，默认值为 1，并且依赖于的值*frequency_type*。  
  
|值*frequency_type*|影响上*frequency_interval*|  
|--------------------------------|-------------------------------------|  
...
|**8**|Hour|  
 ...
 [  **@frequency_subday_interval=**] *frequency_subday_interval*  
 是的数*frequency_subday*作业每次执行之间发生的时间段。 *frequency_subday_interval*是**int**，默认值为 5。  
...
## <a name="remarks"></a>注释  
 **sp_adddynamicsnapshot_job**用于合并复制中使用参数化筛选器的发布。  
