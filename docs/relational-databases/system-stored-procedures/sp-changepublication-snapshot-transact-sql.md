...
`[ @frequency_interval = ] frequency_interval` 指定代理运行的日期。 *frequency_interval* 为 **int**，可以是下列值之一。  
...
`[ @active_start_time_of_day = ] active_start_time_of_day` 第一次计划快照代理的时间，格式为 HHMMSS。 *active_start_time_of_day* 的值为 **int**，默认值为 NULL。  
