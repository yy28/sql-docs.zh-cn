...
## <a name="permissions"></a>权限  
 调用方需要成为 **db_owner** 角色的成员。 根据所请求的操作，不应拒绝调用方 (**db_owner** 在目标全文目录中具有) 的更改或控制权限。  
