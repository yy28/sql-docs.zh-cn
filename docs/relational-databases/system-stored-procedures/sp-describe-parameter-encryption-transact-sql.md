...
-   结果集描述配置数据库列的指定参数的加密密钥[!INCLUDE[tsql](../../includes/tsql-md.md)]语句对应。  
  
-   应加密的结果集描述如何特定参数。 此结果将引用设置中第一个结果集所述的键。  
  
 第一个结果集的每个行所说明一对密钥;加密的列加密密钥和其相应的列主密钥。  
...
|**column_encryption_key_encryption_algorithm_name**|**sysname**|用来生成加密值的列加密密钥的加密算法的名称。|  
