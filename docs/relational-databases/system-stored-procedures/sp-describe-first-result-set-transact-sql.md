...
ms.locfileid: "39536407"
---
# <a name="spdescribefirstresultset-transact-sql"></a>sp_describe_first_result_set (Transact-SQL)
[!INCLUDE[tsql-appliesto-ss2012-all-md](../../includes/tsql-appliesto-ss2012-all-md.md)]

  返回的元数据的第一个可能结果集的[!INCLUDE[tsql](../../includes/tsql-md.md)]批处理。 如果批处理没有返回结果，则返回一个空的结果集。 如果将引发错误[!INCLUDE[ssDE](../../includes/ssde-md.md)]无法确定将通过执行静态分析来执行的第一个查询的元数据。 动态管理视图[sys.dm_exec_describe_first_result_set &#40;TRANSACT-SQL&#41; ](../../relational-databases/system-dynamic-management-views/sys-dm-exec-describe-first-result-set-transact-sql.md)返回相同的信息。  
 ...
 **sp_describe_first_result_set**始终返回状态零成功。 如果该过程引发错误并且作为 RPC 调用该过程，则返回状态由 sys.dm_exec_describe_first_result_set 的 error_type 列中描述的错误类型填充。 如果从 [!INCLUDE[tsql](../../includes/tsql-md.md)] 中调用此过程，则返回值始终为零，甚至在出现错误时也不例外。  
...
|**source_server**|**sysname**|此结果中的此列返回的源服务器的名称（如果结果源自远程服务器）。 在 sys.servers 中所示，给定的名称。 如果列来自本地服务器或无法确定它来自的服务器，则返回 NULL。 仅在请求浏览信息填充。|  
...
-   查询包括创建随后要查询的永久表。  
 ...
 如果所有其他检查都获得成功，则将考虑输入批处理内所有可能的控制流路径。 此会考虑到所有的控制流语句 (GOTO、 IF/ELSE、 WHILE 和[!INCLUDE[tsql](../../includes/tsql-md.md)]TRY/CATCH 块) 以及任何过程，动态[!INCLUDE[tsql](../../includes/tsql-md.md)]批处理或 EXEC 语句，将导致的 DDL 语句从输入批处理中调用的触发器DDL 触发器激发或导致触发器激发，对目标表或由于在外键约束的级联操作修改的表的 DML 语句。 在很多可能的控制路径的情况下，算法将在某个点停止。  
  
 对于每个控制流路径中，第一个语句 （如果有） 的返回结果集由**sp_describe_first_result_set**。  
