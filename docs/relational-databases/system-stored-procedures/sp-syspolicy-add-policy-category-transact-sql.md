...
`[ @name = ] 'name'` 是的策略类别的名称。 *名称*是**sysname**，和是必需的。 *名称*不能为 NULL 或空字符串。  
  
`[ @mandate_database_subscriptions = ] mandate_database_subscriptions` 确定是否为策略类别而托管数据库订阅。 *mandate_database_subscriptions*是**位**值，默认值为 1 （启用）。  
  
`[ @policy_category_id = ] policy_category_id` 为策略类别的标识符。 *policy_category_id*是**int**，并作为 OUTPUT 返回。  
