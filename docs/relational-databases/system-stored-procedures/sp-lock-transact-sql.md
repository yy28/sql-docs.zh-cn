---
title: sp_lock (TRANSACT-SQL) |Microsoft 文档
ms.custom: ''
ms.date: 03/17/2017
ms.prod: sql
...
> [!IMPORTANT]  
>  [!INCLUDE[ssNoteDepFutureAvoid](../../includes/ssnotedepfutureavoid-md.md)] 若要获取有关中的锁定信息[!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)]，使用[sys.dm_tran_locks](../../relational-databases/system-dynamic-management-views/sys-dm-tran-locks-transact-sql.md)动态管理视图。  
...
## <a name="arguments"></a>参数  
 [  **@spid1 =** ] *****会话 ID1*****  
 是[!INCLUDE[ssDE](../../includes/ssde-md.md)]会话 ID 号从**sys.dm_exec_sessions**用户想锁定信息。 *会话 ID1*是**int**默认值为 NULL。 执行**sp_who**以获取有关会话的进程信息。 如果*会话 ID1*未指定，则会显示所有锁的信息。  
  
 [  **@spid2 =** ] *****会话 ID2*****  
 是另一种[!INCLUDE[ssDE](../../includes/ssde-md.md)]会话 ID 号从**sys.dm_exec_sessions** ，可能在同一时间存在一个锁定*会话 ID1*以及哪些用户还希望信息。 *会话 ID2*是**int**默认值为 NULL。  
  
## <a name="return-code-values"></a>返回代码值  
 0（成功）  
  
## <a name="result-sets"></a>结果集  
 **Sp_lock**结果集包含一个行中指定的会话持有的每个锁**@spid1**和**@spid2**参数。 如果既没有**@spid1**也不**@spid2**结果设置报表锁对所有会话的实例中处于活动状态的指定[!INCLUDE[ssDE](../../includes/ssde-md.md)]。  
  
|列名|数据类型|Description|  
|-----------------|---------------|-----------------|  
|**spid**|**int**|请求锁的进程的[!INCLUDE[ssDE](../../includes/ssde-md.md)]会话 ID 号。|  
|**dbid**|**int**|保留锁的数据库的标识号。 可以使用 DB_NAME() 函数来标识数据库。|  
|**ObjId**|**int**|持有锁的对象的标识号。 可以在相关数据库中使用 OBJECT_NAME() 函数来标识对象。 值为 99 时是一种特殊情况，表示用于记录数据库中页分配的其中一个系统页的锁。|  
|**IndId**|**int**|持有锁的索引的标识号。|  
...
|**资源**|**nchar(32)**|标识被锁定资源的值。 值的格式取决于在中标识的资源的类型**类型**列：<br /><br /> **类型**值：**资源**值<br /><br /> RID：格式为 fileid:pagenumber:rid 的标识符，其中 fileid 标识包含页的文件，pagenumber 标识包含行的页，rid 标识页上的特定行。 fileid 匹配**file_id**中的列**sys.database_files**目录视图。<br /><br /> KEY：[!INCLUDE[ssDE](../../includes/ssde-md.md)]内部使用的十六进制数。<br /><br /> PAG：格式为 fileid:pagenumber 的数字，其中 fileid 标识包含页的文件，pagenumber 标识页。<br /><br /> EXT：标识区中的第一页的数字。 该数字的格式为 fileid:pagenumber。<br /><br /> 选项卡： 提供，因为表已在中标识的任何信息**ObjId**列。<br /><br /> DB： 提供，因为该数据库已经确定中的任何信息**dbid**列。<br /><br /> FIL： 与匹配的文件的标识符**file_id**中的列**sys.database_files**目录视图。<br /><br /> APP：被锁定的应用程序资源的唯一标识符。 格式 DbPrincipleId:\<为 16 个字符的资源字符串的前两个 >\<哈希值 >。<br /><br /> MD：随资源类型而变化。 有关详细信息，请参阅说明**resource_description**中的列[sys.dm_tran_locks &#40;TRANSACT-SQL&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-tran-locks-transact-sql.md)。<br /><br /> HBT：没有提供任何信息。 使用**sys.dm_tran_locks**动态管理视图相反。<br /><br /> AU：没有提供任何信息。 使用**sys.dm_tran_locks**动态管理视图相反。|  
