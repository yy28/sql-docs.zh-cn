...
> [!IMPORTANT]  
> [!INCLUDE[ssNoteDepFutureAvoid](../../includes/ssnotedepfutureavoid-md.md)] 若要获取 [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)]中的锁的相关信息，请使用[sys. dm_tran_locks](../../relational-databases/system-dynamic-management-views/sys-dm-tran-locks-transact-sql.md)动态管理视图。  
...
`[ @spid1 = ] 'session ID1'` 是用户想要锁定信息的**dm_exec_sessions** [!INCLUDE[ssDE](../../includes/ssde-md.md)] 的会话 ID 号。 *SESSION ID1*的值为**int** ，默认值为 NULL。 执行**sp_who**以获取有关会话的进程信息。 如果未指定*SESSION ID1* ，则显示有关所有锁的信息。  
  
`[ @spid2 = ] 'session ID2'` 是来自 sys.databases 的另一个 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 的会话 ID 号， **dm_exec_sessions**可能与*会话 ID1*同时具有锁，并且用户也需要其信息。 *SESSION ID2*的值为**int** ，默认值为 NULL。  
  
## <a name="return-code-values"></a>返回代码值  
 0（成功）  
  
## <a name="result-sets"></a>结果集  
 **Sp_lock**结果集在 **\@spid1**和 **\@spid2**参数中指定的会话所持有的每个锁中都包含一行。 如果 **\@spid1**和 **\@spid2**均未指定，则结果集将报告 [!INCLUDE[ssDE](../../includes/ssde-md.md)]实例中当前活动的所有会话的锁。  
