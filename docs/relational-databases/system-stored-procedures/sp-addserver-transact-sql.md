...
`[ @local = ] 'LOCAL'` 指定要添加为本地服务器的服务器。 **\@local**的值为**varchar （10）** ，默认值为 NULL。 将 **\@local**指定为**本地**将 **\@server**定义为本地服务器的名称，并使 @ @SERVERNAME 函数返回*服务器*的值。  
...
`[ @duplicate_ok = ] 'duplicate_OK'` 指定是否允许重复的服务器名称。 **\@duplicate_OK**的值为**varchar （13）** ，默认值为 NULL。 **\@duplicate_OK**的值只能为**duplicate_OK**或 NULL。 如果指定了**duplicate_OK**并且要添加的服务器名称已存在，则不会引发错误。 如果未使用命名参数，则必须指定 **\@local** 。  
