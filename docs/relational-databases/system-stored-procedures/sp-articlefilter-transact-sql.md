...
`[ @filter_name = ] 'filter_name'`要从*filter_name*创建的筛选存储过程的名称。 *filter_name*的值为**nvarchar (386)** , 默认值为 NULL。 您必须为项目筛选指定唯一的名称。  
  
`[ @filter_clause = ] 'filter_clause'`定义水平筛选器的限制 (WHERE) 子句。 当输入限制子句时，将省略关键字 WHERE。 *filter_clause*的值为**ntext**, 默认值为 NULL。  
 ...
 **sp_articlefilter**创建筛选器, 将筛选存储过程的 ID 插入[sysarticles &#40;transact-sql&#41; ](../../relational-databases/system-tables/sysarticles-transact-sql.md)表的**filter**列中, 然后在**筛选器中插入限制子句的文本。_clause**列。  
  
 若要使用水平筛选器创建项目, 请执行不带*filter*参数的[sp_addarticle &#40;transact-sql&#41; ](../../relational-databases/system-stored-procedures/sp-addarticle-transact-sql.md) 。 执行**sp_articlefilter**, 提供包括*filter_clause*在内的所有参数, 然后[执行&#40;sp_articleview transact-sql&#41;](../../relational-databases/system-stored-procedures/sp-articleview-transact-sql.md), 同时提供包括相同*filter_clause*的所有参数。 如果筛选器已经存在, 并且**sysarticles**中的**类型**为**1** (基于日志的项目), 则将删除以前的筛选器, 并创建新的筛选器。  
