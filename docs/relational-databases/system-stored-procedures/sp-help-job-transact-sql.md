---
title: sp_help_job (TRANSACT-SQL) |Microsoft 文档
ms.custom: ''
ms.date: 08/02/2016
ms.prod: sql
...
> [!NOTE]  
>  若要查看特定作业，或者*job_id*或*job_name*必须指定。  同时省略*job_id*和*job_name*返回有关所有作业的信息。
...
|**delete_level**|**int**|**位掩码**，该值指示在什么情况下作业完成时应删除该作业。 可能的值为相同**notify_level_eventlog**。|  
...
|**flag**|**nvarchar(4000)**|**位掩码**的值，用于控制步骤行为。|  
...
|**output_file_name**|**varchar(200)**|输出应写入到哪个命令的文件 ([!INCLUDE[tsql](../../includes/tsql-md.md)]和**CmdExec**仅步骤)。|  
|**last_run_outcome**|**int**|步骤上一次运行的结果：<br /><br /> **0** = 失败<br /><br /> **1** = 成功<br /><br /> **3** = 取消<br /><br /> **5** = 未知|  
...
|**active_start_time**|**int**|在开始作业执行时间**active_start_date。**|  
|**active_end_time**|**int**|时间以结束执行的上的作业**active_end_date**。|  
...
-   **SQLAgentOperatorRole**  
 ...
 成员**SQLAgentUserRole**只能查看他们所拥有的作业。 成员**sysadmin**， **SQLAgentReaderRole**，和**SQLAgentOperatorRole**可以查看所有本地和多服务器作业。  
