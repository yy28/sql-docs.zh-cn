...
`[ @job_id = ] job_id`作业标识号。 *job_id*是唯一**标识符**的，默认值为 NULL。  
  
`[ @job_name = ] 'job_name'`作业的名称。 *job_name*是**系统名称**，默认值为 NULL。  
  
> [!NOTE]  
>  要查看特定作业，必须指定*job_id*或*job_name。*  省略*job_id*和*job_name*以返回有关所有作业的信息。
  
`[ @job_aspect = ] 'job_aspect'`要显示的作业属性。 *job_aspect*是**varchar（9），** 默认值为 NULL，可以是这些值之一。  
...
`[ @job_type = ] 'job_type'`要包含在报表中的作业的类型。 *job_type*是**varchar （12），** 默认值为 NULL。 *job_type*可以是**本地**的或**多服务器**的。  
  
`[ @owner_login_name = ] 'login_name'`作业所有者的登录名。 *login_name*是**系统名称**，默认值为 NULL。  
  
`[ @subsystem = ] 'subsystem'`子系统的名称。 *子系统*为**nvarchar （40），** 默认值为 NULL。  
  
`[ @category_name = ] 'category'`类别的名称。 *类别*是**sysname，** 默认值为 NULL。  
  
`[ @enabled = ] enabled`指示是否显示已启用作业或禁用作业的信息的数字。 *启用*的是**小，** 默认值为 NULL。 **1**表示作业已启用 **，0**表示已禁用的作业。  
  
`[ @execution_status = ] status`作业的执行状态。 *状态*为**int**，默认值为 NULL，可以是这些值之一。  
...
`[ @date_comparator = ] 'date_comparison'`比较运算符用于*比较date_created*和*date_modified。* *date_comparison*是**char（1），** 可以是 *、\<或>。  
  
`[ @date_created = ] date_created`创建作业的日期。 *date_created*是**日期时间**，默认值为 NULL。  
  
`[ @date_last_modified = ] date_modified`上次修改作业的日期。 *date_modified*是**日期时间**，默认值为 NULL。  
  
`[ @description = ] 'description_pattern'`作业的说明。 *description_pattern*是**nvarchar （512），** 默认值为 NULL。 *description_pattern*可以包括用于模式匹配的 SQL Server 通配符。  
  
## <a name="return-code-values"></a>返回代码值  
 **0（** 成功）或**1（** 失败）  
  
## <a name="result-sets"></a>结果集  
 如果未指定参数 **，sp_help_job**返回此结果集。  
...
|**描述**|**nvarchar(512)**|对作业的说明。|  
|**start_step_id**|**int**|执行作业的起始步骤的 ID。|  
|**类别**|**sysname**|作业类别。|  
|**所有者**|**sysname**|作业所有者。|  
|**notify_level_eventlog**|**int**|指示在什么情况下应将通知事件记录到 Microsoft Windows 应用程序日志的**位掩码**。 可以是下列值之一：<br /><br /> **0** = 从不<br /><br /> **1** = 作业成功时<br /><br /> **2** = 作业失败时<br /><br /> **3** = 每当作业完成（无论作业结果如何）|  
|**notify_level_email**|**int**|**Bitmask，** 指示在作业完成时应在什么情况下发送通知电子邮件。 可能的值与**notify_level_eventlog**的值相同。|  
|**notify_level_netsend**|**int**|**Bitmask，** 指示在作业完成时应发送网络消息。 可能的值与**notify_level_eventlog**的值相同。|  
|**notify_level_page**|**int**|**位掩码**，指示在作业完成时应发送页面。 可能的值与**notify_level_eventlog**的值相同。|  
...
|**delete_level**|**int**|**Bitmask，** 指示在作业完成时应删除作业的什么情况。 可能的值与**notify_level_eventlog**的值相同。|  
...
|**current_execution_status**|**int**|当前执行状态：<br /><br /> **1** = 执行<br /><br /> **2** = 等待线程<br /><br /> **3** = 重试之间<br /><br /> **4** = 空闲<br /><br /> **5** = 挂起<br /><br /> **6** = 过时<br /><br /> **7** = 执行完成操作|  
...
|**type**|**int**|作业的类型。<br /><br /> 1 = 本地作业。<br /><br /> **2** = 多服务器作业。<br /><br /> **0** = 作业没有目标服务器。|  
 ...
 如果指定*了job_id*或*job_name，sp_help_job*返回作业步骤、作业计划和作业目标服务器的这些附加结果集。 **sp_help_job**  
...
|**命令**|**恩瓦尔查尔 （3200）**|执行的命令。|  
|**标志**|**nvarchar(4000)**|控制步长行为的值的**位掩码**。|  
|**cmdexec_success_code**|**int**|对于**CmdExec**步骤，这是成功命令的进程退出代码。|  
|**on_success_action**|**nvarchar(4000)**|步骤成功时的操作：<br /><br /> **1** = 成功退出。<br /><br /> **2** = 失败退出。<br /><br /> **3** = 转到下一步。<br /><br /> **4** = 转到步骤。|  
...
|**os_run_priority**|**瓦尔查尔 （4000）**|保留。|  
|**output_file_name**|**瓦尔查尔 （200）**|应写入命令输出的文件（[!INCLUDE[tsql](../../includes/tsql-md.md)]仅和**CmdExec**步骤）。|  
...
|**启用**|**int**|计划是否处于活动状态 （**1**） （**0**）。|  
|**freq_type**|**int**|表示何时执行作业的值：<br /><br /> **1** = 一次<br /><br /> **4** = 每日<br /><br /> **8** = 每周<br /><br /> **16** = 每月<br /><br /> **32** = 每月，相对于**freq_interval**<br /><br /> **64** = **SQLServerAgent**服务启动时运行。|  
|**freq_interval**|**int**|执行作业的天数。 该值取决于**freq_type**的值。 有关详细信息，请参阅[sp_add_schedule&#40;交易-SQL&#41;](../../relational-databases/system-stored-procedures/sp-add-schedule-transact-sql.md)|  
|**freq_subday_type**|**Int**|**freq_subday_interval**的单位。 有关详细信息，请参阅[sp_add_schedule&#40;交易-SQL&#41;](../../relational-databases/system-stored-procedures/sp-add-schedule-transact-sql.md)|  
|**freq_subday_interval**|**int**|作业每次执行之间的**freq_subday_type**期间数。 有关详细信息，请参阅[sp_add_schedule&#40;交易-SQL&#41;](../../relational-databases/system-stored-procedures/sp-add-schedule-transact-sql.md)|  
|**freq_relative_interval**|**int**|每月**freq_interval**的已计划作业的发生。 有关详细信息，请参阅[sp_add_schedule&#40;交易-SQL&#41;](../../relational-databases/system-stored-procedures/sp-add-schedule-transact-sql.md)|  
|**freq_recurrence_factor**|**int**|作业的已计划执行日期之间的间隔月数。|  
|**active_start_date**|**int**|开始执行作业的日期。|  
|**active_end_date**|**int**|结束执行作业的日期。|  
|**active_start_time**|**int**|active_start_date 开始执行作业的时间 **。**|  
|**active_end_time**|**int**|是时候在**active_end_date**上结束作业的执行了。|  
 ...
 **SQLAgentUserRole**的成员只能查看他们拥有的作业。 **系统管理员****、SQLAgentReaderRole**和**SQLAgent运算符Role**的成员可以查看所有本地和多服务器作业。  
...
## <a name="see-also"></a>另请参阅  
 [sp_add_job&#40;转算-SQL&#41;](../../relational-databases/system-stored-procedures/sp-add-job-transact-sql.md)   
 [sp_delete_job&#40;转算-SQL&#41;](../../relational-databases/system-stored-procedures/sp-delete-job-transact-sql.md)   
