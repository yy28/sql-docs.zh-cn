...
@no__t 指定**sp_dropdistpublisher**是否检查发布服务器是否已卸载服务器作为分发服务器。 *no_checks*的值为**bit**，默认值为**0**。  
  
 如果为**0**，则复制将验证远程发布服务器是否已卸载本地服务器作为分发服务器。 如果发布服务器是本地服务器，则复制将验证没有发布对象或分发对象保留在本地服务器上。  
  
 如果为**1**，则即使无法访问远程发布服务器，也将删除与分发发布服务器关联的所有复制对象。 执行此操作后，远程发布服务器必须使用[sp_dropdistributor](../../relational-databases/system-stored-procedures/sp-dropdistributor-transact-sql.md)  = **1** **@no__t**卸载复制。  
  
@no__t 指定在删除发布服务器时是否将分发对象保留在分发服务器上。 *ignore_distributor*为**bit** ，可以是下列值之一：  
