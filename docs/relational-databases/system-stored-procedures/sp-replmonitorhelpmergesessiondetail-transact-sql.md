...
`[ @session_id = ] session_id`指定一个代理会话。 *session_id*的值为**int** , 无默认值。  
 ...
 **sp_replmonitorhelpmergesessiondetail**用于监视合并复制。  
 ...
 在订阅服务器上执行时, **sp_replmonitorhelpmergesessiondetail**仅返回最近5个合并代理会话的详细信息。  
  
## <a name="permissions"></a>权限  
 只有分发服务器上的分发数据库或订阅服务器上的订阅数据库中的**db_owner**或**replmonitor**固定数据库角色的成员才能执行**sp_replmonitorhelpmergesessiondetail**。  
