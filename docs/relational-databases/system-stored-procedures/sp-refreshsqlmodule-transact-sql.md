...
## <a name="arguments"></a>参数  
 ...
 [ **，** @**命名空间**=]  \<类 >   
 是指定模块的类。 当*模块名*是 DDL 触发器，\<类 > 是必需的。 *\<类 >* 是**nvarchar**(20)。 有效输入为：  
...
## <a name="remarks"></a>注释  
 **sp_refreshsqlmodule**基础模块会影响其定义的对象发生更改时应运行。 否则，查询或调用该模块时，可能会产生意外结果。 若要刷新视图，你可以使用**sp_refreshsqlmodule**或**sp_refreshview**与相同的结果。  
