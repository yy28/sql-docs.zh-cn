...
`[ @table_server = ] 'table_server'` 是要为其返回列信息的链接服务器的名称。 *table_server*是**sysname**，无默认值。  
  
`[ @table_name = ] 'table_name'` 是要为其返回列信息的名称。 *table_name*是**sysname**，默认值为 NULL。  
  
`[ @table_schema = ] 'table_schema'` 是要返回其列信息的架构名称。 *table_schema*是**sysname**，默认值为 NULL。  
  
`[ @table_catalog = ] 'table_catalog'` 是要返回其列信息的目录名称。 *table_catalog*是**sysname**，默认值为 NULL。  
  
`[ @column_name = ] 'column'` 是要为其提供信息的数据库列的名称。 *列*是**sysname**，默认值为 NULL。  
