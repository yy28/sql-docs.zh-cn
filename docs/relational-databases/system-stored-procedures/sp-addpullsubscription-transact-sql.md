...
|**排队的故障转移**|支持将订阅作为排队更新订阅，并允许更改为立即更新模式。 在订阅服务器和发布服务器之间建立连接之前，可以在订阅服务器上修改数据，并将数据修改存储在队列中。 建立起持续连接后，即可将更新模式更改为立即更新。 *对 Oracle 发布服务器不支持*。|  
  
 [  **@immediate_sync =**] *immediate_sync*  
 每次运行快照代理时是创建还是重新创建同步文件。 *immediate_sync*是**位**默认值为 1，并且必须设置为相同的值*immediate_sync*中**sp_addpublication**。*immediate_sync*是该发布的属性，必须具有相同的值与它所发布服务器上具有此处。  
