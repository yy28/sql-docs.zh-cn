...
## <a name="arguments"></a>参数  
 [ **@publisher=**] **'***publisher***'**  
 发布服务器的名称。 *发布服务器*是**sysname**，默认值为**%**，表示返回所有发布服务器的信息。  
  
 [ **@publisher_db=**] **'***publisher_db***'**  
 发布服务器数据库的名称。 *publisher_db*是**sysname**，默认值为**%**，表示将返回所有发布服务器数据库的信息。  
  
 [ **@publication=**] **'***publication***'**  
 发布的名称。 *发布*是**sysname**，默认值为**%**，表示将返回所有发布的信息。  
  
 [  **@publication_type=**] *publication_type*  
 是发布的类型。*publication_type*是**int**，默认值为 NULL。 如果提供， *publication_type*必须是以下值之一：  
