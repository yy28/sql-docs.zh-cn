...
`[ @publisher_db = ] 'publisher_db'`发布服务器数据库的名称。 *publisher_db*的值为**sysname**, 默认值 **%** 为, 它返回有关所有发布服务器数据库的信息。  
  
`[ @publication = ] 'publication'`发布的名称。 *发布*为**sysname**, 默认值 **%** 为, 它返回所有发布的信息。  
  
`[ @publication_type = ] publication_type`发布的类型。*publication_type*的值为**int**, 默认值为 NULL。 如果提供, 则*publication_type*必须是以下值之一:  
...
|**publisher_security_mode**|**int**|发布服务器上使用的安全模式：<br /><br /> 0 =  身份[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]验证<br /><br /> **1** = Windows 身份验证|  
|**发行人**|**sysname**|分发服务器的名称。|  
|**distributor_login**|**sysname**|分发服务器登录。|  
|**distributor_password**|**nvarchar(524)**|分发服务器密码（已加密）。|  
|**distributor_security_mode**|**int**|分发服务器上使用的安全模式：<br /><br /> 0 =  身份[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]验证<br /><br /> **1** = Windows 身份验证|  
...
|**段**|**nvarchar(128)**|在 WHERE 子句参数化行筛选器中使用 HOST_NAME() 函数时，指定此函数的值。|  
