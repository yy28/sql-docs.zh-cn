...
|**verify_signature 设置**|**int**|指示全文引擎是否只加载已签名的二进制文件。 默认情况下，仅加载已签名的可信二进制文件。<br /><br /> 1 = 验证是否只加载已签名的可信二进制文件（默认值）。<br /><br /> 0 = 不验证二进制文件是否已签名。|  
