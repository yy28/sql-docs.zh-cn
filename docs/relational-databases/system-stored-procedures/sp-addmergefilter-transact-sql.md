...
`[ @join_articlename = ] 'join_articlename'`必须使用由*join_filterclause*指定的联接子句来联接由*项目*指定的子项目, 以确定子项目中满足合并筛选器条件的行的父项目。 *join_articlename*的值为**sysname**, 无默认值。 项目必须位于*发布*所指定的发布中。  
  
`[ @join_filterclause = ] join_filterclause`必须用于联接由*join_article*指定的*项目*和父项目指定的子项目的 join 子句, 以便确定限定合并筛选器的行。 *join_filterclause*为**nvarchar (1000)** 。  
  
`[ @join_unique_key = ] join_unique_key`指定子项目和父项目之间的联接是一对多、一对一、多对一还是多对多 ( *join_article*) 的联接。 *join_unique_key*的值为**int**, 默认值为0。 **0**指示多对一或多对多联接。 " **1** " 指示一对一或一对多联接。 联接列在*join_article*中形成唯一键时, 或者如果*join_filterclause*在*项目*的外键与*join_article*中的主键之间, 则该值为**1** 。  
  
> [!CAUTION]  
>  仅当对父项目的基础表中的联接列具有保证唯一性的约束时, 才将此参数设置为**1** 。 如果*join_unique_key*设置为**1** , 则可能无法收敛数据。  
  
`[ @force_invalidate_snapshot = ] force_invalidate_snapshot`确认此存储过程所执行的操作是否会使现有快照失效。 *force_invalidate_snapshot*是一个**位**, 默认值为**0**。  
...
`[ @filter_type = ] filter_type`指定要添加的筛选器的类型。 *filter_type*的数据值为**tinyint**, 可以是下列值之一。  
 ...
 **sp_addmergefilter**只能与表项目一起使用。 不支持视图项目和索引视图项目。  
 ...
 此过程也可用于在两个可能或不可能具有联接筛选器的项目之间添加逻辑关系。 *filter_type*用于指定正在添加的合并筛选器是联接筛选器、逻辑关系还是同时为两者。  
