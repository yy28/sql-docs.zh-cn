...
> [!NOTE]  
>  一些其他系统存储过程仅在的[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]实例或客户端 api 中使用，不适合一般客户使用。 [系统存储过程（transact-sql）](https://msdn.microsoft.com/library/ms187961.aspx)中列出了这些过程。 这些过程可能会发生更改，并且不保证兼容性。 列表中的所有过程在中[!INCLUDE[ssSDW](../../includes/sssdw-md.md)]都不可用。  
