...
## <a name="arguments"></a>参数  
 ...
 发送邮件的配置文件的名称。 *Profile_name*属于类型**sysname**，默认值为 NULL。 *Profile_name*必须是现有数据库邮件配置文件的名称。 如果未*profile_name*指定，则**sp_send_dbmail**使用当前用户的默认专用配置文件。 如果用户不具有默认专用配置文件， **sp_send_dbmail**使用的默认公共配置文件**msdb**数据库。 如果用户不具有默认专用配置文件和数据库，没有默认公共配置文件**@profile_name**必须指定。  
  
 [  **@recipients=** ] **'***收件人*****  
 要向其发送邮件的电子邮件地址列表，以分号分隔。 收件人列表的类型是**varchar （max)**。 虽然此参数是可选的但在至少一个**@recipients**， **@copy_recipients**，或者**@blind_copy_recipients** ，则必须指定或**sp_send_dbmail**返回错误。  
  
 [  **@copy_recipients=** ] **'***copy_recipients*****  
 要向其抄送邮件的电子邮件地址列表，以分号分隔。 抄送件收件人列表的类型是**varchar （max)**。 虽然此参数是可选的但在至少一个**@recipients**， **@copy_recipients**，或者**@blind_copy_recipients** ，则必须指定或**sp_send_dbmail**返回错误。  
  
 [ **@blind_copy_recipients=** ] **'***blind_copy_recipients***'**  
 要向其密件抄送邮件的电子邮件地址列表，以分号分隔。 密件副本收件人列表的类型是**varchar （max)**。 虽然此参数是可选的但在至少一个**@recipients**， **@copy_recipients**，或者**@blind_copy_recipients** ，则必须指定或**sp_send_dbmail**返回错误。  
  
 [ **@from_address=** ] **'***from_address***'**  
 电子邮件的“from address”的值。 这是一个可选参数，用于覆盖邮件配置文件中的设置。 此参数的类型是**varchar （max)**。 SMTP 安全设置决定是否接受这些覆盖。 如果未指定参数，则默认值为 NULL。  
  
 [ **@reply_to=** ] **'***reply_to***'**  
 电子邮件的“reply to address”的值。 它只接受一个电子邮件地址作为有效值。 这是一个可选参数，用于覆盖邮件配置文件中的设置。 此参数的类型是**varchar （max)**。 SMTP 安全设置决定是否接受这些覆盖。 如果未指定参数，则默认值为 NULL。  
  
 [ **@subject=** ] **'***subject***'**  
 是电子邮件的主题。 主题属于类型**nvarchar(255)**。 如果未指定主题，则默认为“SQL Server 消息”。  
  
 [ **@body=** ] **'***body***'**  
 是在电子邮件的正文。 消息正文的类型是**nvarchar （max)**，默认值为 NULL。  
  
 [ **@body_format=** ] **'***body_format***'**  
 邮件正文的格式。 参数的类型是**varchar （20)**，默认值为 NULL。 如果已指定，则待发邮件的标头设置会指示邮件正文具有指定格式。 该参数可能包含下列值之一：  
...
-   HTML  
 ...
 [  **@importance=** ] **'***重要性*****  
 邮件的重要性。 参数的类型是**varchar(6)**。 该参数可能包含下列值之一：  
...
-   High  
 ...
 [  **@sensitivity=** ] **'***敏感度*****  
 邮件的敏感度。 参数的类型是**varchar(12)**。 该参数可能包含下列值之一：  
...
-   Confidential  
 ...
 电子邮件附件的文件名列表，以分号分隔。 必须使用绝对路径指定列表中的文件。 附件列表的类型是**nvarchar （max)**。 默认情况下，数据库邮件会将文件附件限制在每个文件 1 MB 大小。  
  
 [  **@query=** ] **'***查询*****  
 要执行的查询。 查询结果可以作为文件附加，或包含在电子邮件的正文中。 查询属于类型**nvarchar （max)**，并且可以包含任何有效[!INCLUDE[tsql](../../includes/tsql-md.md)]语句。 请注意在单独的会话，因此本地变量在脚本调用中执行查询**sp_send_dbmail**对查询不可用。  
  
 [ **@execute_query_database=** ] **'***execute_query_database***'**  
 存储过程在其中运行查询的数据库上下文。 参数的类型是**sysname**，默认值为当前数据库。 此参数才适用如果**@query**指定。  
 ...
 [ **@query_attachment_filename=** ] *query_attachment_filename*  
 指定查询结果集附件使用的文件名。 *query_attachment_filename*属于类型**nvarchar(255)**，默认值为 NULL。 忽略此参数时*attach_query_result*为 0。 当*attach_query_result*为 1 且此参数为 NULL，数据库邮件会创建任意文件名。  
  
 [ **@query_result_header=** ] *query_result_header*  
 指定查询结果是否包含列标题。 Query_result_header 值属于类型**位**。 如果该值为 1，则查询结果包含列标题。 如果该值为 0，则查询结果不包含列标题。 此参数默认为**1**。 此参数才适用如果**@query**指定。  
 
   >[!NOTE]
   > 设置时，可能出现以下错误@query_result_header为 0，并且设置@query_no_truncate为 1:
   > <br> Msg 22050，级别 16，状态 1，行 12： 无法初始化 sqlcmd 库，错误号-2147024809。
  
 [ **@query_result_width** = ] *query_result_width*  
 用于设置查询结果的格式的线条宽度（字符）。 *Query_result_width*属于类型**int**，默认值为 256。 提供的值必须介于 10 和 32767 之间。 此参数才适用如果**@query**指定。  
  
 [ **@query_result_separator=** ] **'***query_result_separator***'**  
 用于分隔查询输出中的列的字符。 该分隔符的类型是**char （1)**。 默认为“ ”（空格）。  
  
 [ **@exclude_query_output=** ] *exclude_query_output*  
 指定是否使用电子邮件返回查询执行的输出。 **exclude_query_output**为 bit，默认值为 0。 当此参数为 0 时，执行**sp_send_dbmail**存储的过程打印作为查询执行的结果的控制台上返回的消息。 当此参数为 1，在执行**sp_send_dbmail**存储的过程不会在查询执行消息中的任何打印控制台。  
  
 [ **@append_query_error=** ] *append_query_error*  
 指定是否在指定的查询返回的错误时发送电子邮件**@query**参数。 **append_query_error**是**位**，默认值为 0。 如果此参数的值为 1，则数据库邮件会发送电子邮件，并在电子邮件的正文中包括查询错误消息。 如果此参数为 0，数据库邮件不发送电子邮件消息，并**sp_send_dbmail**结束时会返回代码 1，指示失败。  
  
 [ **@query_no_truncate=** ] *query_no_truncate*  
 指定是否执行在查询中使用的选项可避免截断大型可变长度数据类型 (**varchar （max)**， **nvarchar （max)**， **varbinary （max)****xml**，**文本**， **ntext**，**映像**，和用户定义数据类型)。 设置该选项后，查询结果将不包含列标题。 *Query_no_truncate*的值为类型**位**。 当该值为 0 或未指定时，查询中的列将截断为 256 个字符。 当该值为 1 时，不截断查询中的列。 此参数的默认值为 0。  
  
> [!NOTE]  
>  与大量的数据，一起使用时 @**query_no_truncate**选项会消耗其他资源，并可降低服务器性能。  
  
 [ **@query_result_no_padding** ] *@query_result_no_padding*  
 类型为 bit。 默认值为 0。 如果设置为 1 时，查询结果将不填充，这可能会减少文件大小。如果您设置@query_result_no_padding设置为 1 且@query_result_width参数，@query_result_no_padding参数将覆盖@query_result_width参数。  
  
 在这种情况下，不发生错误。  
 
  >[!NOTE]
  > 设置时，可能出现以下错误@query_result_no_padding与 1 和提供的参数 @query_no_truncate:
  > <br> Msg 22050，级别 16，状态 1，行 0： 未能执行查询，因为@query_result_no_append和@query_no_truncate选项互斥。 
