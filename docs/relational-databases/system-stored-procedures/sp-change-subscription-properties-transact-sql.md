...
`[ @publication_type = ] publication_type`指定发布的复制类型。 *publication_type*的数据值为**int**, 可以是下列值之一。  
...
|**dts_package_password**||指定包上的密码。 *dts_package_password*的类型为**sysname** , 默认值为 NULL, 指定密码属性保持不变。<br /><br /> 注意:DTS 包必须具有密码。<br /><br /> 仅当发布为事务发布或快照发布时才能指定该值。|  
...
||**0**|使用基本身份验证进行 Web 同步。<br /><br /> 注意:Web 同步要求与 Web 服务器建立 SSL 连接。|  
 ...
 **sp_change_subscription_properties**用于请求订阅。  
 ...
 对于 Oracle 发布服务器, *publisher_db*的值将被忽略, 因为 Oracle 每个服务器实例只允许有一个数据库。  
