---
title: sp_change_subscription_properties（转算-SQL） |微软文档
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
`[ @publisher = ] 'publisher'`是发布者的名称。 *发布者*是**sysname，** 没有默认值。  
...
`[ @value = ] 'value'`是属性的新值。 *值*为**nvarchar（1000），** 没有默认值。  
  
`[ @publication_type = ] publication_type`指定发布的复制类型。 *publication_type*是**int，** 可以是这些值之一。  
...
|**dts_package_password**||指定包上的密码。 *dts_package_password*为**系统名称**，默认值为 NULL，它指定密码属性保持不变。<br /><br /> 注意：DTS 包必须具有密码。<br /><br /> 仅当发布为事务发布或快照发布时才能指定该值。|  
...
||**0**|使用基本身份验证进行 Web 同步。<br /><br /> 注意：Web 同步需要与 Web 服务器的 TLS 连接。|  
...
|**publisher_login**||发布者登录。 仅支持更改*publisher_login*以合并发布。|  
|**publisher_password**||发布者密码。 仅支持更改*publisher_password*以合并发布。|  
|**publisher_security_mode**|**1**|连接发布服务器时，使用 Windows 身份验证。 仅支持更改*publisher_security_mode*以合并发布。|  
 ...
 **sp_change_subscription_properties**用于拉取订阅。  
  
 对于 Oracle 发布商，忽略*publisher_db*的值，因为 Oracle 仅允许每个服务器实例一个数据库。  
  
## <a name="permissions"></a>权限  
 只有**sysadmin**固定服务器角色的成员或**db_owner**固定数据库角色才能**执行sp_change_subscription_properties**。  
  
## <a name="see-also"></a>另请参阅  
 [查看和修改拉取订阅属性](../../relational-databases/replication/view-and-modify-pull-subscription-properties.md)   
 ...
 [sp_addmergepullsubscription_agent&#40;转瞬sp_addmergepullsubscription_agent-SQL&#41;](../../relational-databases/system-stored-procedures/sp-addmergepullsubscription-agent-transact-sql.md)   
 [sp_addpullsubscription&#40;转算-SQL&#41;](../../relational-databases/system-stored-procedures/sp-addpullsubscription-transact-sql.md)   
 [sp_addpullsubscription_agent&#40;转算-SQL&#41;](../../relational-databases/system-stored-procedures/sp-addpullsubscription-agent-transact-sql.md)   
