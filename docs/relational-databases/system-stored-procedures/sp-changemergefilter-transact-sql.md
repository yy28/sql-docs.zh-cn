...
@no__t 为发布的名称。 *发布*为**sysname**，无默认值。  
  
@no__t 是项目的名称。 *项目*是**sysname**，无默认值。  
  
@no__t 是筛选器的当前名称。 *filtername*的值为**sysname**，无默认值。  
  
`[ @property = ] 'property'` 是要更改的属性的名称。 *属性*为**sysname**，无默认值。  
  
@no__t 为指定的属性的新值。 *值*为**nvarchar （1000）** ，无默认值。  
...
@no__t，则确认此存储过程所执行的操作可能会使现有快照失效。 *force_invalidate_snapshot*是一个**位**，默认值为**0**。  
...
@no__t，则确认此存储过程所执行的操作可能需要重新初始化现有订阅。 *force_reinit_subscription*为**位**，默认值为**0**。  
