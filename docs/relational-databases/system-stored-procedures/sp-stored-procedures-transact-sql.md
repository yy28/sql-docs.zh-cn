...
|**PROCEDURE_TYPE**|**int**|过程类型。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 始终返回 2.0。 此值可以为下列值之一：<br /><br /> 0 = SQL_PT_UNKNOWN<br /><br /> 1 = SQL_PT_PROCEDURE<br /><br /> 2 = SQL_PT_FUNCTION|  
  
## <a name="remarks"></a>注释  
 为了获得最大互操作性，网关客户端应只采用 SQL 标准模式匹配（百分号 (%) 和下划线 (_) 通配符）。  
  
 由于不就当前用户对特定存储过程执行访问的权限信息进行必要的检查，因此访问得不到保证。 请注意只使用三部分命名。 这表示在对 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 执行时，只返回本地存储过程而不返回要求四部分命名的远程存储过程。 服务器属性 ACCESSIBLE_SPROC 如果结果集中为 Y **sp_server_info**，仅可以由当前用户执行的存储的过程返回。  
