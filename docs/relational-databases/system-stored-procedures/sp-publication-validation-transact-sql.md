...
## <a name="arguments"></a>参数  
 ...
 指示是否只返回表的行计数。 *rowcount_only*为**smallint** , 可以为以下值之一。  
...
|**2**|执行行计数和二进制校验和。<br /><br /> 注意:对于[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]版本7.0 订阅服务器, 只执行行计数验证。|  
 ...
 [ **@full_or_fast=** ] *full_or_fast*  
 用于计算行计数的方法。 *full_or_fast*的数据值为**tinyint** , 可以是下列值之一。  
...
`[ @shutdown_agent = ] shutdown_agent`指示分发代理是否应在完成验证后立即关闭。 *shutdown_agent*的值为**bit**, 默认值为**0**。 如果为**0**, 则复制代理不会关闭。 如果为**1**, 则复制代理会在最后一篇文章验证后关闭。  
 ...
 **sp_publication_validation**用于事务复制。  
 ...
 如果你的应用程序具有即时更新订阅服务器, 则**sp_publication_validation**可能会检测到虚假错误。 **sp_publication_validation**首先计算发布服务器上的行计数或校验和, 然后在订阅服务器上计算。 由于在发布服务器中完成行计数或校验和之后，但在订阅服务器中完成行计数或校验和之前，即时更新触发器可以将更新从订阅服务器传播到发布服务器，因此值可能会更改。 若要确保在验证发布时，在订阅服务器和发布服务器中的值不发生更改，请在验证期间在发布服务器中停止 Microsoft 分布式事务处理协调器 (MS DTC) 服务。  
