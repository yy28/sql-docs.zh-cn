---
title: sp_help_jobserver (Transact SQL) |Microsoft 文档
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
>  任一*job_id*或*job_name*必须指定，但不能同时指定。  
 ...
 [ **@show_last_run_details=** ] *show_last_run_details*  
 指示结果集是否包含上次运行的执行信息。 *show_last_run_details*是**tinyint**，默认值为**0**。 **0**不包含上一次运行的信息，和**1**未。  
...
|**last_poll_date**|**datetime**|目标服务器上一次轮询主服务器的日期。|  
 ...
 如果**sp_help_jobserver**执行时使用*show_last_run_details*设置为**1**，结果集有这些额外的列。  
...
|**last_run_outcome**|**int**|作业上一次在此服务器上运行的结果：<br /><br /> **0** = 失败<br /><br /> **1** = 成功<br /><br /> **3** = 取消<br /><br /> **5** = 未知|  
