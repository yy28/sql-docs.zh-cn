...
## <a name="arguments"></a>参数  
 ...
 [ @loginame **=** ] *****登录*****  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 本地实例上的用户的登录 ID。 login 的数据类型为 sysname，默认值为 NULL。 *登录名*必须已经存在的本地实例上[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。 如果*登录*指定，则上的所有用户*远程服务器*映射到该特定的本地登录名。 当上的用户*远程服务器*连接到的本地实例[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]若要执行远程存储的过程，它们连接作为*登录*。  
