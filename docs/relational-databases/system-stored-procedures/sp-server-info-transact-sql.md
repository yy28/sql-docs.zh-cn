...
|**22**|ACCESSIBLE_TABLES<br /><br /> 指定是否在**sp_tables**，网关返回唯一表、 视图和等等，可由当前用户 （即，具有至少选择为表的权限的用户） 访问。|是|  
...
|**104**|ACCESSIBLE_SPROC<br /><br /> 指定是否在**sp_stored_procedures**，网关返回仅由当前用户可执行存储的过程。|是|  
