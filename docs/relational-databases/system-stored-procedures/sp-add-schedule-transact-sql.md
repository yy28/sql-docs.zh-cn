...
`[ @schedule_name = ] 'schedule_name'`计划的名称。 *可提供*的值为**sysname**，无默认值。  
...
`[ @freq_interval = ] freq_interval`作业的执行日期。 *freq_interval*的值为**int**，默认值为**1**，具体取决于*freq_type*的值。  
...
|**16** （每月）|在每月的*freq_interval*日期。|  
|**32** （每月相对）|*freq_interval*是下列其中一项：<br /><br /> **1** = 星期日<br /><br /> **2** = 星期一<br /><br /> **3** = 星期二<br /><br /> **4** = 星期三<br /><br /> **5** = 星期四<br /><br /> **6** = 星期五<br /><br /> **7** = 星期六<br /><br /> **8** = 日<br /><br /> **9** = 工作日<br /><br /> **10** = 周末|  
...
`[ @freq_subday_interval = ] freq_subday_interval`每次执行作业之间出现的*freq_subday_type*时间段数。 *freq_subday_interval*的值为**int**，默认值为**0**。 注意:间隔应超过10秒。 在*freq_subday_type*等于**1**的情况下，将忽略*freq_subday_interval* 。  
  
`[ @freq_relative_interval = ] freq_relative_interval`如果*freq_interval*为32（每月相对），则为每个月中的*freq_interval* 。 *freq_relative_interval*的数据值为**int**，默认值为**0**，可以是下列值之一。 如果*freq_type*不等于32，则会忽略*freq_relative_interval* 。  
...
`[ @freq_recurrence_factor = ] freq_recurrence_factor`作业的计划执行之间的周数或月数。 仅当*freq_type*为**8**、 **16**或**32**时，才使用*freq_recurrence_factor* 。 *freq_recurrence_factor*的值为**int**，默认值为**0**。  
  
`[ @active_start_date = ] active_start_date`可以开始执行作业的日期。 *active_start_date*的数据值为**int**，默认值为 NULL，表示今天的日期。 日期的格式为 YYYYMMDD。 如果*active_start_date*不为 NULL，则日期必须大于或等于19900101。  
