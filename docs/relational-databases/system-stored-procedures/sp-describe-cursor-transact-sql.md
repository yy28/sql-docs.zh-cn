...
## <a name="arguments"></a>参数  
 ...
 声明的用于接收游标输出的游标变量的名称。 *output_cursor_variable*是**光标**，无默认值，并且不能与任何游标相关联的调用 sp_describe_cursor 时。 返回的游标是可滚动的动态只读游标。  
 ...
 *global_cursor_name*也可以打开由 ODBC 应用程序然后名为 API 服务器游标的名称通过调用 SQLSetCursorName。  
...
## <a name="cursors-returned"></a>返回的游标  
 sp_describe_cursor 其结果集封装在[!INCLUDE[tsql](../../includes/tsql-md.md)]**光标**输出参数。 这样，[!INCLUDE[tsql](../../includes/tsql-md.md)] 批处理、存储过程和触发器即可逐行处理输出。 这也意味着无法直接从数据库 API 函数调用该过程。 **游标**输出参数必须绑定到程序变量，但是数据库 Api 不支持绑定**光标**参数或变量。  
...
|并发|**tinyint**|1 = 只读<br /><br /> 2 = 滚动锁<br /><br /> 3 = 乐观|  
|scrollable|**tinyint**|0 = 只进<br /><br /> 1 = 可滚动|  
|open_status|**tinyint**|0 = 关闭的<br /><br /> 1 = 打开的|  
|cursor_rows|**decimal(10,0)**|在结果集中行的符合条件的数目。 有关详细信息，请参阅 [@@CURSOR_ROWS (Transact-SQL)](../../t-sql/functions/cursor-rows-transact-sql.md)。|  
|fetch_status|**smallint**|对此游标上次提取的状态。 有关详细信息，请参阅 [@@FETCH_STATUS (Transact-SQL)](../../t-sql/functions/fetch-status-transact-sql.md)。<br /><br /> 0 = 提取成功。<br /><br /> -1 = 提取失败或超出游标的界限。<br /><br /> -2 = 缺少所请求的行。<br /><br /> -9 = 尚未对游标进行提取。|  
