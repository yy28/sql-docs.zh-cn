...
|MONEY|**int**|指定**资金**数据类型。<br /><br /> 1 =**资金**数据类型。<br /><br /> 0 = 未**资金**数据类型。|  
|AUTO_INCREMENT|**int**|指定自动递增。<br /><br /> 1 = 自动递增。<br /><br /> 0 = 不自动递增。<br /><br /> NULL = 属性不适用。<br /><br /> 应用程序可以将值插入具有此属性的列中，但应用程序不能更新列中的值。 除**位**数据类型，AUTO_INCREMENT 是仅对属于精确数字和近似数字数据类型类别的数据类型有效。|  
...
|SQL_DATA_TYPE|**int**|SQL 数据类型在描述符的 TYPE 字段中显示的值。 此列是与 DATA_TYPE 列相同除外**datetime**和 ANSI**间隔**数据类型。 此字段始终返回值。|  
|SQL_DATETIME_SUB|**int**|**日期时间**或 ANSI**间隔**如果 SQL_DATA_TYPE 的值为 SQL_DATETIME 或 SQL_INTERVAL 子代码。 对于数据类型以外**datetime**和 ANSI**间隔**，此字段为 NULL。|  
...
|INTERVAL_PRECISION|**int**|间隔起始精度，如果的值*data_type*是**间隔**; 否则为 NULL。|  
|USERTYPE|**int**|**usertype**来自 systypes 表的值。|  
