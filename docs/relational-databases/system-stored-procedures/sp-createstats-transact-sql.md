...
ms.locfileid: "33239847"
---
# <a name="spcreatestats-transact-sql"></a>sp_createstats (Transact-SQL)
[!INCLUDE[tsql-appliesto-ss2008-asdb-xxxx-xxx-md](../../includes/tsql-appliesto-ss2008-asdb-xxxx-xxx-md.md)]

  调用[CREATE STATISTICS](../../t-sql/statements/create-statistics-transact-sql.md)语句上尚不统计信息对象中的第一列的列创建单列统计信息。 创建单列统计信息会增加直方图的数目，这可能会改进基数估计、查询计划和查询性能。 统计信息对象的第一列具有直方图；其他列没有直方图。  
...
## <a name="remarks"></a>注释  
 sp_createstats 不会创建或更新现有的统计信息对象; 中的第一列的列的统计信息 这包括创建索引、 具有使用 AUTO_CREATE_STATISTICS 选项时，生成的单列统计信息的列和使用 CREATE STATISTICS 语句创建的统计信息的第一列的统计信息的第一列。 sp_createstats 不在已禁用的索引的第一列上创建统计信息，除非在另一个已启用索引中使用该列。 sp_createstats 不在具有禁用的聚集索引的表上创建统计信息。  
