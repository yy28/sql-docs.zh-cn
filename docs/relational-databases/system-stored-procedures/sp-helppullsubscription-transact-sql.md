...
`[ @publisher_db = ] 'publisher_db'`发布服务器数据库的名称。 *publisher_db*的值为**sysname**, 默认值 **%** 为, 表示将返回所有发布服务器数据库。  
  
`[ @publication = ] 'publication'`发布的名称。 *发布*为**sysname**, 默认值 **%** 为, 表示将返回所有发布。 如果此参数等于 ALL, 则仅返回 independent_agent = **0**的请求订阅。  
  
`[ @show_push = ] 'show_push'`指示是否要返回所有推送订阅。 *show_push*的值为**nvarchar (5)** , 默认值为 FALSE, 表示不返回推送订阅。  
...
|**发布服务器登录**|**sysname**|在发布服务器上用于 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 身份验证的登录 ID。|  
|**发布者密码**|**nvarchar(524)**|在发布服务器上用于 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 身份验证的密码（加密）。|  
|**发布者 security_mode**|**int**|在发布服务器上实现的安全模式：<br /><br /> 0 =  身份[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]验证<br /><br /> **1** = Windows 身份验证<br /><br /> **2** = 同步触发器使用静态**sysservers**项执行远程过程调用 (RPC), 并且*发布*服务器必须在**sysservers**表中定义为远程服务器或链接服务器。|  
...
|**distributor_security_mode**|**int**|在分发服务器上实施的安全模式：<br /><br /> 0 =  身份[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]验证<br /><br /> **1** = Windows 身份验证|  
