...
`[ @dbname = ] 'dbname'` 是要为其设置复制数据库选项的数据库。 *db_name* **sysname**，无默认值。  
  
`[ @optname = ] 'optname'` 是要启用或禁用的复制数据库选项。 *optname*为**sysname**，可以是下列值之一。  
...
`[ @value = ] 'value'` 是启用还是禁用给定的复制数据库选项。 *值*为**sysname**，可以为**true**或**false**。 如果此值为**false** ，并且*optname*为**merge publish**，则还将删除对合并发布数据库的订阅。  
