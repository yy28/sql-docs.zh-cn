...
+ 使用 RevoScaleR 中的 `rxGlm` 或 `rxNaiveBayes` 算法的模型
+ R 中的 PMML 模型
+ 使用其他第三方库创建的模型 
...
除了是有效的 SQL 查询外， *\@inputData*中的输入数据也必须包含与存储模型中的列兼容的列。
