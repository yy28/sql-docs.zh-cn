...
`[ @dynamic_snapshot_location = ] 'dynamic_snapshot_location' ]`如果要使用筛选数据快照，则从中读取快照文件的文件夹的路径。 *dynamic_snapshot_location*的默认值为**nvarchar （260）**，默认值为 NULL。 有关详细信息，请参阅 [参数化行筛选器](../../relational-databases/replication/merge/parameterized-filters-parameterized-row-filters.md)。  
...
`[ @hostname = ] 'hostname'`当在参数化筛选器的 WHERE 子句中使用此函数时，将重写 HOST_NAME （）的值。 *hostname*的值为**sysname**，默认值为 NULL。  
