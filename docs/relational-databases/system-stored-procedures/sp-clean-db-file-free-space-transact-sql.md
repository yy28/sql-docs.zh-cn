...
## <a name="arguments"></a>参数  
 ...
 [ @fileid=] '*file_number*  
 要清理的数据文件 ID。 *file_number*是**int**且不能为 NULL。  
...
## <a name="remarks"></a>备注  
 对表的删除操作或更新操作会引发行的移动，通过删除对行的引用可立即释放页上的空间。 但是，在某些情况下，该行仍然作为虚影记录而保留在数据页上。 后台进程定期清除虚影记录。 不返回该残留数据[!INCLUDE[ssDE](../../includes/ssde-md.md)]响应查询。 但是，在数据或备份文件处于其物理安全性面临风险的环境中时，可以使用 sp_clean_db_file_free_space 来清除虚影记录。  
 ...
 相关[sp_clean_db_free_space](../../relational-databases/system-stored-procedures/sp-clean-db-free-space-transact-sql.md)存储的过程将清除数据库中的所有文件。  
