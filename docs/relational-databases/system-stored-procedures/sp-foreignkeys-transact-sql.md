...
`[ @pktab_name = ] 'pktab_name'` 是具有主键的名称。 *pktab_name*是**sysname**，默认值为 NULL。  
  
`[ @pktab_schema = ] 'pktab_schema'` 是包含主键的名称。 *pktab_schema*是**sysname**，默认值为 NULL。 在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 中，该名称包含所有者名称。  
  
`[ @pktab_catalog = ] 'pktab_catalog'` 是包含主键的名称。 *pktab_catalog*是**sysname**，默认值为 NULL。 在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 中，该名称包含数据库名称。  
  
`[ @fktab_name = ] 'fktab_name'` 是具有外键的名称。 *fktab_name*是**sysname**，默认值为 NULL。  
  
`[ @fktab_schema = ] 'fktab_schema'` 是包含外键的名称。 *fktab_schema*是**sysname**，默认值为 NULL。  
