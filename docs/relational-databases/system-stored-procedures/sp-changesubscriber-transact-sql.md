...
`[ @type = ] type`订阅服务器类型。 *类型*为**tinyint**, 默认值为 NULL。 **0** [!INCLUDE[msCoName](../../includes/msconame-md.md)] 表示[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]订阅服务器。 **1**指定非[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]或其他 ODBC 数据源服务器订阅服务器。  
...
`[ @frequency_type = ] frequency_type`用于计划分发任务的频率。 *frequency_type*的数据值为**int**, 可以是下列值之一。  
...
`[ @security_mode = ] security_mode`实现的安全模式。 *security_mode*的数据值为**int**, 可以是下列值之一。  
...
## <a name="permissions"></a>权限  
 只有**sysadmin**固定服务器角色的成员才能执行**sp_changesubscriber**。  
