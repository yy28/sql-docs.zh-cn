...
|**类型**|**int**|项目类型，可以为以下值之一：<br /><br /> **10** = 表<br /><br /> **32** = 存储的过程<br /><br /> **64** = 视图或索引视图<br /><br /> **128** = 用户定义函数<br /><br /> **160** = 仅限同义词架构|  
...
|**threshold**|**int**|用于运行的订阅服务器的百分比值[!INCLUDE[ssEW](../../includes/ssew-md.md)]或以前版本的[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。 **阈值**控制何时合并代理指派的新标识范围。 如果使用了在阈值中指定的百分比值，合并代理将创建新的标识范围。 有关详细信息，请参阅的"合并复制"部分[复制标识列](../../relational-databases/replication/publish/replicate-identity-columns.md)。|  
...
|**compensate_for_errors**|**bit**|指示是否在同步; 期间遇到错误时采取补偿操作其中**1**指示，都将使用补偿操作，和**0**意味着不会采取补偿操作。|  
