...
`[ \@table_qualifier = ] qualifier` 对象限定符的名称。 *限定符* 的值为 **sysname**，默认值为 NULL。 各种 DBMS 产品支持三部分命名 (限定符的对象_qualifier_**。**_所有者_**。**_名称_) 。 在中 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ，此列表示数据库名称。 在某些产品中，它表示对象数据库环境的服务器名称。  
