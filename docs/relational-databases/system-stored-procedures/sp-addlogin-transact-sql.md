...
## <a name="remarks"></a>备注  
 ...
 如果**skip_encryption**在将登录名添加到[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]时已对密码进行哈希处理，则使用 skip_encryption 取消密码哈希会很有用。 如果密码是由早期版本的[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]进行哈希处理的，请使用**skip_encryption_old**。  
