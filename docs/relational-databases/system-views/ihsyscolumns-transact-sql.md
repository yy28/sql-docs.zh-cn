...
|列名称|数据类型|说明|  
|-----------------|---------------|-----------------|  
|**路径名**|**sysname**|列名或过程参数的名称。|  
|**识别**|**int**|此列所属的表的对象 ID，或与此参数关联的存储过程的 ID。|  
...
|**长短**|**bigint**|[Systypes &#40;transact-sql&#41;](../../relational-databases/system-compatibility-views/sys-systypes-transact-sql.md)中的最大物理存储长度。|  
...
|**语言**|**int**|列的语言标识符。|  
|**状态值**|**int**|用于描述列或参数的属性的位图：<br /><br /> **0x08** = 列允许 null 值。<br /><br /> **0x10** = 在添加**varchar**或**varbinary**列后，ANSI 填充已生效。 为**varchar**保留尾随空格，并保留**varbinary**列的尾随零。<br /><br /> **0x40** = 参数是输出参数。<br /><br /> **0x80** = 列是标识列。|  
