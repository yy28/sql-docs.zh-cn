...
ms.openlocfilehash: 64fb6dabb40cb5ff4048a37895df935b158dc8f3
ms.lasthandoff: 04/11/2017

---
# <a name="lesson-1-prepare-the-data-data-science-end-to-end-walkthrough"></a>第 1 课：准备数据（数据科学端到端演练）
如果你已全新安装包含 R Services 的 SQL Server 2016 或 vNext，便可以开始演练。 在本课程中，你将获取方案所需的数据和 R 包。 

1. 从 Github 下载数据和所有 R 脚本。 为方便操作，Github 上提供了一个 PowerShell 脚本。  
...
#### <a name="download-scripts-using-powershell"></a>使用 PowerShell 下载脚本  
...
3.  运行以下 Powershell 命令，将脚本文件下载到本地目录。 如果不指定另一个目录，则默认情况下，将创建文件夹 C:\tempR 并将所有文件保存其中。  
...
4.  下载可能需要花费一段时间。 下载完成后，Windows PowerShell 命令控制台应如下所示：  
    ...
    ![完成 PowerShell 脚本后](../../advanced-analytics/r-services/media/rsql-e2e-psscriptresults.PNG "完成 PowerShell 脚本后")  
  
5.  在 PowerShell 控制台中，可以运行命令 `ls` 查看下载到 DestDir 的文件列表。  有关文件的说明，请参阅[包含的内容](#What-the-Download-Includes)。
...
1.  在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 计算机上，**以管理员身份**打开 RGui.exe。  如果已使用默认值安装 SQL Server R Services，则可以在 C:\Program Files\Microsoft SQL Server\MSSQL13.MSSQLSERVER\R_SERVICES\bin\x64) 中找到 RGui.exe。  
    ...
    如果出现提示，请不要安装到用户库，否则 SQL Server 实例在 R Services 使用的 R 库中找不到包。 有关详细信息，请参阅 [在 SQL Server 上安装新的 R 包](../../advanced-analytics/r-services/install-additional-r-packages-on-sql-server.md)。
...
## <a name="3-prepare-the-environment-using-powershell-script-runsqlrwalkthroughps1"></a>3.使用 PowerShell 脚本 RunSQL_R_Walkthrough.ps1 准备环境  

连同 R 和 T-SQL 脚本一起下载数据文件时，PowerShell 脚本 `RunSQL_R_Walkthrough.ps1` 应会包含在下载内容中。
...
应该在用于生成解决方案的计算机（例如，用于开发和测试 R 代码的笔记本电脑）上运行此脚本。 此计算机（称为数据科学客户端）必须能够使用命名管道协议连接到 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 计算机。  
...
3.  系统将提示以下每个参数：  
    ...
    **数据库服务器名称**：安装 R Services 的 SQL Server 实例的名称。 
    ...
    **用户名**：提供拥有数据库访问特权的帐户。 存在两个选项：
    ...
    + 如果未能指定有效的用户，则该脚本默认会使用集成的 Windows 身份验证。  
      ...
      > [!WARNING] 
      > 使用 PowerShell 脚本中的提示提供凭据时，密码将以纯文本格式写入更新的脚本文件中。 创建所需的 R 对象后，请立即编辑该文件以删除凭据。
...
4.  按 Enter 运行该脚本。  
    ...
    如果批量导入或其他任何步骤失败，请根据[故障排除](#bkmk_Troubleshooting)部分中所述手动加载数据。    
...
如果脚本未加载数据，请检查语法，并确认是否为网络正确指定了服务器名称。 例如，若要连接到命名实例，请务必包含所有子网和计算机名称。
...
如果数据库包含名称相同且架构相同的表，bcp 将插入数据的新副本，而不是覆盖现有数据。 若要避免重复数据，请截断所有现有的表，然后重新运行脚本。
...
该数据是纽约市出租车数据集的一个代表样本，其包含 2013 年逾 1.73 亿次单个行程的记录，包括每次行程支付的车费和小费。  若要详细了解最初如何收集此数据以及如何获取完整数据集，请参阅  
...
此 PowerShell 脚本在 SQL Server 实例上执行多个 [!INCLUDE[tsql](../../includes/tsql-md.md)] 脚本。 下表列出了 [!INCLUDE[tsql](../../includes/tsql-md.md)] 脚本文件。  
...
本演练中使用的 SQL 查询都已经过测试并可在 R 代码中_按原样_运行。 但是，如果你想要进一步试验或开发自己的解决方案，我们建议先使用专用的 SQL 开发环境来测试并优化查询，然后将其添加到 R 代码。  
