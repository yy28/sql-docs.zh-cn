---
title: "步骤 2：使用 PowerShell 将数据导入 SQL Server（数据库内高级分析教程） | Microsoft Docs"
ms.custom: ""
ms.date: "08/05/2016"
...
---
# 步骤 2：使用 PowerShell 将数据导入 SQL Server（数据库内高级分析教程）
...
1.  以管理员身份打开 PowerShell 命令提示符并运行以下命令。  
    ...
    -   安装有 [!INCLUDE[rsql_productname](../../includes/rsql-productname-md.md)] 的 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 实例的名称或地址  
...
3.  使用 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] 和指定的登录名登录到 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例，验证是否可以看到创建的数据库、表、函数和存储过程。  
