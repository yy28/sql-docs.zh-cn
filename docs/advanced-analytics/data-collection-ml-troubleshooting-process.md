...
根据使用的 SQL Server 版本，某些机器学习功能可能不可用或受到限制。 以下文章列出了 Enterprise、Developer、Standard 和Express 版中的机器学习功能。

* [SQL Server 的各版本和支持的功能](https://docs.microsoft.com/sql/sql-server/editions-and-components-of-sql-server-2016)
* [SQL Server各个版本的 R 和 Python 功能](r/differences-in-r-features-between-editions-of-sql-server.md)
...
有关版本和 R 组件下载链接的列表，请参阅[在无 Internet 访问的情况下安装机器学习组件](https://docs.microsoft.com/sql/advanced-analytics/r/installing-ml-components-without-internet-access)。 在具有 Internet 访问的计算机上，会自动标识和安装所需的 R 版本。
...
5. 如果用户需要安装新的 R 包或使用其他用户安装的 R 包，则可能需要在实例上启用包管理，然后分配其他权限。 有关详细信息，请参阅 [启用或禁用 R 包管理](r/r-package-how-to-enable-or-disable.md)。
