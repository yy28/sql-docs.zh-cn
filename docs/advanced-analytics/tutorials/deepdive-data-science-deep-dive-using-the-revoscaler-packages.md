...
此多部分教程介绍了一系列 RevoScaleR 函数，可用于与数据科学相关的任务  。 在此过程中，你将了解如何创建远程计算上下文、在本地和远程计算上下文之间移动数据以及在远程 SQL Server 上执行 R 代码。 还将了解如何在本地和远程服务器上分析和绘制数据，以及如何创建和部署模型。
...
若要在本地和远程计算上下文之间来回切换，则需要两个系统。 本地通常是一个开发工作站，具有足够强大的数据科学工作负荷。 在这种情况下，远程上下文是启用 R 功能 SQL Server。 
...
- 适用于 Visual Studio 的 R 工具 (RTVS) 是一款免费插件，为 Microsoft R 提供 Intellisense、调试和支持。可以将其与 R Server 和 SQL Server 机器学习服务一起使用  。 若要下载，请参阅 [R Tools for Visual Studio](https://www.visualstudio.com/vs/rtvs/)。
