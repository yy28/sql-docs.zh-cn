...
| [教程：深入了解 RevoScaleR](deepdive-data-science-deep-dive-using-the-revoscaler-packages.md) | 了解如何使用 RevoScaleR 包中的函数。 在 R 和 SQL Server 之间移动数据，并切换计算上下文以适应特定的任务。 创建一些模型和图形，并将其在开发环境和数据库服务器之间进行移动。 |
