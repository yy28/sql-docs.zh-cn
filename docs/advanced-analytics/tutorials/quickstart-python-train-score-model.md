...
1. 运行以下脚本以执行该过程。 用于执行存储过程`EXECUTE`的特定语句位于第四行。
...
现在, 您已创建、训练并保存了一个模型, 接下来请转到下一步: 创建用于生成预测的存储过程。 要执行此操作，需要`sp_execute_external_script`调用以运行一个 Python 脚本，该脚本加载序列化模型并向其提供新的数据输入。
...
在此练习中，您学习了如何创建专用于不同任务的存储过程，其中每个存储过程使用系统`sp_execute_external_script`存储过程来启动 Python 进程。 Python 进程的输入作为参数传递给`sp_execute_external` 。 Python 脚本本身和 SQL Server 数据库中的数据变量都作为输入传递。
...
若要了解如何在 SQL Server 中处理 Python 数据类型，请遵循以下快速入门：
