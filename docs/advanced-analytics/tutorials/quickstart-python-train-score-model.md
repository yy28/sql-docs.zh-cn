...
description: 使用 SQL Server 机器学习服务在 Python 中创建简单的预测模型，然后使用新数据预测结果。
ms.prod: sql
...
本快速入门将使用 Python 创建和定型预测模型，并将模型保存到 SQL Server 实例中的表，然后通过 [SQL Server 机器学习服务](../what-is-sql-server-machine-learning.md)使用该模型来根据新数据预测值。
