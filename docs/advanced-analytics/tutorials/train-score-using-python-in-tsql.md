---
title: SQL Server 中的 Python 模型训练和预测使用存储的过程 |Microsoft Docs
...
本练习阐释了 Python 与 SQL Server 的集成，当您将添加[机器学习服务](../install/sql-machine-learning-services-windows-install.md)到数据库引擎实例的功能。 在这种情况下，可以包装内的 Python 代码[存储过程](../../relational-databases/stored-procedures/stored-procedures-database-engine.md)将您的脚本用于生产工作负荷进行操作。 将代码嵌入在存储过程中的功能具有中如何设计、 测试和管理数据科学和机器学习任务的切实优势。 它使您的脚本和模型可访问任何应用程序可以连接到 SQL Server。

在 Python 此练习中，将创建并执行两个存储的过程。 第一个使用经典鸢尾花卉数据集，并生成一个朴素贝叶斯模型来预测鸢尾花物种花卉特征。 第二个过程是进行评分。 它将调用输出一组预测的第一个步骤中生成的模型。 通过将代码放置在存储过程中，操作是包含、 可重用的和可调用其他存储的过程和客户端应用程序。 
...
在此练习中使用的示例数据[ **irissql** ](demo-data-iris-in-sql.md)数据库。

您还需要一个 T-SQL 编辑器，如[SQL Server Management Studio](https://docs.microsoft.com/sql/ssms/download-sql-server-management-studio-ssms?view=sql-server-2017)。
