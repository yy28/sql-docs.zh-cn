---
title: 使用 R 包管理器
description: 使用诸如 install 这样的标准 R 命令将新的 R 包添加到 SQL Server 2016 R Services 或 SQL Server 机器学习服务 (数据库内)。
ms.prod: sql
...
如果有多个 SQL Server 实例, 如 SQL Server 2016 R Services 的并行实例和 SQL Server 机器学习服务, 则如果要在这两个上下文中使用该包, 请分别对每个实例运行安装。 不能跨实例共享包。
