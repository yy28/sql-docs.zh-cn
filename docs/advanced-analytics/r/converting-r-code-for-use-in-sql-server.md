...
+ 需要在[sp_execute_external_script](../../relational-databases/system-stored-procedures/sp-execute-external-script-transact-sql.md) *@no__t 的 1script*参数中运行代码，并将该存储过程参数化。
...
+ 针对可能的数据类型问题创建一份查检表。
    ...
    SQL Server 机器学习服务支持所有 R 数据类型。 但 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 支持的数据类型比 R 更多。因此，将 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 数据发送到 R 时，会执行某些隐式数据类型转换，反之亦然。 可能需要显式转换或转换某些数据。 
...
+ 在某些情况下，必须提前为结果定义输出架构。
    ...
    如果 R 脚本使用参数 `@parallel=1`，还需要输出架构。 原因是 SQL Server 可能会创建多个进程来并行运行查询，最后收集结果。 因此，必须先准备好输出架构，然后才能创建并行进程。
...
+ 提前运行所有查询，并查看 SQL Server 查询计划，以确定可并行执行的任务。
    ...
    如果输入查询可并行化，请将 `@parallel=1` 设置为[sp_execute_external_script](../../relational-databases/system-stored-procedures/sp-execute-external-script-transact-sql.md)的参数的一部分。 
...
+ 利用 T-sql 工具和 ETL 进程。 作为数据工作流的一部分，预先执行特征工程、功能提取和数据清理。
    ...
    当你在专用的 R 开发环境（例如 [!INCLUDE[rsql_rtvs_md](../../includes/rsql-rtvs-md.md)] 或 RStudio）中工作时，你可以将数据提取到计算机上，以迭代方式分析数据，然后写出或显示结果。 
