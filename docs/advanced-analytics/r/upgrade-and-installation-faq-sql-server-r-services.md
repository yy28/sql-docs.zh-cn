...
+ 某些问题只是出现升级从预发行版本。 因此，我们建议你确定你的版本和版本首次在阅读这些说明之前。 若要获取版本信息，请运行`@@VERSION`在 SQL Server Management Studio 从查询中。
+ 升级到最新版本或服务版本越早越好，若要解决在最新版本中修复任何问题。

**适用于：** SQL Server 2016 R Services、 SQL Server 自 2017 年 1 机器学习服务 （数据库）

## <a name="requirements-and-restrictions-on-older-versions-of-sql-server-2016"></a>要求和限制较旧版本的 SQL Server 2016 
...
- 禁用病毒扫描在开始安装之前。 安装程序完成后，我们建议挂起病毒扫描中使用的文件夹[!INCLUDE[ssnoversion](../../includes/ssnoversion.md)]。 最好是挂起整个上扫描[!INCLUDE[ssnoversion](../../includes/ssnoversion.md)]树。
 ...
 - 在 Windows Core 上安装的 SQL Server 实例上安装 Microsoft R Server。 在 SQL Server 2016 RTM 版本中，没有已知的问题时 Microsoft R Server 添加到 Windows Server Core 版本上的实例。 此问题已解决。 如果你遇到此问题，则可以应用中所述的修复[KB3164398](https://support.microsoft.com/kb/3164398)将 R 功能添加到 Windows Server Core 上的现有实例。 有关详细信息，请参阅 [不能在 Windows Server Core 操作系统上安装 Microsoft R Server 独立版](https://support.microsoft.com/kb/3168691)。


## <a name="offline-installation-of-machine-learning-components-for-a-localized-version-of-sql-server-2016"></a>脱机安装的 SQL Server 2016 的本地化版本的机器学习组件

SQL Server 2016 的早期发行版本无法在未连接到 internet 的脱机安装过程中安装特定于区域设置.cab 文件。 此问题已解决在更高版本，但如果安装程序返回一条消息表明它不能安装正确的语言，则可以编辑文件名以允许安装程序以继续。

+ 手动编辑该安装程序文件，以确保安装了正确的语言。 例如，若要安装日语版本的 SQL Server，你将更改文件的名称从 SRS_8.0.3.0_**1033年**到 SRS_8.0.3.0_.cab**1041年**.cab。
...
新安装的任何预发行版本[!INCLUDE[rsql_productname](../../includes/rsql-productname-md.md)]不再受支持。 如果你使用的预发行版本，则升级越早越好。
...
## <a name="r-services-and-r-server-standalone-side-by-side-errors"></a>R Services 和 R Server （独立） 并排显示错误 
...
## <a name="registry-cleanup-to-uninstall-older-components"></a>注册表清理，以卸载较旧的组件
...
## <a name="see-also"></a>另请参阅
 ...
 [SQL Server 机器学习服务 （数据库）](../r/sql-server-r-services.md)

 [SQL Server 机器学习 Server （独立）](../r/r-server-standalone.md)
