...
description: 启用 SQL Server 2016 R Services 或 SQL Server 自 2017 年 1 机器学习 Services （数据库） 上的远程 R 包管理
ms.prod: sql
...
ms.locfileid: "34707655"
---
# <a name="enable-or-disable-remote-package-management-for-sql-server"></a>启用或禁用 SQL Server 的远程包管理
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md-winonly](../../includes/appliesto-ss-xxxx-xxxx-xxx-md-winonly.md)]

本文介绍如何启用远程管理的客户端工作站或不同的计算机学习服务器中的 R 包。 已在 SQL Server 上启用包管理功能后，你可以使用客户端上 RevoScaleR 命令在 SQL Server 上安装包。

> [!NOTE]
> 当前支持的 R 库管理;支持 Python 是在规划之中。

默认情况下，禁用 SQL Server 的外部包管理功能。 你必须运行一个单独的脚本，以启用该功能，如在下一部分中所述。

## <a name="overview-of-process-and-tools"></a>进程和工具的概述

若要启用或禁用 SQL Server 上的包管理，请使用命令行实用工具**RegisterRExt.exe**，包含在**RevoScaleR**包。
...
1. SQL Server 上打开提升的命令提示符并导航到包含的实用工具，RegisterRExt.exe 的文件夹。 默认位置是`<SQLInstancePath>\R_SERVICES\library\RevoScaleR\rxLibs\x64\RegisterRExe.exe`。
...
+ [使用 RevoScaleR 来安装新的 R 程序包](use-revoscaler-to-manage-r-packages.md)
