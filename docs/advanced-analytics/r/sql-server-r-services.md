...
| 在 R 中预先训练的模型 | 预先培训的模型是为特定的用例创建的，由微软的数据科学工程团队维护。您可以使用预先训练的模型按原样对文本中的正反情绪进行评分，或者使用您提供的新数据输入检测图像中的特征。这些模型在R服务中运行，但是不能通过SQL Server设置安装。有关更多信息，请参见[在SQL Server上安装预培训的机器学习模型]。 |
