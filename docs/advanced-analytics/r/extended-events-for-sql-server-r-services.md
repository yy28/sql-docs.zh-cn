---
title: 用于监视 R 和 Python 进程的 SQL Server 机器学习服务的扩展的事件
ms.prod: sql
...
ms.locfileid: "53644876"
---
# <a name="extended-events-for-sql-server-machine-learning-services"></a>用于 SQL Server 机器学习服务的扩展的事件
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md-winonly](../../includes/appliesto-ss-xxxx-xxxx-xxx-md-winonly.md)]

SQL Server 提供了扩展事件，以便在故障排除与相关的操作中使用一套[!INCLUDE[rsql_launchpad](../../includes/rsql-launchpad-md.md)]，以及 Python 或 R 作业发送到[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。

**适用范围：** SQL Server 2016 R Services、 SQL Server 2017 机器学习服务

## <a name="sql-server-events-for-machine-learning"></a>机器学习的 SQL Server 事件
...
有关使用扩展的事件的常规信息，请参阅[扩展事件工具](https://docs.microsoft.com/sql/relational-databases/extended-events/extended-events-tools)。

> [!TIP]
> 对于生成的 SQL Server 扩展事件，尝试使用全新[SSMS XEvent 探查器](https://docs.microsoft.com/sql/relational-databases/extended-events/use-the-ssms-xe-profiler)。 在 Management Studio 中的此新功能显示扩展事件的实时查看器和少受侵入到 SQL Server 比类似的 Profiler 跟踪。

## <a name="additional-events-specific-to-machine-learning-components"></a>其他事件特定于机器学习组件

额外的扩展的事件是可用于组件与相关并由 SQL Server 机器学习服务，如[!INCLUDE[rsql_launchpad](../../includes/rsql-launchpad-md.md)]，和 BXLServer，启动 R 运行时的附属进程。 这些附加的扩展事件从外部进程触发，因此必须捕获为使用外部实用程序。

有关如何执行此操作的详细信息，请参阅部分中，[从外部进程收集事件](#bkmk_externalevents)。
...
|satellite_data_send_start|数据传输开始时触发。| 数据传输开始之前发送的第一个数据块区。|  
...
SQL Server 机器学习服务启动 SQL Server 进程外部运行的一些服务。 若要捕获与这些外部进程相关的事件，必须创建一个事件跟踪配置文件，并将文件放置在该过程的可执行文件所在的同一目录中。  
...
+ **BXLServer**是支持使用外部脚本语言时，R 或 Python 等 SQL 可扩展性的附属进程。 为每个外部语言实例启动 BxlServer 的单独实例。
    ...
    若要捕获与 BXLServer 相关的事件，请将置于 *.config* R 或 Python 安装目录中的文件。  在默认安装中，将为：
...
配置文件必须命名为与可执行文件，使用"[名称].xevents.xml"的格式相同。 换而言之，这些文件的名称如下所示：
...
+ 若要配置跟踪，请编辑*会话名称*占位符、 文件名的占位符 (`[SessionName].xel`)，和你想要捕获，例如，事件的名称`[XEvent Name 1]`， `[XEvent Name 1]`)。  
+ 任意数量的事件包标记可能会出现，并且只要名称属性正确就会收集。
