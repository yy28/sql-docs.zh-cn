...
> [!TIP]
> 对于生成的 SQL Server 扩展事件，请尝试新[SSMS XEvent 探查器](https://docs.microsoft.com/sql/relational-databases/extended-events/use-the-ssms-xe-profiler)。 这一新功能在 Management Studio 中的显示为扩展事件的实时查看器并少受侵入到 SQL Server 比类似的事件探查器跟踪。

## <a name="additional-events-specific-to-machine-learning-components"></a>特定于机器学习组件的其他事件

附加的扩展的事件也可用于与相关并且 SQL Server 计算机学习 Services，例如使用组件的[!INCLUDE[rsql_launchpad](../../includes/rsql-launchpad-md.md)]，和 BXLServer，启动 R 运行时的附属进程。 这些其他扩展事件引发从外部进程，并因此必须捕获为使用外部实用程序。
...
|satellite_data_send_start|数据传输启动时激发。| 数据传输开始之前发送第一个数据块区。|  
...
+ 若要配置跟踪，编辑*会话名称*占位符，文件名的占位符 (`[SessionName].xel`)，和你想要捕获，例如，事件的名称`[XEvent Name 1]`， `[XEvent Name 1]`)。  
+ 任意数量的事件包标记可能出现，而且将会收集，只要名称属性正确。

### <a name="example-capturing-launchpad-events"></a>示例：捕获 Launchpad 事件

下面的示例显示快速启动板服务的事件跟踪定义：
...
[机器学习服务的自定义管理 Studio 报告](../../advanced-analytics/r/monitor-r-services-using-custom-reports-in-management-studio.md)
