...
| 组件 | Description |
|-----------|-------------|
| R 包 | [**RevoScaleR** ](revoscaler-overview.md)是可扩展R的主要库，具有数据操作，转换，可视化和分析功能。  <br/>[**MicrosoftML** ](https://docs.microsoft.com/machine-learning-server/r-reference/microsoftml/microsoftml-package)添加了机器学习算法，以创建用于文本分析，图像分析和情感分析的自定义模型。 <br/>[**sqlRUtils** ](generating-an-r-stored-procedure-for-r-code-using-the-sqlrutils-package.md)提供了帮助函数，用于将R脚本放入T-SQL存储过程，向数据库注册存储过程，以及从R开发环境运行存储过程。<br/>[**mrsdeploy** ](operationalization-with-mrsdeploy.md)提供web服务部署(仅在SQL Server 2017中)。 <br/>[**olapR** ](how-to-create-mdx-queries-using-olapr.md)用于在R中指定MDX查询。|
