...
+ 可通过两种方式获得扩展和优化机会：受益于数据库平台（列存储索引、[资源治理](../../advanced-analytics/r/resource-governance-for-r-services.md)）；以及特定于扩展插件的受益，例如，将适用于 R 和 Python 的 Microsoft 库用于数据科学模型时。 R 是单线程的，而 RevoScaleR 函数是多线程的，后者能够在多个内核之间分配工作负荷。
...
可以使用 Windows 扩展事件 (xEvent) 来监视 SQL Satellite。 有关详细信息，请参阅[适用于 R 的扩展事件](../../advanced-analytics/r/extended-events-for-sql-server-r-services.md)和[适用于 Python 的扩展事件](../../advanced-analytics/python/extended-events-for-python.md)。
