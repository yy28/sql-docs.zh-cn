...
在脚本或模型准备好进行生产使用之后，数据库开发人员可能会将代码或模型嵌入到存储过程中，以便可以从应用程序中调用保存的R或Python代码。从SQL Server存储和运行R代码有许多好处：您可以使用方便的SQL Server接口，并且所有计算都在数据库中进行，从而避免不必要的数据移动。

![rsql_keyscenario1](r/media/rsql-keyscenario1.png)

+ **安全可扩展**。 SQL Server使用新的可扩展性体系结构，可以保护数据库引擎的安全，并隔离R和Python会话。您还可以控制可以执行脚本的用户，并且可以指定可以通过代码访问的数据库。您可以控制分配给运行时的资源量，以防止大量计算危及整体服务器性能。
+ **调度和审计**。在SQL Server中运行外部脚本作业时，您可以控制和审核数据科学家使用的数据。您还可以安排包含外部R或Python脚本的作业和作者工作流，就像安排任何其他T-SQL作业或存储过程一样。

要利用SQL Server中的资源管理和安全功能，部署过程可能包括以下任务：
