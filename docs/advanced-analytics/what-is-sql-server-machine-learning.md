...
传统上，使用R的数据科学家在性能和规模方面都存在问题，尤其是在使用大型数据集时。这是因为公共运行时实现是单线程的，并且只能容纳适合本地计算机上可用内存的那些数据集。与与SQL Server机器学习服务的集成为更好的性能提供了多种特性，并提供更多数据：

+ **RevoScaleR**： 这个R包包含一些最流行的R函数的实现，经过重新设计以提供并行性和扩展性。该软件包还包括通过将计算推送到SQL Server计算机来进一步提高性能和扩展的功能，这些计算机通常具有更大的内存和计算能力。

+ **revoscalepy**。 这个Python库实现了RevoScaleR中最流行的函数，例如远程计算上下文，以及许多支持分布式处理的算法。
 
有关性能的更多信息，请参阅此[性能案例研究](r/performance-case-study-r-services.md)并[R 和数据优化](r/r-and-data-optimization-r-services.md)。
