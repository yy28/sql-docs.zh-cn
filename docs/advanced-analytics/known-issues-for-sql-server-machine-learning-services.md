---
title: 机器学习服务中的已知问题 |Microsoft Docs
ms.prod: sql
...
ms.locfileid: "51704105"
---
# <a name="known-issues-in-machine-learning-services"></a>机器学习服务中的已知的问题
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md-winonly](../includes/appliesto-ss-xxxx-xxxx-xxx-md-winonly.md)]

本指南介绍了已知的问题或限制的机器学习作为 SQL Server 2016 和 SQL Server 2017 中的一个选项提供的组件。
...
### <a name="r-script-runtime-error-sql-server-2017-cu5-cu7-regression"></a>R 脚本运行时错误 （SQL Server 2017 CU5 CU7 回归）
...
**适用于：** SQL Server 2016 R Services，使用 R Server 9.0.0 版或更早版本

### <a name="r-components-missing-from-cu3-setup"></a>缺少 CU3 安装 R 组件
...
### <a name="unable-to-install-python-components-in-offline-installations-of-sql-server-2017-ctp-20-or-later"></a>无法在脱机安装的 SQL Server 2017 CTP 2.0 或更高版本中安装 Python 组件
...
**适用于：** 与 Python 配合使用的 SQL Server 2017
...
### <a name="launchpad-services-fails-to-start-if-the-version-is-different-from-the-r-version"></a>快速启动板服务无法启动的版本是 R 版本不同

如果从数据库引擎单独安装 SQL Server R Services，并且生成版本不同，可能会看到系统事件日志中的以下错误：

> *SQL Server Launchpad 服务无法启动因以下错误： 服务未响应及时的开始或控制请求。*
...
### <a name="remote-compute-contexts-are-blocked-by-a-firewall-in-sql-server-instances-that-are-running-on-azure-virtual-machines"></a>Azure 虚拟机运行的 SQL Server 实例中的防火墙阻止远程计算上下文
...
解决方法是，Azure VM 上打开**高级安全 Windows 防火墙**，选择**出站规则**，并禁用以下规则：**阻止网络中的 R 本地用户帐户的访问权限SQL Server 实例 MSSQLSERVER**。 您还可以将启用，该规则保留，但更改到的安全属性**允许在安全**。
...
### <a name="performance-limits-when-libraries-used-by-sql-server-are-called-from-other-tools"></a>使用 SQL Server 的库从其他工具中调用时的性能限制
...
### <a name="sql-server-data-tools-does-not-support-permissions-required-by-external-scripts"></a>SQL Server Data Tools 不支持所需权限的外部脚本

当你使用 Visual Studio 或 SQL Server Data Tools 以发布数据库项目，如果任何主体具有特定于执行外部脚本的权限时，可能会收到如下错误：

> *TSQL 模型： 检测到反向工程数据库时出错。权限未被识别和未导入。*
...
**适用于：** SQL Server 2016 R Services、 企业版
...
### <a name="access-denied-warning-when-executing-r-scripts-on-sql-server-in-a-non-default-location"></a>在非默认位置中的 SQL Server 中执行 R 脚本时，访问被拒绝警告
...
### <a name="serialization-error-between-old-and-new-versions-of-revoscaler"></a>旧的和新版本的 RevoScaleR 之间序列化错误
...
### <a name="real-time-scoring-does-not-correctly-handle-the-learningrate-parameter-in-tree-and-forest-models"></a>实时评分不会正确处理_learningRate_树和林模型中的参数
...
### <a name="avoid-clearing-workspaces-when-you-execute-r-code-in-a-includessnoversionincludesssnoversion-mdmd-compute-context"></a>避免在执行中的 R 代码时清除工作区[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]计算上下文
...
### <a name="arguments-varstokeep-and-varstodrop-are-not-supported-for-sql-server-data-sources"></a>自变量*varsToKeep*并*varsToDrop*不适用于 SQL Server 数据源

当您使用 rxDataStep 函数将结果写入表时，使用*varsToKeep*并*varsToDrop*是指定要包括或排除操作的一部分的列的简便方法。 但是，对于 SQL Server 数据源不支持这些参数。

### <a name="limited-support-for-sql-data-types-in-spexecuteexternalscript"></a>有限的支持的 SQL 数据类型在 sp\_执行\_外部\_脚本
...
字符串数据从任何往返[!INCLUDE[tsql](../includes/tsql-md.md)]转换为 R，然后到[!INCLUDE[tsql](../includes/tsql-md.md)]试可能会导致损坏。 这是因为不同的编码和在 R 中使用[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]，以及各种排序规则和在 R 中支持的语言和[!INCLUDE[tsql](../includes/tsql-md.md)]。 采用非 ASCII 编码的任何字符串可能不会得到正确处理。

当将字符串数据发送到 R 时，将其转换为 ASCII 表示形式，如有可能。

此限制适用于 SQL Server 和 Python 之间也传递的数据。 应传递为 utf-8 多字节字符，并将其存储为 Unicode。
...
### <a name="variable-scoping-error-when-you-use-the-transformfunc-parameter"></a>变量范围错误，当使用 transformFunc 参数时
...
### <a name="call-to-pretrained-model-fails-if-path-to-model-is-too-long"></a>预先训练的模型调用失败，如果模型的路径太长
...
### <a name="error-when-saving-serialized-model-to-sql-server"></a>错误保存时序列化模型到 SQL Server
...
### <a name="failure-to-initialize-a-varbinary-variable-causes-an-error-in-bxlserver"></a>未能初始化 varbinary 变量 BxlServer 中导致错误
...
### <a name="telemetry-warning-on-successful-execution-of-python-code"></a>在成功执行 Python 代码的遥测数据警告
...
### <a name="revolution-r-enterprise-is-not-supported"></a>不支持 revolution R 企业版
