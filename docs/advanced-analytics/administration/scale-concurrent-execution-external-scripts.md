...
了解 SQL Server 机器学习服务的辅助角色帐户，以及如何更改默认配置以缩放外部脚本的并发执行数。

作为机器学习服务安装过程的一部分，将创建一个新的 Windows*用户帐户池*以支持 @no__t 1 服务执行任务。 这些辅助角色帐户的用途是将不同 SQL Server 用户的外部脚本的并发执行隔离开来。

> [!Note]
> 在 SQL Server 2019 中, **SQLRUserGroup**只具有一个现在为单一 SQL Server Launchpad 服务帐户而不是多个辅助角色帐户的成员。 本文介绍 SQL Server 2016 和2017的辅助角色帐户。
...
- 在默认实例中，该组的名称为 **SQLRUserGroup**。 无论你使用的是 Python 还是 R，名称都是相同的。
...
此池中的帐户数目决定了可以同时处于活动状态的外部脚本会话数。  默认情况下，会创建20个帐户，这意味着20个不同的用户可以同时拥有活动的 Python 或 R 会话。 如果你希望运行超过20个并发脚本, 则可以增加辅助角色帐户的数目。

当同一用户同时执行多个外部脚本时, 该用户运行的所有会话都使用同一辅助角色帐户。 例如，只要资源允许，单个用户可能会有100个不同的 Python 或 R 脚本同时运行，但所有脚本都将使用单个辅助角色帐户运行。

您可以支持的辅助角色帐户的数目以及任何单个用户可以运行的并发会话数仅受服务器资源的限制。 通常，内存是在使用 Python 或 R 运行时时将遇到的第一个瓶颈。
