...
在已连接 Internet 的设备上，累积更新通常通过 Windows 更新进行应用，但也可以使用以下步骤进行有控制的更新。 为数据库引擎应用更新时，安装程序会为你在独立服务器上安装的任何 R 或 Python 功能拉取累积更新。 

在连接已断开的服务器上，需执行额外的步骤。 必须获取数据库引擎的累积更新以及机器学习功能的 CAB 文件。 所有文件必须传输到独立服务器，并手动应用。
