...
4. 上**功能选择**页上，以下选项，应已选择：
    ...
    > [!NOTE]
    > 如果计算机已安装用于SQL Server数据库内分析的计算机学习服务，请避免安装**共享功能**如。这会创建重复的库。
    > 
    > 此外，虽然在SQL Server中运行的R或Python脚本是由SQL Server管理的，以便与其他数据库引擎服务使用的内存不冲突，但是独立的机器学习服务器没有这样的约束，并且可以干扰其他数据库操作。最后，通常用于操作化的RDP会话的远程访问通常被数据库管理员阻塞。
    > 
    > 出于这些原因，我们通常建议您在与SQL Server机器学习服务不同的计算机上安装Machine Learning Server（独立）
