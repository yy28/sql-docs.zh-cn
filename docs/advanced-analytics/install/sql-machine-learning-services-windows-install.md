...
安装和管理 R 包的过程中是不同 SQL Server 2016 和 SQL Server 自 2017 年。 在 SQL Server 2016 中，数据库管理员必须安装用户需要的 R 包。 在 SQL Server 自 2017 年，可以将用户组设置为共享上的每个数据库级别，包或配置数据库角色，以使用户能够安装其自己的包。 有关详细信息，请参阅[在 SQL Server 中安装新的 R 包](../r/install-additional-r-packages-on-sql-server.md)。
