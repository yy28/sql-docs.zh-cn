...
SSMA 安装后，你可以使用 SSMA 访问将数据库迁移到[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]或 Azure SQL DB。 它有助于熟悉 SSMA 用户界面在开始之前。 下图显示 SSMA 用户界面。 这包括元数据资源管理器、 元数据、 工具栏、 输出窗格中，和错误列表窗格中：  
  
![访问图形用户界面的 SSMA](../../ssma/access/media/ssmaforaccessgui.gif "SSMA for 访问图形用户界面")  
  
若要开始迁移，创建新项目，然后添加到访问元数据资源管理器的 Access 数据库。 然后可以右键单击访问元数据资源管理器来执行任务，例如导出操作访问数据库对象的清单对象[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]或 Azure SQL DB，创建评估转换到的报表[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]或 Azure SQL DB，并将转换到的访问架构[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]或 Azure SQL DB 架构。 你还可以执行这些任务通过工具栏和菜单。  
...
如果选择了 Azure SQL DB 从迁移到新项目对话框中的下拉列表中，你必须连接到 Azure SQL DB。 在成功连接之后, 的 Azure SQL DB 数据库层次结构会在 Azure SQL DB 元数据资源管理器中。 将访问架构转换为 Azure SQL DB 架构后，你可以在 Azure SQL DB 元数据资源管理器中选择这些转换后的架构并且然后加载架构转换[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]。  
...
SSMA 包含两个元数据资源管理器来浏览和访问上执行操作和[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]或 Azure SQL DB 数据库。  
  
#### <a name="access-metadata-explorer"></a>访问元数据资源管理器  
访问元数据资源管理器中显示有关已添加到项目的 Access 数据库的信息。 当你将添加 Access 数据库时，SSMA 检索有关该数据库的元数据。 这是可用于访问元数据资源管理器的元数据。  
  
通过使用访问元数据资源管理器，你可以执行以下任务：  
  
-   浏览每个访问数据库中的表。  
  
-   选择要转换的对象，然后将转换到的对象[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]语法。 有关详细信息，请参阅[转换访问数据库对象](http://msdn.microsoft.com/en-us/e0ef67bf-80a6-4e6c-a82d-5d46e0623c6c)。  
  
-   选择用于数据迁移的对象，然后将数据迁移到这些对象从[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]。 有关详细信息，请参阅[将访问数据迁移到 SQL Server](http://msdn.microsoft.com/en-us/f3b18af7-1af0-499d-a00d-a0af94895625)。  
...
此元数据资源管理器可用于选择转换后的访问数据库对象，然后加载 （同步） 的实例的那些对象[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]或 Azure SQL DB。  
  
有关详细信息，请参阅[加载到 SQL Server 转换数据库对象](http://msdn.microsoft.com/en-us/4e854eee-b10c-4f0b-9d9e-d92416e6f2ba)。  
  
### <a name="metadata"></a>元数据  
每个元数据资源管理器右侧是描述所选的对象的选项卡。 例如，如果选择访问元数据资源管理器中的表，则四个选项卡将显示：**表**，**类型映射，属性**，和**数据**。 如果你选择的表中[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]元数据资源管理器中，将显示三个选项卡：**表**， **SQL**，和**数据**。  
  
大多数元数据设置是只读的。 但是，你可以更改以下元数据：  
  
-   在访问元数据资源管理器中，你可能会改变类型映射。 创建报表或转换架构之前，应进行这些更改。  
  
-   在[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]元数据资源管理器，你可以在更改表和索引属性**表**选项卡。 在加载到架构之前，应进行这些更改[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]。 有关详细信息，请参阅[转换访问数据库对象](http://msdn.microsoft.com/en-us/e0ef67bf-80a6-4e6c-a82d-5d46e0623c6c)。  
...
|按钮|函数|  
|----------|------------|  
|**转换、 加载和迁移**|转换访问数据库时，转换将对象加载到[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]或 Azure SQL DB，然后将迁移到的数据[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]或所有在一个步骤中的 Azure SQL DB。|  
