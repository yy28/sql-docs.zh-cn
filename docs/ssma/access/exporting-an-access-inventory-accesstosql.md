---
title: 导出访问清单（AccessToSQL） |Microsoft Docs
ms.prod: sql
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
...
如果你有多个访问数据库，但不确定要将哪些数据库迁移[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]到，则可以在项目中导出所有 Access 数据库的清单。 然后，可以查看和查询清单元数据，以确定要迁移的数据库中的数据库和对象。 此清单使你可以快速找到问题的答案，例如：  
...
若要导出访问清单，必须首先打开或创建 SSMA 项目，然后添加要分析的 Access 数据库。 将数据库添加到 SSMA 项目后，可以将有关这些数据库的元数据导出到[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]指定的数据库和架构。 如有必要，SSMA 会创建用于存储元数据的表。 然后，SSMA 将有关 Access 数据库的元数据添加[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]到数据库。  
...
4.  如果实例[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]配置为接受非默认端口上的连接，请在 " [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] **服务器端口**" 框中输入用于连接的端口号。 对于的默认实例[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]，默认端口号为1433。 对于命名实例，SSMA 尝试从[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Browser 服务获取端口号。  
...
有关连接到[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]的详细信息，请参阅[连接到 SQL Server &#40;AccessToSQL&#41;](../../ssma/access/connecting-to-sql-server-accesstosql.md)。  
...
导出有关 Access 数据库的元数据后，可以查询元数据。 下面的说明介绍了如何使用中[!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]的 "查询编辑器" 窗口来运行查询。  
  
**查询元数据**  
  
1.  从 "**开始**" 菜单，依次指向 "**所有程序**" **、 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] microsoft 2005**或**microsoft [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 2008**或** [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] microsoft 2012**，然后单击 " **SQL Server Management Studio**"。  
...
在运行以下任何查询之前，您应该运行*database_name*查询，以确保查询是针对包含导出的元数据的数据库运行的。 例如，如果将元数据导出到名为 MyAccessMetadata 的数据库，则需要在[!INCLUDE[tsql](../../includes/tsql-md.md)]代码的开头添加以下内容：  
