...
4.  **重新连接提供程序：** 这样，用户可以设置发生连接故障时重新连接设置。 这可以为源和目标服务器设置。  
...
7.  **停止操作：** 中间在操作期间，如果用户想要停止此操作，然后**Ctrl + C**可以使用热键。 SSMA for Access 控制台将等待操作完成，并终止控制台执行。  
...
在脚本文件或服务器连接文件中，可以定义服务器连接参数。 请参阅[创建服务器连接文件&#40;AccessToSQL&#41; ](../../ssma/access/creating-the-server-connection-files-accesstosql.md)部分，了解更多详细信息。  
  
## <a name="script-commands"></a>脚本命令  
脚本文件包含一系列以 XML 格式的迁移工作流命令。 SSMA 控制台应用程序处理以使其不显示在脚本文件中的命令顺序迁移。  
  
例如，典型的数据迁移的 Access 数据库中的特定表如下所示的层次结构： 数据库-&gt;表。  
  
当成功执行脚本文件中的所有命令时，SSMA 控制台应用程序退出，并将该控件返回给用户。 脚本文件的内容可能会更多或更少静态变量的信息包含在[变量值文件](creating-variable-value-files-accesstosql.md)或变量值的脚本文件中的单独部分中。  
...
脚本命令的完整列表可在[执行 SSMA 控制台&#40;AccessToSQL&#41;](../../ssma/access/executing-the-ssma-console-accesstosql.md)  
  
## <a name="script-file-validation"></a>脚本文件验证  
用户可以轻松地验证他/她脚本文件是否符合架构定义文件**A2SSConsoleScriptSchema.xsd**架构文件夹中可用。  
