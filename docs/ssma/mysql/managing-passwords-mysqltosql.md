...
1.  **清除文本：** 在密码节点的值属性中键入数据库密码。 服务器连接文件的脚本文件的服务器部分中的服务器定义节点下找到它。  
    ...
    **加密的密码：** 在这种情况下，指定的密码，在本地计算机中 ProtectedStorage.ssma 以加密形式存储。  
    ...
    -   **保护密码**  
        ...
        -   执行`SSMAforMySQLConsole.exe`与`-securepassword`，并在命令行传递包含中的服务器定义部分的密码节点的连接或脚本文件的服务器添加开关。  
    ...
    -   **删除加密的密码**  
        ...
        执行`SSMAforMySQLConsole.exe`与`-securepassword`和`-remove`开关在命令行传递的服务器 id，以从本地计算机上存在受保护的存储文件中删除加密的密码。  
    ...
    -   **列出其密码进行加密的服务器 Id**  
        ...
        执行`SSMAforMySQLConsole.exe`与`-securepassword`和`-list`切换通过在命令行列出了其密码已加密的服务器 id。  
