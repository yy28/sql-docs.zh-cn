...
1.  **明文形式：** 'password' 节点的值属性中键入数据库密码。 它是在脚本文件或服务器连接文件的服务器部分中的服务器定义节点下找到。  
    ...
    **加密的密码：** 指定的密码，在这种情况下，存储在 ProtectedStorage.ssma 的本地计算机上以加密形式。  
    ...
    -   **保护密码**  
        ...
        -   执行`SSMAforMySQLConsole.exe`与`–securepassword`并在命令行传递包含中的服务器定义部分的密码节点的连接或脚本文件的服务器添加交换机。  
    ...
    -   **删除加密的密码**  
        ...
        执行`SSMAforMySQLConsole.exe`与`–securepassword`和`–remove`切换在将服务器 id，以从本地计算机上存在的受保护的存储文件中删除加密的密码传递的命令行。  
    ...
    -   **列出其密码进行加密的服务器 Id**  
        ...
        执行`SSMAforMySQLConsole.exe`与`–securepassword`和`–list`切换在命令行，若要列出其密码已加密的所有服务器 id。  
