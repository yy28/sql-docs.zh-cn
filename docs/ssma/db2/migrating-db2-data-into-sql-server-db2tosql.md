...
ms.locfileid: "68074103"
---
# <a name="migrating-db2-data-into-sql-server-db2tosql"></a>将 DB2 数据迁移到 SQL Server （DB2ToSQL）
使用[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]成功同步转换后的对象后，可以将数据从 DB2 迁移到[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。  
...
在将数据迁移[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]到之前，请在 "**项目设置**" 对话框中查看项目迁移选项。  
...
迁移数据是一种大容量加载操作，用于将数据行从 DB2 表[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]移入事务中的表。 在项目设置中配置每[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]个事务中加载的行数。  
...
1.  检查下列各项：  
    ...
    -   已将转换的对象与[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]数据库同步。  
...
5.  在 "**连接到 DB2** " 对话框中，输入连接凭据，然后单击 "**连接**"。 有关连接到 DB2 的详细信息，请参阅[连接到 Db2 数据库 &#40;DB2ToSQL&#41;](../../ssma/db2/connecting-to-db2-database-db2tosql.md)  
    ...
    若要连接到目标数据库[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]，请在 "**连接到 SQL Server** " 对话框中输入连接凭据，然后单击 "**连接**"。 有关连接到[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]的详细信息，请参阅[连接到 SQL Server](https://msdn.microsoft.com/b59803cb-3cc6-41cc-8553-faf90851410e)  
