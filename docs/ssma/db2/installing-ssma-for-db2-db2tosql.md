---
title: 安装用于 DB2 的 SSMA （DB2ToSQL） |Microsoft Docs
ms.prod: sql
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
...
[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]DB2 的迁移助手（SSMA）包含一个客户端应用程序，该应用程序用于执行从 DB2 到[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] AZURE SQL DB 的迁移。 它还包含一个扩展包，支持数据迁移，并在迁移的数据库中使用 DB2 系统功能。  
...
|[在 SQL Server 上安装 SSMA 组件 &#40;DB2ToSQL&#41;](../../ssma/db2/installing-ssma-components-on-sql-server-db2tosql.md)|提供有关在的[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]实例上安装扩展包的和说明的信息。|  
