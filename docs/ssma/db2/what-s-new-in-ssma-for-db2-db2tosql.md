...
- SSMA for DB2 已得到增强提高质量和转换的度量值的目标修补程序。
- SSMA for DB2 的 32 位版本基于普遍需求，已恢复。 与以前的实现 （之前 v7.4) 相比，有两个安装包，但它们不能并行安装。 因此，你必须选择最适合您的版本基于连接组件。 最好始终使用 64 位版本，如有可能。
