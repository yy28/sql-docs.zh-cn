---
title: 评估 DB2 架构以进行转换 (DB2ToSQL) |Microsoft Docs
ms.prod: sql
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
...
SSMA 时创建此评估报表时，将转换为所选的 DB2 数据库对象[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]语法，然后显示结果。  
...
3.  右键单击**架构**，然后选择**创建报表**。  
    ...
    评估完成后， [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Migration Assistant for DB2： 评估报告窗口中会显示。  
...
4.  查看任何错误消息，，然后确定你想要使用导致转换问题的对象执行操作：  
    ...
    -   更新在 SSMA 中的 DB2 语法。 您可以更新过程、 函数、 触发器、 封装的函数和打包的过程的语法。 若要更新的语法，在 DB2 元数据资源管理器窗格中选择的对象中，单击**SQL**选项卡，然后修改的 SQL 代码。 导航离开该项时，系统将提示你保存已更新的语法。 您可以查看报告的错误的对象上**报表**选项卡。  
  
    -   在 DB2，可以修改要删除或修改有问题的代码的 DB2 对象。 若要将更新的代码加载到 SSMA，必须更新元数据。 有关详细信息，请参阅[连接到 DB2 数据库&#40;DB2ToSQL&#41;](../../ssma/db2/connecting-to-db2-database-db2tosql.md)。  
  
    -   您可以从迁移中排除对象。 在中[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]元数据资源管理器和 DB2 的元数据资源管理器中，清除项旁边的复选框，然后加载到对象[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]和将数据从 DB2 迁移。  
