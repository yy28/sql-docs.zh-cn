...
"包装（加密）" 是隐藏源代码的过程。 换行有助于保护来自竞争对手和可能被滥用的代码的源代码。 在 Oracle 中，可以使用`DBMS_DDL` subprograms 包装源代码。
...
请考虑下面的示例，我们已声明了两个变量，一个用于存储包规范，另一个用于将包正文存储为字符串。 然后，将这些变量传递到预定义的`DBMS_DDL.CREATE_WRAPPED`存储过程，该存储过程将创建包装包。
...
在 Oracle 中执行上述代码时，将创建一个具有名称`Wrapped_PKG`的包。 此包以加密格式创建，因此你无法查看此包内的文本/代码。

尝试使用 SSMA 转换包`Wrapped_PKG`代码时，会收到以下错误：
