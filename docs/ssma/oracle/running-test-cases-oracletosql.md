...
ms.locfileid: "38982369"
---
# <a name="running-test-cases-oracletosql"></a>运行测试用例 (OracleToSQL)
SSMA 测试人员运行时测试用例，它执行所选测试对象，并创建有关验证结果的报告。 如果结果为在这两个平台上完全相同，测试成功。 Oracle 之间的对象的对应关系和[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]根据当前的 SSMA 项目的架构映射设置确定。  
...
测试完成后，创建测试用例报表。 单击**报表**按钮，以查看[测试用例报表](http://msdn.microsoft.com/8da14323-9dd6-4019-bf79-3e8b972a9bc0)。 测试 （测试用例报表） 的结果会自动存储在[测试结果储存库](http://msdn.microsoft.com/f941cce4-d3e3-4aeb-a88a-4f101a97a9f4)以供将来使用。  
...
在已验证表的架构中创建以下对象[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]。  
...
在中创建以下对象[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]ssmatesterdb 数据库中。  
