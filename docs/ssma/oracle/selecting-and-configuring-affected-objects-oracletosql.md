...
在位于窗口左侧 Oracle 对象树中，请检查表和外键，在其中更改应比较正在相同。  
...
-   如果你选择**True**，SSMA 将 SQL Server 中的列的内容对 Oracle 执行测试后比较此列的内容。 
...
你可以查看 SSMA tester 生成上的选择语句**比较 SQL**页。 测试人员将比较的结果集的行的行基于这些语句。 一个 Oracle 的结果集的每个下一行应等于 SQL Server 中生成的结果集的下一行。
