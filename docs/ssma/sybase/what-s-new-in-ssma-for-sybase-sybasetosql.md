...
有关 SAP ASE 的 SSMA v8.2 发布已使用一组目标的设计用于提高质量和转换指标，以及针对修补程序的修补程序进行增强：
...
SSMA for Sybase 添加的 2010 年 7 月版本：
...
SSMA for Sybase 添加的 2007 年 5 月版本：  
  
* 保存项目时更快地加载数据库内容的功能。  
* 对用户输入的注释中的 SQL Server 的支持格式的 SQL 模式。  
* 中对象转换的改进。  
