...
每个元数据资源管理器右侧是描述所选的对象的选项卡。 例如，如果 Sybase 元数据资源管理器中选择一个表，会显示六个选项卡：**表**， **SQL**，**类型映射**，**数据**，**属性**，并**报表**。 **报表**仅在创建包含所选的对象的报表之后，选项卡包含的信息。 如果您选择的表中[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]或 SQL Azure 元数据资源管理器中，会显示三个选项卡：**表**， **SQL**，和**数据**。  
