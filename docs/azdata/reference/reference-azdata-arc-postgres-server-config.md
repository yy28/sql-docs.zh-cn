...
description: azdata arc postgres server config 命令的参考文章。
author: MikeRayMSFT
ms.author: mikeray
ms.reviewer: seanw
ms.date: 09/22/2020
...
|命令|说明|
| --- | --- |
[azdata arc postgres server config init](#azdata-arc-postgres-server-config-init) | 初始化 PostgreSQL 服务器组的 CRD 和规范文件。
[azdata arc postgres server config add](#azdata-arc-postgres-server-config-add) | 在配置文件中为 json 路径添加值。
[azdata arc postgres server config remove](#azdata-arc-postgres-server-config-remove) | 在配置文件中为 json 路径删除值。
[azdata arc postgres server config replace](#azdata-arc-postgres-server-config-replace) | 在配置文件中为 json 路径替换值。
[azdata arc postgres server config patch](#azdata-arc-postgres-server-config-patch) | 基于 json 修补程序文件来修补配置文件。
## <a name="azdata-arc-postgres-server-config-init"></a>azdata arc postgres server config init
初始化 PostgreSQL 服务器组的 CRD 和规范文件。
```bash
...
#### `--path -p`
应将 PostgreSQL 服务器组的 CRD 和规范写入到的路径。
