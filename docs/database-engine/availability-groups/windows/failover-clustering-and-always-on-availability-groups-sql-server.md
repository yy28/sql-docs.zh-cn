...
helpviewer_keywords: 
  ...
  - "故障转移群集实例 [SQL Server], 查看故障转移群集 [SQL Server]"
  - "仲裁 [SQL Server]"
...
---
# 故障转移群集和 AlwaysOn 可用性组 (SQL Server)
...
> [!NOTE]  
>  有关不同版本的 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 中故障转移群集内的**节点数**和 **AlwaysOn 可用性组**的详细信息，请参阅 [SQL Server 2012 各个版本支持的功能](http://go.microsoft.com/fwlink/?linkid=232473) (http://go.microsoft.com/fwlink/?linkid=232473)。  
  
### FCI 上承载可用性副本的注意事项  
  
> [!IMPORTANT]  
>  如果计划在 SQL Server 故障转移群集实例 (FCI) 上承载可用性副本，请确保 Windows Server 2008 主机节点满足故障转移群集实例 (FCI) 的 AlwaysOn 先决条件和限制。 有关详细信息，请参阅[针对 AlwaysOn 可用性组的先决条件、限制和建议 (SQL Server)](../../../database-engine/availability-groups/windows/prereqs, restrictions, recommendations - always on availability.md)。  
 ...
 Marcel 配置具有两个节点 `NODE01` 和 `NODE02`的一个 WSFC 群集。 他在 `fciInstance1` 和 `NODE01` 上安装一个 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 故障转移群集实例 `NODE02`，其中 `NODE01` 是 `fciInstance1` 的当前所有者。  
...
-   **博客：**  
     ...
     [将 SQL Server 的 Windows 故障转移群集（可用性组或 FCI）配置为有限安全](http://blogs.msdn.com/b/sqlAlways%20On/archive/2012/06/05/configure-windows-failover-clustering-for-sql-server-availability-group-or-fci-with-limited-security.aspx)  
