...
ms.topic: "article"
helpviewer_keywords: 
  - "伙伴 [SQL Server]，将客户端连接到"
  - "数据库镜像 [SQL Server]，将客户端连接到"
  - "客户端连接 [SQL Server]，数据库镜像"
  - "连接 [SQL Server]，数据库镜像"
 ...
 **本主题内容：**  
 ...
 [建立到数据库镜像会话的初始连接](#InitialConnection)  
 介绍了初始伙伴名称和故障转移伙伴名称，解释了数据访问接口如何建立到镜像数据库的初始连接，并说明了连接到镜像数据库所用的连接字符串属性。  
  
 [连接重试算法（用于 TCP/IP 连接）](#RetryAlgorithm)  
 描述针对 TCP/IP 连接的连接重试算法。  
  
 [重新连接到数据库镜像会话](#Reconnecting)  
 说明了建立到数据库镜像会话的连接因任何原因而失败时，如何重新连接到镜像数据库；同时说明了重定向对客户端应用程序的影响。  
  
 [已过时故障转移伙伴名称的影响](#StalePartnerName)  
 讨论了数据库管理员更改镜像数据库的镜像伙伴之一时产生的影响。  
...
> [!IMPORTANT]  
>  如果将协议前缀和 **Server** 属性 (`Server=tcp:`*\<servername>*) 绑定在一起，则会导致与 **Network** 属性不兼容；如果在这两个位置都指定了协议，则可能会导致错误生成。 因此，建议连接字符串使用 **Network** 属性指定协议，并且仅在 **Server** 属性 (`"Network=dbmssocn; Server=`*\<servername>*`"`) 中指定服务器名称。  
...
|-------------------|----------------------|-------------------|------------------------------------------------------------------------------|  
|原始镜像配置。|Partner_A|Partner_B|Partner_A 作为初始伙伴名称保存在缓存中。 客户端成功连接到 Partner_A。 客户端下载镜像服务器的名称 Partner_B 并将其保存在缓存中，同时忽略客户端提供的故障转移伙伴名称。|  
