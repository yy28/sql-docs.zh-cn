---
title: "允许使用 Windows 身份验证对数据库镜像端点进行网络访问 (SQL Server) | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
## 示例  
 下面的 [!INCLUDE[tsql](../../includes/tsql-md.md)] 示例为属于 `Otheruser` 域的 `Adomain` 用户帐户创建了一个 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 登录名。 并授予了此用户对预先存在的 `Mirroring_Endpoint` 数据库镜像端点的连接权限。  
