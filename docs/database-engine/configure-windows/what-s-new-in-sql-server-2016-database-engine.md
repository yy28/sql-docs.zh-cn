---
title: "SQL Server 2016（数据库引擎）中的新增功能 | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
  - "SQL2016_rc1"
ms.date: "01/13/2017"
...
- 若要下载 [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]，请转到**[评估中心](https://www.microsoft.com/en-us/evalcenter/evaluate-sql-server-2016)**![download](../../analysis-services/media/download.png "download")。

- 是否拥有 Azure 帐户？  然后转到**[此处](https://azure.microsoft.com/en-us/marketplace/partners/microsoft/sqlserver2016rc0evaluationwindowsserver2012r2/?wt.mc_id=sqL16_vm)**启动装有 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 的虚拟机。

![注释](../../analysis-services/instances/install-windows/media/ssrs-fyi-note.png "注释")有关最新的发行说明，请参阅 [SQL Server 2016 发行说明](../../sql-server/sql-server-2016-release-notes.md)。
...
-     已添加对更具一般性的查询提示模型的支持：`OPTION (USE HINT('<hint1>', '<hint2>'))`。 有关详细信息，请参阅[查询提示 (Transact-SQL)](Query%20Hints%20\(Transact-SQL\).md)。  
...
-  扩展了支持大量功能的版本。 这包括向所有版本增添了行级安全性、Always Encrypted、动态数据屏蔽、数据库审核、内存中 OLTP 和多个其他功能。 有关详细信息，请参阅 [SQL Server 2016 的各版本和支持的功能](../../sql-server/sql-server-2016-的各版本和支持的功能.md)。   
...
####  <a name="a-namecolumnstorea-columnstore-indexes"></a><a name="columnstore"></a>列存储索引
...
有关更多详细信息，请参阅联机丛书的[列存储索引指南](../Topic/Columnstore%20Indexes%20Guide.md)一节中的以下主题：

- [列存储索引版本的功能摘要](../Topic/Columnstore%20Indexes%20Versioned%20Feature%20Summary.md) — 包括新增功能。
...
####  <a name="a-namescopedconfigurationa-database-scoped-configurations"></a><a name="scopedconfiguration"></a>数据库作用域配置
...
####  <a name="a-nameinmemorya-in-memory-oltp"></a><a name="InMemory"></a>内存中 OLTP
...
- [内存中 OLTP 不支持的 SQL Server 功能](Unsupported%20SQL%20Server%20Features%20for%20In-Memory OLTP.md)
...
- 对数据大小不再有任何限制。 请参阅[估算内存优化表的内存需求](Estimate%20Memory%20Requirements%20for%20Memory-Optimized Tables.md)。
...
- [透明数据加密 (TDE)](../../relational-databases/security/encryption/transparent-data-encryption-tde.md) 支持。 如果为数据库配置了加密，则[内存优化文件组](../../relational-databases/in-memory-oltp/the-memory-optimized-filegroup.md)中的文件现在也会加密。

有关详细信息，请参阅[内存中 OLTP&#40;内存中优化&#41;](../../relational-databases/in-memory-oltp/in-memory-oltp-in-memory-optimization.md)。


####  <a name="a-namequeryoptimizera-query-optimizer"></a><a name="QueryOptimizer"></a>查询优化器
...
####  <a name="a-namelivestatsa-live-query-statistics"></a><a name="LiveStats"></a>实时查询统计信息
...
####  <a name="a-namequerystorea-query-store"></a><a name="QueryStore"></a>查询存储
...
####  <a name="a-nametta-temporal-tables"></a><a name="TT"></a>临时表
...
####  <a name="a-namestripedbackuptoazurea-striped-backups-to-microsoft-azure-blob-storage"></a><a name="StripedBackupToAzure"></a>向 Microsoft Azure Blob 存储进行条带备份
...
####  <a name="a-namefilesnapshotbackupa-file-snapshot-backups-to-microsoft-azure-blob-storage"></a><a name="FileSnapshotBackup"></a>向 Microsoft Azure Blob 存储进行文件快照备份
...
####  <a name="a-namemanagedbackupa-managed-backup"></a><a name="ManagedBackup"></a>托管备份
...
####  <a name="a-namemultipletempdba-tempdb-database"></a><a name="multipleTempDB"></a>TempDB 数据库
 对 TempDB 做了多项增强：
...
####  <a name="a-nameforjsona-built-in-json-support"></a><a name="ForJson"></a>内置的 JSON 支持
...
####  <a name="a-namestretcha-stretch-database"></a><a name="stretch"></a> Stretch 数据库
...
####  <a name="a-nameutf8a-support-for-utf-8"></a><a name="UTF8"></a>支持 UTF-8
...
####  <a name="a-namedefaultdba-new-default-database-size-and-autogrow-values"></a><a name="DefaultDB"></a>新的默认数据库大小和自动增长值
模型数据库的新值和新数据库（基于模型）的默认值。 数据和日志文件的初始大小现在为 8 MB。 数据和日志文件的默认自动增长增量现在为 64MB。


###  <a name="a-nametsqla-transact-sql-enhancements"></a><a name="TSQL"></a>Transact-SQL 增强功能
...
- 新的查询提示 **NO_PERFORMANCE_SPOOL** 可以防止将后台打印运算符添加到查询计划。 在配合假脱机操作运行许多并发查询时，这可以提高性能。 有关详细信息，请参阅[查询提示 (Transact-SQL)](../Topic/Query%20Hints%20\(Transact-SQL\).md)。
...
- 自动增长选项：跟踪标志 1117 已由 ALTER DATABASE 的 AUTOGROW_SINGLE_FILE 和 AUTOGROW_ALL_FILES 选项取代，跟踪标志 1117 不再有效。 有关详细信息，请参阅 [ALTER DATABASE 文件和文件组选项 &#40;Transact-SQL&#41;](../Topic/ALTER%20DATABASE%20File%20and%20Filegroup%20Options%20\(Transact-SQL\).md) 以及 [sys.filegroups &#40;Transact-SQL&#41;](../../relational-databases/system-catalog-views/sys-filegroups-transact-sql.md) 的新列 is_autogrow_all_files。
- 混合盘区分配：对于用户数据库，对象前 8 页的默认分配将从使用混合页盘区更改为使用统一盘区。 跟踪标志 1118 已由 ALTER DATABASE 的 SET MIXED_PAGE_ALLOCATION 选项取代，跟踪标志 1118 不再有效。 有关详细信息，请参阅 [ALTER DATABASE SET 选项 &#40;Transact-SQL&#41;](../Topic/ALTER%20DATABASE%20SET%20Options%20\(Transact-SQL\).md) 以及 [sys.databases &#40;Transact-SQL&#41;](../../relational-databases/system-catalog-views/sys-databases-transact-sql.md) 的新列 `is_mixed_page_allocation_on`。


###  <a name="a-namesystemtablea-system-view-enhancements"></a><a name="SystemTable"></a>系统视图增强功能
...
- 添加了两个新的查询提示（MIN_GRANT_PERCENT 和 MAX_GRANT_PERCENT）用于指定内存授予。 请参阅[查询提示 (Transact-SQL)](../Topic/Query%20Hints%20\(Transact-SQL\).md)。
...
###  <a name="a-namesecuritya-security-enhancements"></a><a name="Security"></a>安全性改进

####  <a name="a-namerlsa-row-level-security"></a><a name="RLS"></a>行级安全性
...
####  <a name="a-namemaskinga-dynamic-data-masking"></a><a name="Masking"></a>动态数据屏蔽
...
####  <a name="a-namepermsa-new-permissions"></a><a name="Perms"></a>新权限
...
####  <a name="a-nametdea-transparent-data-encryption"></a><a name="TDE"></a>透明数据加密
- 透明数据加密已得到增强，支持对加密使用 Intel AES-NI 硬件加速。 这可以减少启用透明数据加密所需的 CPU 开销。

###  <a name="a-nameaesa-aes-encryption-for-endpoints"></a><a name="AES"></a>终结点的 AES 加密
- 终结点的默认加密已从 RC4 更改为 AES。

####  <a name="a-namenewcredentialtypea-new-credential-type"></a><a name="newcredentialtype"></a>新凭据类型
- 现在可以在数据库级别创建凭据（此外，还包括以前提供的服务器级别凭据）。 有关详细信息，请参阅 [CREATE DATABASE SCOPED CREDENTIAL &#40;Transact-SQL&#41;](../../t-sql/statements/create-database-scoped-credential-transact-sql.md)。


###  <a name="a-namehighavailabilitya-high-availability-enhancements"></a><a name="HighAvailability"></a>高可用性增强
...
###  <a name="a-namerepla-replication-enhancements"></a><a name="Repl"></a>复制增强
...
###  <a name="a-nametoolsa-tools-enhancements"></a><a name="Tools"></a>工具增强
...
- 不再从主功能树安装 SQL Server 管理工具；有关详细信息，请参阅[安装带有 SSMS 的 SQL Server 管理工具](Install%20SQL%20Server%20Management%20Tools%20\(SSMS\).md)。
...
####  <a name="a-nameuaa-upgrade-advisor"></a><a name="UA"></a>升级顾问
...
[安装带有 SSMS 的 SQL Server 管理工具](Install%20SQL%20Server%20Management%20Tools%20\(SSMS\).md)
