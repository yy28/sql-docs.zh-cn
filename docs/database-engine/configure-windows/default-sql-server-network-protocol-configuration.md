...
Evaluation  | 新安装  | 已启用   | 禁用  | 对于网络连接为禁用。
...
>[!NOTE]
> 如果该实例正在 [!INCLUDE[ssNoVersion_md](../../includes/ssnoversion-md.md)] 故障转移群集上运行，它将在 [!INCLUDE[ssNoVersion_md](../../includes/ssnoversion-md.md)] 安装过程中监听为 [!INCLUDE[ssNoVersion_md](../../includes/ssnoversion-md.md)] 选择的每个 IP 地址的那些端口。
...
* [使用 Named Pipes 创建有效的连接字符串](Creating%20a%20Valid%20Connection%20String%20Using%20Named%20Pipes.xml)
