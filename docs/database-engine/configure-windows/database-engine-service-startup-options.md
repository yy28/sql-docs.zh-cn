...
ms.topic: "article"
helpviewer_keywords: 
  - "单用户模式 [SQL Server]，启动选项"
  - "覆盖默认启动选项"
  - "最低配置模式 [SQL Server]，启动选项"
  - "默认启动选项"
  - "暂时覆盖默认启动选项 [SQL Server]"
  - "启动选项 [SQL Server]"
  - "启动 SQL Server，选项"
...
## 关于启动选项  
 ...
 可以使用 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 配置管理器设置启动选项。 有关信息，请参阅[配置服务器启动选项 (SQL Server Configuration Manager)](../../database-engine/configure-windows/configure-server-startup-options-sql-server-configuration-manager.md)。  
...
|**-n**|不要使用 Windows 应用程序日志来记录 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 事件。 如果你使用 **-n** 启动 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例，我们建议你同时使用 **-e** 启动选项。 否则，将不会记录 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 事件。|  
...
|**-T**  *trace#*|指示 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例启动时，指定的跟踪标志 (*trace#*) 应同时生效。 跟踪标记用于以非标准行为启动服务器。 有关详细信息，请参阅[跟踪标志 (Transact-SQL)](../Topic/Trace%20Flags%20\(Transact-SQL\).md)。<br /><br /> **\*\*重要提示\*\***：当使用 **-T** 选项指定跟踪标志时，请使用大写“T”来传递跟踪标志号。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 也可接受小写“t”，只是它用于设置仅 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 支持工程师才需要的其他内部跟踪标志。 （不读取“控制面板”启动窗口中指定的参数。）|  
|**-x**|禁用下列监视功能：<br /><br /> [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 性能监视器计数器<br /><br /> 保留 CPU 时间和高速缓存命中率统计信息<br /><br /> 收集 DBCC SQLPERF 命令的信息<br /><br /> 收集某些动态管理视图的信息<br /><br /> 许多扩展事件事件点<br /><br /> <br /><br /> **\*\*警告\*\***：当使用 **–x** 启动选项会导致你在诊断 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的性能和功能问题时获得的信息大大减少。|  
...
> [!NOTE]  
>  使用 **net start** 启动 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 时，启动选项使用的是左斜线 (/)，而不是连字符 (-)。  
...
## 兼容性支持  
 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 不支持 **-h** 参数。 启用 AWE 时，在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 早期版本的 32 位实例使用此参数以便为热添加内存元数据保留虚拟内存地址空间。 有关详细信息，请参阅 [SQL Server 2016 中不再使用的 SQL Server 功能](../Topic/Discontinued%20SQL%20Server%20Features%20in%20SQL%20Server%202016.md)。  
