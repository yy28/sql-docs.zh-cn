...
helpviewer_keywords: 
  - "启动服务状态 [SQL Server]"
  - "安装程序 [SQL Server], 用户帐户"
  - "Windows 权限 [SQL Server]"
  - "修改用户帐户"
  - "默认帐户"
  - "域 [SQL Server], 用户帐户"
  - "启动帐户 [SQL Server]"
  ...
  - "识别实例的服务 [SQL Server]"
  - "权限 [SQL Server], 服务"
  - "SQL Server 代理服务, 用户帐户"
  - "Windows NT 权限 [SQL Server]"
  - "用户帐户 [SQL Server]"
  - "标识不识别实例的服务 [SQL Server]"
  - "安装 SQL Server, 用户帐户"
  - "禁用启动状态 [SQL Server]"
  - "用户帐户 [SQL Server], 用户"
  - "本地服务帐户 [SQL Server]"
  ...
  - "不识别实例的服务 [SQL Server]"
  - "服务 [SQL Server], 安装时配置"
  - "Windows 帐户 [SQL Server]"
  - "SQL Server 服务, 用户帐户"
  - "用户帐户 [SQL Server], 服务"
  - "MSSQLServer"
  - "标识识别实例的服务 [SQL Server]"
  - "服务 [SQL Server], 帐户"
  - "访问控制列表"
  - "可选帐户 [SQL Server]"
  - "服务帐户 [SQL Server]"
...
##  <a name="a-nameservicedetailsa-services-installed-by-includessnoversiontokenssnoversionmdmd"></a><a name="Service_Details"></a>[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 安装的服务  
...
##  <a name="a-nameservpropa-service-properties-and-configuration"></a><a name="Serv_Prop"></a>服务属性和配置  
...
###  <a name="a-namedefaultacctsa-default-service-accounts"></a><a name="Default_Accts"></a>默认服务帐户  
...
####  <a name="a-namechangingaccountsa-changing-account-properties"></a><a name="Changing_Accounts"></a>更改帐户属性  
...
###  <a name="a-namenewaccountsa-managed-service-accounts-group-managed-service-accounts-and-virtual-accounts"></a><a name="New_Accounts"></a>托管服务帐户、组托管服务帐户和虚拟帐户  
...
-   <a name="VA_Desc"></a>**虚拟帐户**  
    ...
    |[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 代理服务，位于 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的默认实例上|**NT SERVICE\SQLSERVERAGENT**|  
    |[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 代理服务，位于名为 **PAYROLL** 的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例上|**NT SERVICE\SQLAGENT$PAYROLL**|  
...
###  <a name="a-nameautostarta-automatic-startup"></a><a name="Auto_Start"></a>自动启动  
...
###  <a name="a-nameconfigureservicesa-configuring-services-during-unattended-installation"></a><a name="Configure_services"></a>在无人参与的安装过程中配置服务  
...
###  <a name="a-namefirewalla-firewall-port"></a><a name="Firewall"></a>防火墙端口  
...
##  <a name="a-nameservperma-service-permissions"></a><a name="Serv_Perm"></a>服务权限  
...
###  <a name="a-nameservsida-service-configuration-and-access-control"></a><a name="Serv_SID"></a>服务配置和访问控制  
...
###  <a name="a-namewindowsa-windows-privileges-and-rights"></a><a name="Windows"></a>Windows 特权和权限  
...
|**[!INCLUDE[ssRS](../../includes/ssrs-md.md)]:**<br /><br /> （所有权限都将授予 Per-service SID。 默认实例：**NT SERVICE\ReportServer**。 命名实例：**NT SERVICE\\$***InstanceName*。）|**以服务身份登录** (SeServiceLogonRight)|  
...
###  <a name="a-namereviewingaclsa-file-system-permissions-granted-to-sql-server-per-service-sids-or-local-windows-groups"></a><a name="Reviewing_ACLs"></a>授予 SQL Server Per-service SID 或本地 Windows 组的文件系统权限  
...
###  <a name="a-namefilesystemothera-file-system-permissions-granted-to-other-windows-user-accounts-or-groups"></a><a name="File_System_Other"></a>授予其他 Windows 用户帐户或组的文件系统权限  
...
||\<报表服务器 Web 服务帐户>|*\<install>*\Reporting Services\ReportServer\global.asax|Full|  
...
###  <a name="a-nameunusuallocationsa-file-system-permissions-related-to-unusual-disk-locations"></a><a name="Unusual_Locations"></a>与非寻常磁盘位置相关的文件系统权限  
...
###  <a name="a-namereviewadditionalconsiderationsa-reviewing-additional-considerations"></a><a name="Review_additional_considerations"></a>查看其他注意事项  
...
###  <a name="a-nameregistrya-registry-permissions"></a><a name="Registry"></a>注册表权限  
...
###  <a name="a-namepipesa-named-pipes"></a><a name="Pipes"></a>命名管道  
...
##  <a name="a-nameprovisioninga-provisioning"></a><a name="Provisioning"></a>预配  
...
###  <a name="a-namedeprova-database-engine-provisioning"></a><a name="DE_Prov"></a>数据库引擎预配  
 以下帐户将作为登录名添加到 [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)]中。  
  
####  <a name="a-namewinprincipalsa-windows-principals"></a><a name="Win_Principals"></a>Windows 主体  
 在安装过程中， [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 安装程序要求至少将一个用户帐户命名为 **sysadmin** 固定服务器角色的成员。  
  
####  <a name="a-namesaa-sa-account"></a><a name="sa"></a>sa 帐户  
...
####  <a name="a-nameloginsa-sql-server-per-service-sid-login-and-privileges"></a><a name="Logins"></a>SQL Server Per-service SID 登录名和特权  
...
####  <a name="a-nameagenta-sql-server-agent-login-and-privileges"></a><a name="Agent"></a>SQL Server 代理登录名和特权  
...
####  <a name="a-namehadrona-includesshadrctokensshadrcmdmd-and-sql-failover-cluster-instance-and-privileges"></a><a name="Hadron"></a>[!INCLUDE[ssHADRc](../../includes/sshadrc-md.md)] 和 SQL 故障转移群集实例和特权  
...
####  <a name="a-namewritera-sql-writer-and-privileges"></a><a name="Writer"></a>SQL 编写器和特权  
...
####  <a name="a-namesqlwmia-sql-wmi-and-privileges"></a><a name="SQLWMI"></a>SQL WMI 和特权  
...
#### <a name="ssrs-provisioning"></a>SSRS 设置  
...
###  <a name="a-namessasa-ssas-provisioning"></a><a name="SSAS"></a>SSAS 预配  
...
###  <a name="a-namessrsa-ssrs-provisioning"></a><a name="SSRS"></a>SSRS 预配  
...
##  <a name="a-nameupgradea-upgrading-from-previous-versions"></a><a name="Upgrade"></a>从早期版本升级  
...
##  <a name="a-nameappendixa-appendix"></a><a name="Appendix"></a>附录  
...
###  <a name="a-nameservacctsa-description-of-service-accounts"></a><a name="Serv_Accts"></a>服务帐户的描述  
 服务帐户是用来启动 Windows 服务的帐户，如 [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)]。  
  
####  <a name="a-nameanyosa-accounts-available-with-any-operating-system"></a><a name="Any_OS"></a>可用于任何操作系统的帐户  
...
>  如果您将应用程序配置为使用域帐户，则可以隔离应用程序的特权，但必须手动管理密码或创建自定义解决方案以管理这些密码。 许多服务器应用程序使用此策略以增强安全性，但此策略要求额外的管理和复杂性。 在这些部署中，服务管理员将在维护任务方面花费大量时间，如管理服务密码和服务主体名称 (SPN)，但这些维护服务是 Kerberos 身份验证所必需的。 此外，这些维护任务可能会中断服务。  
 ...
 <a name="Local_User"></a>**Local User 帐户**  
...
###  <a name="a-nameidentifyinstanceawareandunawarea-identifying-instance-aware-and-instance-unaware-services"></a><a name="Identify_instance_aware_and_unaware"></a>辨别识别实例的服务和不识别实例的服务  
...
###  <a name="a-namelocalizedservicenamesa-localized-service-names"></a><a name="Localized_service_names"></a>本地化的服务名称  
