...
## 选项  
 **启用 C2 审核跟踪**  
 审核对语句和对象的所有访问尝试，并记录到文件中，对于默认 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 实例，该文件位于 \MSSQL\Data 目录中，对于 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 命名实例，该文件位于 \MSSQL$*instancename*\Data 目录中。 有关详细信息，请参阅 [c2 审核模式服务器配置选项](../../database-engine/configure-windows/c2-audit-mode-server-configuration-option.md)。  
