---
title: "排查连接到 SQL Server 数据库引擎时发生的问题 | Microsoft Docs"
ms.custom: ""
ms.date: "02/07/2017"
...
* 本主题不包含有关 SQL Azure 连接的信息。 有关帮助，请参阅[排查到 Azure SQL 数据库的连接问题](https://azure.microsoft.com/en-us/documentation/articles/sql-database-troubleshoot-common-connection-issues/)。 

## 收集 SQL Server 实例的相关信息
...
3.  如果网络配置正确，你将收到一条响应，例如，“来自 \<IP 地址> 的回复”，后跟一些附加信息。 如果收到一个错误，例如“无法访问目标主机。” 或“请求超时。”， 则表示 TCP/IP 配置不正确。 （确保 IP 地址正确并且键入正确。）此时的错误可能表示客户端计算机、服务器计算机或某些网络相关设备（如路由器）出现了问题。 Internet 有许多可供排查 TCP/IP 问题的资源。 2006 年发布的[如何排查基本 TCP/IP 问题](http://support.microsoft.com/kb/169790)一文是一个合理的起点。
