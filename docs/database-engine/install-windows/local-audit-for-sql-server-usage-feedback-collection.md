...
---
# SQL Server 使用反馈收集的本地审核
## 简介

Microsoft SQL Server 2016 (SQL Server) 包含了一些支持 Internet 的功能，可以从你的计算机或设备收集信息（“标准计算机信息”）并且发送到 Microsoft。 [SQL Server 使用反馈收集](http://support.microsoft.com/kb/3153756)的本地审核组件将服务收集的数据写入指定文件夹（表示将发送给 Microsoft 的数据（日志））。 本地审核的用途是使客户可以出于合规性、监管或隐私验证原因而查看 Microsoft 使用此功能收集的所有数据。  

在 SQL Server 2016 CU2 中，本地审核在实例级别针对 SQL Server 数据库引擎和 Analysis Services (SSAS) 进行配置。 本地审核尚未针对 SQL Server Integration Services (SSIS) 进行启用。 在安装过程中安装的其他 SQL Server 组件以及在安装之后下载或安装的 SQL Server Tools 没有使用反馈收集的本地审核功能。 
...
1. 创建注册表项设置以配置本地审核目标目录。 
    ...
    对于数据库引擎，在 HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSSQL13.\<INSTANCENAME\>\\CPE 处创建项。 
    
    对于 Analysis Services，在 HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSAS13.\<INSTANCENAME\>\\CPE 处创建项。

### 获取 SQL Server CEIP 服务登录帐户

执行以下步骤可获取 SQL Server CEIP 遥测服务登录帐户
 
1. 启动“服务”— 单击“Windows”按钮，然后输入 services.msc。 

2. 导航到相应的服务。 例如，对于数据库引擎，找到 **SQL Server CEIP 服务实例名称****。对于 Analysis Services，找到 **SQL Server Analysis Services CEIP 实例名称**。 
...
> 注意：在 SQL Server 安装路径外部配置本地审核目录路径以避免允许审核功能和修补导致与 SQL Server 有关的潜在问题。
  ...
  ||设计决策|建议|  
  |------|-----------------|----------|  
  |![Checkbox](../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Checkbox")|空间可用性 |对于大约具有 10 个数据库的中等工作负荷，计划每个实例每天使用大约 2 MB 磁盘空间。|  
|![Checkbox](../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Checkbox")|单独的目录 | 为每个实例创建一个目录。 例如，对名为 `MSSQLSERVER` 的 SQL Server 实例使用 C:\\SQLCEIPAudit\\MSSQLSERVER\\DB\\。 这简化了文件管理。
|![Checkbox](../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Checkbox")|单独的文件夹 |对每个服务使用特定文件夹。 例如对于给定实例名称，对数据库引擎使用一个文件夹。 如果 SSAS 的实例使用相同实例名称，则为 SSAS 创建单独的文件夹。 将数据库引擎和 Analysis Services 实例配置到相同文件夹会导致所有本地审核都从这两个实例写入相同日志文件。| 
|![Checkbox](../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Checkbox")|向 SQL Server CIEP 遥测服务登录帐户授予权限|向 SQL Server CEIP 遥测服务登录帐户启用“列出文件夹内容”、“读取”和“写入”访问权限|


### 向 SQL Server CIEP 遥测服务登录帐户授予权限
...
1. 导航到相应的 CPE 路径。 
    ...
    对于数据库引擎，使用 HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSSQL13.\<INSTANCENAME\>\\CPE。 
    
    对于数据库引擎，使用 HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSAS13.\<INSTANCENAME\>\\CPE。
