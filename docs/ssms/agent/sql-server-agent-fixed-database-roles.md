...
|----------|-------------|-----------------------------------|-------------------------------------------|-----------|  
|创建/修改/删除|否|用户帐户控制<br /><br />无法更改作业所有权。|用户帐户控制|否|  
|视图列表（枚举）|用户帐户控制<br /><br />可以获得可在 **sp_notify_operator** 和 Management Studio 的“作业属性”对话框中使用的可用运算符列表。|用户帐户控制|是|用户帐户控制<br /><br />只能在 Management Studio 的“作业步骤属性”对话框中使用代理列表。|  
...
|视图列表（枚举）|用户帐户控制<br /><br />可以获得可在 **sp_notify_operator** 和 Management Studio 的“作业属性”对话框中使用的可用运算符列表。|用户帐户控制|是|是|用户帐户控制<br /><br />只能在 Management Studio 的“作业步骤属性”对话框中使用代理列表。|  
...
|视图列表（枚举）|用户帐户控制|用户帐户控制<br /><br />可以获得可在 **sp_notify_operator** 和 Management Studio 的“作业属性”对话框中使用的可用运算符列表。|用户帐户控制|是|是|用户帐户控制|  
|启用/禁用|否|否|用户帐户控制<br /><br />**SQLAgentOperatorRole** 的成员可以通过使用存储过程 **sp_update_job** 并指定 **@enabled** 和 **@job_id** （或 **@job_name**）参数的值来启用或禁用它们尚未拥有的本地作业。 如果此角色的成员为此存储过程指定任何其他参数，则执行此过程将会失败。|否|用户帐户控制<br /><br />**SQLAgentOperatorRole** 的成员可以通过使用存储过程 **sp_update_schedule** 并指定 **@enabled** 和 **@schedule_id** （或 **@name**）参数的值来启用或禁用它们尚未拥有的本地作业。 如果此角色的成员为此存储过程指定任何其他参数，则执行此过程将会失败。|不适用|  
