...
[SSMS 提供了多种语言](download-sql-server-management-studio-ssms.md#available-languages-ssms-179)，但如果系统区域设置不匹配 SSMS 语言，SSMS 安装程序将阻止在计算机上安装。 
...
3. 操作系统以所需语言运行后，[SSMS 会提供多种语言](download-sql-server-management-studio-ssms.md#available-languages-ssms-179)。 第一次安装新的 SSMS 语言时，请使用完整包。 对于后续安装，可以使用升级包。
