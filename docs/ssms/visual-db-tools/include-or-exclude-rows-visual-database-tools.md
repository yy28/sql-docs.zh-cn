---
description: 包含或排除行 (Visual Database Tools)
title: 包含或排除行
...
当查询运行时， [!INCLUDE[ssDE](../../includes/ssde_md.md)] 将检查搜索条件并将其应用到要搜索的表的每一行。 如果行满足搜索条件，就会包括在查询中。 例如，一个查找特定区域内所有雇员的搜索条件可以类似如下形式：  
